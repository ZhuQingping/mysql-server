...
static bool store_param_bulk(NET *net, MYSQL_STMT *stmt, unsigned int param_nr,
                             unsigned long row_nr) {
  MYSQL_BIND *param = stmt->params + param_nr;
  unsigned long length;
  void *buffer;

  /* Lengths of these types are fixed and already set to
   * MYSQL_BIND::buffer_length during fix_param_bind() */
  if (param->buffer_type == MYSQL_TYPE_TINY ||
    ...
    length = param->buffer_length;
    buffer = (void *)((char *)param->buffer + length * row_nr);
  } else if (param->buffer_type == MYSQL_TYPE_TIME ||
             param->buffer_type == MYSQL_TYPE_DATE ||
             param->buffer_type == MYSQL_TYPE_DATETIME ||
             param->buffer_type == MYSQL_TYPE_TIMESTAMP) {
    /* Length is used for packet buffer allocating (it is how much we will
       write to the NET in store_param() below), use buffer_length for
       temporal types, which is set in fix_param_bind(). Data of temporal in
       MYSQL_BIND::buffer are stored as array of MYSQL_TIME (it is how the
       input data is laid out by the user), so use sizeof(MYSQL_TIME) * row_nr
       as offset to get the actual buffer. */
    length = param->buffer_length;
    buffer = (void *)((char *)param->buffer + sizeof(MYSQL_TIME) * row_nr);
  } else {
    length = stmt->params[param_nr].length[row_nr];
    buffer = ((void **)param->buffer)[row_nr];
  }

  /* If the parameter is NULL, we do not come here. */
  assert(!(param->is_null && *param->is_null));
  /* Backup buffer and length in param and serialize the actual param data to
   * packet buffer, then restore them after store. */
  void *old_buffer = param->buffer;
  unsigned long *old_length = param->length;
  param->buffer = buffer;
  param->length = &length;

  bool ret = store_param(net, param, 0);

  /* Restore buffer and length. */
  param->buffer = old_buffer;
  param->length = old_length;

  return ret;
}

bool mysql_int_serialize_param_data_bulk(MYSQL_STMT *stmt, uchar **ret_data,
                                         ulong *ret_length) {
  /*
  Pack data for COM_STMT_BULK_EXECUTE protocol:
     ...
     4             2      Flags (cursor type):
      ...
      (Note that STMT_BULK_FLAG_SEND_UNIT_RESULTS is not supportd in
       mysql server)
     -----------------------------------------
  ...
  How to bind array data with mysql C client, here's a simple example:

    MYSQL *conn = mysql_init(nullptr);

    // Connect to server and prepare statement ...

    // Set array size to 3.
    int array_size = 3;
    mysql_stmt_attr_set(stmt, STMT_ATTR_ARRAY_SIZE, &array_size)

    char *values[3] = {"Alice", "Bob", "Charlie"};
    unsigned long value_lengths[3];
    for (int i = 0; i < 3; ++ i) {
      value_lengths[i] = strlen(values[i]);
    }
    bool null_array[3] = {false, false, true};

    MYSQL_BIND bind;
    memset(&bind, 0, sizeof(bind));
    bind.buffer_type = MYSQL_TYPE_STRING;
    bind.buffer = values;
    bind.length = value_lengths;
    bind.is_null = null_array;
  ...
  */
  ...
  unsigned int param_count = stmt->param_count;
  MYSQL_BIND *param_end = params + param_count;

  DBUG_TRACE;

  assert(net->vio);
  assert(stmt->array_size > 0);

  for (uint i = 0; i < stmt->param_count; ++i) {
    MYSQL_BIND *param = &stmt->params[i];
    /* Buffer can be nullptr only if buffer_type is NULL. */
    if (!param->buffer && param->buffer_type != MYSQL_TYPE_NULL) {
      return true;
    }
  }
  ...
  /* Store types of parameters in first. */
  for (MYSQL_BIND *param = params; param < param_end; param++) {
    store_param_type(&net->write_pos, param);
  }

  /* Store actual data. */
  for (size_t j = 0; j < stmt->array_size; ++j) {
    for (size_t i = 0; i < stmt->param_count; ++i) {
      ...
      if (stmt->params[i].is_null && stmt->params[i].is_null[j]) {
        /*
          ...
          array in MYSQL_BIND. Store null array in MYSQL_BIND::is_null instead.
        */
        ...
        switch (stmt->params[i].buffer_type) {
          ...
          default:
            ...
            size += 9; /* net_store_length() may use max 9 bytes */
            size += (size_t)stmt->params[i].length[j];
            break;
        }
      }

      if (my_realloc_str(net, size)) {
        return true;
      }

      DBUG_EXECUTE_IF("bulk_test_invalid_indicator_flag", {
        indicator = 2; /* Invalid indicator. */
      });

      /* Set indicator. */
      *(net->write_pos)++ = indicator;

      if (indicator == STMT_INDICATOR_NONE) {
        /* Actually store data from bind buffer. */
        if (store_param_bulk(net, stmt, i, j)) {
          return true;
        }
      }
    }
  ...
  DBUG_EXECUTE_IF("bulk_test_invalid_packet_data", {
    // Append an extra byte.
    my_realloc_str(net, 1);
    *(net->write_pos)++ = 255;
  });

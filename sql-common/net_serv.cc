...
#ifdef MYSQL_SERVER
  double factor = get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
                          rds_dstore_support_binlog_check
                      ? rds_packet_size_factor
                      : 1;
#else
  double factor = 1.0;
#endif

  if (length >= net->max_packet_size * factor) {
    DBUG_PRINT("error", ("Packet too large. Max size: %lu, factor:%f",
                         net->max_packet_size, factor));
...
void my_net_shrink_buffer(NET *net, ulong min_buf_size,
  ...
  *max_interval_packet = min_buf_size;
   ...
   * of reallocs */
  if (mip * PCT_LIMIT_INTERVAL_PACKET >= net->max_packet) {
    return;
  }

  /* In the last interval packets were significantly smaller than max_packet,
   * so do shrink the buffer, net->max_packet is updated in net_realloc */
  net_realloc(net, mip);
}

...
static char *mysql_fill_packet_header(MYSQL *mysql, char *buff,
  ...
  */
  ...
  if (client_flag & CLIENT_PROTOCOL_41) {
    ...
    /* Filter out unsupported extended capabilities. */
    uint64_t client_extended_capabilities =
        mysql->extended_server_capabilities & CLIENT_STMT_BULK_OPERATIONS;
    DBUG_EXECUTE_IF("bulk_test_client_dont_support_bulk", {
      client_extended_capabilities = 0; /* Invalid indicator. */
    });
    int4store(buff_p + 28, client_extended_capabilities >> 32);
    end = buff + 32;
  } else {
    assert(buff_size >= 5);
...
static mysql_state_machine_status csm_parse_handshake(
  ...
  */
  ...
  if (pkt_end >= end + 18) {
    ...
    /* Server will only set extended_server_capabilities when
    rds_support_bulk_execution is enabled in server, otherwise, it's always 0.
  */
    mysql->extended_server_capabilities = ((uint64_t)uint4korr(end + 14)) << 32;
  }
  end += 18;
...
int STDCALL mysql_real_query(MYSQL *mysql, const char *query, ulong length) {
  ...
  DBUG_EXECUTE_IF("inject_ER_NOT_NET_ERROR", {
    DBUG_SET("-d,inject_ER_NOT_NET_ERROR");
    mysql->net.last_errno = ER_REPLICA_FATAL_ERROR;
    return 1;
  });
...
/**
  Open a new wal replication stream.

  Compose and send COM_WAL_DUMP command
  using information in the MYSQL_WAL_RPL structure.

  Caller must set the following MYSQL_WAL_RPL's slots:
  flushed_lsn, server_id, flags which is used to
  ...
  @retval  0   Success.
*/
int STDCALL mysql_wallog_open(MYSQL *mysql, MYSQL_WAL_RPL *rpl) {
  DBUG_TRACE;
  assert(mysql);
  assert(rpl);

  DBUG_EXECUTE_IF("rpl_wal_log_open_failed", {
    DBUG_SET("-d, rpl_wal_log_open_failed");
    return 1;
  });
  ...
  /*
    ...
    it'll be checked in the simple_command().
  */
  command = COM_WAL_DUMP;
  size_t alloc_size = sizeof(Rpl_request_dump_msg);
  ...
  Rpl_request_dump_msg *dump_msg =
      pointer_cast<Rpl_request_dump_msg *>(command_buffer);
  dump_msg->common_header.type = RplWalMsgType::RPL_WAL_REQUEST_DUMP_MSG;
  dump_msg->common_header.version = rpl_wal_default_version;
  int2store(pointer_cast<uchar *>(&dump_msg->common_header.flags), rpl->flags);
  int4store(pointer_cast<uchar *>(&dump_msg->common_header.server_id),
            rpl->server_id);
  int8store(pointer_cast<uchar *>(&dump_msg->wal_stream_id), rpl->stream_id);
  int8store(pointer_cast<uchar *>(&dump_msg->flushed_lsn), rpl->flushed_lsn);

  if (simple_command(mysql, command, command_buffer,
                     sizeof(Rpl_request_dump_msg), 1)) {
    my_free(command_buffer);
    return -1;
  }
...
/**
  ...
  @retval  0     Success.
*/
int STDCALL mysql_wallog_fetch(MYSQL *mysql, MYSQL_WAL_RPL *rpl) {
  DBUG_TRACE;
  assert(mysql);
  assert(rpl);
...
bool STDCALL server_looks_like_mariadb(MYSQL *mysql) {
  /* Server looks like mariadb only if:
  a) server version contains 'MariaDB' or '-mariadb-'.
  b) CLIENT_MYSQL (flag defined in MariaDB which is same as
  CLIENT_LONG_PASSWORD), is not set in server capabilities. */
  return !(mysql->server_capabilities & CLIENT_LONG_PASSWORD) &&

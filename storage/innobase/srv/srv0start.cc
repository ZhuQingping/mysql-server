...
dberr_t srv_start(bool create_new_db) {
  ...
  if (create_new_db) {
    ...
    /* The purge system needs to create the purge view and therefore requires
    the trx_sys is inited. The first argument is: the number of worker purge
    threads for fast purge as coordinator purge thread does not do actual purge
    in this purge mode, it assigns tasks to worker purge threads only; the
    number of worker purge threads plus one coordinator purge thread for normal
    purge. */

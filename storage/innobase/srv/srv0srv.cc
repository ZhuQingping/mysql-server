...
/** The number of purge threads to use. This variable is:
 1) For fast purge: the number of worker purge threads and it does not include
 coordinator purge thread. So, many purge threads (but at least one) could be
 created as workers and one additional thread is created as coordinator. Thus
 the total number of purge threads is actually srv_n_purge_threads + 1.
 2) For normal purge: the number of worker purge threads plus coordinator purge
 thread. SO, many purge threads (may be zero) could be created as workers and
 one is created as coordinator. Thus the total number of purge threads is
 actually srv_n_purge_threads. */
ulong srv_n_purge_threads = 4;

/* the number of pages to purge in one batch */
ulong srv_purge_batch_size = 20;

/** The proxy variable for sysvar innodb_rds_purge_subbatch_size. It sets the
subbatch size for fast purge, so, batch size will be divided on
(batch_size / subbatch_size) parts. */
uint rds_srv_purge_subbatch_size;

/** The proxy variable for sysvar innodb_rds_fast_purge_enabled. If it equals
false then fast purge is disabled, so, scanning of undo logs and running of
purge tasks follow strictly sequentially. */
bool rds_srv_fast_purge_enabled;
...
static ulint srv_n_rds_rows_skipped_by_offset_pushdown_old = 0;
    ...
    srv_slot_t *slot) /*!< in/out: thread slot */
{
  ...
  switch (type) {
    ...
    case SRV_WORKER:
      ut_a(srv_n_purge_threads > FAST_PURGE_ENABLED_INVERSION);
      ...
      switch (type) {
        ...
        case SRV_WORKER:
          ...
          ut_a(srv_sys->n_threads_active[type] <
               srv_n_purge_threads - FAST_PURGE_ENABLED_INVERSION);
          break;
      }
...
static void srv_init(void) {
  ...
  /* The number of worker purge threads plus coordinator purge thread. */
  srv_threads.m_purge_workers_n =
      srv_n_purge_threads + rds_srv_fast_purge_enabled;
  ...
  if (!srv_read_only_mode) {
    /* Number of worker purge threads + coordinator thread + master thread */
    n_sys_threads = srv_n_purge_threads + rds_srv_fast_purge_enabled + 1;
...
static void srv_refresh_innodb_monitor_stats(void) {
  ...
  srv_n_rds_rows_skipped_by_offset_pushdown_old =
      srv_stats.n_rds_rows_skipped_by_offset_pushdown;
...
void srv_export_innodb_status(void) {
  ...
  export_vars.innodb_rds_rows_skipped_by_offset_pushdown =
      srv_stats.n_rds_rows_skipped_by_offset_pushdown;
...
static void srv_master_sleep(void) {
  ...
  DBUG_EXECUTE_IF("ib_pause_master_thread",
...
/* increase priority of purge releated thread when fast purge enabled
 @param[in]   priority: expect priority which we want to set for
 the thread
 @param[in]   thread_name: the current thread name
 */
void incre_priority_fast_purge(int priority, const char *thread_name) {
#ifdef UNIV_LINUX
  if (!rds_srv_fast_purge_enabled) {
    return;
  }
  auto old_pri = getpriority(PRIO_PROCESS, (pid_t)syscall(SYS_gettid));
  setpriority(PRIO_PROCESS, (pid_t)syscall(SYS_gettid), -20);
  auto new_pri = getpriority(PRIO_PROCESS, (pid_t)syscall(SYS_gettid));
  ib::info() << "Set " << thread_name << " priority from " << old_pri << " to "
             << new_pri << " errno: " << errno;
...
void srv_worker_thread() {
  ...
  incre_priority_fast_purge(-20, "purge_worker");
  ...
  ut_a(srv_sys->n_threads_active[SRV_WORKER] <=
       srv_n_purge_threads - 1 + rds_srv_fast_purge_enabled);
...
static ulint srv_do_purge(ulint *n_total_purged) {
  ...
  const auto n_threads =
      srv_threads.m_purge_workers_n - rds_srv_fast_purge_enabled;
...
void srv_purge_coordinator_thread() {
  ...
  ib::info() << "fast purge enabled : " << rds_srv_fast_purge_enabled;
  incre_priority_fast_purge(-20, "purge_coordinator");
  ...
  /* Ensure that all the worker threads quit. Only worker threads do actual
     purge and its number equals srv_n_purge_threads. */
  srv_release_threads(SRV_WORKER,
                      srv_n_purge_threads - 1 + rds_srv_fast_purge_enabled);
...
void srv_purge_wakeup(void) {
  ...
  if (srv_force_recovery < SRV_FORCE_NO_BACKGROUND) {
    ...
    if (srv_threads.m_purge_workers_n > FAST_PURGE_ENABLED_INVERSION) {

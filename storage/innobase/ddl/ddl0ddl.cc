...
dberr_t lock_table(trx_t *trx, dict_table_t *table,
  ...
  /* This is in ALTER TABLE, also check the rds_ddl_lock_wait_timeout.
  It is the similar mechanism with the DDL_timeout_guard in
  mysql_execute_command. */
  THD *thd = trx->mysql_thd;
  ulong backup_timeout = 0;
  ulong cur_innodb_lock_wait_timeout_val =
      static_cast<ulong>(thd_lock_wait_timeout(thd).count());
  if (cur_innodb_lock_wait_timeout_val >
      thd->variables.rds_ddl_lock_wait_timeout) {
    backup_timeout = cur_innodb_lock_wait_timeout_val;
    thd_set_lock_wait_timeout(thd, thd->variables.rds_ddl_lock_wait_timeout);
  }

  dberr_t err = lock_table_for_trx(table, trx, mode);

  if (backup_timeout != 0) {
    thd_set_lock_wait_timeout(thd, backup_timeout);
  }

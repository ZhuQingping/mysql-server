...
/** Maximum number of concurrent buffer pool watches: worker purger threads +
    coordinator purge thread + master thread. This takes into account that
    srv_n_purge_threads is the number of purge workers and does not cover
    coordinator thread for fast purge; the number of all purge related threads
    including coordinator thread for normal purge. */
#define BUF_POOL_WATCH_SIZE \
  (srv_n_purge_threads + rds_srv_fast_purge_enabled + 1)

...
struct srv_stats_t {
  ...
  /** Number of rows skipped by offset pushdown. */
  ulint_ctr_64_t n_rds_rows_skipped_by_offset_pushdown;
...
struct Srv_threads {
  ...
  /** Number of threads related to purge and size of array below. It will be
   assigned srv_n_purge_threads + 1 for fast purge which includes the number of
   purge worker threads and one purge coordinator thread. It will be assigned
   srv_n_purge_threads for normal purge. */
  size_t m_purge_workers_n;

  /** Purge threads. Note that the m_purge_workers[0] is the same shared
...
extern uint rds_srv_purge_subbatch_size;
extern bool rds_srv_fast_purge_enabled;
...
/* Inversion of bool sysvar innodb_rds_fast_purge_enabled is used several times
in data integrity checks within the code related to worker purge threads. Which
condition innodb_purge_threads should satisfy in order to provide existence of
at least one worker purge thread? If fast purge is enabled, it means just the
number of working purge threads, so it should be greater than zero. If fast
purge is disabled, it should be greater than one, because it includes one
coordinator purge thread. */
#define FAST_PURGE_ENABLED_INVERSION (1UL - rds_srv_fast_purge_enabled)
...
struct export_var_t {
  ...
  ulint
      innodb_rds_rows_skipped_by_offset_pushdown; /*!<srv_n_rds_rows_skipped_by_offset_pushdown*/

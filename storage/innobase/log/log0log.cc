...
static void log_sys_create() {
  ...
  log.self_write_paused.store(false);
...
void log_pause_writer_threads(log_t &log) {
...
void log_resume_writer_threads(log_t &log) {
  /* protected by LOCK_global_system_variables */
  if (log.writer_threads_paused.load()) {
    log.writer_threads_paused.store(false);
...
void log_pause_write_for_local_backup() {
  log_sys->self_write_paused.store(true);
  if (srv_log_writer_threads) {
    log_pause_writer_threads(*log_sys);
  }
}

void log_resume_write_for_local_backup() {
  if (srv_log_writer_threads) {
    log_resume_writer_threads(*log_sys);
  }
  log_sys->self_write_paused.store(false);
}

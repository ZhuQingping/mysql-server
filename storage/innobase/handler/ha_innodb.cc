...
static SHOW_VAR innodb_status_variables[] = {
    ...
    {"rows_read", (char *)&export_vars.innodb_rows_read, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"rds_rows_skipped_by_offset_pushdown",
     (char *)&export_vars.innodb_rds_rows_skipped_by_offset_pushdown, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
...
static int innobase_commit(handlerton *hton, /*!< in/out: InnoDB handlerton */
                           THD *thd,         /*!< in: MySQL thread handle of the
                                             ...
                                             be committed */
                           bool commit_trx,  /*!< in: true -
                                             commit transaction false - the
                                             current SQL statement ended */
                           bool);            /*!< in: just for DStore, not
                                                           used in InnoDB. */
...
/** Set the lock wait timeout for the current connection.
Currently this would be only called for ALTER TABLE statements,
to replace it with a user specified ddl lock wait timeout
@param[in,out]	thd	thread handle
@param[in]	timeout	the timeout to set */
void thd_set_lock_wait_timeout(THD *thd, ulong timeout) {
  THDVAR_SET(thd, lock_wait_timeout, &timeout);
}
...
static bool innobase_start_full_local_backup(local_backup_config *config,
  ...
  std::unique_ptr<Backup_Callback> callback =
      current_lb_use_obs ? std::make_unique<Local_Backup_Obs_Callback>(config)
                         : std::make_unique<Local_Backup_Callback>(config);
  return local_backup_sys->do_backup(callback.get());
}
...
static int innobase_commit(handlerton *hton, /*!< in: InnoDB handlerton */
                           THD *thd,         /*!< in: MySQL thread handle of the
                                             user for whom the transaction should
                                             be committed */
                           bool commit_trx,  /*!< in: true - commit
...
int ha_innobase::open(const char *name, int, uint open_flags,
  ...
  if (ib_table->is_intrinsic()) {
    ...
    m_prebuilt->session = thd_to_innodb_session(ha_thd());
  }
...
int ha_innobase::index_end(void) {
  ...
  if (m_prebuilt->old_index && m_prebuilt->old_index->last_sel_cur) {
    ut_ad(is_sharing_data());
    m_prebuilt->old_index->last_sel_cur->release();
  } else {
...
dict_index_t *ha_innobase::innobase_get_index(
    uint keynr, /*!< in: use this index; MAX_KEY means always
                ...
                generated by InnoDB */
    bool share) /*!< in: share the same temp innodb table */
{
  KEY *key;
  dict_index_t *index;
  ...
  if (share && !is_sharing_data()) return nullptr;

  INNOBASE_SHARE *_share = share ? m_old_share : m_share;
  dict_table_t *_table = share ? m_old_table : m_prebuilt->table;

  if (keynr != MAX_KEY && table->s->keys > 0) {
    key = table->key_info + keynr;

    index = innobase_index_lookup(_share, keynr);
    ...
    if (index != nullptr) {
      ...
      table exists */
      if (_share->idx_trans_tbl.index_mapping != nullptr) {
        log_errlog(WARNING_LEVEL, ER_INNODB_FAILED_TO_FIND_IDX_WITH_KEY_NO,
                   key ? key->name : "NULL", keynr, _table->name.m_name);
      }

      index = dict_table_get_index_on_name(_table, key->name);
    }
  } else {
    key = nullptr;
    index = _table->first_index();
  }

  if (index == nullptr) {
    log_errlog(ERROR_LEVEL, ER_INNODB_FAILED_TO_FIND_IDX_FROM_DICT_CACHE, keynr,
               key ? key->name : "NULL", _table->name.m_name);
  }
                ...
                InnoDB */
{
  ...
  active_index = keynr;
  m_prebuilt->index = innobase_get_index(keynr, false);
  m_prebuilt->old_index = innobase_get_index(keynr, true);
  ...
  ut_ad(m_prebuilt->search_tuple != nullptr);
  ut_ad(m_prebuilt->m_stop_tuple != nullptr);
...
int ha_innobase::create(const char *name, TABLE *form,
  ...
  int error = innobase_basic_ddl::create_impl(ha_thd(), name, form, create_info,
                                              table_def, srv_file_per_table,
                                              true, false, 0, 0, nullptr);
  if (thd->recycle_state->is_recycle() && error == 0) {
    if (thd->recycle_state->has_autoinc_col()) {
      dd_set_autoinc(table_def->se_private_data(), 0);
    }
    if (dd_table_has_instant_cols(*table_def)) {
      dberr_t ret = dd_clear_instant_table(*table_def, true, true);
      if (ret != DB_SUCCESS) {
        error = HA_ERR_GENERIC;
      }
    }
 ...
 *   @return error number */
int handler::ha_check_support_recyclebin(
    /*======================*/
  ...
  THD *thd = ha_thd();
  bool has_fts = false;
  bool is_fil_per_table = true;
  bool is_tablespace_discard = false;
  const dd::Table *tab = &(dd_table->table());
  const dd::Partition *part = reinterpret_cast<const dd::Partition *>(dd_table);

  DBUG_ENTER("handler::ha_check_support_recyclebin");

  dd::cache::Dictionary_client *client = dd::get_dd_client(thd);
  dd::cache::Dictionary_client::Auto_releaser releaser(client);

  if (dd_table_is_partitioned(*dd_table)) {
    dd::Partition *dd_part = dd_table->leaf_partitions().front();
    tab = &(dd_part->table());
    part = reinterpret_cast<const dd::Partition *>(dd_part);
    ...
    if (part_table.length() >= FN_REFLEN ||
        DBUG_EVALUATE_IF("recyclebin_partition_1", 1, 0)) {
      return HA_ERR_INTERNAL_ERROR;
    }

    normalize_table_name(norm_name, part_table.c_str());
  } else {
  ...
  err = dd_table_open_on_dd_obj(
      thd, client, *tab,
      (!dd_table_is_partitioned(*tab)
           ? nullptr
           : reinterpret_cast<const dd::Partition *>(part)),
      norm_name, table, nullptr);

  if (err == 0 && !table) {
    err = HA_ERR_NO_SUCH_TABLE;
    support_recyclebin = false;
    if (table) {
      dd_table_close(table, thd, nullptr, false);
    }
    DBUG_RETURN(err);
  ...
  has_fts = dd_table_contains_fulltext(*dd_table);
  is_fil_per_table = dict_table_is_file_per_table(table);
  is_tablespace_discard = dict_table_is_discarded(table);

  dd_table_close(table, thd, nullptr, false);

  support_recyclebin = !has_fts && is_fil_per_table && !is_tablespace_discard;
                             ...
                             TL_IGNORE */
{
  ...
  if (srv_read_only_mode && !m_prebuilt->table->is_intrinsic() &&
    ...
    if (sql_command == SQLCOM_CHECKSUM ||
      ...
      /* INSERT/REPLACE SELECT statements place row locks on the rows of
      selected-from table and parallel query (PQ) does not have a row locking
      logic. Such locks are however only necessary for replication when the
      binlog is statement-based. Therefore,  for INSERT SELECT without FOR
      UPDATE or LOCK IN SHARE MODE  and REPLACE SELECT statements: when
      parallel insert_select is true and the binlog is row based we do not
      place locks on the selected-from table so as to enable parallel INSERT
      SELECT and parallel REPLACE SELECT. We identify INSERT INTO SELECT without
      FOR UPDATE or LOCK IN SHARE MODE as follows: INSERT INTO SELECT / LOCK IN
      SHARE MODE / FOR UPDATE locks types: TL_READ/ TL_READ_WITH_SHARED_LOCKS /
      TL_WRITE resp. for the SELECT part and  TL_WRITE_CONCURRENT_INSERT for the
      INSERT part in all three cases.
      */

      if (thd->pq_support_features_switch_flag(
              PQ_SUPPORT_FEATURES_SWITCH_INSERT_SELECT) &&
          thd->variables.force_parallel_execute &&
          ((sql_command == SQLCOM_INSERT_SELECT) ||
           sql_command == SQLCOM_REPLACE_SELECT) &&
          thd->is_current_stmt_binlog_format_row() &&
          /* No LOCK IN SHARE MODE or FOR UPDATE */ lock_type == TL_READ) {
        m_prebuilt->select_lock_type = LOCK_NONE;
        m_stored_select_lock_type = LOCK_NONE;
      } else {
        m_prebuilt->select_lock_type = LOCK_S;
...
static MYSQL_SYSVAR_UINT(rds_purge_subbatch_size, rds_srv_purge_subbatch_size,
                         PLUGIN_VAR_OPCMDARG,
                         "Minimal subbatch size for fast purge.", nullptr,
                         nullptr, 150, 1, 300, 0);

static MYSQL_SYSVAR_BOOL(rds_fast_purge_enabled, rds_srv_fast_purge_enabled,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                         "Allow fast purge mode.", nullptr, nullptr, false);
...
static MYSQL_SYSVAR_UINT(
    lb_global_io_retry_number, innodb_lb_global_io_retry_number,
    PLUGIN_VAR_RQCMDARG,
    "Retry number for local backup innodb io operation in whole task.", nullptr,
    nullptr, 3, 0, UINT_MAX, 0);

static MYSQL_SYSVAR_UINT(
    lb_local_io_retry_times, innodb_lb_local_io_retry_times,
    PLUGIN_VAR_RQCMDARG,
    "Retry times for local backup one innodb io operation.", nullptr, nullptr,
    5, 0, UINT_MAX, 0);

static MYSQL_SYSVAR_INT(lb_local_io_hang_timeout_ms,
                        innodb_lb_local_io_hang_timeout_ms, PLUGIN_VAR_RQCMDARG,
                        "timeout ms of local backup innodb io hange.", nullptr,
                        nullptr, 3600000, 0, INT_MAX, 0);

static MYSQL_SYSVAR_INT(lb_local_io_thread_quit_timeout_ms,
                        innodb_lb_local_io_thread_quit_timeout_ms,
                        PLUGIN_VAR_RQCMDARG,
...
static SYS_VAR *innobase_system_variables[] = {
    ...
    MYSQL_SYSVAR(rds_purge_subbatch_size),
    MYSQL_SYSVAR(rds_fast_purge_enabled),
...
bool pq_create_trx(THD *thd) {
  /* If thd has not created trx, then create it */
  trx_t *trx = check_trx_exists(thd);
  if (!trx) return true;

  /* Imagine that trx has owned its read_view, which must be created
  when spliting divided table (or cut table), then we have no need to
  create new read_view again. */
  if (srv_read_only_mode || MVCC::is_view_active(trx->read_view)) {
    return false;
  }
  ...
  /* If PQ opens, The transaction was actived  by call function trx_start_low()
  when execting first sql statement, when exacting the second sql statement,
  trx_is_started assert happends, so delete ut_ad(!trx_is_started(trx)) */
...
void ha_innobase::get_create_info(const char *table, const dd::Table *table_def,
  ...
  bool has_autoinc = dict_table_has_autoinc_col(dict_table);
  m_thd->recycle_state->set_autoinc_flag(has_autoinc);

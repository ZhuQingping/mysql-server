...
class ha_innobase : public handler {
  ...
  */
  ...
  int pq_ref_build_ranges(void *, Key_ref &) override;
  int pq_worker_scan_next(void *scan_ctx, uchar *buf) override;
  void init_shared_info() override;
  void set_shared_info(PQ_shared_info *info) override;
  bool is_sharing_data() override;
  void reset_shared_info() override;
  int pq_leader_scan_end(void *parallel_scan_ctx) override;
  int pq_worker_scan_end() override;

  int pq_range_scan_init(uint keyno, void *&scan_ctx, uint n_threads);
  int pq_ref_scan_init(uint keyno, void *&scan_ctx, uint n_threads);
  int pq_index_scan_init(uint keyno, void *&scan_ctx, uint n_threads);
 ...
 protected:
  ...
  virtual dict_index_t *innobase_get_index(uint keynr, bool share = false);
  ...
  */
  ...
  /**
    In Parallel Query, when multiple threads have to read the same intrinsic
    table, the leader thread opens an instance of ha_innobase, and some
    members of this are saved into an instance of PQ_shared_info. Then the
    worker thread opens an instance of ha_innobase too, but, for the purpose
    of sharing of data (single materialization!), its m_share is re-pointed to
    that of the leader (fetched from PQ_shared_info). Before the re-pointing,
    the original m_share is saved in m_old_share by set_shared_info(), as it
    will still be needed (see below).

    Then change_active_index() sets prebuilt->index to the chosen index from
    the leader's table, and sets prebuilt->old_index to the chosen index from
    m_old_share.

    Then row_search_no_mvcc() stores prebuilt->old_index into prebuilt's
    innodb_session_t. Finally populate_offsets() gets old_index from this
    innodb_session_t, and will use it to access a worker-local rec_cache_t.

    This logic is used only for an intrinsic table, which was not
    originally designed for sharing between multiple threads.
  */
  INNOBASE_SHARE *m_old_share{nullptr};

  /** @see m_old_share. Saves the original value of m_prebuilt->table. */
  dict_table_t *m_old_table{nullptr};

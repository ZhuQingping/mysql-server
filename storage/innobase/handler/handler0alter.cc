...
bool alter_part_factory::create_for_reorg(alter_part_array &to_drop,
  ...
  ut_ad((m_ha_alter_info->handler_flags &
         Alter_inplace_info::REORGANIZE_PARTITION) != 0);
  ut_ad(m_ha_alter_info->modified_part_info->get_num_subparts() ==
        m_old_part_info->get_num_subparts());
  ...
  uint parts_per_part =
      part_info->is_sub_partitioned() ? part_info->get_num_subparts() : 1;
  ...
  */
  ...
  if ((ha_alter_info->handler_flags & (Alter_inplace_info::ADD_PK_INDEX |
    ...
    if (m_part_info->is_sub_partitioned() &&
        (m_part_info->sub_part_info->part_type == partition_type::HASH) &&
        m_part_info->sub_part_info->list_of_part_fields &&
        m_part_info->sub_part_info->part_field_list.is_empty()) {
      return HA_ALTER_INPLACE_NOT_SUPPORTED;
    }
  }
...
bool ha_innopart::prepare_for_copy_partitions(
  ...
  if (ha_alter_info->modified_part_info->is_sub_partitioned()) {
    total_parts *= ha_alter_info->modified_part_info->get_num_subparts();
  }

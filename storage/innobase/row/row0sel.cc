                              ...
                              access the clustered index */
{
  ...
  return row_sel_get_clust_rec_for_mysql(prebuilt, sec_index, rec, thr, out_rec,
...
Record_buffer *row_sel_get_record_buffer(const row_prebuilt_t *prebuilt) {
  if (prebuilt->m_mysql_handler == nullptr) {
    return nullptr;
  }

  /* When searching the partition range boundary in parallel query
  for range scan or ref scan, we can't use record buffer as we
  may get an out-of-boundary partition range. */
...
inline void row_sel_dequeue_cached_row_for_mysql(
    ...
    row_prebuilt_t *prebuilt) /*!< in/out: prebuilt struct */
{
  ...
  UNIV_MEM_INVALID(buf, rec_size);

  auto tbl = prebuilt->m_mysql_table;
  if (tbl) {
    auto sz = std::min<size_t>(tbl->s->null_bytes, rec_size);
    if (sz) {
      /*
        The first bytes of 'buf' represent the "NULL bytes", one bit for each
        nullable column. But for a non-nullable column, its bit will never be
        set. However, hash join code hashes NULL bytes, including such bit
        (see StoreFromTableBuffers()). This is also true for bits of
        non-existent columns (like if there are 7 columns, the 8th bit is
        never set). For repeatability of hashing, and silencing of Valgrind,
        we set those "phantom bits" to 0 once for all.
      */
      errno_t err [[maybe_unused]] = memset_s(buf, sz, '\0', sz);
      assert(err == 0);
    }
  }
...
dberr_t row_search_no_mvcc(byte *buf, page_cur_mode_t mode,
                           ...
                           ulint direction) {
  dict_index_t *index = prebuilt->index;
  dict_index_t *old_index = prebuilt->old_index;
  dict_index_t *used_priv_index = old_index ? old_index : index;
  ...
  if (old_index != nullptr) {
    if (UNIV_UNLIKELY(old_index->last_sel_cur == nullptr)) {
      dict_allocate_mem_intrinsic_cache(old_index);
    }
  } else if (!index->last_sel_cur) {
    dict_allocate_mem_intrinsic_cache(index);
  }

  RegisterActiveIndexes r_a_i(prebuilt->session, old_index);

  mtr = &used_priv_index->last_sel_cur->mtr;
  ...
  if (direction != 0) {
    ...
    } else if (used_priv_index->last_sel_cur->invalid ||
    ...
    block_relocated:
      ...
      used_priv_index->last_sel_cur->release();
      ...
      information. */
      ut_ad(used_priv_index->last_sel_cur->rec != nullptr);
      pcur->m_btr_cur.page_cur.rec = used_priv_index->last_sel_cur->rec;
      pcur->m_btr_cur.page_cur.block = used_priv_index->last_sel_cur->block;
      err = row_search_traverse(moves_up, match_mode, pcur, mtr);
      if (err != DB_SUCCESS) {
        return (err);
      }
  ...
  for (/* No op */; err == DB_SUCCESS;
    ...
    /* If this comes from PQ worker read_record, the match_mode must be 0,
    otherwise, if it is not 0, then it must not be PQ worker, and the
    pq_ctx must be nullptr. */
    ut_ad(match_mode == 0 || prebuilt->pq_ctx == nullptr);
    ...
    } else if (prebuilt->pq_ctx != nullptr) {
      auto pq_ctx = down_cast<PQ_Ctx_intrinsic *>(prebuilt->pq_ctx.get());
      if (pq_ctx->out_of_range(rec, offsets)) {
        err = DB_END_OF_RANGE;
        break;
      }
    }
    ...
    used_priv_index->last_sel_cur->rec = pcur->get_rec();
    used_priv_index->last_sel_cur->block = pcur->get_block();
    ...
    if (prebuilt->pq_index_read) {
      prebuilt->pq_tuple = row_rec_to_index_entry_low(
          rec, index,
          rec_get_offsets(rec, index, nullptr, ULINT_UNDEFINED,
                          UT_LOCATION_HERE, &heap),
          prebuilt->pq_heap);
      /* Do a deep copy, like what we do for non-intrinsic table. */

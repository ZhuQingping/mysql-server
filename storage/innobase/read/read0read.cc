...
void ReadView::copy_readview(const ReadView &other) {
  copy_prepare(other);
  copy_complete();
...
void MVCC::view_close(ReadView *&view, bool own_mutex) {
  ...
  if (!own_mutex) {
    ...
    if (!view->skip_view_list) {
      ...
      // This view is the read view of PQ worker and is cloned from PQ
      // leader; it should be explicitly deleted when view closes.
      ut::delete_(view);
    }

...
/** Forward declaration of function to avoid moving of it in code */
static void trx_purge_wait_for_workers_to_complete(ulint n_purge_threads);
...
struct Purge_groups_t {
  ...
  /** Method for re-using of existing Purge_groups_t instance. */
  void reset() {
    m_groups.assign(m_groups.size(), nullptr);
    m_total_rec = 0;
    init();
  }
  ...
  void assign(que_thr_t **thrs) const {
    const std::size_t n_purge_threads = m_groups.size();
    for (std::size_t grpid = 0; grpid < n_purge_threads; ++grpid) {
...
/** This function runs a purge batch in normal purge mode.
@param[in]      n_purge_threads  number of purge threads
...
static ulint trx_purge_attach_undo_recs(const ulint n_purge_threads,
  ...
  DBUG_EXECUTE_IF("testing_purge_mode_recs", {
    static bool executed_once;
    if (!executed_once) {
      ib::info(ER_IB_MSG_ATTACHING_PURGE_RECORDS, "NORMAL", "starting");
      DBUG_SET("+d,testing_purge_mode_tasks");
      executed_once = true;
    }
  });
  ...
  DBUG_EXECUTE_IF("testing_purge_mode_recs", {
    static bool executed_once;
    if (!executed_once) {
      ib::info(ER_IB_MSG_ATTACHING_PURGE_RECORDS, "NORMAL", "finishing");
      executed_once = true;
    }
  });
...
/** Take n_purge_threads free nodes from query graph, fill them by tasks from
purge_groups and put to queue for run. This function works in fast purge mode
only.
@param[in]     n_purge_threads  the number of worker purge threads
@param[in,out] purge_groups     attached undo records for query nodes */
static void trx_purge_subbatch_to_run(const ulint n_purge_threads,
                                      const Purge_groups_t &purge_groups) {
  ut_a(rds_srv_fast_purge_enabled);
  ulint i = 0;
  que_thr_t *run_thrs[MAX_PURGE_THREADS];
  /* This loop bases firstly on the fact that the number of purge_sys->query's
  nodes equals the number of purge worker threads and secondly on the fact that
  all these nodes have already completed their purge tasks which is provided by
  function trx_purge_wait_for_workers_to_complete(). */
  for (auto thr : purge_sys->query->thrs) {
    if (n_purge_threads <= i) break;
    purge_node_t *node;
  ...
  ut_a(i == n_purge_threads);
  purge_groups.assign(run_thrs);
  /* This is  a fast purge. If purge mode is normal (not fast purge) running
   tasks will be provided into trx_purge() function. */
  DBUG_EXECUTE_IF("testing_purge_mode_tasks", {
    ib::info(ER_IB_MSG_RUNNING_PURGE_TASKS, "FAST");
    DBUG_SET("-d,testing_purge_mode_tasks");
  });
  que_thr_t *thr = nullptr;
  for (ulint i = 0; i < n_purge_threads; ++i) {
    /* Sequence of thrs, returned by que_fork_scheduler_round_robin(), is the
    same as above prepared array run_thrs. Code pattern is used to save this
    function untouched because it is of quite general use. */
    thr = que_fork_scheduler_round_robin(purge_sys->query, thr);
    ut_a(thr != nullptr);
    /* Submit the purge tasks to srv_sys->tasks queue to run. */
    srv_que_task_enqueue_low(thr);
  }
  purge_sys->n_submitted += n_purge_threads;
}

/** This function runs a purge batch. It realizes the logic of fast purge
 approach, if innodb_rds_fast_purge_enabled is ON. The idea is to divide
 batch_size by all_parts parts and to overlap attaching of (i+1)-th part with
 running of i-th part. This approach provides partial concurrent of operations
 and results in acceleration of purge. If all_parts=1 it means no concurrency.
@param[in]      n_purge_threads  number of purge threads
@param[in]      batch_size       number of pages to purge
@return number of undo log pages handled in the batch */
static ulint trx_purge_attach_undo_recs_and_run(const ulint n_purge_threads,
                                                ulint batch_size) {
  ut_a(rds_srv_fast_purge_enabled);
  ulint n_pages_handled = 0;
  ...
  for (ulint i = 0; n_pages_handled < batch_size; ++i) {
    ...
    purge_groups.add(rec);
    /* Fast purge section. */
    /* Check if the next part has been formed. */
    DBUG_EXECUTE_IF("testing_purge_mode_recs", {
      static bool executed_once;
      if (!executed_once && (all_parts > 1) && (parts == 1) &&
          (n_pages_handled >= batch_size / all_parts)) {
        ib::info(ER_IB_MSG_ATTACHING_PURGE_RECORDS, "FAST", "starting");
        DBUG_SET("+d,testing_purge_mode_tasks");
        executed_once = true;
      }
    });
    if ((parts != all_parts) &&
        (n_pages_handled >= (parts * batch_size / all_parts))) {
      purge_groups.distribute_if_needed();
      /* Waiting for all purge query nodes to be released. */
      if (parts > 1) {
        trx_purge_wait_for_workers_to_complete(n_purge_threads);
      }
      /* Launch the running of the current part. */
      trx_purge_subbatch_to_run(n_purge_threads, purge_groups);
      /* Reset of purge_groups before the filling by the next portion of purge
       tasks. */
      purge_groups.reset();
      parts++; /* increment of parts' counter */
    }
    DBUG_EXECUTE_IF("testing_purge_mode_recs", {
      static bool executed_once;
      if (!executed_once && (all_parts > 1) && (parts == 2)) {
        ib::info(ER_IB_MSG_ATTACHING_PURGE_RECORDS, "FAST", "finishing");
        executed_once = true;
      }
    });
  }
  ...
  purge_groups.distribute_if_needed();
  /* The most of the sub-batches have been done, leaving only the last
     incomplete sub-batch. */
  if (all_parts > 1) {
    trx_purge_wait_for_workers_to_complete(n_purge_threads);
  }
  trx_purge_subbatch_to_run(n_purge_threads, purge_groups);
...
static constexpr int purge_wait_sleep_timeout = 20;
/** Wait for pending purge jobs to complete.
@param[in]  the number of purge query graph's nodes  n_purge_threads */
static void trx_purge_wait_for_workers_to_complete(ulint n_purge_threads) {
  /* We don't know if waiting for innodb_purge_threads is efficient or not for
  now, maybe it would be faster if we wait for one thread and assign a task, so
  do the second, third thread, etc. That is why we introduce two innodb monitors
  for developers to observe the status running online to see how long does it
  wait here, so developer will have idea to improve fast purge later. Adding
  monitors is just to pave the way for better observation and potential further
  improvement if necessary. They can help to observe the feature running itself,
  so we can have better understanding in how to set parameters for the feature.
  Monitor MONITOR_PURGE_NODES_WAITING_MAX calculates the waiting time while all
  innodb_purge_threads nodes will become free. Monitor
  MONITOR_PURGE_NODES_WAITING_AVERAGE calculates the average waiting time while
  1,2,...,innodb_purge_threads nodes will become free. How it can help to answer
  the above question? The idea is following. If the values of monitors are
  closed one another there are little chances to improve the feature in such
  way. ANd the more these two values differ, the higher the odds. For example,
  let's take two opposite cases. Suppose the first monitor is N and the second
  one is about N too. It means all nodes are freed approximately simultaneously
  and we win nothing. However, if the second monitor is near
  (N / innodb_purge_threads), it means (innodb_purge_threads - 1) nodes are
  freed at once and we can win, sending them to run not waiting for the last
  node to be released. The third monitor MONITOR_PURGE_NODES_WAITING_MIN gives
  another piece of useful information showing the waiting time of freeing the
  first query node. How to view the monitors? Firstly, we have to launch them.
  It can be done by inserting the line
  "loose-innodb_monitor_enable=purge_nodes_waiting_max_usec;
  purge_nodes_waiting_average_usec;purge_nodes_waiting_min_usec" in .cnf-file.
  Secondly, we have to see the values on running server. It can be done from
  mysql client by request "select * from INFORMATION_SCHEMA.innodb_metrics where
  subsystem='purge' and status='enabled'; */

  ulint sleep_counter = 0;
  /* counter for wait time of n_purge_threads free nodes. */
  ulint final_monitor_time = 0;
  /* average wait time for 1 to n_purge_threads free nodes. */
  ulint tracking_monitor_time = 0;
  /* counter for wait time of the first free node. */
  ulint first_monitor_time = 0;
  /* for which value of available free nodes we have already calculated
  waiting time. */
  ulint currently_calculated_waiting_time = 0;
  bool monitor_calculations = false;
  if (MONITOR_IS_ON(MONITOR_PURGE_NODES_WAITING_MAX) ||
      MONITOR_IS_ON(MONITOR_PURGE_NODES_WAITING_AVERAGE) ||
      MONITOR_IS_ON(MONITOR_PURGE_NODES_WAITING_MIN)) {
    monitor_calculations = true;
  }
  ulint i = 0;
  ulint n_submitted = purge_sys->n_submitted;

  while (true) {
    ulint n_busy_nodes = n_submitted - purge_sys->n_completed.load();
    if (n_busy_nodes == 0) {
      if (monitor_calculations) {
        final_monitor_time = purge_wait_sleep_timeout * sleep_counter;
        if (currently_calculated_waiting_time == 0) {
          first_monitor_time = final_monitor_time;
        }
        for (ulint j = currently_calculated_waiting_time + 1;
             j <= n_purge_threads; j++) {
          tracking_monitor_time += final_monitor_time;
        }
      }
      break;
    }
    ...
    if (monitor_calculations) {
      ulint current_free_nodes = n_purge_threads - n_busy_nodes;
      ulint tracking_monitor_time_increment =
          purge_wait_sleep_timeout * sleep_counter;
      if (currently_calculated_waiting_time == 0) {
        first_monitor_time = tracking_monitor_time_increment;
      }
      for (ulint j = currently_calculated_waiting_time + 1;
           j <= current_free_nodes; j++) {
        tracking_monitor_time += tracking_monitor_time_increment;
      }
      currently_calculated_waiting_time = current_free_nodes;
    }
    if (++i < 10) {
      std::this_thread::yield();
    } else {
      ...
      std::this_thread::sleep_for(
          std::chrono::microseconds(purge_wait_sleep_timeout));
      if (monitor_calculations) {
        sleep_counter++;
      }
      i = 0;
  ...
  MONITOR_SET(MONITOR_PURGE_NODES_WAITING_MAX, final_monitor_time);
  MONITOR_SET(MONITOR_PURGE_NODES_WAITING_AVERAGE,
              tracking_monitor_time / n_purge_threads);
  MONITOR_SET(MONITOR_PURGE_NODES_WAITING_MIN, first_monitor_time);
                ...
                bool truncate)         /*!< in: truncate history if true */
{
  ...
  if (rds_srv_fast_purge_enabled) {
    n_pages_handled =
        trx_purge_attach_undo_recs_and_run(n_purge_threads, batch_size);
    /* Wait for completion of all running tasks. */
    trx_purge_wait_for_workers_to_complete(n_purge_threads);
    goto purge_completed;
  }
  n_pages_handled = trx_purge_attach_undo_recs(n_purge_threads, batch_size);
  DBUG_EXECUTE_IF("testing_purge_mode_tasks", {
    ib::info(ER_IB_MSG_RUNNING_PURGE_TASKS, "NORMAL");
    DBUG_SET("-d,testing_purge_mode_tasks");
  });

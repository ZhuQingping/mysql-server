...
void ha_heap::init_shared_info() {
  m_shared_info.m_type = PQ_temp_table_type::TEMP_HEAP;
  m_shared_info.m_table = nullptr;
  m_shared_info.m_share = file->s;
}

void ha_heap::set_shared_info(PQ_shared_info *info) {
  assert(info->m_type == PQ_temp_table_type::TEMP_HEAP &&
         info->m_table == nullptr);
  assert(file->old_s == nullptr ||
         file->s == static_cast<HP_SHARE *>(info->m_share));
  if (file->old_s == nullptr) {
    file->old_s = file->s;
    file->s = static_cast<HP_SHARE *>(info->m_share);
  }
}

bool ha_heap::is_sharing_data() { return file && file->old_s; }

void ha_heap::reset_shared_info() {
  assert(file && file->s);
  file->s = file->old_s;
  file->old_s = nullptr;
}

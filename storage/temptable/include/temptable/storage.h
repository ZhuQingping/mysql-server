...
class Storage {
 public:
  ...
  /**
     Divides the sequence of data pages into a specified number of ranges of
     pages, and applies a callback to each range.

     As pages are stored as a linked list, and the pointer-to-next is stored
     in the page, the division process has no other choice than traversing the
     list page by page, to put a "range boundary" every Nth page, even though
     it would be faster it it could directly jump over N pages to set the
     boundary.

     In the future, if this causes a performance problem, we could implement a
     "directory of pages" in this engine, built while the table is inserted
     into. More precisely, it could be a sorted vector or a set, which
     indicates the page which marks the 1/10, 2/10,... 9/10 of the whole
     storage (10 is just an example). As more pages are created, this would be
     updated. If pages are deleted (is it possible?) it would also need to be
     updated. With this, it would be easier to do "jumps" (the directory would
     give an approximate first target for the jump).

     @param number_of_ranges  how many ranges to make
     @param callback          callback to execute (is passed the current range's
     start)
   */
  template <typename CALLBACK>
  void do_over_ranges_of_pages(size_t number_of_ranges,

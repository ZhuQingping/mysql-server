...
class Handler : public ::handler {
 public:
...
#if 0
  /* Parallel Query support temptable as divided/cut table, need to call
  cmp_ref() to check whether reach the range boundary. */
  /** Not implemented.
  @return 0 */
  ...
  void init_shared_info() override {
    m_shared_info.m_type = PQ_temp_table_type::TEMP_MEM;
    m_shared_info.m_table = m_opened_table;
    m_shared_info.m_share = nullptr;
  }

  int pq_table_scan_init(void *&scan_ctx, uint n_threads);
  ...
  void set_shared_info(PQ_shared_info *info) override {
    assert(info->m_type == PQ_temp_table_type::TEMP_MEM &&
           info->m_share == nullptr);
    assert(m_old_table == nullptr ||
           m_opened_table == static_cast<Table *>(info->m_table));

    if (m_old_table == nullptr) {
      m_old_table = m_opened_table;
      m_opened_table = static_cast<Table *>(info->m_table);
    }
  }

  bool is_sharing_data() override { return m_old_table; }

  void reset_shared_info() override {
    if (m_old_table) {
      m_opened_table = m_old_table;
      m_old_table = nullptr;
    }
  }
 ...
 private:
  ...
  /** saved table */
  Table *m_old_table{nullptr};
  std::shared_ptr<Parallel_worker> pq_worker;
  bool pq_is_attach_ctx{false};
  std::shared_ptr<PQ_Ctx_Base> cur_pq_ctx{};
...
inline void Handler::opened_table_validate() {
  ...
  assert(m_opened_table->mysql_table_share() == handler::table->s ||
         m_old_table->mysql_table_share() == handler::table->s);
}

...
static bool vio_init(Vio *vio, enum enum_vio_type type, my_socket sd,
  ...
  if (type == VIO_TYPE_SOCKET || type == VIO_TYPE_TCPIP)
    vio->get_err_msg = vio_get_err_msg;
  else
    vio->get_err_msg = nullptr;
  ...
  if (!ret) {
    ...
    */
    if (sd != mysql_socket_getfd(vio->mysql_socket)) {
      if (vio->inactive == false) vio->vioshutdown(vio, SHUT_RDWR);  // NOLINT
    }
#ifdef HAVE_KQUEUE
    else {
      /*

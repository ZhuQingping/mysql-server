...
TEST(HashJoinTest, InnerJoinIntOneToOneMatch) {
  ...
  {
    // Create HashJoinIterator in a scope so that it is destroyed before the THD
    // is destroyed (happens in Server_initializer::TearDown).
    HashJoinIterator hash_join_iterator(
        ...
        /*probe_input_batch_mode=*/false, nullptr, /*use_bloom_filter*/ false,
        /*pq_hash_join*/ nullptr, /*access_path=*/nullptr);
...
TEST(HashJoinTest, HashTableCaching) {
...

  {
    ...
    HashJoinIterator hash_join_iterator(
        ...
        /*use_bloom_filter*/ false, /*pq_hash_join*/ nullptr,
        /*access_path=*/nullptr);
    ...
    ASSERT_FALSE(hash_join_iterator.Init());
    EXPECT_EQ(3,
              build_iterator->num_read_calls());  // Unchanged due to caching.
...
TEST(HashJoinTest, HashJoinResetNullFlagBeforeBuild) {
...

  {
    ...
    // set the NULL row flag before Init was called; Init() should reset the
    // NULL row flag before reading from the build table.
    EXPECT_EQ(0, hash_join_iterator.Read());
    EXPECT_EQ(3, test_helper.left_qep_tab->table()->field[0]->val_int());
    EXPECT_FALSE(test_helper.left_qep_tab->table()->field[0]->is_null());
...
TEST(HashJoinTest, InnerJoinWithBloomFilter) {
  my_testing::Server_initializer initializer;
  initializer.SetUp();

  HashJoinTestHelper test_helper(initializer, {3, 4}, {3, 4, 5});

  {
    ...
    HashJoinIterator hash_join_iterator(
        ...
        /*probe_input_batch_mode=*/false, nullptr, /*use_bloom_filter=*/true,
        /*pq_hash_join=*/nullptr, /*access_path=*/nullptr);
    ...
    expected_result.emplace(4);

    EXPECT_EQ(0, hash_join_iterator.Read());
    longlong result = test_helper.left_qep_tab->table()->field[0]->val_int();
    ASSERT_EQ(1, expected_result.erase(result));
    ...
    // See that the Bloom filter has both accepted and rejected values. Since
    // the probe input has three values, we should expect a total of three
    // accepted and rejected values.
    EXPECT_EQ(2, hash_join_iterator.GetBloomFilter().GetNumAcceptedValues());
    EXPECT_EQ(1, hash_join_iterator.GetBloomFilter().GetNumRejectedValues());
  }
}
...
TEST(HashJoinTest, HashJoinChunkFiles) {
  ...
  HashJoinIterator hash_join_iterator(
      ...
      {*test_helper.join_condition}, true, JoinType::INNER,
      test_helper.extra_conditions,
      /*probe_input_batch_mode=*/false, nullptr, false, nullptr, nullptr);
  ...
  // hash table rows. But as a safeguard, this calculation is adjusted to
  // yield a few extra chunks rather than risk having too few chunks. So the
  // number of remaining rows is instead divided by a reduced count of hash
  // table rows The reduced count is obtained by multiplying the hash table
  // row count by a 'reduction factor' of 0.9. E.g. for 64-bit rows:

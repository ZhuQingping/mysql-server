...
TEST_F(ConnectJoinTest, OuterJoinInSemiJoin) {
  /* Join elimination must be off as the code below expects to find a 3-table
  plan. Hint SET_VAR(optimizer_switch='left_join_elimination=OFF') unfortunately
  cannot be used in the query, it would have no effect (ParseAndResolve()
  parses hints but does not take call their action ; indeed their action
  happens in mysql_execute_command (look for SET_VAR there), which is not
  called). So we turn elimination off like this: */
  m_thd->variables.optimizer_switch &= ~OPTIMIZER_SWITCH_LEFT_JOIN_ELIMINATION;

...
static char *output_merged_file = 0;
static char *merge_dirname = 0;
...
static const char *tmp_postfix = ".tmp";
static unsigned int len_tmp_postfix = strlen(tmp_postfix);
static bool *truncate_option = 0;
static bool *collect_server_id = 0;
static char *server_id_file = 0;
static bool *opt_get_last_event_time = 0;
static bool *opt_get_prev_gtids = 0;
static bool *opt_get_all_gtids = 0;
static bool *merge_option = 0;
static bool first_file = true;
static unsigned int file_number = 0;
static const char *merge_fix = ".merged";
static unsigned int len_merge_fix = strlen(merge_fix);
...
Gtid_set *gtid_set_find_max_log_size = nullptr;
...
static char *opt_find_max_event_size_str = nullptr;
static size_t cur_max_log_event_size = 0;
...
static Exit_status dump_local_log_entries(PRINT_EVENT_INFO *print_event_info,
                                          const char *logname);
static Exit_status collect_serverid_to_file(const char *logname,
                                            bool &append_flag,
                                            uint32 &rotate_server_id,
                                            bool &need_check_sum);
static Exit_status get_last_event_time(char **argv);
static Exit_status get_prev_gtids(char **argv);
static Exit_status get_all_gtids(char **argv);
static Exit_status append_fake_rotate_event_to_file(const char *logname,
                                                    uint32 rotate_server_id,
                                                    bool checksum_on);
...
static std::string normalize_string(const std::string &input);
...
static bool shall_skip_gtids(const Log_event *ev) {
  ...
  switch (ev->get_type_code()) {
    ...
    case binary_log::ANONYMOUS_GTID_LOG_EVENT: {
      ...
      if (opt_find_max_event_size_str != nullptr) {
        filtered = filtered || !gtid_set_find_max_log_size->contains_gtid(
                                   gtid->get_sidno(true), gtid->get_gno());
      }
...
/**
  For --truncate, determine if the file needs to be truncated,
  and if need, create a new file and initialize it.
*/
Exit_status init_tmp_file(IO_CACHE *out_file, bool &need_init_tmp_file,
                          File &tmp_fd, const char *tmp_logname) {
  DBUG_ENTER("init_tmp_file");
  DBUG_EXECUTE_IF("truncate_open_file_fail", tmp_logname = nullptr;);
  if ((tmp_fd = my_open(tmp_logname, O_RDWR | O_CREAT | O_APPEND,
                        MYF(MY_WME))) < 0) {
    error("Open or create file failed.");
    DBUG_RETURN(ERROR_STOP);
  }

  if (init_io_cache(out_file, tmp_fd, 0, WRITE_CACHE, 0, 0,
                    MYF(MY_WME | MY_NABP))) {
    error("Failed to init IO cache.");
    DBUG_RETURN(ERROR_STOP);
  }

  need_init_tmp_file = true;
  DBUG_RETURN(OK_CONTINUE);
}

/**
  The main function is to truncate the log file and process each event.

  @param[in,out] out_file IO cache of new generate file.
  @param[in] ev Log_event to process.
  @param[in,out] count The count of non description or previous event.
...
*/
Exit_status truncate_log_file(IO_CACHE *out_file, Log_event *ev,
                              ulonglong &count) {
  Exit_status ret_status = OK_CONTINUE;
  int ret = 0;
  char *buf = ev->temp_buf;
  uint event_type = ev->common_header->type_code;
  DBUG_ENTER("truncate_log_file");

  /*According to different event types to determine if the event is to be
   * written to the cache*/
  switch (event_type) {
    case binary_log::START_EVENT_V3:
    case binary_log::FORMAT_DESCRIPTION_EVENT:
      /*Magic number must be written before the format description event
       * write.*/
      ret = my_b_write(out_file, (const uchar *)(BINLOG_MAGIC),
                       BIN_LOG_HEADER_SIZE);
      DBUG_EXECUTE_IF("truncate_write_io_cache_fail", ret = 1;);
      if (ret) {
        error("Write io_cache failed.");
        goto err;
      }

      break;
    case binary_log::PREVIOUS_GTIDS_LOG_EVENT:
      /*The timestamp of the event is not exact, write to file without
       * comparison.*/
      break;
    default:
      DBUG_EXECUTE_IF("truncate_xid_event_write_fail",
                      { DBUG_SET("+d,truncate_enter_xid_event"); });
      DBUG_EXECUTE_IF("truncate_xid_event_flush_io_cache_fail",
                      { DBUG_SET("+d,truncate_enter_xid_event"); });
      DBUG_EXECUTE_IF("truncate_enter_xid_event", {
        if (event_type == binary_log::XID_EVENT) {
          ev->common_header->when.tv_sec += 2;
        }
      });
      if (stop_datetime >= (my_time_t)(ev->common_header->when.tv_sec)) {
        count++;
      } else {
        DBUG_EXECUTE_IF("truncate_enter_xid_event", {
          if (event_type == binary_log::XID_EVENT) {
        ...
        if (event_type == binary_log::GTID_LOG_EVENT ||
          ...
          /*
            The specified time point is less than the beginning of the
            transaction and exits normally.
            e.g:
            T1: begin: 8:00:00, end: 8:00:05
            T2: begin: 8:00:10, end: 8:00:11
            stop_datetime: 8:00:07
            result: reserve T1, truncate T2
          */
          ret_status = OK_STOP;
          goto end;
        } else if (event_type == binary_log::XID_EVENT) {
          /*
            Specify the time between transaction commit, save all events
            of this transaction until xid event.
            e.g:
            T1: begin_event: 8:00:00, xid_event: 8:00:03
            stop_datetime: 8:00:01
            result: reserve complete T1
          */
          ret_status = OK_STOP;
        } else {
          count++;
  ...
  ret = my_b_write(out_file, (uchar *)buf, ev->common_header->data_written);
  DBUG_EXECUTE_IF("truncate_write_io_cache_fail1", ret = 1;);
  if (ret) {
    error("Write io_cache failed.");
    goto err;
  }
...
err:
  ret_status = ERROR_STOP;

end:
  DBUG_RETURN(ret_status);
}

/**
  The main function is to merge log files that have have the same
  format_description_event.

  @param[in,out] out_file IO cache of new generate file.
  @param[in] ev Log_event to process.
  @param[in] mark the frist log file.
  ...
  events to process has been reached and the program should terminate.
*/
Exit_status merge_log_file(IO_CACHE *out_file, Log_event *ev,
                           bool &first_file) {
  char *buf = ev->temp_buf;
  static my_off_t ev_end_pos = 0;
  static size_t ev_length = 0;
  static longlong last_committed = 0;
  static longlong sequence_number = 0;
  static longlong last_sequence_num = 0;
  static bool checksum_on;

  /* Group commit: last_committed offset.*/
  int last_committed_offset = LOG_EVENT_HEADER_LEN + 26;
  bool append_flag = true;
  uint event_type = ev->common_header->type_code;

  DBUG_ENTER("merge_log_file");

  /*
   According to different event types to determine
   if the event is to be written to the cache.
  */
  switch (event_type) {
    case binary_log::START_EVENT_V3:
    case binary_log::FORMAT_DESCRIPTION_EVENT:
       ...
       * write.*/
      if (first_file) {
        checksum_on = (ev->common_footer->checksum_alg !=
                       binary_log::BINLOG_CHECKSUM_ALG_OFF);
        if (my_b_write(out_file, (const uchar *)(BINLOG_MAGIC),
                       BIN_LOG_HEADER_SIZE)) {
          error("Write io_cache failed.");
          DBUG_RETURN(ERROR_STOP);
        }
      } else if (checksum_on != (ev->common_footer->checksum_alg !=
                                 binary_log::BINLOG_CHECKSUM_ALG_OFF)) {
        error(
            "Parameter %d (binlog file) from the last is inconsistent"
            " with the frist binlog format",
            file_number);
        DBUG_RETURN(ERROR_STOP);
      }
      file_number--;
      last_sequence_num = sequence_number;
      append_flag = first_file;
      break;
    case binary_log::PREVIOUS_GTIDS_LOG_EVENT:
      append_flag = first_file;
      break;
    case binary_log::STOP_EVENT:
    case binary_log::ROTATE_EVENT:
      /* Only write the last rotate event.*/
      if (file_number > 0) append_flag = false;
      break;
    case binary_log::GTID_LOG_EVENT:
    case binary_log::ANONYMOUS_GTID_LOG_EVENT:
      /* Use the sequence_number of the previous file as the base.*/
      sequence_number = static_cast<Gtid_log_event *>(ev)->sequence_number;
      last_committed = static_cast<Gtid_log_event *>(ev)->last_committed;

      if (!first_file) {
        last_committed += last_sequence_num;
        sequence_number += last_sequence_num;
        int8store(buf + last_committed_offset, last_committed);
        int8store(buf + last_committed_offset + 8, sequence_number);
      }
      break;
    default:
      break;
  ...
  if (append_flag) {
    ev_length = ev->common_header->data_written;

    /* Start modifying the end_log_pos if it is not the first file*/
    if (first_file)
      ev_end_pos = ev->common_header->log_pos;
    else {
      ev_end_pos += ev_length;
      int4store(buf + LOG_POS_OFFSET, ev_end_pos);
    }
    ...
    if (checksum_on) {
      ...
      crc =
          checksum_crc32(crc, (const unsigned char *)buf,
                         ev->common_header->data_written - BINLOG_CHECKSUM_LEN);
      int4store(buf + ev->common_header->data_written - BINLOG_CHECKSUM_LEN,
                crc);
    }

    if (my_b_write(out_file, (uchar *)buf, ev->common_header->data_written)) {
      error("Write io_cache failed.");
      DBUG_RETURN(ERROR_STOP);
    }
  }
...
static Exit_status process_event(PRINT_EVENT_INFO *print_event_info,
  ...
  */
  if (((rec_count >= offset) &&
    ...
    // reached stop time
    if ((!truncate_option) &&
        ((my_time_t)(ev->common_header->when.tv_sec) >= stop_datetime)) {
      /* end the program */
      retval = OK_STOP;
      goto end;
    }
    ...
    if (opt_find_max_event_size_str != nullptr) {
      cur_max_log_event_size =
          std::max(ev->common_header->data_written, cur_max_log_event_size);
      goto end;
    }
...
static struct my_option my_long_options[] = {
    ...
    */
    ...
    {"find-max-event-size", OPT_FIND_MAX_EVENT_SIZE,
     "Finds and prints the maximum event size within the specified GTID set.",
     &opt_find_max_event_size_str, &opt_find_max_event_size_str, nullptr,
...
#include "sslopt-longopts.h"
    ...
    {"print-table-metadata", OPT_PRINT_TABLE_METADATA,
     ...
     nullptr},
    {"collect-server-id", OPT_COLLECT_SERVER_ID,
     "Collect distinct server_ids in a binlog file and write its to the file "
     "specified by the --server-id-file option.",
     &collect_server_id, &collect_server_id, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"server-id-file", OPT_SERVER_ID_FILE,
     "Find out the different server_id "
     "and record it in the specified file.",
     &server_id_file, &server_id_file, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0,
     0},
    {"get-last-event-time", OPT_GET_LAST_EVENT_TIME,
     "Get the time of the last event "
     "in file specified by the --get-last-event-time option.",
     &opt_get_last_event_time, &opt_get_last_event_time, 0, GET_BOOL, NO_ARG, 0,
     0, 0, 0, 0, 0},
    {"get-prev-gtids", OPT_GET_PREV_GTIDS,
     "Get gtids of the prev gtid log event "
     "in file specified by the --get-prev-gtids option.",
     &opt_get_prev_gtids, &opt_get_prev_gtids, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0,
     0, 0},
    {"get-all-gtids", OPT_GET_ALL_GTIDS,
     " Get gtids of the prev gtid log event and the gtid log events from a "
     "binary log specified by the --get-all-gtids"
     "option.",
     &opt_get_all_gtids, &opt_get_all_gtids, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0,
     0},
    {"truncate", OPT_TRUNCATE,
     "Clear events after the point in time "
     "according to the explicitly specified stop-datetime, keeping the last "
     "ratote event in order to make the integrity of the entire binlog.",
     &truncate_option, &truncate_option, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
    {"merged-file", 'm',
     "Specify the merged file name with merge binlog feature."
     " Use default parameters if this parameter is empty.",
     &output_merged_file, &output_merged_file, 0, GET_STR, REQUIRED_ARG, 0, 0,
     0, 0, 0, 0},
    {"merge-dir", OPT_MERGE_DIR,
     "Specify the directory where the merged binlog "
     "is located with merge feature."
     " Use default parameters if this parameter is empty.",
     &merge_dirname, &merge_dirname, 0, GET_STR, REQUIRED_ARG, 0, 0, 0, 0, 0,
     0},
    {"merge", OPT_MERGE,
     "If they have the same FORMAT_DESCRIPTION_EVENT, "
     "merge all given files.",
     &merge_option, &merge_option, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0, 0, 0},
...
static Exit_status dump_single_log(PRINT_EVENT_INFO *print_event_info,
  ...
  bool append_flag = true;
  bool need_check_sum = false;
  uint32 rotate_server_id = 0;

  switch (opt_remote_proto) {
    case BINLOG_LOCAL:
      if (collect_server_id) {
        rc = collect_serverid_to_file(logname, append_flag, rotate_server_id,
                                      need_check_sum);
        if ((rc == OK_CONTINUE) && append_flag) {
          rc = append_fake_rotate_event_to_file(logname, rotate_server_id,
                                                need_check_sum);
        }
      } else {
...
static Exit_status dump_local_log_entries(PRINT_EVENT_INFO *print_event_info,
                                          const char *logname) {
  Exit_status ret = OK_CONTINUE;
  ulonglong count = 0;
  char *tmp_logname = 0;
  char *src_logname = 0;
  static char *merge_logname = 0;
  File tmp_fd = -1;
  IO_CACHE out_cache, *out_file = &out_cache;
  bool need_init_tmp_file = false;
  bool need_init_merged_file = false;
  ...
  transaction_parser.reset();
  if (truncate_option && stop_datetime == MYTIME_MAX_VALUE) {
    warning(
        "Not using the --stop-datetime option,please explicitly specify "
        "--stop-datetime option, the file not need to be truncate.");
    return ERROR_STOP;
  }

  if (merge_dirname) {
    uint merge_dirname_len = strlen(merge_dirname);
    uint logname_len = strlen(logname);
    if ((merge_dirname_len + logname_len + 1) > FN_REFLEN) {
      error("Binlog name too long.");
      goto err;
    }

    if (!(src_logname = (char *)my_malloc(key_memory_log_event,
                                          merge_dirname_len + logname_len + 1,
                                          MYF(MY_WME)))) {
      error("Generate new file name out of memory!");
      goto err;
    }

    strncpy(src_logname, merge_dirname, merge_dirname_len);
    strncpy(src_logname + merge_dirname_len, logname, logname_len);
    src_logname[merge_dirname_len + logname_len] = '\0';
    logname = src_logname;
  }
  ...
  if (truncate_option) {
    uint logname_len = strlen(logname);
    DBUG_EXECUTE_IF("truncate_logname_too_long", logname_len = FN_REFLEN;);
    if ((logname_len + len_tmp_postfix + 1) > FN_REFLEN) {
      error("Log name too long.");
      goto err;
    }

    tmp_logname = (char *)my_malloc(
        key_memory_log_event, logname_len + len_tmp_postfix + 1, MYF(MY_WME));
    DBUG_EXECUTE_IF("truncate_tmp_logname_is_null", tmp_logname = nullptr;);
    if (!tmp_logname) {
      error("Generate new file name out of memory!");
      goto err;
    }

    strncpy(tmp_logname, logname, logname_len + 1);
    strncpy(tmp_logname + logname_len, tmp_postfix, len_tmp_postfix + 1);
    tmp_logname[len_tmp_postfix + logname_len] = '\0';
  }

  if (merge_option && !output_merged_file && first_file) {
    uint logname_len = strlen(logname);

    if ((logname_len + len_merge_fix + 1) > FN_REFLEN) {
      error("Binlog name too long.");
      goto err;
    }

    if (!(merge_logname = (char *)my_malloc(key_memory_log_event,
                                            logname_len + len_merge_fix + 1,
                                            MYF(MY_WME)))) {
      error("Generate new file name out of memory!");
      goto err;
    }

    strncpy(merge_logname, logname, logname_len + 1);
    strncpy(merge_logname + logname_len, merge_fix, len_merge_fix + 1);
    merge_logname[len_merge_fix + logname_len] = '\0';
  } else if (merge_option && output_merged_file && first_file) {
    uint logname_len = strlen(output_merged_file);

    if ((logname_len + 1) > FN_REFLEN) {
      error("Binlog name too long.");
      goto err;
    }

    if (!(merge_logname = (char *)my_malloc(key_memory_log_event,
                                            logname_len + 1, MYF(MY_WME)))) {
      error("Generate new file name out of memory!");
      goto err;
    }

    strncpy(merge_logname, output_merged_file, logname_len);
    merge_logname[logname_len] = '\0';
  }

  if (merge_option && !need_init_merged_file) {
    if (init_tmp_file(out_file, need_init_merged_file, tmp_fd, merge_logname) !=
        OK_CONTINUE)
      goto err;
  }
  ...
  for (;;) {
    ...
    if (ev == nullptr) {
      ...
      if (error_type == Binlog_read_error::READ_EOF) goto end;
    ...
    if (truncate_option) {
      if (!need_init_tmp_file &&
          (init_tmp_file(out_file, need_init_tmp_file, tmp_fd, tmp_logname) !=
           OK_CONTINUE))
        goto err;

      if ((ret = truncate_log_file(out_file, ev, count)) != OK_CONTINUE) {
        if (ev) delete ev;
        goto end;
      }
    }
    ...
    if (merge_option &&
        (ret = merge_log_file(out_file, ev, first_file)) != OK_CONTINUE) {
      if (ev) delete ev;
      goto end;
    }
    ...
    ret = process_event(print_event_info, ev, old_off, logname);
    if (ret != OK_CONTINUE) goto end;
  }
...
err:
  ...
  if (need_init_tmp_file && end_io_cache(out_file)) ret = ERROR_STOP;

  if (need_init_merged_file && end_io_cache(out_file)) ret = ERROR_STOP;

  if (tmp_fd >= 0) my_close(tmp_fd, MYF(MY_WME));

  if (truncate_option && (ret == OK_STOP) && (count == 0)) {
    warning(
        "This file don't need to truncate,because of all the events "
        "are after the specified time.");
    DBUG_EXECUTE_IF("truncate_delete_tmp_logname_fail", tmp_logname = nullptr;);
    if (my_delete(tmp_logname, MYF(0))) {
      error("Failed to delete tmp file: %s", tmp_logname);
      ret = ERROR_STOP;
    }
  }

  if (tmp_logname != NULL) my_free(tmp_logname);

  if (merge_option && need_init_merged_file && (ret == ERROR_STOP)) {
    warning("Inconsistent binlog format.");
    if (my_delete(merge_logname, MYF(0))) {
      error("Failed to delete merged file: %s", merge_logname);
      ret = ERROR_STOP;
    }
  }

  if (!file_number && merge_logname != NULL) my_free(merge_logname);

  if (src_logname != NULL) my_free(src_logname);
...
static Exit_status get_last_event_time(char **argv) {
  ...
  uchar buff[LOG_EVENT_HEADER_LEN] = {'\0'};
  ...
  for (;;) {
    mysqlbinlog_file_reader.seek(pos);
    ssize_t ret = mysqlbinlog_file_reader.read(buff, LOG_EVENT_HEADER_LEN);
    if (ret != 0 &&
        LOG_EVENT_HEADER_LEN != mysqlbinlog_file_reader.position() - pos) {
      error("binlog event is not complete, pos:%lld, read pos:%lld", pos,
            mysqlbinlog_file_reader.position());
      goto err;
    }
    if (LOG_EVENT_HEADER_LEN != ret) break;

    event_type = buff[EVENT_TYPE_OFFSET];
    if (event_type == binary_log::QUERY_EVENT) {
      timestamp = uint4korr(buff);
    }

    pos = pos + (my_off_t)uint4korr(buff + EVENT_LEN_OFFSET);
    if (pos >= stop_position) break;
  }

  if (timestamp == 0) {
    error("can't find query event");
    goto err;
  }
...
err:
  ...
  if (mysqlbinlog_file_reader.is_open()) mysqlbinlog_file_reader.close();
  return retval;
}
...
static Exit_status get_prev_gtids(char **argv) {
  ...
  for (;;) {
    ...
    if (ev->get_type_code() == binary_log::FORMAT_DESCRIPTION_EVENT) {
      glob_description_event = dynamic_cast<Format_description_event &>(*ev);
      mysqlbinlog_file_reader.seek(ev->common_header->log_pos);
      continue;
    }

    DBUG_EXECUTE_IF("inject_without_prev_gtid_event",
                    ev->common_header->type_code = binary_log::ENUM_END_EVENT;);
    if (ev->get_type_code() == binary_log::PREVIOUS_GTIDS_LOG_EVENT) {
      char *str = dynamic_cast<Previous_gtids_log_event *>(ev)->get_str(
          NULL, &Gtid_set::commented_string_format);
      DBUG_EXECUTE_IF("inject_str_is_null", str = NULL;);
      if (str == NULL) {
        error("Failed to parse Previous_gtids_log_event.");
        goto err;
      }
      // str format:
      // # {uuid1}:{sid1},
    ...
    error("Can't find Previous_gtids_log_event.");
    goto err;
  }
  goto end;
err:
...
/* Get gtids of the prev gtid log event and the gtid log events from a binary
 * log specified by the --get-all-gtids */
static Exit_status get_all_gtids(char **argv) {
  Exit_status retval = OK_CONTINUE;
  char *logname = argv[0];
  ulong max_event_size = 0;
  ...
  rpl_sidno cur_sidno = -1;
  rpl_gno cur_gno = -1;
  while ((ev = mysqlbinlog_file_reader.read_event_object()) != nullptr) {
    switch (ev->get_type_code()) {
      case binary_log::PREVIOUS_GTIDS_LOG_EVENT: {
        Previous_gtids_log_event *prev_gtids_ev =
            (Previous_gtids_log_event *)ev;
        prev_gtids_ev->add_to_set(&gtids_in_binlog);
        break;
      }
      case binary_log::GTID_LOG_EVENT: {
      ...
      case binary_log::QUERY_EVENT: {
        ...
        std::string query{query_ev->query};
        bool need_save_gtid =
            query_ev->ddl_xid != binary_log::INVALID_XID || query == "COMMIT" ||
            query.find("XA COMMIT") == 0 || query.find("XA ROLLBACK") == 0;
        if (!need_save_gtid) {
          std::string normalize_query = normalize_string(query_ev->query);
          if (normalize_query.find("FLUSH PRIVILEGES") == 0 ||
              normalize_query.find("OPTIMIZE TABLE") == 0 ||
              normalize_query.find("FLUSH ENGINE") == 0 ||
              normalize_query.find("ANALYZE TABLE") == 0) {
            need_save_gtid = true;
          }
        }
        if (need_save_gtid && cur_gno >= 0) {
          gtids_in_binlog.ensure_sidno(cur_sidno);
          gtids_in_binlog._add_gtid(cur_sidno, cur_gno);
          cur_sidno = -1;
          cur_gno = -1;
  ...
  char *str = nullptr;
  gtids_in_binlog.to_string(&str);
  fprintf(result_file, "%s\n", str);
  my_free(str);
  global_sid_lock->unlock();
  if (mysqlbinlog_file_reader.is_open()) {
...
/**
  Reads a local binlog and output the server id it sees.
...
static Exit_status collect_serverid_to_file(const char *logname,
                                            ...
                                            bool &need_check_sum) {
  my_off_t pos = BIN_LOG_HEADER_SIZE;
  uchar buff[LOG_EVENT_HEADER_LEN];
  Exit_status retval = OK_CONTINUE;
  uint32 ev_server_id = 0;
  uint event_type = 0;
  FILE *out_fd = NULL;
  ulong max_event_size = 0;
  ...
  if (!server_id_file) {
    error("Server-id-file option not specified.");
    return ERROR_STOP;
  }
  ...
  Format_description_log_event *fdle = nullptr;
  DBUG_EXECUTE_IF("collect_serverid_my_open_fail", logname = "not_exists";);
  if (mysqlbinlog_file_reader.open(logname, pos, &fdle, true)) {
    error("%s", mysqlbinlog_file_reader.get_error_str());
    return ERROR_STOP;
  }

  DBUG_ASSERT(fdle != nullptr);
  need_check_sum =
      fdle->common_footer->checksum_alg != binary_log::BINLOG_CHECKSUM_ALG_OFF;
  DBUG_EXECUTE_IF("collect_serverid_open_file_fail", server_id_file = nullptr;);
  if (!(out_fd = my_fopen(server_id_file, O_WRONLY | O_APPEND, MYF(MY_WME)))) {
    error("Could not create server_id file '%s'.", server_id_file);
    goto err;
  }
  ...
  for (;;) {
    ...
    ssize_t ret = mysqlbinlog_file_reader.read(buff, LOG_EVENT_HEADER_LEN);
    if (LOG_EVENT_HEADER_LEN != ret) break;
    ev_server_id = uint4korr(buff + SERVER_ID_OFFSET);
    server_ids.insert(ev_server_id);
    pos = pos + (my_off_t)uint4korr(buff + EVENT_LEN_OFFSET);
  }

  event_type = buff[EVENT_TYPE_OFFSET];
  if (event_type == binary_log::ROTATE_EVENT) {
    /* append a rotate event to the end of log file that is not end of it */
    append_flag = false;
  }

  rotate_server_id = ev_server_id;

  for (auto id : server_ids) fprintf(out_fd, "%u\n", id);
...
err:
  ...
  if (out_fd) my_fclose(out_fd, MYF(0));

  return retval;
}

inline void calc_fake_event_checksum(uchar *event_ptr, size_t event_len) {
  uint32 crc = checksum_crc32(0L, NULL, 0);
...
/**
  The main function is to append fake rotate event to the log file.

  @param[in] the name of bin file.
  @param[in,out] server_id.
  @param[in] need to check or not.
...
static Exit_status append_fake_rotate_event_to_file(const char *logname,
  ...
  if ((fd = my_open(logname, O_RDWR | O_CREAT | O_APPEND, MYF(MY_WME))) < 0) {
    error("Open or create file failed.");
    return ERROR_STOP;
  }

  if (init_io_cache(file, fd, 0, WRITE_CACHE, 0, 0, MYF(MY_WME | MY_NABP))) {
    error("Failed to init IO cache.");
    my_close(fd, MYF(MY_WME));
    return ERROR_STOP;
  }
  my_off_t pos = my_b_filelength(file);
  Exit_status retval = OK_CONTINUE;
  uchar event[64];
  const char *p = "fake_rotate_event";
  size_t ident_len = strlen(p);
  size_t event_len = ident_len + LOG_EVENT_HEADER_LEN +
                     Binary_log_event::ROTATE_HEADER_LEN +
                     (checksum_on ? BINLOG_CHECKSUM_LEN : 0);

  int4store(event, 0);
  event[EVENT_TYPE_OFFSET] = binary_log::ROTATE_EVENT;
  int4store(event + SERVER_ID_OFFSET, rotate_server_id);
  int4store(event + EVENT_LEN_OFFSET, static_cast<uint32>(event_len));
  int4store(event + LOG_POS_OFFSET, pos + event_len);
  int2store(event + FLAGS_OFFSET, LOG_EVENT_ARTIFICIAL_F);
  int8store(event + LOG_EVENT_HEADER_LEN, BIN_LOG_HEADER_SIZE);

  memcpy(event + LOG_EVENT_HEADER_LEN + Binary_log_event::ROTATE_HEADER_LEN, p,
         ident_len);

  if (checksum_on) calc_fake_event_checksum(event, event_len);

  my_b_write(file, event, event_len);

  if (my_b_flush_io_cache(file, 1)) {
    error("Flush io_cache failed.");
    return ERROR_STOP;
  }
...
static int args_post_process(void) {
  ...
  if (opt_find_max_event_size_str != nullptr) {
    if (raw_mode || opt_include_gtids_str || opt_exclude_gtids_str) {
      error(
          "The option --find-max-event-size cannot be used in combination with "
          "--raw,"
          "--include-gtids or --exclude-gtids.");
      return ERROR_STOP;
    }
  }
  global_sid_lock->rdlock();
  ...
  if (opt_find_max_event_size_str != nullptr) {
    if (gtid_set_find_max_log_size->add_gtid_text(
            opt_find_max_event_size_str) != RETURN_STATUS_OK) {
      error("Could not configure --find-max-event-size '%s'",
            opt_find_max_event_size_str);
      global_sid_lock->unlock();
      return ERROR_STOP;
    }
  }
...
inline bool gtid_client_init() {
  bool res = (!(global_sid_lock = new Checkable_rwlock) ||
              ...
              !(gtid_set_find_max_log_size = new Gtid_set(global_sid_map)));
  if (res) {
    gtid_client_cleanup();
  }
  return res;
 ...
 * 2. Collapsing consecutive whitespace into single spaces
...
static std::string normalize_string(const std::string &input) {
  ...
  auto it = input.begin();
  while (it != input.end()) {
    // Skip any leading or consecutive whitespace characters
    while (it != input.end() && std::isspace(static_cast<unsigned char>(*it))) {
      ++it;
    }
    // If we've already added content to result and we're not at the end,
    // and the previous character was whitespace, add a single space
    if (!result.empty() && it != input.end() &&
        std::isspace(static_cast<unsigned char>(*(it - 1)))) {
      result += ' ';
    }
    // Process non-whitespace characters: convert to uppercase and add to result
    while (it != input.end() &&
...
int main(int argc, char **argv) {
  ...
  */
  ...
  if (opt_get_last_event_time || opt_get_prev_gtids || opt_get_all_gtids) {
    if (opt_get_last_event_time)
      retval = get_last_event_time(argv);
    else if (opt_get_all_gtids) {
      retval = get_all_gtids(argv);
    } else
      retval = get_prev_gtids(argv);
  ...
  if (opt_find_max_event_size_str != nullptr)
    fprintf(result_file, "# The max log event size is %zu Byte.\n",
            cur_max_log_event_size);

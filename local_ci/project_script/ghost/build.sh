...
sh start_ci.sh -t ${build_type} -o no_format,${test_type} 2>&1 | tee ci_ghost.log

exit_flag=0
if [ -n "`grep "Compile failed" ${workdir}/mysql-server/local_ci/script/logs/compile.log`" ]; then
  echo "Failure Cause: Compile failed"
  exit_flag=1
elif [ -f ${workdir}/mysql-server/local_ci/script/logs/run_ghost.log ]; then
  if [ -n "`grep -E "Data validation failed" ${workdir}/mysql-server/local_ci/script/logs/run_ghost.log`" ]; then
...
rm_docker ${workdir}/mysql-server/local_ci/script/ci_ghost.log

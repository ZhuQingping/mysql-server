...
bool partition_info::pq_copy_from(partition_info *part_info) {
  bitmap_copy(&read_partitions, &part_info->read_partitions);
  bitmap_copy(&lock_partitions, &part_info->lock_partitions);
  return false;
}

partition_info *partition_info::clone(THD *thd, bool reset) {
  partition_info *clone =
      is_subpart_info()
          ? new (thd->mem_root)
                sub_partition_info(*down_cast<sub_partition_info *>(this))
          : new (thd->mem_root) partition_info(*this);
  if (!clone) {
  ...
  clone->partition.subpartitions.clear();
  if (reset) clone->defined_max_value = false;
  return clone;
}

// Clone one partition element
partition_element *partition_element::clone(THD *thd,
                                            const partition_info *part_info,
                                            bool reset) {
  partition_element *part_clone = new (thd->mem_root) partition_element(*this);
  if (!part_clone) {
  ...
  if (tablespace_name != nullptr)
    part_clone->tablespace_name = strmake_root(thd->mem_root, tablespace_name,
                                               strlen(tablespace_name) + 1);

  /*
    Mark that RANGE and LIST values needs to be fixed so that we don't
                use old values. fix_column_value_functions would evaluate the
    values from Item expression.
  */
  if (reset && part_info) {
    ...
    while (part_elem_value *list_value = list_it++) {
      ...
      for (uint i = 0; i < part_info->num_columns; col_val++, i++) {
        col_val->fixed = 0;
      }
    }
  }
...
partition_info *partition_info::get_clone(THD *thd, bool reset /* = false */) {
  ...
  partition_info *part_info_clone = clone(thd, reset);
  if (!part_info_clone) return nullptr;

  sub_partition_info *subpart_info_clone = nullptr;
  if (sub_part_info) {
    subpart_info_clone =
        down_cast<sub_partition_info *>(sub_part_info->clone(thd, reset));
    if (!subpart_info_clone) return nullptr;
    part_info_clone->sub_part_info = subpart_info_clone;
    subpart_info_clone->set_parent(part_info_clone);
  }

  while ((part = (part_it++))) {
    partition_element *subpart;
    partition_element *part_clone = part->clone(thd, this, reset);
    if (!part_clone) return nullptr;

    if (!part->subpartitions.is_empty()) {
      List_iterator<partition_element> subpart_it(part->subpartitions);
      while ((subpart = (subpart_it++))) {
        partition_element *subpart_clone =
            subpart->clone(thd, subpart_info_clone, reset);
        if (!subpart_clone) return nullptr;

        part_info_clone->sub_part_info->partitions.push_back(subpart_clone);
        part_clone->subpartitions.push_back(subpart_clone);
      }
    }
    part_info_clone->partitions.push_back(part_clone);
  }

  uint part_id = 0;
  if (is_sub_partitioned()) {
    List_iterator<Sub_partition> sub_part_it(partition.subpartitions);
    Sub_partition *sub_partition;
    while ((sub_partition = (sub_part_it++))) {
      Sub_partition *sub_part =
          partition.subpartitions[part_id]->clone(thd, part_id, reset);
      if (!sub_part ||
          part_info_clone->partition.subpartitions.push_back(sub_part))
        return nullptr;
      sub_part->part_info = subpart_info_clone;
      part_id++;
    }
  }
  return part_info_clone;
}
...
bool partition_info::add_named_partition(const char *part_name, size_t length) {
  ...
  if (part_def->is_subpart) {
    ...
    if (is_sub_partitioned()) {
      ...
      uint end = start + sub_part_info->num_parts;
  ...
  if (column_list)
    ...
    if (part_expr)
      part_expr->walk(&Item::intro_version, enum_walk::POSTFIX,
                      (uchar *)&version);
    if (sub_part_info && sub_part_info->part_expr)
      sub_part_info->part_expr->walk(&Item::intro_version, enum_walk::POSTFIX,
                                     (uchar *)&version);
    if (version == 0) {
...
char *sub_partition_info::create_default_subpartition_name(
    uint subpart_no, const char *part_name) {
...
bool sub_partition_info::set_up_default_partitions(
    Partition_handler *part_handler, HA_CREATE_INFO *info, uint) {
  if (part_type != partition_type::HASH) {
    const char *error_string;
    if (part_type == partition_type::RANGE)
      error_string = partition_keywords[PKW_RANGE].str;
  ...
  partition_info *parent = get_parent();
  List_iterator<partition_element> part_it(parent->partitions);
  DBUG_TRACE;
  ...
  if (unlikely((parent->get_tot_partitions()) > MAX_PARTITIONS)) {
    my_error(ER_TOO_MANY_PARTITIONS_ERROR, MYF(0));
    goto end;
  }
  i = 0;
  do {
    ...
    do {
      ...
      if (likely(subpart_elem != nullptr &&
                 ...
                 (!partitions.push_back(subpart_elem)))) {
        char *ptr =
    ...
    Sub_partition *sub_part = new (*THR_MALLOC) Sub_partition(this, i);
    if (!sub_part || parent->partition.subpartitions.push_back(sub_part))
      goto end;
  } while (++i < parent->num_parts);
  result = false;
end:
  return result;
}
...
bool partition_info::set_up_defaults_for_partitioning(
  ...
  if (!default_partitions_setup) {
    ...
    if (use_default_partitions &&
        set_up_default_partitions(part_handler, info, start_no))
      return true;
    if (is_sub_partitioned() && sub_part_info->use_default_partitions) {
      sub_part_info->default_partitions_setup = true;
      uint dummy = 0;
      return sub_part_info->set_up_default_partitions(part_handler, info,
                                                      dummy);
    }
  }
  return false;
    ...
    if (is_sub_partitioned()) {
      ...
      do {
        ...
        if (!my_strcasecmp(system_charset_info, sub_part_elem->partition_name,
                           partition_name)) {
          *part_id = j + (i * get_num_subparts());
          return sub_part_elem;
        }
      } while (++j < get_num_subparts());
...
bool partition_info::check_range_constants(THD *thd) {
  ...
  uint tot_parts =
      num_parts * (is_subpart_info() ? get_parent()->num_parts : 1);
  assert(tot_parts > 0);
  if (column_list) {
    ...
    range_col_array =
        (part_column_list_val *)sql_calloc(tot_parts * size_entries);
    if (unlikely(range_col_array == nullptr)) {
      mem_alloc_error(tot_parts * size_entries);
      goto end;
    }
    ...
    do {
      ...
      // Subpartition should be checked in the scope of one partition
      ((i + 1) % num_parts) ? first = false : first = true;
    } while (++i < tot_parts);
  } else {
    ...
    range_int_array =
        (longlong *)(*THR_MALLOC)->Alloc(tot_parts * sizeof(longlong));
    if (unlikely(range_int_array == nullptr)) {
      mem_alloc_error(tot_parts * sizeof(longlong));
      goto end;
    }
    i = 0;
    do {
      part_def = it++;
      if ((i != (tot_parts - 1)) || !defined_max_value) {
      ...
      if (!first) {
        if (unlikely(current_largest > part_range_value) ||
            (unlikely(current_largest == part_range_value) &&
             (part_range_value < LLONG_MAX || i != (tot_parts - 1) ||
              !defined_max_value)))
          goto range_not_increasing_error;
      }
  ...
  // Fix must be after sub-partitions finish so that range_xx_array is
  // initialized.
  if (is_subpart_info()) get_parent()->fix_subs_val_array_pointers();
  result = false;
end:
  return result;
...
bool partition_info::check_list_constants_on_partition(
    THD *thd, List_iterator<partition_element> &list_func_it) {
  ...
  void *ptr;
  List_iterator<partition_element> org_list_func_it = list_func_it;
  DBUG_TRACE;

  num_list_values = 0;
  // This function would be called multiple times for subpart_info,
  // reset null values to default value.
...
bool partition_info::check_list_constants(THD *thd) {
  ...
  uint i = 0;
  while (i < tot_parts / num_parts) {
    if (check_list_constants_on_partition(thd, list_func_it)) return true;

    if (is_subpart_info()) {
      Sub_partition *sub = get_parent()->partition.subpartitions[i];
      sub->has_null_value = has_null_value;
      sub->has_null_part_id = has_null_part_id;
      sub->num_list_values = num_list_values;
      sub->list_array = list_array;
      sub->list_col_array = list_col_array;
    }
    i++;
  }
  return false;
...
/*
  Helper function for check_partition_info.

  SYNOPSIS
    check_partition_info_helper()
    thd                 Thread object
    file                A reference to a handler of the table
    info                Create info
    add_or_reorg_part   Is it ALTER TABLE ADD/REORGANIZE command
    use_default_subparts      Whether use default subpartition defination
    use_default_num_subparts  Whether use default subpartition numbers
...
*/
bool partition_info::check_partition_info_helper(
    THD *thd, handler *file, HA_CREATE_INFO *info, bool add_or_reorg_part,
    bool use_default_subparts, bool use_default_num_subparts) {
  bool result = true;
  ...
  if (!add_or_reorg_part) {
    ...
    // Only main partition needs fix_parser_data here because main partition
    // will fix subpartitions' too.
    if (thd->lex->sql_command == SQLCOM_CREATE_TABLE && !is_subpart_info() &&
        fix_parser_data(thd))
      goto end;
  }

  // if not define subpartition option, not allow define subpartition defination
  if (unlikely(!is_sub_partitioned() &&
               !(use_default_subparts && use_default_num_subparts))) {
  ...
  if (!default_partitions_setup) {
    ...
    if (use_default_partitions) {
      if (is_subpart_info()) {
        sub_partition_info *subpart_info =
            down_cast<sub_partition_info *>(this);
        if (subpart_info && subpart_info->set_up_default_partitions(
                                file->get_partition_handler(), info, (uint)0))
          goto end;
      } else if (set_up_default_partitions(file->get_partition_handler(), info,
                                           (uint)0)) {
        goto end;
      }
    }
...
bool partition_info::check_partition_info(THD *thd, handlerton **eng_type,
                                          ...
                                          bool use_default_subparts,
                                          bool use_default_num_subparts) {
  ...
  if (check_partition_info_helper(thd, file, info, add_or_reorg_part,
                                  use_default_subparts,
                                  use_default_num_subparts))
    goto end;
  /* Check for sub partition expression. */
  if (is_sub_partitioned()) {
    // TODO: Set it when sub_partition_info is created?
    sub_part_info->default_engine_type = default_engine_type;
    if (sub_part_info->check_partition_info_helper(
            thd, file, info, add_or_reorg_part, true, true))
      goto end;
  }
  ...
  */
  ...
  i = 0;
  {
    ...
    uint prev_num_subparts_not_set = get_num_subparts() + 1;
    do {
      partition_element *part_elem = part_it++;
      ...
      if (!is_sub_partitioned()) {
        ...
        if (prev_num_subparts_not_set == (get_num_subparts() + 1) &&
            (num_subparts_not_set == 0 ||
             num_subparts_not_set == get_num_subparts()))
    ...
    if (!table_engine_set && num_parts_not_set != 0 &&
      ...
      DBUG_PRINT("info", ("num_parts_not_set = %u num_parts = %u",
                          num_parts_not_set, get_num_subparts()));
      my_error(ER_MIX_HANDLER_ERROR, MYF(0));
      goto end;
    }
  }

  if (ha_is_externally_disabled(*table_engine)) {
    const char *en{ha_resolve_storage_engine_name(table_engine)};
...
bool partition_info::set_part_expr(char *start_token, Item *item_ptr,
  ...
  if (is_subpart) {
    sub_part_info->list_of_part_fields = false;
    sub_part_info->part_expr = item_ptr;
    sub_part_info->part_func_string = func_string;
    sub_part_info->part_func_len = expr_len;
  } else {
...
bool partition_info::check_partition_field_length() {
  ...
  if (is_sub_partitioned()) {
    for (i = 0; i < sub_part_info->num_part_fields; i++)
      store_length +=
          get_partition_field_store_length(sub_part_info->part_field_array[i]);
    if (store_length > MAX_KEY_LENGTH) return true;
  }
  return false;
}
...
bool partition_info::set_up_charset_field_preps() {
  ...
  if (is_sub_partitioned() && !sub_part_info->list_of_part_fields &&
      check_part_func_fields(sub_part_info->part_field_array, false)) {
    /* Set up arrays and buffers for those fields */
    ptr = sub_part_info->part_field_array;
    while ((field = *(ptr++))) {
      if (field_is_partition_charset(field)) {
        tot_subpart_fields++;
      }
    ...
    if (!(char_ptrs = (uchar **)sql_calloc(size))) goto error;
    sub_part_info->part_field_buffers = char_ptrs;
    if (!(char_ptrs = (uchar **)sql_calloc(size))) goto error;
    sub_part_info->restore_part_field_ptrs = char_ptrs;
    size = (tot_subpart_fields + 1) * sizeof(Field *);
    if (!(char_ptrs = (uchar **)(*THR_MALLOC)->Alloc(size))) goto error;
    sub_part_info->part_charset_field_array = (Field **)char_ptrs;
    ptr = sub_part_info->part_field_array;
    i = 0;
    while ((field = *(ptr++))) {
      ...
      size = field->pack_length();
      if (!(field_buf = (uchar *)sql_calloc(size))) goto error;
      sub_part_info->part_charset_field_array[i] = field;
      sub_part_info->part_field_buffers[i++] = field_buf;
    }
    sub_part_info->part_charset_field_array[i] = nullptr;
  }
  return false;
error:
...
void partition_info::report_part_expr_error() {
  Item *expr = part_expr;
  DBUG_TRACE;
...
bool partition_info::fix_partition_values(part_elem_value *val,
  ...
  if (col_val->max_value) {
    ...
    if (part_id % num_parts == (num_parts - 1)) {
      // When alter...add subpartition, subpartitions for alter_part_info are
      // not filled.
      if (is_subpart_info() &&
          !get_parent()->partition.subpartitions.is_empty())
        get_parent()
            ->partition.subpartitions[part_id / num_parts]
...
bool partition_info::fix_parser_data(THD *thd) {
  ...
  if (is_sub_partitioned() && sub_part_info->fix_parser_data(thd)) return true;
...
bool partition_info::has_same_partitioning(partition_info *new_part_info) {
  ...
  */
  ...
  if (part_type != partition_type::HASH) {
    ...
    */
    if (!is_sub_partitioned() || !new_part_info->is_sub_partitioned() ||
        column_list || new_part_info->column_list ||
        !sub_part_info->list_of_part_fields ||
        !new_part_info->sub_part_info->list_of_part_fields ||
        new_part_info->sub_part_info->num_parts != sub_part_info->num_parts ||
        new_part_info->sub_part_info->part_field_list.elements !=
            sub_part_info->part_field_list.elements ||
        new_part_info->sub_part_info->use_default_partitions !=
            sub_part_info->use_default_partitions)
      return false;
  } else {
  ...
  if (is_sub_partitioned()) {
    ...
    List_iterator<char> old_field_name_it(sub_part_info->part_field_list);
    List_iterator<char> new_field_name_it(
        new_part_info->sub_part_info->part_field_list);
    char *old_name, *new_name;
    while ((old_name = old_field_name_it++)) {
      new_name = new_field_name_it++;
      if (!new_name || my_strcasecmp(system_charset_info, new_name, old_name))
      ...
      */
      ...
      if (is_sub_partitioned()) {
        ...
        */
        ...
        if (!sub_part_info->use_default_partitions) {
...
bool partition_info::init_partition_bitmap(MY_BITMAP *bitmap,
  ...
  uint32 *bitmap_buf;
  uint bitmap_bits =
      get_num_subparts() ? (get_num_subparts() * num_parts) : num_parts;
...
uint partition_info::get_tot_partitions() const {
  return num_parts * (is_sub_partitioned() ? sub_part_info->num_parts : 1);
}

uint partition_info::get_num_subparts() const {
  if (!sub_part_info) return 0;
  return sub_part_info->num_parts;
}

uint partition_info::get_subpart_func_len() const {
  if (!sub_part_info) return 0;
  return sub_part_info->part_func_len;
}

Item *partition_info::get_subpart_expr() {
  if (!sub_part_info) return nullptr;
  return sub_part_info->part_expr;
}

bool partition_info::get_use_default_subparts() const {
  if (!sub_part_info) return true;
  return sub_part_info->use_default_partitions;
}

bool partition_info::get_use_default_num_subparts() const {
  if (!sub_part_info) return true;
  return sub_part_info->use_default_num_partitions;
}

bool partition_info::is_sub_partitioned() const { return has_sub_part; }

void partition_info::fix_subs_val_array_pointers() {
  assert(is_sub_partitioned());

  uint i = 0;
  if (partition.subpartitions.is_empty()) return;
  List_iterator<Sub_partition> it(partition.subpartitions);
  do {
    Sub_partition *sub_part = it++;
    // Here we need reset index because Alter table might mess the part_id
    // during remove and append subpartition operation.
    sub_part->index = i;
    sub_part->fix_val_array_pointer();
  } while (++i < num_parts);
}

void Sub_partition::fix_val_array_pointer() {
  uint num_cols = 1;
  if (part_info->column_list) num_cols = part_info->part_field_list.elements;
  uint offset = index * part_info->num_parts * num_cols;
  if (part_info->part_type == partition_type::RANGE) {
    if (part_info->column_list)
      range_col_array = part_info->range_col_array + offset;
    else
...
Sub_partition *Sub_partition::clone(THD *thd, uint part_id, bool reset) {
  ...
  if (!clone || DBUG_EVALUATE_IF("sub_partition_clone_failed", true, false)) {
    mem_alloc_error(sizeof(Sub_partition));
    return nullptr;
  }

  if (reset) {
    range_int_array = nullptr;
    list_array = nullptr;
    range_col_array = nullptr;
    list_col_array = nullptr;
    num_list_values = 0;
    has_null_value = false;
    has_null_part_id = 0;
    index = part_id;
...
void sub_partition_info::fix_val_array_pointer(int sub_id) {
  Sub_partition *sub_part = m_parent->partition.subpartitions[sub_id];
  range_int_array = sub_part->range_int_array;
  list_array = sub_part->list_array;
  range_col_array = sub_part->range_col_array;
  list_col_array = sub_part->list_col_array;
  defined_max_value = sub_part->defined_max_value;
  if (part_type == partition_type::LIST) {
    num_list_values = sub_part->num_list_values;
    has_null_value = sub_part->has_null_value;
    has_null_part_id = sub_part->has_null_part_id;
  }
}

/*
  1. If rds_extended_partitions_enabled = 0,
  - partition tables are valid, including range / list / key / hash.
  - some subpartition tables are valid, including range-hash / range-key /
      list-key / list-hash
  - some subpartition tables are invalid, including
      key-key/key-hash/key-range/key-list/
      hash-hash/hash-key/hash-range/hash-list

  2. If rds_extended_partitions_enabled = 1,
  - partition table are valid, incluing range / list / key / hash.
  - all supartition tables are valid, including
      range-hash/range-key
      list-key/list-hash
      key-key/key-hash/key-range/key-list
      hash-hash/hash-key/hash-range/hash-list
*/

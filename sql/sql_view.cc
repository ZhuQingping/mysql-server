...
bool mysql_create_view(THD *thd, Table_ref *views, enum_view_create_mode mode) {
  ...
  */
  ...
  if ((mysql_bin_log.is_open() &&
    ...
    if (thd->set_ddl_sql(buff.lex_cstring())) {
      LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
             "Failed to set DDL info during VIEW creation: memory allocation "
             "failed.");
      goto err_with_rollback;
    }
...
/**
  If the related list(index_hints/outline_index_hints) for storing index hints
  in the base table is not created, the related list in the view will be used.
  Otherwise, it traverses view's related list and injects hint into the base
  table.

  @param[in] view_ref point to view's TABLE_LIST
  @param[in/out] thd transfer index hint to the list related to the base table
  in it.
*/
static void move_index_hint_from_view_to_base_table(Table_ref *view_ref,
                                                    THD *thd) {
  if (view_ref->index_hints && view_ref->index_hints->size() > 0) {
    if (!thd->lex->query_tables->index_hints) {
      std::swap(thd->lex->query_tables->index_hints, view_ref->index_hints);
    } else {
      List_iterator<Index_hint> iter(*view_ref->index_hints);
      Index_hint *hint = nullptr;
      while ((hint = iter++)) {
        thd->lex->query_tables->index_hints->push_back(hint);
      }
      view_ref->index_hints->clear();
    }
  }

  if (view_ref->outline_index_hints &&
      view_ref->outline_index_hints->size() > 0) {
    if (!thd->lex->query_tables->outline_index_hints) {
      std::swap(thd->lex->query_tables->outline_index_hints,
                view_ref->outline_index_hints);
    } else {
      List_iterator<Index_hint> iter(*view_ref->outline_index_hints);
      Index_hint *hint = nullptr;
      while ((hint = iter++)) {
        thd->lex->query_tables->outline_index_hints->push_back(hint);
      }
      view_ref->outline_index_hints->clear();
    }
  }
}
...
bool parse_view_definition(THD *thd, Table_ref *view_ref) {
  ...
  */
  ...
  /* When hint_work_for_view=on, index_hint added to a view
  (drived from single table) manually or using statement outline
  can be moved to the original table. The 'next_global' is used to
  determine whether a view is drived from single table. */
  if (hint_work_for_view && thd->lex->query_tables &&
      !thd->lex->query_tables->next_global) {
    move_index_hint_from_view_to_base_table(view_ref, thd);
  }

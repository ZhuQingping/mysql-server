...
os_thread_id_t get_cur_os_thread_id() {
#ifdef _WIN32
...
int log_sink_trad(void *instance [[maybe_unused]], log_line *ll) {
  ...
  if (ll->count > 0) {
...

    {
      ...
      */
      len = snprintf(buff_line, buff_size,
                     "%.*s %u %llu [%.*s] [MY-%06u] [%.*s] %.*s", (int)iso_len,
                     iso_timestamp, thread_id,
                     (ulonglong)get_cur_os_thread_id(), (int)label_len, label,
                     errcode, (int)subsys_len, subsys, (int)msg_len, msg);

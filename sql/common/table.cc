...
/**
  ...
  @retval         true          Failure
*/
bool open_conf_table(THD *thd, TABLE_REF_PTR &table_list,
  ...
  auto grd = create_scope_guard([&]() { commit_and_close_conf_table(thd); });
...
/**
  Open some configure tables, and check the definition. The caller should fill
  table_list and it's basic name and lock type.

  Report client error if failed.

  @param[in]      thd           Thread context
  @param[in/out]  table_list    CONF tables
  @param[in]      def           Table define

  @retval         false         Success
  @retval         true          Failure
*/
bool open_conf_tables(THD *thd, Table_ref **table_list,
                      const TABLE_FIELD_DEF *def) {
  Conf_table_intact table_intact(thd);
  DBUG_ENTER("open_conf_tables");
  ...
  int index = 0;
  for (Table_ref *table = *table_list; table != nullptr;
       table = table->next_global) {
    /* Check the conf table engine */
    if (!(table->table->file->ht->is_supported_system_table(
            table->db, table->table_name, true))) {
      my_error(ER_UNSUPPORTED_ENGINE, MYF(0),
               ha_resolve_storage_engine_name(table->table->file->ht),
               table->db, table->table_name);
      DBUG_RETURN(true);
    }

    /* Check the conf table definition */
    if (table_intact.check(thd, table->table, def + index)) {
      my_error(ER_CANNOT_LOAD_FROM_TABLE_V2, MYF(0), table->get_db_name(),
               table->table_name);
      DBUG_RETURN(true);
    }
...
void log_conf_error(int nr, Conf_error err) {
  switch (err) {
    ...
    case Conf_error::CONF_NOT_FOUND:
...
Conf_records *Conf_reader::read_all_rows(Conf_error *error) {
  ...
  *error = Conf_error::CONF_OK;
  ...
  if (!container) {
    *error = Conf_error::CONF_NOT_FOUND;
    DBUG_RETURN(nullptr);
  }

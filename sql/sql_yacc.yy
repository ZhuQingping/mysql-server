...
%token<lexer.keyword> DSTORE_DDL_COMMENT_SYM      1211
%token<lexer.keyword> PRODUCT_VERSION_SYM 1212
...
opt_sub_part:
          %empty { $$= NULL; }
        | SUBPARTITION_SYM BY part_type_def opt_num_subparts
          {
            $$= NEW_PTN PT_sub_partition(@3, $3, $4);
          }
        ;
...
sub_part_list:
          sub_part_definition
          {
            $$= NEW_PTN Mem_root_array<PT_subpart_definition *>(YYMEM_ROOT);
            if ($$ == NULL || $$->push_back($1))
              MYSQL_YYABORT; // OOM
          }
...
sub_part_definition:
          SUBPARTITION_SYM ident_or_text opt_part_values opt_part_options
          {
            $$= NEW_PTN PT_subpart_definition(@1, $2, $3.type, $3.values, @3, $4);
          }
        ;
...
create_table_option:
        ...
        | DSTORE_DDL_COMMENT_SYM opt_equal TEXT_STRING_sys
          {
            if (rds_use_ddl_info_replay && YYTHD->for_ddl_info_replay) {
              $$= NEW_PTN PT_create_dstore_ddl_comment($3);
            } else {
              $$= NEW_PTN PT_create_dstore_ddl_comment(NULL_STR);
            }
          }
        ;
...
common_index_option:
        ...
        | DSTORE_DDL_COMMENT_SYM TEXT_STRING_sys
          {
            if (rds_use_ddl_info_replay && YYTHD->for_ddl_info_replay) {
              $$= NEW_PTN PT_index_dstore_ddl_comment(to_lex_cstring($2));
            } else {
              $$= NEW_PTN PT_index_dstore_ddl_comment(NULL_CSTR);
            }
          }
        ;
...
drop_table_stmt:
          DROP opt_temporary table_or_tables if_exists table_list opt_restrict opt_dstore_ddl
          {
            ...
            if (Select->add_tables(YYTHD, $5, TL_OPTION_UPDATING,
                                   YYPS->m_lock_type, YYPS->m_mdl_type))
              MYSQL_YYABORT;
            if (rds_use_ddl_info_replay && YYTHD->for_ddl_info_replay && $7 == false) {
              MYSQL_YYABORT;
            }
          }
        ;
...
opt_dstore_ddl:
          %empty { $$= false; }
        | DSTORE_DDL_COMMENT_SYM opt_equal TEXT_STRING_sys {
          if (my_strcasecmp(system_charset_info, $3.str, "FOR_DD_REPLAY=1;") == 0) {
            $$= true;
          }
        }
        ;
...
drop_database_stmt:
          DROP DATABASE if_exists ident opt_dstore_ddl
          {
            LEX *lex=Lex;
            lex->sql_command= SQLCOM_DROP_DB;
            lex->drop_if_exists=$3;
            lex->name= $4;
            if (rds_use_ddl_info_replay && YYTHD->for_ddl_info_replay && $5 == false) {
              MYSQL_YYABORT;
            }
          }
        ;
...
show_product_version:
          SHOW PRODUCT_VERSION_SYM
          {
            Lex->sql_command= SQLCOM_SHOW_PRODUCT_VERSION;
          }
        ;

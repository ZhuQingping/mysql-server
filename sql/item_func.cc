 ...
 */
int temporal_rank(enum_field_types type) {
  switch (type) {
    case MYSQL_TYPE_DATETIME:
      return 5;
    case MYSQL_TYPE_TIMESTAMP:
...
static int get_var_with_binlog(THD *thd, enum_sql_command sql_command,
  ...
  */
  ...
  // When a statement is written in row format - logging
  // user variable is un-necessary. It so happens that some statements are
  // statement logged but thd->is_current_stmt_binlog_format_row() will
  // indicates that the logging format is row, cf. THD::binlog_query, the last
  // test case (CREATE EVENT with user variable) in rpl_events illustrate that
  // situation. Because pq is not enabled in such cases (DELETE of all rows, and
  // DDLs),  we check whether thd->is_pq_worker() to circumvent this issue.
  if (thd->is_current_stmt_binlog_format_row() && thd->is_pq_worker()) {
    return 0;
  }
  if (var_entry == nullptr) {
    /*
...
static bool check_table_and_trigger_access(Item **args, bool check_trigger_acl,
  ...
  if (is_hidden_by_rds(schema_name_ptr->ptr(), schema_name_ptr->length(),
...
longlong Item_func_internal_dd_char_length::val_int() {
  ...
  switch (field_type) {
    ...
    case MYSQL_TYPE_MEDIUM_BLOB:
      ...
      break;
    case MYSQL_TYPE_LONG_BLOB:
      field_length = (get_instance_storage_engine_type() == DB_TYPE_DSTORE)
                         ? 1069547496
                         : 4294967295;
      break;
    case MYSQL_TYPE_ENUM:
...
void Item_var_func::init() { current_thd->lex->set_has_user_or_system_var(); }

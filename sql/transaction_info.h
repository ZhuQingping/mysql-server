...
class Transaction_ctx {
  ...
  */
  struct {
    ...
    bool tid_written{false};  // The session not wrote an XID
...
class Ha_trx_info {
  ...
  */
  ...
  void set_ht(handlerton *ht) { m_ht = ht; }
 ...
 */
  ...
  /**
    Move DStore engine to the front of the list.

    This method reorders the linked list to ensure that the DStore storage
    engine (if present) is positioned at the head. This is needed when we
    do DStore DDL commit.
    If DStore is not found in the list or is already at the front, the list
    remains unchanged.
  */
  void move_dstore_to_front();

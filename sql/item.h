...
extern size_t need_extra(Item_sum *, bool);
...
/**
  This class is used to collect depended parameters which are related with
  specified nested query block. Only columns of tables belonging to upper
  level query blocks will be collected.
*/
struct Dependent_item_params {
  int nest_level{0};
  std::set<Field *> parameters;
  Query_block
      *first_select;  ///< First query block inside this query expression
};
...
class Item : public Parse_tree_node {
 ...
 public:
  /// Item clone expression used for parallel query

  /// During resolve/optimize phase, a item may be subsituted by a new one, for
  /// example
  // convert_constant_item()/resolve_const_item(), this point to old item for
  // new item
  Item *origin_item{nullptr};

  // During create tmp table, a item_sum may change its argument to a new
  // item_field, and this points to the original item
  Item *origin_sum_arg{nullptr};
  ...
  // If a field is const during create_tmp_table, this field can skip sending to
  // message queue, we record this to avoid item->const_item() may be changed by
  // ConstIterator.
  bool skip_send_to_mq{false};
  ...
  // If a hidden item has been resolved, then we should mark it to
  // prevent it to be resolved multiple times in GROUP/ORDER/HAVING.
  bool is_resolved_hidden{false};

  /// The origin item that we are cloned from. @See
  ...
  virtual bool replace_with_base_item(uchar *);

  bool replace_with_base_item2(uchar *, Item **);

  virtual bool set_fixed(uchar *) { return false; }

  /// Called when PQ falls back to serial execution.
  virtual bool pq_restore(uchar *) { return false; }
  ...
  */
  ...
  // After fix_fields is modified, check whether refix_fields needs to be
  // modified.
  virtual bool refix_fields(THD *, Item **);
  // If be called in pq processing, it will call refix_fields, otherwise,
  // fix_fields is called.
  bool fix_fields_or_refix_fields(THD *thd, Item **ref) {
    if (thd->is_in_pq_phase()) return refix_fields(thd, ref);
    return fix_fields(thd, ref);
  }
  /**
  ...
  */
  ...
  virtual bool eq_with_binary_cmp_arg(const Item *item, bool binary_cmp) const {
    return eq(item, binary_cmp);
  }
  ...
  */
  ...
  using pq_derived_info_t = struct {
    ...
    /**
      ...
      Table_ref::setup_materialized_derived_tmp_table to create the materialize
      temporary table for the derived table, it calls prepare_query_term to
      recalculate and update the m_nullable attribute of Items involved in set
      operations. To ensure that PQ cloning creates temporary tables based on
      the original m_nullable values (keeping the null_fields count consistent
      with the temporary table corresponding to orig). It is necessary to save
      and restore the m_nullable.
    */
    bool nullable{false};
  };
  std::optional<pq_derived_info_t> m_saved_pq_derived_info;

  inline void save_pq_derived_info() {
    m_saved_pq_derived_info = {marker, m_nullable};
  }

  inline void swap_pq_derived_info() {
    if (m_saved_pq_derived_info.has_value()) {
      std::swap(marker, m_saved_pq_derived_info->marker);
      std::swap(m_nullable, m_saved_pq_derived_info->nullable);
    }
  }
...
class Item_sp_variable : public Item {
 ...
 public:
  ...
  // never be called.
  bool refix_fields(THD *, Item **) override;
...
class Item_field : public Item_ident {
 ...
 protected:
  ...
  // It's only used for Item_field::refix_fields.
  void set_field_for_pq(Field *field);
  ...
  */
  ...
  bool eq_with_binary_cmp_arg(const Item *item, bool binary_cmp) const override;
  double val_real() override;
  longlong val_int() override;
  ...
  bool fix_fields(THD *, Item **) override;
  bool refix_fields(THD *, Item **reference) override;
  void make_field(Send_field *tmp_field) override;
  ...
  */
  ...
  size_t pq_extra_len(bool group) override {
    return field->item_sum_ref ? need_extra(field->item_sum_ref, group) : 0;
  }

  bool set_fixed(uchar *status) override {
    fixed = *status;
    return false;
  }
...
class Item_int : public Item_num {
 ...
 public:
  ...
  bool pq_copy_from(THD *, Query_block *, Item *) override;
};
...
class Item_ref : public Item_ident {
  ...
  */
  ...
  void set_ref_pointer(Item **ptr) { m_ref_item = ptr; }
  ...
  bool eq(const Item *item, bool binary_cmp) const override {
    ...
    // ref[0] may also be a ref item. This is caused by the clone
    // of Item_ref. For example: ref[0] may be a Item_view_ref.
    // May search for a referenced item that is not yet resolved:
    if (m_ref_item == nullptr) return false;
    return ref_item()->real_item()->eq(it, binary_cmp);
  }
  double val_real() override;
  longlong val_int() override;
  longlong val_time_temporal() override;
...
class Item_view_ref final : public Item_ref {
 ...
 public:
  Item_view_ref(Name_resolution_context *context_arg, Item **item,
                ...
                const char *table_name_arg, const char *field_name_arg,
                Table_ref *tl, Table_ref *inner_table = nullptr)
  ...
  */
  table_map used_tables() const override {
    ...
    auto first_inner_table_of_most_inner_view_ref = first_inner_table;
    /*
    ...
    */
    while (true) {
      ...
      if (inner_ref->ref_type() == VIEW_REF) {
        auto first_inner_table_of_inner_view_ref =
            down_cast<const Item_view_ref *>(inner_ref)->first_inner_table;
        if (first_inner_table_of_inner_view_ref != nullptr)
          first_inner_table_of_most_inner_view_ref =
              first_inner_table_of_inner_view_ref;
      }
    }
    ...
    */
    if (!inner_item->const_for_execution() ||
        first_inner_table_of_most_inner_view_ref == nullptr) {
    ...
    /*
      ...
      Regarding the "augment" case, here are more details. If we have:
      SELECT derived.a FROM t1 LEFT JOIN (SELECT 1 AS a FROM t2) AS derived
                            ON FALSE;
      and merge it as:
      SELECT Y FROM t1 LEFT JOIN t2 ON FALSE;

      with Y being an Item_view_ref (=derived.a) wrapping the constant 1, then
      'this' is Y and inner_item is '1', and this->first_inner_table is t2. In
      ...
      NULL properly and not 1. That it why this->first_inner_table (t2) must
      be included in used_tables(). However, we can have a more "nested"
      situation:
      ...
      and then, derived2 is not part of any left join, and thus has a
      first_inner_table equal to nullptr. In this case, we have to reach to
      first_inner_table of the underlying item of derived2.a, which is
      derived.a. If we do not do that, we'll miss the dependency on t2. This
      is why we introduced first_inner_table_of_most_inner_view_ref. Note that
      val*() functions do not need to dive to most inner Item_view_ref like we
      do here, because they call val*() on direct-child item this->ref_item()
      (derived.a in our example), so the first_inner_table member of the most
      inner Item_view_ref will be used in the end after several recursive
      calls. If this implementation of used_tables() did like val*() does,
      i.e. called this->ref_item()->used_tables() (crucial difference with
      current code: calling used_tables() on the the direct child
    ...
    */
    return field != nullptr
               ? field->table_ref->map()
               : inner_item->used_tables() |
                     first_inner_table_of_most_inner_view_ref->map();
  }
...
class Item_outer_ref final : public Item_ref {
  ...
  */
  ...
  // never be called because OUTER_REF not support. ses pq_not_support_ref
  // function.
  bool refix_fields(THD *, Item **) override;
  table_map used_tables() const override {
...
class Item_cache : public Item_basic_constant {
 ...
 public:
  ...
  bool eq(const Item *item, bool) const override {
    ...
    // for cloned cache item, we cannot directly compare their address because
    // we also clone their example items. If their cached example is not equal,
    // they must not be equal. Otherwise, they are considered as equal.
    // The reason is that all subclass of Item_cache are inited with no more
    // members except for "example" item.
    if (!item || item->type() != Item::CACHE_ITEM) return false;
    Item_cache *cache_item = down_cast<Item_cache *>(const_cast<Item *>(item));
    if ((example && !cache_item->example) ||
        (!example && cache_item->example) ||
        (!example->eq(cache_item->example, false))) {
      return false;
    }
...
class Item_cache_datetime : public Item_cache {
 ...
 public:
  ...
  void store_value(Item *item, longlong val_arg);
  void store_value(Item_cache_datetime *orig_item);
  void store(Item *item) override;
  double val_real() override;
  longlong val_int() override;
  longlong val_time_temporal() override;
...
void mark_as_dependent(THD *thd, Query_block *last, Query_block *current,
                       Item_ident *resolved_item, Item_ident *mark_item);

...
bool Exchange::convert_mq_data_to_record(uchar *data, int msg_len,
  ...
  uint size_field = m_recv_items->size();
  assert(size_field >= m_table->s->fields);
   ...
   */
  if (DBUG_EVALUATE_IF("pq_worker_error10", true, false) ||
      msg_len >
  ...
  Item *item = nullptr;
  int j;
  for (auto it = m_recv_items->begin(); it != m_recv_items->end(); it++) {
    item = *it;
    ...
    if (!const_item && !null_field) { /** case of (0, 0) */
      item_field->set_notnull();
    } else if (!const_item && null_field) { /** case of (0, 1) */
      item_field->set_null();
    } else { /** case of (1, *) */
      if (m_first_record) {
        item->null_value = null_field;
      } else {
        item->null_value &= null_field;
      }
    }
...
bool Exchange_nosort::get_next(void **datap, uint32 *m_len, bool &done,
                               bool &first_readdone) {
  MQ_RESULT result;
  done = first_readdone = false;
  MQueue_handle *reader = get_mq_handle(m_next_queue);
  if (reader->has_readdone()) {
    done = true;
    return false;
  }
...
bool Exchange_nosort::read_next(void **datap, uint32 *m_len) {
  bool readerdone;
  int nvisited = 0;
  bool read_result, first_readdone;
  THD *thd = get_thd();
  ...
  while (!thd->is_killed() && !thd->pq_error) {
    ...
    if (readerdone) {
      ...
      /** #active readers will decrease by one only for the first read_done */
      if (first_readdone) {
        m_active_readers--;
      }

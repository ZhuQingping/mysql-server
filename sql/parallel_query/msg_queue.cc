...
MQ_RESULT MQueue_handle::send_bytes(uint32 nbytes, const void *data,
  ...
  if (thd->is_pq_error()) {
    set_detached_status(MQ_HAVE_DETACHED);
    return MQ_DETACHED;
  }
 ...
 */
MQ_RESULT MQueue_handle::send(void *data, uint32 len, bool nowait) {
  MQ_RESULT res;
  uint32 nbytes = len;
  uint32 written;
...
MQ_RESULT MQueue_handle::send(Field_raw_data *fm) {
  ...
  if (MQ_SUCCESS == res) {
    if (fm->m_len == PTR_SIGN) {
      res = send_bytes(sizeof(fm->m_ptr), &fm->m_ptr, &written);
    } else {
      res = send_bytes(fm->m_len, fm->m_ptr, &written);
    }
    assert((res == MQ_SUCCESS &&
            (written == fm->m_len ||
             (fm->m_len == PTR_SIGN && written == sizeof(fm->m_ptr)))) ||
           res == MQ_DETACHED ||
           (!written && m_queue->detached == MQ_TMP_DETACHED));
  }
  return res;
...
bool Batch_buffer::write(uchar *ptr, size_t len) {
  if (m_total_length + len <= m_max_memory_buffer) {
    memcpy(m_buffer + m_total_length, ptr, len);
    m_total_length += len;
    return false;
  }

  if (!m_using_cache) {
    if (my_b_inited(&m_cache)) {
      close_cached_file(&m_cache);
    }

    m_cache.type = WRITE_CACHE;
    if (open_cached_file(&m_cache, mysql_tmpdir, TEMP_PREFIX, DISK_BUFFER_SIZE,
                         MYF(MY_WME)))
      return true;

    if (my_b_safe_write(&m_cache, m_buffer, m_total_length)) {
      return true;
    }
    m_using_cache = true;
  }
  ...
  m_total_length += len;
  return my_b_safe_write(&m_cache, ptr, len);
}

int Batch_buffer::read(uchar *ptr, size_t len) {
  if (m_read_cur + len > m_total_length) {
    return 0;
  }

  if (m_total_length <= m_max_memory_buffer) {
    memcpy(ptr, m_buffer + m_read_cur, len);
    m_read_cur += len;
    return len;
  }

  if (my_b_read(&m_cache, ptr, len)) {
    if (m_cache.error != -1) {
      m_read_cur += m_cache.error;
    }
    return m_cache.error;
  }
  m_read_cur += len;
  return len;
}

void Batch_buffer::release() { m_slot->release_buffer(); }

Batch_buffer *Batch_buffer_slot::get_next_buffer() {
  if (m_all_buf.size() <= buf_used) {
    Batch_buffer *new_buffer = nullptr;
    if (!DBUG_EVALUATE_IF("bat_buf_is_null", true, false)) {
      new_buffer = new (std::nothrow) Batch_buffer(m_max_mem_size, this);
    }
    if (!new_buffer) goto err;
    m_all_buf.push_back(new_buffer);
  }
  assert(buf_used <= m_all_buf.size());
  return m_all_buf[buf_used++];
err:
  sql_print_error("alloc space in Batch_buffer_slot::get_next_buffer error");
  return nullptr;
}

void Batch_buffer_slot::release_buffer() {
  assert(buf_used > 0);
  buf_used--;
  if (buf_used == 0) {
    m_manager->release_slot(this);
  }
}

Batch_buffer_slot *Batch_buffer_manager::get_next_slot() {
  Batch_buffer_slot *slot = nullptr;
  mysql_mutex_lock(&m_mutex);
  while (slot == nullptr) {
    if (!m_slot_free.empty()) {
      slot = m_slot_free.top();
      m_slot_free.pop();
    } else if (m_max_slot > m_all_slot.size()) {
      slot = new (std::nothrow) Batch_buffer_slot(m_max_mem_size, this);
      if (slot) {
        m_all_slot.push_back(slot);
      } else {
        sql_print_error(
            "alloc space in Batch_buffer_manager::get_next_slot error");
      }
    } else {
...
void Batch_buffer_manager::release_slot(Batch_buffer_slot *slot) {
  mysql_mutex_lock(&m_mutex);
  m_slot_free.push(slot);

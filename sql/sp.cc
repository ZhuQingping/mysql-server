...
static bool sp_binlog_create_routine_stmt(THD *thd, sp_head *sp,
  ...
  DBUG_EXECUTE_IF("create_routine_set_ddl_sql_error", { log_query.length(0); });
  if (thd->set_ddl_sql(log_query.lex_cstring())) {
    LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
           "Failed to set DDL info during ROUTINE create.");
    return true;
  }
...
bool sp_create_routine(THD *thd, sp_head *sp, const LEX_USER *definer,
  ...
  if (sp_already_exists) {
    ...
    */
    if ((mysql_bin_log.is_open() || need_ddl_info_sql(thd)) &&

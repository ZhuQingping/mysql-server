...
#include "sql/sql_plan_cache.h"  // emulates_hint
...
struct st_opt_hint_info opt_hint_info[] = {
    ...
    {"OFFSET_PUSHDOWN", false, false, false},
...
void Opt_hints::append_hint_type(String *str, opt_hints_enum type) {
  ...
  if (!hints_map.switch_on(type) && type != PQ_HINT_ENUM)
...
PT_hint *Opt_hints_qb::get_complex_hints(opt_hints_enum type) {
  ...
  if (type == OFFSET_PUSHDOWN_HINT_ENUM) return offset_pushdown_hint;
...
bool Opt_hints_qb::apply_offset_pushdown_hint() {
  return (offset_pushdown_hint && offset_pushdown_hint->switch_on());
}
...
void Opt_hints_qb::apply_ptrc_hints(JOIN *join, table_map *force_tab_map,
  ...
  for (uint hint_idx = 0; hint_idx < ptrc_hints.size(); hint_idx++) {
    ...
    if (hint->type() == PRC_SUBQUERY_HINT_ENUM) {
      if (!hint_table_list->empty()) {  // PRC_SUBQUERY is about caching the
                                        // subquery's result, specifying
        // tables makes non sense.
        push_warning_printf(thd, Sql_condition::SL_WARNING,
                            ER_WARN_INVALID_HINT,
                            ER_THD(thd, ER_WARN_INVALID_HINT), "PRC_SUBQUERY");
        return;
      }
      if (hint->switch_on())
      ...
      DBUG_ASSERT(hint->type() == PRC_JOIN_HINT_ENUM);
      ...
      for (const Hint_param_table *hint_table = hint_table_list->begin();
        ...
        for (uint i = 0; i < join->tables; i++) {
          ...
          if (table && !compare_table_name(hint_table, table)) {
            hint_table_found = true;
    ...
    if (*force_tab_map & *ignore_tab_map) {
      // Clear hint influence
      *force_tab_map = *ignore_tab_map = 0;
      push_warning_printf(thd, Sql_condition::SL_WARNING,
...
bool hint_table_state(const THD *thd, const Table_ref *table_list,
  ...
  return thd->optimizer_switch_flag(optimizer_switch) ||
         plan_cache::emulates_hint(thd, type_arg);
}

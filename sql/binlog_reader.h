...
class Basic_binlog_file_reader : public IBasic_binlog_file_reader {
  ...
  */
  bool open(const char *file_name, my_off_t offset = 0,
            ...
            bool force_read_fd = false) {
    DBUG_TRACE;
    if (m_ifile.open(file_name)) return true;

    Format_description_log_event *fd = read_fdle(offset, force_read_fd);
  ...
  */
  ...
  ssize_t read(unsigned char *buffer, size_t length) {
    return m_ifile.read(buffer, length);
  }
 ...
 private:
  ...
  /**
     ...
     @param[in] force_read_fd Force to read first format description event.
  ...
  */
  Format_description_log_event *read_fdle(my_off_t offset,
    ...
    */
    while (position() < offset || force_read_fd) {
      m_event_start_pos = position();

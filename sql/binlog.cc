...
my_thread_handle purge_thread_id;
static bool terminate_purge = false;
static bool should_purge = false;
static uint32 max_consecutive_reset_numbers = 60;
mysql_mutex_t LOCK_purge_binlog;
mysql_cond_t COND_purge_binlog;
...
char dstore_missed_gtids_on_binlog[FN_REFLEN];
char *rds_dstore_missed_gtids_on_binlog_ptr = nullptr;
...
/**
  Forces the thread to wait `binlog_group_commit_sync_delay`, if set.
 */
void process_sync_stage_delay();
}  // namespace
...
*/
static void deinit_purge_binlog_thd(THD *thd) {
  DBUG_TRACE;
...
*/
static void init_purge_binlog_thd(THD **p_thd) {
  DBUG_TRACE;
  THD *thd = *p_thd;
  thd->thread_stack = reinterpret_cast<char *>(p_thd);
  thd->set_command(COM_DAEMON);
  thd->security_context()->skip_grants();
  thd->system_thread = SYSTEM_THREAD_PURGE_EXPIRED_BINLOG;
  thd->store_globals();
  thd->set_time();
}

/**
  The main function of the binlog purge thread.
  - purge expiration binlogs when get a binlog purge signal

  @param p_thd  Pointer to pointer to thread structure

  @return
    @retval nullptr  OK. always, the purge binlog thread will swallow any error
                     for going to wait for next binlog purge signal until
                     it is terminated.
*/
void *binlog_purge_thread(void *p_thd) {
  DBUG_TRACE;
  struct timespec abstime;
  THD *thd = static_cast<THD *>(p_thd);
  mysql_thread_set_psi_id(thd->thread_id());
  my_thread_init();
  mysql_mutex_lock(&LOCK_purge_binlog);
  rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_INITIALIZED;
  terminate_purge = false;
  mysql_mutex_unlock(&LOCK_purge_binlog);
  {
    init_purge_binlog_thd(&thd);
    thd->set_skip_readonly_check();
    for (;;) {
      // Absolute system time at which the sleep time expires.
      set_timespec(&abstime, rds_binlog_purge_thread_wait_timeout);

      mysql_mutex_lock(&LOCK_purge_binlog);
      if (terminate_purge) break;
      THD_ENTER_COND(thd, &COND_purge_binlog, &LOCK_purge_binlog,
                     &stage_suspending, nullptr);

      while (!(should_purge || terminate_purge)) {
        rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_SLEEPING;
        int ret = mysql_cond_timedwait(&COND_purge_binlog, &LOCK_purge_binlog,
                                       &abstime);
        rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_AWAKENED;
        // After waiting timeout, this thread will purge expired
        // binary logs automatically on master.
      ...
      if (opt_bin_log) mysql_bin_log.auto_purge();
    }
    mysql_mutex_unlock(&LOCK_purge_binlog);
    thd->reset_skip_readonly_check();
    deinit_purge_binlog_thd(thd);
  }
  my_thread_end();
  mysql_mutex_lock(&LOCK_purge_binlog);
  rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_EXIT;
  mysql_mutex_unlock(&LOCK_purge_binlog);
...
/**
  Get stable statue of the binlog purge thread.

  @retval stable statue.
*/
ulong get_stable_statue_of_binlog_purge_thread() {
  int max = 20;
  mysql_mutex_lock(&LOCK_purge_binlog);
  ulong status = rds_binlog_purge_thread_status;
  mysql_mutex_unlock(&LOCK_purge_binlog);
  /*
    BINLOG_PURGE_THEAD_STATUS_RESET is a transient status of the
    binlog purge thread, the rds_binlog_purge_thread_status
    shall be stable in twenty retries.
  */
  while (status == BINLOG_PURGE_THEAD_STATUS_RESET) {
    // sleep 1 millisecond
    my_sleep(1000);
    mysql_mutex_lock(&LOCK_purge_binlog);
    status = rds_binlog_purge_thread_status;
    mysql_mutex_unlock(&LOCK_purge_binlog);
    max--;
    if (max <= 0) break;
  }
...
/**
  Create a thread to purge expired binary logs.
*/
void create_binlog_purge_thread() {
  my_thread_attr_t attr;
  int error = 0;
  THD *thd;

  ulong status = get_stable_statue_of_binlog_purge_thread();
  if (status > BINLOG_PURGE_THEAD_STATUS_RESET) return;

  mysql_mutex_lock(&LOCK_purge_binlog);
  rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_INITIALIZING;
  mysql_mutex_unlock(&LOCK_purge_binlog);

  if (!(thd = new THD)) {
    LogErr(ERROR_LEVEL, ER_BINLOG_FAILED_TO_PURGE_THREAD_OOM,
           debug_info_prefix);
    mysql_mutex_lock(&LOCK_purge_binlog);
    rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_NOT_CREATED;
    mysql_mutex_unlock(&LOCK_purge_binlog);
    return;
  }
  ...
  if (my_thread_attr_init(&attr)) {
    LogErr(ERROR_LEVEL, ER_BINLOG_FAILED_TO_INIT_THREAD_ATTR_FOR_PURGE,
           debug_info_prefix);
    delete thd;
    mysql_mutex_lock(&LOCK_purge_binlog);
    rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_NOT_CREATED;
  ...
  if (DBUG_EVALUATE_IF("simulate_create_purge_thread_failure", error = 1, 0) ||
#ifndef _WIN32
      (error = pthread_attr_setscope(&attr, PTHREAD_SCOPE_SYSTEM)) ||
#endif
      (error = mysql_thread_create(key_thread_purge_binlog, &purge_thread_id,
                                   &attr, binlog_purge_thread,
                                   static_cast<void *>(thd)))) {
    LogErr(ERROR_LEVEL, ER_BINLOG_FAILED_TO_CREATE_PURGE_THREAD,
           debug_info_prefix, error);
    delete thd;
    mysql_mutex_lock(&LOCK_purge_binlog);
    rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_NOT_CREATED;
    DBUG_EXECUTE_IF("simulate_create_purge_thread_failure", {
      rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_INITIALIZING;
    });
    mysql_mutex_unlock(&LOCK_purge_binlog);
  }
...
class binlog_cache_mngr {
  ...
  */
  ...
  uchar *get_binlog_write_redo_buffer() {
    if (binlog_write_redo_buffer == nullptr) {
      binlog_write_redo_buffer =
          (uchar *)my_malloc(key_memory_binlog_cache_mngr,
                             binlog_write_redo_buffer_size, MYF(MY_WME));
    }
    return binlog_write_redo_buffer;
  }
  void free_binlog_write_redo_buffer() {
    if (binlog_write_redo_buffer != nullptr) {
      my_free(binlog_write_redo_buffer);
      binlog_write_redo_buffer = nullptr;
    }
  }

 private:
  uchar *binlog_write_redo_buffer{nullptr};
...
class Binlog_event_writer : public Basic_ostream {
  ...
  */
  ...
  virtual void write_record(THD *) {}
  virtual void update_flags(uint8_t) {}
};

/*
This class use binlog write into wal, binlog event will first write into buffer,
and finally write into wal.
*/
class Binlog_write_into_redo : public Binlog_event_writer {
 private:
  uint32 position{0};
  uchar *write_buffer{nullptr};

 public:
  Binlog_write_into_redo(MYSQL_BIN_LOG::Binlog_ofile *binlog_file, uchar *buf)
      : Binlog_event_writer(binlog_file), write_buffer(buf) {}

  /**
    write binlog event into write_buffer first.
    @param buffer  binlog event buffer
    @param length  size of buffer
    @return true if the write operation failed (e.g., buffer overflow),
          false if the write was successful.
  */
  bool write(const unsigned char *buffer, my_off_t length) override {
    if (position + length > (my_off_t)binlog_write_redo_buffer_size) {
      LogErr(WARNING_LEVEL, ER_FAILED_TO_WRITE_TO_FILE,
             "binlog event size over size 63k.");
      return false;
    }
    memcpy(write_buffer + position, buffer, length);
    position += length;
    return false;
  }

  /**
    write binlog event into wal and reset buffer position.
  */
  void write_record(THD *thd) override {
    std::stringstream log_info;
    if (position > (my_off_t)binlog_write_redo_buffer_size || position == 0) {
      log_info << "binlog event size over size 63k, gno is "
               << uint8korr(write_buffer + 1);
      LogErr(WARNING_LEVEL, ER_CDE_DEFAULT_LOG_ERROR, log_info.str().c_str());
      return;
    }
    ha_commit_wals(thd, write_buffer, position);
    position = 0;
  }

  /**
    Update buffer flags, such as generating update flags for DDL information.
  */
  void update_flags(uint8_t flags) override { write_buffer[0] |= flags; }
};
...
static int binlog_close_connection(handlerton *, THD *thd) {
  ...
  cache_mngr->free_binlog_write_redo_buffer();
...
bool MYSQL_BIN_LOG::write_transaction(THD *thd, binlog_cache_data *cache_data,
  ...
  */
  ...
  if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
    ...
    thd->set_gtid_info(gtid_event.get_sid()->bytes, gtid_event.get_gno());
  }

  bool ret = true;
  bool is_less_than_buffer_size =
      gtid_event.transaction_length + gtid_event.get_event_length() <
              (my_off_t)binlog_write_redo_buffer_size
          ? true
          : false;
  if (!rds_write_binlog_into_redo || is_less_than_buffer_size) {
  ...
  */
  ...
  } else if (rds_write_binlog_into_redo && is_less_than_buffer_size) {
    uchar buf[4];
    int4store(buf, cache_data->get_byte_position());
    // Write the length of the log events of the trx into the buffer.
    writer->write((uchar *)buf, sizeof(buf));
    // Write the content of the log events of the trx into the buffer.
    ret = mysql_bin_log.write_cache(thd, cache_data, writer);
  }
  // for ddl transaction, we need write ddl_info into redo
  if (need_ddl_info(thd) && rds_write_binlog_into_redo) {
    ddl_info_to_redo(thd, writer);
  }
  ...
  if (!ret && rds_write_binlog_into_redo) {
    if (is_less_than_buffer_size) {
      writer->write_record(thd);
    } else {
      std::stringstream log_info;
      log_info << "binlog event size over size 63k, gno is "
               << gtid_event.get_gno();
  ...
  if (flags.finalized) {
    ...
    */
    ...
    /*
      ...
      correct.
    */
    std::unique_ptr<Binlog_event_writer> writer{nullptr};
    ...
    if (!error) {
      if (!rds_write_binlog_into_redo) {
        writer = std::make_unique<Binlog_event_writer>(
            mysql_bin_log.get_binlog_file());
      } else {
        writer = std::make_unique<Binlog_write_into_redo>(
            mysql_bin_log.get_binlog_file(),
            m_cache_mngr.get_binlog_write_redo_buffer());
      }
      if ((error = mysql_bin_log.write_transaction(thd, this, writer.get(),
...
*/
static int binlog_commit(handlerton *, THD *, bool, bool) {
  DBUG_TRACE;
  /*
...
int MYSQL_BIN_LOG::rollback(THD *thd, bool all) {
  ...
  */
  ...
  if ((error == 0 && stuff_logged) || cache_mngr->has_incident()) {
    ...
    if (unlikely(rds_binlog_enable_group_replication)) {
      if (RUN_HOOK(
    ...
    if (rds_write_binlog_into_redo) {
      if (rds_binlog_concurrent_commit) {
        error = concurrent_commit(thd, all, /* skip_commit */ true);
      } else {
        error = group_commit(thd, all, /* skip_commit */ true);
      }
    } else if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
               (opt_binlog_order_commits ||
                Clone_handler::need_commit_order())) {
      error = single_commit(thd, all, /* skip_commit */ true);
    } else {
...
void MYSQL_BIN_LOG::cleanup() {
  ...
  if (inited) {
    ...
    mysql_mutex_destroy(&LOCK_redo);
    mysql_mutex_destroy(&LOCK_sync);
    mysql_mutex_destroy(&LOCK_binlog_end_pos);
    mysql_mutex_destroy(&LOCK_xids);
    mysql_mutex_destroy(&LOCK_tids);
...
void MYSQL_BIN_LOG::init_pthread_objects() {
  ...
  mysql_mutex_init(m_key_LOCK_after_commit, &LOCK_after_commit,
                   MY_MUTEX_INIT_FAST);
  mysql_mutex_init(m_key_LOCK_redo, &LOCK_redo, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(m_key_LOCK_sync, &LOCK_sync, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(m_key_LOCK_binlog_end_pos, &LOCK_binlog_end_pos,
                   MY_MUTEX_INIT_FAST);
  mysql_mutex_init(m_key_LOCK_xids, &LOCK_xids, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(m_key_LOCK_tids, &LOCK_tids, MY_MUTEX_INIT_FAST);
...
void MYSQL_BIN_LOG::find_and_set_log_located_by_dstore_missed_gtids(
    const Gtid_set &missed_gtids) {
  DBUG_TRACE;
  LOG_INFO linfo;
  auto log_index = this->get_log_index();
  std::list<std::string> filename_list = log_index.second;
  int error = log_index.first;
  list<string>::reverse_iterator rit;
  Gtid_set binlog_previous_gtid_set{missed_gtids.get_sid_map()};

  if (missed_gtids.is_empty()) goto end;

  if (error != LOG_INFO_EOF) {
    sql_print_warning(
        "Failed to read the binary log index file on setting "
        "rds_dstore_missed_gtids_on_binlog.");
    goto end;
  }

  /*
    Iterate over all the binary logs in reverse order, and read
    only the Previous_gtids_log_event, to find the first one,
    that includes the missed gtids in dstore, then set the
    log name to rds_dstore_missed_gtids_on_binlog. Every
    binary log begins with a Previous_gtids_log_event, that
    includes all GTIDs in all previous binary logs.
  */
  rit = filename_list.rbegin();
  while (rit != filename_list.rend()) {
    binlog_previous_gtid_set.clear();
    ...
    switch (read_gtids_from_binlog(filename, nullptr, &binlog_previous_gtid_set,
      ...
      case ERROR:
        sql_print_warning(
            "Failed to read Previous_gtids_log_event from filename='%s'",
            filename);
        goto end;
      case GOT_GTIDS:
      case GOT_PREVIOUS_GTIDS:
        if (!missed_gtids.is_subset(&binlog_previous_gtid_set)) {
          strmake(dstore_missed_gtids_on_binlog, filename,
                  sizeof(dstore_missed_gtids_on_binlog) - 1);
          rds_dstore_missed_gtids_on_binlog_ptr = dstore_missed_gtids_on_binlog;
          goto end;
        }
      default:
        break;
...
int MYSQL_BIN_LOG::purge_logs_before_date(time_t purge_time, bool auto_purge) {
  ...
  if (thd != nullptr &&
    ...
    rds_binlog_purge_thread_status =
        BINLOG_PURGE_THEAD_STATUS_REQUESTING_LOCK_INDEX;
    mysql_mutex_unlock(&LOCK_purge_binlog);
  }
  mysql_mutex_lock(&LOCK_index);
  if (thd != nullptr &&
    ...
    mysql_mutex_lock(&LOCK_purge_binlog);
    rds_binlog_purge_thread_status =
        BINLOG_PURGE_THEAD_STATUS_PURGING_BINARY_LOGS;
    mysql_mutex_unlock(&LOCK_purge_binlog);
  }
  to_log[0] = 0;
  ...
  while (!(log_is_active = is_active(log_info.log_file_name))) {
    ...
    */
      ...
      no_of_log_files_purged++;
      if (no_of_log_files_purged > rds_max_num_of_binary_logs_purged_per_time &&
          thd != nullptr &&
          thd->system_thread == SYSTEM_THREAD_PURGE_EXPIRED_BINLOG)
        break;
    } else
...
err:
  ...
  if (thd != nullptr &&
    ...
    mysql_mutex_lock(&LOCK_purge_binlog);
    rds_binlog_purge_thread_status =
        BINLOG_PURGE_THEAD_STATUS_RELEASED_LOCK_INDEX;
    mysql_mutex_unlock(&LOCK_purge_binlog);
  }
  return error;
}
...
void MYSQL_BIN_LOG::inc_prep_tids(THD *thd) {
  DBUG_TRACE;
#ifndef NDEBUG
  int result = ++m_atomic_prep_tids;
  DBUG_PRINT("debug", ("m_atomic_prep_tids: %d", result));
#else
  m_atomic_prep_tids++;
#endif
  thd->get_transaction()->m_flags.tid_written = true;
}

void MYSQL_BIN_LOG::dec_prep_tids(THD *thd) {
  DBUG_TRACE;
  int32 result = --m_atomic_prep_tids;
  DBUG_PRINT("debug", ("m_atomic_prep_tids: %d", result));
  thd->get_transaction()->m_flags.tid_written = false;
  if (result == 0) {
    mysql_mutex_lock(&LOCK_tids);
    mysql_cond_signal(&m_prep_tids_cond);
    mysql_mutex_unlock(&LOCK_tids);
  }
}
...
int MYSQL_BIN_LOG::new_file_impl(
  ...
  mysql_mutex_lock(&LOCK_tids);
  while (get_prep_tids() > 0) {
    mysql_cond_wait(&m_prep_tids_cond, &LOCK_tids);
  }
  mysql_mutex_unlock(&LOCK_tids);
  ...
  if (!is_relay_log && !rds_write_binlog_into_redo) {
  ...
  */
  DBUG_EXECUTE_IF("binlog_sleep_before_crash_between_close_and_open",
                  { sleep(5); });
...
int MYSQL_BIN_LOG::rotate(bool force_rotate) {
  ...
  if (DBUG_EVALUATE_IF("force_rotate", 1, 0) || force_rotate ||
      (m_binlog_file->get_real_file_size() >= (my_off_t)max_size) ||
      DBUG_EVALUATE_IF("simulate_max_binlog_size", true, false)) {
    error = new_file_impl(true, nullptr);
  }
  return error;
}
...
bool MYSQL_BIN_LOG::write_cache(THD *thd, binlog_cache_data *cache_data,
  ...
  if (rds_write_binlog_into_redo) {
    if (!rds_binlog_concurrent_commit) mysql_mutex_assert_owner(&LOCK_redo);
  } else {
    mysql_mutex_assert_owner(&LOCK_log);
  }
...
bool MYSQL_BIN_LOG::read_binlog_in_use_flag(
  ...
  if (!(ev->common_header->flags & LOG_EVENT_BINLOG_IN_USE_F ||
      ...
      DBUG_EVALUATE_IF("set_force_bin_log_recovery", true, false)) {
...
int MYSQL_BIN_LOG::prepare(THD *thd, bool all) {
  ...
  */
  if (get_instance_storage_engine_type() != DB_TYPE_DSTORE) {
    ...
    /*
      On dstore, Innodb data dictionary updated by DDL requests
      that thd->durability_property is HA_REGULAR_DURABILITY.
    */
  ...
  */
  if (stmt_stuff_logged || trx_stuff_logged) {
    ...
    */
    ...
    if (rds_binlog_lock_enable && !thd->backup_binlog_lock.is_acquired() &&
    ...
    if (rds_write_binlog_into_redo) {
      if (rds_binlog_concurrent_commit) {
        rc = concurrent_commit(thd, all, skip_commit);
      } else {
        rc = group_commit(thd, all, skip_commit);
      }

    } else if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
               (opt_binlog_order_commits ||
                Clone_handler::need_commit_order())) {
      rc = single_commit(thd, all, skip_commit);
    } else {
      rc = ordered_commit(thd, all, skip_commit);
    }

    DEBUG_SYNC(thd, "binlog_protection_acquired");
...
int MYSQL_BIN_LOG::process_flush_stage_queue(my_off_t *total_bytes_var,
  ...
  int flush_error = 1;
  if (rds_write_binlog_into_redo) {
    mysql_mutex_assert_owner(&LOCK_redo);
  } else {
    mysql_mutex_assert_owner(&LOCK_log);
  }
  ...
  /*
    Flush thread caches to binary log if rds_write_binlog_into_redo is false.
    Otherwise flush thread caches to redo log.
  */
  for (THD *head = first_seen; head; head = head->next_to_commit) {
  ...
  *total_bytes_var = total_bytes;
  ...
  DBUG_EXECUTE_IF("flush_queue_wait_for_more_trxs", assert(no_flushes == 16););
#endif
  return flush_error;
}
  ...
  */
  ...
  for (THD *head = first; head; head = head->next_to_commit) {
    ...
    */
    ...
    if (head->get_transaction()->m_flags.tid_written) dec_prep_tids(head);
  }
}
...
bool MYSQL_BIN_LOG::change_stage(THD *thd [[maybe_unused]],
  ...
  assert(rds_write_binlog_into_redo || enter_mutex);
  assert(queue);
  /*
...
int MYSQL_BIN_LOG::finish_commit(THD *thd) {
  ...
  */
  if (unlikely(!is_open()) ||
      DBUG_EVALUATE_IF("simulate_binlog_is_closed", true, false)) {
    binlog_cache_mngr *cache_mngr = thd_get_cache_mngr(thd);
  ...
  if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
      !rds_write_binlog_into_redo) {
    if (rds_dstore_support_binlog_check &&
        global_gtid_mode.get() == Gtid_mode::ON) {
      DBUG_EXECUTE_IF("crash_write_gtid_before_log", DBUG_SUICIDE(););
      if (!need_ddl_info(thd)) {
        write_gtid_info_into_redo(thd);
      } else {
        write_gtid_info_and_ddl_info_into_redo(thd);
      }
    } else if (need_ddl_info(thd)) {
      ddl_info_to_redo(thd, nullptr);
    }
  }

  ::finish_transaction_in_engines(thd, all, false);
  DBUG_EXECUTE_IF("sleep_before_update_gtid", sleep(1););
  DBUG_EXECUTE_IF("crash_write_gtid_after_log", DBUG_SUICIDE(););
  ...
  if (thd->get_transaction()->m_flags.tid_written) {
    dec_prep_tids(thd);
  }
  ...
  if (committed_low && (thd->commit_error != THD::CE_COMMIT_ERROR) &&
    ...
    if (unlikely(!rds_binlog_skip_run_after_commit_hook)) {
      (void)RUN_HOOK(transaction, after_commit, (thd, all));
    }
  ...
  if ((opt_binlog_order_commits || Clone_handler::need_commit_order()) &&
    ...
    */
    ...
    if (leave_mutex_before_commit_stage) {
      bool write_gtid_info_into_redo = false;
      if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
          rds_dstore_support_binlog_check && !rds_write_binlog_into_redo &&
          global_gtid_mode.get() == Gtid_mode::ON) {
        ha_set_max_start_recovery_point(thd);
        write_gtid_info_into_redo = true;
      }
      mysql_mutex_unlock(leave_mutex_before_commit_stage);
      if (write_gtid_info_into_redo) {
  ...
  if (DBUG_EVALUATE_IF("force_rotate", 1, 0) || force_rotate ||
    ...
    */
    ...
    if (error)
      ...
      mysql_mutex_lock(&LOCK_purge_binlog);
      ulong binlog_purge_thread_status = rds_binlog_purge_thread_status;
      mysql_mutex_unlock(&LOCK_purge_binlog);
      if (rds_purge_binary_logs_asynchronously &&
          binlog_purge_thread_status >= BINLOG_PURGE_THEAD_STATUS_RESET &&
          consecutive_reset_numbers < max_consecutive_reset_numbers &&
          !terminate_purge) {
        // Signal the purge binlog thread to purge
        // expired binary logs asynchronously.
        if (binlog_purge_thread_status == BINLOG_PURGE_THEAD_STATUS_RESET) {
          consecutive_reset_numbers++;
        } else {
          consecutive_reset_numbers = 0;
        }
        mysql_mutex_lock(&LOCK_purge_binlog);
        should_purge = true;
        rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_RESET;
        mysql_cond_signal(&COND_purge_binlog);
        mysql_mutex_unlock(&LOCK_purge_binlog);
      } else {
        // Purge expired binary logs synchronously.
        auto_purge();
        if (rds_purge_binary_logs_asynchronously &&
            binlog_purge_thread_status == BINLOG_PURGE_THEAD_STATUS_RESET) {
          mysql_mutex_lock(&LOCK_purge_binlog);
          rds_binlog_purge_thread_status = BINLOG_PURGE_THEAD_STATUS_GONE;
          mysql_mutex_unlock(&LOCK_purge_binlog);
          LogErr(ERROR_LEVEL, ER_BINLOG_PURGE_THREAD_GONE, debug_info_prefix);
        }
      }
    }
  }
...
int MYSQL_BIN_LOG::group_commit(THD *thd, bool all, bool skip_commit) {
  DBUG_TRACE;
  int flush_error = 0;
  my_off_t total_bytes = 0;
  ...
  /*
    Stage #1: flushing the log events of transactions to redo log
  ...
  */
  ...
  if (unlikely(!is_open()) ||
      DBUG_EVALUATE_IF("simulate_binlog_is_closed", true, false)) {
    final_queue = fetch_and_process_flush_stage_queue(true);
    leave_mutex_before_commit_stage = &LOCK_redo;
    /*
      The binary log is closed, go to commit stage directly.
      Binlog cache should be cleared, but instead of doing
      it here, do that work in 'finish_commit' function
      so that leader and followers thread binlog caches
      will be cleared.
    */
    goto commit_stage;
  }
  DEBUG_SYNC(thd, "waiting_in_the_middle_of_flush_stage");
  DBUG_EXECUTE_IF("flush_queue_wait_for_more_trxs", sleep(2););
  flush_error = process_flush_stage_queue(&total_bytes, &wait_queue);
  ...
  /*
    Stage #2: Fetching the final queue with transactions to be committed.
  */
  ...
  /*
    Shall introduce a delay only if the binlog_group_commit_sync_delay
    is bigger than zero.
  */
  if (flush_error == 0 && opt_binlog_group_commit_sync_delay > 0)
    Commit_stage_manager::get_instance().wait_count_or_timeout(
        opt_binlog_group_commit_sync_no_delay_count,
        opt_binlog_group_commit_sync_delay, Commit_stage_manager::SYNC_STAGE);
  ...
  /*
    Stage #3: Commit all transactions in group.
  */
commit_stage:
  if (leave_mutex_before_commit_stage)
    mysql_mutex_unlock(leave_mutex_before_commit_stage);
  ...
  /*
    Finish the commit. We don't need the return value here since
    it is in thd->commit_error, which is returned below.
  */
  (void)finish_commit(thd);

  /*
    flush error is handled above (using binlog_error_action).
    Hence treat only COMMIT_ERRORs as errors.
  */
  return thd->commit_error == THD::CE_COMMIT_ERROR;
}

int MYSQL_BIN_LOG::single_commit(THD *thd, bool all, bool skip_commit) {
  DBUG_TRACE;
  CONDITIONAL_SYNC_POINT_FOR_TIMESTAMP("before_assign_session_to_bgc_ticket");
  thd->rpl_thd_ctx.binlog_group_commit_ctx().assign_ticket();
  init_thd_variables(thd, all, skip_commit);
  ...
  /*
    Stage #0: ensure replica threads commit order as they appear in the
    slave's relay log for transactions flushing to binary log.

    Although it's not used in the MySQL/DStore physical replication, it is
    needed for inbound-replication used in migration of live databases.
  */
  if (Commit_order_manager::wait_for_its_turn_before_flush_stage(thd) ||
      ending_trans(thd, all) ||
      Commit_order_manager::get_rollback_status(thd)) {
  ...
  /*
    Stage #1: flushing the log events of transactions to redo log
  */
  auto *active_lock = &LOCK_log;
  mysql_mutex_lock(active_lock);
  ...
  DEBUG_SYNC(thd, "waiting_in_the_middle_of_flush_stage");
  DBUG_EXECUTE_IF("flush_queue_wait_for_more_trxs", sleep(2););
  flush_error = process_flush_stage_for_thd(thd, total_bytes);
  ...
  if (flush_error == 0)
    flush_error = process_after_flush_stage_for_thd(
        thd, flush_end_pos, update_binlog_end_pos_after_sync);

  if (flush_error != 0 ||
      DBUG_EVALUATE_IF("after_flush_simulate_error", true, false)) {
    /*
      Handle flush error (if any) after leader finishes it's flush stage.
    */
    handle_binlog_flush_or_sync_error(
  ...
  /*
    Stage #2: Syncing binary log file to disk and group commit sync delay
              handling.

    Shall introduce a delay only if the binlog_group_commit_sync_delay
    is bigger than zero.
  */
  mysql_mutex_unlock(active_lock);
  active_lock = &LOCK_sync;
  mysql_mutex_lock(active_lock);

  if (opt_binlog_group_commit_sync_delay > 0) {
    ::process_sync_stage_delay();
  }

  if (flush_error == 0)
    sync_error = process_sync_stage_for_thd(thd, total_bytes,
                                            update_binlog_end_pos_after_sync);

  DEBUG_SYNC(thd, "bgc_after_sync_stage_before_commit_stage");

  /*
    Stage #3: Commit the transaction in order.

    We need to acquire LOCK_commit to commit in order if
    `binlog_order_commits` is set.
  */
commit_stage:
  mysql_mutex_unlock(active_lock);

  if (opt_binlog_order_commits || Clone_handler::need_commit_order()) {
    active_lock = &LOCK_commit;
    mysql_mutex_lock(active_lock);
    ...
    if (flush_error == 0 && sync_error == 0)
      sync_error = call_after_sync_hook(thd);

    process_commit_stage_for_thd(thd);
    mysql_mutex_unlock(active_lock);

    active_lock = &LOCK_after_commit;
    mysql_mutex_lock(active_lock);
    process_after_commit_stage_for_thd(thd);
    mysql_mutex_unlock(active_lock);
  } else {
    if (flush_error == 0 && sync_error == 0)
      sync_error = call_after_sync_hook(thd);
  ...
  */
  ...
  process_rotate_stage_for_thd(thd);
...
int MYSQL_BIN_LOG::concurrent_commit(THD *thd, bool all, bool skip_commit) {
  DBUG_TRACE;
  bool gtid_error = false;
  int flush_error = 0;
  my_off_t flushed_bytes = 0;
  ...
  gtid_error = assign_automatic_gtid(thd);
  if (!gtid_error)
    std::tie(flush_error, flushed_bytes) = flush_thread_caches(thd);

  if (gtid_error || flush_error != 0) {
    handle_binlog_flush_or_sync_error(
        thd, false /* need_lock_log */,
        (thd->commit_error == THD::CE_FLUSH_GNO_EXHAUSTED_ERROR)
            ? ER_THD(thd, ER_GNO_EXHAUSTED)
...
bool MYSQL_BIN_LOG::assign_automatic_gtid(THD *thd) {
  DBUG_TRACE;
  bool error = false;
  rpl_sidno locked_sidno = 0;

  assert(thd->variables.gtid_next.type != UNDEFINED_GTID);

  /* Generate GTID */
  if (thd->variables.gtid_next.type == AUTOMATIC_GTID) {
    global_sid_lock->rdlock();
    if (gtid_state->generate_automatic_gtid(
            thd, thd->get_transaction()->get_rpl_transaction_ctx()->get_sidno(),
...
void MYSQL_BIN_LOG::write_into_binlog(const unsigned char *buffer,
                                      uint16_t gtid_len, uint32_t &position) {
  mysql_mutex_lock(&LOCK_log);
  Binlog_event_writer writer(get_binlog_file());

  // Write the GTID event.
  writer.write(buffer + position, gtid_len);
  position += gtid_len;

  // Write a group of log events of a transaction.
  uint32_t trx_len = uint4korr(buffer + position);
  position += 4;
  writer.write(buffer + position, trx_len);
  position += trx_len;

  flush();
  update_binlog_end_pos();

  // Check if the current log file has reached its maximum size and rotate it.
  if (m_binlog_file->get_real_file_size() >= (my_off_t)max_size) {
    new_file_without_locking(nullptr);
  }
  mysql_mutex_unlock(&LOCK_log);
}
...
/**
  The method executed by read binlog event from wal and write into binary log
  files.

  @param buffer  binlog event buffer

*/
int write_extra_commit_wal_callback(uint64 end_plsn,
                                    const unsigned char *buffer, uint32,
                                    uint64 &sync_plsn) {
  uint32_t position = 0;
  ...
  if (flag & HAS_BINLOG_EVENT) {
    ...
    uint16_t gtid_length = uint2korr(buffer + position);
    position += GTID_LEN;
    if (mysql_bin_log.is_open()) {
      mysql_bin_log.write_into_binlog(buffer, gtid_length, position);
    }
  }
  sync_plsn = end_plsn;
...
/**
  Find the starting position of DDL info in the wal buffer.

 @param buffer Pointer to the input buffer reading from wal.
 @param buffer_length Length of the input buffer.
 @retval uint32_t The starting position of DDL info. Returns 0 if the
 parsing process exceeds the buffer boundaries.
 */
uint32_t get_ddl_info_start_postion(const unsigned char *buffer,
                                    uint32_t buffer_length) {
  uint32_t position = 0;
  uint32_t flag = uint4korr(buffer);
  position += BUF_SIZE_FOR_FLAG;
  if (flag & HAS_GTID_INFO) {
    position += BUF_SIZE_FOR_GNO;
    if (position >= buffer_length) return 0;
    position += UUID_LEN;
  } else if (flag & HAS_BINLOG_EVENT) {
    /* format if binlog in wal: GTID + BINLOG_EVENT */
    /* skip gtid info. */
    position += BUF_SIZE_FOR_GNO;
    if (position >= buffer_length) return 0;
    uint16_t gtid_len = uint2korr(buffer + position);
    position += (BUF_SIZE_FOR_GTID_LEN + gtid_len);
    if (position >= buffer_length) return 0;
    /* skip binlog event info. */
    uint32_t trx_len = uint4korr(buffer + position);
    position += (BUF_SIZE_FOR_TRX_LEN + trx_len);
  }
  if (position >= buffer_length) return 0;
  return position;
}

void ddl_info_to_redo(THD *thd, Binlog_event_writer *writer) {
  uint32_t ddl_sql_len =
      need_ddl_info_sql(thd) ? thd->ddl_info_sql().length() : 0;
  DBUG_PRINT("print_ddl_sql_text", ("DDL info sql to wal text is %s",
                                    std::string(thd->ddl_info_sql().ptr(),
                                                thd->ddl_info_sql().length())
                                        .c_str()));
  if (ddl_sql_len > max_ddl_info_len()) {
    LogErr(ERROR_LEVEL, ER_WRITE_INVALID_DDL_INFO_LEN, ddl_sql_len);
  }
  if (writer == nullptr) {
    // just write trx_id + ddl_info into wal
    write_only_ddl_info_into_redo(thd);
  } else {
    // append ddl_info after binlog events.
    uchar trx_id_buf[BUF_SIZE_FOR_TRX_ID];
    // write trx_id
    int8store(trx_id_buf,
              need_ddl_info_xid(thd)
                  ? thd->get_transaction()->xid_state()->get_xid()->get_my_xid()
                  : 0);
    writer->write((uchar *)trx_id_buf, sizeof(trx_id_buf));

    uchar buf[BUF_SIZE_FOR_DDL_SQL_LEN];
    int2store(buf, ddl_sql_len);
    writer->write((uchar *)buf, sizeof(buf));
    if (need_ddl_info_sql(thd)) {
      writer->write(
          reinterpret_cast<const unsigned char *>(thd->ddl_info_sql().ptr()),
          ddl_sql_len);
    }
    static_cast<Binlog_write_into_redo *>(writer)->update_flags(HAS_DDL_INFO);
    sql_print_information("write ddl info into wal ddl sql_size %u.",
                          ddl_sql_len);
  }
  thd->is_ddl_info_written = true;
}

void write_gtid_info_into_redo(THD *thd) {
  uint16_t buff_size = UUID_LEN + GNO_LEN + FLAG_LEN;
  uint32_t flag = 0;
  uchar *buff = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, buff_size, MYF(0));
  if (buff == nullptr) {
    sql_print_error("write_ddl_info_into_redo: Couldn't allocate memory!\n");
    return;
  }
  flag |= HAS_GTID_INFO;
  int4store(buff, flag);
  uint32_t position = 0;
  position += FLAG_LEN;
  int8store(buff + position, thd->get_gno());
  position += GNO_LEN;
  my_memcpy(buff + position, buff_size - position, thd->get_uuid(), UUID_LEN);
  ha_commit_wals(thd, buff, buff_size);
  my_free(buff);
}

void write_gtid_info_and_ddl_info_into_redo(THD *thd) {
  uint16_t buff_size = UUID_LEN + GNO_LEN + FLAG_LEN + BUF_SIZE_FOR_TRX_ID +
                       BUF_SIZE_FOR_DDL_SQL_LEN + thd->ddl_info_sql().length();
  uchar *buff = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, buff_size, MYF(0));
  if (buff == nullptr) {
    sql_print_error("write_ddl_info_into_redo: Couldn't allocate memory!\n");
    return;
  ...
  position += GNO_LEN;
  // write uuid
  my_memcpy(buff + position, UUID_LEN, thd->get_uuid(), UUID_LEN);
  position += UUID_LEN;
  // write trx_id
  ...
  // write ddl sql len
  int2store(buff + position, thd->ddl_info_sql().length());
  position += BUF_SIZE_FOR_DDL_SQL_LEN;
  // write ddl sql
  if (need_ddl_info_sql(thd)) {
    my_memcpy(buff + position, buff_size - position, thd->ddl_info_sql().ptr(),
              thd->ddl_info_sql().length());
  }
  ha_commit_wals(thd, buff, buff_size);
  thd->is_ddl_info_written = true;
...
void write_gtid_executed_info_into_redo(THD *thd) {
  if (global_gtid_mode.get() != Gtid_mode::ON) {
    return;
  }
  uint32_t flag = 0;
  flag |= HAS_GTID_EXECUTED;
  global_sid_lock->wrlock();
  Gtid_set *gs = const_cast<Gtid_set *>(gtid_state->get_executed_gtids());
  uchar *buff = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED,
                                   binlog_write_redo_buffer_size, MYF(0));
  if (buff == nullptr) {
    global_sid_lock->unlock();
    sql_print_error("write_ddl_info_into_redo: Couldn't allocate memory!\n");
    return;
  ...
  int4store(buff, flag);
  uint16_t gtid_len = gs->to_string((char *)buff + FLAG_LEN + GTID_LEN);
  global_sid_lock->unlock();
  int2store(buff + FLAG_LEN, gtid_len);
  uint16_t buff_size = gtid_len + FLAG_LEN + GTID_LEN + 1;
  if (buff_size > GTID_EXECUTED_WARNING_LEN) {
    LogErr(WARNING_LEVEL, ER_DSTORE_GTID_EXECUTED_TOO_LONG, debug_info_prefix);
  }
  ha_commit_wals(thd, buff, buff_size, true);
  my_free(buff);
}

/*
  The implementation of dstore support binlog check trace
*/
int my_dstore_support_binlog_check_trace_impl(enum loglevel level,
                                              const char *format, ...) {
  char ibuff[1000];
  va_list args;
  va_start(args, format);
  int res = 0;
  res = vsnprintf_s(ibuff, sizeof(ibuff), (sizeof(ibuff) - 1), format, args);
  va_end(args);
  my_message_local(level, EE_DEBUG_INFO, ibuff);
  return res;
}

int MYSQL_BIN_LOG::process_flush_stage_for_thd(THD *thd,
                                               my_off_t &total_bytes) {
  DBUG_TRACE;
  mysql_mutex_assert_owner(&LOCK_log);

  // Flush to the SE first.
  ha_flush_logs(true);

  assign_automatic_gtids_to_flush_group(thd);
  int error{0};
  std::tie(error, total_bytes) = flush_thread_caches(thd);

  // Check if binlog file has exceeded the allowed size and mark it for
  // rotation
  thd->rpl_thd_ctx.binlog_group_commit_ctx().set_max_size_exceeded(
      total_bytes > 0 &&
      (m_binlog_file->get_real_file_size() >= (my_off_t)max_size));
...
int MYSQL_BIN_LOG::process_after_flush_stage_for_thd(
    THD *thd, my_off_t flush_end_pos, bool update_binlog_end_pos_after_sync) {
  // Call the AFTER_FLUSH hooks, if any
  int flush_error{0};
  const char *file_name_ptr = log_file_name + dirname_length(log_file_name);
  ...
  // If not to update binlog end position after sync, update now
  if (!update_binlog_end_pos_after_sync) update_binlog_end_pos();

  return flush_error;
}

int MYSQL_BIN_LOG::process_sync_stage_for_thd(
    THD *thd, my_off_t total_bytes, bool update_binlog_end_pos_after_sync) {
  int sync_error{0};

  if (total_bytes > 0) {
    std::tie(sync_error, std::ignore) = sync_binlog_file(false);
  }

  // Update binlog end position since it was delayed to this point
  if (sync_error == 0 &&
      (update_binlog_end_pos_after_sync ||
       DBUG_EVALUATE_IF("force_update_binlog_end_pos_after_sync", true,
                        false))) {
    const char *binlog_file = nullptr;
    my_off_t pos = 0;

    thd->get_trans_fixed_pos(&binlog_file, &pos);
    if (binlog_file != nullptr && pos > 0) {
      update_binlog_end_pos(binlog_file, pos);
    }
  }
...
void MYSQL_BIN_LOG::process_commit_stage_for_thd(THD *thd) {
  mysql_mutex_assert_owner(&LOCK_commit);

  // Update the maximum committed sequence-number for dependency tracking
  if (thd->commit_error == THD::CE_NONE &&
      thd->get_transaction()->sequence_number != SEQ_UNINIT) {
    mysql_mutex_lock(&LOCK_replica_trans_dep_tracker);
    m_dependency_tracker.update_max_committed(thd);
    mysql_mutex_unlock(&LOCK_replica_trans_dep_tracker);
  }
  ...
  // Update the internal structures holding the GTID_EXECUTED set
...
void MYSQL_BIN_LOG::process_after_commit_stage_for_thd(THD *thd) {
  // Call the AFTER_COMMIT hooks, if any
  if ((thd->get_transaction()->m_flags.run_hooks &&
       thd->commit_error != THD::CE_COMMIT_ERROR) ||
      DBUG_EVALUATE_IF("force_run_after_commit_hooks", true, false)) {
    bool all = thd->get_transaction()->m_flags.real_commit;
    (void)RUN_HOOK(transaction, after_commit, (thd, all));
    thd->get_transaction()->m_flags.run_hooks = false;
  }
...
void MYSQL_BIN_LOG::process_rotate_stage_for_thd(THD *thd) {
  const auto [check_rotate, force_rotate] =
      Binlog_group_commit_ctx::aggregate_rotate_settings(thd);

  if (force_rotate || (check_rotate && thd->commit_error == THD::CE_NONE) ||
...
void process_sync_stage_delay() {
  long to_wait = DBUG_EVALUATE_IF("bgc_set_infinite_delay", LONG_MAX,
...
/**
  Terminate the binlog purge thread.
*/
void terminate_binlog_purge_thread() {
  DBUG_TRACE;
  int error = 0;
  ...
  // Notify suspended purge binlog thread.
  mysql_mutex_lock(&LOCK_purge_binlog);
  terminate_purge = true;
  mysql_cond_signal(&COND_purge_binlog);
  mysql_mutex_unlock(&LOCK_purge_binlog);

  if (purge_thread_id.thread != 0) {
    error = my_thread_join(&purge_thread_id, nullptr);
    purge_thread_id.thread = 0;
  }

  if (error != 0)
    LogErr(WARNING_LEVEL, ER_BINLOG_FAILED_TO_JOIN_PURGE_THREAD,
           debug_info_prefix);
}

...
static bool parse_query_bind_params_bulk(THD *thd, uint param_count,
  ...
  *out_parameters =
      static_cast<PS_PARAM *>(thd->mem_calloc(param_count * sizeof(PS_PARAM)));
  if (!*out_parameters) return true; /* purecov: inspected */

  PS_PARAM *params = *out_parameters;

  /*
    Only parse parameter types here, parse the actual data during executing.
  */
...
bool Protocol_classic::parse_packet(union COM_DATA *data,
  ...
  switch (cmd) {
    ...
    case COM_STMT_BULK_EXECUTE: {
      ...
      if (packet_left < 6) goto malformed;

      // Get the statement id
      data->com_stmt_bulk_execute.stmt_id = uint4korr(read_pos);
      read_pos += 4;
      packet_left -= 4;

      data->com_stmt_bulk_execute.flags = uint2korr(read_pos);
      read_pos += 2;
      packet_left -= 2;
      ...
      data->com_stmt_bulk_execute.parameters = nullptr;
      data->com_stmt_bulk_execute.read_pos = read_pos;
      data->com_stmt_bulk_execute.packet_left = packet_left;

      if (!stmt || stmt->m_param_count == 0) {
        break;
      }

      if (parse_query_bind_params_bulk(
              m_thd, stmt->m_param_count,
              &data->com_stmt_bulk_execute.parameters,
              &data->com_stmt_bulk_execute.read_pos,
              &data->com_stmt_bulk_execute.packet_left)) {
        goto malformed;
      }
      break;
    }
...
int Protocol_classic::get_command(COM_DATA *com_data,
  ...
  *cmd = (enum enum_server_command)(uchar)input_raw_packet[0];
  ...
  if (*cmd >= COM_END || (*cmd >= COM_GAP_BEG && *cmd <= COM_GAP_END))

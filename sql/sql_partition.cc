...
static int get_partition_id_linear_key_nosub(partition_info *part_info,
                                             uint32 *part_id,
                                             longlong *func_value);
static int get_partition_id_list_col_sub(partition_info *part_info,
                                         uint32 *part_id, uint32 main_part_id);
static int get_partition_id_list_sub(partition_info *part_info, uint32 *part_id,
                                     uint32 main_part_id);
static int get_partition_id_range_col_sub(partition_info *part_info,
                                          uint32 *part_id, uint32 main_part_id);
static int get_partition_id_range_sub(partition_info *part_info,
                                      uint32 *part_id, uint32 main_part_id);
    ...
    if (part_info->use_default_num_partitions) {
      if (part_handler->get_num_parts(normalized_path, &part_info->num_parts)) {
        return true;
               ...
               part_info->sub_part_info->use_default_num_partitions) {
      uint num_parts;
      ...
      part_info->sub_part_info->num_parts = num_parts / part_info->num_parts;
    }
  }
...
static bool set_up_field_array(TABLE *table, partition_info *part_info) {
  ...
  uint inx;
  int result = false;
  bool is_sub_part = part_info->is_subpart_info();
  DBUG_TRACE;
...
static bool handle_list_of_fields(List_iterator<char> it, TABLE *table,
  ...
  result = set_up_field_array(table, part_info);
end:
  return result;
}
...
static bool fix_fields_part_func(THD *thd, partition_info *part_info,
                                 TABLE *table, bool is_create_table_ind) {
  Item *func_expr = part_info->part_expr;
  bool result = true;
  int error;
  LEX *old_lex = thd->lex;
  LEX lex;
  ...
  */
  ...
  if ((error = check_signed_flag(part_info))) goto end;
  result = set_up_field_array(table, part_info);
end:
...
static void set_up_partition_key_maps(TABLE *table, partition_info *part_info) {
  ...
  for (i = 0; i < keys; i++) {
    ...
    if (part_info->is_sub_partitioned()) {
      ...
      check_fields_in_PF(part_info->sub_part_info->part_field_array,
                         &all_fields, &some_fields);
...
get_subpart_id_func get_subpartition_id_helper(partition_info *sub_part_info) {
  get_subpart_id_func get_subpartition_id = nullptr;

  assert(sub_part_info->part_type != partition_type::NONE);

  partition_type part_type = sub_part_info->part_type;
  if (part_type == partition_type::RANGE) {
    if (sub_part_info->column_list)
      get_subpartition_id = get_partition_id_range_col_sub;
    else
      get_subpartition_id = get_partition_id_range_sub;
  } else if (part_type == partition_type::LIST) {
    if (sub_part_info->column_list)
      get_subpartition_id = get_partition_id_list_col_sub;
    else
      get_subpartition_id = get_partition_id_list_sub;
  } else { /* HASH Partitioning */
    if (sub_part_info->list_of_part_fields) {
      if (sub_part_info->linear_hash_ind)
        get_subpartition_id = get_partition_id_linear_key_sub;
      else
        get_subpartition_id = get_partition_id_key_sub;
    } else {
      if (sub_part_info->linear_hash_ind)
        get_subpartition_id = get_partition_id_linear_hash_sub;
      else
        get_subpartition_id = get_partition_id_hash_sub;
    }
  }
  return get_subpartition_id;
}
...
static void set_up_partition_func_pointers(partition_info *part_info) {
    ...
    } else /* HASH partitioning */
    {
      if (part_info->list_of_part_fields) {
        if (part_info->linear_hash_ind)
          part_info->get_part_partition_id = get_partition_id_linear_key_nosub;
        else
          part_info->get_part_partition_id = get_partition_id_key_nosub;
      } else {
        if (part_info->linear_hash_ind)
          part_info->get_part_partition_id = get_partition_id_linear_hash_nosub;
        else
          part_info->get_part_partition_id = get_partition_id_hash_nosub;
      }
    }
    part_info->get_subpartition_id =
        get_subpartition_id_helper(part_info->sub_part_info);
  ...
  */
  ...
  if (part_info->is_sub_partitioned() &&
      part_info->sub_part_info->part_charset_field_array) {
...
/**
        Helper function for fix_partition_func. This helper function is to fix
        (sub)partition info itself.

        @param thd                  The thread object
        @param table                TABLE object for which partition fields are
   set-up
        @param part_info            Partition info
        @param is_create_table_ind  Indicator of whether openfrm was called as
   part of CREATE or ALTER TABLE

        @retval True means some error happens. Otherwise false;
*/
bool fix_partition_func_helper(THD *thd, TABLE *table,
                               partition_info *part_info,
                               bool is_create_table_ind) {
  assert(part_info);
  if (part_info->part_type == partition_type::NONE) return false;
  /*
  ...
  */
  if (part_info->part_type == partition_type::HASH) {
    ...
    if (part_info->list_of_part_fields) {
      List_iterator<char> it(part_info->part_field_list);
      if (unlikely(handle_list_of_fields(it, table, part_info))) goto end;
    } else {
      if (unlikely(
              fix_fields_part_func(thd, part_info, table, is_create_table_ind)))
        goto end;
...
bool fix_partition_func(THD *thd, TABLE *table, bool is_create_table_ind) {
  ...
  if (!is_create_table_ind || thd->lex->sql_command != SQLCOM_CREATE_TABLE ||
      thd->recycle_state->get_type() != im::recycle_bin::RECYCLE_BIN_NONE) {
  ...
  if (part_info->is_sub_partitioned()) {
    if (fix_partition_func_helper(thd, table, part_info->sub_part_info,
                                  is_create_table_ind))
      goto end;
    if (unlikely(set_up_partition_bitmaps(part_info->sub_part_info))) goto end;
  }
  assert(part_info->part_type != partition_type::NONE);
  if (fix_partition_func_helper(thd, table, part_info, is_create_table_ind))
    goto end;
...
/**
  Generate the partition information syntax on partition-by parts from the
        partition data structure.
  Used to support of generate_partition_syntax.
  ...
  @param  current_comment_start   NULL, or comment string encapsulating the
                                  PARTITION BY clause.
  @param	fptr                    VFD file number

        @retval True means error happens. Otherwise false.
*/
static bool generate_partition_by_syntax(partition_info *part_info,
                                         bool print_expr,
                                         const char *current_comment_start,
                                         const File &fptr) {
  int err = 0;
  if (part_info->is_subpart_info()) {
    err += add_string(fptr, "\n");
  ...
  DBUG_EXECUTE_IF("generate_partition_by_syntax_failed", err = 1;);
  if (err) return true;
  return false;
}
...
char *generate_partition_syntax(partition_info *part_info, uint *buf_length,
  ...
  if (generate_partition_by_syntax(part_info, print_expr, current_comment_start,
                                   fptr))
    goto close_file;
  if (part_info->is_sub_partitioned() &&
      generate_partition_by_syntax(part_info->sub_part_info, print_expr,
                                   current_comment_start, fptr))
    goto close_file;

  tot_num_parts = part_info->partitions.elements;
  num_subparts = part_info->get_num_subparts();
  ...
  if (!part_info->use_default_partitions) {
    ...
    do {
      ...
      if (part_elem->part_state != PART_TO_BE_DROPPED &&
        ...
        if (!part_info->is_sub_partitioned() ||
            part_info->sub_part_info->use_default_partitions) {
          ...
          do {
            ...
            err +=
                add_partition_values(fptr, part_info->sub_part_info, sub_elem);
            if (show_partition_options)
...
static int get_part_id_linear_hash(partition_info *part_info, uint32 *part_id,
                                   longlong *func_value) {
  DBUG_TRACE;

  if (part_val_int(part_info->part_expr, func_value))
    return HA_ERR_NO_PARTITION_FOUND;

  *part_id = get_part_id_from_linear_hash(
      *func_value, part_info->linear_hash_mask, part_info->num_parts);
  return false;
}
...
inline static uint32 get_part_id_linear_key(partition_info *part_info,
  ...
  *func_value = part_info->table->file->calculate_key_hash_value(
...
static int get_part_id_charset_func_subpart(partition_info *part_info,
  ...
  copy_to_part_field_buffers(part_info->sub_part_info->part_charset_field_array,
                             part_info->sub_part_info->part_field_buffers,
                             part_info->sub_part_info->restore_part_field_ptrs);
  res =
      part_info->get_subpartition_id_charset(part_info, part_id, main_part_id);
  restore_part_field_pointers(
      part_info->sub_part_info->part_charset_field_array,
      part_info->sub_part_info->restore_part_field_ptrs);
  return res;
...
static int get_partition_id_linear_hash_nosub(partition_info *part_info,
                                              uint32 *part_id,
                                              longlong *func_value) {
  return get_part_id_linear_hash(part_info, part_id, func_value);
}
...
static int get_partition_id_linear_key_nosub(partition_info *part_info,
                                             uint32 *part_id,
                                             longlong *func_value) {
  *part_id = get_part_id_linear_key(part_info, func_value);
  return 0;
}
...
static int get_partition_id_with_sub(partition_info *part_info, uint32 *part_id,
  ...
  if (!validate_partition_type(part_info)) {
    my_error(ER_PARTITION_SUBPARTITION_ERROR, MYF(0));
    return HA_ERR_NOT_ALLOWED_PARTITION_TYPE;
  }
  ...
  num_subparts = part_info->get_num_subparts();
  if (unlikely((error = part_info->get_subpartition_id(part_info, &sub_part_id,
                                                       loc_part_id)))) {
    return error;
  }
...
/*
  ...
  DESCRIPTION
    ...
    get_partition_id_list_col_sub
    get_partition_id_list_sub
    get_partition_id_range_col_sub
    get_partition_id_range_sub
...
static int get_partition_id_list_col_sub(partition_info *part_info,
  ...
  partition_info *sub_part_info = part_info->sub_part_info;
  sub_part_info->fix_val_array_pointer(main_part_id);
  int error = get_partition_id_list_col(sub_part_info, part_id, &func_value);
  if (unlikely(error)) {
    sub_part_info->err_value = func_value;
  }
  return error;
}
...
static int get_partition_id_list_sub(partition_info *part_info, uint32 *part_id,
  ...
  partition_info *sub_part_info = part_info->sub_part_info;
  sub_part_info->fix_val_array_pointer(main_part_id);
  int error = get_partition_id_list(sub_part_info, part_id, &func_value);
  if (unlikely(error)) {
    sub_part_info->err_value = func_value;
  }
  return error;
...
static int get_partition_id_range_col_sub(partition_info *part_info,
  ...
  partition_info *sub_part_info = part_info->sub_part_info;
  sub_part_info->fix_val_array_pointer(main_part_id);
  int error = get_partition_id_range_col(sub_part_info, part_id, &func_value);
  if (unlikely(error)) {
    sub_part_info->err_value = func_value;
  }
  return error;
...
static int get_partition_id_range_sub(partition_info *part_info,
  ...
  partition_info *sub_part_info = part_info->sub_part_info;
  sub_part_info->fix_val_array_pointer(main_part_id);
  int error = get_partition_id_range(sub_part_info, part_id, &func_value);
  if (unlikely(error)) {
    sub_part_info->err_value = func_value;
  }
  return error;
...
static int get_partition_id_hash_sub(partition_info *part_info, uint32 *part_id,
                                     uint32 main_part_id
                                         MY_ATTRIBUTE((unused))) {
  longlong func_value;
  return get_part_id_hash(part_info->sub_part_info->num_parts,
                          part_info->sub_part_info->part_expr, part_id,
                          &func_value);
}

static int get_partition_id_linear_hash_sub(
    partition_info *part_info, uint32 *part_id,
    uint32 main_part_id MY_ATTRIBUTE((unused))) {
  longlong func_value;
  return get_part_id_linear_hash(part_info->sub_part_info, part_id,
                                 &func_value);
}
...
static int get_partition_id_key_sub(partition_info *part_info, uint32 *part_id,
  ...
  *part_id = get_part_id_key(part_info->table->file,
                             part_info->sub_part_info->part_field_array,
                             part_info->sub_part_info->num_parts, &func_value);
  return false;
}
...
static int get_partition_id_linear_key_sub(
    ...
    uint32 main_part_id MY_ATTRIBUTE((unused))) {
  longlong func_value;
  *part_id = get_part_id_linear_key(part_info->sub_part_info, &func_value);
  return false;
}
...
static int get_sub_part_id_from_key(const TABLE *table, uchar *buf,
  ...
  if (likely(rec0 == buf)) {
    res = part_info->get_subpartition_id(part_info, part_id, main_part_id);
  } else {
    Field **part_field_array = part_info->sub_part_info->part_field_array;
    set_field_ptr(part_field_array, buf, rec0);
    res = part_info->get_subpartition_id(part_info, part_id, main_part_id);
    set_field_ptr(part_field_array, rec0, buf);
  }
          ...
          for (i = 0; i < part_info->num_parts; i++) {
            if (!get_sub_part_id_from_key(table, buf, key_info, key_spec,
              ...
              part_spec->start_part =
                  sub_part + i * part_info->get_num_subparts();
              // For HASH subpartition, we can do some optimization, but for
              // other type, we just give a wide range without hole.
              if (part_info->sub_part_info->part_type !=
                      partition_type::RANGE &&
                  part_info->sub_part_info->part_type != partition_type::LIST)
                part_spec->end_part =
                    sub_part + (part_info->get_num_subparts() *
    ...
    */
    if (part_part != num_parts) {
      ...
      */
      ...
      part_spec->start_part = part_part * part_info->get_num_subparts();
      part_spec->end_part =
          part_spec->start_part + part_info->get_num_subparts() - 1;
    } else {
      assert(sub_part != num_parts);
      part_spec->start_part = sub_part;
      part_spec->end_part = sub_part + (part_info->get_num_subparts() *
                                        (part_info->num_parts - 1));
      for (i = 0, part_id = sub_part; i < part_info->num_parts;
           i++, part_id += part_info->get_num_subparts())
...
bool mysql_unpack_partition(THD *thd, char *part_buf, uint part_info_len,
  ...
  */
  ...
  if ((thd->recycle_state->get_type() == im::recycle_bin::RECYCLE_BIN_NONE) &&
      is_create_table_ind && old_lex->sql_command == SQLCOM_CREATE_TABLE) {
    /*
  ...
  if (part_info->is_sub_partitioned()) part_info->sub_part_info->table = table;
  part_handler = table->file->get_partition_handler();
...

  {
    ...
    */
    ...
    size_t subpart_func_len = part_info->get_subpart_func_len();
    ...
    */
    ...
    if (subpart_func_len)
      memcpy(subpart_func_string, part_info->sub_part_info->part_func_string,
             subpart_func_len);
    part_info->part_func_string = part_func_string;
    if (subpart_func_string)
      part_info->sub_part_info->part_func_string = subpart_func_string;
  }
...
static void set_engine_all_partitions(partition_info *part_info,
  ...
  do {
    ...
    if (part_info->is_sub_partitioned()) {
      ...
      } while (++j < part_info->get_num_subparts());
    }
  } while (++i < part_info->num_parts);
}
...
static bool check_alter_part_table(const THD *thd, const Alter_info *alter_info,
                                   const partition_info *part_info,
                                   const partition_info *tab_part_info) {
  assert(part_info);
  if (part_info->part_type != tab_part_info->part_type &&
      part_info->part_type != partition_type::NONE) {
    /*
  ...
  if (part_info->partitions.is_empty()) {
  ...
  if ((tab_part_info->column_list &&
       part_info->num_columns != tab_part_info->num_columns) ||
  ...
  if (alter_info->flags & Alter_info::ALTER_ADD_PARTITION) {
    ...
    */
    ...
    if (part_info->is_subpart_info()) {
      if (part_info->num_parts != tab_part_info->num_parts) {
...
/*
  Remove subpartitions for one partition as well as Subpartition.

  @param[in] thd Pointer of THD
  @param[in] sub_part_info sub partition info
  @param[in/out] part_it Iterator for sub_partitions
  @param[in/out] sub_it Iterator for Subpartition
  @param[in] main_part_id index of current main partition
  @param[in] is_first True means to remove all of subpartitions of first
  main partition.

  @retval True means error happens. Otherwise false.
*/
static bool remove_sub_partitions(const THD *thd, partition_info *sub_part_info,
                                  List_iterator<partition_element> &part_it,
                                  List_iterator<Sub_partition> &sub_it,
                                  uint main_part_id, bool is_first) {
  if (!(sub_part_info && sub_part_info->get_parent()->is_sub_partitioned()))
    return false;
  uint num_parts = sub_part_info->num_parts;
  do {
    part_it.remove();
    part_it++;
  } while (--num_parts > 0);
  // Replace the first subpartitions
  if (is_first) {
    Sub_partition *sub = new (thd->mem_root) Sub_partition(
        down_cast<sub_partition_info *>(sub_part_info), main_part_id);
    if (!sub) return true;
    sub_it.replace(sub);
  } else {
    sub_it.remove();
...
/*
  Move iterator to the subpartition position for next partition.

  @param[in] sub_part_info sub partition info
  @param[in/out] part_it Iterator for sub_partitions
  @param[in/out] prev_part_it Iterator used to append sub_partitions after it

  @retval void
*/
static void advance_sub_partitions(
    partition_info *sub_part_info, List_iterator<partition_element> &part_it,
    List_iterator<partition_element> &prev_part_it) {
  if (!(sub_part_info && sub_part_info->get_parent()->is_sub_partitioned()))
    return;
  uint num_parts = sub_part_info->num_parts;
  do {
...
/*
  Append subpartitions to sub_partition_info::partitions after the removed
  partition.

  @param[in] sub_part_info sub partition info
  @param[in] subpartitions sub-partitions which should be appended after
  prev_part_it
  @param[in/out] part_it Iterator for sub_partitions
  @param[in/out] prev_part_it Iterator used to append sub_partitions after it
  @param[in/out] sub_it Iterator for Subpartition
  @param[in] main_part_id index of current main partition
  @param[in] is_first True means first sub-partition to append.

  @retval True means error happens. Otherwise false.
*/
static bool append_sub_partitions(
    const THD *thd, partition_info *sub_part_info,
    List<partition_element> &subpartitions,
    List_iterator<partition_element> &part_it,
    List_iterator<partition_element> &prev_part_it,
    List_iterator<Sub_partition> &sub_it, uint main_part_id, bool is_first) {
  if (!(sub_part_info && sub_part_info->get_parent() &&
        sub_part_info->get_parent()->is_sub_partitioned()))
    return false;
  uint num_parts = sub_part_info->num_parts;
  List_iterator<partition_element> sub_part_it(subpartitions);
  uint j = 0;
  part_it = prev_part_it;
  do {
    partition_element *sub_part_elem = sub_part_it++;
    sub_part_elem->part_state = PART_TO_BE_ADDED;
    if (prev_part_it.is_before_first()) {
      sub_part_info->partitions.push_front(sub_part_elem);
      part_it++;
    } else
      part_it.after(sub_part_elem);
    prev_part_it++;
  } while (++j < num_parts);
  part_it++;
...
static bool create_subpart_info(const THD *thd, partition_info *alt_part_info,
                                partition_info *tab_part_info) {
  assert(tab_part_info->is_sub_partitioned());
  sub_partition_info *sub_clone =
      new (thd->mem_root) sub_partition_info(alt_part_info);
  if (!sub_clone) return true;
  alt_part_info->sub_part_info = sub_clone;
  alt_part_info->sub_part_info->part_type =
      tab_part_info->sub_part_info->part_type;
  alt_part_info->sub_part_info->num_parts = tab_part_info->get_num_subparts();
  alt_part_info->has_sub_part = true;
  alt_part_info->sub_part_info->column_list =
      tab_part_info->sub_part_info->column_list;
  return false;
}
...
uint prep_alter_part_table(THD *thd, TABLE *table, Alter_info *alter_info,
  ...
  if (alter_info->flags &
    ...
    if (!table->part_info || (!table->part_info->is_sub_partitioned() &&
                              alt_part_info && alt_part_info->sub_part_info)) {
      my_error(ER_PARTITION_MGMT_ON_NONPARTITIONED, MYF(0));
      return true;
    }
    if (!part_handler) {
    ...
    */
    ...
    // for add partiton/reorganize partition, if not define subpartition info,
    // so create subpart_info
    if (((alter_info->flags & Alter_info::ALTER_ADD_PARTITION) ||
         (alter_info->flags & Alter_info::ALTER_REORGANIZE_PARTITION)) &&
        tab_part_info->is_sub_partitioned() && !alt_part_info->sub_part_info &&
        create_subpart_info(thd, alt_part_info, tab_part_info))
      goto err;
    ...
    if ((alter_info->flags & Alter_info::ALTER_ADD_PARTITION) ||
        (alter_info->flags & Alter_info::ALTER_REORGANIZE_PARTITION)) {
      if (check_alter_part_table(thd, alter_info, thd->work_part_info,
                                 tab_part_info) ||
          (tab_part_info->is_sub_partitioned() &&
           check_alter_part_table(thd, alter_info,
                                  thd->work_part_info->sub_part_info,
                                  tab_part_info->sub_part_info)))
        goto err;
    ...
    if (alter_info->flags & Alter_info::ALTER_ADD_PARTITION) {
      ...
      */
      ...
      if (tab_part_info->is_sub_partitioned()) {
        check_total_partitions =
            new_total_partitions * alt_part_info->get_num_subparts();
      }
      ...
      */
      {
        ...
        do {
          ...
          if (tab_part_info->is_sub_partitioned()) {
            Sub_partition *sub_part = new (thd->mem_root)
                Sub_partition(tab_part_info->sub_part_info,
                              tab_part_info->num_parts + part_count);
            if (!sub_part ||
                tab_part_info->partition.subpartitions.push_back(sub_part))
              goto err;
            List_iterator<partition_element> part_it(part_elem->subpartitions);
            uint j = 0;
            do {
              partition_element *sub_part_elem = part_it++;
              if (*new_part_info) sub_part_elem->part_state = PART_TO_BE_ADDED;
              if (tab_part_info->sub_part_info->partitions.push_back(
                      sub_part_elem)) {
                mem_alloc_error(1);
                goto err;
              }
            } while (++j < tab_part_info->get_num_subparts());
          }
      ...
      */
      {
        ...
        List_iterator<Sub_partition> sub_part_it(
            tab_part_info->partition.subpartitions);
        do {
          partition_element *p_elem = part_it++;
          if (tab_part_info->is_sub_partitioned()) sub_part_it++;
          if (*new_part_info &&
              (all_parts ||
          ...
          if (++part_count > num_parts_remain) {
            if (*new_part_info)
              ...
              part_it.remove();
              if (tab_part_info->is_sub_partitioned()) sub_part_it.remove();
            }
          }
        } while (part_count < tab_part_info->num_parts);
      ...
      */
      ...
      if (tab_part_info->is_sub_partitioned() &&
          alt_part_info->get_num_subparts() &&
          alt_part_info->get_num_subparts() !=
              tab_part_info->get_num_subparts()) {
      ...
      */
      {
        List_iterator<partition_element> tab_it(tab_part_info->partitions);
        List_iterator<partition_element> sub_part_it, prev_sub_part_it;
        if (tab_part_info->is_sub_partitioned()) {
          sub_part_it.init(tab_part_info->sub_part_info->partitions);
          prev_sub_part_it = sub_part_it;
          sub_part_it++;
        }
        List_iterator<Sub_partition> sub_it(
            tab_part_info->partition.subpartitions);
        ...
        do {
          ...
          if (tab_part_info->is_sub_partitioned()) sub_it++;
            ...
            if (!found_first) {
              ...
              do {
                ...
                if (alt_part_count == 0) {
                  ...
                  if (remove_sub_partitions(thd, tab_part_info->sub_part_info,
                                            sub_part_it, sub_it, part_count,
                                            true) ||
                      append_sub_partitions(thd, tab_part_info->sub_part_info,
                                            alt_part_elem->subpartitions,
                                            sub_part_it, prev_sub_part_it,
                                            sub_it, part_count, true))
                    goto err;
                } else {
                  tab_it.after(alt_part_elem);
                  if (append_sub_partitions(thd, tab_part_info->sub_part_info,
                                            alt_part_elem->subpartitions,
                                            sub_part_it, prev_sub_part_it,
                                            sub_it, part_count + alt_part_count,
                                            false))
                    goto err;
                }
              ...
              uint dummy = 0;
              if (remove_sub_partitions(thd, tab_part_info->sub_part_info,
                                        sub_part_it, sub_it, dummy, false))
                goto err;
            }
          } else {
            if (found_first) found_last = true;
            advance_sub_partitions(tab_part_info->sub_part_info, sub_part_it,
                                   prev_sub_part_it);
          }
        } while (++part_count < tab_part_info->num_parts);
    ...
    *partition_changed = true;
    ...
    if (alter_info->flags & Alter_info::ALTER_ADD_PARTITION ||
        alter_info->flags & Alter_info::ALTER_REORGANIZE_PARTITION) {
      if (tab_part_info->get_use_default_subparts() &&
          !alt_part_info->get_use_default_subparts() &&
          tab_part_info->is_sub_partitioned()) {
        tab_part_info->sub_part_info->use_default_partitions = false;
        tab_part_info->sub_part_info->use_default_num_partitions = false;
      }
      if (tab_part_info->check_partition_info(
              thd, (handlerton **)nullptr, table->file, nullptr, true,
              alt_part_info->get_use_default_subparts(),
              alt_part_info->get_use_default_num_subparts())) {
        goto err;
      }
      /*
...
setup_subparts:
  ...
  */
  if (part_info->is_sub_partitioned() &&
      part_info->sub_part_info->num_part_fields == 1) {
    Field *field = part_info->sub_part_info->part_field_array[0];
    switch (field->type()) {
      case MYSQL_TYPE_TINY:
      case MYSQL_TYPE_SHORT:
      case MYSQL_TYPE_LONG:
...
static int get_part_iter_for_interval_via_walking(
  ...
  part_iter->ret_null_part = part_iter->ret_null_part_orig = false;
  if (is_subpart) {
    field = part_info->sub_part_info->part_field_array[0];
    total_parts = part_info->get_num_subparts();
...
static uint32 get_next_partition_id_list(PARTITION_ITERATOR *part_iter) {
  if (part_iter->part_nums.cur >= part_iter->part_nums.end) {
    if (part_iter->ret_null_part && part_iter->part_info->has_null_value) {
      part_iter->ret_null_part = false;
...
static uint32 get_next_subpartition_via_walking(PARTITION_ITERATOR *part_iter) {
  Field *field = part_iter->part_info->sub_part_info->part_field_array[0];
  uint32 res;

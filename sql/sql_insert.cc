  ...
  {  // Statement plan is available within these braces
    ...
    */
    ...
    for (;;) {
      ...
      if (owner() == nullptr || !owner()->more_bulk_iterations() || has_error ||
          thd->is_killed())
        break;

      /* One more iteration for this PS is coming, so update PS parameters for
      it. Do not confuse this - bulk execution of a PS - with "bulk insert"
      (as in ha_start_bulk_insert() earlier above) which is rather an
      optimized insertion path in certain storage engines when inserting many
      (e.g. >=100) rows. */
      if (owner()->set_parameters_bulk(thd, nullptr)) {
        has_error = true;
        break;
      }
    }  // end of for(;;)

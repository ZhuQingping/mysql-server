...
void Outline::evoke_index_hint(MEM_ROOT *mem_root, List<Index_hint> *list) {
  ...
  if (m_indexes.size() == 0 && m_type == Outline_type::INDEX_USE) {
    Index_hint *hint = new (mem_root) Index_hint(NULL, 0);
...
void prepare_hints(THD *thd, Table_ref *table, Outline *outline) {
  if (!table->outline_index_hints)
    table->outline_index_hints = new (thd->mem_root) List<Index_hint>();
  outline->evoke_index_hint(thd->mem_root, table->outline_index_hints);
}
...
void reset_hints(Table_ref *table) {
  if (table->outline_index_hints) {
    table->outline_index_hints->clear();
  }
}

void reset_hints(Query_block *select_lex) {
  if (select_lex->outline_optimizer_list) {
    select_lex->outline_optimizer_list->clear();
  }
}
...
/**
  ...
  @retval         table_ref
*/
static Table_ref *iterate_next(Table_ref *table_ref) {
  return table_ref->next_global;
}
...
/**
  ...
  @retval         Query_block
*/
static Query_block *iterate_next(Query_block *query_block) {
  return query_block->next_query_block();
}
...
void System_outline::find_and_fill_hints(THD *thd, T *lex_objects,
  ...
  auto create_hint_list = [&](
      const Statement_outline_vector &outlines) -> auto {
    /* position in TABLE_LIST/SELECT_LEX global list */
    uint element_nr = 1;
    uint outline_nr = 0;
    element = lex_objects;
    uint outline_size = outlines.size();
    reset_hints(lex_objects);
    /**
  ...
  Statement_outline_vector all_match_outlines;
  // Find all outlines whose schema is not "" and digest is equal to
  // query'digest.
  if (schema.size() && (stmt_outline = find(pos, schema, digest)))
    all_match_outlines.insert(all_match_outlines.end(),
                              stmt_outline->outlines().begin(),
                              stmt_outline->outlines().end());

  // Find all outlines whose schema is "" and digest is equal to query'digest.
  String_outline empty_schema("");
  if ((stmt_outline = find(pos, empty_schema, digest)))
    all_match_outlines.insert(all_match_outlines.end(),
                              stmt_outline->outlines().begin(),
                              stmt_outline->outlines().end());

  if (all_match_outlines.size()) create_hint_list(all_match_outlines);

...
#define EXPLAIN_TOKEN_SIZE SIZE_OF_A_TOKEN
#define ANALYZE_TOKEN_SIZE SIZE_OF_A_TOKEN
#define EXPLAIN_FORMAT_MINIMAL_LENGTH (3 * SIZE_OF_A_TOKEN)
#define IDENT_LENGTH_SIZE SIZE_OF_A_TOKEN
#define LEAST_TOKEN_SIZE SIZE_OF_A_TOKEN
...
/**
  ...
  to "EXPLAIN" or "EXPLAIN ANALYZE [FORMAT = xxx]" or "EXPLAIN FORMAT = xxx".
...
uint calculate_strip_length_for_explain(
  ...
  if (digest_storage && digest_storage->m_byte_count > EXPLAIN_TOKEN_SIZE) {
    ...
    if (tok == DESCRIBE) {
      // ANALYZE
      if (digest_storage->m_byte_count - current_byte >= ANALYZE_TOKEN_SIZE) {
        current_byte = read_token(digest_storage, current_byte, &tok);
        if (tok != ANALYZE_SYM) current_byte -= SIZE_OF_A_TOKEN;
      }
      strip_length = current_byte;
      // FORMAT = { json | traditional | tree }
      if (digest_storage->m_byte_count - current_byte >=
          EXPLAIN_FORMAT_MINIMAL_LENGTH + LEAST_TOKEN_SIZE) {
        /*
          Make sure that there is enough size for "FORMAT = xxx" and at least
          one more token.
         */
        uint tok_format, tok_eq, tok_type = 0;
        current_byte = read_token(digest_storage, current_byte, &tok_format);
        current_byte = read_token(digest_storage, current_byte, &tok_eq);
        current_byte = read_token(digest_storage, current_byte, &tok_type);
        ...
        if (tok_format == FORMAT_SYM && tok_eq == EQ) {
          ...
          if (tok_type == JSON_SYM || tok_type == TOK_GENERIC_VALUE) {
            ...
            if (digest_storage->m_byte_count - current_byte >=
                IDENT_LENGTH_SIZE + LEAST_TOKEN_SIZE) {
              current_byte = read_token(digest_storage, current_byte, &tok);
              if (digest_storage->m_byte_count - current_byte >=
                  tok + LEAST_TOKEN_SIZE) {
                current_byte += tok;
                strip_length = current_byte;
                goto end;
              }
          ...
          // The wrong scenario happened, including (1) Unexpected EXPLAIN
          // format type token (2) Incomplete identifier payload (3) Missing
          // identifier length
          assert(0);
        }
      }

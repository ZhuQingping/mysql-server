...
LEX_CSTRING OUTLINE_SCHEMA_NAME = {STRING_WITH_LEN("mysql")};

/* outline table name */
LEX_CSTRING OUTLINE_TABLE_NAME = {STRING_WITH_LEN("outline")};
...
static const TABLE_FIELD_TYPE
    mysql_outline_table_fields[MYSQL_OUTLINE_FIELD_COUNT] = {
        {{STRING_WITH_LEN("Id")}, {STRING_WITH_LEN("bigint")}, {NULL, 0}},
        ...
        {{STRING_WITH_LEN("Scope")},
         {STRING_WITH_LEN("enum('','FOR JOIN','FOR ORDER BY','FOR GROUP BY')")},
         {STRING_WITH_LEN("utf8mb3")}},
        {{STRING_WITH_LEN("State")},
         {STRING_WITH_LEN("enum('N','Y')")},
         {STRING_WITH_LEN("utf8mb3")}},
        {{STRING_WITH_LEN("Position")}, {STRING_WITH_LEN("bigint")}, {NULL, 0}},
        {{STRING_WITH_LEN("Hint")}, {STRING_WITH_LEN("text")}, {NULL, 0}}};
...
void Outline_reader::log_error(Conf_error err) {
  log_conf_error(ER_APPLY_OUTLINE, err);
}
...
/**
  ...
  @retval         Conf_error
*/
Conf_error open_outline_table(THD *thd, Table_ref_ptr &table_list, bool write) {
...
bool add_outline(THD *thd, Conf_record *r) {
  ...
  assert(table_list->table);
  // temporally disable the binlog replication mark
  // we don't set Sql_log_bin to false, which may cause assertion in
  // MYSQL_BIN_LOG::prepare
  ulonglong old_thd_option_bits = thd->variables.option_bits;
  thd->variables.option_bits = old_thd_option_bits & (~OPTION_BIN_LOG);
  ...
  thd->variables.option_bits = old_thd_option_bits;
  DBUG_RETURN(false);

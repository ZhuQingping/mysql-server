...
bool Sql_cmd_outline_admin_proc::check_access(THD *thd) {
  Security_context *sctx = thd->security_context();

  if (sctx->check_access(SUPER_ACL) ||
      !strcmp("root", thd->security_context()->priv_user().str)) {
    return false;
  }
  my_error(ER_OUTLINE_PRIVILEGE_REQUIRED, MYF(0),
           "command denied to current user");
  return true;
}
...
bool Sql_cmd_optimizer_outline_proc_add::pc_execute(THD *thd) {
  ...
  if ((error = add_outline(thd, (Conf_record *)(record)))) DBUG_RETURN(true);

  // When there is insert/delete on table mysql.outline, the master node
  // triggers the op "flush" that calls this reload operation. We
  // also need to generate one redo log message to sync the modification on
  // table mysql.outline.
...
bool Sql_cmd_outline_proc_flush::pc_execute(THD *thd) {
  ...
  // When there is insert/delete on table mysql.outline, the master node
  // triggers the op "flush" that calls this reload operation. We
  // also need to generate one binog message to sync the modification on
  // table mysql.outline.
...
void Sql_cmd_outline_proc_preview::send_result(THD *thd, bool error) {
  ...
  if (record->query.length == 0) {
    my_error(ER_OUTLINE_PREVIEW_INVALID, MYF(0));
    DBUG_VOID_RETURN;
  }

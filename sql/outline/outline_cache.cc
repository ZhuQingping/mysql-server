...
Query_block *reverse_query_blocks(Query_block *all_selects) {
  std::stack<Query_block *> t_stack;
  Query_block *select_lex = nullptr;
  Query_block **next;
  for (Query_block *sl = all_selects; sl; sl = sl->next_select_in_list()) {
    t_stack.push(sl);
  }
  while ((!t_stack.empty())) {
    Query_block *cur = t_stack.top();
    cur->outline_next_query_block = nullptr;
    if (!select_lex)
      select_lex = cur;
    else
      *next = cur;
    next = &cur->outline_next_query_block;
    t_stack.pop();
  }
  return select_lex;
}
...
static void invoke_optimizer_outlines(THD *thd, Query_block *all_selects,
  ...
  Query_block *select_lex = reverse_query_blocks(all_selects);

  System_outline::instance()
      ->find_and_fill_hints<Query_block, Outline_category::CATEGORY_OPTIMIZER>(
          thd, select_lex, schema, digest);

  select_lex = all_selects;
  while (select_lex) {
    if (select_lex->outline_optimizer_list)
      contextualize_optimizer_hint(thd, select_lex);
...
void invoke_outlines(THD *thd, const char *db,
  ...
  all_selects = thd->lex->all_query_blocks_list;
...
static void generate_outline_preview(
  ...
  if (table_list && table_list->outline_index_hints) {
    List_iterator_fast<Index_hint> it(*table_list->outline_index_hints);
    while ((index_hint = it++)) {
...
bool preview_statement_outline(THD *thd, LEX_CSTRING &db, LEX_CSTRING &query,
  ...
  select_lex = reverse_query_blocks(thd->lex->all_query_blocks_list);
  ...
  while (select_lex) {
    generate_outline_preview(thd, db.str, buff, select_lex->select_number,
                             select_lex, container);
    select_lex = select_lex->outline_next_query_block;
  }

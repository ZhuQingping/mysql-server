...
/* flag bits */
#define HAS_BINLOG_EVENT (1 << 0)
#define IS_BIG_TRX (1 << 1)
#define HAS_DDL_INFO (1 << 2)
#define HAS_GTID_INFO (1 << 3)
#define HAS_GTID_EXECUTED (1 << 4)

#define my_dstore_support_binlog_check_trace(format, ...)                \
  if (rds_binlog_dstore_trace) {                                         \
    my_dstore_support_binlog_check_trace_impl(INFORMATION_LEVEL, format, \
                                              __VA_ARGS__);              \
  }
...
#define FLAG_LEN 4
#define GTID_LEN 2
#define GTID_EXECUTED_WARNING_LEN 51200
/* The wal buffer size is 63K, the maximum possible size of ddl_info can be 63K
 * when only ddl info in the wal */
#define MAX_DDL_INFO_SIZE 64512

/* The buffer size break down for different elements in the wal buffer */
/* flag */
#define BUF_SIZE_FOR_FLAG 4
/* gno */
#define BUF_SIZE_FOR_GNO 8
/* gtid_len */
#define BUF_SIZE_FOR_GTID_LEN 2
/* trx_len */
#define BUF_SIZE_FOR_TRX_LEN 4
/* transaction id */
#define BUF_SIZE_FOR_TRX_ID 8
/* ddl_info_len */
#define BUF_SIZE_FOR_DDL_SQL_LEN 2
...
extern char *rds_dstore_missed_gtids_on_binlog_ptr;
...
class MYSQL_BIN_LOG : public TC_LOG {
 ...
 private:
  ...
  /** The instrumentation key to use for @ LOCK_redo. */
  PSI_mutex_key m_key_LOCK_redo;
  ...
  /** The instrumentation key to use for @ LOCK_tids. */
  PSI_mutex_key m_key_LOCK_tids;
  ...
  */
  ...
  mysql_cond_t m_prep_tids_cond;
  std::atomic<int32> m_atomic_prep_tids{0};
  ...
  /**
    Increase the prepared TID counter.
   */
  void inc_prep_tids(THD *thd);

  /**
    Decrease the prepared TID counter.

    Signal m_prep_tids_cond if the counter reaches zero.
   */
  void dec_prep_tids(THD *thd);

  int32 get_prep_tids() { return m_atomic_prep_tids; }
 ...
 public:
  ...
  uint32 consecutive_reset_numbers;
  /* This is relay log */
  bool is_relay_log;
  ...
  */
  ...
  void set_psi_keys(
      ...
      PSI_mutex_key key_LOCK_binlog_end_pos, PSI_mutex_key key_LOCK_redo,
      PSI_mutex_key key_LOCK_sync, PSI_mutex_key key_LOCK_sync_queue,
      PSI_mutex_key key_LOCK_xids, PSI_mutex_key key_LOCK_tids,
    ...
    m_key_LOCK_redo = key_LOCK_redo;
    m_key_LOCK_sync = key_LOCK_sync;
    m_key_LOCK_xids = key_LOCK_xids;
    m_key_LOCK_tids = key_LOCK_tids;
  ...
  */
  ...
  /**
    ...
    log name to rds_dstore_missed_gtids_on_binlog.

    @param[in]  missed gtids in dstore storage engine.
  */
  void find_and_set_log_located_by_dstore_missed_gtids(
  ...
  */
  ...
  /**
    Processes the Binlog commit flush stage for a single thread:

    1. Assigns the next GNO to the THD::gtid_next.
    2. Flushes the thread caches
    3. Checks if there is the need to rotate the current binary-log file after
    the current THD caches are flushed.

    @param thd The session to processes the stage for.
    @param total_bytes[out] The total size, in bytes, of the transaction binlog
                            caches.

    @return Zero if the operation succeeds, non-zero error code if the flush
            fails.
   */
  int process_flush_stage_for_thd(THD *thd, my_off_t &total_bytes);
  /**
    Processes the Binlog commit flush stage for a single thread:

    1. Calls the AFTER_FLUSH hooks, if any.
    2. Updates the binary-log end position if not postponed to after sync.

    @param thd The session to processes the stage for.
    @param flush_end_pos The binlog end position after the flush.
    @param update_binlog_end_pos_after_sync Whether we should update the end
                                            position now or after sync.

    @return Zero if the operation succeeds, non-zero error code if the flush
            fails.
   */
  int process_after_flush_stage_for_thd(THD *thd, my_off_t flush_end_pos,
                                        bool update_binlog_end_pos_after_sync);
  /**
    Processes the Binlog commit sync stage for a single thread:

    1. Sync the binary-log file.
    2. Updates the binary-log end position if was not already done.

    @param thd The session to processes the stage for.
    @param total_bytes The total size, in bytes, of the transaction binlog
                       caches.
    @param update_binlog_end_pos_after_sync Whether we should update the end
                                            position now or if it was updated
                                            already.

    @return Zero if the operation succeeds, non-zero error code if the flush
            fails.
   */
  int process_sync_stage_for_thd(THD *thd, my_off_t total_bytes,
                                 bool update_binlog_end_pos_after_sync);
  /**
    Processes the Binlog commit stage for a single thread:

    1. Updates the maximum committed sequence number for dependency tracking.
    2. Commits the transaction in the storage engines.
    3. Updates the internal structures that manage the GTID_EXECUTED set.
    4. Sets the transaction as prepared in the transaction-coordinator.
    5. Decrements the count of prepared XID.

    @param thd The session to processes the stage for.
   */
  void process_commit_stage_for_thd(THD *thd);
  /**
    Processes the Binlog after-commit stage for a single thread:

    1. Calls the AFTER_COMMIT hooks, if any.

    @param thd The session to processes the stage for.
   */
  void process_after_commit_stage_for_thd(THD *thd);
  /**
    Processes the Binlog commit rotate stage for a single thread:

    1. If the rotation is forced or the transaction binary-log cache size makes
       the binary-log file exceed the max size, rotates the binary-log.
    2. If the rotate is called and marks the binary-logs for purging, purge.

    @param thd The session to processes the stage for.
   */
  void process_rotate_stage_for_thd(THD *thd);
  ...
  */
  ...
  /**
    Flush and commit the transaction in group with one phase commit.

    This shall execute an ordered flush and commit all outstanding
    transactions in group and is the main function for the binary
    log group commit logic with one phase commit and include three
    main stages.

    Stage 1:
      -- Generate GTIDs for all transactions in the queue.
      -- Write the session caches for all transactions in
         the queue to the redo log.

    Stage 2:
      -- Fetching the final queue with transactions to be committed.

    Stage 3:
      -- Commit all transactions in group.
    ...
    @param[in] skip_commit
                   ...
                   be skipped and @c false otherwise (the normal case).
  */
  int group_commit(THD *thd, bool all, bool skip_commit = false);

  /**
    Flush and commit the transaction concurrently with one phase commit.

    Include three main steps as following:
      -- Assign a GTID for the transaction.
      -- Flush the session cache of the transaction into wal.
  ...
  */
  int concurrent_commit(THD *thd, bool all, bool skip_commit = false);

  /**
    Assign automatic generated GTID for the thread which has
    the AUTOMATIC_GTID type.

    @param thd The thread.
    @return Returns false if succeeds, otherwise true is returned.
  */
  bool assign_automatic_gtid(THD *thd);

  /**
    Flush and commit the transaction.

    This will execute an ordered flush and commit of the parameter underlying
    transaction and follows the same stages as the `ordered_commit`. The
    function performs the ordered commit in four stages.
    ...
    The stage#2 syncs the binary log.

    The stage#3 executes under LOCK_commit and commits the transaction is order.
    ...
    @return Zero if the operation was successful, error code otherwise.
   */
  int single_commit(THD *thd, bool all, bool skip_commit = false);
  ...
  */
  ...
  /**
   * @brief Write a group of log events of a transaction into the current binary
   * log file.
   *
   * This function locks the binlog mutex to ensure thread safety, writes the
   * provided buffer content into the binary log, and updates the position and
   * end position of the binary log. It also handles the rotation of the binary
   * log if necessary.
   *
   * @param buffer The buffer containing the binary log event data.
   * @param gtid_len The length of the GTID (Global Transaction ID) in the
   * buffer.
   * @param [in,out] position A reference to the current position in the buffer,
   * which will be updated by this function.
   */
  void write_into_binlog(const unsigned char *buffer, uint16_t gtid_len,
                         uint32_t &position);
...
/**
  ...
  @return
    @retval 0    OK. always, the purge binlog thread will swallow any error
...
int write_extra_commit_wal_callback(uint64 end_plsn,
                                    const unsigned char *buffer, uint32 length,
                                    uint64 &sync_plsn);
...
/**
   Write DDL information into the redo.

   If the rds_write_binlog_into_redo=ON is set, the DDL info is appended after
   the binlog event and then written together into the redo log, at which point
   the Binlog_event_writer needs to be provided; when the
   rds_write_binlog_into_redo is turned off or binlog is not enabled, the DDL
   info is written into the redo log alone.

   @param thd    Pointer to the THD object containing the DDL
                 information to be written.
   @param writer Pointer to the binlog event writer. If provided, the DDL
                 information is appended after the binlog event;
                 otherwise(nullptr), it is directly written into the redo log.
 */
void ddl_info_to_redo(THD *thd, Binlog_event_writer *writer);

/**
 * Writes GTID information into the redo log.
 *
 * This function allocates a buffer to store the GTID information, including
 * the flag, GNO (Global Transaction Number), and UUID (Universally Unique
 * Identifier).
 *
 * @param thd Pointer to the THD (Thread) object representing the current
 * session.
 */
void write_gtid_info_into_redo(THD *thd);

/**
 * Writes both GTID information and DDL information into the redo log.
 *
 * This function allocates a buffer large enough to hold GTID (Global
 * Transaction ID), transaction ID (trx_id), and DDL (Data Definition Language)
 * information. It then constructs the buffer by writing these components
 * sequentially and commits the data to the redo log.
 *
 * @param thd Pointer to the THD (Thread) object representing the current
 * session.
 */
...
/**
 * Writes GTID executed information into the redo log.
 *
 * This function writes the GTID executed set into the redo log. It first sets a
 * flag to indicate the presence of GTID executed information, acquires a write
 * lock on the global SID lock to safely access the GTID set, and allocates a
 * buffer of dynamic size based on the `binlog_write_redo_buffer_size`
 * configuration. The GTID set is then converted to a string and stored in the
 * buffer, along with the flag and length information. Finally, the data is
 * committed to the redo log, and the buffer is freed.
 *
 * @param thd Pointer to the THD (Thread) object representing the current
 * session.
 */

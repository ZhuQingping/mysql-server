...
struct Wallog_IO_observer;
struct Binlog_relay_IO_param;
struct Wallog_IO_param;
struct Binlog_storage_observer;
struct Wallog_trx_observer;
struct Binlog_transmit_observer;
struct Wallog_transmit_observer;
...
#ifdef HAVE_PSI_RWLOCK_INTERFACE
extern PSI_rwlock_key key_rwlock_Wallog_IO_delegate_lock;
#endif

class Wallog_IO_delegate : public Delegate {
 public:
  Wallog_IO_delegate()
      : Delegate(
#ifdef HAVE_PSI_RWLOCK_INTERFACE
            key_rwlock_Wallog_IO_delegate_lock
#endif
        ) {
  }

  typedef Wallog_IO_observer Observer;
  int wal_thread_start(THD *thd, Master_info *mi);
  int wal_thread_stop(THD *thd, Master_info *mi);
  int before_request_wal_transmit(THD *thd, Master_info *mi, ushort flags);
  int after_read_wal(THD *thd, Master_info *mi, const char *packet, ulong len,
                     const char **wal_buf, ulong *wal_len);
  int after_write_wal(THD *thd, Master_info *mi, const char *wal_buf,
                      ulong wal_len, uint64_t received_lsn,
                      uint64_t flushed_lsn, uint64_t replayed_lsn);
  int after_reset_wal_slave(THD *thd, Master_info *mi);
  int applier_wal(THD *thd, int &out);

 private:
  void init_param(Wallog_IO_param *param, Master_info *mi);
};

#ifdef HAVE_PSI_RWLOCK_INTERFACE
extern PSI_rwlock_key key_rwlock_Wallog_trx_delegate_lock;
#endif

class Wallog_trx_delegate : public Delegate {
 public:
  Wallog_trx_delegate()
      : Delegate(
#ifdef HAVE_PSI_RWLOCK_INTERFACE
            key_rwlock_Wallog_trx_delegate_lock
#endif
        ) {
  }

  typedef Wallog_trx_observer Observer;
  int trx_wait(uint64_t target_lsn);
  int get_standby_flushed_lsn(uint64_t *standby_flushed_lsn);
};

#ifdef HAVE_PSI_RWLOCK_INTERFACE
extern PSI_rwlock_key key_rwlock_Wallog_transmit_delegate_lock;
#endif

class Wallog_transmit_delegate : public Delegate {
 public:
  Wallog_transmit_delegate()
      : Delegate(
#ifdef HAVE_PSI_RWLOCK_INTERFACE
            key_rwlock_Wallog_transmit_delegate_lock
#endif
        ) {
  }

  typedef Wallog_transmit_observer Observer;
  int wallog_transmit_start(THD *thd, ushort flags, uint64 log_pos,
                            bool *observe_transmission);
  int wallog_transmit_stop(THD *thd, ushort flags);
  int wallog_reserve_header(THD *thd, ushort flags, String *packet);
  int wallog_before_send_event(THD *thd, ushort flags, String *packet,
                               uint64 log_pos);
  int wallog_after_send_event(THD *thd, ushort flags, String *packet);
  int wallog_after_reset_master(THD *thd, ushort flags);
};
...
extern Wallog_trx_delegate *wallog_trx_delegate;
extern Server_state_delegate *server_state_delegate;
extern Binlog_transmit_delegate *binlog_transmit_delegate;
extern Wallog_transmit_delegate *wallog_transmit_delegate;
extern Binlog_relay_IO_delegate *binlog_relay_io_delegate;
extern Wallog_IO_delegate *wallog_io_delegate;

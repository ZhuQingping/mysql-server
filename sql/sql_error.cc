...
void Diagnostics_area::reset_diagnostics_area() {
  ...
  if (!accumulate_counts()) {
    /* If accumulating counts, refrain from resetting affected_rows and
       cond_count. For example, in a PS bulk execution of an UPDATE, we want
       to report to the client the total number of modified rows and of
       warnings ; and note that such reset code is called at the beginning of
       each iteration of the bulk execution... */
    m_affected_rows = 0;
    m_last_statement_cond_count = 0;
  }
  m_last_insert_id = 0;

...
bool Events::opt_event_disable_slave = true;
  ...
  */
  ...
  // Binlog the create event.
  {
    ...
    if (create_query_string(thd, &log_query)) {
      ...
      if (DBUG_EVALUATE_IF("create_event_set_ddl_sql_error",
                           thd->set_ddl_sql(String().lex_cstring()),
                           thd->set_ddl_sql(log_query.lex_cstring()))) {
        LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
               "Failed to set DDL info during EVENT creation.");
        goto err_with_rollback;
      }
...
bool Events::drop_event(THD *thd, LEX_CSTRING dbname, LEX_CSTRING name,
  ...
  {
    assert(thd->query().str && thd->query().length);
    if (DBUG_EVALUATE_IF("drop_event_set_ddl_sql_error",
                         thd->set_ddl_sql(String().lex_cstring()),
                         thd->set_ddl_sql(thd->query()))) {
      LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
             "Failed to set DDL info during EVENT drop.");
      goto err_with_rollback;
    }

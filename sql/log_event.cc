...
bool Query_log_event::write(Basic_ostream *ostream) {
  ...
  */
  ...
  if (thd && needs_rds_recycle_bin_mode) {
    *start++ = Q_RECYCLE_BIN_MODE;
    *start++ = thd->variables.rds_recycle_bin_mode;
  }
...
/**
  Returns whether or not the statement held by the `LEX` object parameter
  requires `Q_RECYCLE_BIN_MODE` to be logged together with the statement.
 */
static bool is_rds_recycle_bin_mode_needed(const LEX *lex) {
  if (!rds_recycle_bin_binlog_session_variables) {
    return false;
  }
  enum enum_sql_command cmd = lex->sql_command;
  switch (cmd) {
...
Query_log_event::Query_log_event(THD *thd_arg, const char *query_arg,
  ...
  */
  ...
  needs_rds_recycle_bin_mode = is_rds_recycle_bin_mode_needed(lex);
...
void Query_log_event::print_query_header(
  ...
  */
  ...
  if (rds_recycle_bin_mode != print_event_info->rds_recycle_bin_mode) {
    my_b_printf(file, "SET @@session.rds_recycle_bin_mode=%d%s\n",
                rds_recycle_bin_mode, print_event_info->delimiter);
  }
}
...

  {
    ...
    if (ignored_error_code((expected_error)) ||
      ...
      */
      ...
      if (rds_recycle_bin_mode != 0xff) {
        assert(rds_recycle_bin_mode == 0 || rds_recycle_bin_mode == 1);
        thd->variables.rds_recycle_bin_mode = rds_recycle_bin_mode;
      } else {
        thd->variables.rds_recycle_bin_mode =
            global_system_variables.rds_recycle_bin_mode;
      }
  ...
  if ((server_id != ::server_id || rli->replicate_same_server_id) &&
    ...
    */
    if ((error = rli->inc_group_relay_log_pos(
      ...
      if (thd->backup_binlog_lock.is_protection_acquired()) {
...
bool Gtid_log_event::write(Basic_ostream *ostream) {
  if (rds_write_binlog_into_redo) {
    // write flags
    uint32_t flags = HAS_BINLOG_EVENT;
    // store flag, uuid, gno, gtid len, 128 is big enough.
    uchar buf[128];
    if (transaction_length > binlog_cache_size) {
      flags |= BIG_TRANSACTION;
    }
    uint16_t position = 0;
    int4store(buf + position, flags);
    position += FLAG_LEN;

    int8store(buf + position, spec.gtid.gno);
    position += ENCODED_GNO_LENGTH;

    sid.to_string((char *)buf + position);

    position += UUID_LEN;
    // write gtid_len
    uint16_t gtid_length = get_event_length();
    int4store(buf + position, gtid_length);
    position += GTID_LEN;
    ostream->write((uchar *)buf, position);
  }

...
bool Event_db_repository::update_event(THD *thd, Event_parse_data *parse_data,
  ...
  */
  ...
  if (thd->for_ddl_info_replay && sctx->is_skip_grants_user() &&
      strcasecmp(thd->lex->definer->user.str, "skip-grants user") == 0 &&
      strcasecmp(thd->lex->definer->host.str, "skip-grants host") == 0) {
    thd->lex->definer->user = {definer.user().c_str(), definer.user().length()};
    thd->lex->definer->host = {definer.host().c_str(), definer.host().length()};
  }

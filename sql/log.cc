...
inline int make_iso8601_timestamp_safe(char (&buf)[iso8601_size],
                                       ulonglong utime,
                                       enum enum_iso8601_tzmode mode) {
  return make_iso8601_timestamp(buf, utime, mode);
}
...
bool File_query_log::write_oma(THD *thd, ulonglong current_utime,
  ...
  if (!(specialflag & SPECIAL_SHORT_LOG_FORMAT)) {
    ...
    make_iso8601_timestamp_safe(my_timestamp, current_utime,
                                iso8601_sysvar_logtimestamps);
    my_timestamp[iso8601_size - 1] = '\0';

    /* Write timestamp line to log */
    int result = snprintf(buff, sizeof(buff), "# Time: %s\n", my_timestamp);
    if (result < 0) goto err; /* Check for snprintf error */
    buff_len = static_cast<size_t>(result);
    /* Note that my_b_write() assumes it knows the length for this */
    if (my_b_write(&log_file, (uchar *)buff, buff_len)) goto err;

    /* Write user/host and thread ID information */
    result = snprintf(buff, sizeof(buff), "%5u", thd->thread_id());
    if (result < 0) goto err; /* Check for snprintf error */
    buff_len = static_cast<size_t>(result);

    if (my_b_printf(&log_file, "# User@Host: %s  Id: %s\n", user_host, buff) ==
        static_cast<size_t>(-1)) /* Use explicit cast for unsigned comparison */
      goto err;
  }
...
static bool log_oma_is_filtered_user(THD *thd) {
  ...
  /*
    many users which don't start with 'm' can skip full string comparison.
    The following situations do not record OMA logs:
    1. mysqld process initialization (user.length=0 and host.length=0).
    2. Binlog replay (skip-grants user).
    3. System memory table initialization (skip-grants user).
  */
  bool is_sysuser = ((cur_user.str[0] == 's') &&
                     !(strcmp(cur_user.str, "skip-grants user"))) ||
                    (cur_user.length == 0);
  bool is_syshost = ((cur_host.str[0] == 's') &&
                     !(strcmp(cur_host.str, "skip-grants host"))) ||
                    (cur_host.length == 0);
...
void log_oma(THD *thd, bool force_write) {
  ...
  if (log_oma_applicable(thd, log_rewritten_query)) {
    if (log_oma_is_filtered_user(thd)) {
      return;
    }
    THD_STAGE_INFO(thd, stage_logging_oma);

...
bool check_change_password(THD *thd, const char *host, const char *user,
  ...
  if (!thd->slave_thread &&
    ...
    if (opt_rds_permission_control &&
        (check_reserved_user_db(thd, user, "mysql") != 0)) {
      return true;
    }
...
/*
  Check whether the current thread is a slave thread.

  @param thd               The current thread.

  @retval false            Not a slave thread.
  @retval true             It's a slave thread.
*/
...
/*
  Check whether current user is allowed to create empty user.

  @param thd               The current thread.
  @param user_from         Current user name.
  @param user_to           Change to user name.
  @param wrong_users       Wrong users name.

  @retval false            Allowd.
  @retval true             Not allowed.
*/
static bool validate_user_transfer_restriction(THD *thd, LEX_USER *user_from,
                                               LEX_USER *user_to,
                                               String &wrong_users) {
  if (!opt_rds_allow_empty_users && !is_slave_applier(thd) &&
      !is_rds_reserved_user(thd->security_context()->user().str) &&
      !strcmp(user_to->user.str, "")) {
  ...
  { /* Critical section */
    ...
    while ((tmp_user_name = user_list++)) {
      ...
      */
      ...
      /*
        When opt_rds_allow_empty_users is set to true, empty users cannot be
        created unless the current user is a rds reserved user or the function
        is being executed by a slave thread.
      */
      if (validate_user_transfer_restriction(thd, user_name, user_name,
                                             wrong_users)) {
        result = 1;
        continue;
      }
  ...
  { /* Critical section */
    ...
    while ((tmp_user_from = user_list++)) {
      ...
      */
      ...
      /*
        When opt_rds_allow_empty_users is set to true, a user cannot be renamed
        as an empty user unless the current user is a rds reserved user or the
        function is being executed by a slave thread.
      */
      if (validate_user_transfer_restriction(thd, user_from, user_to,
                                             wrong_users)) {
        result = 1;
        continue;
      }

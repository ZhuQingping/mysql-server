...
ACL_internal_access_result Mysql_internal_schema_access::check(
  ...
  /*
    Do not perform permission control on mysql db during instance
    initialization or instance restart.
  */
  if (mysqld_server_started && opt_rds_permission_control) {
    if (!precheck_user_permission(current_thd)) {
      /*
        We don't allow any write privileges on the mysql database
        except for built-in operations.
      */
      if ((current_thd->lex->sql_command == SQLCOM_CREATE_USER) ||
          (current_thd->lex->sql_command == SQLCOM_DROP_USER) ||
          (current_thd->lex->sql_command == SQLCOM_RENAME_USER) ||
          (current_thd->lex->sql_command == SQLCOM_ALTER_USER) ||
          (current_thd->lex->sql_command == SQLCOM_REVOKE_ALL) ||
          (current_thd->lex->sql_command == SQLCOM_REVOKE) ||
          (current_thd->lex->sql_command == SQLCOM_GRANT) ||
          (current_thd->lex->sql_command == SQLCOM_SET_PASSWORD)) {
        return ACL_INTERNAL_ACCESS_CHECK_GRANT;
      }
      if (check_internal_schema_permission(want_access)) {
...
const ACL_internal_table_access *Mysql_internal_schema_access::lookup(
    const char *) const {
  /* There are no per table rules for the mysql. */
  return nullptr;
}

ACL_internal_access_result Sys_internal_schema_access::check(
    Access_bitmask want_access, Access_bitmask *, bool) const {
  /*
    Do not perform permission control on sys db during instance
    initialization and instance restart.
  */
  if (mysqld_server_started && opt_rds_permission_control) {
    if (!precheck_user_permission(current_thd)) {
      /*
        We don't allow any write privileges on the sys database.
      */
      if (check_internal_schema_permission(want_access)) {
        return ACL_INTERNAL_ACCESS_DENIED;
      }
...
const ACL_internal_table_access *Sys_internal_schema_access::lookup(
    const char *) const {
  /* There are no per table rules for the sys. */
  return nullptr;
}
...
bool check_routine_access(THD *thd, Access_bitmask want_access, const char *db,
  ...
  /*
    ...
    Since the sys and mysql contain routines, so need do check access.
  */
  assert((want_access & CREATE_PROC_ACL) == 0);
  if (opt_rds_permission_control && (is_sys_db(db) || is_mysql_db(db))) {
      ...
      if (!db &&
        ...
        while ((priv = priv_it++) && !error) {
          ...
          if (revoke_grant) {
            ...
            /*
             If opt_rds_restricted_dynamic_privileges_control is enabled,
             we should reject to grant restricted dynamic privileges.
             Except:
             1. It's rds reserved users, we allow they do anything.
             2. It's 'GRANT ALL' query, because it will copy all currently
                available dynamic privileges to list.
             3. It's during initialization, because it will also grant the
                dynamic privileges during initialization.
           */
            if (opt_rds_restricted_dynamic_privileges_control &&
                !thd->security_context()->check_access(SUPER_ACL) &&
                !is_rds_reserved_user(sctx_user.str) &&  // NOT Reserved User
                !grant_all_current_privileges &&         // NOT 'GRANT ALL'
                (!opt_initialize && !opt_initialize_insecure) &&
                !thd->is_dd_system_thread() &&
                !thd->is_server_upgrade_thread() &&
                is_in_restricted_dynamic_privileges_list(priv->str)) {
              error = true;
              my_error(ER_NOT_ALLOWED_GRANT_RDS_RESTRICTED_DYNAMIC_PRIVILEGES,
                       MYF(0), priv->str);
              break;
            }
...
bool sp_revoke_privileges(THD *thd, const char *sp_db, const char *sp_name,
  ...
  */
  ...
  /*
   Disable DDL info writing before calling log_and_commit_acl_ddl().
   Reason: the function implicitly commits SP-related ACL operations, so
   separate DDL info is redundant.
   Note: do this at the call site (not inside log_and_commit_acl_ddl())
   to avoid impacting other ACL operations (e.g., CREATE/DROP USER) that
   depend on it for actual(explicit) commits.
  */

...
static bool send_server_handshake_packet(MPVIO_EXT *mpvio, const char *data,
  ...
  char MARIADB_SUFFIX[] = " (MariaDB parts)";
  char *buff = (char *)my_alloca(1 + SERVER_VERSION_LENGTH +
                                 sizeof(MARIADB_SUFFIX) + data_len + 64);
  ...
  *end++ = protocol_version;
  ...
  protocol->set_extended_client_capabilities(0);
  ...
  /*
    COM_STMT_BULK_EXECUTE is to adapt to the MariaDB protocol. The MariaDB
    client will use the COM_STMT_BULK_EXECUTE protocol only if the following
    conditions are met in handshake packet:
    a) Server version string contains 'MariaDB' or '-mariadb-'.
    b) CLIENT_LONG_PASSWORD flag is not set in capability field.
    c) CLIENT_STMT_BULK_OPERATIONS flag is set in extended capability field.

    There are 2 variables to control COM_STMT_BULK_EXECUTE feature:
    1) rds_imitate_mariadb_server: Whether to tell the client that this is
       MariaDB server, corresponding to a) and b).
    2) rds_support_bulk_execution: Whether to tell the client that server
       supports COM_STMT_BULK_EXECUTE protocol, corresponding to c).

    If it's MariaDB client, such as MariaDB c client or MariaDB jdbc, to enable
    COM_STMT_BULK_EXECUTE, MySQL server needs to enable
    rds_imitate_mariadb_server and rds_support_bulk_execution. If it's MySQL
    client that supports COM_STMT_BULK_EXECUTE protocol, enabling
    rds_support_bulk_execution is enough.
  */
  bool imitate_mariadb = rds_imitate_mariadb_server;
  bool support_bulk_execute = current_thd->variables.rds_support_bulk_execution;
  if (imitate_mariadb) {
    /*
      To adapt to the mariadb client, add a suffix to the version string.
    */
    end = my_stpnmov(end - 1, MARIADB_SUFFIX, sizeof(MARIADB_SUFFIX)) + 1;
  }
  ...
  *end++ = 0;
  ...
  ulong client_capabilities = protocol->get_client_capabilities();
  if (imitate_mariadb) {
    /*
      To adapt to the mariadb client, unset CLIENT_LONG_PASSWORD flag in
      capability field.
    */
    client_capabilities &= ~((ulong)CLIENT_LONG_PASSWORD);
  }

  int2store(end, static_cast<uint16>(client_capabilities));
  ...
  memset(end + 8, 0, 10);
  if (support_bulk_execute) {
    /*
      To adapt to the mariadb client, set CLIENT_STMT_BULK_OPERATIONS flag in
      extended capability field.
    */
    int4store(end + 14, CLIENT_STMT_BULK_OPERATIONS >> 32);
  }
  end += 18;
  /* write scramble tail */
  end = (char *)memcpy(end, data + AUTH_PLUGIN_DATA_PART_1_LENGTH,
  ...
  */
  ...
  if (protocol->has_client_capability(CLIENT_PROTOCOL_41)) {
    ...
    if (thd->variables.rds_support_bulk_execution) {
      protocol->set_extended_client_capabilities((uint64_t)uint4korr(end + 28)
                                                 << 32);
    }
...
static inline bool check_restrictions_for_com_connect_command(THD *thd) {
  ...
  if (!thd->is_admin_connection() &&
      ...
      !Connection_handler_manager::get_instance()
           ->check_and_incr_reserved_conn_count(thd)) {

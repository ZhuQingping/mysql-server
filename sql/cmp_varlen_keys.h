...
/**
  ...
  @return value according to their key, s.t.,
      *   -  if s1 < s2;
      *   0  if s1 = s2;
      *   +  if s1 > s2;
 */
inline bool cmp_varlen_keys(
    ...
    const uchar *s1, const uchar *s2, int *cmp_value = nullptr) {
  ...
  for (const st_sort_field &sort_field : sort_field_array) {
    ...
    if (sort_field.maybe_null) {
      ...
      if (k1_nullbyte != k2_nullbyte) {
        if (cmp_value) *cmp_value = k1_nullbyte - k2_nullbyte;
    ...
    if (res) {
      if (cmp_value) *cmp_value = res;
      return res < 0;
    }
    if (kp1_len != kp2_len) {
      if (sort_field.reverse) {
        if (cmp_value) *cmp_value = kp2_len - kp1_len;
        return kp2_len < kp1_len;
      } else {
        if (cmp_value) *cmp_value = kp1_len - kp2_len;
  ...
  if (use_hash) {
    ...
    int cmp_result = memcmp(kp1, kp2, 8);
    if (cmp_value) *cmp_value = cmp_result;
    return cmp_result < 0;
  } else {

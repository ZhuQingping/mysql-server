...
using im::recycle_bin::Recycle_check_result;
using std::max;
using std::min;
using std::string;
using std::to_string;
...
static bool trans_intermediate_ddl_commit(THD *thd, bool error) {
  ...
  /*
    For non-atomic DDL operations, intermediate commits are performed
    during execution. DDL info check writing must be disabled in this
    context.
   */
...
static bool rea_create_base_table(
  ...
  /* During full synchronization, DRS synchronizes tables in the recycle bin
     by executing the create table command. We need to parse the table name
     and fill the recycle bin options fields. */
  if (im::recycle_bin::is_recycle_db(db)) {
    uint64_t recycled_time;
    auto origin_name = im::recycle_bin::parse_origin_table_by_table_name(
        table_name, &recycled_time);
    if (origin_name.first.length() && origin_name.second.length()) {
      table_def->options().set(im::recycle_bin::ORIGIN_SCHEMA.str,
                               origin_name.first.c_str());
      table_def->options().set(im::recycle_bin::ORIGIN_TABLE.str,
                               origin_name.second.c_str());
      table_def->set_last_altered(dd::my_time_t_to_ull_datetime(recycled_time));
    }
  }
 ...
 public:
  ...
  bool write_bin_log() {
    if (m_bin_log_is_open) {
      ...
      */
      if (m_thd->recycle_state->get_type() ==
          im::recycle_bin::RECYCLE_BIN_PURGE) {
        if (m_thd->binlog_query(THD::STMT_QUERY_TYPE, m_thd->query().str,
                                m_thd->query().length, m_is_trans,
                                false /* direct */, m_no_db /* suppress_use */,
                                0 /* errcode */))
          return true;
      } else if (m_thd->binlog_query(
                     THD::STMT_QUERY_TYPE, m_built_query.ptr(),
                     m_built_query.length(), m_is_trans, false /* direct */,
                     m_no_db /* suppress_use */, 0 /* errcode */))
        return true;
    }

    if (need_ddl_info_sql(m_thd)) {
      return generate_dstore_drop_ddl_info(m_thd);
    }
    return false;
  }
        ...
        */
        ...
        if (drop_ctx.has_no_gtid_single_table_group() ||
          ...
          */
          ...
          /* Disable DDL info writing for non-atomic table drops which commit
  ...
  if (drop_ctx.has_base_atomic_tables() || drop_ctx.has_views() ||
    ...
    */
    ...
    for (Table_ref *table : drop_ctx.base_atomic_tables) {
      ...
      if (res == im::recycle_bin::Recycle_check_result::SUPPORTED) {
        if (im::recycle_bin::recycle_base_table(thd, table, post_ddl_htons,
                                                fk_invalidator)) {
          thd->clear_error();
          my_error(ER_RECYBIN_DROP_TABLE_FAILED, MYF(0));
          goto err_with_rollback;
        }
      } else if (res == im::recycle_bin::Recycle_check_result::CHECK_ERROR) {
        my_error(ER_RECYBIN_CHECK_FAILED, MYF(0));
        goto err_with_rollback;
      } else {
    ...
    if (drop_ctx.drop_database) {
      ...
      */
      // still need to record the dstore_ddl_info
      if (need_ddl_info_sql(thd)) {
        if (generate_dstore_drop_ddl_info(thd)) return true;
      }
...
static bool prepare_key(
  ...
  */
  ...
  key_info->dstore_ddl_comment.length =
      key->key_create_info.dstore_ddl_comment.length;
  key_info->dstore_ddl_comment.str =
      key->key_create_info.dstore_ddl_comment.str;
  ...
  */
  ...
  // dstore not support KEY_BLOCK_SIZE
  if (key->key_create_info.block_size && create_info->db_type &&
      create_info->db_type->db_type == DB_TYPE_DSTORE) {
    if (thd->variables.sql_mode & MODE_STRICT_ALL_TABLES) {
      my_error(ER_CHECK_NOT_IMPLEMENTED, MYF(0), "KEY_BLOCK_SIZE");
      if (thd->is_error()) {
        return true;
      }
    } else {
      push_warning_printf(
          thd, Sql_condition::SL_WARNING, ER_CHECK_NOT_IMPLEMENTED,
          ER_THD(thd, ER_CHECK_NOT_IMPLEMENTED), "KEY_BLOCK_SIZE");
    }
  }
...
bool mysql_prepare_create_table(
  ...
  */
  ...
  if (create_info->row_type == ROW_TYPE_DYNAMIC)
    create_info->table_options |= HA_OPTION_PACK_RECORD;
  // ROW FORMAT is not supported in DSTORE
  if (create_info->row_type != ROW_TYPE_DEFAULT && create_info->db_type &&
      create_info->db_type->db_type == DB_TYPE_DSTORE) {
    if (thd->variables.sql_mode & MODE_STRICT_ALL_TABLES) {
      my_error(ER_CHECK_NOT_IMPLEMENTED, MYF(0), "ROW_FORMAT");
      return true;
    } else {
      push_warning_printf(thd, Sql_condition::SL_WARNING,
                          ER_CHECK_NOT_IMPLEMENTED,
                          ER_THD(thd, ER_CHECK_NOT_IMPLEMENTED), "ROW_FORMAT");
    }
  }
...
static bool create_table_impl(
  ...
  if (part_info) {
    ...
    */
    ...
    if (part_info->check_partition_info(
            thd, &engine_type, file.get(), create_info, false,
            part_info->get_use_default_subparts(),
            part_info->get_use_default_num_subparts()))
      return true;
    ...
    if (create_info->db_type != engine_type) {
      ...
      */
      ...
      /*
        For Dstore Engine We should recover the tablespace when we only specify
        engine = innodb in partition clause.
      */
      if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
          create_info->db_type->db_type == DB_TYPE_DSTORE &&
          (create_info->tablespace != nullptr &&
           strcmp(create_info->tablespace, "dstore_default") == 0)) {
        create_info->tablespace = nullptr;
      }
...
bool mysql_create_table(THD *thd, Table_ref *create_table,
  ...
  /*
    For Dstore engine table without specified the tablespace name,
    we need to set dstore_default tablespace as default.
  */
  bool is_temp_table = create_info->options & HA_LEX_CREATE_TMP_TABLE;
  DBUG_EXECUTE_IF("mock_tablespace_len0", create_info->tablespace = "";);
  if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
      create_info->db_type->db_type == DB_TYPE_DSTORE && !is_temp_table &&
      (create_info->tablespace == nullptr ||
       strlen(create_info->tablespace) == 0)) {
    create_info->tablespace = "dstore_default";
...
static bool alter_table_drop_histograms(THD *thd, Table_ref *table,
  ...
  bool alter_drop_index = (alter_info->flags & Alter_info::ALTER_DROP_INDEX);
  bool convert_character_set =
...
bool mysql_rename_table(THD *thd, handlerton *base, const char *old_db,
  ...
  */
  ...
  /* Adapts to the translate mode of the taurus rds_recycle_bin_binlog_mode.
     If the source database is __recyclebin__, remove the recycle bin table
     options fields. */
  if (im::recycle_bin::is_recycle_db(old_db)) {
  ...
  /* Adapts to the translate mode of the taurus rds_recycle_bin_binlog_mode.
     If the target database is __recyclebin__, set the recycle bin table
     options fields. */
  if (im::recycle_bin::is_recycle_db(new_db)) {
    assert(!to_table_def->options().exists(im::recycle_bin::ORIGIN_SCHEMA.str));
    assert(!to_table_def->options().exists(im::recycle_bin::ORIGIN_TABLE.str));

    uint64_t recycled_time;
    auto origin_name = im::recycle_bin::parse_origin_table_by_table_name(
        new_name, &recycled_time);
    if (origin_name.first.length() && origin_name.second.length()) {
      to_table_def->options().set(im::recycle_bin::ORIGIN_SCHEMA.str,
                                  origin_name.first.c_str());
      to_table_def->options().set(im::recycle_bin::ORIGIN_TABLE.str,
                                  origin_name.second.c_str());
      to_table_def->set_last_altered(
          dd::my_time_t_to_ull_datetime(recycled_time));
    }

    if (thd->recycle_state->is_recycle()) {
#ifdef HAVE_PSI_SP_INTERFACE
      remove_all_triggers_from_perfschema(old_db, *to_table_def);
#endif
  ...
  */
  if (!(flags & NO_FK_RENAME) && !thd->recycle_state->is_recycle() &&
...
bool mysql_create_like_table(THD *thd, Table_ref *table, Table_ref *src_table,
  ...
  */
  ...
  if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
      src_table->table->s->db_type()->db_type == DB_TYPE_DSTORE &&
      (src_table->table->s->tablespace == nullptr ||
       strlen(src_table->table->s->tablespace) == 0)) {
    tablespace_set.insert("dstore_default");
  }
  ...
  */
  ...
  /*
    We should recover the tablespace for create temporary xxx like t1,
    where t1 is not a normal table
  */
  DBUG_EXECUTE_IF("mock_tablespace_len0", create_info->tablespace = "";);
  if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
      local_create_info.options & HA_LEX_CREATE_TMP_TABLE &&
      local_create_info.db_type->db_type == DB_TYPE_DSTORE &&
      (create_info->tablespace == nullptr ||
       strlen(create_info->tablespace) == 0)) {
    ...
    if (new_field) {
      ...
      */
      ...
      /*
        ...
        in InnoDB.
      */
      if (ha_alter_info->create_info->db_type->db_type == DB_TYPE_DSTORE ||
  ...
  */
...

  {
    ...
    */
    ...
    /* For inplace DDL operations, when generating DDL info, we will convert
     * them into drop table + create table statements. */
    if (need_ddl_info_sql(thd) &&
        inplace_supported != HA_ALTER_INPLACE_INSTANT) {
      if (generate_dstore_alter_ddl_info(thd, ha_alter_info->create_info, table,
                                         const_cast<dd::Table *>(table_def),
                                         altered_table, altered_table_def,
                                         alter_ctx)) {
        LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
               "Generate dstore alter ddl info failed!");
    ...
    */
    ...
    if ((db_type->flags & HTON_SUPPORTS_ATOMIC_DDL)) {
      ...
      */
      /*
        The alter table command is executed by drs in some scenarios during
        full synchronization. If the altered table is in __recyclebin__ schema,
        add options of ORIGIN_SCHEMA and ORIGIN_TABLE by parsing table name.
      */
      if (im::recycle_bin::is_recycle_db(alter_ctx->new_db)) {
        uint64_t recycled_time;
        auto origin_name = im::recycle_bin::parse_origin_table_by_table_name(
            alter_ctx->new_name, &recycled_time);
        if (origin_name.first.length() && origin_name.second.length()) {
          altered_table_def->options().set(im::recycle_bin::ORIGIN_SCHEMA.str,
                                           origin_name.first.c_str());
          altered_table_def->options().set(im::recycle_bin::ORIGIN_TABLE.str,
                                           origin_name.second.c_str());
          altered_table_def->set_last_altered(
              dd::my_time_t_to_ull_datetime(recycled_time));
        }
      }
  ...
  */
  ...
  if (db_type->flags & HTON_SUPPORTS_ATOMIC_DDL) {
    ...
    */
    ...
    //  update dstore statistics by backgroud thread for recreate table
    bool is_recreate = alter_info->flags & Alter_info::ALTER_RECREATE;
    if ((is_recreate) &&
        ha_alter_info->create_info->db_type->db_type == DB_TYPE_DSTORE) {
      new_table_list.table->file->ha_extra(
          HA_EXTRA_FLUSH_STATISTICS_BY_BACKGROUND);
    }
...
bool mysql_prepare_alter_table(THD *thd, const dd::Table *src_table,
  ...
  */
  if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
      !(used_fields & HA_CREATE_USED_TABLESPACE)) {
    bool is_temp_table = table->s->tmp_table;
    DBUG_EXECUTE_IF("mock_tablespace_len0", create_info->tablespace = "";);
    if (create_info->db_type->db_type == DB_TYPE_DSTORE && !is_temp_table &&
        (create_info->tablespace == nullptr ||
         strlen(create_info->tablespace) == 0)) {
      create_info->tablespace = "dstore_default";
      Tablespace_hash_set tablespace_set(PSI_INSTRUMENT_ME);
...
bool adjust_fk_for_recycle_drop_table(
    THD *thd, const char *new_db, const char *new_table_name, handlerton *hton,
    Foreign_key_parents_invalidator *fk_invalidator = nullptr) {
  ...
  if (new_table->foreign_keys() == nullptr) return true;
  ...
  /* children tables */
  for (const dd::Foreign_key_parent *fk : new_table->foreign_key_parents()) {
    if (my_strcasecmp(table_alias_charset, fk->child_schema_name().c_str(),
                      new_db) == 0 &&
        my_strcasecmp(table_alias_charset, fk->child_table_name().c_str(),
                      new_table_name) == 0)
  ...
  new_table->drop_foreign();
  if (thd->dd_client()->update(new_table)) return true;
...
bool mysql_alter_table(THD *thd, const char *new_db, const char *new_name,
  ...
  */
  ...
  if (!validate_partition_type(table->part_info) &&
  ...
  */
  histograms::columns_set columns;
  histograms::columns_set tmp_key_set;
  for (const auto column : alter_info->drop_list) {
    if (column->type == Alter_drop::COLUMN) {
      columns.emplace(column->name);
    } else if (column->type == Alter_drop::KEY &&
               dstore_use_histogram_auto_update) {
      /*
        In MySQL+DStore, we automatically add histograms to all indexes columns
        as these are used for index statistics. So if we remove an index, we
        also want to remove the corresponding histogram.

        We have to take some extra care if a column is used in multiple
        indexes, such as this case:

        CREATE TABLE t (i1 INT, i2 INT, i3 INT, INDEX (i1, i2), INDEX (i2, i3))

        If we drop the first index, we only want to remove the histogram for
        column "i1". The histogram "i2" must be kept as it's part of the second
        index.
      */
      const KEY *drop_key = nullptr;
      for (uint i = 0; i < table->s->keys; i++) {
        KEY *key = &table->key_info[i];
        if (my_strcasecmp(system_charset_info, column->name, key->name) == 0) {
          drop_key = key;
          break;
        }
      }
      ...
      tmp_key_set.emplace(drop_key->name);
      for (uint i = 0; i < drop_key->user_defined_key_parts; ++i) {
        bool key_field_in_other_key = false;
        for (uint j = 0; j < table->s->keys; j++) {
          KEY *key = &table->key_info[j];
          if (my_strcasecmp(system_charset_info, drop_key->name, key->name) ==
              0) {
            continue;
          }

          /*
            search key field in other key exclude previous handled drop key in
            drop list
          */
          if (tmp_key_set.find(key->name) != tmp_key_set.end()) {
            continue;
          }

          for (uint k = 0; k < key->user_defined_key_parts; ++k) {
            if (my_strcasecmp(system_charset_info,
                              drop_key->key_part[i].field->field_name,
                              key->key_part[k].field->field_name) == 0) {
              key_field_in_other_key = true;
              break;
            }
          }
          if (key_field_in_other_key) {
            break;
          }
        }

        if (!key_field_in_other_key) {
          columns.emplace(drop_key->key_part[i].field->field_name);
        }
      }
    }
  }
  ...
  */
  if (!(new_table->file->ha_table_flags() & HA_NO_COPY_ON_ALTER)) {
    ...
    if (alter_ctx.is_table_renamed() && !table->s->tmp_table &&
        new_table->s->tmp_table)
      new_table->file->set_old_name(alter_ctx.get_path());
...
end_inplace_noop:
  ...
  */
  ...
  // delete dstore statistics when alter table engine to non-dstore
  if (new_db_type->db_type != old_db_type->db_type &&
      old_db_type->db_type == DB_TYPE_DSTORE &&
      old_db_type->post_delete_statistics) {
    char table_name_path[FN_REFLEN + 1];
    (void)build_table_filename(table_name_path, sizeof(table_name_path) - 1,
                               alter_ctx.db, alter_ctx.table_name, "", 0);
    old_db_type->post_delete_statistics(thd, table_name_path);
  }
...
#ifndef WORKAROUND_TO_BE_REMOVED_BY_WL6049
  {
    ...
    bool is_rename_table = alter_info->flags & Alter_info::ALTER_RENAME;
    if (is_rename_table && create_info->db_type->db_type == DB_TYPE_DSTORE) {
      char new_table_name_path[FN_REFLEN + 1];
      (void)build_table_filename(new_table_name_path,
                                 sizeof(new_table_name_path) - 1,
                                 alter_ctx.new_db, alter_ctx.new_name, "", 0);
      dd::cache::Dictionary_client::Auto_releaser releaser_rename(
          thd->dd_client());
      const dd::Table *from_table_def = nullptr;
      if (!thd->dd_client()->acquire(alter_ctx.new_db, alter_ctx.new_name,
                                     &from_table_def)) {
        assert(from_table_def != nullptr);
        // from_table_def and to_table_def are same in rename
        table_list_reopen.table->file->post_rename_table_statistics(
            alter_ctx.get_path(), new_table_name_path, from_table_def,
            from_table_def);
      }
    }

    bool is_add_index = alter_info->flags & Alter_info::ALTER_ADD_INDEX;
    bool is_drop_index = alter_info->flags & Alter_info::ALTER_DROP_INDEX;
    bool is_rename_index = alter_info->flags & Alter_info::ALTER_RENAME_INDEX;
    if ((is_add_index || is_drop_index || is_rename_index) &&
        create_info->db_type->db_type == DB_TYPE_DSTORE) {
      table_list_reopen.table->file->ha_extra(
          HA_EXTRA_ALTER_TABLE_FLUSH_STATISTICS);
    }

    /*
      update dstore statistics by backgroud thread
      1. add index
      2. change engine to dstore
      3. recreate table
    */
    bool is_change_engine = new_db_type->db_type != old_db_type->db_type;
    bool is_recreate = alter_info->flags & Alter_info::ALTER_RECREATE;
    if ((is_add_index || is_change_engine || is_recreate) &&
        create_info->db_type->db_type == DB_TYPE_DSTORE) {
      table_list_reopen.table->file->ha_extra(
          HA_EXTRA_FLUSH_STATISTICS_BY_BACKGROUND);
    }
...
err_new_table_cleanup:
  if (new_table && alter_ctx.is_table_renamed() && !table->s->tmp_table &&
      new_table->s->tmp_table)
    new_table->file->set_old_name(nullptr);
...
void Foreign_key_parents_invalidator::force_invalidate(THD *thd) {
  for (auto parent_it : m_parent_map) {
    ...
    // And storage engine internal dictionary cache as well.
    if ((parent_it.second.first)->dict_cache_reset)
      ((parent_it.second.first))
          ->dict_cache_reset(parent_it.first.first.c_str(),
                             parent_it.first.second.c_str());
  }
  m_parent_map.clear();

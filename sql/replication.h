...
/**
  This callback is called during the transaction is committed.

  @param[in] target_lsn  lsn required for transaction commit

  @return 0: success;  non-zero: error
*/
typedef int (*trx_wait_t)(uint64_t target_lsn);

/**
  This callback is called before the transaction wait.

  @param[in] standby_flushed_lsn  flushed lsn of the standby

  @return 0: success;  non-zero: error
*/
typedef int (*get_standby_flushed_lsn_t)(uint64_t *standby_flushed_lsn);
...
/**
   Observe wallog logging storage
*/
typedef struct Wallog_trx_observer {
  uint32 len;

  trx_wait_t trx_wait;
  get_standby_flushed_lsn_t get_standby_flushed_lsn;
} Wallog_trx_observer;
...
/**
   Replication wallog transmitter (wallog dump) observer parameter.
*/
typedef struct Wallog_transmit_param {
  uint32 server_id;
  uint32 flags;
  /* Let us keep 1-16 as output flags and 17-32 as input flags */
  static const uint32 F_OBSERVE = 1;
  ...
  void set_dont_observe_flag() { flags |= F_DONT_OBSERVE; }
  /**
     If F_OBSERVE is set by any plugin, then it should observe wallog
     transmission, even F_DONT_OBSERVE is set by some plugins.

     If both F_OBSERVE and F_DONT_OBSERVE are not set, then it is an old
     plugin. In this case, it should always observe wallog transmission.
   */
  bool should_observe() {
    return (flags & F_OBSERVE) || !(flags & F_DONT_OBSERVE);
  }
} Wallog_transmit_param;

/**
  This callback is called when wallog dumping starts

  @param param Observer common parameter
  @param log_pos wallog position to transmit from

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*wallog_transmit_start_t)(Wallog_transmit_param *param,
                                       uint64 log_pos);

/**
  This callback is called when wallog dumping stops
  ...
  @retval 1 Failure
*/
typedef int (*wallog_transmit_stop_t)(Wallog_transmit_param *param);

/**
  This callback is called to reserve bytes in packet header for wal
  transmission
  ...
  @retval 1 Failure
*/
typedef int (*wallog_reserve_header_t)(Wallog_transmit_param *param,
                                       unsigned char *header,
                                       unsigned long size, unsigned long *len);

/**
  This callback is called before sending an wallog packet to slave

  @param param Observer common parameter
  @param packet Wallog packet to send
  @param len Length of the wallog packet
  @param log_pos Wallog position of the event packet to send

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*wallog_before_send_event_t)(Wallog_transmit_param *param,
                                          unsigned char *packet,
                                          unsigned long len, uint64 log_pos);

/**
  This callback is called after an wallog packet is sent to the
  slave or is skipped.

  @param param             Observer common parameter
  @param event_buf         Wallog packet buffer sent

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*wallog_after_send_event_t)(Wallog_transmit_param *param,
...
/**
  ...
  @retval 1 Failure
*/
typedef int (*wallog_after_reset_master_t)(Wallog_transmit_param *param);

/**
   Observe and extends the wallog dumping thread.
*/
typedef struct Wallog_transmit_observer {
  uint32 len;

  wallog_transmit_start_t wallog_transmit_start;
  wallog_transmit_stop_t wallog_transmit_stop;
  wallog_reserve_header_t wallog_reserve_header;
  wallog_before_send_event_t wallog_before_send_event;
  wallog_after_send_event_t wallog_after_send_event;
  wallog_after_reset_master_t wallog_after_reset_master;
} Wallog_transmit_observer;
...
/**
  Replication wal IO observer parameter
*/
typedef struct Wallog_IO_param {
  uint32_t server_id;
  my_thread_id thread_id;
...
/**
  This callback is called when slave wal IO thread starts
  ...
  @retval 1 Failure
*/
typedef int (*wal_thread_start_t)(Wallog_IO_param *param);

/**
  This callback is called when slave wal IO thread stops
  ...
  @retval 1 Failure
*/
typedef int (*wal_thread_stop_t)(Wallog_IO_param *param);

/**
  This callback is called before slave requesting wallog transmission from
  master

  This is called before slave issuing WALLOGOG_DUMP command to master
  to request wallog.

  @param param Observer common parameter
  @param flags wallog dump flags

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*before_request_wal_transmit_t)(Wallog_IO_param *param,
                                             uint32 flags);

/**
  This callback is called after read an wal packet from master

  @param param Observer common parameter
  @param packet The wal packet read from master
  @param len Length of the wal packet read from master
  @param wal_buf The wal packet return after process
  @param wal_len The length of wal packet return after process

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*after_read_wal_t)(Wallog_IO_param *param, const char *packet,
                                unsigned long len, const char **wal_buf,
                                unsigned long *wal_len);

/**
  This callback is called after written an wal packet to wal log

  @param param Observer common parameter
  @param wal_buf Wal packet written to wal log
  @param wal_len Length of the wal packet written to wal log
  @param flags flags for wal log

  @retval 0 Success
  @retval 1 Failure
*/
typedef int (*after_write_wal_t)(Wallog_IO_param *param, const char *wal_buf,
                                 unsigned long wal_len, uint32 flags);

/**
  This callback is called after reset slave wal log IO status
  ...
  @retval 1 Failure
*/
typedef int (*after_reset_wal_slave_t)(Wallog_IO_param *param);

/**
  This callback is called before wal log gets applied
  ...
  @retval 1 Failure
*/
typedef int (*applier_wal_t)(Wallog_IO_param *param, Trans_param *trans_param,
                             int &out);

/**
   Observes and extends the service of slave wal IO thread.
*/
typedef struct Wallog_IO_observer {
  uint32 len;

  wal_thread_start_t wal_thread_start;
  wal_thread_stop_t wal_thread_stop;
  before_request_wal_transmit_t before_request_wal_transmit;
  after_read_wal_t after_read_wal;
  after_write_wal_t after_write_wal;
  after_reset_wal_slave_t after_reset_wal_slave;
  applier_wal_t applier_wal;
} Wallog_IO_observer;
...
int unregister_wallog_trx_observer(Wallog_trx_observer *observer, void *p);
...
/**
   Register a wallog transmit observer

   @param observer The wallog transmit observer to register
   @param p pointer to the internal plugin structure
...
/**
   Unregister a wallog transmit observer

   @param observer The wallog transmit observer to unregister
   @param p pointer to the internal plugin structure
...
/**
   Register a wallog IO (slave IO thread) observer

   @param observer The wallog IO observer to register
   @param p pointer to the internal plugin structure
...
/**
   Unregister a wallog IO (slave IO thread) observer

   @param observer The wallog IO observer to unregister
   @param p pointer to the internal plugin structure

   @retval 0 Success
   @retval 1 Observer not exists
*/
int unregister_wallog_io_observer(Wallog_IO_observer *observer, void *p);

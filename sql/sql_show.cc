...
#include "sql/recyclebin/recycle_auth.h"
...
void store_key_options(THD *thd, String *packet, TABLE *table, KEY *key_info);
...
void append_directory(THD *thd, String *packet, const char *dir_type,
                      const char *filename) {
...
*/
bool print_on_update_clause(Field *field, String *val, bool lcase) {
...
*/
bool print_default_clause(THD *thd, Field *field, String *def_value,
                          bool quoted) {
...
*/
bool should_print_encryption_clause(THD *thd, TABLE_SHARE *share, bool *print) {
...
*/
void print_foreign_key_info(THD *thd, const LEX_CSTRING *db,
...
/**
  Whether to display reserved users' session info in processlist.
  Super users and reserved users can see reserved users' session info
  in processlist, other users can not see.

  @param cur_thd      The current THD.
  Global_THD_manager::get_instance()->do_for_all_thd_copy

  @returns            true if filter
*/
static bool display_filter_for_normal_user(THD *cur_thd) {
  if (!opt_rds_processlist_control) return false;

  if (cur_thd->is_reserved_user_session() ||
      check_global_access_noerr(cur_thd, SUPER_ACL) == 0)
    return false;
...
class List_process_list : public Do_THD_Impl {
 ...
 public:
  List_process_list(const char *user_value, Thread_info_array *thread_infos,
    ...
    m_display_filter = display_filter_for_normal_user(m_client_thd);
  }

  void operator()(THD *inspect_thd) override {
    DBUG_TRACE;

    /* not using inspect_thd->security_context, no need to put into lock */
    if (m_display_filter && inspect_thd->is_reserved_user_session()) return;
    ...
    /* TRX_START_TIME */
    thd_info->trx_start_time_in_secs = inspect_thd->trx_start_time_in_secs;
...
void mysqld_list_processes(THD *thd, const char *user, bool verbose,
  ...
  if (has_cursor && thd->lex->unit->query_result()->start_execution(thd)) {
    return;
  }
  ...
  field->set_nullable(true);
  field_list.push_back(field = new Item_return_int("Trx_executed_time",
  ...
  for (size_t ix = 0; ix < thread_infos.size(); ++ix) {
    ...
    protocol->store(thd_info->trx_start_time_in_secs > 0
                        ? (longlong)(now - thd_info->trx_start_time_in_secs)
                        : 0);
...
class Fill_process_list : public Do_THD_Impl {
 ...
 public:
  ...
  void operator()(THD *inspect_thd) override {
    ...
    /* TRX_EXECTURED_TIME */
    table->field[8]->store(
        (longlong)(inspect_thd->trx_start_time_in_secs
                       ? (time(nullptr) - inspect_thd->trx_start_time_in_secs)
                       : 0),
        false);
...
ST_FIELD_INFO processlist_fields_info[] = {
    ...
    {"TRX_EXECUTED_TIME", 7, MYSQL_TYPE_LONG, 0, 0, "Trx_executed_time", 0},
    {nullptr, 0, MYSQL_TYPE_STRING, 0, 0, nullptr, 0}};
...
static Mysql_internal_schema_access mysql_internal_schema_access;
static Sys_internal_schema_access sys_interna_schema_access;
static im::recycle_bin::Recycle_internal_schema_access
...
void initialize_mysql_db_acl() {
  ACL_internal_schema_registry::register_schema(MYSQL_SCHEMA_NAME,
                                                &mysql_internal_schema_access);
}

void initialize_sys_db_acl() {
  ACL_internal_schema_registry::register_schema(SYS_SCHEMA_NAME,
                                                &sys_interna_schema_access);
}

void initialize_recyclebin_schema_acl() {
  ACL_internal_schema_registry::register_schema(
      im::recycle_bin::RECY_SCHEMA_NAME, &recycle_internal_schema_access);
}
...
/*
  show product_veserion print in format: TaurusDB V2.0.xx,
  V2.0.xx comes from TAURUS_PRODUCT_VERSION
*/
bool mysqld_show_product_version(THD *thd) {
  DBUG_TRACE;
  Item *field;
  mem_root_deque<Item *> field_list(thd->mem_root);
  Protocol *protocol = thd->get_protocol();
  field_list.push_back(
      field = new Item_empty_string("product_version", NAME_CHAR_LEN));
  if (thd->send_result_metadata(field_list,
                                Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF)) {
    return true;
  }
  protocol->start_row();
  std::string product_version = TAURUS_PRODUCT_VERSION;
  product_version = "TaurusDB " + product_version;

  protocol->store(product_version.c_str(), system_charset_info);
  if (protocol->end_row()) {
    return true;
  }
  my_eof(thd);

...
bool JOIN::optimize(bool finalize_access_paths) {
  ...
  /// If plan cache is used for this execution, do whatever it needs.
  if (this == query_block->cached_plan) {
    m_root_access_path = nullptr;
    if (zero_result_cause) {
      create_access_paths_for_zero_rows();
      goto setup_subq_exit;
    }
    goto do_make_tmp_tables_info;
  }
  ...
  */
  ...
  {  // WL#6570
    for (Table_ref *tl = query_block->leaf_tables; tl; tl = tl->next_leaf) {
      ...
      } else if (tl->is_transformed_in_predicate()) {
        Item_func_in *in_predicate =
            dynamic_cast<Item_func_in *>(tl->get_transformed_in_predicate());
        if (in_predicate->fill_transform_tmp_table(thd, tl->table)) return true;
      }
    }
  }
...
uint QEP_TAB::effective_index() const {
  switch (type()) {
    ...
    case JT_CONST:
      // In JOIN::estimate_rowcount(), there is a case where JT_CONST is
      // chosen, but with no associated key.
      return (ref().key != -1) ? uint(ref().key) : MAX_KEY;
...
bool JOIN::optimize_distinct_group_order() {
  ...
  /*
    After removing the constant ORDER BY, determine whether the query block has
    a LIMIT clause but no ORDER BY clause, and avoid executing PQ.
  */
  disable_pq_if_limit_without_orderby(thd, query_block, order.empty(),
                                      implicit_grouping);
...
/**
  Determine whether index scan is allowed for the given table.
  when the optimizer selects a loose index scan,
  the "enabled index scan" feature will not take effect.

  @param thd    Thread context
  @param table  Table object

  @return true Index scan is enabled for this table.
  @return false Index scan is disabled for this table.
*/
static inline bool enable_choose_index_scan(const THD *thd,
                                            const JOIN_TAB *tab) {
  ...
  if (likely(table != nullptr)) {
    return thd->optimizer_switch_flag(
               OPTIMIZER_SWITCH_DSTORE_ENABLE_INDEXSCAN) ||
           table->file->ht->db_type != DB_TYPE_DSTORE;
  }
  return true;
}
    ...
    if (tab->type() == JT_ALL) {
      ...
      */
      if (!tab->table()->no_keyread &&                    //  1
          ...
          enable_choose_index_scan(thd, tab))             //  4
      {
        /*
    ...
    if (has_semijoin && sj_is_materialize_strategy(pos->sj_strategy)) {
      ...
      */
      ...
      Semijoin_mat_exec *const sjm_exec = new (thd->mem_root) Semijoin_mat_exec(
          ...
          /*pq_shared_info=*/nullptr);

      tab->set_sj_mat_exec(sjm_exec);

      if (!sjm_exec || setup_semijoin_materialized_table(
                           tab, query_block->get_free_tableno(), pos,
                           best_positions + sjm_index))
...
static bool change_cond_ref_to_const(THD *thd, I_List<COND_CMP> *save_list,
  ...
  return false;
  /* [1] the 2nd line records a change (change_item_tree), the first one does
     not (as it does a plain assignment). This then confuses PQ cloning.
     Here's a scenario where it breaks, with an explanation.

     We have (from test parallel_query.pq_subquery_correlated):
     SELECT (SELECT t2.char_col FROM t2
             WHERE t2.char_col = t1.char_col AND
                   t2.char_col <=> 'aaaa' ) AS c2 FROM t1 ;

     which returns a wrong result with PQ: returns 'aaaa', while the correct
     result is NULL as there should be no match in the WHERE.

     Equality propagation changes the WHERE, from:
     t2.char_col = t1.char_col AND t2.char_col <=> 'aaaa'
     to:
     'aaaa' = t1.char_col AND t2.char_col <=> 'aaaa'
     then to:
     t1.char_col = 'aaaa' AND t2.char_col <=> 'aaaa'

     and PQ cloning of that leads to:
     t1.char_col = t1.char_col AND t2.char_col <=> 'aaaa'
     which leads of course to wrong results.

     The problem starts its life in the present function (before PQ kicks in)
     when processing the equality at the left of AND.
     Consider this piece from the present function:
     ...
     the item in question ('func') is:
     t2.char_col = t1.char_col
     and 'value' (the constant to propagate) is 'aaaa', so the line (*)
     changes args[0] of the equality, making the equality become
     'aaaa' = t1.char_col
     and this change of args[0] is recorded with THD::change_item_tree(),
     which sets 'aaaa'->origin_item to t2.char_col ('aaaa' is Item_string)
     (origin_item is PQ-specific information).
     Then (**) and (***) intend to swap arguments of "=":
     right after (**) we have
     t1.char_col = t1.char_col
     and after (***) we have
     t1.char_col = 'aaaa'.
     But note that (**) has done a non-recorded pointer assignment,
     unlike (***).

     So 'aaaa'->origin_item is now t1.char_col, as it has replaced t1.char_col
     on the right side of "=". As a result, when we later do pq_clone() of
     this equality, come to cloning the right argument i.e. call Item_string's
     pq_clone(), we call ORIGIN_ITEM_CLONE, and this clones origin_item i.e.
     t1.char_col. And when we clone the left argument, we also clone
     t1.char_col (which has no origin_item). So the PQ worker ends up with
     t1.char_col=t1.char_col.

     In FE2022011000115 we tried to replace (**) with
     std::swap(args[0],args[1]). This solved the case above, but introduced a
     non-PQ BUG2025041529773; because swap() placed the constant on the right
     side, and then change_item_tree() on the right side replaced the constant
     with itself, but recorded a change for this, so after
     rollback_item_tree_changes() we still had the constant on the right side,
     and as it is in a transient MEM_ROOT, it crashed on next execution.

     Then we tried to replace (**) with a change_item_tree(), but it broke PQ
     again, because Item_*::pq_clone() functions decide to clone origin_item
     only for certain Item sub-classes: here, Item_string::pq_clone() does,
     but Item_field::pq_clone() does not; leading to a wrong Frankenstein-like
     Item_func_eq clone in the end. As this seems intractable, code was
     restored to its initial state (plain assignment) and PQ was disabled.
  */
}
...
bool JOIN::make_join_plan() {
  ...
  */
  ...
  if (const_tables) {
    ...
    // depend on columns of constant tables. This makes more Items become
    // constant, thus more join conditions get evaluated at optimization
    // time, making us hit the problem of CanHaveNullInNonNullable
    // (grep for this tag).
...
bool JOIN::init_planner_arrays() {
  ...
  const uint table_count_for_maps = query_block->get_free_tableno();
...
/**
  ...
  @param  check_index    TRUE <=> when field item belongs to tbl, check if it
                         is covered by #keyno
...
bool uses_index_fields_only(Item *item, TABLE *tbl, uint keyno,
                            bool other_tbls_ok, bool check_index) {
  ...
  switch (item_type) {
    case Item::FUNC_ITEM: {
      ...
      if (Item_func::UDF_FUNC == func_type && !check_index) {
        return false;
      }
    ...
    case Item::FIELD_ITEM: {
      ...
      */
      ...
      /*
      virtual column is not computed until the code flow returns to handler,
      so it can't be pushed to storage engine.
      */
      if (item_field->field->is_virtual_gcol()) {
        return false;
      }
      return true;
    }
...
static bool update_ref_and_keys(THD *thd, Key_use_array *keyuse,
  ...
  */
  if (!keyuse->empty()) {
    ...
    if (null_terminate_key_use_list(keyuse)) return true;

    use = save_pos = keyuse->begin();
    const Key_use *key_end = keyuse->end() - 1;
...
Key_use_array *create_keyuse_for_table(
  ...
  if (null_terminate_key_use_list(keyuses)) return nullptr;
  ...
  if (sj_is_materialize_strategy(lt->get_sj_strategy())) {
    ...
    */
    ...
    if (sjm->inner_table_index + sjm->table_count - 1 == (uint)last_tab) {
      // We're at last table of sjmat nest, so we can now push the SJ
      // condition to sj-inner tables. This SJ condition must first be
      // located: by make_outerjoin_info() it may have been attached to the
      // tmp table, in such case: 'JOIN tmp-table ON', or 'JOIN (tmp-table,
      // ...) ON', or attached to the first table in the containing nest, in
      // such case: 'JOIN (other-table, tmp-table, ...) ON'
      ...
      while (join_cond == nullptr && tr->embedding != nullptr) {
        // Should use Table_ref::join_cond_optim() during the optimization
        // phase, refer to Query_block::get_optimizable_conditions.
        join_cond = tr->embedding->join_cond_optim();
        tr = tr->embedding;
      }
  ...
  */
  if (lt->table_ref->embedding && lt->table_ref->embedding->is_aj_nest() &&
      ...
      /*
        ...
        Catch this case:
        Should use Table_ref::join_cond_optim() during the optimization phase,
        Refer to Query_block::get_optimizable_conditions.
      */
      !lt->table_ref->join_cond_optim()) {
  ...
  *subsumes = false;
  ...
  if (!right_is_null) {
    /*
      ...
      3. Field type is not same as right_item's result type. For example,
        Field type is int, right_item's result type is decimal, WHERE
        condition can't be removed because precision will be lost when
        decimal is converted into int. TODO: my_decimal2int should return
        conversion status. We opened a mysql bug for this problem:
        https://bugs.mysql.com/bug.php?id=108765.
        We should replace the code for condition 3 with the fix from
        Oracle when it's ready.
    */
    ...
    if (!((field->type() == MYSQL_TYPE_STRING ||  // 1
        ...
        (Item::type_to_result(field->type()) ==
         right_item->result_type()))  // 3
    {
...
static inline bool is_column_then_constant(Item *left_item, Item *right_item) {
  return (left_item->type() == Item::FIELD_ITEM && right_item->const_item());
}
...
/**
  ...
  We also try to remove redundant inequalities of the form 'field OP value',
  where OP is >, >=, etc, or BETWEEN, and also 'field IS NULL', if the table is
  accessed by a range index scan.
...
static bool reduce_cond_for_table(THD *thd, Item *cond, table_map null_extended,
  ...
  *reduced = nullptr;
  ...
  if (cond->type() == Item::COND_ITEM) {
    ...
    if (op_type == Item_func::TRIG_COND_FUNC) {
    ...
    // Now check if 'func' is already satisfied by use of index range scan
    else if (thd->variables.rds_empty_redundant_check_in_range_scan &&
             !(func->used_tables() & null_extended) &&
             (op_type == Item_func::LT_FUNC || op_type == Item_func::GT_FUNC ||
              op_type == Item_func::EQ_FUNC || op_type == Item_func::LE_FUNC ||
              op_type == Item_func::GE_FUNC || op_type == Item_func::BETWEEN ||
              op_type == Item_func::ISNULL_FUNC)) {
      const bool is_null = op_type == Item_func::ISNULL_FUNC;
      assert(is_null || func->argument_count() >= 2);
      Item *left_item = func->arguments()[0]->real_item();
      Item *right_item = is_null ? nullptr : func->arguments()[1]->real_item();
      {
        /**  We have 3 possibilities:
         *  1. left_item is field and right_item is constant (path_1)
         *  2. right_item is field and left_item is constant (path_2)
         *  3. none of the above in which case we will have nothing to do
         * (is_null is an special case which is additionally verified below,
         * proceeds as path_1)
         */
        bool path_1 =
            is_null ? true : is_column_then_constant(left_item, right_item);
        bool path_2 =
            is_null ? false : is_column_then_constant(right_item, left_item);
        if (path_1 || path_2) {
          if (!path_1 && path_2) {
            left_item = func->arguments()[1]->real_item();
            right_item = func->arguments()[0]->real_item();
          }

          if (is_null && left_item->type() != Item::FIELD_ITEM) {
            // if OP is not IS NULL, type() is checked in
            // is_column_then_constant()
            *reduced = cond;
            return false;
          }

          // Here we are guiaranteed that left_item is field and right is
          // constant
          Field *field = down_cast<Item_field *>(left_item)->field;
          JOIN_TAB *join_tab = field->table->reginfo.join_tab;
          if (join_tab && join_tab->type() == JT_RANGE &&
              !(field->type() == MYSQL_TYPE_BIT) &&
              // Dynamic range is more complex, not supported
              join_tab->use_quick != QS_DYNAMIC_RANGE &&
              // comparable_in_index() is used by the range optimizer to decide
              // if it can handle a range. However this function has a special
              // permissive case for equality conditions under binary
              // collations, which alas makes it impossible to remove the
              // condition. That's why we call the function again below,
              // pretending this is not equality. This doesn't apply to IS NULL
              (is_null ||
            ...
            auto *range_scan_ap = join_tab->range_scan();
            if (range_scan_ap &&
                range_scan_ap->type == AccessPath::INDEX_RANGE_SCAN) {
              // get fields used in the range scan
              Field_map used_fields;
              get_fields_used(range_scan_ap, used_fields.get_bitmap());
              // First clause in if() ensures that field is used by the range
              // scan. Second clause in if() ensures that ranges has size 1. It
              // means we do not support disjoint ranges caused by OR operators
              // in the WHERE clause (like 'idxcol > 3 OR idxcol < 5'). Note
              // that an AND operator produces a single range (like 'idxcol > 3
              // AND idxcol < 5').
              if (used_fields.is_set(field->field_index()) &&
                  range_scan_ap->index_range_scan().num_ranges == 1) {
                QUICK_RANGE *range =
                    range_scan_ap->index_range_scan().ranges[0];
                uchar *key_max = range->max_key;
                uchar *key_min = range->min_key;

                KEY_PART *key_parts =
                    range_scan_ap->index_range_scan().used_key_part;
                // If we have an index on columns A,B,C a typical range bound is
                // of type A=3 AND B=4 AND C>5. In that case, key_min is laid
                // out like this:
                //  NA 3 NB 4 NC 5
                // where NA is one byte (0 here).
                // For a range like A IS NULL, key_min/max will have NA=1.
                // In 'func' we have a condition which could be B=constant.
                // We are going to search which value this range has for B (it
                // is 4). So we have to locate in key_min the value for B.
                // 'key_offset' is an offset in bytes which we calculate, and
                // which points to that desired value.
                int key_offset = 0;

                // Find the position in the key_min/max list of the upper bound
                // of interest, It works because the possible scenarii are
                // conditioned on the query being a index range scan
                uint i = 0;
                for (; i < range_scan_ap->index_range_scan().num_used_key_parts;
                     ++i) {
                  if (field->eq(key_parts[i].field)) {
                    break;
                  }
                  key_offset += key_parts[i].store_length;
                }
                // key_part is the last one used by the range scan:
                bool last_key_part =
                    ((i + 1) ==
                     range_scan_ap->index_range_scan().num_used_key_parts);
                assert(i <
                       range_scan_ap->index_range_scan().num_used_key_parts);
                bool is_key_max_set = range->is_key_max_set(i);
                bool is_key_min_set = range->is_key_min_set(i);
                key_max += key_offset;
                key_min += key_offset;
                bool key_max_is_null = false;
                bool key_min_is_null = false;
                if (field->is_nullable()) {
                  // byte 0 of key_min/max is the null-byte. If set, key_min/max
                  // is NULL
                  if (is_key_max_set) key_max_is_null = *key_max;
                  if (is_key_min_set) key_min_is_null = *key_min;
                  // skip NULL byte to access the real value
                  if (!is_null) {
                    ++key_max;
                    ++key_min;
                  }
                ...
                KEY_PART &key_part = key_parts[i];
                TABLE *table = field->table;
                my_bitmap_map *old_sets[2];
                // Prefix index not handled: if prefix shorter than item, range
                // matches too much, filter is necessary in SQL-layer
                if (!(key_part.flag & HA_PART_KEY_SEG)) {
                  // Depending on the operator in 'func', we have to compare
                  // key_min/max with an argument of the operator. We set up the
                  // necessary comparisons in this array, then common code does
                  // the comparisons.
                  // Continuing with the A/B/C example above, we're going to see
                  // if 'constant' is 4. If so, we can remove B=constant.
                  Item *cmp_with_min = nullptr, *cmp_with_max = nullptr;
                  // For BETWEEN, no need to consider path_2, left item is
                  // always going to be the column, and the right item the
                  // bound, similarly, the first right item is always going to
                  // be the lower bound and the second the upper bound
                  bool negated = false;
                  if (path_1 && op_type == Item_func::BETWEEN) {
                    // Not compatible with NOT BETWEEN - same as OR clause,
                    // disjoint intervals
                  ...
                  // strict inequality (<,>). When we have a condition with
                  // strict inequality eg. col1<k (or col1>k), there are two
                  // cases we need to consider:
                  //    1. If col1 is not the last key part used by the range
                  //    scan, the range condition on that key part has to be an
                  //    equality condition, we should therefore not remove
                  //    col1<k (or col1>k).
                  //    2. Otherwise: if the range scan has a strict upper bound
                  //    < (resp. lower bound >) on col1 the NEAR_MAX (resp.
                  //    NEAR_MIN) flag will be set, so we check for that.
                  //    valid_cmp will convey the validity of the comparison.
                  //    Note: If the key part is DESC, we have to check NEAR_MIN
                  //    instead NEAR_MAX (and conversly) to preserve the logic.
                  bool descending_key_part = key_part.flag & HA_REVERSE_SORT;
                  bool valid_cmp = true;
                  bool is_strict_inequality = (op_type == Item_func::LT_FUNC ||
                                               op_type == Item_func::GT_FUNC);
                  uint16_t range_flag = range->flag;
                  if ((path_1 && (op_type == Item_func::LT_FUNC ||
                                  op_type == Item_func::LE_FUNC)) ||
                      (path_2 && (op_type == Item_func::GT_FUNC ||
                                  op_type == Item_func::GE_FUNC))) {
                    cmp_with_max = right_item;
                    valid_cmp =
                        is_strict_inequality
                            ? (last_key_part ? (descending_key_part
                                                    ? (range_flag & NEAR_MIN)
                                                    : (range_flag & NEAR_MAX))
                                             : false)
                            : true;
                  }
                  if ((path_2 && (op_type == Item_func::LT_FUNC ||
                                  op_type == Item_func::LE_FUNC)) ||
                      (path_1 && (op_type == Item_func::GT_FUNC ||
                                  op_type == Item_func::GE_FUNC))) {
                    cmp_with_min = right_item;
                    valid_cmp =
                        is_strict_inequality
                  ...
                  if (op_type == Item_func::EQ_FUNC) {
                    cmp_with_max = cmp_with_min = right_item;
                  }

                  // Swap min and max if DESC index on that key part
                  if (descending_key_part) {
                    std::swap(cmp_with_min, cmp_with_max);
                  }

                  if ((cmp_with_max || cmp_with_min) && !negated && valid_cmp) {
                    // Prevent hitting ASSERT_COLUMN_MARKED_FOR_WRITE in
                    // stored_field_cmp_to_item():
                    dbug_tmp_use_all_columns(table, old_sets, table->read_set,
                                             table->write_set);
                    // We handle INT and NULL outside of
                    // stored_field_cmp_to_item() because it returns 0 for both
                    // regardless of the values we want to compare. The reason
                    // for that is that this function was initially designed as
                    // a comparison function after storing an item into a field
                    // to evaluate whether there had been some distortion. Since
                    // we are using it for something different we have to handle
                    // these two special cases seperatly. So we wrote the INT
                    // comparison outside the function. For comparisons to NULL,
                    // we also process them outside the function: we do not
                    // handle them because we cannot identify NULL in the field.

                    // Because we rely on stored_field_cmp_to_item() to stay the
                    // way it is now, we put a canon on it so that we can assess
                    // the effect that any modification of that function would
                    // have on our feature.
                    auto cmp = [field, &key_part](Item *cmp_item,
                                                  const uchar *key) -> bool {
                      field->set_key_image(key, key_part.length);
                      // stored_field_cmp_to_item does not compare ints
                      ...
                      if (res_type == INT_RESULT) {
                        ...
                        Integer_value b(cmp_item->val_int(),
                                        cmp_item->unsigned_flag);
                        // < > = comparison to NULL not handled
                        if (cmp_item->null_value) return false;
                        return a == b;
                      }

                      bool ret = stored_field_cmp_to_item(current_thd, field,
                                                          cmp_item) == 0;
                      // < > = comparison to NULL not handled
                      if (cmp_item->null_value) return false;
                      return ret;
                    ...
                    /*
                      Only compare if :
                      - key_max/min is set for that key part
                      - NO_MAX/MIN_RANGE flags aren't set otherwise means that
                      key_min/max might be random (could be redundant with the
                      first condition)
                    */
                    bool can_remove = true;
                    if (cmp_with_max) {
                      if (is_key_max_set && !key_max_is_null &&
                    ...
                    if (cmp_with_min) {
                      if (is_key_min_set && !key_min_is_null &&
                    ...
                    dbug_tmp_restore_column_maps(table->read_set,
                                                 table->write_set, old_sets);
                  ...
                  if (is_null && key_part.field->is_nullable()) {
                    // We know that cond is IS NULL, we just have to verify that
                    // both the upper and lower bounds are NULL
        ...
        // if op_type == Item_func::EQ_FUNC; equality may be achieved with index
        // lookup too, proceed below.
      }
    }
...
bool JOIN::finalize_table_conditions(THD *thd) {
  ...
  */
  ...
  for (uint i = const_tables; i < tables; i++) {
    ...
    */
    //  Save initial state before upcoming transformations. If we are not
    //  going to cache the plan, we skip this, to save CPU and memory.
...
bool optimize_cond(THD *thd, Item **cond, COND_EQUAL **cond_equal,
  ...
  */
  ...
  if (*cond) {
    ...
    // Save initial state before upcoming transformations. If we are not
    // going to cache the plan, we skip this, to save CPU and memory.
    plan_cache::collect_item_params(*cond, params1);
  }
  ...
  */
  ...
  if (*cond) {
    ...
    plan_cache::cmp_item_params_after_reduce_cond(thd, params1, *cond);
    step_wrapper.add("resulting_condition", *cond);
  }
  if (thd->is_error()) return true;
  return false;
  ...
  for (size_t ix = 0; ix < keyuse_array.size(); ++ix) {
    ...
    */
    ...
    if (keyuse->used_tables &
      ...
      {
        /* At this stage, JOIN_TABs in the 'join_tab' array are in the order
           of query_block->leaf_tables (see init_planner_arrays()), thus in
           the order of Table_ref::tableno() (see setup_tables()). However,
           due to join elimination, there can be gaps in the sequence of
           values of tableno(), e.g. if we had three tables, tableno 0-1-2,
           and tableno 1 has been eliminated, then tableno 2 is at position 1
           in 'join_tab', not 2. So we must search like this for the right
           tableno. map2table is alas not yet filled. */
        uint i;
        for (i = 0; i < tables; ++i) {
          if (join_tab[i].table_ref->tableno() == tableno) {
            TABLE *tmp_table = join_tab[i].table();
      ...
      [num_evaluations, limit, &rows](const AccessPath *subpath, const JOIN *) {
        ...
        if (const TABLE *table = GetBasicTable(subpath);
          ...
          if (((subpath->type == AccessPath::INDEX_SCAN) ||
               (subpath->type == AccessPath::PQ_BLOCK_SCAN &&
                subpath->pq_block_scan().qep_tab->type() == JT_INDEX_SCAN)) &&
...
/**
  Check if given expression can be evaluated in storage engine
  When other_tbls_ok, the expression can use any fields in any other tables;
  otherwise, any field that doesn't belong to the table tbl is not allowed.
*/
bool item_pushdownable(Item *const item, TABLE *const tbl, const uint keyno,
                       const bool other_tbls_ok) {
  return uses_index_fields_only(item, tbl, keyno, other_tbls_ok, false);
}

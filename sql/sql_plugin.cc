  ...
  for (struct st_mysql_plugin **builtins = mysql_mandatory_plugins;
    ...
    for (struct st_mysql_plugin *plugin = *builtins; plugin->info; plugin++) {
      ...
      */
      ...
      /*
        We disable the Dstore engine and related plugins when we are in the
        ONLY_INNODB mode to avoid the upgrade from RDS to DStore version
        failure.
      */
      const char *dstore_plugins[] = {DSTORE_ENGINE_NAME,
                                      "DSTORE_TRX",
      ...
      if (storage_engine_mode == static_cast<ulong>(ONLY_INNODB)) {
        for (size_t i = 0; dstore_plugins[i] != nullptr; ++i) {
          if (!my_strcasecmp(&my_charset_latin1, plugin->name,
                             dstore_plugins[i])) {
            force_load_option = PLUGIN_OFF;
            break;
          }
        }
      }
...
const CHARSET_INFO *get_thd_charset_info(THD *thd) {
  if (thd) return thd->charset();
  return NULL;
}

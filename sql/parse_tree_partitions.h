...
class PT_base_part_definition : public Parse_tree_part_node {
 ...
 public:
  ...
  PT_base_part_definition(
      const POS &pos, const LEX_STRING &name, partition_type type,
      PT_part_values *const opt_part_values, const POS &values_pos,
      Mem_root_array<PT_partition_option *> *opt_part_options)
      : pos(pos),
        name(name),
        type(type),
...
/**
  ...
  @ingroup ptn_partitioning
*/
class PT_subpart_definition : public PT_base_part_definition {
  typedef PT_base_part_definition super;

 public:
  PT_subpart_definition(const POS &pos, const LEX_STRING &name,
                        partition_type type,
                        PT_part_values *const opt_part_values,
                        const POS &values_pos,
                        Mem_root_array<PT_partition_option *> *opt_part_options)
      : super(pos, name, type, opt_part_values, values_pos, opt_part_options) {}
...
/**
  ...
  @ingroup ptn_partitioning
*/
class PT_part_definition : public PT_base_part_definition {
  typedef PT_base_part_definition super;

  Mem_root_array<PT_subpart_definition *> *opt_sub_partitions;
...
class PT_base_partition {
 protected:
  PT_part_type_def *const part_type_def;
  const uint opt_num_parts;

 public:
  PT_base_partition(PT_part_type_def *part_type_def, uint opt_num_parts)
      : part_type_def(part_type_def), opt_num_parts(opt_num_parts) {}
};
...
/**
  ...
  @ingroup ptn_partitioning
*/
class PT_sub_partition : public PT_base_partition, public Parse_tree_part_node {
  typedef Parse_tree_part_node super;

 public:
  const POS part_type_pos;
  sub_partition_info part_info;

  PT_sub_partition(const POS &pos, PT_part_type_def *part_type_def,
                   uint opt_num_parts)
      : PT_base_partition(part_type_def, opt_num_parts), part_type_pos(pos) {}

  bool contextualize(Partition_parse_context *) override;
};
...
/**
  ...
  @ingroup ptn_partitioning
*/
class PT_partition : public PT_base_partition, public Parse_tree_node {
 ...
 public:
  PT_partition(PT_part_type_def *part_type_def, uint opt_num_parts,
               ...
               Mem_root_array<PT_part_definition *> *part_defs)
      : PT_base_partition(part_type_def, opt_num_parts),

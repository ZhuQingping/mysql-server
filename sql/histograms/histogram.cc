...
// Disabled for dstore so we can create histogram on columns with unique index,
// as it's needed for various statistics (index dives for instance)
#if 0
/**
...
bool update_histogram(THD *thd, Table_ref *table, const columns_set &columns,
  ...
  */
  ...
  for (const std::string &column_name : columns) {
    ...
    // Disabled for dstore so we can create histogram on columns with unique
    // index, as it's needed for various statistics (index dives for instance)
#if 0
    /*
  ...
  */
  ...
  /*
    collect all resolved_fields histogram and replace to
    table_share.m_histograms after store_histogram commit
  */
  std::unordered_map<uint16_t, HistogramAndMemRoot> tmp_histograms;
  ...
  for (const Field *field : resolved_fields) {
    ...
    */
    ...
    tmp_histograms.emplace(field->field_index(),
  ...
  /*
    replace m_histograms of table_share
  */
  mysql_rwlock_wrlock(&table->table->s->LOCK_m_histograms);
  /*
    Refresh the table share with the new histograms
  */
  for (auto &tmp_histogram : tmp_histograms) {
    table->table->s->m_histograms->erase(tmp_histogram.first);
    table->table->s->m_histograms->emplace(tmp_histogram.first,
                                           std::move(tmp_histogram.second));
  }
  mysql_rwlock_unlock(&table->table->s->LOCK_m_histograms);
...
bool drop_histograms(THD *thd, Table_ref &table, const columns_set &columns,
  ...
  // ensure that any open tables are closed before returning
  auto tables_guard = create_scope_guard([thd, &needs_lock]() {
    if (needs_lock) {
      close_thread_tables(thd);
    }
  });
  ...
  if (needs_lock) {
    ...
    */
    ...
    /**
      Modify replace histogram into table_share::m_histograms.
      So we open the table ensure table_share object is valid.
    */
    if (open_and_lock_tables(thd, &table, 0)) {
      return true;
    }
  ...
  for (const std::string &column_name : columns) {
    ...
    if (needs_lock) {
      ...
      mysql_rwlock_wrlock(&table.table->s->LOCK_m_histograms);
      table.table->s->m_histograms->erase(field->field_index());
      mysql_rwlock_unlock(&table.table->s->LOCK_m_histograms);
    }
...
bool Histogram::store_histogram(THD *thd) const {
  ...
  DEBUG_SYNC(thd, "store_histogram_before_write_lock");
...
bool Histogram::get_selectivity_dispatcher(Item *item, const enum_operator op,
  ...
  DBUG_PRINT("print_histgram_recived_values",
             ("Enter get_selectivity_dispatcher"));
  switch (this->get_data_type()) {
    ...
    case Value_map_type::STRING: {
      ...
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %s",
                  current_thd->query().str,
                  std::string(str->ptr(), str->length()).c_str()));
    ...
    case Value_map_type::INT: {
      ...
      if (item->is_null()) return true;
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %lld",
                  current_thd->query().str, value));
    ...
    case Value_map_type::UINT: {
      ...
      if (item->is_null()) return true;
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %llu",
                  current_thd->query().str, value));
    ...
    case Value_map_type::DOUBLE: {
      ...
      if (item->is_null()) return true;
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %lf",
                  current_thd->query().str, value));
    ...
    case Value_map_type::DECIMAL: {
      ...
      if (item->is_null() || value == nullptr) return true;
#ifndef NDEBUG
      String print_buf;
      item->val_str(&print_buf);
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %s",
                  current_thd->query().str,
                  std::string(print_buf.ptr(), print_buf.length()).c_str()));
#endif  // NDEBUG
    ...
    case Value_map_type::DATETIME: {
      ...
      my_TIME_to_str(temporal_value, print_buff, 6);
      DBUG_PRINT("print_histgram_recived_values",
                 ("For query: %s, Histogram received values: %s",
                  current_thd->query().str,

...
class Diagnostics_area {
  ...
  */
  ...
  void accumulate_counts(bool x) { m_accumulate_counts = x; }

  bool accumulate_counts() const { return m_accumulate_counts; }
 ...
 */
  ...
  */
  ...
  /** In a bulk execution of a prepared statement, the server must send to the
  client the total number of affected rows (of all iterations), so a same DA
  must accumulate counts of each iteration in a grand total. */
  bool m_accumulate_counts{false};

...
#include "sql/sql_plan_cache.h"  // invalidate_cached_plan
...
class Statement_backup {
  ...
  */
  void restore_thd(THD *thd, Prepared_statement *stmt) {
    ...
    /**
     Certain DDL statements (e.g., DROP SPATIAL REFERENCE SYSTEM) may internally
     initiate a prepared statement during execution. Re-set the DDL info to
     ensure proper context preservation across the nested statement execution.
     */
    if (need_ddl_info_sql(thd)) {
      /*
        The DDL sql size has been checked in mysql_execute_command,
        so this should always succeed (except for OOM).
      */
      (void)thd->set_ddl_sql(m_query_string);
    }
  ...
  *stmt = nullptr;
  ...
  switch (cmd) {
    ...
    case COM_STMT_BULK_EXECUTE: {
      stmt_id = com_data->com_stmt_bulk_execute.stmt_id;
...
bool Prepared_statement::insert_parameters_bulk(THD *thd, String *query) {
  uchar **read_pos = &m_bulk.read_pos;
  size_t *packet_left = &m_bulk.packet_left;
  size_t query_length = 0;

  /* Reserve an extra space of 32 bytes for each placeholder parameter. A
  statement with CF_PS_ARRAY_BINDING_OPTIMIZED handles all iterations
  internally, only the first statement in bulk will be printed to general
  log. Append an extra comment in the begin of the statement to inform
  customers that there might be more parameter sets. */
  if (m_with_log && query) {
    static const char *EXTRA_COMMENT =
        "/* Bulk exection, might have more parameter sets */";
    static const size_t EXTRA_COMMENT_SIZE = strlen(EXTRA_COMMENT);
    bool append_comment =
        sql_command_flags[m_lex->sql_command] & CF_PS_ARRAY_BINDING_OPTIMIZED;
    if (query->reserve(m_query_string.length + 32 * m_param_count +
                       (append_comment ? EXTRA_COMMENT_SIZE : 0))) {
      return true;
    }
    if (append_comment) {
      query->append(EXTRA_COMMENT, EXTRA_COMMENT_SIZE);
    }
  }

  uint i;
  Item_param **it;
  for (i = 0, it = m_param_array; i < m_param_count; ++i, ++it) {
    const enum enum_field_types ptype = m_bulk.parameters[i].type;
    const uchar punsigned_type = m_bulk.parameters[i].unsigned_type;
    uchar pnull_bit = false;
    const uchar *pvalue = nullptr;
    ulong plength = 0;

    /* Step 1: Parse from packet */

    /* Step 1.1: Parse indicator */
    if (*packet_left < 1) {
      return true;
    }

    uchar indicator = **read_pos;
    *read_pos += 1;
    *packet_left -= 1;

    if (indicator != STMT_INDICATOR_NONE) {
      if (indicator != STMT_INDICATOR_NULL) {
        return true;
      }
      pnull_bit = true;
    } else {
      /* Step 1.2: Parse bind data */
      if (ptype == MYSQL_TYPE_BOOL)
      ...
      plength = get_ps_param_len(ptype, *read_pos, *packet_left, &header_len,
                                 &buffer_underrun);
    ...
    /* Step 2: Set Item_param */
    Item_param *const param = *it;
    set_parameter_type(param, ptype, punsigned_type,
                       thd->variables.character_set_client);
    // Client may have provided invalid metadata:
    if (param->data_type_source() == MYSQL_TYPE_INVALID) {
      return true;
    ...
    if (pnull_bit) {
      ...
      if (set_parameter_value(param, &pvalue, plength)) {
        return true;
      }
    ...
    if (m_with_log && query) {
      ...
      size_t num_bytes = param->pos_in_query - query_length;
      if (query->length() + num_bytes + val->length() >
          std::numeric_limits<uint32>::max()) {
        return true;
      }
      if (query->append(m_query_string.str + query_length, num_bytes) ||
          query->append(*val)) {
        return true;
      }

      query_length = param->pos_in_query + 1;
    } else {
      if (param->convert_value()) return true; /* out of memory */
    }
    param->sync_clones();
  ...
  // Copy part of query string after last parameter marker
  if (m_with_log && query &&
      query->append(m_query_string.str + query_length,
                    m_query_string.length - query_length))
    return true;

  return false;
}

void mysqld_stmt_bulk_execute(THD *thd, Prepared_statement *stmt, ulong flags) {
  DBUG_TRACE;
  ...
  /* STMT_BULK_FLAG_CLIENT_SEND_TYPES must be set and
   * STMT_BULK_FLAG_SEND_UNIT_RESULTS is not supported. */
  if (!(flags & STMT_BULK_FLAG_CLIENT_SEND_TYPES) ||
      (flags & STMT_BULK_FLAG_SEND_UNIT_RESULTS)) {
    my_printf_error(ER_UNSUPPORTED_PS,
                    "Flags %lu is invalid for bulk execution", MYF(0), flags);
    return;
  }

  uint command_flags = sql_command_flags[stmt->m_lex->sql_command];
  /* Only support insert/replace/update/replace commands. */
  if (!(command_flags & CF_PS_ARRAY_BINDING_SAFE)) {
    DBUG_PRINT("error", ("Command is not supported in bulk execution."));
    my_printf_error(ER_UNSUPPORTED_PS,
                    "Command is not supported in bulk execution", MYF(0));
    return;
  }

  /* Reject statement that has no parameter. Because without parameters, server
   * doesn't know how many interations to execute. */
  if (stmt->m_param_count == 0) {
  ...
  for (uint i = 0; i < stmt->m_param_count; ++i) {
    if (stmt->m_param_array[i]->param_state() == Item_param::LONG_DATA_VALUE) {
      DBUG_PRINT("error",
  ...
  if (!(command_flags & CF_PS_ARRAY_BINDING_OPTIMIZED) &&
      !thd->in_multi_stmt_transaction_mode()) {
    /* A statement with CF_PS_ARRAY_BINDING_OPTIMIZED handles all iterations
       internally, so in case of error in the middle it rolls back all
       previous iterations. Statements without such flag do one execution per
       iteration, therefore, in autocommit mode, that implies a commit per
       iteration, so in case of error in the middle, previous iterations are
       already committed and the user has no way to know what they were, and
       what to undo. That is why we require the user to be in non-autocommit
       mode, he/she can then roll back the entire transaction and get back to
       a known state. If this is not good enough, we could look at silently
       creating a savepoint here and rolling back to it in case of error. */
    my_printf_error(ER_UNSUPPORTED_PS,
                    "This bulk execution requires to not be in AUTOCOMMIT mode",
                    MYF(0));
    return;
  }

  if ((command_flags & CF_PS_ARRAY_BINDING_OPTIMIZED) && opt_bin_log &&
      thd->variables.binlog_format != BINLOG_FORMAT_ROW) {
    /* A statement with CF_PS_ARRAY_BINDING_OPTIMIZED handles all iterations
       internally, only the first parameter set can be recorded in binlog
       STATEMENT or MIX format mode. So if binlog is enabled, only ROW format
       is allowed.  */
    my_printf_error(ER_UNSUPPORTED_PS,
                    "This bulk execution requires binlog_format to be ROW if "
                    "binlog is enabled.",
                    MYF(0));
    return;
  }

  DBUG_EXECUTE_IF("bulk_test_slow_query", { my_sleep(2000000); });
  ...
  // secondary engine should be disabled in this way since it only supports DML
  const auto saved_secondary_engine = thd->secondary_engine_optimization();
  thd->set_secondary_engine_optimization(
  ...
  assert(!thd->get_stmt_da()->accumulate_counts());
  ...
  for (;;) {
    ...
    if (stmt->set_parameters_bulk(thd, &expanded_query)) break;

    if (stmt->execute_loop(thd, &expanded_query, false)) break;

    if (!stmt->more_bulk_iterations()) break;

    // Notify our Diagnostics_area to accumulate counters for all iterations.
    thd->get_stmt_da()->accumulate_counts(true);

    // Commands with this flag handle their iterations internally.
    assert(!(command_flags & CF_PS_ARRAY_BINDING_OPTIMIZED));
  }
  ...
  thd->get_stmt_da()->accumulate_counts(false);
}
...
bool Prepared_statement::prepare(THD *thd, const char *query_str,
  ...
  */
  ...
  // Disable transform if execute failed for
  // Item_func_in::fill_transform_tmp_table.
  m_lex->disable_in_predicate_transform_to_subq =
      disable_in_predicate_transform_to_subq;
...
bool Prepared_statement::set_parameters_bulk(THD *thd, String *expanded_query) {
  if (insert_parameters_bulk(thd, expanded_query)) {
    reset_stmt_parameters(this);
    my_printf_error(ER_UNSUPPORTED_PS, "Incorrect bulk execution packet",
                    MYF(0));
    return true;
  }
...
bool Prepared_statement::execute_loop(THD *thd, String *expanded_query,
  ...
  assert(!thd->get_stmt_da()->is_set() ||
         thd->get_stmt_da()->accumulate_counts());
  ...
  */
  ...
  auto execute_guard = create_scope_guard([&]() {
    ...
    */
    ...
    // Used to record whether to disable IN-list conversion for
    // Item_func_in::fill_transform_tmp_table. Note: such an assignment should
    // before THD::cleanup_after_query.
    disable_in_predicate_transform_to_subq =
        m_lex->disable_in_predicate_transform_to_subq;

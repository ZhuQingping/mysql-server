...
class sub_partition_info;
class Base_partition {};

/*
  Partition is used to manage necessary information for main partition.
        Currently, it's not used because partition_info can do it.
  e.g
  PARTITION BY (col1) SUBPARTITION BY (col2) {
    ================Partition ==============================
  ...
  As the above example showing, One Partition has num_parts Subpartitions.
  Subpartition helps to manager sub-partitions information for one main
  partition.
*/
class Sub_partition : Base_partition {
 public:
  sub_partition_info *part_info;
  uint index;
  uint num_list_values{0};
  bool has_null_value{false};
  uint has_null_part_id{0};
  bool defined_max_value{false};
  Sub_partition(sub_partition_info *sub_part_info, uint id)
      : part_info(sub_part_info), index(id) {}
  union {
  ...
  void fix_val_array_pointer();
  Sub_partition *clone(THD *thd, uint part_id, bool reset);
};
class Partition : Base_partition {
 public:
  List<Sub_partition> subpartitions;
  partition_info *part_info;
  void set_up_partition_func_pointers();
};
...
class partition_info {
  ...
  */
  ...
  // For plan cache, not for PQ
  bool pc_copy_from(partition_info *part_info);
  bool pq_copy_from(partition_info *part_info);
  ...
  uint get_num_subparts() const;
  uint get_subpart_func_len() const;
  Item *get_subpart_expr();
  bool get_use_default_subparts() const;
  bool get_use_default_num_subparts() const;
  ...
  bool check_list_constants_on_partition(
  ...
  bool check_partition_info(THD *thd, handlerton **eng_type, handler *file,
                            HA_CREATE_INFO *info, bool check_partition_function,
  ...
  */
  ...
  virtual inline bool is_subpart_info() const { return false; }
  ...
  */
  ...
  // Clone current partition_info
  partition_info *clone(THD *thd, bool reset);
  virtual partition_info *get_parent() { return nullptr; }
  virtual void set_parent(partition_info *) {}
  virtual void fix_val_array_pointer(int) {}
  void fix_subs_val_array_pointers();

  // This function isn't marked to be a virtual function because of compliling
  // error.
  bool set_up_default_partitions(Partition_handler *part_handler,
                                 HA_CREATE_INFO *info, uint start_no);

 protected:
  bool check_partition_info_helper(THD *thd, handler *file,
                                   HA_CREATE_INFO *info,
...
class sub_partition_info : public partition_info {
  typedef partition_info super;

 private:
  partition_info *m_parent{nullptr};

 public:
  sub_partition_info() : super() {}
  sub_partition_info(partition_info *parent) : super() { m_parent = parent; }
  void set_parent(partition_info *part_info) override { m_parent = part_info; }
  inline bool is_subpart_info() const override { return true; }
  partition_info *get_parent() override { return m_parent; }
  void fix_val_array_pointer(int sub_id) override;
  bool set_up_default_partitions(Partition_handler *part_handler,
                                 HA_CREATE_INFO *info, uint start_no);

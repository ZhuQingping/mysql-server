  ...
  switch (item->result_type()) {
    ...
    case STRING_RESULT:
      ...
      */
      ...
      if (new_field) new_field->set_derivation(item->collation.derivation);
      break;
    case DECIMAL_RESULT:
...
Field *create_tmp_field_for_schema(const Item *item, TABLE *table) {
...
// TODO move this function into pq_clone.cc later
void Temp_table_param::pq_copy(Temp_table_param *orig) {
  if (!orig) {
    sql_print_information("pq copy a NULL tmp_table_param");
    return;
  }
...
/**
  ...
  @param force_disk_table     Enforce using the disk storage engine
  @param parallel_exec        Create temporary tables for data sending/receiving
                              between the leader and workers in parallel.
  ...
  for (Item *item : fields) {
    ...
    if (not_all_columns) {
      ...
      if (item->const_item() || item->basic_const_item())
        item->skip_send_to_mq = true;

      // const value and not table, will not create tmp fields
      // e.g. SELECT (SELECT 1 WHERE 2 IS NULL) FROM t0 GROUP BY t0.c0;
      if ((parallel_exec || thd->is_pq_worker()) && hidden_field_count <= 0 &&
          item->const_item()) {
        // mark this item and then we can identify it without sending it to MQ
        item->real_item()->skip_create_tmp_table = true;
        continue;  // We don't have to store this
      }

      // In implicit group-by queries, constants and execution-time constants
      // need to be passed between the gather and worker stages. To achieve
      // this, the fields must be placed into a temporary table. See reference:
      // parallel_query.BUG2024092003503.
      if (hidden_field_count <= 0 && !(parallel_exec || thd->is_pq_worker())) {
    ...
    if (store_column && is_sum_func && group == nullptr &&
      ...
      Field *new_field;
      Item_sum *old_item_sum_ref = nullptr;
      if (param->schema_table) {
        ...
        */
        if (item->get_tmp_table_field())
          old_item_sum_ref = item->get_tmp_table_field()->item_sum_ref;
      ...
      */
      if (modify_items && type == Item::SUM_FUNC_ITEM) {
        new_field->item_sum_ref = down_cast<Item_sum *>(item);
        down_cast<Item_sum *>(item)->set_result_field(new_field);
      } else if (item->type() == Item::FIELD_AVG_ITEM) {
        ...
        // In make_pq_gather_operator function, it will call
        // make_tmp_tables_info function to create tmp_fields[REF_SLICE_TMP1].
        // And change_to_use_tmp_fields function will change all fields in
        // tmp_fields[REF_SLICE_TMP1] to Item::FIELD_ITEM type. When running
        // make_leader_tmp_table function called by make_pq_leader_plan
        // function, if it has a item in tmp_fields[REF_SLICE_TMP1] which it's a
        // hidden field, MYSQL_TYPE_BIT type, in create_tmp_field function, it
        // will create a new_field losing item_sum_ref ptr if
        // item->field->item_sum_ref is not nullptr because it calls
        // create_tmp_field_from_item to create a field. So, this place should
        // save orig_field->item_sum_ref to new_field->item_sum_ref.
        new_field->item_sum_ref = old_item_sum_ref;
      }
    ...
    if (!unique_constraint_via_hash_field) {
      ...
      rec_per_key_t *rec_per_key_float =
          share->mem_root.ArrayAlloc<rec_per_key_t>(
              param->keyinfo->user_defined_key_parts);
      if (rec_per_key == nullptr ||     /* purecov: inspected */
          rec_per_key_float == nullptr) /* purecov: inspected */
...
bool setup_tmp_table_handler(THD *thd, TABLE *table, ulonglong select_options,
  ...
  if (table->file == nullptr) {
    if (locked_plugin) {  // restore previous state
...
PQ_temp_table_type get_tmp_table_type(TABLE_SHARE *share) {
  if (!share) return PQ_temp_table_type::TEMP_UNKNOWN;
  if (share->db_type() == innodb_hton) {
    return PQ_temp_table_type::TEMP_INNODB;
  } else if (share->db_type() == heap_hton) {
    return PQ_temp_table_type::TEMP_HEAP;
  } else if (share->db_type() == temptable_hton) {
    return PQ_temp_table_type::TEMP_MEM;
  }

  return PQ_temp_table_type::TEMP_UNKNOWN;
}
...
static bool create_tmp_table_with_fallback(THD *thd, TABLE *table) {
  ...
  */
  ...
  DBUG_EXECUTE_IF("force_on_disk_tmp_table",
  ...
  if (error == HA_ERR_RECORD_FILE_FULL &&
      table->s->db_type() == temptable_hton) {
    bool forceTmpTableHton = false;

    DBUG_EXECUTE_IF("force_on_disk_tmp_table",
                    { DBUG_SET("-d,temptable_allocator_record_file_full"); });

    /** Check if there is a index on 0 size field. If this is the case
     * try to force usage of TmpTable. INNODB cannot index 0 size fields.
     */
    for (uint32_t i = 0; i < table->s->keys; ++i) {
      KEY *key = table->s->key_info + i;
      if (0 == key->key_length) {
        forceTmpTableHton = true;
        break;
      }
    }

    if (forceTmpTableHton) {
      const uint32_t maxTries = 10;

      for (uint32_t i = 0; error == HA_ERR_RECORD_FILE_FULL && i < maxTries;
           ++i) {
        /** Give the OS a bit of time to free some space for temp table */
        sleep(1);

        std::ostringstream oss_warning;
        oss_warning
            << "Failed to create a tmp table in TempTable storage engine - "
               "due to lack of space. We have an indexed field of size 0. "
               "We try to not fall back to creating on disk tmp table in "
               "INNODB engine - "
               "as this engine cannot create index on field of size 0. "
               "Retry "
            << i + 1 << " out of " << maxTries;
        std::string warning = oss_warning.str();
        assert(warning.length() < MYSQL_ERRMSG_SIZE);
      ...
      if (error == HA_ERR_RECORD_FILE_FULL) {
        ...
        oss_warning
            << "Failed to create a tmp table in TempTable storage engine - "
               "due to lack of space. We have tried "
            << maxTries
            << " times. Although we have "
               "an indexed field of size 0, we now fallback to INNODB storage "
               "engine. "
               "As it cannot index 0 size field it will generate appropriate "
               "error message.";
  ...
  if (error == HA_ERR_RECORD_FILE_FULL &&
    ...
    table->s->db_plugin = ha_lock_engine(nullptr, innodb_hton);
...
void free_tmp_table(TABLE *table) {
  ...
  if (table->pq_shared_table && table->s->ref_count() > 0) {
    /*
      In single-threaded execution, plugin unlocking and other operations
      on such tmp table are done by close_tmp_table() which is called by
      Query_block::cleanup(). Whereas in PQ execution:
      - These are not called, only Query_block::destroy() is (which brings us
      here).
      - close_tmp_table() is carried by QEP_TAB::cleanup().
      - But if PQ execution fails before QEP_TABs are created, then
      QEP_TAB::cleanup() is not called. Therefore we need to handle this

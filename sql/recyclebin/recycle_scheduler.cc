...
ulonglong rds_recycle_scheduler_interval = 1;
/* Whether run recycle scheduler thread */
bool rds_recycle_scheduler = false;
/* Whether print the detailed information when purge table */
bool rds_recycle_scheduler_purge_table_print = false;
...
bool Recycle_scheduler::purge_table(struct Recycle_show_result *recycle_table) {
  ...
  String cmd;
  if (cmd.append(STRING_WITH_LEN("call dbms_recyclebin.purge_table(\'")) ||
      cmd.append(table, strlen(table)) || cmd.append("\')")) {
    DBUG_RETURN(true);
  }
  thd->set_query(cmd.lex_cstring());

  thd->variables.rds_recycle_bin_mode = true;
  ...
  bool res = drop_base_recycle_table(thd, tables);
  bool result;
  if (res || error_handler.is_error() || thd->killed) {
    result = (trans_rollback_stmt(thd) || trans_rollback_implicit(thd));
  } else {
    result = (trans_commit_stmt(thd) || trans_commit_implicit(thd));
    if (result) trans_rollback(thd);
  }

#ifdef HAVE_REPLICA_MDL
  if (rpl_is_ddl_command(thd) > 0 && thd->has_mdl_logged) {
    rpl_log_mdl_unlock_requests(thd);
  }
#endif /* HAVE_REPLICA_MDL */

  close_thread_tables(thd);
  thd->mdl_context.release_transactional_locks();
#ifdef HAVE_REPLICA_MDL
  thd->release_table_prelock();
#endif /* HAVE_REPLICA_MDL */

  if (result) log_purge_error(table, "transaction commit or rollback failed");
  thd->reset_query();
...
*/
static void post_init_recycle_thread(THD *thd) {
...
/**
  ...
  @param[in]    thd     scheduler thread
*/
static void deinit_recycle_thread(THD *thd) {
...
void Recycle_scheduler::run(THD *thd) {
  ...
  while (is_running()) {
    ...
    ulonglong lc_retention = rds_recycle_bin_retention;
    curr_time_sec = static_cast<my_time_t>(my_micro_time() / 1000000);
    ulonglong min_future = ULLONG_MAX;

    /*
       1. Read_only node only replays purge_table from binlog.
       2. When rds_recycle_bin_skip_acl_check is true,
          read_only node cann't replay purge_table from binlog.
    */
    if (!read_only && !rds_recycle_bin_skip_acl_check) {
      ...
      /**
        Next round time calculation rule:
          ...
          2. Wait rds_recycle_scheduler_interval if not any recycled table.
          3. Wake up if modify the recycle_bin_retention.
      */
      for (auto it = container.cbegin(); it != container.cend(); it++) {
        if (is_running() && !read_only && !rds_recycle_bin_skip_acl_check) {
          long long delta =
              curr_time_sec - get_epoch_time((*it)->recycled_time);

          /* If the keeped time has exceeded retention, then purge it. */
          if (delta >= 0 && static_cast<ulonglong>(delta) >= lc_retention) {
            if (rds_recycle_scheduler_purge_table_print) {
              std::stringstream ss;
            ...
            ulonglong lc_future = lc_retention - static_cast<ulonglong>(delta);
    ...
    if (min_future == ULLONG_MAX)
      set_timespec(&abs_timeout, rds_recycle_scheduler_interval);
    else
...
*/
static void pre_init_recycle_thread(THD *thd) {
  DBUG_ENTER("pre_init_recycle_thread");
  thd->security_context()->set_master_access(~0U);

...
/**
  Proc base for dbms_recyclebin

  1) Uniform schema: dbms_recyclebin
*/
...
/**
  Sql command base for dbms_recyclebin

  1) dbms_recyclebin didn't require any privileges;
*/
...
/**
  2) dbms_recyclebin.purge_table(table_name);
...
class Sql_cmd_recycle_proc_purge : public Sql_cmd_recycle_proc_base {
  ...
  */
  ...
  /* Require DROP_ACL privilege on purge table, don't do this in check_access
   * before check_parameter */
  virtual bool prepare(THD *thd) override;
};
...
class Sql_cmd_recycle_proc_restore : public Sql_cmd_recycle_proc_base {
  ...
  */
  ...
  /**
    ...
    Require ALTER_ACL and DROP_ACL privilege on restore table,
...
/**
  4) dbms_recycle.restore_database(db_name, new_db_name);
*/
class Sql_cmd_recycle_proc_restore_db : public Sql_cmd_recycle_proc_base {
 public:
  explicit Sql_cmd_recycle_proc_restore_db(THD *thd,
  ...
  */
  ...
  const char *m_origin_db = nullptr;
  const char *m_new_db = nullptr;
};

class Recycle_proc_restore_db : public Recycle_proc_base {
  using Sql_cmd_type = Sql_cmd_recycle_proc_restore_db;

  /* All the parameters */
  enum enum_parameter {
    RECYCLE_PARAM_DATABASE = 0,
    RECYCLE_PARAM_NEW_DATABASE = 1,
    RECYCLE_PARAM_LAST
  };
 ...
 public:
  explicit Recycle_proc_restore_db(PSI_memory_key key)
      : Recycle_proc_base(key), m_parameters_2(key) {
    /* Only OK or ERROR protocol packet */
    m_result_type = Result_type::RESULT_OK;

    /* Init parameters */
    for (size_t i = RECYCLE_PARAM_DATABASE; i < RECYCLE_PARAM_LAST; i++) {
      m_parameters.assign_at(
          i, get_field_type(static_cast<enum enum_parameter>(i)));
    }

    for (size_t i = RECYCLE_PARAM_DATABASE; i < RECYCLE_PARAM_NEW_DATABASE;
         i++) {
      m_parameters_2.assign_at(
          i, get_field_type(static_cast<enum enum_parameter>(i)));
    }
  ...
  */
  ...
  virtual ~Recycle_proc_restore_db() {}

  /* Proc name */
  virtual const std::string str() const override {
    return std::string("restore_db");
  }

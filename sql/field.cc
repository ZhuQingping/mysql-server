...
/*
 * @TODO: the count variable will move to MQ
 *
 ...
 */
void Field::store_extra(const uchar *extra, size_t len) {
  if (len == 0 || extra == nullptr) return;
...
type_conversion_status Field_new_decimal::reset() {
  ...
  if (extra_length > 0) memset(ptr + bin_size, 0, extra_length);
...
Field *Field_enum::new_field(MEM_ROOT *root, TABLE *new_table) const {
  ...
  if (!res || !(res->typelib = copy_typelib(root, typelib))) return nullptr;
  return res;
}

...
class Field {
 ...
 public:
  /// structures for parallel query
  /** ref to Item_sum for rebuilt */
  Item_sum *item_sum_ref{nullptr};
  /** extra length to store avg->count, will removed in furture */
  uint32 extra_length{0};
  /** store extra_length in (field->ptr - 8) */
  bool pq_worker_fields{false};
  void store_extra(const uchar *extra, size_t len);

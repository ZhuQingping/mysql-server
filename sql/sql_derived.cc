...
TABLE *Common_table_expr::clone_tmp_table(THD *thd, Table_ref *tl) {
  ...
  */
  ...
  // When cloning tmp table from another tmp table using TABLE_SHARE,
  // the hidden_field_count should be copied. At the same time,
  // the hash_field used to enforce unique constraint is also needed to
  // refer to the first field in the cloned tmp table.
  t->hidden_field_count = first->hidden_field_count;
  if (first->hash_field) t->hash_field = t->field[0];
...
static bool derived_contain_rollup_item(Query_expression *derived) {
  for (Item *item : VisibleFields(*derived->get_unit_column_types())) {
    if (item->type() == Item::FUNC_ITEM) {
      Item_func *func = static_cast<Item_func *>(item);
      if (func->functype() == Item_func::ROLLUP_GROUP_ITEM_FUNC) {
        return true;
      }
    }
  }
...

{
  ...
  if (table == nullptr) {
    ...
    // When derived table contain group by rollup item, it will create tmp table
    // for rollup base on derived tmp table. Then "get_unit_column_types()"
    // refer to rollup tmp table fields instead of derived tmp fields. while PQ

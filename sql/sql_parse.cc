...
const std::string Command_names::m_names[] = {
    ...
    "",              // 253
    "",              // 254
    "Error"          // Last command number 255
};

/** This guard is used to adjust the lock_wait_timeout for
ALTER TABLE(currently) to the min value of lock_wait_timeout and
rds_ddl_lock_wait_timeout, also make sure it would be restored finally */
class DDL_timeout_guard {
 public:
  /** Constructor
  @param[in]	thd	thread handler */
  explicit DDL_timeout_guard(THD *thd) : m_thd(thd), m_backup(0) {
    if (m_thd->variables.rds_ddl_lock_wait_timeout <
        m_thd->variables.lock_wait_timeout) {
      m_backup = m_thd->variables.lock_wait_timeout;
      thd->variables.lock_wait_timeout =
          thd->variables.rds_ddl_lock_wait_timeout;
    }
  }
  ...
  ~DDL_timeout_guard() {
    if (m_backup != 0) {
      m_thd->variables.lock_wait_timeout = m_backup;
    }
  }

 private:
  /** Thread handler to run the alter table */
  THD *m_thd;

  /** Backup lock_wait_timeout if necessary */
  ulong m_backup;
};

static const LEX_CSTRING INSTALL_PLUGIN = {STRING_WITH_LEN("INSTALL PLUGIN")};
static const LEX_CSTRING UNINSTALL_PLUGIN = {
    STRING_WITH_LEN("UNINSTALL PLUGIN")};
static const LEX_CSTRING INSTALL_COMPONENT = {
    STRING_WITH_LEN("INSTALL COMPONENT")};
static const LEX_CSTRING UNINSTALL_COMPONENT = {
    STRING_WITH_LEN("UNINSTALL COMPONENT")};
...
void init_sql_command_flags() {
  ...
  */
  ...
  sql_command_flags[SQLCOM_UPDATE] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_PS_ARRAY_BINDING_SAFE;
  sql_command_flags[SQLCOM_UPDATE_MULTI] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_PS_ARRAY_BINDING_SAFE;
  // This is INSERT VALUES(...), can be VALUES(stored_func()) so we trace it
  sql_command_flags[SQLCOM_INSERT] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_PS_ARRAY_BINDING_SAFE |
  ...
  sql_command_flags[SQLCOM_DELETE] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_PS_ARRAY_BINDING_SAFE;
  sql_command_flags[SQLCOM_DELETE_MULTI] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED;
  sql_command_flags[SQLCOM_REPLACE] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED | CF_PS_ARRAY_BINDING_SAFE |
      CF_PS_ARRAY_BINDING_OPTIMIZED;
...
*/
static void shrink_packet_buffer(THD *thd, unsigned long *max_interval_packet,
                                 unsigned long long *net_buffer_shrink_time) {
  /* When net_buffer_shrink_interval is 0, need not shrink. */
  if (net_buffer_shrink_interval == 0) {
    return;
  }

  /*
    Converts the microsecond of start_time for reading net buff
    to the second of start_time.
  */
  auto start_time = thd->start_utime / 1000000;
  /*
    When the interval of the last shrink net buff is less than
    or equal to net_buffer_shrink_interval, need not shrink.
  */
  if (start_time <= *net_buffer_shrink_time + net_buffer_shrink_interval) {
    return;
  }

  my_net_shrink_buffer(thd->get_protocol_classic()->get_net(),
                       thd->variables.net_buffer_length, max_interval_packet);
  *net_buffer_shrink_time = start_time;
}
...
static bool deny_updates_if_read_only_option(THD *thd, Table_ref *all_tables) {
  ...
  // Due to the current lack of complete index statistics in dstore, the analyze
  // command is currently allowed only in read-only mode.
  if (read_only && lex->sql_command == SQLCOM_ANALYZE &&
...
/**
  Check if a statement should be restarted without parallel query. This is a
  fallback mechanism that is used to catch queries that failes to execute using
  parallel query, but can be executed without parallel query. The conditions for
  restarting a query is currently:

  1) The error ER_PARALLEL_FAIL_INIT must have been raised.
  2) The statement must be a SQL command.
  3) The error must happend before we start sending results to the client.

  Currently, 2 and 3 is implied by 1. But we keep 2 and 3 as a safeguard if
  suddenly ER_PARALLEL_FAIL_INIT starts to be raised from other places in the
  code (as of writing, it is only raised one place in the code).

  Note that this is mostly a copy-paste of the function
  "check_secondary_engine_statement", and any changes that happens there should
  probably be applied to this function as well.
...
*/
static void retry_without_parallel_query(THD *thd, Parser_state *parser_state,
                                         const char *query_string,
                                         size_t query_length) {
  if (!thd->variables.parallel_fail_retry) {
    // This feature is disabled by the system variable, so just return.
    return;
  }
  ...
  const Sql_condition *error_condition = thd->get_stmt_da()->error_condition();
  if (error_condition == nullptr ||
      error_condition->mysql_errno() != ER_PARALLEL_FAIL_INIT) {
    return;
  }

  // Only SQL DML commands can be restarted.
  if (thd->lex->m_sql_cmd == nullptr || !thd->lex->m_sql_cmd->is_dml()) return;
  ...
  // Set parallel_cost_threshold to the max value to avoid parallel query being
  // chosen again.
  ...
  // Restore the variables that was temporarily changed.
  thd->variables.option_bits = saved_option_bits;
  thd->variables.parallel_cost_threshold = saved_parallel_cost_threshold;
}
...
bool dispatch_command(THD *thd, const COM_DATA *com_data,
  ...
  /* keep it within 1 byte */
  static_assert(COM_END == 255);
  ...
  */
  ...
  switch (command) {
    ...
    case COM_STMT_BULK_EXECUTE: {
      if (!thd->variables.rds_support_bulk_execution ||
          !(thd->get_protocol_classic()->get_extended_client_capabilites() &
            CLIENT_STMT_BULK_OPERATIONS)) {
        /* Refuse COM_STMT_BULK_EXECUTE request if rds_support_bulk_execution is
        disabled or client doesn't provide CLIENT_STMT_BULK_OPERATIONS
        capability. In most cases, if rds_support_bulk_execution is disable,
        client won't
        send extended capabilities flag to server. But double check to confirm.
      */
        my_error(ER_UNKNOWN_COM_ERROR, MYF(0));
        break;
      }
      ...
      if (!mysql_stmt_precheck(thd, com_data, command, &stmt)) {
        stmt->m_bulk.read_pos =
            (uchar *)thd->alloc(com_data->com_stmt_bulk_execute.packet_left);
        if (stmt->m_bulk.read_pos == nullptr) {
          my_error(ER_OUTOFMEMORY, MYF(0));
        }
        memcpy_s(stmt->m_bulk.read_pos,
                 com_data->com_stmt_bulk_execute.packet_left,
                 com_data->com_stmt_bulk_execute.read_pos,
                 com_data->com_stmt_bulk_execute.packet_left);
        stmt->m_bulk.packet_left = com_data->com_stmt_bulk_execute.packet_left;
        stmt->m_bulk.parameters = com_data->com_stmt_bulk_execute.parameters;

        mysqld_stmt_bulk_execute(thd, stmt,
                                 com_data->com_stmt_bulk_execute.flags);

        // Clean up in case the next execution is non-bulk
        stmt->m_bulk.parameters = nullptr;
        stmt->m_bulk.packet_left = 0;
        stmt->m_bulk.read_pos = nullptr;
      }
      break;
    }
    ...
    case COM_QUERY: {
      ...
      retry_without_parallel_query(thd, &parser_state, orig_query.str,
                                   orig_query.length);
...
/* SHOW PROFILE begin */
        ...
        */
        ...
        dispatch_sql_command(thd, &parser_state, true);
        retry_without_parallel_query(thd, &parser_state, beginning_of_next_stmt,
                                     length);
...
static bool lock_binlog_for_backup(THD *thd) {
  ...
  if (!rds_binlog_lock_enable) {
    my_error(ER_BINLOG_LOCK_NOT_ENABLED, MYF(0));
    DBUG_RETURN(true);
  }
...
int mysql_execute_command(THD *thd, bool first_level) {
  ...
  if (need_ddl_info_sql(thd) && thd->set_ddl_sql(thd->query())) {
    return 1;
  }
  ...
  */
  ...
  if (thd->retry_without_pq) {
    ...
    push_warning(thd, Sql_condition::SL_WARNING, ER_PARALLEL_QUERY_ERROR,
                 "Failed to execute the query using parallel query, the query "
                 "was restarted without parallel query.");
  }
  ...
  */
  if (stmt_causes_implicit_commit(thd, CF_IMPLICIT_COMMIT_BEGIN)) {
    ...
    */
    ...
    /* Temporarily disable DDL info writing because we are performing an
      implicit commit of last active transaction, not committing the DDL
      statement itself.
    */
    Disable_ddl_info_write_guard disable_ddl_info_gaurd(thd);
  ...
  /*
    Wether need to throttle the concurrency of ddl querys, see
    Sys_rds_dstore_ddl_max_concurrency for more details.
  */
  bool need_ddl_throttle =
      (get_ddl_command_type(thd) != DDLCOM_NONE &&
       ...
       !thd->is_server_upgrade_thread() && !thd->is_init_file_system_thread() &&
       !thd->is_dd_system_thread() &&
       thd->system_thread != SYSTEM_THREAD_BACKGROUND &&
       !thd->for_ddl_info_replay);
  bool need_release_ddl_concurrency = false;

  if (need_ddl_throttle) {
    need_release_ddl_concurrency = ddl_concurrency_throttle(thd);
    if (!need_release_ddl_concurrency && thd->is_error()) {
      return -1;
    }
  }

  auto autoRelease = create_scope_guard([&]() {
    if (need_release_ddl_concurrency) {
      ddl_concurrency_release(thd->thread_id());
    }
  });
  ...
  */
  ...
  // if replaying ddl info, we need insert {end_lsn, ddl info} at trace table at
  // first
  /* All currently identified DDL operations will disable the autocommit flag.
   After disabling autocommit, inserting {end_lsn, ddl info} into the trace
   table will start a session-level transaction,
   ensuring the overall atomicity of both the record insertion and DDL
   execution. */
  if (rds_use_ddl_info_replay && thd->for_ddl_info_replay) {
    const DDL_info_replay_buf &ddl_info_replay = thd->get_ddl_info_replay_buf();
    if (ddl_info_replay.need_write_to_trace) {
      int error = log_ddl_info_to_trace(
          thd, ddl_info_replay.end_lsn,
          reinterpret_cast<const char *>(ddl_info_replay.data),
          ddl_info_replay.data_len);
      thd->clean_ddl_info_for_trace();
      if (error) {
        return error;
      }
    ...
    DBUG_EXECUTE_IF("ddl_replay_after_insert_trace_core", {
      std::this_thread::sleep_for(std::chrono::seconds(2));
      DBUG_SUICIDE();
    });
  }
  /*
    Check whether the sql_filter function is turned on. If the maximum number
    of concurrency of a certain type is reached, this thread will be killed.
    query in stored procedure,function and triggers will not be limited.
  */
  ...
  */
  ...
  switch (lex->sql_command) {
    ...
    case SQLCOM_LOAD: {
      ...
      if (lex->sql_command == SQLCOM_CREATE_INDEX ||
          lex->sql_command == SQLCOM_DROP_INDEX) {
        DDL_timeout_guard guard(thd);
    ...
    case SQLCOM_DROP_TABLE: {
      ...
      if (!lex->drop_temporary) {
        if (check_table_access(thd, DROP_ACL, all_tables, false, UINT_MAX,
                               false))
          goto error; /* purecov: inspected */
        thd->recycle_state->set_type(
            im::recycle_bin::RECYCLE_BIN_RECYCLE_TABLE);
      }
    ...
    case SQLCOM_SHOW_PRODUCT_VERSION:
      res = static_cast<int>(mysqld_show_product_version(thd));
      break;
    case SQLCOM_SET_OPTION: {
    ...
    case SQLCOM_DROP_DB: {
      if (check_and_convert_db_name(&lex->name, false) != Ident_name_check::OK)
        break;
      if (opt_rds_permission_control &&
          (check_internal_db(thd, lex->name.str) != 0)) {
        break;
      }
      if (check_access(thd, DROP_ACL, lex->name.str, nullptr, nullptr, true,
                       false))
        break;
      thd->recycle_state->set_type(im::recycle_bin::RECYCLE_BIN_RECYCLE_DB);
      res = mysql_rm_db(thd, to_lex_cstring(lex->name), lex->drop_if_exists);
      if (opt_rds_sync_comment && !res) {
        delete_db_comment(thd, to_lex_cstring(lex->name));
      }
      break;
    }
    case SQLCOM_ALTER_DB: {
    ...
    {
      if (rds_disable_user_defined_functions) {
        my_error(ER_DISABLE_USER_DEFINED_FUNCTIONS, MYF(0));
        break;
      }
    ...
    case SQLCOM_CREATE_USER: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "CREATE USER") != 0)) {
        break;
      }
    ...
    case SQLCOM_DROP_USER: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "DROP USER") != 0)) {
        break;
      }
    ...
    case SQLCOM_RENAME_USER: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "RENAME USER") != 0)) {
        break;
      }
    ...
    case SQLCOM_REVOKE_ALL: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "REVOKE") != 0)) {
        break;
      }
      if (check_access(thd, UPDATE_ACL, "mysql", nullptr, nullptr, true,
                       true) &&
    ...
    case SQLCOM_GRANT: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "GRANT") != 0)) {
        break;
      }
    ...
    case SQLCOM_RESET:
      ...
      */
      ...
      if (opt_rds_permission_control &&
          ...
          (check_global_access_noerr(thd, SUPER_ACL) != 0)) {
    ...
    case SQLCOM_ANALYZE: {
      // Due to the current lack of complete index statistics in dstore, the
      // analyze command is currently allowed only in read-only mode.
      if (read_only && get_instance_storage_engine_type() == DB_TYPE_DSTORE) {
        thd->set_skip_readonly_check();
      }
    }
      [[fallthrough]];
    ...
    case SQLCOM_ADMIN_PROC: {
      ...
      /* Don't support xa command in Dstore. */
      if ((get_instance_storage_engine_type() == DB_TYPE_DSTORE) &&
          is_xa_transaction_boundary_stmt(lex->sql_command)) {
        my_error(ER_NOT_ALLOWED_COMMAND, MYF(0));
        goto error;
      }

      if (lex->sql_command == SQLCOM_INSTALL_PLUGIN &&
          opt_rds_permission_control &&
          check_install_uninstall_plugin_user(thd, INSTALL_PLUGIN.str) != 0) {
        break;
      } else if (lex->sql_command == SQLCOM_UNINSTALL_PLUGIN &&
                 opt_rds_permission_control &&
                 check_install_uninstall_plugin_user(
                     thd, UNINSTALL_PLUGIN.str) != 0) {
        break;
      } else if (lex->sql_command == SQLCOM_INSTALL_COMPONENT &&
                 opt_rds_permission_control &&
                 check_install_uninstall_plugin_user(
                     thd, INSTALL_COMPONENT.str) != 0) {
        break;
      } else if (lex->sql_command == SQLCOM_UNINSTALL_COMPONENT &&
                 opt_rds_permission_control &&
                 check_install_uninstall_plugin_user(
                     thd, UNINSTALL_COMPONENT.str) != 0) {
        break;
      }
    ...
    case SQLCOM_ALTER_USER: {
      if (opt_rds_permission_control &&
          (check_reserved_user(thd, lex->users_list, "ALTER USER") != 0)) {
        break;
      }
...
/**
  ...
  @param clean_pq_variables Clean parallel query variables
*/

void dispatch_sql_command(THD *thd, Parser_state *parser_state,
                          bool clean_pq_variables) {
  DBUG_TRACE;
  ...
  if (get_max_digest_length() != 0)
    parser_state->m_input.m_compute_digest = true;
  if (im::opt_outline_enabled) im::enable_digest_by_outline(parser_state);
  LEX *lex = thd->lex;
  ...
  thd->lex->destroy();
  thd->end_statement();
  thd->cleanup_after_query(clean_pq_variables);
...
Table_ref *Query_block::add_table_to_list(
  ...
  */
  ...
  bool is_internal_thread = thd->is_bootstrap_system_thread() ||
                            thd->is_server_upgrade_thread() ||
                            thd->system_thread == SYSTEM_THREAD_BACKGROUND;
  if (!is_internal_thread &&
      is_hidden_by_rds(ptr->db, ptr->db_length, ptr->table_name,

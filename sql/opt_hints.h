...
class Opt_hints_qb : public Opt_hints {
  ...
  /// Array of partial result cache hints
  ...
  */
  ...
  /**
    Checks if ptrc hints are applicable and
    get the table map.

    @param[in] join JOIN object
    @param[out] force_table_map table map forced to use ptrc
    @param[out] ignore_table_map table map are forced not to use ptrc
    @param[out] force_subquery_ptrc true means there is global hint to force
    subquery to use ptrc.
    @param[out] ignore_subquery_ptrc true means there is global hint to force
    not to use ptrc.
    @param[out] force_join_ptrc true means there is global hint to force NLJ to
    use ptrc.
    @param[out] ignore_join_ptrc true means there is global hint to force NLJ
    not to use ptrc.
  */

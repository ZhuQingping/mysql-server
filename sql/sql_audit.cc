...
int mysql_audit_notify(THD *thd, mysql_event_general_subclass_t subclass,
  ...
  event.general_default_db = thd->db();
  if (msg != nullptr) {
    if (!strcmp(msg, "Execute")) {
      event.general_sql_command = sql_statement_names[SQLCOM_EXECUTE];
      event.general_sql_cmd_enum = SQLCOM_EXECUTE;
    } else if (!strcmp(msg, "Prepare")) {
      event.general_sql_command = sql_statement_names[SQLCOM_PREPARE];
      event.general_sql_cmd_enum = SQLCOM_PREPARE;
    } else if (!strcmp(msg, "Close stmt")) {
      event.general_sql_command =
          sql_statement_names[SQLCOM_DEALLOCATE_PREPARE];
      event.general_sql_cmd_enum = SQLCOM_DEALLOCATE_PREPARE;
    } else {
      event.general_sql_command = sql_statement_names[thd->lex->sql_command];
      event.general_sql_cmd_enum = thd->lex->sql_command;
    }
  } else {
    event.general_sql_command = sql_statement_names[thd->lex->sql_command];
    event.general_sql_cmd_enum = thd->lex->sql_command;

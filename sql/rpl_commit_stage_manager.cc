...
void Commit_stage_manager::signal_done(THD *queue, StageID stage) {
  ...
  for (THD *thd = queue; thd; thd = thd->next_to_commit) {
    ...
    DBUG_EXECUTE_IF("flush_queue_wait_for_more_trxs", queue_size++;);
  }
  DBUG_EXECUTE_IF("flush_queue_wait_for_more_trxs", assert(queue_size == 16););

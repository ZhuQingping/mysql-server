...
inline bool ShouldCopyRowId(const TABLE *table) {
  ...
  return !table->const_table && !(table->is_nullable() && table->null_row) &&
         // For some virtual table or some materialized table which has not been
         // instantiated, we can't copy rowid.
         table->is_created();
}
...
ALWAYS_INLINE uchar *StoreFromTableBuffersRaw(const TableCollection &tables,
  ...
  for (const Table &tbl : tables.tables()) {
    ...
    // TABLE might be set to be null row, e.g read_const. In order to maintain
    // null row information, we need set TABLE to be nullable.

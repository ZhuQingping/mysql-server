...
bool Query_block::prepare(THD *thd, mem_root_deque<Item *> *insert_field_list) {
  ...
  */
  ...
  check_suite_for_pq_after_prepare(thd);
  assert(!thd->is_error());
  return false;
}
  ...
  */
  ...
  /*
    ...
    * check_only_full_group_by() is dependent on processing done by
    ...
    Also, We force "FULL_GROUP_BY" check in parallel query as the query can
    produce random results, which may be incompatible to normal query (ie.
    produces the different results compared with normal query).
   */
...
void Query_block::remap_tables(THD *thd) {
  ...
  /* As all tables get fresh maps, there is no risk of conflict between old
     and new maps so we can forget about old maps. Moreover, we have not yet
     done join elimination (which happens at the end of the resolution of the
     top Query_block, so, after all derived-table merging which is the caller
     here), so there are no gaps in table maps yet, neither in the merged
     query block nor in this parent one; the renumbering by using consecutive
     values as is done here, is thus correct. */
...
/**
  ...
  @param sub_select         Pointer to Item_exists_subselect.
*/
void Query_block::pop_sj_candidates(Item_exists_subselect *sub_select) {
  if (sj_candidates->size() == 0) {
    return;
  }
  auto last = sj_candidates->at(sj_candidates->size() - 1);
  if (last == sub_select) {
    sj_candidates->pop_back();
  }
}
...
bool Query_block::setup_conds(THD *thd) {
  ...
  */
  ...
  if (m_where_cond) {
    ...
    // Query_block::setup_conds function is also called in pq_select_prepare
    // function. Therefore, fix_fields_or_refix_fields function needs to be
    // invoked in this place. In PQ process, it has called refix_fields to
    // resolve item. In other process, it has called fix_fields to resolve item.
    if ((!m_where_cond->fixed &&
         m_where_cond->fix_fields_or_refix_fields(thd, &m_where_cond)) ||
...
bool Query_block::convert_subquery_to_semijoin(
  ...
  */
  ...
  if (subq_query_block->active_options() & OPTION_DEFAULT_WEAK_COLUMN)
    add_base_options(OPTION_DEFAULT_WEAK_COLUMN);
...
bool Query_block::merge_derived(THD *thd, Table_ref *derived_table) {
  ...
  */
  ...
  if (derived_query_block->active_options() & OPTION_SCHEMA_TABLE)
    add_base_options(OPTION_SCHEMA_TABLE);
  if (derived_query_block->active_options() & OPTION_DEFAULT_WEAK_COLUMN)
    add_base_options(OPTION_DEFAULT_WEAK_COLUMN);
...
/**
  ...
  @param[in] pq_resolved            True if order is resolved in parallel query.
   As each work clone the resolved order from leader, this order can be
   found in ref_item_array.
...
bool find_order_in_list(THD *thd, Ref_item_array ref_item_array,
                        ...
                        bool is_window_order, bool pq_resolved) {
  ...
  if (pq_resolved)
    return find_order_in_list_for_pq(thd, ref_item_array, order, fields);
    ...
    if (((is_group_field || is_window_order) &&
      ...
      from_field =
          find_field_in_tables(thd, (Item_ident *)order_item, tables, nullptr,
                               &view_ref, IGNORE_ERRORS, thd->is_pq_leader(),
  ...
  */
  ...
  // find_order_in_list function is also called in JOIN::pq_copy_from.
  // Therefore, fix_fields_or_refix_fields function needs to be invoked in this
  // place. In PQ process, it has called refix_fields to resolve item. In
  // other process, it has called fix_fields to resolve item.
  bool ret = (!order_item->fixed &&
              (order_item->fix_fields_or_refix_fields(thd, order->item) ||
...
bool Query_block::check_only_full_group_by(THD *thd, bool skip_error_message) {
  bool rc = false;
  ...
  if (is_grouped()) {
    ...
    */
    MEM_ROOT root(PSI_NOT_INSTRUMENTED, MEM_ROOT_BLOCK_SIZE);
    {
      ...
      rc = gc.check_query(thd, skip_error_message);
      gc.to_opt_trace(thd);
  ...
  if (!rc && is_distinct()) {
    ...
    rc = dc.check_query(thd, skip_error_message);
  }

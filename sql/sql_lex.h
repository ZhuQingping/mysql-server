...
class Query_expression {
 ...
 public:
  ...
  // shared tmp table info for PQ, if a derived table or CTE
  PQ_shared_info *m_pq_shared_info{nullptr};
  // wrap materialize access path with limit/offset
  bool wrapped_materialize_with_limit{false};

  /// Return type for Query_expression::subquery_suite_for_parallel_query().
  enum class PQSubqueryExecution {
    /// this subquery is not suitable for PQ
    kImpossible,
    /// this subquery is suitable for PQ and should be executed only once and by
    /// the leader.
    kOnceByLeader,
    /// this subquery is suitable for PQ and should be executed possibly N times
    /// and by each worker.
    kMultipleByWorkers
  };

  /// Determines whether the subquery is supported in PQ.
 ...
 public:
   ...
   */
  bool create_access_paths(THD *thd);
  ...
  */
  ...
  bool is_union_all_of_simple() const {
    if (m_query_term->term_type() != QT_UNION) return false;
    for (auto operand :
         down_cast<Query_term_set_op *>(m_query_term)->m_children) {
      if (operand->term_type() != QT_QUERY_BLOCK) return false;
    }
    return !down_cast<Query_term_set_op *>(m_query_term)->m_is_materialized;
  }
  ...
  */
  ...
  ha_rows saved_select_limit_cnt, saved_offset_limit_cnt;
  ...
  */
  ...
  void create_iterator_from_accesspath(THD *thd);
  ...
  void set_query_blocks_to_materialize(
      Mem_root_array<MaterializePathParameters::QueryBlock> &querys) {
    m_query_blocks_to_materialize = std::move(querys);
  }
...
// Allows for shorter invokation of the enum's constants
using PQSubqueryExecution = Query_expression::PQSubqueryExecution;
...
class Query_block : public Query_term {
  ...
  */
  ...
  bool wrapped_in_intersect_except();
  ...
  */
  ...
  /// When adding a table and giving it a tableno, the tableno must always be
 ...
 public:
  ...
  /*
    ...
    collaboration of leader and worker ("full-blown PQ"), while
  ...
  */
  ...
  /** check table list for PQ */
  bool pq_check_table_list();
  /** determine whether suite for PQ */
  bool suite_for_parallel_query(THD *thd);
  // find and replace aggr/ref item in base_ref_item
  bool find_base_ref_array(Item **found, Item_ref *ref = nullptr,
                           bool is_ref = false);
  ...
  */
  ...
  bool check_only_full_group_by(THD *thd, bool skip_error_message = false);
 ...
 public:
  ...
  /// A backup of m_hwm_free_tableno before query block optimization.
  /// In stored procedures where the same query block is optimized multiple
  /// times (e.g., EXPLAIN in a loop), temporary tables created during
  /// optimization consume table numbers. This backup allows
  /// restore_properties() to reset the counter, enabling table numbers to be
  /// reused across executions and preventing unbounded growth that could lead
  /// to crashes.
  uint m_saved_hwm_free_tableno{0};
 ...
 public:
  // optimizer hint list of the statement from outline.
  List<im::Lex_optimizer_hint> *outline_optimizer_list;
  // List of all query blocks for statement outline. This is all_selects_list
  // in a reverse order. Reference im::reverse_query_blocks in details.
  Query_block *outline_next_query_block{nullptr};

  /**
   pop the last collection of subqueries candidate for semijoin.
  */
  void pop_sj_candidates(Item_exists_subselect *sub_select = nullptr);

#ifndef NDEBUG
  size_t get_sj_candidates_size() { return sj_candidates->size(); }
#endif  // NDEBUG

  // the smallest tableno which is guaranteed to be free
  uint get_free_tableno() {
    // this line is "defensive programming": so that if some code increases
    // leaf_table_count, and is ignorant of m_hwm_free_tableno, that is fine.
...
struct LEX : public Query_tables_list {
 ...
 private:
  ...
  /// True means this Query_block references a user or system variable.
  bool m_has_user_or_system_var{false};
 ...
 public:
  ...
  void set_has_user_or_system_var() { m_has_user_or_system_var = true; }
  bool has_user_or_system_var() const { return m_has_user_or_system_var; }
 ...
 public:
  ...
  // Disable transform to subq in re-prepare if execute failed.
...
/// @see WalkItem (same idea)
template <class T>
inline bool WalkQueryBlock(Query_block *sl, T &&functor) {
  return sl->walk(&Item::walk_helper_thunk<T>, enum_walk::POSTFIX,
                  reinterpret_cast<uchar *>(&functor));
}

bool walk_join_condition(mem_root_deque<Table_ref *> *tables,
                         Item_processor processor, enum_walk walk, uchar *arg);

/**
   PQ resolution of clone items can be confused in
   situations like:
      SELECT t1.c from t1, (select from t1) dt; ('c' is resolved to the
   top-level t1) which after merging leads to
      SELECT t1.c from t1, t1;
   PQ resolution of the clone of 'c' may resolve to the 2nd t1, as we have
   two tables with the same name.
   pq_check_table_list() has a check that all tables have different names,
   which blocks the case above. But it can also happen with a query block and
   a subquery. Like in:
      CREATE VIEW v AS SELECT * FROM t1;
      SELECT * FROM v vo WHERE (SELECT 1 FROM v WHERE v.a=1 AND vo.a>=2);
   which gives, after merging:
      SELECT * FROM t1 WHERE (SELECT 1 FROM t1 WHERE t1.a=1 AND t1.a>=2);
   and then the PQ clone of t1.a>=2 is resolved to the inner subquery,
   wrongly.

   A unique-name check across query block boundaries (and then rejection from
   PQ) would be unacceptable, it may reject TPC-DS queries. What we do
   instead is using the origin (pre-PQ, source-of-clone) item as a hint to
   guide name resolution for the clone.
*/
inline bool pq_can_resolve_Item_field_in(const Item_field *item,
                                         const Query_block *query_block) {
  if (item->cloned_origin_item) {
    auto source = down_cast<Item_field *>(item->cloned_origin_item);
    if (source->fixed && source->table_ref->query_block->select_number !=

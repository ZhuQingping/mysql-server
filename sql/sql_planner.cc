...
static bool has_primary_key_prefix(const KEY *const covering_key,
                                   const KEY *const primary_key) {
  const uint cov_card = actual_key_parts(covering_key);
  const uint pri_card = actual_key_parts(primary_key);
  if (cov_card <= pri_card) return false;
  for (uint keypart = 0; keypart < pri_card; ++keypart) {
    if (primary_key->key_part[keypart].field !=
        covering_key->key_part[keypart].field)
      return false;
  }
  return true;
}
...
Key_use *Optimize_table_order::find_best_ref(
  ...
  */
  ...
  for (Key_use *keyuse = tab->keyuse(); keyuse->table_ref == tab->table_ref;) {
    bool promoted_covering_index = false;
    ...
    */
    ...
    if (cur_keytype != FULLTEXT) {
      ...
      */
      ...
      // Only interested in promoting covering indexes for the case where all
      // keyparts of PK have equality predicates, i.e. PK got UNIQUE set above
      seen_pk_UNIQUE |=
          (key == table->s->primary_key) && (cur_keytype == UNIQUE);

      if (seen_pk_UNIQUE && !table->file->primary_key_is_clustered() &&
          table->covering_keys.is_set(key) &&
          has_primary_key_prefix(table->key_info + key,
                                 table->key_info + table->s->primary_key) &&
          thd->variables.rds_prefer_covering_index_over_pk) {
        // This key is a covering index. It has a prefix which is the
        // primary (unclustered) key, so we prefer the covering index
        // because it will be faster for an engine with an unclustered
        // primary index
        cur_keytype = UNIQUE;
        promoted_covering_index = true;
        trace_access_idx.add("promoted_covering_index_over_PK", true);
      }
      ...
      if (all_key_parts_covered && !ref_or_null_part) /* use eq key */
      {
        ...
        if ((keyinfo->flags & HA_NOSAME || promoted_covering_index) &&
    ...
    /*
      ...
      so far if:
       ...
       2b) It is a promoted covering index (Dstore)
    ...
    */
    ...
    if (best_found_keytype >= NOT_UNIQUE && cur_keytype >= NOT_UNIQUE)
      ...
      if (promoted_covering_index && (cur_ref_cost == best_ref_cost))
        new_candidate = true;
    } else if (best_found_keytype > cur_keytype)
    ...
    if (new_candidate) {
      ...
      *used_key_parts = cur_used_keyparts;
      best_ref = start_key;
      best_ref->m_promoted_covering_index = promoted_covering_index;

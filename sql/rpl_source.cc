...
#include "sql/rpl_wal_sender.h"  // Wal_sender
...
bool com_wal_dump(THD *thd, char *packet,
                  size_t packet_length [[maybe_unused]]) {
  DBUG_TRACE;
  uint64_t start_lsn;
  ...
  Rpl_request_dump_msg *dump_msg = pointer_cast<Rpl_request_dump_msg *>(packet);
  bool read_msg_res = Rpl_wal_msg_utils::read_request_dump_msg(
      dump_msg, wal_stream_id, start_lsn, thd->server_id, flags);
  if (read_msg_res) {
    // read request wal dump message failed.
    return true;
  }
...
void mysql_wal_send(THD *thd, uint64_t start_lsn, uint32_t flags,
                    uint64_t wal_stream_id) {
  Wal_sender wal_sender(thd, start_lsn, flags, wal_stream_id);
  sql_print_information(
      "Start walsender, slave server id is %u, slave wal stream id is %lu, "
      "start_lsn from slave is %lu.",
      thd->server_id, wal_stream_id, start_lsn);
...
class Find_zombie_dump_thread : public Find_THD_Impl {
 public:
  ...
  bool operator()(THD *thd) override {
    ...
    bool match_dump_type = thd->get_command() == COM_BINLOG_DUMP ||
                           thd->get_command() == COM_BINLOG_DUMP_GTID ||
                           thd->get_command() == COM_WAL_DUMP;
    if (thd != cur_thd && match_dump_type) {
      String tmp_uuid;
...
bool reset_master(THD *thd, bool unlock_global_read_lock) {
  ...
  if (mysql_bin_log.is_open()) {
    ...
    */
    ...
    if (rds_dstore_support_binlog_check &&
      ...
      persist_gtids_in_wal_to_file(gtid_state->get_executed_gtids());
  ...
  */
  if (!ret) {
    ...
    (void)RUN_HOOK(wallog_transmit, wallog_after_reset_master,
                   (thd, 0 /* flags */));
  }

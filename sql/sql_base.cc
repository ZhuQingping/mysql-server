...
static bool read_histograms(THD *thd, TABLE_SHARE *share,
  ...
  /*
    We have a background thread to execute the ANALYZE TABLE xxx UPDATE
    HISTOGRAM, If the ANALYZE TABLE together with DDL alter table xxx, deadlock
    may happens when we acquired.
    MDL in the following sequence.
    ALTER TABLE xxx and ANALYZE TABLE xxx UPDATE HISTOGRAM ON col1, col2

    t1:
    alter table xxx: Each column acquire MDL_SHARE_READ column_statistic
    lock.
    ANALYZE TABLE XXX UPDATE HISTOGRAM: Acquire table xxx MDL_SHARE_READ
    lock.

    t2:
    alter table xxx: Upgrade the MDL_SHARE_READ lock of table xxx to
    MDL_EXCLUSIVE lock. (wait for table xxx MDL_SHARE_READ).
    ANALYZE TABLE XXX:
    UPDATE HISTOGRAM: Acquire col1, col2 MDL_EXCLUSIVE column_statistic lock to
    store the histogram information. (Wait for MDL_SHARE_READ column_statistic
    lock).

    t3: Deadlock happends, rollback Alter table xxx.

    Why does "ALTER TABLE t ..." take MDL_SHARE_READ on column_statistics?
    This happens if the table is not in the table definition cache. This will
    cause the server to fetch the table definition from the dictionary, which in
    turn will call "read_histograms". "read_histograms" will take MDL_SHARE_READ
    on column_statistics with MDL_STATEMENT duration. MDL_STATEMENT means that
    the lock is released automatically at the end of the statement.

    So we use MDL_EXPLICIT instead of MDL_STATEMENT. because MDL_STATEMENT Locks
    with statement duration are automatically released at the end of statement
    or transactionand. use MDL_EXPLICIT we can release the locks at the end of
    function.
  */
  auto cleanup_guard = create_scope_guard([&mdl_requests, thd] {
    const size_t request_count = mdl_requests.elements();
    MDL_request_list::Iterator iter_req(mdl_requests);
    for (size_t index = 0; index < request_count; ++index) {
      MDL_request *req = iter_req++;
      thd->mdl_context.release_lock(req->ticket);
    }
  });

  DEBUG_SYNC(thd, "read_histogram_after_acquire_lock");
  ...
  for (const auto column : table_def->columns()) {
    ...
    if (histogram != nullptr) {
      /*
        We clone the histogram on the MEM_ROOT inside HistogramAndMemRoot, which
        makes the lifetime of the histogram bound to the lifetime of
        HistogramAndMemRoot.
      */
      HistogramAndMemRoot histogram_and_mem_root;
      histogram_and_mem_root.histogram =
          histogram->clone(&histogram_and_mem_root.mem_root);
      share->m_histograms->emplace(column->ordinal_position() - 1,
                                   std::move(histogram_and_mem_root));
    }
  }
...
Table_ref *find_table_in_local_list(Table_ref *tbl_list, const char *db_name,
                                    const char *table_name,
                                    const char *alias_name) {
  auto name_length = [](const char *name) {
    return (name == nullptr ? 0 : strlen(name));
  };

  for (; tbl_list; tbl_list = tbl_list->next_leaf) {
    // next_leaf refers to the resolved primary tables
    assert(tbl_list->table);
    bool db_equal =
        (!tbl_list->db_length && !name_length(db_name)) ||
        (name_length(db_name) &&
         !my_strcasecmp(system_charset_info, tbl_list->db, db_name));
    bool table_name_equal =
        (!tbl_list->table_name_length && !name_length(table_name)) ||
        (name_length(table_name) &&
         !my_strcasecmp(system_charset_info, tbl_list->table_name, table_name));
    if (db_equal && table_name_equal &&
        (!alias_name ||
         (alias_name && tbl_list->alias &&
          // SELECT t.* FROM t RIGHT JOIN t AS t1 ON t.a IS NOT NULL GROUP BY
          // t.a,t.b; find error table because cmp alias_name error t == t1, but
          // t != t1
          !my_strcasecmp(system_charset_info, tbl_list->alias, alias_name))))
      break;
  }
  return tbl_list;
}
...
static bool tdc_wait_for_old_version(THD *thd, const char *db,
  ...
  mysql_mutex_lock(&LOCK_open);
  if (thd->is_pq_leader() && (share = get_cached_table_share(db, table_name)) &&
...
share_found:
  if (thd->is_pq_leader() && !(flags & MYSQL_OPEN_IGNORE_FLUSH)) {
    if (share->has_old_version()) {
      /*
...
static bool open_and_process_table(THD *thd, LEX *lex, Table_ref *const tables,
  ...
  */
  if (tables->is_derived() || tables->is_table_function() ||
      tables->is_recursive_reference() || tables->is_transformed_in_predicate())
    goto end;
  ...
  */
  for (tables = *start; tables; tables = tables->next_global) {
    ...
    */
    ...
    // shared_mode/NOWAIT/skip_locked in DSTORE is not supported
    if (tbl && tbl->file && tbl->file->ht->db_type == DB_TYPE_DSTORE) {
      if (!rds_allow_skip_locked_now_wait &&
          ((thd->lex->is_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_NOWAIT)) ||
           (thd->lex->is_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SKIP_LOCKED)))) {
        my_error(ER_CHECK_NOT_IMPLEMENTED, MYF(0),
                 "NOWAIT, and SKIP LOCKED record-locking clauses");
        error = true;
        goto err;
      }
    }
...
Field *find_field_in_tables(THD *thd, Item_ident *item, Table_ref *first_table,
  ...
  for (cur_table = first_table; cur_table != last_table;
    ...
    if (item->type() == Item::FIELD_ITEM &&
        !pq_can_resolve_Item_field_in(down_cast<Item_field *>(item),
                                      cur_table->query_block))
      continue;
...
/**
  ...
  @param[in] pq_resolved       if true, item in fields has resolved to
  base_ref_items using "pq_replace_base_item". Then, item in fields can be
  hidden and ref_item_array must not be empty. Besides, the privilege muse have
  been checked in leader.
...
bool setup_fields(THD *thd, Access_bitmask want_privilege, bool allow_sum_func,
  ...
  */
  if (!ref_item_array.is_null() && !pq_resolved) {
  ...
  for (auto it = fields->begin(); it != fields->end(); ++it) {
    ...
    Item *item = *it;
    assert(!item->hidden || pq_resolved);
    Item **item_pos = &*it;
    // setup_fields function is also called in pq_select_prepare function.
    // Therefore, fix_fields_or_refix_fields function needs to be invoked in
    // this place. In PQ process, it has called refix_fields to resolve item. In
    // other process, it has called fix_fields to resolve item.
    if ((!item->fixed && item->fix_fields_or_refix_fields(thd, item_pos)) ||
    ...
    if (typed_items != nullptr && typed_it != typed_items->end()) {
      typed_item = *typed_it++;
      assert(!typed_item->hidden || pq_resolved);
    }
    ...
    // only visible items are allowed to split. These hidden items that
    // contain aggregate function will be split in setup group (or order).
    // E.g., "select companynr from t3 group by companynr order by sum(price) +
    // 1", order->item will be split in having_cond()->split_sum_func2"
    if (split_sum_funcs && !item->hidden) {
      /*
...
/**
   ...
   @param  only_rmv_second_engine If true, do not remove table definition cache
                                  And only second engine table. If false, remove
...
void tdc_remove_table(THD *thd, enum_tdc_remove_table_type remove_type,
  ...
  if (!only_rmv_second_engine) remove_table(it);
...
bool open_trans_system_tables_for_read(THD *thd, Table_ref *table_list) {
  return open_trans_system_tables_for_rw(thd, table_list, true);
}

/**
  Open and lock transactional system tables for write.
  ...
  @return Error status.
*/
bool open_trans_system_tables_for_write(THD *thd, Table_ref *table_list) {
  return open_trans_system_tables_for_rw(thd, table_list, false);
}

/**
  Open and lock transactional system tables for read or write.
  ...
  @param thd        Thread context.
  @param table_list List of tables to open.
  @param is_for_read true: Open and lock transactional system tables for read;
                     false: Open and lock transactional system tables for write.

  @return Error status.
*/
bool open_trans_system_tables_for_rw(THD *thd, Table_ref *table_list,
                                     bool is_for_read) {
  uint counter;

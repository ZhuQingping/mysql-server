...
class Aggregator {
  ...
  */
  ...
  virtual bool has_blob_in_aggr() = 0;
};
...
class Item_sum : public Item_func {
 ...
 public:
  ...
  bool has_blob_in_aggr() {
    return aggr == nullptr ? false : aggr->has_blob_in_aggr();
  }
  ...
  */
  ...
  /// rebuilt sum function for parallel query.
  /// @TODO: only rebuilt sum->arguments
...
class Aggregator_distinct : public Aggregator {
 ...
 public:
  ...
  bool add() override;
  void endup() override;
  bool save_distinct_to_mq(Batch_buffer *);
  ...
  // table is set in setup function. And if const_distinct is set to CONST_NULL
  // or CONST_NOT_NULL, table is nullptr.
  bool has_blob_in_aggr() override {
    return table && table->s && table->s->blob_fields != 0;
  }
  bool has_using_tree_to_unqiue() {
    if (const_distinct != NOT_CONST) return true;
    return tree;
  }
  Const_distinct get_const_distinct() { return const_distinct; }
  bool merge_count_distinct_tree(Item_sum *item_sum);
};
...
class Aggregator_simple : public Aggregator {
 public:
  ...
  bool arg_is_null(bool use_null_value) override;
  longlong arg_val_int() override;
  bool has_blob_in_aggr() override { return false; }
};
...
class Item_sum_count : public Item_sum_int {
 ...
 public:
  ...
  Item_sum_count(Item_int *number, bool fake = false)
      : Item_sum_int(number), count(0), is_fake(fake) {}
  /**
  ...
  */
  ...
  Item_sum_count(const POS &pos, PT_item_list *list, PT_window *w,
                 bool fake = false)
      : Item_sum_int(pos, list, w), count(0), is_fake(fake) {
    set_distinct(true);
  }
   ...
   * records from table, the faked-item_sum_sum have the same result with the
   * original Item_sum_count, then is_fake = false;
   */
  bool is_fake{false};
  Item *pq_clone(THD *thd, Query_block *select) override;
  Item_sum *pq_rebuild_sum_func(THD *thd, Query_block *select,
...
enum PqAvgType { PQ_LEADER, PQ_WORKER, PQ_REBUILD, PQ_INVALID };
...
class Item_sum_avg final : public Item_sum_sum {
 public:
  ...
  // rebuilt Item_sum_avg function in leader.
  ...
  void init_store_info() {
    m_info.used = true;
    m_info.s_decimals = decimals;
    m_info.s_max_length = max_length;
    m_info.s_null_value = null_value;
  }

  void restore_orig_info() {
    if (m_info.used) {
      decimals = m_info.s_decimals;
      max_length = m_info.s_max_length;
      null_value = m_info.s_null_value;
      pq_avg_type = PqAvgType::PQ_INVALID;
    }
  }

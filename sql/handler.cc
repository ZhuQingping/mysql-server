...
#include "sql/ddl_info.h"                    // need_ddl_info()
#include "sql/ddl_info_file.h"               // g_ddl_flush_lsn_mgr
...
static uint64_t pre_plsn = UINT64_MAX;
static uint64_t cur_plsn = UINT64_MAX;
...
bool match_disabled_storage_engines(std::vector<std::string> check_se_names) {
  for (const auto &name : check_se_names) {
    if (!is_storage_engine_name_externally_disabled(name.c_str())) {
      LogErr(ERROR_LEVEL, ER_STORAGE_ENGINE_MODE_MATCH_ERROR, name.c_str());
      return true;
    }
  }
  return false;
...
bool is_storage_engine_mode_match(std::vector<std::string> check_se_names,
                                  const char *default_storage_engine,
                                  const char *default_engine_name) {
  // default_storage_engine matches default_engine_name returns 0
  if (my_strcasecmp(system_charset_info, default_storage_engine,
                    default_engine_name) != 0) {
    LogErr(ERROR_LEVEL, ER_STORAGE_ENGINE_MODE_MATCH_DEFAULT_ENGINE_ERROR,
           default_storage_engine,
           storage_engine_mode_names[storage_engine_mode]);
    return true;
  }
  return match_disabled_storage_engines(check_se_names);
}

/**
  Returns true if check failed.
  For ONLY_DSTORE, disabled storage engines are InnoDB, CSV, Memory, MyISAM,
  MRG_MyISAM. default_storage_engine should be Dstore. For ONLY_INNODB, they are
  Dstore, CSV, Memory, MyISAM, MRG_MyISAM. default_storage_engine should be
  InnoDB. For BOTH_STORAGE_ENGINE, we don't check.
 */
bool ha_check_storage_engine_mode(ulong storage_engine_mode,
                                  const char *default_storage_engine) {
  if (storage_engine_mode == static_cast<ulong>(BOTH_STORAGE_ENGINE)) {
    return false;
  }

  // push disabled storage engines names to check
  std::vector<std::string> check_se_names;
  check_se_names.push_back(CSV_ENGINE_NAME);
  check_se_names.push_back(MEMORY_ENGINE_NAME);
  check_se_names.push_back(MYISAM_ENGINE_NAME);
  check_se_names.push_back(MRG_MYISAM_ENGINE_NAME);
  if (storage_engine_mode == static_cast<ulong>(ONLY_DSTORE)) {
    check_se_names.push_back(INNODB_ENGINE_NAME);
    return is_storage_engine_mode_match(check_se_names, default_storage_engine,
                                        DSTORE_ENGINE_NAME);
  } else {  // ONLY_INNODB
    check_se_names.push_back(DSTORE_ENGINE_NAME);
    return is_storage_engine_mode_match(check_se_names, default_storage_engine,
                                        INNODB_ENGINE_NAME);
  }
}
  ...
  switch (hton->state) {
    ...
    case SHOW_OPTION_YES: {
      ...
      */
      ...
      if (hton->prepare) {
        // Increment total_ha_2pc for InnoDB when the plugin is not dstore,
        // since dstore's prepare interface is not yet implemented.
        if (((storage_engine_mode == static_cast<ulong>(ONLY_DSTORE)) &&
             !my_strcasecmp(system_charset_info, default_storage_engine,
                            "Dstore")) ||
            (strcmp(plugin->name.str, "Dstore") != 0)) {
          total_ha_2pc++;
        }
      }
      break;
...
void trans_register_ha(THD *thd, bool all, handlerton *ht_arg,
  ...
  /*
    Register transaction start time for statement transaction(In autocommit
    mode, the normal transaction is equivalent to the statement transaction)
    / non-auto-commit transaction/ internal transaction triggered by an
    query.e.g DDL/DCL
  */
  if (!thd->trx_start_time_in_secs) thd->trx_start_time_in_secs = time(nullptr);
/*
...
int ha_commit_low(THD *thd, bool all, bool run_after_commit) {
  ...
  */
  ...
  if (ha_list) {
    ...
    if (unlikely(get_ddl_command_type(thd) != DDLCOM_NONE && is_real_trans &&
                 trn_ctx->rw_ha_count(trx_scope) > 1 &&
                 thd->is_ddl_info_written)) {
      ha_list.move_dstore_to_front();
    }
    handlerton *dstore_ht = nullptr;
    for (auto &ha_info : ha_list) {
      int err;
      auto ht = ha_info.ht();
      if (ht->db_type == DB_TYPE_DSTORE) {
        dstore_ht = ht;
      }
#ifndef NDEBUG
      if (get_ddl_command_type(thd) != DDLCOM_NONE && is_real_trans &&
          dstore_ht != nullptr && ht->db_type == DB_TYPE_INNODB &&
          trn_ctx->rw_ha_count(trx_scope) > 1) {
        DEBUG_SYNC(thd, "ddl_crash_safe_before_innodb_commit");
      }
#endif  // !NDBUG
      if (unlikely(rds_dstore_enable_atomic_ddl &&
                   get_ddl_command_type(thd) != DDLCOM_NONE && is_real_trans &&
                   dstore_ht != nullptr &&
                   trn_ctx->rw_ha_count(trx_scope) > 1 &&
                   !thd->is_system_thread() && thd->is_ddl_info_written &&
                   !thd->m_skip_dstore_extral_wal_check)) {
        if ((err = ht->commit(ht, thd, all, true))) {
          char errbuf[MYSQL_ERRMSG_SIZE];
          my_error(ER_ERROR_DURING_COMMIT, MYF(0), err,
                   my_strerror(errbuf, MYSQL_ERRMSG_SIZE, err));
          error = 1;
        }
        DBUG_EXECUTE_IF("crash_afer_dstore_commit_done", { DBUG_SUICIDE(); });
      } else {
        if ((err = DBUG_EVALUATE_IF("test_error_during_commit", HA_ERR_GENERIC,
                                    ht->commit(ht, thd, all, false)))) {
          char errbuf[MYSQL_ERRMSG_SIZE];
          my_error(ER_ERROR_DURING_COMMIT, MYF(0), err,
                   my_strerror(errbuf, MYSQL_ERRMSG_SIZE, err));
          error = 1;
      ...
      /* Mark a DDL transaction as finished after the transaction commited to
      InnoDB, which advances the recovery point. */
      if (unlikely(rds_dstore_enable_atomic_ddl &&
                   get_ddl_command_type(thd) != DDLCOM_NONE && is_real_trans &&
                   dstore_ht != nullptr && ht->db_type == DB_TYPE_INNODB &&
                   trn_ctx->rw_ha_count(trx_scope) > 1)) {
        g_ddl_flush_lsn_mgr.markDdlCommitFinish(
            dstore_ht, trn_ctx->xid_state()->get_xid()->get_my_xid());
      }
      assert(!thd->status_var_aggregated);
      DBUG_EXECUTE_IF("ddl_crash_safe_crash_after_commit_in_dstore", {
        if (ht->db_type == DB_TYPE_DSTORE && is_real_trans) {
          DBUG_SUICIDE();
        };
      });
...
err:
  ...
  if (all) {
    trn_ctx->cleanup();
    /*
      By 2 phase xa commit, ha_commit_low will be directly called through
      trans_xa_commit->process_internal_xa_commit->(MYSQL_BIN_LOG::commit) and
      ha_commit_trans is not called; timestamp should be reset here.
    */
    thd->trx_start_time_in_secs = 0;
  }
  /*
  ...
  */
  ...
  if (ha_list) {
    for (auto &ha_info : ha_list) {
      ...
      auto ht = ha_info.ht();
      if (ht->commit(ht, thd, false, false)) {
        /*
...
int ha_prepare_low(THD *thd, bool all) {
  ...
  /* Explicitly set HA_REGULAR_DURABILITY to ensure DDL's prepare redo log
  is flushed to disk(in InnoDB). */
  if (unlikely(get_ddl_command_type(thd) != DDLCOM_NONE &&
  ...
  if (ha_list) {
    for (auto const &ha_info : ha_list) {
      ...
      /* If this is a DDL transaction on DStore and doing final commit, then we
      will write extra DDL info and save it's flush lsn before commit. */
      if (unlikely(rds_dstore_enable_atomic_ddl &&
                   get_ddl_command_type(thd) != DDLCOM_NONE && is_real_trans &&
                   ht->db_type == DB_TYPE_DSTORE)) {
        std::string query_str;
        if (get_ddl_command_type(thd) == DDLCOM_PRIVI &&
            thd->rewritten_query().length()) {
          query_str.assign(thd->rewritten_query().ptr(),
                           thd->rewritten_query().length());
        } else {
          query_str.assign(thd->query().str, thd->query().length);
        }
        sql_print_information(
            "DDL crash safe: Transaction with XID %llu start commit, thread id "
            "is %llu, query text is %s",
            thd->get_transaction()->xid_state()->get_xid()->get_my_xid(),
            thd->thread_id(), query_str.c_str());
        g_ddl_flush_lsn_mgr.markDdlCommitStart(
            ht, thd->get_transaction()->xid_state()->get_xid()->get_my_xid());
        /* We will write DDL info when writing
        binlog(MYSQL_BIN_LOG::write_transaction) if rds_write_binlog_into_redo
        is ON, so we must skip writing here. */
        if ((!opt_bin_log || thd->is_binlog_cache_empty(true)) &&
            need_ddl_info(thd) && !rds_write_binlog_into_redo) {
...
int handler::ha_index_init(uint idx, bool sorted) {
  ...
  // Temptable can be divided into blocks in parallel query, then inited =
  // handler::PQ.
  assert(inited == NONE || inited == PQ);
...
int handler::ha_rnd_init(bool scan) {
  ...
  DBUG_TRACE;
  assert(table_share->tmp_table != NO_TMP_TABLE || m_lock_type != F_UNLCK);
  assert(inited == NONE || (inited == RND && scan) || inited == PQ);
...
int handler::records_from_index(ha_rows *num_rows, uint index) {
  ...
  /*
    Mark access index only for covering index read scenario,
    storage engine(like Dstore) can avoid go back to heap table
    read records.
  */
  if (index < MAX_KEY && table->covering_keys.is_set(index)) {
...
/**
 * @brief Write binlog into Write-Ahead Logs (WALs)
 *
 * This function resolves the storage engine plugin by name and calls the
 * `set_extra_commit` method of the plugin to commit the write-ahead logs.
 *
 * @param buffer Pointer to the write-ahead log data.
 * @param length Length of the write-ahead log data.
 * @param write_extra_wal write wal type is binlog extra
 */
void ha_commit_wals(THD *thd, const unsigned char *buffer, unsigned long length,
                    bool write_extra_wal) {
  plugin_ref plugin;
  LEX_CSTRING name = {STRING_WITH_LEN(DSTORE_ENGINE_NAME)};
  if ((plugin = ha_resolve_by_name(thd, &name, false))) {
    handlerton *hton = plugin_data<handlerton *>(plugin);
    if (hton && hton->set_extra_commit) {
      hton->set_extra_commit(thd, buffer, length, write_extra_wal);
    }
    plugin_unlock(thd, plugin);
  }
}
...
/**
 * Sets the maximum start recovery point for the DStore engine.
 *
 * This function retrieves the DStore engine plugin, checks if it supports the
 * `set_recovery_plsn_for_taurus` method, and invokes it if available. The
 * purpose of this method is to update the engine's internal state to reflect
 * the latest point up to which recovery should be performed.
 *
 */
void ha_set_max_start_recovery_point(THD *thd) {
  ...
  if ((plugin = ha_resolve_by_name(thd, &name, false))) {
    handlerton *hton = plugin_data<handlerton *>(plugin);
    if (hton && hton->get_current_flushed_plsn &&
        hton->set_recovery_plsn_for_taurus) {
      hton->set_recovery_plsn_for_taurus(pre_plsn);
      pre_plsn = cur_plsn;
      cur_plsn = hton->get_current_flushed_plsn();
    }
    plugin_unlock(thd, plugin);
  }
}
...
@param thd	  Thread context.
@param plugin	Storage engine plugin, only dstore engine support.
@retval false (always) */
static bool ha_register_dummy_trx_handlerton(THD *thd, plugin_ref plugin,
                                             void *) {
  handlerton *hton = plugin_data<handlerton *>(plugin);
  if (hton->state == SHOW_OPTION_YES && hton->register_dummy_trx) {
    hton->register_dummy_trx(thd);
  }
  return false;
}

void ha_register_dummy_trx(THD *thd) {
  plugin_foreach(thd, ha_register_dummy_trx_handlerton,
                 MYSQL_STORAGE_ENGINE_PLUGIN, nullptr);
}
...
int handler::ha_reset() {
  ...
  /* Reset information about pushed aggregate */
  cancel_pushed_aggregate();
...
bool handler::filter_dup_records() {
  assert((inited == INDEX || inited == PQ) && m_unique);
...
int handler::ha_pq_init(uint dop, uint keyno) {
  ...
  assert(table_share->tmp_table != NO_TMP_TABLE || m_lock_type != F_UNLCK);
  assert(inited == NONE || inited == INDEX || inited == PQ || inited == RND);
  inited = (result = pq_leader_scan_init(
                keyno, /* reference to pointer */ pq_ctx, dop))
               ? NONE
               : PQ;
  end_range = NULL;
...
bool handler::ha_reverse_scan() {
  // Only leader read this variable, worker use config.m_pq_reverse_scan
  // which copied from leader.
  assert(ha_thd()->is_pq_leader());
  return pq_reverse_scan;
}

void handler::ha_set_reverse_scan(bool reverse) {
  // Only leader set this variable.
  assert(ha_thd()->is_pq_leader());
  pq_reverse_scan = reverse;
}
...
int handler::ha_pq_end() {
  ...
  if (pq_ctx) {
    assert(ha_thd()->is_pq_leader());
    DBUG_RETURN(pq_leader_scan_end(pq_ctx));
  } else {
    assert(ha_thd()->is_pq_worker());
    DBUG_RETURN(pq_worker_scan_end());
  }
}

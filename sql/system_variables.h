...
// Values for rds_binlog_purge_thread_status sysvar
enum enum_rds_binlog_purge_thread_status {
  BINLOG_PURGE_THEAD_STATUS_NOT_CREATED = 0,
  BINLOG_PURGE_THEAD_STATUS_EXIT = 1,
  BINLOG_PURGE_THEAD_STATUS_GONE = 2,
  BINLOG_PURGE_THEAD_STATUS_RESET = 3,
  BINLOG_PURGE_THEAD_STATUS_INITIALIZING = 4,
  BINLOG_PURGE_THEAD_STATUS_INITIALIZED = 5,
  BINLOG_PURGE_THEAD_STATUS_SLEEPING = 6,
  BINLOG_PURGE_THEAD_STATUS_AWAKENED = 7,
  BINLOG_PURGE_THEAD_STATUS_REQUESTING_LOCK_INDEX = 8,
  BINLOG_PURGE_THEAD_STATUS_PURGING_BINARY_LOGS = 9,
  BINLOG_PURGE_THEAD_STATUS_RELEASED_LOCK_INDEX = 10
};
...
struct System_variables {
  ...
  */
  ...
  bool big_tables;
  /// When true, a redundant condition (meaning that it is enforced by the
  /// range access method anyway) in the WHERE clause is removed.
  bool rds_empty_redundant_check_in_range_scan;

  /// Enables PQ fallback to serial execute
  bool parallel_graceful_fallback;

  // Whether support COM_STMT_BULK_EXECUTE protocol.
  bool rds_support_bulk_execution;
  ...
  */
  ...
  /* If the number of rows in the table changed by more than this fraction,
   cached plan cannot be used. */
  double rds_plan_cache_allow_change_ratio;

  /** The DDL lock wait timeout */
  ulong rds_ddl_lock_wait_timeout;

  /**
    @sa Sys_rds_show_hidden_table
  */
  ...
  */
  ...
  void pq_copy_from(System_variables leader);

  /// 'True' <=> the user wants us to cache a plan, and to use a cached plan.
  bool rds_plan_cache;

  /* A bitmap for switching querys on/off for PQ */
  ulonglong pq_support_features_switch;

  bool rds_recycle_bin_mode;

  /* For engines like Dstore which have non-clustered primary keys */
  bool rds_prefer_covering_index_over_pk;
};
...
struct System_status_var {
  ...
  */
  ...
  bool pq_merge_status(const System_status_var &worker);
};

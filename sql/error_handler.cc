...
bool Ignore_index_hints_syntax_handler::handle_condition(
    THD *m_thd, uint sql_errno, const char *,
    Sql_condition::enum_severity_level *level, const char *) {
  switch (sql_errno) {
    case ER_WRONG_USAGE:
    case ER_KEY_DOES_NOT_EXITS:
      (*level) = Sql_condition::SL_WARNING;
      push_warning_printf(
          m_thd, *level, ER_OUTLINE_APPLY_FAILED,
          ER_THD(m_thd, ER_OUTLINE_APPLY_FAILED),
          "This is unresolved field for INDEX hint. Skip statement outline.");
      break;
  }
  return false;
}
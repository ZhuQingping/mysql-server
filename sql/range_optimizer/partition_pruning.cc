...
struct PART_PRUNE_PARAM {
  ...
  */
  ...
  PARTITION_ITERATOR
  subpart_iter; /* Same as above, but for used subpartitions */
  ...
  // Used to store the start offset of min/max_key position. For partition,
  // both them are all 0. But for subpartitions, min/max_key offset should
  // skip all of min_max/key for partition.
  uint min_offset{0};
  uint max_offset{0};
  uint cur_min_flag, cur_max_flag;

  // Used to mark whether key_parts are from start of first column of
  // partition/subpartition columns or expression. If it's true, an iterator
  // can be used to check values between min and max key. Otherwise, the
  // iterator won't be created.
  bool key_from_start{false};
  uint32 part_id;  // main partition id
};
...
bool prune_partitions(THD *thd, TABLE *table, Query_block *query_block,
  ...
  */
  ...
  if (tree->merges.is_empty()) {
    ...
    init_all_partitions_iterator(part_info, &prune_param.subpart_iter);
    if (!tree->keys[0] ||
...
static void mark_full_partition_used_with_parts(partition_info *part_info,
                                                uint32 part_id) {
  uint32 start = part_id * part_info->get_num_subparts();
  uint32 end = start + part_info->get_num_subparts();
  DBUG_TRACE;
...
static int find_used_partitions_imerge(THD *thd, PART_PRUNE_PARAM *ppar,
  ...
  for (SEL_TREE *ptree : imerge->trees) {
    ...
    init_all_partitions_iterator(ppar->part_info, &ppar->subpart_iter);
...
/*
  Context is used to store and restore PART_PRUNE_PARAM status for
  find_used_partitions of subpartitions.
*/
class PPar_subpart_context {
 public:
  SEL_ARG *proc_key_tree;
  uchar *proc_min_key;
  uchar *proc_max_key;
  uint cur_part_fields;
  uint cur_subpart_fields;
 ...
 public:
  void init(SEL_ARG *key_tree, PART_PRUNE_PARAM *ppar) {
    proc_key_tree = key_tree;
    proc_min_key = ppar->cur_min_key;
    proc_max_key = ppar->cur_max_key;
    cur_part_fields = ppar->cur_part_fields;
    cur_subpart_fields = ppar->cur_subpart_fields;
    arg_stack_end = ppar->arg_stack_end;
    min_offset = ppar->cur_min_key - ppar->min_key;
    max_offset = ppar->cur_max_key - ppar->max_key;
    ignore_part_fields = ppar->ignore_part_fields;
    save_min_flag = ppar->cur_min_flag;
    save_max_flag = ppar->cur_max_flag;
    key_from_start = ppar->key_from_start;
    // The following information must be rollback for subpartition because
    // find_used_partitions always increase them. In order to make
    // subpartition use find_used_partitions to prune, they must be
    // rollbacked.
    ppar->arg_stack_end--;
  ...
  void reset(SEL_ARG *&key_tree, PART_PRUNE_PARAM *ppar) {
    ...
    ppar->cur_min_key = proc_min_key;
    ppar->cur_max_key = proc_max_key;
    ppar->min_offset = min_offset;
    ppar->max_offset = min_offset;
    ppar->cur_min_flag = 0;
    ppar->cur_max_flag = 0;
    ppar->key_from_start = false;
  }
  void restore(PART_PRUNE_PARAM *ppar) {
    ppar->ignore_part_fields = ignore_part_fields;
    ppar->cur_part_fields = cur_part_fields;
    ppar->cur_subpart_fields = cur_subpart_fields;
...
static int find_used_partitions(THD *thd, PART_PRUNE_PARAM *ppar,
  ...
  PARTITION_ITERATOR *part_iter;
  int last_part_partno;
  uint32 part_fields = ppar->part_info->num_part_fields;
  uint32 num_keys;
  if (ppar->part_info->is_subpart_info()) {
    part_iter = &ppar->subpart_iter;
    last_part_partno = ppar->last_subpart_partno;
    num_keys = ppar->subpart_fields;
  } else {
    part_iter = &ppar->part_iter;
    last_part_partno = ppar->last_part_partno;
    num_keys = ppar->part_fields;
  }
  if (!ppar->key_from_start)
    ppar->key_from_start = (ppar->key[key_tree_part].field ==
                            ppar->part_info->part_field_array[0]);
  ...
  *(ppar->arg_stack_end++) = key_tree;
  ...
  if (ignore_part_fields && key_tree->part <= last_part_partno) {
    /*
  ...
  */
  if (key_tree_type == SEL_ROOT::Type::KEY_RANGE) {
    if (ppar->part_info->get_part_iter_for_interval &&
        key_tree->part <= last_part_partno) {
      ...
      if (key_tree->next_key_part &&
          ...
          key_tree->next_key_part->root->part <= last_part_partno &&
        ...
        */
        ...
        if (!tmp_min_flag)
          key_tree->next_key_part->store_min_key(
              ppar->key, &tmp_min_key, &tmp_min_flag, last_part_partno, true);
        if (!tmp_max_flag)
          key_tree->next_key_part->store_max_key(
              ppar->key, &tmp_max_key, &tmp_max_flag, last_part_partno, false);
        flag = tmp_min_flag | tmp_max_flag;
      } else
        flag = key_tree->min_flag | key_tree->max_flag;

      if (tmp_min_key != ppar->min_key + ppar->min_offset)
        flag &= ~NO_MIN_RANGE;
      else
        flag |= NO_MIN_RANGE;
      if (tmp_max_key != ppar->max_key + ppar->max_offset)
        flag &= ~NO_MAX_RANGE;
      else
        flag |= NO_MAX_RANGE;

      min_len = tmp_min_key - ppar->min_key - ppar->min_offset;
      max_len = tmp_max_key - ppar->max_key - ppar->max_offset;
      /*
      ...
      */
      if (ppar->key_from_start) {
        ...
        // All part keys are stored in ppar->key array which primary part key is
        // stored first, then subpartition part keys are stored.
        // It will copy all part keys's length of primary partition or
        // subpartition.
        // For primary partition, range is 0 to part_info.last_part_partno.
        // For subpartition, range is
        // subpartition.last_part_partno - subpartition.num_keys + 1 to
        // subpartition.last_part_partno.
        for (i = last_part_partno - num_keys + 1; i <= last_part_partno;
             i++, j++)
          store_length_array[j] = ppar->key[i].store_length;
      ...
      if (res == -1) {
        ...
        init_all_partitions_iterator(ppar->part_info, part_iter);
      }
      /*
        Save our intent to mark full partition as used if we will not be able
        to obtain further limits on subpartitions
      */
      if (key_tree_part < last_part_partno) {
        /*
    ...
    if (!extended_partitions_enabled ||
        !ppar->part_info->is_sub_partitioned() ||
        ppar->part_info->sub_part_info->part_type == partition_type::HASH) {
      if (key_tree_part == ppar->last_subpart_partno &&
        ...
        uint32 part_id;
        while ((part_id = part_iter->get_next(part_iter)) !=
               NOT_A_PARTITION_ID) {
          for (uint i = 0; i < ppar->part_info->get_num_subparts(); i++)
            if (bitmap_is_set(&ppar->subparts_bitmap, i))
              bitmap_set_bit(&ppar->part_info->read_partitions,
                             part_id * ppar->part_info->get_num_subparts() + i);
        }
        goto pop_and_go_right;
      }
    } else if (key_tree_part > ppar->last_part_partno) {
      if (key_tree_part <= ppar->last_subpart_partno &&
          (nullptr !=
           ppar->part_info->sub_part_info->get_part_iter_for_interval)) {
        /* Mark each partition as used in each subpartition.  */
        uint32 part_id;
        uint subparts = ppar->part_info->get_num_subparts();
    ...
    if (key_tree->is_singlepoint()) {
      if (key_tree_part == last_part_partno &&
          ppar->cur_part_fields == part_fields &&
        ...
        */
        store_selargs_to_rec(ppar, ppar->arg_stack, part_fields);
      ...
      if (key_tree_part == ppar->last_subpart_partno &&
        ...
        */
        ...
        while ((part_id = part_iter->get_next(part_iter)) !=
               NOT_A_PARTITION_ID) {
          if (part_info->get_subpartition_id(part_info, &subpart_id, part_id)) {
            res = 0;
            goto pop_and_go_right;
          }
          bitmap_set_bit(&part_info->read_partitions,
                         part_id * part_info->get_num_subparts() + subpart_id);
        }
...
process_next_key_part:
  ...
  if (set_full_part_if_bad_ret) {
    if (res == -1) {
      ...
      uint32 part_id;
      bool found = false;
      while ((part_id = part_iter->get_next(part_iter)) != NOT_A_PARTITION_ID) {
...
static bool create_partition_index_description(PART_PRUNE_PARAM *ppar) {
  ...
  if (ppar->subpart_fields) {
    my_bitmap_map *buf;
    uint32 bufsize = bitmap_buffer_size(ppar->part_info->get_num_subparts());
    if (!(buf = (my_bitmap_map *)alloc->Alloc(bufsize))) return true;
    bitmap_init(&ppar->subparts_bitmap, buf,
                ppar->part_info->get_num_subparts());
  }
  range_par->key_parts = key_part;
  Field **field = nullptr;
  if (ppar->part_fields) {
    field = part_info->part_field_array;
  } else if (ppar->subpart_fields) {
    field = part_info->sub_part_info->part_field_array;
  }

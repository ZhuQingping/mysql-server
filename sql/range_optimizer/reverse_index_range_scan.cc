...
bool ReverseIndexRangeScanIterator::shared_reset() {
  last_range = nullptr;
  ...
  RANGE_SEQ_IF seq_funcs = {quick_range_rev_seq_init, quick_range_seq_next,
                            nullptr};
  uint mode = m_mrr_flags;
  if (table()->file->do_parallel_scan) mode |= HA_MRR_SKIP_BY_PQ;
  if (int error = table()->file->multi_range_read_init(
          &seq_funcs, this, ranges.size(), mode, &empty_buf);
      error != 0) {

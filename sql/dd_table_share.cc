...
static const char *object_relfileid = "relfileid";
static const char *object_relblknum = "relblknum";
static const char *object_indexrelid = "indexrelid";
...
static bool prepare_share(THD *thd, TABLE_SHARE *share,
  ...
  share->m_histograms =
      new malloc_unordered_map<uint, HistogramAndMemRoot>(PSI_INSTRUMENT_ME);
...
static bool fill_share_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  if (storage_engine_mode != static_cast<ulong>(ONLY_INNODB) &&
      share->db_type()->db_type == DB_TYPE_DSTORE &&
      share->tablespace != nullptr && strlen(share->tablespace) != 0 &&
      strcmp(share->tablespace, "dstore_default") == 0) {
...
static bool fill_index_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  */
  ...
  if (thd->for_ddl_info_replay) {
    if (idx_obj->se_private_data().exists(object_indexrelid) &&
        idx_obj->se_private_data().exists(object_relfileid) &&
        idx_obj->se_private_data().exists(object_relblknum)) {
      keyinfo->flags |= HA_USES_DSTORE_DDL_COMMENT;
      idx_obj->se_private_data().get(object_indexrelid,
                                     &(keyinfo->dstore_index_oid));
      idx_obj->se_private_data().get(object_relfileid,
                                     &(keyinfo->dstore_index_file_id));
      idx_obj->se_private_data().get(object_relblknum,
                                     &(keyinfo->dstore_index_block_num));
    }
  }
...
/*
  Helper function to set partition type

  @param thd      Pointer of cuurent THD
  @param type     Partition type defined by partition-by
  @param share    Share to be updated with partitioning details.
  @param engine   Engine defined by partition-by
  @param part_info    Pointer of partition info

  @retval True means error happens internally. Otherwise false.
*/
static bool set_partition_info_type(THD *thd,
                                    dd::Table::enum_partition_type type,
                                    TABLE_SHARE *share,
                                    const dd::String_type &engine,
                                    partition_info *part_info) {
  handlerton *hton = plugin_data<handlerton *>(
      ha_resolve_by_name_raw(thd, lex_cstring_handle(engine)));
  assert(hton && ha_storage_engine_is_enabled(hton));
...
/*
  Helper function to set partition expression

  @param part_expr    Expression defined by partition-by
  @param share    Share to be updated with partitioning details.
  @param part_info    Pointer of partition info

  @retval True means error happens internally. Otherwise false.
*/
static bool set_partition_info_expr(dd::String_type part_expr,
                                    TABLE_SHARE *share,
                                    partition_info *part_info) {
...
/*
  Helper function to set default type of partition

  @param type     Type of default partition
  @param share    Share to be updated with partitioning details.
*/
static void set_partition_info_default(
    dd::Table::enum_default_partitioning type, partition_info *part_info) {
  switch (type) {
    case dd::Table::DP_NO:
...
static bool fill_partitioning_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  partition_info *part_info = new (&share->mem_root) partition_info;
  if (!part_info) return true;

  if (set_partition_info_type(thd, tab_obj->partition_type(), share,
                              tab_obj->engine(), part_info))
    return true;

  sub_partition_info *sub_part_info = nullptr;
  if (tab_obj->subpartition_type() != dd::Table::PT_NONE) {
    sub_part_info = new (&share->mem_root) sub_partition_info;
    if (!sub_part_info ||
        set_partition_info_type(thd, tab_obj->subpartition_type(), share,
                                tab_obj->engine(), sub_part_info))
      return true;
    part_info->has_sub_part = true;
  }
  part_info->sub_part_info = sub_part_info;

  if (set_partition_info_expr(tab_obj->partition_expression(), share,
                              part_info))
    return true;

  dd::String_type subpart_expr = tab_obj->subpartition_expression();
  if (subpart_expr.length() &&
      set_partition_info_expr(subpart_expr, share, sub_part_info))
    return true;
  ...
  for (const dd::Partition *part_obj : tab_obj->partitions()) {
    ...
    for (const dd::Partition *sub_part_obj : part_obj->subpartitions()) {
      ...
      if (setup_partition_from_dd(thd, &share->mem_root,
                                  part_info->sub_part_info, curr_sub_part_elem,
                                  sub_part_obj)) {
        return true;
      }

      if (sub_part_info->partitions.push_back(curr_sub_part_elem,
                                              &share->mem_root))
        return true;
  ...
  if (part_info->is_sub_partitioned()) {
    part_info->sub_part_info->num_parts =
        part_info->partitions[0]->subpartitions.elements;
    set_partition_info_default(tab_obj->default_subpartitioning(),
                               sub_part_info);
  }
  set_partition_info_default(tab_obj->default_partitioning(), part_info);

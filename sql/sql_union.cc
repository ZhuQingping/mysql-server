...
#include "sql/sql_plan_cache.h"  // is_active, set_ready, cached_plan_count
...
bool Query_result_union::create_result_table(
  ...
  for (Item *item : VisibleFields(column_types)) {
    ...
    // save derived info for PQ
    item->save_pq_derived_info();
  }
...
bool Query_expression::optimize(THD *thd, TABLE *materialize_destination,
  ...
  if (thd->lex->m_sql_cmd != nullptr &&
    ...
    if (create_access_paths(thd)) return true;
    ...
    status = make_pq_unit_plan(this, thd);
    if (status == PQ_exec_status::ABORT_EXEC) return true;
    ...
    m_query_blocks_to_materialize = set_operation()->setup_materialize_set_op(
        ...
        /*query_block_boundary=*/true,
        /*sj_mat_exec=*/nullptr, /*pq_can_share=*/false, &error);
    // Used for PQ.
  ...
  // Assign aggr iterators
  for (Query_block *query_block = first_query_block(); query_block != nullptr;
       query_block = query_block->next_query_block()) {
    JOIN *join_ = query_block->join;
    QEP_TAB *const last_tab = join_->get_last_tab();
    bool valid_ctx =
        last_tab && last_tab->m_aggregate_context && join_->root_access_path();
    if (valid_ctx) {
      WalkAccessPaths(
          join_->root_access_path(), join_,
          WalkAccessPathPolicy::ENTIRE_QUERY_BLOCK,
          [last_tab](const AccessPath *path, const JOIN *) {
            switch (path->type) {
              case AccessPath::PARTIAL_RESULT_CACHE:
                // can not work with ptrc, reset the pointer
                last_tab->m_aggregate_context = nullptr;
                if (last_tab->table() && last_tab->table()->file) {
                  last_tab->table()->file->cancel_pushed_aggregate();
                }
                break;
              case AccessPath::UNQUALIFIED_COUNT:
                if (last_tab->m_aggregate_context) {
                  last_tab->m_aggregate_context->m_count_iter =
                      static_cast<UnqualifiedCountIterator *>(path->iterator);
                }
                break;
              case AccessPath::AGGREGATE:
                if (last_tab->m_aggregate_context) {
                  last_tab->m_aggregate_context->m_aggr_iter =
                      static_cast<AggregateIterator *>(path->iterator);
                }
                break;
              default:
                break;
...
void Query_expression::create_iterator_from_accesspath(THD *thd) {
  JOIN *join;
  if (!is_set_operation()) {
    join = first_query_block()->join;
  } else if (set_operation()->m_is_materialized) {
    join = set_operation()->query_block()->join;
  } else {
    join = nullptr;
  }
  m_root_iterator = CreateIteratorFromAccessPath(
...
static AccessPath *add_materialized_access_path(
    ...
    TABLE *dest, bool query_block_boundary, Semijoin_mat_exec *sj_mat_exec,
    bool pq_can_share, ha_rows limit = HA_POS_ERROR) {
  AccessPath *path = qt->query_block()->join->root_access_path();
  path =
      NewMaterializeAccessPath(thd, std::move(query_blocks),
                               /*invalidators=*/nullptr, dest, path,
                               /*cte=*/nullptr, /*unit=*/nullptr,
                               /*ref_slice=*/-1,
                               /*rematerialize=*/true, limit,
                               /*reject_multiple_rows=*/false,
                               query_block_boundary, sj_mat_exec, pq_can_share);
...
AccessPath *make_set_op_access_path(
    ...
    bool calc_found_rows, bool query_block_boundary,
    Semijoin_mat_exec *sj_mat_exec, bool pq_can_share, bool *has_error) {
  ...
  switch (qt->term_type()) {
      ...
      if (!qts->m_is_materialized) {
        ...
        Mem_root_array<MaterializePathParameters::QueryBlock> query_blocks =
            qts->setup_materialize_set_op(
                ...
                query_block_boundary, sj_mat_exec, pq_can_share, has_error);
        ...
        path = add_materialized_access_path(thd, qts, query_blocks, dest,
                                            query_block_boundary, sj_mat_exec,
                                            pq_can_share, max_rows);
      ...
      if (union_all_subpaths != nullptr) {
        ...
        for (size_t i = start_idx; i < qts->m_children.size(); ++i) {
          ...
          if (term->term_type() != QT_QUERY_BLOCK) {
            child_path = make_set_op_access_path(
                thd, nullptr, term, nullptr, calc_found_rows,
                query_block_boundary, sj_mat_exec, pq_can_share, has_error);
          }
    ...
    case QT_UNARY: {
      ...
      path = make_set_op_access_path(thd, qts, qts->m_children[0], nullptr,
                                     calc_found_rows, query_block_boundary,
                                     sj_mat_exec, pq_can_share, has_error);
...
bool Query_block::wrapped_in_intersect_except() {
  for (auto qb = parent(); qb != nullptr; qb = qb->parent()) {
...
Query_term_set_op::setup_materialize_set_op(THD *thd, TABLE *dst_table,
                                            ...
                                            bool query_block_boundary,
                                            Semijoin_mat_exec *sj_mat_exec,
                                            bool pq_can_share, bool *error) {
  assert(!*error);
  Mem_root_array<MaterializePathParameters::QueryBlock> query_blocks(
      thd->mem_root);
  ...
  for (Query_term *term : m_children) {
    ...
    if (term->term_type() != QT_QUERY_BLOCK)
      child_path = make_set_op_access_path(
          thd, nullptr, term, nullptr, calc_found_rows, query_block_boundary,
          sj_mat_exec, pq_can_share, error);

    MaterializePathParameters::QueryBlock param;
    PQ_exec_status status = PQ_exec_status::SEQ_EXEC;
    if (join->suite_for_parallel_query()) {
      assert(term->term_type() == QT_QUERY_BLOCK);
      m_block->master_query_expression()->store_limit();
      m_block->master_query_expression()->set_limit(
          thd, static_cast<Query_block *>(term));
      status = make_pq_leader_plan(join, thd);
      m_block->master_query_expression()->restore_limit();
      // Update child_path after make_pq_leader_plan because it might be
      // replaced by PARALLEL_SCAN.
      child_path = join->root_access_path();
    }
    ...
    if (status == PQ_exec_status::ABORT_EXEC) {
      *error = true;
      query_blocks.clear();
      return query_blocks;
    } else if (status == PQ_exec_status::PARL_EXEC) {
      Switch_ref_item_slice(join, join->last_slice_before_pq);
      param = term->query_block()->setup_materialize_query_block(child_path,
                                                                 dst_table);
    } else {
      param = term->query_block()->setup_materialize_query_block(child_path,
                                                                 dst_table);
...
bool Query_expression::create_access_paths(THD *thd) {
  bool has_error = false;

  auto cleanup_guard = create_scope_guard([&] {
    if (item != nullptr && m_root_access_path) {
      if ((uncacheable & UNCACHEABLE_DEPENDENT) &&
          !(uncacheable & (UNCACHEABLE_RAND | UNCACHEABLE_SIDEEFFECT))) {
        // This is a correlated subquery - so caching it is worth doing - and
        // its result is deterministic (no RAND), and its evaluation is without
        // side-effect, so caching won't change the observable results. Note
        // that here we test for correlation only in the subquery (unit) itself,
        // not in the left argument of any associated IN operator. Thus, caching
        // will not be done when subquery materialization is used, which is
        // considered ok as that other feature is already some sort of caching.
  ...
  // temporary table. If we can, we materialize the UNION DISTINCT blocks
  // first, and then stream the remaining UNION ALL blocks (if any) by means
  // of AppendIterator.
  //
  ...
  if (global_parameters()->order_list.size() != 0 ||
    ...
    // would want to refer to rows in the table after the sort (sort by row
    // ID). We could probably be more intelligent here now.
    ...
    // we would risk incorrect results and/or infinite loops, as we'd be
    // seeing our own records as they get inserted.
    //
  ...
  if (!streaming_allowed || !is_simple()) {
    ...
    param.path = make_set_op_access_path(
        ...
        /*query_block_boundary=*/true,
        /*sj_mat_exec=*/nullptr, /*pq_can_share=*/false, &has_error);
    param.join = nullptr;
...
bool Query_expression::ExecuteIteratorQuery(THD *thd) {
  ...
  *send_records_ptr = 0;
...

  {
    ...
    if (handler) {
      handler->set_detached_status(MQ_NOT_DETACHED);
    }

    PFSBatchMode pfs_batch_mode(m_root_iterator.get());

    for (;;) {
      DEBUG_SYNC(thd, "Iterator_read");
      ...
      if (!execute_error && query_result->send_data(thd, *fields)) {
        execute_error = true;
      }
    ...
    if ((execute_error || DBUG_EVALUATE_IF("pq_worker_error4", true, false))) {
      if (thd->is_pq_worker()) {
        /*
          There may not be a 'handler', if this is a worker which here
          evaluates a correlated subquery which produces an error.
          Note that this error is sent to the client a few times instead of
        ...
        */
        if (handler) {
          ...
          /*
            'handler==nullptr' so it is a correlated subquery. It should make
            the complete statement fail iff it really had an error to report,
            otherwise it should simply terminate this evaluation of the
            subquery. This matters for

            INSERT IGNORE ... SELECT (correlated subq) FROM ...

            if the correlated subquery hits an error like "subquery returns
            more than one row" (ER_SUBQUERY_1_ROW), the subquery's evaluation
            stops, the worker's error handler turns the error into a warning,
            the subquery evaluates to NULL, and the worker must let the INSERT
  ...
  // For UNION/UNION ALL, found_rows() applies to the outermost block, this has
  // no impact on parallel query.
...
static void cleanup_tmp_tables(Table_ref *list) {
  for (auto tl = list; tl; tl = tl->next_local) {
    ...
    if ((tl->is_view_or_derived() || tl->is_recursive_reference() ||
        ...
        tl->is_transformed_in_predicate()) {
...
static void destroy_tmp_tables(Table_ref *list) {
  for (auto tl = list; tl; tl = tl->next_local) {
    ...
    if (tl->is_view_or_derived() || tl->is_recursive_reference() ||
        tl->schema_table || tl->is_table_function() ||
        tl->is_transformed_in_predicate()) {
      if (tl->table->pq_shared_table) {
        // worker also create a tmp table for sharing leader's
        // materialized result.
        assert(current_thd->is_pq_worker());
      }
  ...
  if (join) {
    if (full) {
      ...
      if (join != cached_plan) {
        ...
        // Current JOIN is the cached plan, may be reused later, keep it:
        plan_cache::set_ready(this);
        auto context = join->plan_cache_exec_context;
        if (context && context->arena)
          cleanup_items(context->arena->item_list());
        // The temporary table will be released after the SQL statement
        // execution is completed. It must be set to null here; otherwise,
        // there is a risk of dangling pointer access and double release.
        for (uint tmp = join->primary_tables;
             tmp < join->primary_tables + join->tmp_tables; tmp++) {
          join->qep_tab[tmp].set_table(nullptr);
        }
      }
      join = nullptr;
    } else
...
PQSubqueryExecution Query_expression::subquery_suite_for_parallel_query()
    const {
  /*
    PQ has supported several types of subqueries, such that
      - UNCACHEABLE_DEPENDENT, no support
      - lateral, no support
      - RECURSIVE, no support
      - derived table, support
      - uncorrelated scalar subquery, support
      - other cases, no support
  */
  if (m_lateral_deps || is_recursive()) {
    return PQSubqueryExecution::kImpossible;
  }
  ...
  // For uncachable subquery (eg. the subquery contains random contents), we
  // should not let leader execute it and then cache its value.
  if (!uncacheable) {
    // uncorrelated scalar subquery
    if (item->substype() == Item_subselect::SINGLEROW_SUBS) {
      return down_cast<Item_singlerow_subselect *>(item)->is_maxmin()
                 ? PQSubqueryExecution::kImpossible
                 : PQSubqueryExecution::kOnceByLeader;
    }
  }
  if ((item->substype() == Item_subselect::EXISTS_SUBS ||
    ...
    if (uncacheable == UNCACHEABLE_DEPENDENT &&
      ...
      switch (place()) {
        case CTX_WHERE:
        case CTX_ON:  // in fact, if ON is not later moved to WHERE by
                      // simplify_joins(), we'll reject it in
      ...
      // Cloning a list of CTE definitions is not implemented
      if (m_with_clause != nullptr) return PQSubqueryExecution::kImpossible;
      // A limitation to simplify implementation: no subquery or derived table
      // inside the subquery. But derived tables inside the subquery are
      // accepted (occurs in TPC-DS Q1-10-81).
      auto iu = first_query_block()->first_inner_query_expression();
      while (iu && iu->derived_table && iu->uncacheable == 0)
        iu = iu->next_query_expression();
      if (iu) return PQSubqueryExecution::kImpossible;
      return PQSubqueryExecution::kMultipleByWorkers;
    }
    if (uncacheable == 0) return PQSubqueryExecution::kOnceByLeader;
  }

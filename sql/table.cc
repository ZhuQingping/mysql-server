...
/* SYS_SCHEMA name */
LEX_CSTRING SYS_SCHEMA_NAME = {STRING_WITH_LEN("sys")};
  ...
  if (multi_alloc_root(&mem_root, &share, sizeof(*share), &key_buff, key_length,
    ...
    */
    ...
    mysql_mutex_init(key_TABLE_SHARE_LOCK_ha_data, &share->LOCK_ha_data,
                     MY_MUTEX_INIT_FAST);
    mysql_rwlock_init(key_rwlock_table_share_lock_m_histogram,
                      &share->LOCK_m_histograms);
  }
  return share;
}
...
void TABLE_SHARE::destroy() {
  ...
  if (tmp_table == NO_TMP_TABLE) mysql_rwlock_destroy(&LOCK_m_histograms);
...
bool unpack_value_generator(THD *thd, TABLE *table,
  ...
  auto restore_guard =
      create_scope_guard([thd, parent_digest, parent_locker]() {
...
/**
  Fix KEY_PART_INFO::field->gcol_info->expr_item value.
  ...
  @param error_reported   Error report flag.

  @retval false           Success
  @retval true            Failure
*/
static bool fix_functional_index_field_value_generator(THD *thd, TABLE *table,
                                                       bool is_create_table,
                                                       bool *error_reported) {
  for (uint key_idx = 0; key_idx < table->s->keys; ++key_idx) {
    const KEY &key_info = table->key_info[key_idx];
    for (uint part_idx = 0; part_idx < key_info.actual_key_parts; ++part_idx) {
      const KEY_PART_INFO &key_part = key_info.key_part[part_idx];
      if (key_part.field->gcol_info != nullptr &&
          key_part.field->gcol_info->expr_item == nullptr &&
          key_part.field->is_field_for_functional_index()) {
        // This is a generated column where the generated expression has not
        // been resolved yet. Functional indexes are implemented using hidden
        // virtual generated columns, and "key_part.field" is the same field
        // that is found in TABLE::field.
        //
        // However, if we have a functional index that is longer than the max
        // index length (typically expressions that returns a string),
        // "key_part.field" is replaced with a new Field object having a
        // different max length property (happens inside
        // create_key_part_field_with_prefix_length()). This means that
        // "key_part.field" is no longer found in the array TABLE::field, and
        // its "expr_item" is not resolved. So this loop ensures that these new
        // fields are properly resolved.
        if (unpack_value_generator(thd, table, &key_part.field->gcol_info,
                                   VGS_CHECK_CONSTRAINT,
                                   key_part.field->field_name, nullptr,
  ...
  */
  ...
  if (share->vfields) {
    ...
    *vfield_ptr = nullptr;  // End marker
    ...
    //  Unpack virtual generated Field of functional indexs.
    if (fix_functional_index_field_value_generator(
            thd, outparam, is_create_table, &error_reported)) {
      goto err;
    }
  }
...
bool TABLE::add_tmp_key(Field_map *key_parts, bool invisible, bool modify_share,
                        const char *keyname) {
  ...
  if (modify_share) {
    ...
    if (!keyname)
      ...
      for (int k = 0; k < NAME_CHAR_LEN; k++) {
        s->key_names[s->keys].name[k] = keyname[k];
        // Keep the ending 0
...
*/
bool Table_ref::process_index_hints(THD *thd, TABLE *tbl) {
  /* initialize the result variables */
  tbl->keys_in_use_for_query = tbl->keys_in_use_for_group_by =
      tbl->keys_in_use_for_order_by = tbl->s->usable_indexes(thd);
  Ignore_index_hints_syntax_handler ignore_handler;
  bool has_index_outline = false;

  if (outline_index_hints) {
    std::swap(outline_index_hints, index_hints);
    has_index_outline = true;
    thd->push_internal_handler(&ignore_handler);
  }

  auto cleanup_grd = create_scope_guard([&] {
    if (outline_index_hints) outline_index_hints->clear();
    if (has_index_outline) thd->pop_internal_handler();
  });
...
err:
  if (has_index_outline) {
    std::swap(outline_index_hints, index_hints);
    cleanup_grd.reset();
    has_index_outline = false;

    Opt_trace_context *const trace = &thd->opt_trace;
    Opt_trace_object trace_wrapper(trace);
    Opt_trace_object outline_work(trace, "index_hint_outline");
    outline_work.add("chosen", false);
    outline_work.add_utf8("cause", "Index hints from outline are incorrect.");
    goto retry;
  }
...
/**
 * determine whether this table can be divided/cut for parallel query
 *
 * @retval true if it can
 */
bool TABLE::suite_for_pq_division() {
  // Temptable is only used in MySQL for internal tmp tables, verify:
  assert(!(file->ht->db_type == DB_TYPE_TEMPTABLE &&
           s->tmp_table == NO_TMP_TABLE));
  // - Base table, or
  // - internal temporary table materializing a derived table.
  // We want to ban tables created by the user with CREATE TEMPORARY TABLE:
  // in such case, InnoDB ones have TRANSACTIONAL_TMP_TABLE.
  // We also want to ban semi-join materialized table (INTERNAL_TMP_TABLE):
  // - Temptable doesn't support division if there's an index, so
  // a tmp table for materialization-lookup cannot be divided
  // - a tmp table materialization-scan could be divided, in theory, but it
  // would require investigation (main.subquery_sj_mat_bka_nobnl shows wrong
  // changes in a SELECT's result, in test for Bug#52068)
  return (s->tmp_table == NO_TMP_TABLE ||
          s->tmp_table == NON_TRANSACTIONAL_TMP_TABLE) &&
         // only two engines:
         (file->ht->db_type == DB_TYPE_INNODB ||
          file->ht->db_type == DB_TYPE_TEMPTABLE) &&
         // partitioned table
         (!part_info || part_info->num_partitions_used() == 1) &&
         !(pos_in_table_list &&
           pos_in_table_list->is_fulltext_searched())  // fulltext match search
      ;
}

bool ORDER::convert_to_list(SQL_I_List<ORDER> &list, THD *thd) {
  ...
  while (tmp_order) {
    ...
    if (!tmp_order->item_initial) {
      tmp_order->item_initial = tmp_order->item[0];
    }
    list.link_in_list(tmp_order, &tmp_next);

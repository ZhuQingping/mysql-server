...
#include "sql/recyclebin/recycle_scheduler.h"
...
static constexpr const unsigned long long OPTIMIZER_SWITCH_DEFAULT{
    ...
    OPTIMIZER_SWITCH_SIMPLIFY_COUNT_NOT_NULL |
    OPTIMIZER_SWITCH_OFFSET_PUSHDOWN | OPTIMIZER_SWITCH_NLJ_DISTINCT_OPTIMIZE |
    OPTIMIZER_SWITCH_PARTIAL_RESULT_CACHE |
    OPTIMIZER_SWITCH_LEFT_JOIN_ELIMINATION |
    OPTIMIZER_SWITCH_DSTORE_ENABLE_INDEXSCAN |
    OPTIMIZER_SWITCH_AGGREGATE_PUSHDOWN};

static constexpr const unsigned long MYSQLD_NET_RETRY_COUNT{10};

static constexpr const unsigned long long RDS_SECURITY_HARDENING_DEFAULT{0};
...
/**
  Turning on binlog_order_commits is not allowed while using the dstore storage
  engine.
 ...
 */
static bool binlog_order_commits_check(sys_var *, THD *, set_var *var) {
  bool new_value = var->save_result.ulonglong_value;
  if (get_instance_storage_engine_type() == DB_TYPE_DSTORE && new_value) {
    my_error(ER_DSTORE_TURN_ON_BINLOG_ORDER_COMMITS_NOT_ALLOWED, MYF(0));
    return true;
  }
  return false;
}
...
static Sys_var_ulong Sys_pfs_max_statement_classes(
    ...
    DEFAULT((ulong)SQLCOM_END +
            (ulong)(COM_END - (COM_GAP_END - COM_GAP_BEG + 1)) + 5 +
...
static Sys_var_ulong Sys_pq_msg_queue_size(
    "pq_msg_queue_size", "the size of message queue: [1MB, 1024MB]",
...
static Sys_var_ulong Sys_pq_msg_queue_spin_lock(
    ...
    "the count of spin lock when message queue is blocked",
...
static Sys_var_bool Sys_sql_innodb_parallel_select_count(
    ...
    HINT_UPDATEABLE SESSION_VAR(innodb_parallel_select_count),
    CMD_LINE(OPT_ARG), DEFAULT(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
...
static Sys_var_ulong Sys_pq_hash_join_max_hash_table_refills(
    ...
    "The upper limit of estimated hash table re-fills before disabling "
    "parallel query. Only effective if parallel hash_join_spill_to_disk is "
    "OFF.",
...
static Sys_var_ulong Sys_parallel_cost_threshold(
    ...
    HINT_UPDATEABLE SESSION_VAR(parallel_cost_threshold),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, ULONG_MAX), DEFAULT(1000),
    BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG);

static Sys_var_ulong Sys_parallel_rows_threshold(
    "parallel_rows_threshold", "Rows threshold for parallel query.",
    HINT_UPDATEABLE SESSION_VAR(parallel_rows_threshold),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, ULONG_MAX), DEFAULT(10000),
    BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG);

static bool check_parallel_default_dop(sys_var *self MY_ATTRIBUTE((unused)),
                                       THD *thd, set_var *var) {
  ulonglong new_parallel_default_dop = var->save_result.ulonglong_value;
  if (new_parallel_default_dop > parallel_max_threads)
    push_warning_printf(
        thd, Sql_condition::SL_WARNING,
        ER_SET_PARALLEL_DEFAULT_DOP_EXCEEDS_PARALLEL_MAX_THREADS,
        ER_THD(thd, ER_SET_PARALLEL_DEFAULT_DOP_EXCEEDS_PARALLEL_MAX_THREADS),
        new_parallel_default_dop, parallel_max_threads);
  return false;
}
static Sys_var_ulong Sys_parallel_default_dop(
    ...
    NOT_IN_BINLOG, ON_CHECK(check_parallel_default_dop));
...
static Sys_var_bool Sys_parallel_fail_retry(
    "parallel_fail_retry",
    "Whether to automatically retry failed parallel queries with parallel "
    "query disabled.",
    SESSION_VAR(parallel_fail_retry), CMD_LINE(OPT_ARG), DEFAULT(true));

static Sys_var_double Sys_parallel_tuple_cost(
    "parallel_tuple_cost",
    "The cost of transmitting a single record between worker and gather "
    "during parallel queries.",
...
static Sys_var_double Sys_parallel_setup_cost(
    "parallel_setup_cost",
    "The cost of starting and initializing all worker threads during parallel "
    "queries.",
...
static Sys_var_ulong Sys_op_over_pq_offset_threshold(
    ...
    "The minimum threshold value for offset in offset pushdown takes "
    "precedence over parallel query.",
...
static Sys_var_bool Sys_parallel_limit_no_order_by(
    ...
    "enable PQ execution when the query has a LIMIT clause but no ORDER BY "
    "clause",
...
static Sys_var_enum Sys_storage_engine_mode(
    "storage_engine_mode",
...
// Support for graceful fallback in PQ was added late in PQ, as a "feature",
// and thus must be switchable to off, per policy
static Sys_var_bool Sys_parallel_graceful_fallback(
    "parallel_graceful_fallback",
    "Allows a parallel query execution, if it fails to set up, to directly "
    "fall back to serial execution, instead of doing a full "
    "re-parsing/optimization/execution of the SQL statement serially.",
    HINT_UPDATEABLE SESSION_VAR(parallel_graceful_fallback), CMD_LINE(OPT_ARG),
    DEFAULT(true));

static Sys_var_int32 Sys_rds_max_num_of_binary_logs_purged_per_time(
    "rds_max_num_of_binary_logs_purged_per_time",
    "The max number of binary logs purged asynchronously "
    "by a dedicated thread per time.",
    GLOBAL_VAR(rds_max_num_of_binary_logs_purged_per_time),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(1, INT32_MAX), DEFAULT(100),
...
static bool check_explicit_defaults_for_timestamp(sys_var *self, THD *thd,
  ...
  /*
      Customized bypass for Dstore distributed engine:
      Allow modifying explicit_defaults_for_timestamp within a transaction ONLY
     if:
      1. pseudo_replica_mode is ON (ensures compatibility during binlog
     replay/recovery).
      2. The current storage engine is DB_TYPE_DSTORE.
      3. The change scope is SESSION (prevents accidental global stability
     impact).
  */

  if (thd->in_active_multi_stmt_transaction() &&
      thd->variables.pseudo_replica_mode &&
      get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
...
static PolyLock_mutex PLock_purge_binlog(&LOCK_purge_binlog);
static const char *rds_binlog_purge_thread_status_names[] = {
    "NOT_CREATED",
    "EXIT",
    "GONE",
    "RESET",
    ...
    "SLEEPING",
    "AWAKENED",
    "REQUESTING_LOCK_INDEX",
    "PURGING_BINARY_LOGS",
    "RELEASED_LOCK_INDEX",
    NullS};
static Sys_var_enum Sys_rds_binlog_purge_thread_status(
    "rds_binlog_purge_thread_status",
    "Display the current status of the binlog purge thread."
    "NOT_CREATED means that the binlog purge thread is not created;"
    "EXIT means that the binlog purge thread exits;"
    "GONE means that the binlog purge thread went away unusually;"
    "RESET is a transient status of the binlog purge thread, it is set "
    "before signalling purge;"
    "INITIALIZING means that the binlog purge thread is initializing;"
    "INITIALIZED means that the binlog purge thread initialized;"
    "SLEEPING means that the binlog purge thread is sleeping;"
    "AWAKENED means that the binlog purge thread is awakened;"
    "REQUESTING_LOCK_INDEX means that the binlog purge thread "
    "is requesting LOCK_index;"
    "PURGING_BINARY_LOGS means that the binlog purge thread "
    "is purging binary logs;"
    "RELEASED_LOCK_INDEX means that the binlog purge thread "
    "released LOCK_index.",
    READ_ONLY GLOBAL_VAR(rds_binlog_purge_thread_status),
    CMD_LINE(REQUIRED_ARG), rds_binlog_purge_thread_status_names,
    DEFAULT(BINLOG_PURGE_THEAD_STATUS_NOT_CREATED), &PLock_purge_binlog,
    NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));
...
static bool check_storage_engine(sys_var *self, THD *thd, set_var *var) {
  ...
  if (!opt_initialize && !opt_noacl) {
    ...
    if (ha_check_storage_engine_mode(storage_engine_mode, se_name.str)) {
      push_warning_printf(
          thd, Sql_condition::SL_WARNING,
          ER_STORAGE_ENGINE_MODE_NOT_MATCH_DEFAULT_ENGINE,
          ER_THD(thd, ER_STORAGE_ENGINE_MODE_NOT_MATCH_DEFAULT_ENGINE),
          self->name.str, se_name.str, se_name.str);
      // se_name changed
      ha_get_default_storage_engine_from_mode(&se_name);
      if ((plugin = ha_resolve_by_name(thd, &se_name, false))) {
        plugin_unlock(thd, var->save_result.plugin);
...
static Sys_var_double Sys_plan_cache_allow_change_ratio(
    ...
    "cached plan will be invalidated. A value of 0 means the cached plan is "
    "always valid.",
...
static Sys_var_ulong Sys_rds_ddl_lock_wait_timeout(
    "rds_ddl_lock_wait_timeout",
    "Timeout in seconds to wait for a lock for the following DDL operations"
    " before returning an error: ALTER TABLE, CREATE INDEX AND DROP INDEX."
    " Note that this timeout may be replaced by the lock_wait_timeout, or the"
    " innodb_lock_wait_timeout in InnoDB layer, if those are smaller.",
    HINT_UPDATEABLE SESSION_VAR(rds_ddl_lock_wait_timeout),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(1, LONG_TIMEOUT), DEFAULT(LONG_TIMEOUT),
    BLOCK_SIZE(1));

static Sys_var_ulong Sys_rds_binlog_purge_thread_wait_timeout(
    "rds_binlog_purge_thread_wait_timeout",
    "The waiting timeout in seconds for binlog purge thread. After timeout, "
    "this thread will purge expired binary logs automatically.",
    GLOBAL_VAR(rds_binlog_purge_thread_wait_timeout), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(1, LONG_TIMEOUT), DEFAULT(600), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));
...
static Sys_var_double Sys_rds_packet_size_factor(
    "rds_packet_size_factor", "The maximum multiplier for max_allowed_packet.",
    GLOBAL_VAR(rds_packet_size_factor), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(1, 4), DEFAULT(1.4), NO_MUTEX_GUARD, NOT_IN_BINLOG);
...
static Sys_var_ulonglong Sys_vfd_max_open_files(
    "vfd_max_open_files",
    "Maximum number of files the VFD Manager can open, per connection.",
    GLOBAL_VAR(vfd_max_open_files), CMD_LINE(OPT_ARG), VALID_RANGE(1, 256),
...
static const char *optimizer_switch_names[] = {
    ...
    "dstore_use_histogram_range_estimation",
    "simplify_count_not_null",
    "offset_pushdown",
    "nlj_distinct_optimize",
    "partial_result_cache",
    "left_join_elimination",
    "dstore_enable_indexscan",
    "aggregate_pushdown",
    "default",
    NullS};
static Sys_var_flagset Sys_optimizer_switch(
    ...
    " derived_condition_pushdown, offset_pushdown, partial_result_cache,"
    " left_join_elimination, dstore_enable_indexscan, aggregate_pushdown} and "
    "val is one of "
    "{on, off, default}",
...
static Sys_var_bool Sys_rds_permission_control(
    "rds_permission_control",
    "This option indicates whether the RDS Service access control is enabled.",
    GLOBAL_VAR(opt_rds_permission_control), CMD_LINE(OPT_ARG), DEFAULT(true));

static Sys_var_bool Sys_rds_sync_comment(
    "rds_sync_comment",
    "This option indicates whether rds_comment should "
    "sync with database or user updates.",
    GLOBAL_VAR(opt_rds_sync_comment), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static bool check_rds_reserved_users(sys_var *self, THD *, set_var *var) {
  if (var->value == nullptr) {
    return false;
  }
  ...
  if (!is_valid_rds_reserved_users(var->save_result.string_value.str,
...
static bool update_rds_reserved_users_hash(sys_var *, THD *, enum_var_type) {
  return (0 != rds_reserved_users_update(rds_reserved_users_ptr));
}

static Sys_var_charptr Sys_rds_reserved_users(
    "rds_reserved_users", "RDS Service reserved users.",
    GLOBAL_VAR(rds_reserved_users_ptr), CMD_LINE(REQUIRED_ARG),
    IN_SYSTEM_CHARSET, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(check_rds_reserved_users),
    ON_UPDATE(update_rds_reserved_users_hash));
...
static bool check_transaction_isolation(sys_var *, THD *thd, set_var *var) {
  if (storage_engine_mode == static_cast<ulong>(ONLY_DSTORE) &&
    ...
    my_error(ER_DSTORE_TRX_CHANGE_TO_REPEATABLE_READ_NEED_SUPER, MYF(0));
    return true;
  }
...
static Sys_var_transaction_isolation Sys_transaction_isolation(
    ...
    tx_isolation_names, DEFAULT(ISO_READ_COMMITTED), NO_MUTEX_GUARD,
...
/**
  Create binlog purge thread after turning on
  rds_purge_binary_logs_asynchronously.
  Terminate binlog purge thread after turning off
  rds_purge_binary_logs_asynchronously.
  ...
  @return @c false.
*/
static bool fix_rds_purge_binary_logs_asynchronously_after_update(
    sys_var *, THD *, enum_var_type type MY_ATTRIBUTE((unused))) {
  DBUG_ASSERT(type == OPT_GLOBAL);

  if (opt_bin_log) {
    if (rds_purge_binary_logs_asynchronously) {
      mysql_mutex_unlock(&LOCK_global_system_variables);
      create_binlog_purge_thread();
      mysql_mutex_lock(&LOCK_global_system_variables);
    } else {
      terminate_binlog_purge_thread();
    }
  }
...
static bool fix_sql_log_bin_after_update(sys_var *, THD *thd,
  ...
  /* Security Audit and Warning Logic:
      For cloud environments (like RDS), we monitor changes to SQL_LOG_BIN to
      prevent accidental data inconsistency between source and replicas.

      We exclude internal automated agents to avoid log flooding:
      1. Skip during server initialization (!opt_initialize).
      2. Only audit if global binlogging is enabled (opt_bin_log).
      3. Respect the custom toggle (opt_warn_sql_log_bin).
      4. Filter out 'localhost' internal/reserved accounts used by maintenance
     agents.
  */
  if (!opt_initialize && opt_bin_log && opt_warn_sql_log_bin &&
      thd->security_context()->host_or_ip().length != 0 &&
      ((strcmp(thd->security_context()->host_or_ip().str, "localhost") &&
        !is_rds_reserved_user(thd->security_context()->user().str)) ||
       DBUG_EVALUATE_IF("mock_host_is_remote", true, false))) {
    if (!thd->variables.sql_log_bin) {
      sql_print_warning(
          "User %s SET SQL_LOG_BIN=OFF, which may causes "
          "inconsistence between master and slave.",
          thd->security_context()->user().str);
      push_warning_printf(
          thd, Sql_condition::SL_WARNING, ER_WARNING_SQL_LOG_BIN_OFF,
          "Set SQL_LOG_BIN=OFF may causes inconsistence between "
          "master and slave.");
    } else {
      sql_print_information("User %s SET SQL_LOG_BIN=ON.",
                            thd->security_context()->user().str);
    }
  }
...
static Sys_var_charptr Sys_disabled_storage_engines(
    ...
    IN_SYSTEM_CHARSET, DEFAULT("InnoDB,CSV,Memory,MyISAM,MRG_MyISAM"));
...
static Sys_var_bool Sys_rds_write_binlog_into_redo(
    "rds_write_binlog_into_redo", "Enable binlog write into redo.",
    READ_ONLY GLOBAL_VAR(rds_write_binlog_into_redo), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_binlog_concurrent_commit(
    "rds_binlog_concurrent_commit",
    "Enable binlog concurrent commit with one phase commit on dstore.",
    GLOBAL_VAR(rds_binlog_concurrent_commit), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_bool Sys_rds_warn_alter_table_add_auto_incre_column(
    "rds_warn_alter_table_add_auto_incre_column",
    "Report a warning when alter table add increment column if the option is "
    "TRUE",
    GLOBAL_VAR(rds_warn_alter_table_add_auto_incre_column), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

/**
  After setting rds_binlog_dstore_consistent to true, do the following things:
    -- write gtid executed into redo
    -- make rds_dstore_missed_gtids be empty
    -- make rds_dstore_missed_gtids_on_binlog be empty

  @param     thd    Current thread

  @return @c false.
*/
static bool fix_rds_binlog_dstore_consistent_after_update(sys_var *, THD *thd,
                                                          enum_var_type) {
  if (rds_binlog_dstore_consistent && rds_dstore_missed_gtids_ptr != nullptr &&
      rds_dstore_missed_gtids_ptr[0] != '\0' &&
      get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
      rds_dstore_support_binlog_check && !rds_write_binlog_into_redo &&
      global_gtid_mode.get() == Gtid_mode::ON) {
    write_gtid_executed_info_into_redo(thd);
    rds_dstore_missed_gtids_ptr[0] = '\0';
    if (rds_dstore_missed_gtids_on_binlog_ptr != nullptr) {
      rds_dstore_missed_gtids_on_binlog_ptr[0] = '\0';
    }
  }
...
static Sys_var_bool Sys_rds_binlog_dstore_consistent(
    "rds_binlog_dstore_consistent",
    "Indicates whether the data in the binary log and "
    "the dstore storage engine are consistent.",
    GLOBAL_VAR(rds_binlog_dstore_consistent), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(fix_rds_binlog_dstore_consistent_after_update));

static Sys_var_bool Sys_rds_dstore_support_binlog_check(
    "rds_dstore_support_binlog_check",
    "Indicates whether to support checking the consistency of data in the "
    "binary log and dstore storage engine.",
    GLOBAL_VAR(rds_dstore_support_binlog_check), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_binlog_enable_group_replication(
    "rds_binlog_enable_group_replication",
    "Indicates whether to enable binlog based group replication."
    "Note: please set it to true if group replication is used.",
    GLOBAL_VAR(rds_binlog_enable_group_replication), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_binlog_skip_run_after_commit_hook(
    "rds_binlog_skip_run_after_commit_hook",
    "Indicates whether to skip running after_commit hook on commit stage."
    "Note: please set it to false if group replication is used.",
    READ_ONLY GLOBAL_VAR(rds_binlog_skip_run_after_commit_hook),
    CMD_LINE(OPT_ARG), DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_binlog_dstore_trace(
    "rds_binlog_dstore_trace",
    "Indicates whether to write trace information for binlog and destore.",
    GLOBAL_VAR(rds_binlog_dstore_trace), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_dstore_use_histogram_auto_update(
    "dstore_use_histogram_auto_update",
    "enable automatically update histograms based on table data changed",
    GLOBAL_VAR(dstore_use_histogram_auto_update), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));
...
static Sys_var_charptr Sys_rds_log_archive_backup_path(
    "rds_log_archive_path", "Path for incremental local backup data",
    GLOBAL_VAR(rds_log_archive_path_ptr), CMD_LINE(OPT_ARG), IN_FS_CHARSET,
    DEFAULT(""), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_dstore_missed_gtids(
    "rds_dstore_missed_gtids",
    "A unique set of identifiers for missed transactions "
    "in the dstore storage engine.",
    READ_ONLY GLOBAL_VAR(rds_dstore_missed_gtids_ptr), CMD_LINE(OPT_ARG),
    IN_FS_CHARSET, DEFAULT(""), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_dstore_missed_gtids_on_binlog(
    "rds_dstore_missed_gtids_on_binlog",
    "The file name of a binary log where the missed transactions "
    "in the dstore storage engine are located.",
    READ_ONLY GLOBAL_VAR(rds_dstore_missed_gtids_on_binlog_ptr),
    CMD_LINE(OPT_ARG), IN_FS_CHARSET, DEFAULT(""), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_ulong Sys_rds_local_backup_lock_timeout(
    "rds_local_backup_lock_timeout",
    "Timeout in seconds for local backup to wait to get MDL",
    GLOBAL_VAR(rds_full_local_backup_lock_timeout), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(1, LONG_TIMEOUT), DEFAULT(300), BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_binlog_lock_enable(
    "rds_binlog_lock_enable",
    "Enables or disables binlog metadata locks. ON indicates that this "
    "function is enabled. OFF indicates that the function "
    "is disabled. The default value is ON.",
    GLOBAL_VAR(rds_binlog_lock_enable), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_ulong Sys_rds_local_backup_lock_binlog_timeout(
    "rds_local_backup_lock_binlog_timeout",
    "Timeout in seconds for local backup binlog lock",
    GLOBAL_VAR(rds_full_local_backup_lock_binlog_timeout),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(1, LONG_TIMEOUT), DEFAULT(300),
    BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_local_backup_fetch_gtid(
    "rds_local_backup_fetch_gtid",
    "Enable or disable fetching the binlog GTID informations when backup.",
    READ_ONLY GLOBAL_VAR(rds_full_local_backup_fetch_gtid), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_local_backup_lock_optimize(
    "rds_local_backup_lock_optimize",
    "Hold BACKUP MDL for innodb backup or entrie backup process. If enabled, "
    "hold BACKUP MDL for only innodb backup process",
    GLOBAL_VAR(rds_full_local_backup_lock_optimize), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

static Sys_var_uint Sys_rds_local_backup_file_interface_type(
    ...
    READ_ONLY GLOBAL_VAR(rds_local_backup_file_interface_type),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 1), DEFAULT(1), BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_dstore_lb_use_io_thread(
    "rds_dstore_lb_use_io_thread",
    "Whether use io thread for local backup dstore part.",
    GLOBAL_VAR(rds_dstore_lb_use_io_thread), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_uint Sys_rds_dstore_lb_global_io_retry_number(
    "rds_dstore_lb_global_io_retry_number",
    "Retry number for local backup dstore io operation in whole task.",
    GLOBAL_VAR(rds_dstore_lb_global_io_retry_number), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, UINT_MAX), DEFAULT(3), BLOCK_SIZE(1));

static Sys_var_uint Sys_rds_dstore_lb_local_io_retry_times(
    "rds_dstore_lb_local_io_retry_times",
    "Retry times for local backup one dstore io operation.",
    GLOBAL_VAR(rds_dstore_lb_local_io_retry_times), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, UINT_MAX), DEFAULT(5), BLOCK_SIZE(1));

static Sys_var_int32 Sys_rds_dstore_lb_local_io_hang_timeout_ms(
    "rds_dstore_lb_local_io_hang_timeout_ms",
    "Timeout ms of local backup dstore io hange.",
    GLOBAL_VAR(rds_dstore_lb_local_io_hang_timeout_ms), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, INT_MAX), DEFAULT(100000), BLOCK_SIZE(1));

static Sys_var_int32 Sys_rds_dstore_lb_local_io_thread_quit_timeout_ms(
    "rds_dstore_lb_local_io_thread_quit_timeout_ms",
    "Timeout ms of waiting dstore io thread quit.",
    GLOBAL_VAR(rds_dstore_lb_local_io_thread_quit_timeout_ms),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, INT_MAX), DEFAULT(1000),
    BLOCK_SIZE(1));

static bool recycle_bin_mode_update(sys_var *self, THD *thd,
                                    enum_var_type type) {
  bool res = false;
  bool is_global = self->is_global_persist(type);
  System_variables *sv = is_global ? &global_system_variables : &thd->variables;
  if (!read_only && sv->rds_recycle_bin_mode) {
    if (is_global) mysql_mutex_unlock(&LOCK_global_system_variables);
    res = im::recycle_bin::recreate_recycle_bin_schema(thd);
    if (res != false) {
      my_error(ER_PREPARE_RECYCLE_TABLE_ERROR, MYF(0),
               "create database __recyclebin__ failed");
    }
    if (is_global) mysql_mutex_lock(&LOCK_global_system_variables);
  }
  return res;
}
...
static Sys_var_uint Sys_rds_dstore_lb_io_buffer_size(
    "rds_dstore_lb_io_buffer_size",
    "Buffer size for reading tablspace file during backup.",
    GLOBAL_VAR(rds_dstore_lb_io_buffer_size), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(8192, 32 * 1024 * 1024), DEFAULT(64 * 1024), BLOCK_SIZE(8192));

static Sys_var_bool Sys_rds_local_backup_use_obs(
    "rds_local_backup_use_obs", "Use OBS for local backup of Taurus on dstore",
    GLOBAL_VAR(rds_local_backup_use_obs), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_local_backup_obs_host(
    "rds_local_backup_obs_host", "Host name of OBS service for backup",
    GLOBAL_VAR(rds_local_backup_obs_host), CMD_LINE(REQUIRED_ARG),
    IN_FS_CHARSET, DEFAULT(nullptr), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_local_backup_obs_bucket(
    "rds_local_backup_obs_bucket", "Name of OBS bucket for backup",
    GLOBAL_VAR(rds_local_backup_obs_bucket), CMD_LINE(REQUIRED_ARG),
    IN_FS_CHARSET, DEFAULT(nullptr), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_local_backup_obs_access_key(
    "rds_local_backup_obs_access_key", "Access key of OBS service for backup",
    GLOBAL_VAR(rds_local_backup_obs_ak), CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
    DEFAULT(nullptr), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_local_backup_obs_secret_key(
    "rds_local_backup_obs_secret_key", "Secret key of OBS service for backup",
    GLOBAL_VAR(rds_local_backup_obs_sk), CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
    DEFAULT(nullptr), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

static Sys_var_uint Sys_rds_local_backup_obs_uri_style(
    "rds_local_backup_obs_uri_style",
    "Uri style of OBS service for backup. "
    "0 => OBS_URI_STYLE_VIRTUALHOST; "
    "1 => OBS_URI_STYLE_PATH",
    GLOBAL_VAR(rds_local_backup_obs_uri_style), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, 1), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_uint Sys_rds_lb_obs_buffer_size(
    "rds_lb_obs_buffer_size",
    "Buffer size for local backup read from or upload to OBS.",
    GLOBAL_VAR(rds_lb_obs_buffer_size), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(512 * 1024, UINT_MAX), DEFAULT(16 * 1024 * 1024),
    BLOCK_SIZE(1));

static Sys_var_uint Sys_rds_local_backup_sleep_interval(
    "rds_local_backup_sleep_interval",
    "Sleep interval for full local backup, after this value times write, do a "
    "sleep.",
    GLOBAL_VAR(rds_local_backup_sleep_interval), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, UINT_MAX), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_uint Sys_rds_local_backup_sleep_time_ms(
    "rds_local_backup_sleep_time_ms",
    "Sleep time in milliseconds for full local backup.",
    GLOBAL_VAR(rds_local_backup_sleep_time_ms), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, 10000), DEFAULT(10), BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_lb_obs_enable_scc_decrypt(
    "rds_lb_obs_enable_scc_decrypt",
    "Enable scc decrypt when use OBS for local backup of Taurus on dstore.",
    READ_ONLY GLOBAL_VAR(rds_lb_obs_enable_scc_decrypt), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr));

static Sys_var_uint Sys_rds_lb_obs_max_connected_time(
    "rds_lb_obs_max_connected_time",
    "Max connected time in seconds for local backup obs sdk api calls.",
    GLOBAL_VAR(rds_lb_obs_max_connected_time), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, 7 * 24 * 60 * 60), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_lb_obs_encryption(
    "rds_lb_obs_encryption",
    "Enable OBS encryption for local backup of Taurus on dstore.",
    GLOBAL_VAR(rds_lb_obs_encryption), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));

static Sys_var_charptr Sys_rds_lb_obs_kms_key_id(
    "rds_lb_obs_kms_key_id",
    "Kms key id of OBS encryption for local backup of Taurus on dstore.",
...
static Sys_var_ulong Sys_recycle_bin_retention(
    ...
    DEFAULT(3 * 24 * 60 * 60), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(0), ON_UPDATE(recycle_scheduler_retention_update));

static bool recycle_scheduler_update(sys_var *, THD *, enum_var_type) {
  bool res = false;
  bool value = im::recycle_bin::rds_recycle_scheduler;
  mysql_mutex_unlock(&LOCK_global_system_variables);
  ...
  mysql_mutex_lock(&LOCK_global_system_variables);
  if (res) {
    im::recycle_bin::rds_recycle_scheduler = false;
...
static Sys_var_bool Sys_recycle_scheduler(
    ...
    GLOBAL_VAR(im::recycle_bin::rds_recycle_scheduler), CMD_LINE(OPT_ARG),
...
static Sys_var_bool Sys_recycle_scheduler_purge_table_print(
    ...
    GLOBAL_VAR(im::recycle_bin::rds_recycle_scheduler_purge_table_print),
    CMD_LINE(OPT_ARG), DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_recycle_bin_skip_acl_check(
    "rds_recycle_bin_skip_acl_check",
    "Controls the permission check for the recyclebin operation..",
    GLOBAL_VAR(rds_recycle_bin_skip_acl_check), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_rds_purge_binary_logs_asynchronously(
    "rds_purge_binary_logs_asynchronously",
    "Specifies whether the server purges expired binary logs asynchronously "
    "by a dedicated thread.",
    GLOBAL_VAR(rds_purge_binary_logs_asynchronously), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
    ON_UPDATE(fix_rds_purge_binary_logs_asynchronously_after_update),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_var_rds_empty_redundant_check_in_range_scan(
    "rds_empty_redundant_check_in_range_scan",
    "From the condition evaluated at the SQL layer, remove any comparison "
    "already evaluated by the range index scan method in the storage engine",
    SESSION_VAR(rds_empty_redundant_check_in_range_scan), CMD_LINE(OPT_ARG),
    DEFAULT(true));

static Sys_var_bool Sys_rds_recycle_bin_binlog_session_variables(
    "rds_recycle_bin_binlog_session_variables",
    "Record the session variables of recycle bin to binlog.",
    GLOBAL_VAR(rds_recycle_bin_binlog_session_variables), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static Sys_var_ulong Sys_rds_admin_max_connections(
    "rds_admin_max_connections",
    "The number of simultaneous clients allowed through admin port",
    GLOBAL_VAR(opt_rds_admin_max_connections), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(1, 1000), DEFAULT(MAX_RDS_ADMIN_CONNECTIONS_DEFAULT),
    BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_EARLY);

static Sys_var_ulong Sys_rds_reserved_connections(
    "rds_reserved_connections",
    "The number of connections for reserved users "
    "allowed to connect. It's not count in normal connections "
    "and admin connections",
    GLOBAL_VAR(max_rds_reserved_connections), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(1, 100000), DEFAULT(MAX_RDS_RESERVED_CONNECTIONS_DEFAULT),
    BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG);

static Sys_var_bool Sys_rds_admin_port_using_per_thread(
    "rds_admin_port_using_per_thread",
    "This option indicates whether admin port connections should use per "
    "thread",
    GLOBAL_VAR(opt_rds_admin_port_using_per_thread), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_local_socket_using_per_thread(
    "rds_local_socket_using_per_thread",
    "This option indicates whether local socket connections should use per "
    "thread",
    GLOBAL_VAR(opt_rds_local_socket_using_per_thread), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_dstore_enable_atomic_ddl(
    "rds_dstore_enable_atomic_ddl", "Enable dstore atomic DDL feature.",
    READ_ONLY GLOBAL_VAR(rds_dstore_enable_atomic_ddl), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

/*
  1) Currently Dstore cannot reserve page space for the TD slot according
  to the fillfactor. It can only guarantee every index page has at least
  4 TD slots. If concurrency of DDL query beyond DSTORE::DEFAULT_TD_COUNT,
  when they write ddl log table's index and locate to a same index page,
  they may fail due to can't alloc TD slot. But handlerton::post_ddl can't
  tolerate failure, so it will lead to an assertion. According to the CCB
  conclusion, we limit the concurrency of DDL query to a safe level.
  2) It is possible to cause deadlock when each one of concurrent threads
  occupied a TD with a normal transaction while it want to alloc another TD
  with an autonomous transaction in the same page. So we must set the max
  concurrency of ddl not over than (number of TD)/2 .
  Based on the above two points, the safe value of concurrency of DDL query
  is DSTORE::DEFAULT_TD_COUNT/2 = 2, that is too small. We decided to take
  a little risk and adjust the default DDL concurrency to 4.
*/
static Sys_var_uint Sys_rds_dstore_ddl_max_concurrency(
    "rds_dstore_ddl_max_concurrency",
    "This option indicates the max concurrency of DDL query, "
    "the higher concurrency may lead to the risk of crash.",
    GLOBAL_VAR(rds_dstore_ddl_max_concurrency), CMD_LINE(OPT_ARG),
    VALID_RANGE(1, 64),
#ifdef NDEBUG
    DEFAULT(4),
#else
    /*Some testcases may need higher concurrency. */
    DEFAULT(64),
#endif
    BLOCK_SIZE(1));

static Sys_var_uint Sys_rds_dstore_ddl_throttle_timeout(
    "rds_dstore_ddl_throttle_timeout",
    "The timeout in seconds while waiting for DDL throttle concurrency slot.",
    GLOBAL_VAR(rds_dstore_ddl_throttle_timeout), CMD_LINE(OPT_ARG),
    VALID_RANGE(1, 31536000), DEFAULT(60), BLOCK_SIZE(1));

static Sys_var_bool Sys_rds_dstore_write_ddl_sql_into_redo(
    "rds_dstore_write_ddl_sql_into_redo", "Whether write ddl sql into redo.",
    READ_ONLY GLOBAL_VAR(rds_dstore_write_ddl_sql_into_redo), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(nullptr));

static Sys_var_bool Sys_rds_use_ddl_info_replay(
    "rds_use_ddl_info_replay",
    "Enable replay ddl info for restore and replication process based on "
...
static Sys_var_ulong Sys_rds_physical_rpl_sender_packet_size(
    "rds_physical_rpl_sender_packet_size",
    "Expects the size of a wal data packet sent by the wal_sender thread in "
    "one attempt.",
    GLOBAL_VAR(rds_physical_rpl_sender_packet_size), CMD_LINE(OPT_ARG),
    VALID_RANGE(1, ULONG_MAX), DEFAULT(51200), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG);

static Sys_var_ulong Sys_rds_physical_rpl_sender_wait_times(
    "rds_physical_rpl_sender_wait_times",
    "The number of times to re-read wal logs when the size of "
    "existing wal logs is smaller than rds_physical_rpl_sender_packet_size. ",
    GLOBAL_VAR(rds_physical_rpl_sender_wait_times), CMD_LINE(OPT_ARG),
    VALID_RANGE(0, ULONG_MAX), DEFAULT(500), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG);

static Sys_var_ulong Sys_rds_physical_rpl_sender_sleep_us(
    "rds_physical_rpl_sender_sleep_us",
    "The sleep wait time of the wal_sender thread when the size of "
    "existing wal logs is smaller than rds_physical_rpl_sender_packet_size. ",
    GLOBAL_VAR(rds_physical_rpl_sender_sleep_us), CMD_LINE(OPT_ARG),
    VALID_RANGE(0, ULONG_MAX), DEFAULT(100), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG);
...
/* Only user with SUPER_ACL privilege can modify rds_support_bulk_execution
   and rds_imitate_mariadb_server. As COM_STMT_BULK_EXECUTE is a new command,
   with support in the protocol, for safety reasons we prefer to be conservative
   and require an admin's action to enable it. */
static Sys_var_bool Sys_rds_support_bulk_execution(
    "rds_support_bulk_execution",
    "Support bulk execution for prepared statements",
    SESSION_VAR(rds_support_bulk_execution), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_super_acl),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_imitate_mariadb_server(
    "rds_imitate_mariadb_server",
    "Tell the client that this is mariadb server.",
    GLOBAL_VAR(rds_imitate_mariadb_server), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_super_acl),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_NORMAL);
...
static Sys_var_flagset Sys_pq_support_features_switch(
    ...
    "pq_support_features_switch=option=val[,option=val...], where "
    "option is one of {simple_aggregate, count_distinct} and val is one of "
    "{on, off, default}",
...
static Sys_var_ulong Sys_parallel_batch_max_slot(
    ...
    "max size for m_all_slot in Batch_buffer_manager",
...
static Sys_var_ulong Sys_parallel_batch_max_mem_size(
    "parallel_batch_max_mem_size", "max mem size for Batch_buffer",
    SESSION_VAR(parallel_batch_max_mem_size), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, ULONG_MAX), DEFAULT(1024 * 1024), BLOCK_SIZE(1),
    NO_MUTEX_GUARD, NOT_IN_BINLOG);

static Sys_var_bool Sys_rds_show_hidden_table(
    "rds_show_hidden_table", "Whether to show hidden tables.",
    GLOBAL_VAR(rds_show_hidden_table), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_super_acl),
    ON_UPDATE(nullptr), nullptr, sys_var::PARSE_NORMAL);
...
static Sys_var_uint Sys_partial_result_check_hit_ratio_frequency(
    ...
    BLOCK_SIZE(1));
/**
  Check restricted dynamic privileges list is valid.

  This method checks if the set value contains the invalid dynamic
  privilege names and report errors.
  ...
  @returns Status of validation
    @retval true : If the set variable is null or contains invalid names.
    @retval false : If the set variable is null or contains invalid names.
*/
static bool check_rds_restricted_dynamic_privileges_list(sys_var *self,
                                                         THD *thd,
                                                         set_var *var) {
  if (var->value == nullptr) return false;
  if (var->save_result.string_value.str == nullptr) return true;

  /* Check whether the value is the valid dynamic privileges */
  if (!is_valid_dynamic_privileges_list(thd, var->save_result.string_value.str,
                                        var->save_result.string_value.length)) {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), self->name.str,
...
/**
  This function reload the restricted_dynamic_privileges_set base on the
  system variable rds_restricted_dynamic_privileges_list changes.

  @returns Status of updating
    @retval true : failure
    @retval false : success
*/
static bool update_rds_restricted_dynamic_privileges_list(sys_var *, THD *,
                                                          enum_var_type) {
  return rds_restricted_dynamic_privileges_list_init_and_reload();
}

static Sys_var_charptr Sys_rds_restricted_dynamic_privileges_list(
    "rds_restricted_dynamic_privileges_list",
    "List of restricted dynamic privileges, which allow the users "
    "to break the RDS service.",
    GLOBAL_VAR(rds_restricted_dynamic_privileges_list_ptr), CMD_LINE(OPT_ARG),
    IN_SYSTEM_CHARSET, DEFAULT("SYSTEM_VARIABLES_ADMIN"), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(check_rds_restricted_dynamic_privileges_list),
    ON_UPDATE(update_rds_restricted_dynamic_privileges_list));

static Sys_var_bool Sys_rds_restricted_dynamic_privileges_control(
    "rds_restricted_dynamic_privileges_control",
    "This option indicates whether the RDS Service deny the users "
    "to grant restricted dynamic privileges.",
    GLOBAL_VAR(opt_rds_restricted_dynamic_privileges_control),
    CMD_LINE(OPT_ARG), DEFAULT(true));
...
static Sys_var_bool Sys_rds_processlist_control(
    "rds_process_list_control",
    "This option indicates whether Users (not built-in users or non-super "
    "users) can see the built-in users in information_schema.processlist.",
    GLOBAL_VAR(opt_rds_processlist_control), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEFAULT(0),
    sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_internal_schema_ddl_control(
    "rds_internal_schema_ddl_control",
    "This option indicates whether the RDS Service "
    "ddl access for internal schema control is enabled.",
    GLOBAL_VAR(opt_rds_internal_schema_ddl_control), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_internal_schema_dml_control(
    "rds_internal_schema_dml_control",
    "This option indicates whether the RDS Service "
    "dml access for internal schema control is enabled.",
    GLOBAL_VAR(opt_rds_internal_schema_dml_control), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_allow_empty_users(
    "rds_allow_empty_users",
    "This option indicates whether the creation of empty users "
    "or the modification of other users to empty users should be "
    "allowed.",
    GLOBAL_VAR(opt_rds_allow_empty_users), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEFAULT(0),
    sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_extended_partitions_enabled(
    "rds_extended_partitions_enabled",
    "This option indicates whether supportes more partition types.",
    GLOBAL_VAR(extended_partitions_enabled), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_rds_sqlfilter_control(
    "rds_sqlfilter_control",
    "This option indicates whether the sql filter is enabled.",
    GLOBAL_VAR(opt_rds_sqlfilter_control), CMD_LINE(OPT_ARG), DEFAULT(false));

static Sys_var_bool Sys_rds_sqlfilter_affect_admin_user(
    "rds_sqlfilter_affect_admin_user",
    "This option indicates whether sql filter affects admin user(root).",
    GLOBAL_VAR(rds_sqlfilter_affect_admin_user), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_sqlfilter_case_sensitive(
    "rds_sqlfilter_case_sensitive",
    "This option indicates whether comparisons of sql filter keys are "
    "case-sensitive.",
    GLOBAL_VAR(opt_rds_sqlfilter_case_sensitive), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_rds_sqlfilter_sys_table_control(
    "rds_sqlfilter_sys_table_control",
    "This option indicates whether sql filter control the traffic of system "
    "table.",
    GLOBAL_VAR(rds_sqlfilter_sys_table_control), CMD_LINE(OPT_ARG),
    DEFAULT(false));

static Sys_var_ulong Sys_rds_sqlfilter_rules_max_count(
    "rds_sqlfilter_rules_max_count",
    "The maximum number for sql filter rules that is allowed to be added",
    GLOBAL_VAR(rds_sqlfilter_rules_max_count), CMD_LINE(OPT_ARG),
    VALID_RANGE(1, 1500), DEFAULT(100), BLOCK_SIZE(1));

static bool check_rds_node_id(sys_var *self, THD *, set_var *var) {
  ...
  // The length of rds_node_id should not be greater than 36.
  if (check_string_char_length(to_lex_cstring(var->save_result.string_value),
                               "", MAX_NODE_ID, system_charset_info, true)) {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), self->name.str,
             var->save_result.string_value.str);
    return true;
  }
...
static Sys_var_bool Sys_rds_warn_sql_log_bin(
    "rds_warn_sql_log_bin",
    "Report a warning when turning off 'SQL_LOG_BIN' if the option is TRUE",
    GLOBAL_VAR(opt_warn_sql_log_bin), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEFAULT(0),
    sys_var::PARSE_NORMAL);

static Sys_var_charptr Sys_rds_node_id(
    "rds_node_id", "Uniquely identifies a node which is set by agent.",
    GLOBAL_VAR(node_id_ptr), CMD_LINE(OPT_ARG), IN_SYSTEM_CHARSET, DEFAULT(""),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_rds_node_id));

static Sys_var_bool Sys_rds_load_data_infile_enable(
    "rds_load_data_infile_enable",
    "Control whether the syntax of LOAD DATA INFILE can be used.",
    GLOBAL_VAR(rds_load_data_infile_enable), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEFAULT(0),
    sys_var::PARSE_NORMAL);

static Sys_var_ulong Sys_conn_log_error_verbosity(
    "rds_conn_log_error_verbosity",
    "How detailed the connection log should be."
    "0, disable."
    "1, log abnormal connection setup and abnormal disconnection info."
    "2, log all connection info.",
    GLOBAL_VAR(rds_conn_log_error_verbosity), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, 2), DEFAULT(1), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG);

static Sys_var_bool Sys_rds_maintenance_operation_write_binlog(
    "rds_maintenance_operation_write_binlog",
    "Control maintenance operation written to binlog",
    GLOBAL_VAR(opt_rds_maintenance_operation_write_binlog), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG);

static Sys_var_bool Sys_rds_disable_loadable_function(
    "rds_disable_user_defined_functions",
    "Disable CREATE FUNCTION Statement for Loadable Functions.",
    GLOBAL_VAR(rds_disable_user_defined_functions), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static Sys_var_bool Sys_event_disable_slave(
    "event_disable_slave", "Disable event status in slave side",
    GLOBAL_VAR(Events::opt_event_disable_slave), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEFAULT(0), sys_var::PARSE_NORMAL);

static const char *rds_security_hardening_names[] = {"use_load_file", "default",
                                                     NullS};

static Sys_var_flagset Sys_rds_security_hardening(
    "rds_security_hardening",
    "rds_security_hardening=option=val[,option=val...], where option is one of "
    "{use_load_file} and val is one of {on, off, default}.",
    GLOBAL_VAR(rds_security_hardening), CMD_LINE(OPT_ARG),
    rds_security_hardening_names, DEFAULT(RDS_SECURITY_HARDENING_DEFAULT),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));
...
// less than this number, but we keep an upper limit so we don't risk creating
// a very larg number of HashJoinChunks due to wrong statistics etc. We always
// use a power of two number of files, which allows us to do some
// optimizations when calculating which chunk a row should be placed in.
//
// Note that all temporary files are managed through the VfdManager, meaning
// that the number of open files will be a lot lower than this number
// (vdf_max_open_files currently defaults to 32). So this number basically
// means "how many VfdFiles/HashJoinChunks can I allocate per input".
//
// Creating too many temporary files can trigger dentry accumulation on the
// physical machine, leading to system lag. Therefore, we have modified the
// variable kMaxChunks to be configurable. Originally, the value of kMaxChunks
// was a constant.
static Sys_var_ulong Sys_kMaxChunks(
    "kMaxChunks",
    "The maximum number of HashJoinChunks that is allocated for each of the "
    "inputs in case we spill to disk.",
    GLOBAL_VAR(kMaxChunks), CMD_LINE(REQUIRED_ARG), VALID_RANGE(32, 8192),
    DEFAULT(8192), BLOCK_SIZE(1));

static Sys_var_bool Sys_pq_master_enable(
    "pq_master_enable", "whether to enable master's parallel execution",
    GLOBAL_VAR(pq_master_enable), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG);
static Sys_var_bool Sys_loose_partition_restriction(
    "rds_loose_partition_restriction",
    "This option indicates whether loose partition table restriction.",
    GLOBAL_VAR(loose_partition_restriction), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_allow_skip_locked_now_wait(
    "rds_allow_skip_locked_now_wait",
    "This option indicates whether we enable keywords NOWAIT and SKIP LOCKED in"
    "select xxx for update syntax",
    GLOBAL_VAR(rds_allow_skip_locked_now_wait), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static bool check_scc_conf(sys_var *self [[maybe_unused]],
                           THD *thd [[maybe_unused]], set_var *var) {
  if (var->save_result.string_value.str == nullptr ||
      strlen(var->save_result.string_value.str) == 0) {
    return false;
  }
  if (FILE *file = fopen(var->save_result.string_value.str, "r")) {
    fclose(file);
  } else {
    char errbuf[MYSYS_STRERROR_SIZE];
    my_error(ER_CANT_OPEN_FILE, MYF(0), var->save_result.string_value.str,
  ...
  // retry can be done by executing SET GLOBAL, built-in retry not necessary
  if (scc_init_with_conf_retry(var->save_result.string_value.str,
                               SCC_API_NO_RETRY)) {
    my_error(ER_DA_RDS_SCC_INIT, MYF(0));
    return true;
  }
  sql_print_information("Scc init success.");
  rds_scc_initialized = true;
  return false;
}

static Sys_var_charptr Sys_rds_scc_conf("rds_scc_conf", "SCC config file path",
                                        GLOBAL_VAR(rds_scc_conf),
                                        CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
                                        DEFAULT(0), NO_MUTEX_GUARD,
                                        NOT_IN_BINLOG, ON_CHECK(check_scc_conf),
                                        ON_UPDATE(0), DEFAULT(0),
                                        sys_var::PARSE_NORMAL);
static Sys_var_ulong Sys_outline_partitions(
    "rds_outline_partitions", "How many parititon of system outline structure.",
...
static Sys_var_bool Sys_opt_outline_enabled(
    ...
    GLOBAL_VAR(im::opt_outline_enabled), CMD_LINE(OPT_ARG), DEFAULT(false),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));
...
static Sys_var_bool Sys_rds_view_hint_enabled(
    "rds_hint_work_for_view",
    "Controls whether hints work for view. "
    "If this parameter is set to ON, index hint added to view "
    "drived from single table manually or using statement outline "
    "can be effective. Neither index hint added to view drived "
    "from multiple tables nor optimizer hint added to view works.",
    GLOBAL_VAR(hint_work_for_view), CMD_LINE(OPT_ARG), DEFAULT(true),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

static Sys_var_bool Sys_rds_prefer_covering_index(
    "rds_prefer_covering_index_over_pk",
    "If true, allow preference of covering index over primary key when PK is "
    "not clustered and indexes have the same cost for EQ_REF access",
    SESSION_VAR(rds_prefer_covering_index_over_pk), CMD_LINE(OPT_ARG),
    DEFAULT(true), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0));

/* FFIC: dynamic switch for extended crash diagnostics.
   The backing variable is defined in sql/mysqld.cc.
   Users can toggle without restarting:
     SET GLOBAL rds_ffic_verbose_crash_diagnostics = ON;
   Or set at startup:
     mysqld --rds-ffic-verbose-crash-diagnostics */
static Sys_var_bool Sys_rds_ffic_verbose_crash_diagnostics(
    ...
    "around the faulting PC, system-wide memory stats, process resource "
    "limits, "
    "system load averages, loaded segment addresses for offline symbol "
    "resolution, and OS version. Toggle at runtime with SET GLOBAL. Default "
    "ON.",

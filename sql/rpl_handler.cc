...
Wallog_trx_delegate *wallog_trx_delegate;
Server_state_delegate *server_state_delegate;

Binlog_transmit_delegate *binlog_transmit_delegate;
Wallog_transmit_delegate *wallog_transmit_delegate;
Binlog_relay_IO_delegate *binlog_relay_io_delegate;
Wallog_IO_delegate *wallog_io_delegate;
...
int delegates_init() {
  ...
  alignas(Wallog_trx_delegate) static char
      place_wal_trx_mem[sizeof(Wallog_trx_delegate)];
  ...
  alignas(Wallog_transmit_delegate) static char
      place_wallog_transmit_mem[sizeof(Wallog_transmit_delegate)];
  alignas(Binlog_relay_IO_delegate) static char
      place_relay_io_mem[sizeof(Binlog_relay_IO_delegate)];
  alignas(Wallog_IO_delegate) static char
      place_wal_io_mem[sizeof(Wallog_IO_delegate)];
  ...
  wallog_trx_delegate = new (place_wal_trx_mem) Wallog_trx_delegate;
  if (!wallog_trx_delegate->is_inited()) {
    LogErr(ERROR_LEVEL, ER_WAL_RPL_WALLOG_TRX_DELEGATES_INIT_FAILED);
    return 1;
  }
  ...
  wallog_transmit_delegate =
      new (place_wallog_transmit_mem) Wallog_transmit_delegate;
  if (!wallog_transmit_delegate->is_inited()) {
    LogErr(ERROR_LEVEL, ER_WAL_RPL_WALLOG_TRANSMIT_DELEGATES_INIT_FAILED);
    return 1;
  }
  ...
  wallog_io_delegate = new (place_wal_io_mem) Wallog_IO_delegate;
  if (!wallog_io_delegate->is_inited()) {
    LogErr(ERROR_LEVEL, ER_WAL_RPL_WALLOG_IO_DELEGATES_INIT_FAILED);
    return 1;
  }
...
void delegates_destroy() {
  ...
  if (binlog_storage_delegate)
    binlog_storage_delegate->~Binlog_storage_delegate();
  if (wallog_trx_delegate) wallog_trx_delegate->~Wallog_trx_delegate();
  if (server_state_delegate) server_state_delegate->~Server_state_delegate();
  if (binlog_transmit_delegate)
    binlog_transmit_delegate->~Binlog_transmit_delegate();
  if (wallog_transmit_delegate)
    wallog_transmit_delegate->~Wallog_transmit_delegate();
  if (binlog_relay_io_delegate)
    binlog_relay_io_delegate->~Binlog_relay_IO_delegate();
  if (wallog_io_delegate) wallog_io_delegate->~Wallog_IO_delegate();
}
...
static void delegates_update_plugin_ref_count() {
  ...
  if (wallog_trx_delegate) wallog_trx_delegate->update_plugin_ref_count();
  if (server_state_delegate) server_state_delegate->update_plugin_ref_count();
  if (binlog_transmit_delegate)
    binlog_transmit_delegate->update_plugin_ref_count();
  if (wallog_transmit_delegate)
    wallog_transmit_delegate->update_plugin_ref_count();
  if (binlog_relay_io_delegate)
    binlog_relay_io_delegate->update_plugin_ref_count();
  if (wallog_io_delegate) wallog_io_delegate->update_plugin_ref_count();
}
...
void delegates_update_lock_type() {
  ...
  if (wallog_trx_delegate) wallog_trx_delegate->update_lock_type();
  ...
  if (wallog_transmit_delegate) wallog_transmit_delegate->update_lock_type();
  if (binlog_relay_io_delegate) binlog_relay_io_delegate->update_lock_type();
  if (wallog_io_delegate) wallog_io_delegate->update_lock_type();
}
...
void Wallog_IO_delegate::init_param(Wallog_IO_param *param, Master_info *mi) {
  param->mysql = mi->mysql;
  param->channel_name = mi->get_channel();
  param->user = const_cast<char *>(mi->get_user());
  param->host = mi->host;
  param->port = mi->port;
  param->received_lsn = 0;
  param->flushed_lsn = 0;
  param->replayed_lsn = 0;
  param->server_id = 0;
  param->thread_id = -1;
  param->source_connection_auto_failover =
      mi->is_source_connection_auto_failover();
...
int Wallog_IO_delegate::wal_thread_start(THD *thd, Master_info *mi) {
  Wallog_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
  param.thread_id = thd->thread_id();

  int ret = 0;
  FOREACH_OBSERVER(ret, wal_thread_start, (&param));
  return ret;
}

int Wallog_IO_delegate::wal_thread_stop(THD *thd, Master_info *mi) {
  Wallog_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
  param.thread_id = thd->thread_id();
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, wal_thread_stop, (&param));
  return ret;
}

int Wallog_IO_delegate::before_request_wal_transmit(THD *thd, Master_info *mi,
                                                    ushort flags) {
  Wallog_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, before_request_wal_transmit, (&param, (uint32)flags));
  return ret;
}

int Wallog_IO_delegate::after_read_wal(THD *thd, Master_info *mi,
                                       const char *packet, ulong len,
                                       const char **wal_buf, ulong *wal_len) {
  Wallog_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
  param.thread_id = thd->thread_id();
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, after_read_wal,
                   (&param, packet, len, wal_buf, wal_len));
  return ret;
}

int Wallog_IO_delegate::after_write_wal(THD *thd, Master_info *mi,
                                        const char *wal_buf, ulong wal_len,
  ...
  Wallog_IO_param param;
  init_param(&param, mi);
  param.received_lsn = received_lsn;
  param.flushed_lsn = flushed_lsn;
  param.replayed_lsn = replayed_lsn;
  param.server_id = thd->server_id;
  param.thread_id = thd->thread_id();
  uint32 flags = 0;

  int ret = 0;
  FOREACH_OBSERVER(ret, after_write_wal, (&param, wal_buf, wal_len, flags));
  return ret;
}

int Wallog_IO_delegate::after_reset_wal_slave(THD *thd [[maybe_unused]],
                                              Master_info *mi
                                              [[maybe_unused]]) {
  // TODO: implement
  return 0;
...
int Wallog_IO_delegate::applier_wal(THD *thd [[maybe_unused]],
...
int register_wallog_io_observer(Wallog_IO_observer *observer, void *p) {
  return wallog_io_delegate->add_observer(observer, (st_plugin_int *)p);
}

int unregister_wallog_io_observer(Wallog_IO_observer *observer, void *) {
  return wallog_io_delegate->remove_observer(observer);
}
...
int Wallog_trx_delegate::trx_wait(uint64_t lsn) {
  DBUG_TRACE;
  DBUG_PRINT("enter", ("wallog_lsn: %llu", (ulonglong)lsn));
  int ret = 0;
  FOREACH_OBSERVER(ret, trx_wait, (lsn));
  return ret;
}

int Wallog_trx_delegate::get_standby_flushed_lsn(
    uint64_t *standby_flushed_lsn) {
  int ret = 0;
  FOREACH_OBSERVER(ret, get_standby_flushed_lsn, (standby_flushed_lsn));
  return ret;
}

int Wallog_transmit_delegate::wallog_transmit_start(
    THD *thd, ushort flags, uint64 log_pos, bool *observe_transmission) {
  Wallog_transmit_param param;
  param.flags = flags;
  param.server_id = thd->server_id;

  int ret = 0;
  FOREACH_OBSERVER(ret, wallog_transmit_start, (&param, log_pos));
  *observe_transmission = param.should_observe();
  return ret;
}

int Wallog_transmit_delegate::wallog_transmit_stop(THD *thd, ushort flags) {
  Wallog_transmit_param param;
  param.flags = flags;
  param.server_id = thd->server_id;

  DBUG_EXECUTE_IF("crash_wallog_transmit_hook", DBUG_SUICIDE(););

  int ret = 0;
  FOREACH_OBSERVER(ret, wallog_transmit_stop, (&param));
  return ret;
}

int Wallog_transmit_delegate::wallog_reserve_header(THD *thd, ushort flags,
                                                    String *packet) {
  /*
    NOTE2ME: Maximum extra header size for each observer, I hope 32
    bytes should be enough for each Observer to reserve their
  ...
  for (; info; info = iter++) {
    ...
    {  // `unlock_guard` scope
      ...
      if (((Observer *)info->observer)->wallog_reserve_header &&
          ((Observer *)info->observer)
              ->wallog_reserve_header(&param, header, RESERVE_HEADER_SIZE,
                                      &hlen)) {
        ret = 1;
        break;
      }
...
int Wallog_transmit_delegate::wallog_before_send_event(THD *thd, ushort flags,
                                                       String *packet,
                                                       uint64 log_pos) {
  Wallog_transmit_param param;
  param.flags = flags;
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, wallog_before_send_event,
                   (&param, pointer_cast<uchar *>(packet->ptr()),
                    packet->length(), log_pos));
  return ret;
}

int Wallog_transmit_delegate::wallog_after_send_event(THD *thd, ushort flags,
                                                      String *packet) {
  Wallog_transmit_param param;
  param.flags = flags;
  param.server_id = thd->server_id;
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, wallog_after_send_event, (&param, packet->ptr()));
  return ret;
}

int Wallog_transmit_delegate::wallog_after_reset_master(THD *thd,
                                                        ushort flags) {
  Wallog_transmit_param param;
  param.flags = flags;
  param.server_id = thd->server_id;
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, wallog_after_reset_master, (&param));
  return ret;
}

int register_wallog_trx_observer(Wallog_trx_observer *observer, void *p) {
  DBUG_TRACE;
  int result = wallog_trx_delegate->add_observer(observer, (st_plugin_int *)p);
  return result;
}

int unregister_wallog_trx_observer(Wallog_trx_observer *observer, void *) {
  return wallog_trx_delegate->remove_observer(observer);
}

int register_wallog_transmit_observer(Wallog_transmit_observer *observer,
                                      void *p) {
  return wallog_transmit_delegate->add_observer(observer, (st_plugin_int *)p);
}

int unregister_wallog_transmit_observer(Wallog_transmit_observer *observer,
                                        void *) {
  return wallog_transmit_delegate->remove_observer(observer);
}

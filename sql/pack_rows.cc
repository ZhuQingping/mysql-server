...
void TableCollection::AddTable(Table *tab) {
  ...
  if (table->has_storage_handler()) {
    // Partial result cache adds a virtual LRU table to the collection, with
...
void TableCollection::print(String &str_cols) {
  ...
  // m_tables is a set of pointers, for repeatability of the printouts in mtr
  // tests, we sort by name:

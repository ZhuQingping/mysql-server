...
/**
  to control maintenance operation:ANALYZE/OPTIMIZE/REPAIR write binlog or not.

  we do not to write binlog in read_only mode but standby node could write in
  dstore.

  @param thd The thread handler.

  @retval true to write binlog.
  @retval false not to write binlog.
*/
static bool is_maintenance_operation_write_to_binlog(THD *thd) {
  if (get_instance_storage_engine_type() == DB_TYPE_DSTORE) {
    return !thd->lex->no_write_to_binlog &&
           opt_rds_maintenance_operation_write_binlog &&
           (!read_only || (thd->system_thread == SYSTEM_THREAD_SLAVE_SQL ||
...
static bool mysql_admin_table(
  ...
  */
  ...
  /* RAII guard to restore THD::query_id, which may be incremented during
   * multi-table OPTIMIZE. */
  struct QueryIdGuard {
    THD *thd_ref;
    query_id_t original;
    ~QueryIdGuard() { thd_ref->query_id = original; }
  } query_id_guard{thd, thd->query_id};

  for (table = tables; table; table = table->next_local) {
    { /** scope for data dictionary auto releaser */
      ...
      /*
        For OPTIMIZE with multiple tables, increment query_id after optimizing
        each table. Since each table is recreated in a separate transaction and
        XID_STATE::m_xid is derived from THD::query_id, each table's query_id
        must be unique to ensure XID-based DDL crash recovery works correctly.
      */
      if (thd->lex->sql_command == SQLCOM_OPTIMIZE && table != tables) {
        thd->set_query_id(next_query_id());
        /* Set THD::is_ddl_info_written to false to force writing the DDL info.
          We do not need to set the DDL SQL explicitly, even if
          rds_dstore_write_ddl_sql_into_redo is ON, because dstore will perform
          a recreate operation for each table and set the generated DDL SQL
          in ha_cde::create. */
        thd->reset_ddl_info();
      }
        ...
        */
        ...
        if (!thd->locked_tables_mode && repair_table_use_frm) {
          ...
          */
          ...
          if (result_code == HA_ADMIN_FAILED) {
            /*
              In the abnormal branch processing, the temporary da objects
              added to the THD need to be cleared.
            */
            goto send_result;
          }
        } else {
        ...
        if (table->table) {
          ...
          */
          if (alter_info->flags & Alter_info::ALTER_ADMIN_PARTITION) {
            ...
            if (!validate_partition_type(table->table->part_info)) {
      ...
      */
      ...
      if ((!read_only || thd->is_cmd_skip_readonly()) &&
          ignore_grl_on_analyze) {
        // Acquire the lock
    ...
    send_result_message:
      ...
      switch (result_code) {
        ...
        case HA_ADMIN_TRY_ALTER: {
          ...
          save_flags = alter_info->flags;
          {
            ...
            */
            /*
              ha_cde::optimize will also answer HA_ADMIN_TRY_ALTER, so we should
              not write DDL info at this point.
            */
          ...
          */
          ...
          if (operator_func == &handler::ha_analyze)
            /*
              ...
              as needing reopening. Any subsequent statement that uses this
              table will have to call handler::open() which will cause this
              information to be updated. OTOH, such subsequent statements won't
              have to wait for already running statements to go away since we do
              not invalidate TABLE_SHARE.
            */
      ...
      close_thread_tables(thd);
    } /** end of scope for data dictionary auto releaser */
    /** releasing data dictionary must be done before
     *  releasing MDL tickets.
     */
...
bool Sql_cmd_analyze_table::handle_histogram_command(THD *thd,
  ...
  if (table->next_local != nullptr) {
    ...
    */
    ...
    // Due to the current lack of complete index statistics in dstore, the
    // analyze command is currently allowed only in read-only mode.
    if ((read_only && !thd->is_cmd_skip_readonly()) || thd->tx_read_only) {
      ...
      */
      ...
      if (!res) {
        /*
           If a histogram was updated.
           We request TABLE_SHARE to go away from the only secondary engine
...
bool Sql_cmd_analyze_table::execute(THD *thd) {
  ...
  if (!res && is_maintenance_operation_write_to_binlog(thd)) {
    /*

...
bool mysql_create_db(THD *thd, const char *db, HA_CREATE_INFO *create_info) {
  DBUG_TRACE;
  /* Instance default storage engine can only be InnoDB or Dstore */
  assert(get_instance_storage_engine_type() == DB_TYPE_INNODB ||
  ...
  if (thd->is_dd_system_thread() &&
    ...
    */
    ...
    if (schema_dir_exists) {
      ...
      if (schema_dirp != nullptr && schema_dirp->number_off_files <= 2) {
        // Directory is empty: delete it and continue
        if (rm_dir_w_symlink(path, true)) {
          my_error(ER_SCHEMA_DIR_EXISTS, MYF(0), path);
          return true;
        }
        // Continue to create the directory below
        schema_dir_exists = false;
        set_my_errno(ENOENT);
      } else {
        // Directory is not empty or cannot be accessed: return error
        my_error(ER_SCHEMA_DIR_EXISTS, MYF(0), path);
        return true;
      }
    }
...
bool mysql_rm_db(THD *thd, const LEX_CSTRING &db, bool if_exists) {
  ...
  MY_DIR *schema_dirp = my_dir(path, MYF(MY_DONT_SORT));
  // Custom deleter lambda for MY_DIR cleanup
  auto dirender = [](MY_DIR *dirp) { my_dirend(dirp); };
  // RAII wrapper for directory resource management
  std::unique_ptr<MY_DIR, decltype(dirender)> grd{schema_dirp, dirender};
  ...
  if (schema == nullptr)  // Schema not found in DD
  {
    ...
    {
      // Check if directory is empty (only contains . and ..)
      if (schema_dirp->number_off_files <= 2 &&
          DBUG_EVALUATE_IF("pretend_no_schema_in_drop_schema", false, true)) {
        // Directory is empty: delete directory and return success
        ...
        // Return success (deleted 0 tables)
        my_ok(thd, 0);
        return false;
      } else {
        // Directory is not empty: return error
        my_error(ER_SCHEMA_DIR_UNKNOWN, MYF(0), db.str, path);
        return true;
      }
    }
  ...
  } else  // Schema found in DD
  {
    ...
    /* When dropping a database that contains no tables, generate the
     * corresponding SQL with DDL info. */
    else if (need_ddl_info_sql(thd)) {
      generate_dstore_drop_ddl_info(thd);
    }

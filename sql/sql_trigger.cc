...
static bool finalize_trigger_ddl(THD *thd, const char *db_name, TABLE *table,
  ...
  */
  ...
  if (thd->set_ddl_sql(stmt_query.lex_cstring())) {
    LogErr(ERROR_LEVEL, ER_CDE_DEFAULT_LOG_ERROR,
           "Failed to set DDL info during TRIGGER DDL finalize.");
    return true;
  }
...
bool Sql_cmd_create_trigger::execute(THD *thd) {
  ...
  /*
     If opt_rds_permission_control is on,we allow Super users and reserved users
     to create triggers on tables in the 'mysql' schema.
   */
  if (opt_rds_permission_control) {
    if ((check_global_access_noerr(thd, SUPER_ACL) != 0) &&
      ...
      if (!my_strcasecmp(system_charset_info, "mysql", m_trigger_table->db)) {
        my_error(ER_NO_TRIGGERS_ON_SYSTEM_SCHEMA, MYF(0));  // NOLINT
        return true;
      }
    }
  } else {
...
bool Sql_cmd_drop_trigger::execute(THD *thd) {
  ...
  /*
     If opt_rds_permission_control is on, we allow Super users and reserved
     users to drop triggers on tables in the 'mysql' schema.
   */
  if (opt_rds_permission_control) {
    if ((check_global_access_noerr(thd, SUPER_ACL) != 0) &&
        !is_rds_reserved_user(thd->security_context()->user().str)) {
      // NOLINTNEXTLINE
      if (!my_strcasecmp(system_charset_info, "mysql", tables->db)) {
        my_error(ER_SPECIFIC_ACCESS_DENIED_ERROR, MYF(0), "SUPER");  // NOLINT
        return true;
      }
    }
  }

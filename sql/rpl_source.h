...
/**
  Process a COM_WAL_DUMP packet.

  This function parses the packet and then calls mysql_wal_send.

  @param thd The dump thread.
  @param packet The COM_WAL_DUMP packet.
  @param packet_length The length of the packet in bytes.
  @retval true Error
  @retval false Success
*/
bool com_wal_dump(THD *thd, char *packet, size_t packet_length);

/**
  Low-level function where the dump thread iterates over the wal
  log and sends wal data to the slave.  This function is common for
  COM_WAL_DUMP.

  @param thd The dump thread.

  @param start_lsn The start lsn in the wal.

  @param flags flags in COM_WAL_DUMP packets.

  @param wal_stream_id Standby wal stream id.

  @note This function will start reading at the given postion(start_lsn) and
  send wal data to the slave.
*/

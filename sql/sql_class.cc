...
THD::THD(bool enable_plugins)
  ...
  */
  ...
  m_vfd_manager = std::make_unique<VfdManager>(vfd_max_open_files);
  recycle_state = std::make_unique<im::recycle_bin::Recycle_state>();
}
...
void THD::cleanup_connection(void) {
  ...
  if (check_cleanup) {
    ...
    /* check prepared stmts cached are cleaned up */
    assert(plan_cache::cached_plan_count.load(std::memory_order_relaxed) == 0);
...
void THD::cleanup(void) {
  ...
  */
  ...
  DBUG_ASSERT(!backup_binlog_lock.is_protection_acquired());
  ...
  */
  ...
  /* Protects user_vars. */
  mysql_mutex_lock(&LOCK_thd_data);
  if (is_pq_leader()) user_vars.clear();
  mysql_mutex_unlock(&LOCK_thd_data);
...
void THD::awake(THD::killed_state state_to_set) {
  ...
  */
  ...
  if (state_to_set != THD::KILL_QUERY && state_to_set != THD::KILL_TIMEOUT) {
    ...
    if (!slave_thread)
      MYSQL_CALLBACK(this->conn_event_functions, post_kill_notification,
                     (this));
  }
...
void THD::cleanup_after_query(bool clean_pq_variables) {
  /*
  ...
  */
  ...
  // the next optimization if necessary.
  m_vfd_manager->SetNeedsMutex(/*needs_mutex=*/false);

  // We clean up the PTRCs at the end of the execution of each statement,
  // except if this is a sub-statement of a trigger or stored function. In the
  // latter case, we cannot, because there is only one MEM_ROOT for all PTRCs
  // used by the sub-statement and the parent query as their executions are
  // interlaced (consider "SELECT stored_func() FROM etc", and stored_func()
  // contains a SELECT too). If this is rather a stored procedure, we can
  // clean up the PTRCs at the end of each statement of the procedure.
...
void THD::change_item_tree(Item **place, Item *new_value,
  ...
  if (!is_void_pointer)
    new_value->origin_item = (*place) != new_value ? (*place) : nullptr;
  *place = new_value;
}
...
Prepared_statement *Prepared_statement_map::find(ulong id) {
  /*
    If EXECUTE/BULK_EXECUTE is invoked right after PREARE in the session,
    MariaDB Connector/J will pass 0xFFFFFFFF as statement index and server will
    use last prepare statement directly. To adapt MariaDB Connector/J, do the
    same here if rds_imitate_mariadb_server is enabled.
  */
  if (rds_imitate_mariadb_server && IS_LAST_STMT_ID(id)) {
...
void THD::set_query(LEX_CSTRING query_arg) {
  ...
  /*
  During stored procedure execution, reset_query() is not called after a query
  is executed. So, we need to reset the DDL info here if it exists.
  */
  reset_ddl_info();
  mysql_mutex_lock(&LOCK_thd_query);
...
bool THD::validate_ddl_info_size(LEX_CSTRING query_arg) {
  if (!query_arg.length || query_arg.length > max_ddl_info_len()) {
    my_error(ER_DSTORE_DDL_INFO_SIZE_INVALID, MYF(0), query_arg.length,
             max_ddl_info_len());
    return true;
  }
  return false;
...
bool THD::set_ddl_sql(LEX_CSTRING query_arg, bool skip_prefix) {
  bool err = false;
  /* If DDL info switch is off, return success early. This avoids adding
   redundant switch checks at numerous call sites throughout the code. */
  if (!rds_dstore_write_ddl_sql_into_redo) {
    return false;
  }
  /* Make sure query_arg is not zero. */
  if (validate_ddl_info_size(query_arg)) {
    return true;
  }
  ...
  enum_sql_command sql_com = lex->sql_command;
  bool suppress_use =
      (sql_com == SQLCOM_CREATE_DB || sql_com == SQLCOM_ALTER_DB ||
       sql_com == SQLCOM_DROP_DB || skip_prefix);
  if (m_db.str && !suppress_use) {
    err = gen_query.append("USE ") || gen_query.append(m_db.str) ||
          gen_query.append("; ");
  }
  if (!err) {
    err = gen_query.append(query_arg.str);
  }
  if (err) {
    gen_query.length(0);
  }

  /* Make sure query_arg is less than limited size. */
  if (validate_ddl_info_size(gen_query.lex_cstring())) {
    return true;
  }

  swap_ddl_sql(gen_query);
  return false;
}
...
void THD::leave_locked_tables_mode() {
  if (locked_tables_mode == LTM_LOCK_TABLES) {
    ...
    */
    ...
    /* Make sure we don't release the global backup lock. */
...
void THD::swap_ddl_sql(String &query_arg) {
  assert(this == current_thd);
  assert(query_arg.length() <= max_ddl_info_len());

  mysql_mutex_lock(&LOCK_thd_query);
  m_query_with_ddl_sql.swap(query_arg);
  (void)m_query_with_ddl_sql.c_ptr_safe();
  mysql_mutex_unlock(&LOCK_thd_query);
}

void THD::set_gtid_info(const unsigned char *uuid, int64_t gno) {
  assert(this == current_thd);
  mysql_mutex_lock(&LOCK_thd_query);
  m_gno = gno;
  my_memcpy(m_uuid, UUID_LEN, uuid, UUID_LEN);
  mysql_mutex_unlock(&LOCK_thd_query);
}
...
void my_ok(THD *thd, ulonglong affected_rows, ulonglong id,
  ...
  da->set_ok_status(affected_rows, id, message);
  if (da->accumulate_counts()) {
    /* According to MySQL's documentation, ROW_COUNT's result should be
       similar to that of mysql_affected_rows(). So we correct it: */
    thd->set_row_count_func(da->affected_rows());
  }
}

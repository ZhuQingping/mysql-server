...
PSI_stage_info MDL_key::m_namespace_to_wait_state_name[NAMESPACE_END] = {
    ...
    {0, "Waiting for binlog lock", 0, PSI_DOCUMENT_ME}};
...
class MDL_lock {
  ...
  */
  struct alignas(CPU_LEVEL1_DCACHE_LINESIZE) MDL_lock_strategy {
    /**
...
MDL_ticket *MDL_context::find_ticket(MDL_request *mdl_request,
  ...
  *result_duration = h.m_dur;
  ...
  if (!h.m_ticket) {
    auto thd = get_thd();
    // during XA recovery, get_thd() maybe NULL
    if (thd && thd != invalid_thd && thd->is_pq_worker()) {
      // If PQ worker tries to get a ticket, it may wait on its leader if the
      // leader has done LOCK TABLES WRITE ; that would cause a deadlock.
      ...
      if (t) {
        ...
        // and instead makes a personal copy. A benefit is that enables MDL
        // deadlock detection. So we force the calling function to clone the
        // ticket:
        *result_duration = MDL_DURATION_END;
        return t;
      }
    }
  }
...
void MDL_lock::object_lock_notify_conflicting_locks(MDL_context *ctx,
  ...
  /*
    Add MDL_context::m_needs_thr_lock_abort condition to protect from
    unaccessable THD. Normally we can't see a MDL_wait::VICTIM and a
    disposed THD in m_granted list, but we met one. So we add a
    protection here to avoid dereference of deleted THD pointer. This
    condition will be called in notify_shared_lock function too. Then
    we move the original THD pointer condition into the while loop.
  */
  while ((conflicting_ticket = it++)) {
    if (conflicting_ticket->get_ctx() != ctx &&
        (conflicting_ticket->get_type() == MDL_SHARED ||
         conflicting_ticket->get_type() == MDL_SHARED_HIGH_PRIO) &&
        conflicting_ticket->get_ctx()->get_needs_thr_lock_abort() == true) {
      MDL_context *conflicting_ctx = conflicting_ticket->get_ctx();
      if (conflicting_ctx->get_owner()->get_thd() == nullptr) continue;
...
void MDL_ticket_store::remove(enum_mdl_duration dur, MDL_ticket *ticket) {
  ...
  if (unlikely(m_map == nullptr ||
               DBUG_EVALUATE_IF("simulate_mdl_ticket_store_remove_m_map_null",
                                true, false))) {
    DBUG_TRACE;
    DBUG_PRINT("info",
               ("MDL_ticket_store m_map=nullptr m_count: %lu", m_count));
    DBUG_EXECUTE_IF("simulate_mdl_ticket_store_remove_m_map_null",
                    { DBUG_SUICIDE(); });
    return;
  }

...
void Ha_trx_info_list::move_dstore_to_front() {
  if (m_underlying->ht()->db_type == DB_TYPE_DSTORE) {
    return;
  }

  Ha_trx_info *prev = m_underlying;
  Ha_trx_info *curr = m_underlying->m_next;
  while (curr != nullptr) {
    if (curr->ht()->db_type == DB_TYPE_DSTORE) {
      prev->m_next = curr->m_next;
      curr->m_next = m_underlying;
      m_underlying = curr;
      return;
    }
    prev = curr;

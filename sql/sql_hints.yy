...
%token NO_OFFSET_PUSHDOWN_HINT 1060
%token OFFSET_PUSHDOWN_HINT 1061
/*
...
qb_level_hint:
          OFFSET_PUSHDOWN_HINT '(' opt_qb_name')'
          {
            $$= NEW_PTN PT_qb_level_hint($3, true, OFFSET_PUSHDOWN_HINT_ENUM, 0);
            if ($$ == NULL)
              YYABORT; // OOM
          }
          |
          NO_OFFSET_PUSHDOWN_HINT '(' opt_qb_name')'
          {
            $$= NEW_PTN PT_qb_level_hint($3, false, OFFSET_PUSHDOWN_HINT_ENUM, 0);
            if ($$ == NULL)
              YYABORT; // OOM
          }
          |
...
pq_hint:
          PQ_HINT '(' opt_hint_param_table_list ')'
          {
            $$= NEW_PTN PT_table_level_hint(NULL_CSTR, $3, true, PQ_HINT_ENUM, 0);
            if ($$ == NULL)
              YYABORT; // OOM
          }
        | PQ_HINT
          '(' opt_hint_param_table_list HINT_ARG_NUMBER ')'
          {
            longlong n;
            if (parse_int(&n, $4.str, $4.length) || n > UINT_MAX32 || n <= 0)
            {
              scanner->syntax_warning("Incorrect number for degree of parallel");
              $$= NULL;
            } else {
              $$= NEW_PTN PT_table_level_hint(NULL_CSTR, $3, true, PQ_HINT_ENUM, n);
              if ($$ == NULL)
                YYABORT; // OOM
            }
          }
        | PQ_HINT
          '(' HINT_ARG_QB_NAME opt_hint_param_table_list_empty_qb ')'
          {
            $$= NEW_PTN PT_table_level_hint($3, $4, true, PQ_HINT_ENUM, 0);
            if ($$ == NULL)
              YYABORT; // OOM
          }
        | PQ_HINT
          '(' HINT_ARG_QB_NAME opt_hint_param_table_list_empty_qb HINT_ARG_NUMBER ')'
          {
            longlong n;
            if (parse_int(&n, $5.str, $5.length) || n > UINT_MAX32 || n <= 0)
            {
              scanner->syntax_warning("Incorrect number for degree of parallel");
              $$= NULL;
            } else {
              $$= NEW_PTN PT_table_level_hint($3, $4, true, PQ_HINT_ENUM, n);
              if ($$ == NULL)
                YYABORT; // OOM
            }
          }

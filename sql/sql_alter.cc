...
/**
 * Checks if the ALTER TABLE operation attempts to add an AUTO_INCREMENT column.
 *
 * @param thd The current thread handler (contains connection context)
 * @param alter_info Structure containing information about the ALTER operation
 * @return true if attempting to add an AUTO_INCREMENT column, false otherwise
 *
 */
bool Sql_cmd_alter_table::check_auto_increment_in_alter(
    THD *thd, const Alter_info &alter_info) {
  if (((alter_info.flags & Alter_info::ALTER_ADD_COLUMN) ==
       Alter_info::ALTER_ADD_COLUMN) ||
      ((alter_info.flags & Alter_info::ALTER_CHANGE_COLUMN) ==
       Alter_info::ALTER_CHANGE_COLUMN)) {
    for (const auto &create_field : alter_info.create_list) {
      if ((create_field.auto_flags & Field::NEXT_NUMBER) != 0) {
        push_warning_printf(thd, Sql_condition::SL_WARNING,
                            ER_ALTER_TABLE_ADD_AUTO_INCREMENT,
                            ER_THD(thd, ER_ALTER_TABLE_ADD_AUTO_INCREMENT), "");
        sql_print_warning(
            "If the table contains data, adding an auto-increment column may "
            "lead to inconsistencies between primary and replica node.");
        return true;
      }
    }
  }
...
bool Sql_cmd_alter_table::execute(THD *thd) {
  ...
  */
  ...
  if (rds_warn_alter_table_add_auto_incre_column &&
      storage_engine_mode == ONLY_DSTORE) {
    check_auto_increment_in_alter(thd, alter_info);
  }

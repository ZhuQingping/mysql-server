...
static const char *json_extra_tags[ET_total] = {
    ...
    "offset pushdown",                     // ET_USING_OFFSET_PUSHDOWN
    nullptr,                               // ET_USING
    ...
    "IN-list converted",                   // ET_IN_LIST_CONVERTED
    "parallel_execute",                    // ET_PARALLEL_EXE
    "aggregate"                            // ET_USING_AGGREGATE
};
...
static const char K_PARALLEL_QUERY_GATHER[] = "parallel_execute";
static const char K_PARALLEL_QUERY_DOP[] = "parallel_degree";
static const char K_PARALLEL_QUERY_CUT_TABLE[] = "cut_table";
static const char K_PARALLEL_QUERY_DIVIDED_TABLE[] = "divided_table";
...
class context : public Explain_context {
  ...
  */
  ...
  /**
    Set Parallel Query node to @c ctx

    @retval false               Ok
    @retval true                Error
  */
  virtual void set_pq(pq_ctx *ctx MY_ATTRIBUTE((unused))) { DBUG_ASSERT(0); }
...
class join_ctx : public unit_ctx, virtual public qep_row {
 ...
 public:
  ...
  void set_pq(pq_ctx *ctx) override {
    DBUG_ASSERT(!pq);
    pq = ctx;
  }
...
/**
  Node class for CTX_PQ
  When the query do parallel query, an outer node named "parallel_query"
  is added:
    query_block: {
      select_id: 1,
      parallel_execute: {
  ...
  This "parallel_query" layout displays information about the leader of
  parallel query, the implementation of pq_ctx is similar to that of sort_ctx.
  It contains a join_ctx which displays information about the worker.
*/
class pq_ctx : public join_ctx {
 public:
  pq_ctx(enum_parsing_context type_arg, const char *name_arg,
         context *parent_arg)
      : context(type_arg, name_arg, parent_arg),
 ...
 protected:
  bool format_body(Opt_trace_context *json, Opt_trace_object *obj) override {
    if (!col_cut_table_name.is_empty())
      obj->add_utf8(K_PARALLEL_QUERY_CUT_TABLE, col_cut_table_name.str);
    if (!col_divided_table_name.is_empty())
      obj->add_utf8(K_PARALLEL_QUERY_DIVIDED_TABLE, col_divided_table_name.str);
    if (!col_pq_dop.is_empty())
      obj->add(K_PARALLEL_QUERY_DOP, col_pq_dop.value);
...
bool join_ctx::find_and_set_derived(context *subquery) {
  ...
  if (pq) return pq->find_and_set_derived(subquery);
...
bool join_ctx::format_body(Opt_trace_context *json, Opt_trace_object *obj) {
  ...
  if (!col_read_cost.is_empty() && type != CTX_PQ) {
    char buf[32];  // 32 is enough for digits of a double
  ...
  switch (ctx_arg) {
    ...
    case CTX_PQ: {
      DBUG_ASSERT(current_context->type == CTX_JOIN ||
                  ...
                  current_context->type == CTX_SIMPLE_ORDER_BY ||
                  current_context->type == CTX_SIMPLE_DISTINCT);
      pq_ctx *ctx = new (*THR_MALLOC)
          pq_ctx(CTX_PQ, K_PARALLEL_QUERY_GATHER, current_context);
      if (ctx == nullptr) return true;
      current_context->set_pq(ctx);
      current_context = ctx;
      break;
    }
    default:

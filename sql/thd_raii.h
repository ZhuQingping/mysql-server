...
/**
  @brief RAII guard for temporarily disabling DStore extra WAL check
         and DDL info writing

  Sets THD::m_skip_dstore_extral_wal_check and THD::is_ddl_info_written
  to true on construction, and restore them on destruction.
 */
class Disable_ddl_info_write_guard final {
 public:
  /**
   @param thd  non-NULL - pointer to the THD context in which DStore
                          extra WAL check and DDL info writing need to
                          be disabled.
               NULL     - if no operation is needed.
   */
  explicit Disable_ddl_info_write_guard(THD *thd) : m_thd(thd) {
    if (m_thd) {
      m_original_skip_dstore_extral_wal_check =
          m_thd->m_skip_dstore_extral_wal_check;
      m_original_is_ddl_info_written = m_thd->is_ddl_info_written;

      m_thd->m_skip_dstore_extral_wal_check = true;
      m_thd->is_ddl_info_written = true;
    }
  }

  ~Disable_ddl_info_write_guard() {
    if (m_thd) {
      m_thd->m_skip_dstore_extral_wal_check =
          m_original_skip_dstore_extral_wal_check;
      m_thd->is_ddl_info_written = m_original_is_ddl_info_written;
    }
  }

  // Disable copy and move operations
  Disable_ddl_info_write_guard(const Disable_ddl_info_write_guard &) = delete;
  Disable_ddl_info_write_guard &operator=(
      const Disable_ddl_info_write_guard &) = delete;

 private:
  THD *m_thd;
  bool m_original_skip_dstore_extral_wal_check{false};
  bool m_original_is_ddl_info_written{false};
};

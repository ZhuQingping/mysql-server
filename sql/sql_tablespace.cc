...
bool map_errors(int se_error, const char *statement_txt,
  ...
  switch (se_error) {
    ...
    case HA_ADMIN_NOT_IMPLEMENTED:
      my_error(ER_TABLESPACE_CHECK_NOT_IMPLEMENTED, MYF(0), statement_txt);
      break;
...
bool Sql_cmd_create_tablespace::execute(THD *thd) {
  ...
  */
  ...
  if (hton->flags & HTON_SUPPORTS_TABLE_ENCRYPTION) {
    ...
    my_error(ER_TABLESPACE_CHECK_NOT_IMPLEMENTED, MYF(0), "ENCRYPTION");
    return true;
  }
...
bool Sql_cmd_drop_tablespace::execute(THD *thd) {
  ...
  */
  ...
  if ((strcmp(old_ts_def->engine().c_str(), DSTORE_ENGINE_NAME) != 0) ||
      (strcmp(old_ts_def->name().c_str(), "dstore_default") != 0)) {
    if (dc.drop(old_ts_def)) {
      return true;
    }
  } else {
    push_warning(thd, Sql_condition::SL_WARNING,
                 WARN_DROP_IS_REBUILD_TABLESPACE,
                 ER_THD(thd, WARN_DROP_IS_REBUILD_TABLESPACE));
  }

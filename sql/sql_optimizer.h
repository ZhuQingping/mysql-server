...
class JOIN {
  ...
  */
  Item *having_cond;
  // It's used to save part of pushed having condition, which is same as
  // sort_table_cond. It's used for check whether query is applicable for
  ...
  */
  ...
  /** slice no. used to create PQ's tmp table */
 ...
 public:
  ...
  /** rewritten leader's QEP_TAB */
  bool make_leader_tmp_table();
  /** make a tmp table for sending partial results */
  bool make_worker_tmp_table();
  // make leader's first tmp table in PQ
  bool make_leader_rewritten_tab();
  // determine whether suite for parallel query
  bool suite_for_parallel_query();

  /** Check whether query is suit for switching options in
   * pq_support_features_switch. */
  bool check_pq_support_features_switch();

  // determine whether suite for parallel query By cost
  bool is_suit_for_pq_based_cost();

  /** alloc qep_tab and ref_item */
  bool alloc_qep1(uint n);
  /** copy info. from orig JOIN */
  bool pq_copy_from(JOIN *orig);
  /** alloc indirection slice */
  bool alloc_indirection_slices1();
  /** setup tmp table info */
  bool setup_tmp_table_info(JOIN *orig);
  /** restore optimized variables for generating QEP */
 ...
 private:
  ...
  /// False means cached plan has never been hit in any past execution of this
  /// prepared statement. Otherwise, True.
 ...
 public:
  ...
  /// Friend functions for plan cache
  friend bool plan_cache::exec_cached_plan(Query_block *sl);
  QEP_TAB *get_last_tab() {
    if (qep_tab) {
      return &(qep_tab[primary_tables + tmp_tables - 1]);
    }
    return nullptr;
  }
};
...
bool uses_index_fields_only(Item *item, TABLE *tbl, uint keyno,
                            bool other_tbls_ok, bool check_index = true);
...
inline bool null_terminate_key_use_list(Key_use_array *array) {
  return array->push_back(
      Key_use(nullptr, nullptr, 0, 0, 0, 0, 0, 0, false, nullptr, 0));
}

/**
  Check if given expression can be evaluated in storeage engine

  This function is either called by index condition pushdown
  or by aggregate pushdown

  When this function is called from index condition pushdown,
  the input parameter check_index is true. For field items that
  belong to the table tbl, check if it is covered by index #keyno
  in the table tbl.

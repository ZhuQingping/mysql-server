...
class Query_arena {
  ...
  */
  ...
  bool is_stmt_prepared_or_executed() const {
...
#define RETURN_NAME_AS_STRING(NAME) \
  ...
  switch (thread) {
    ...
    RETURN_NAME_AS_STRING(SYSTEM_THREAD_PURGE_EXPIRED_BINLOG);
    default:
...
class Global_backup_lock {
 public:
  ...
  /**
   ...
   released.
  @param[in, out] thd     Reference to connection.
  ...
  */
  ...
  /**
   ...
   is active.
   @param[in, out]  thd              Reference to connection.
   @param[in]       duration          MDL lock duration
   @param[in]       lock_wait_timeout Lock wait timeout
  ...
  */
  ...
  /**
   ...
   @param[in] mdl_request       Pointer to request to initialize
   @param[in] duration          MDL lock duration
  */
  ...
  */
  ...
  /**
   Check whether the global backup lock is acquired.
   @retval true if the lock is acquired, otherwise false
  */
  bool is_acquired() const { return m_lock != NULL; }

  /**
   Check whether the protection request against a global backup lock is
   acquired.
   @retval true if the protection request is acquired, otherwise false
  */
  bool is_protection_acquired() const { return m_prot_lock != NULL; }

  /**
    Set explicit duration for global backup lock.

    @param[int, out]      thd      Reference to thread.
  */
  void set_explicit_lock_duration(THD *thd);

 private:
  /** MDL namespace of the gloabl backup lock, i.e. BINLOG. */
  MDL_key::enum_mdl_namespace m_namespace;

  /** the gloabl backup lock*/
  MDL_ticket *m_lock;

  /** the protection lock against a global backup lock*/
...
/* store the ddl_info buf reading from wal in replay process */
class DDL_info_replay_buf {
 public:
  uint64_t end_lsn;
  const unsigned char *data;
  size_t data_len;
  bool need_write_to_trace;

  DDL_info_replay_buf() { clean(); }

  void clean() {
    data = nullptr;
    data_len = 0;
    end_lsn = 0xFFFFFFFFFFFFFFFF;
    need_write_to_trace = false;
  }
};
...
class THD : public MDL_context_owner,
  ...
  */
  ...
  /* Store the ddl statement with dstore specified comments for replay. */
  String m_query_with_ddl_sql;

  /* Store the ddl info buffer reading from wal in replay process */
  DDL_info_replay_buf ddl_info_replay;
  /* Store the gno */
  int64_t m_gno;
 ...
 public:
  ...
  /* Skip dstore extral wal size check. */
  bool m_skip_dstore_extral_wal_check{false};
  ...
  */
  THD_event_functions *conn_event_functions{nullptr};
  ...
  */
  ...
  /**
     Identifier of filter item in SQL Filter; it uniquely represents one filter
     rule and user can visit it through table mysql.rds_sql_filter_rules or call
 ...
 public:
  ...
  /**
    Transaction start time, expressed in seconds.
  */
  time_t trx_start_time_in_secs;
  /**
  ...
  */
  ...
  /**
    Get socket wait timeout. will add more configurable params
    for different scenarios wait timeout.
  ...
  */
  ...
  Global_backup_lock backup_binlog_lock{MDL_key::BINLOG};
  ...
  */
  ...
  /// instead /sven
  bool slave_thread;
  /* Indicates whether the thread is performing DDL information replay */
  bool for_ddl_info_replay;
  /* Indicates whether the ddl_info is alreay written to redo, to avoid
   * duplicate writing. */
  bool is_ddl_info_written;
  ...
  */
  ...
  void cleanup_after_query(bool clean_pq_variables = false);
  void store_globals();
 ...
 public:
  ...
  /**
    ...
    @param[in] is_void_pointer  When true, indicates 'new_value' is actually a
                                void** (e.g., what 'new' operator returns).
                                It is an incomplete Item * object, so its
                                members cannot be accessed, otherwise, an error
                                will be detected under ASAN.
  */
  void change_item_tree(Item **place, Item *new_value,
                        bool is_void_pointer = false);
  ...
  */
  ...
  /**
    Validates the size of DDL info.

    Checks if the query length is within the valid range (greater than zero
    and not exceeding the maximum allowed size).

    @param ddl_info  The query text used to construct DDL info.

    @retval false size is valid.
    @retval true  size is out of valid range. Error reported.
  */
  bool validate_ddl_info_size(LEX_CSTRING query_arg);

  /**
    Set DDL info based on the provided query in LEX_CSTRING type.

    This function constructs a complete DDL query string, optionally prepending
    a "USE" statement if necessary, and stores it as the initial DDL information
    for the thread.

    @param query_arg The DDL query to be set as initial information.
    @param skip_prefix Whether to skip appending the "USE XXDB" prefix in the
    header.

    @retval false if success
    @retval true if failed to set ddl info(e.g. ddl info is too long).
  */
  bool set_ddl_sql(LEX_CSTRING query_arg, bool skip_prefix = false);

 private:
  /**
   * Swap DDL sql with the provided string.
   *
   * This function swaps the provided query string with the current
   * `m_query_with_ddl_sql` and handles locking. If the provided query string
   * exceeds the maximum allowed length, it triggers an error and resets the DDL
   * information.
   *
   * @param query_arg The query string to swap with the current DDL sql
   */
  void swap_ddl_sql(String &query_arg);

 public:
  /**
   * Sets the GTID information for the current thread.
   *
   * This function updates the GTID (Global Transaction Identifier) information
   * associated with the current thread. It stores the transaction's UUID and
   * the GTID number (gno) in the thread's context.
   *
   * The function acquires a mutex to ensure thread-safe access to the internal
   * state of the thread.
   *
   * @param uuid A pointer to the UUID of the transaction.
   * @param gno  The GTID number (sequence number) of the transaction.
   */
  void set_gtid_info(const unsigned char *uuid, int64_t gno);

  int64_t get_gno() { return m_gno; }

  const unsigned char *get_uuid() const { return m_uuid; }

  /**
   * Retrieve the DDL info sql.
   *
   * This function returns the current DDL information string.
   * In debug mode, it checks whether the current thread is the owner of the
   * lock.
   *
   * @retval const String& The current DDL info sql.
   */
  const String &ddl_info_sql() const {
#ifndef NDEBUG
    if (current_thd != this) mysql_mutex_assert_owner(&LOCK_thd_query);
#endif
    return m_query_with_ddl_sql;
  }

  /**
   * Reset the DDL information to an empty state.
   *
   * This function clears any stored DDL information by swapping
   * it with an empty string. If the current DDL information is not empty, it
   * calls `swap_ddl_sql()` to perform the reset.
   */
  inline void reset_ddl_info() {
    if (unlikely(ddl_info_sql().length())) {
      String empty;
      swap_ddl_sql(empty);
    }
    is_ddl_info_written = false;
  }

  /**
    Get the DDL info buffer from the replay process.

    This function returns a reference to the DDL info replay buffer, which
    stores the DDL info queries and end lsn that is to be logged in
    "ddl_info_replay_trace" table.

    @return Reference to the DDL_info_replay_buf.
  */
  const DDL_info_replay_buf &get_ddl_info_replay_buf() const {
    return ddl_info_replay;
  }

  /**
    Store ddl_info buffer to thd for later logging into the
    `ddl_info_replay_trace` table.

    @param end_lsn The end log sequence number (LSN) associated with the DDL
    operation.
    @param buffer Pointer to the DDL info buffer to be saved.
    @param buffer_size The size of the DDL info buffer.
  */
  void save_ddl_info_for_trace(uint64_t end_lsn, const unsigned char *buffer,
                               size_t buffer_size) {
    ddl_info_replay.clean();
    ddl_info_replay.end_lsn = end_lsn;
    ddl_info_replay.data_len = buffer_size;
    ddl_info_replay.data = buffer;
  }

  /**
    Set whether to write DDL info for replay to the trace table, .

    This function enables or disables the writing of DDL information to the
    `ddl_info_replay_trace` table.

    @param value Boolean flag indicating whether to write to the table
    `ddl_info_replay_trace`.
  */
  void set_write_to_ddl_info_replay_trace(bool value) {
    ddl_info_replay.need_write_to_trace = value;
  }

  /**
    clean ddl_info buffer in thd.
  */
  void clean_ddl_info_for_trace() { ddl_info_replay.clean(); }
  ...
  */
  ...
  bool is_reserved_user_session() { return is_reserved_user_thd; }
  void mark_as_reserved_user_session() { is_reserved_user_thd = true; }
  ...
  */
  ...
  /**
    Variable to mark if current session is work for rds_reserved_user.
  */
  bool is_reserved_user_thd;
 ...
 public:
  ...
  // A VfdManager that can be used to limit the number of open files for this
  // THD/connection. Currently used by hash join.
  std::unique_ptr<VfdManager> m_vfd_manager;

  // Only be used in plan_cache::collect_item_params function. If
  // walk_const_item is set to true, const_item->walk will be called in
  // Item_equal::walk.
  bool walk_const_item{false};

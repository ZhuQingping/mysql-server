...
Relay_log_info::Relay_log_info(bool is_slave_recovery,
  ...
  relay_log.set_psi_keys(
      ...
      key_RELAYLOG_LOCK_log_end_pos, PSI_NOT_INSTRUMENTED,
      key_RELAYLOG_LOCK_sync, PSI_NOT_INSTRUMENTED, key_RELAYLOG_LOCK_xids,
      PSI_NOT_INSTRUMENTED, PSI_NOT_INSTRUMENTED, PSI_NOT_INSTRUMENTED,
...
int Relay_log_info::inc_group_relay_log_pos(ulonglong log_pos,
  ...
  if (need_data_lock) {
    if (rds_binlog_lock_enable) {
      ...
      */
      ...
      if (info_thd->backup_binlog_lock.acquire_protection(
              info_thd, MDL_EXPLICIT, timeout)) {
        return 1;
      }
    }
    mysql_mutex_lock(&data_lock);
  ...
  */
  ...
  if (need_data_lock) {
    mysql_mutex_unlock(&data_lock);
    if (info_thd->backup_binlog_lock.is_protection_acquired()) {

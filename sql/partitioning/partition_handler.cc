...
bool loose_partition_restriction = false;
...
bool Partition_share::populate_partition_name_hash(partition_info *part_info) {
  uint num_subparts = part_info->get_num_subparts();
  DBUG_TRACE;
...
bool Partition_helper::print_partition_error(int error) {
  ...
  if ((error == HA_ERR_NO_PARTITION_FOUND) &&
    ...
    if (m_part_info->part_type == partition_type::RANGE ||
        m_part_info->part_type == partition_type::LIST)
      m_part_info->print_no_partition_found(thd, m_table);
    else {
      if (m_part_info->is_sub_partitioned())
        m_part_info->sub_part_info->print_no_partition_found(thd, m_table);
      else {
        m_part_info->print_no_partition_found(thd, m_table);
      }
    }
  ...
  } else if (error == HA_ERR_NOT_ALLOWED_PARTITION_TYPE) {
    my_error(ER_PARTITION_SUBPARTITION_ERROR, MYF(0));
    return false;
  } else if (error == HA_ERR_REORG_WRONG_VALUES_RANGE) {
    my_error(ER_REORG_WRONG_VALUES_RANGE, MYF(0));
    return false;
  }
...
void Partition_helper::prepare_change_partitions() {
  ...
  uint num_subparts =
      m_part_info->is_sub_partitioned() ? m_part_info->get_num_subparts() : 1;
...
int Partition_helper::copy_partitions(ulonglong *const deleted) {
  ...
  if (m_part_info->linear_hash_ind) {
    if (m_part_info->part_type == partition_type::HASH)
      ...
      set_linear_hash_mask(m_part_info->sub_part_info,
                           m_part_info->get_num_subparts());
  }
  ...
  */
  ...
  while (true) {
    ...
    if (m_part_info->get_partition_id(m_part_info, &new_part, &func_value)) {
      ...
      */
      if (!loose_partition_restriction) {
        result = HA_ERR_REORG_WRONG_VALUES_RANGE;
        goto error;
      } else {
        (*deleted)++;
      }
...
int Partition_helper::ph_rnd_end() {
  int error = 0;
  DBUG_TRACE;
  // Because pq scan init skips to do ph_rnd_init, here we skip to do actual
  // work according to m_scan_value temporarily.
  if (m_handler->pq_table_scan) return error;

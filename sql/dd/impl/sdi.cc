...
bool drop_all_impl(THD *thd, const DDT *tp) {
  ...
  const handlerton &hton = ptr_as_cref(resolve_hton(thd, t));
  /* For dstore we don't store sdi information in tablespace.
     Because of that we should just make sdi_delete be no-op.
     However if sdi_delete is not null - the server will not take care
     of removing any .sdi files - which we need in DStore.
     Thus we set sdi_get to nullptr and stop server from checking
     asserts here for DStore. */
  if (hton.db_type == DB_TYPE_DSTORE) return false;

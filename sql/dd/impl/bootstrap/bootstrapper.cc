...
bool restart(THD *thd) {
  ...
  */
  ...
  DBUG_EXECUTE_IF("force_taurus_dd_version_upgrade",
                  { dd::TAURUS_EXTENDED_DD_VERSION = 98; });
...
bool initialize_dd_properties(THD *thd) {
  ...
  */
  ...
  uint actual_taurus_extended_dd_version = dd::TAURUS_EXTENDED_DD_VERSION;

  bootstrap::DD_bootstrap_ctx::instance().set_actual_dd_version(
      actual_dd_version);
  bootstrap::DD_bootstrap_ctx::instance().set_actual_taurus_extended_dd_version(
      actual_taurus_extended_dd_version);
  ...
  if (!opt_initialize) {
    ...
    */
    ...
    exists = false;
    if (dd::tables::DD_properties::instance().get_if_exists(
            thd, "TAURUS_EXTENDED_DD_VERSION",
            &actual_taurus_extended_dd_version, &exists)) {
      LogErr(ERROR_LEVEL, ER_DD_NO_VERSION_FOUND);
      return true;
    }
    if (!exists) actual_taurus_extended_dd_version = 0;
    if (actual_taurus_extended_dd_version != dd::TAURUS_EXTENDED_DD_VERSION) {
      bootstrap::DD_bootstrap_ctx::instance()
          .set_actual_taurus_extended_dd_version(
              actual_taurus_extended_dd_version);
      if (opt_no_dd_upgrade) {
        push_deprecated_warn(thd, "--no-dd-upgrade", "--upgrade=NONE");
        LogErr(ERROR_LEVEL, ER_DD_UPGRADE_OFF);
      ...
      if (actual_taurus_extended_dd_version > dd::TAURUS_EXTENDED_DD_VERSION) {
        LogErr(ERROR_LEVEL, ER_DD_DOWNGRADE_EXTENDED_VERSION_NOT_SUPPORTED);
        return true;
      }
    }
  ...
  if (bootstrap::DD_bootstrap_ctx::instance().is_initialize())
    ...
    */
    if (bootstrap::DD_bootstrap_ctx::instance().is_dd_upgrade()) {
      LogErr(SYSTEM_LEVEL, ER_DD_UPGRADE,
             get_extended_data_dictionary_version(
                 actual_dd_version, actual_taurus_extended_dd_version),
             get_extended_data_dictionary_version(
                 dd::DD_VERSION, dd::TAURUS_EXTENDED_DD_VERSION));
  ...
  */
  if (opt_initialize) {
    if (setprop(thd, "DD_VERSION", dd::DD_VERSION) ||
        ...
        setprop(thd, "TAURUS_EXTENDED_DD_VERSION",
                dd::TAURUS_EXTENDED_DD_VERSION))
      return dd::end_transaction(thd, true);
    ...
    */
    ...
    // Currently, mysqld does not support minor dd downgrade. When
    // actual_dd_version is greater than target DD_VERSION in binary,
    // is_minor_downgrade function return false and it reported error message to
    // error_log in initialize_dd_properties function and mysqld failes to be
    // restarted.
    assert(dd_version <= dd::DD_VERSION);

    if (dd_version < dd::DD_VERSION ||
        DBUG_EVALUATE_IF("dd_version_upgrade_test", true, false)) {
      if (dd::tables::DD_properties::instance().set(
              thd, "TAURUS_EXTENDED_DD_VERSION",
              dd::TAURUS_EXTENDED_DD_VERSION) ||
          DBUG_EVALUATE_IF("upgrade_version_set_fail", true, false))
        return dd::end_transaction(thd, true);
    } else if (dd_version == dd::DD_VERSION) {
      uint taurus_dd_version = 0;
      bool exists_dd = false;
      if (!dd::tables::DD_properties::instance().get_if_exists(
              thd, "TAURUS_EXTENDED_DD_VERSION", &taurus_dd_version,
              &exists_dd)) {
        // If actual data dictionary does not include
        // TAURUS_EXTENDED_DD_VERSION key, it considers
        // TAURUS_EXTENDED_DD_VERSION value in actual data dictionary is 0.
        // When older version mysqld which does not include
        // DD::TAURUS_EXTENDED_DD_VERSION definition and it upgrades to the
        // newer version mysqld, the exists_dd is false.
        if (!exists_dd) taurus_dd_version = 0;

        // Currently, taurus_extended_dd_version is same with DD_VERSION and
        // it cannot to be downgraded. In initialize_dd_properties function it
        // checks whether actual_taurus_extended_dd_version is greater than
        // target TAURUS_EXTENDED_DD_VERSION, if yes, it reports error
        // messages to error_log file and mysqld failes to be restarted.
        assert(taurus_dd_version <= dd::TAURUS_EXTENDED_DD_VERSION);

        if (taurus_dd_version < dd::TAURUS_EXTENDED_DD_VERSION &&
...
uint get_extended_data_dictionary_version(uint dd_version,
                                          uint taurus_extended_dd_version) {
  return dd_version * 100 + taurus_extended_dd_version;
}

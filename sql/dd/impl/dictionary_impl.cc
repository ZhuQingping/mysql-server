...
uint Dictionary_impl::get_target_dd_version() {
  return get_extended_data_dictionary_version(dd::DD_VERSION,
...
uint Dictionary_impl::get_actual_dd_version(THD *thd) {
  ...
  bool error [[maybe_unused]] = tables::DD_properties::instance().get(
      thd, "DD_VERSION", &dd_version, &exists);
  assert(!error);
  assert(exists);

  exists = false;
  error = tables::DD_properties::instance().get_if_exists(
      thd, "TAURUS_EXTENDED_DD_VERSION", &taurus_dd_version, &exists);
  assert(!error);
  if (!exists) taurus_dd_version = 0;
  return dd_version * 100 + taurus_dd_version;
}

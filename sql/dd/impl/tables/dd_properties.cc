...
DD_properties::DD_properties() : m_properties() {
  ...
  /*
    ...
    the following purposes:
      ...
      TAURUS_EXTENDED_DD_VERSION          Actual TAURUS DD version.
  */
  m_property_desc = {
      ...
      {"TAURUS_EXTENDED_DD_VERSION", Property_type::UNSIGNED_INT_32}};
}
...
// Read the integer property for the given key.
bool DD_properties::get_if_exists(THD *thd, const String_type &key, uint *value,
                                  bool *exists) {
  assert(m_property_desc.find(key) != m_property_desc.end() &&
         m_property_desc[key] == Property_type::UNSIGNED_INT_32);
  String_type val_str;
  if (unchecked_get(thd, key, &val_str, exists)) return true;
  if (*exists) return dd::Properties::from_str(val_str, value);
  return false;
}

...
static void fill_dd_indexes_from_keyinfo(
  ...
  */
  ...
  for (int key_nr = 1; key != end; ++key, ++key_nr) {
    ...
    if (key->dstore_ddl_comment.str) {
      dd::String_type comment_str(key->dstore_ddl_comment.str);
      std::unique_ptr<dd::Properties> props(
          dd::Properties::parse_properties(comment_str));
      idx_obj->set_se_private_data(*props);
    }
...
// Get partitoin type for dd::Table
static dd::Table::enum_partition_type get_partition_type(
    const partition_info *part_info) {
  dd::Table::enum_partition_type type = dd::Table::PT_NONE;
  switch (part_info->part_type) {
    case partition_type::RANGE:
      if (part_info->column_list)
        type = dd::Table::PT_RANGE_COLUMNS;
      else
        type = dd::Table::PT_RANGE;
      break;
    case partition_type::LIST:
      if (part_info->column_list)
        type = dd::Table::PT_LIST_COLUMNS;
      else
        type = dd::Table::PT_LIST;
      break;
      ...
      if (part_info->list_of_part_fields) {
        ...
        if (part_info->linear_hash_ind) {
          if (part_info->key_algorithm == enum_key_algorithm::KEY_ALGORITHM_51)
            type = dd::Table::PT_LINEAR_KEY_51;
          else
            type = dd::Table::PT_LINEAR_KEY_55;
        } else {
          if (part_info->key_algorithm == enum_key_algorithm::KEY_ALGORITHM_51)
            type = dd::Table::PT_KEY_51;
          else
            type = dd::Table::PT_KEY_55;
        }
      } else {
        if (part_info->linear_hash_ind)
          type = dd::Table::PT_LINEAR_HASH;
        else
          type = dd::Table::PT_HASH;
      }
      break;
    default:
  ...
  if (part_info->is_auto_partitioned) {
    if (type == dd::Table::PT_KEY_55) {
      type = dd::Table::PT_AUTO;
    } else if (type == dd::Table::PT_LINEAR_KEY_55) {
      type = dd::Table::PT_AUTO_LINEAR;
    } else {
      /*
...
/*
  Helper function to set partition expression for partition and subpartition.

  @param    thd          Pointer of current THD
  @param    tab_obj      DD table object used to store partition information
  @param    part_info    Parition info.

  @retval  False means success. Otherwise true.
*/
static bool set_partition_expr(THD *thd, dd::Table *tab_obj,
...
/*
  Helper function to set partition definition to part_obj so as to store it
  into dd.

  @param      thd                Pointer of current THD
  @param      part_elem          Current parition definition.
  @param      tab_obj            DD table object used to store partition
  information.
  @param      create_info        Create info.
  @param      create_fields      List of fields being created.
  @param      part_info          Parition info.
  ...
  @retval     False means success. Otherwise true.
*/
static bool set_partition_definition(THD *thd, partition_element *part_elem,
                                     dd::Table *tab_obj,
                                     const HA_CREATE_INFO *create_info,
                                     const List<Create_field> &create_fields,
                                     partition_info *part_info,
                                     dd::Partition *part_obj, int part_index) {
  assert(part_info);
  ...
  if (part_elem->part_comment) part_obj->set_comment(part_elem->part_comment);
  part_obj->set_number(part_index);
  ...
  if (part_info->part_type == partition_type::RANGE) {
    if (part_info->column_list) {
      ...
      if (add_part_col_vals(part_info, list_value, 0, part_obj, create_info,
                            create_fields, &part_desc_str)) {
        return true;
      }
    ...
    while ((list_value = list_val_it++)) {
      if (part_info->column_list) {
        ...
        if (part_info->part_field_list.elements > 1U) part_desc_str.append("(");
        ...
        if (list_value->unsigned_flag) {
          ...
          val_obj->set_value_utf8(dd::Properties::to_str(list_value->value));
...
static bool fill_dd_partition_from_create_info(
  ...
  if (part_info) {
    tab_obj->set_partition_type(get_partition_type(part_info));
    if (set_partition_expr(thd, tab_obj, part_info)) return true;

    /* Set up subpartitioning. */
    if (part_info->is_sub_partitioned()) {
      tab_obj->set_subpartition_type(
          get_partition_type(part_info->sub_part_info));
      if (set_partition_expr(thd, tab_obj, part_info->sub_part_info))
        return true;
    }
    ...
    /* Add partitions and subpartitions. */
    {
      ...
      while ((part_elem = part_it++)) {
        ...
        if (set_partition_definition(thd, part_elem, tab_obj, create_info,
                                     create_fields, part_info, part_obj,
                                     part_num))
          return true;
        ...
        if (part_info->is_sub_partitioned()) {
          ...
          while ((sub_elem = sub_it++)) {
            ...
            if (set_partition_definition(
                    thd, sub_elem, tab_obj, create_info, create_fields,
                    part_info->sub_part_info, sub_obj, sub_part_num))
              return true;
            sub_part_num++;
          }
        }
...
bool get_implicit_tablespace_options(THD *thd, const Table *table,
  ...
  if (table->partition_type() == Table::PT_NONE) {
    ...
    if (part) {
      ...
      if (table->subpartition_type() == Table::PT_NONE) {

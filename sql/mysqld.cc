...
#include "sql/sql_plan_cache.h"                  // cached_plan_count
...
#include "sql/rpl_wal_mgr.h"                        // mysql_wal
#include "sql/sql_plan_cache.h"                     // cached_plan_count
#ifdef _WIN32
...
#include "storage/dstore/dict/cde_dict_bg_stat.h"
...
#ifdef WITH_JEMALLOC_HEADER
#include <jemalloc/jemalloc.h>

namespace {
static inline bool get_background_thread() {
  /** In case that newp is NULL, failing je_mallctl won't change oldval. */
  bool old_val = true;
  size_t val_size = sizeof(bool);
  int ret = mallctl("background_thread", (void *)&old_val, &val_size, NULL, 0);
  assert(ret == 0);
  sql_print_information("get background_thread old_val: %d, mallctl ret: %d",
                        old_val, ret);
  return old_val;
}

static inline void switch_background_thread(bool set_val) {
  bool old_val, new_val = set_val;
  size_t val_size = sizeof(bool);
  int ret = mallctl("background_thread", (void *)&old_val, &val_size, &new_val,
                    val_size);
  assert(ret == 0);
  sql_print_information("switch background_thread old_val: %d, mallctl ret: %d",
                        old_val, ret);
}

static inline void check_background_thread(bool before) {
  bool old_val, new_val = before;
  size_t val_size = sizeof(bool);
  int ret = mallctl("background_thread", (void *)&old_val, &val_size, &new_val,
                    val_size);
  assert(ret == 0);
  ...
  if (ret != 0) {
    sql_print_warning("Unexpected mallctl() failure.\n");
  }
  if (old_val != new_val) {
    sql_print_warning("background_thread should be %d.\n", before);
  }
}
};  // namespace
#endif
...
static PSI_mutex_key key_LOCK_purge_binlog;
...
static PSI_mutex_key key_BINLOG_LOCK_redo;
...
static PSI_mutex_key key_BINLOG_LOCK_tids;
...
static PSI_cond_key key_COND_purge_binlog;
...
// wal replication
static PSI_mutex_key key_LOCK_wal_flushed_lsn;
static PSI_cond_key key_COND_wal_update_cond;
...
int argc_cached;
char **argv_cached;
bool opt_oma_log = true;

bool rds_binlog_lock_enable = true;

const char *debug_info_prefix = "[TAURUS-DEBUG]";
...
bool opt_rds_maintenance_operation_write_binlog = true;
...
/** Choose Dstore or InnoDB as storage engine.
Both option is only for emergency offline situation. */
ulong storage_engine_mode = 0;
const char *storage_engine_mode_names[] = {"ONLY_DSTORE", "ONLY_INNODB",
                                           "BOTH_STORAGE_ENGINE", NullS};
...
ulong rds_binlog_purge_thread_status;
...
bool rds_write_binlog_into_redo = false;
bool rds_binlog_concurrent_commit = true;
bool rds_binlog_dstore_consistent = false;
bool rds_dstore_enable_atomic_ddl = true;
uint32_t rds_dstore_ddl_max_concurrency = 4;
uint32_t rds_dstore_ddl_throttle_timeout = 60;
/* rds_dstore_write_ddl_info_into_redo is not bound as a SYS_VAR because we
   assume that DDL info should always be written even if it has no valid
   content. */
bool rds_dstore_write_ddl_info_into_redo = true;
bool rds_dstore_write_ddl_sql_into_redo = true;
bool rds_use_ddl_info_replay = false;
bool rds_dstore_support_binlog_check = true;
bool rds_binlog_enable_group_replication = false;
bool rds_binlog_skip_run_after_commit_hook = true;
bool rds_binlog_dstore_trace = true;
bool rds_warn_alter_table_add_auto_incre_column = true;
double rds_packet_size_factor = 1.4;

char dstore_missed_gtids[binlog_write_redo_buffer_size];
char *rds_dstore_missed_gtids_ptr = nullptr;
...
int32 rds_max_num_of_binary_logs_purged_per_time = 100;
ulong rds_binlog_purge_thread_wait_timeout = 600;
...
bool opt_rds_permission_control;
bool opt_rds_processlist_control;
bool opt_rds_internal_schema_ddl_control;
bool opt_rds_internal_schema_dml_control;
bool opt_rds_allow_empty_users;
bool opt_rds_sync_comment;
bool thread_cache_size_specified = false;
bool rds_disable_user_defined_functions = true;
bool host_cache_size_specified = false;
bool table_definition_cache_specified = false;
ulong locked_account_connection_count = 0;
bool rds_load_data_infile_enable = false;
ulonglong rds_security_hardening;
ulonglong kMaxChunks = 8192;
bool pq_master_enable;
/* Controls whether index hints work for view */
bool hint_work_for_view = true;

/**
 * When true, handle_fatal_signal() emits an extended FFIC crash report
 * containing register dumps, stack memory, instruction bytes, and OS info.
 * Controlled by the sys var rds_ffic_verbose_crash_diagnostics (GLOBAL,
 * dynamic). Default ON; enable with: SET GLOBAL
 * rds_ffic_verbose_crash_diagnostics = ON
 */
...
ulong rds_conn_log_error_verbosity;

bool rds_purge_binary_logs_asynchronously = false;

/**
  Used to automatically update histograms based on table data changed.
*/
bool dstore_use_histogram_auto_update = true;
/**
 The max number of connections allowed for reserved users
*/
ulong max_rds_reserved_connections = 0;
/**
 The max number of connections allowed through admin port
*/
ulong opt_rds_admin_max_connections = 0;
/**
 Indicates whether admin port connections should use per thread
*/
bool opt_rds_admin_port_using_per_thread = false;
/**
 Indicates whether local socket connections should use per thread
*/
bool opt_rds_local_socket_using_per_thread = false;
/**
  Imitate mariadb server, which means telling client that this is
  mariadb server during setting up connection. Differences between
  Mariadb server and MySQL server are:
  1. Mariadb server will unset CLIENT_LONG_PASSWORD in server
     capabilities flag.
  2. Server version of mariadb server will contains "MariaDB" or
     "-maria-".
  3. Mariadb server will set extended capabilities flag, which only
     contains CLIENT_STMT_BULK_OPERATIONS flag now.
*/
bool rds_imitate_mariadb_server;
/**
 Indicates whether to show hidden tables.
*/
bool rds_show_hidden_table;

bool rds_allow_skip_locked_now_wait;
/**
...
bool opt_rds_sqlfilter_control;
/* Check root user was affected by sql filter or not. */
bool rds_sqlfilter_affect_admin_user;
bool opt_rds_sqlfilter_case_sensitive;
bool rds_sqlfilter_sys_table_control;

/** The maximum number for sql filter rules that is allowed to be added */
ulong rds_sqlfilter_rules_max_count = 100;
const char *node_id_ptr;

ulong rds_physical_rpl_sender_packet_size = 0;
ulong rds_physical_rpl_sender_wait_times = 0;
ulong rds_physical_rpl_sender_sleep_us = 0;

bool rds_local_backup_use_obs = false;
bool current_lb_use_obs = false;
char *rds_local_backup_obs_host = nullptr;
char *rds_local_backup_obs_bucket = nullptr;
char *rds_local_backup_obs_ak = nullptr;
char *rds_local_backup_obs_sk = nullptr;
uint32_t rds_local_backup_obs_uri_style = 0;
uint32_t rds_lb_obs_buffer_size = 0;
/* Enable scc decrypt when use OBS for local backup of Taurus on dstore. */
bool rds_lb_obs_enable_scc_decrypt = true;
...
bool rds_lb_obs_encryption = false;
/* Kms key id of OBS encryption for local backup of Taurus on dstore. */
char *rds_lb_obs_kms_key_id = nullptr;
...
bool rds_recycle_bin_binlog_session_variables = true;

bool rds_recycle_bin_skip_acl_check = false;
...
#endif
bool opt_warn_sql_log_bin;
char *rds_reserved_users_ptr;
...
bool opt_rds_restricted_dynamic_privileges_control = false;
char *rds_restricted_dynamic_privileges_list_ptr;
std::set<std::string> restricted_dynamic_privileges_set;

// SCC crypto component
char *rds_scc_conf;
...
mysql_rwlock_t LOCK_restricted_dynamic_privileges_list;
...
/**
 * Checks the consistency between the GTID set in the DStore and the binary log.
 *
 * This function compares the GTID set stored in the DStore (gtids_in_wal) with
 * the executed GTID set (executed_gtids). If they are consistent, it sets
 * `rds_binlog_dstore_consistent` to true; otherwise, it identifies the missing
 * GTIDs and updates the status accordingly.
 *
 */

void check_binlog_dstore_consistent() {
  ...
  DBUG_EXECUTE_IF("simulate_exeucted_gtids_as_missed_gtids",
                  { gtids_in_wal->clear(); });

  char *gtid_in_wal_str = nullptr;
  char *gtid_executed_str = nullptr;
  gtids_in_wal->to_string(&gtid_in_wal_str);
  executed_gtids->to_string(&gtid_executed_str);
  sql_print_information("gtids_in_wal is: %s, executed_gtids is: %s",
                        gtid_in_wal_str, gtid_executed_str);
  if (executed_gtids->is_empty()) {
    sql_print_information("Executed is empty, will clear gtid_in_wals");
    gtids_in_wal->clear();
    persist_gtids_in_wal_to_file(gtids_in_wal, false);
  }
  if (gtids_in_wal->is_subset(executed_gtids)) {
    if (executed_gtids->is_subset(gtids_in_wal)) {
      rds_binlog_dstore_consistent = true;
      sql_print_information(
          "gtids_in_wal is equal to gtid_executeds, "
          "rds_binlog_dstore_consistent is set true.");
    } else {
      rds_binlog_dstore_consistent = false;
      Gtid_set dstore_missed_gtids_set(
          gtid_state->get_executed_gtids()->get_sid_map());
      dstore_missed_gtids_set.add_gtid_set(executed_gtids);
      dstore_missed_gtids_set.remove_gtid_set(gtids_in_wal);
      dstore_missed_gtids_set.to_string(dstore_missed_gtids);
      rds_dstore_missed_gtids_ptr = dstore_missed_gtids;
      if (!rds_binlog_dstore_consistent &&
          rds_dstore_missed_gtids_ptr != nullptr &&
          rds_dstore_missed_gtids_ptr[0] != '\0') {
        mysql_bin_log.find_and_set_log_located_by_dstore_missed_gtids(
            dstore_missed_gtids_set);
      }
      sql_print_warning(
          "gtids_in_wal is: %s, executed_gtids is: %s. rds_dstore_missed_gtids "
          "is %s, rds_binlog_dstore_consistent is set "
          "false.",
          gtid_in_wal_str, gtid_executed_str, rds_dstore_missed_gtids_ptr);
    }

  } else {
    rds_binlog_dstore_consistent = false;
    sql_print_error(
        "gtids_in_wal is: %s, executed_gtids is: %s. "
        "rds_binlog_dstore_consistent "
        "is set false.",
        gtid_in_wal_str, gtid_executed_str);
  }
  my_free(gtid_in_wal_str);
  my_free(gtid_executed_str);
  gtid_in_wal_str = nullptr;
  gtid_executed_str = nullptr;
    ...
    if (killing_thd->kill_immunizer) {
      ...
      */
      ...
      if (get_instance_storage_engine_type() == DB_TYPE_DSTORE) {
        ha_kill_connection(killing_thd);
      }
      MYSQL_CALLBACK(killing_thd->conn_event_functions, post_kill_notification,
                     (killing_thd));
    }
...
static void clean_up(bool print_message) {
  ...
  if (gtid_state != nullptr && global_sid_lock != nullptr &&
    ...
    persist_gtids_in_wal_to_file(gs);
  }

  /*
    The background thread that updates statistics may have an active
  ...
  */
  ...
  im::recycle_bin::recycle_scheduler_deinit();
  /*
...
static void clean_up_mutexes() {
  ...
  mysql_rwlock_destroy(&LOCK_restricted_dynamic_privileges_list);
  ...
  mysql_mutex_destroy(&LOCK_purge_binlog);
  mysql_cond_destroy(&COND_purge_binlog);
#ifdef _WIN32
...
void switch_background_thread_drv(bool set_val [[maybe_unused]]) {
#ifdef WITH_JEMALLOC_HEADER
  switch_background_thread(set_val);
#endif
}
void check_background_thread_drv(bool before [[maybe_unused]]) {
#ifdef WITH_JEMALLOC_HEADER
  check_background_thread(before);
#endif
}
}  // namespace mysqld_funcs_unit_test
...
SHOW_VAR com_status_vars[] = {
    ...
    {"lock_binlog_for_backup",
     (char *)offsetof(System_status_var,
                      com_stat[(uint)SQLCOM_LOCK_BINLOG_FOR_BACKUP]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    {"optimize",
    ...
    {"unlock_tables",
     ...
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    {"unlock_binlog",
     (char *)offsetof(System_status_var, com_stat[(uint)SQLCOM_UNLOCK_BINLOG]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
    {"update",
    ...
    {"show_product_version",
     (char *)offsetof(System_status_var,
                      com_stat[(uint)SQLCOM_SHOW_PRODUCT_VERSION]),
     SHOW_LONG_STATUS, SHOW_SCOPE_ALL},
...
int init_common_variables() {
  ...
  */
  mysql_bin_log.set_psi_keys(
      ...
      key_BINLOG_LOCK_binlog_end_pos, key_BINLOG_LOCK_redo,
      key_BINLOG_LOCK_sync, key_BINLOG_LOCK_sync_queue, key_BINLOG_LOCK_xids,
      key_BINLOG_LOCK_tids, key_BINLOG_LOCK_log_info,
  ...
  // wal replication
  mysql_wal.set_psi_keys(key_LOCK_wal_flushed_lsn, key_COND_wal_update_cond);
#endif
  ...
  */
  ...
  // wal replication
  mysql_wal.init_pthread_objects();
...
#define SUPER_LARGE_PAGESIZE (256 * 1024 * 1024) /* 256MB */
  ...
  */
  ...
  if (opt_oma_log && opt_oma_logname && !(log_output_options & LOG_FILE) &&
      !(log_output_options & LOG_NONE))
    LogErr(WARNING_LEVEL, ER_LOG_FILES_GIVEN_LOG_OUTPUT_IS_TABLE,
           "--oma_log_file option");
  ...
  if (opt_oma_logname &&
      !is_valid_log_name(opt_oma_logname, strlen(opt_oma_logname))) {
    LogErr(ERROR_LEVEL, ER_LOG_FILE_INVALID, "--oma_log_file", opt_oma_logname);
    return 1;
  }
...
#define FIX_LOG_VAR(VAR, ALT) \
  ...
  FIX_LOG_VAR(opt_oma_logname,
              make_query_log_name(oma_logname_path, QUERY_LOG_OMA));
  ...
  // init reserved users
  if ((rds_reserved_users_init() != 0) ||
      (rds_reserved_users_update(rds_reserved_users_ptr) != 0)) {
    sql_print_error(
        "An error occurred while storing rds_reserved_users to a hash.");
    return 1;
  }
  /* init restricted dynamic privileges list */
  if (rds_restricted_dynamic_privileges_list_init_and_reload() != 0) {
    sql_print_error(
        "An error occurred while storing "
...
static int init_thread_environment() {
  ...
  mysql_rwlock_init(key_rwlock_LOCK_restricted_dynamic_privileges_list,
  ...
  mysql_mutex_init(key_LOCK_purge_binlog, &LOCK_purge_binlog,
                   MY_MUTEX_INIT_FAST);
  mysql_cond_init(key_COND_purge_binlog, &COND_purge_binlog);
  Events::init_mutexes();
#if defined(_WIN32)
...
static int init_server_components() {
  ...
  */
  ...
  /* We try to recover XID from existing XID storage file if needed. */
  if (storage_engine_mode == ONLY_DSTORE && rds_dstore_enable_atomic_ddl &&
      restore_xid_from_file_store() != DDL_INFO_OP_SUCCESS) {
    unireg_abort(MYSQLD_ABORT_EXIT);
  }
  ...
  */
  ...
  if (!opt_initialize && !opt_noacl) {
    ...
    // check storage_engine_mode, disabled_storage_engine and
    // default_storage_engine.
    if (ha_check_storage_engine_mode(storage_engine_mode,
                                     default_storage_engine)) {
      LogErr(ERROR_LEVEL, ER_STORAGE_ENGINE_MODE_ERROR,
             storage_engine_mode_names[storage_engine_mode],
             opt_disabled_storage_engines, default_storage_engine);
      unireg_abort(MYSQLD_ABORT_EXIT);
    }

    // For ONLY_DSTORE, do not allow to use REPEATABLE-READ transaction
    // isolation.
    if (storage_engine_mode == static_cast<ulong>(ONLY_DSTORE) &&
        global_system_variables.transaction_isolation == ISO_REPEATABLE_READ) {
      LogErr(ERROR_LEVEL, ER_DSTORE_TRX_CANT_USE_REPEATABLE_READ);
      unireg_abort(MYSQLD_ABORT_EXIT);
    }
  ...
  if (total_ha_2pc > 1 || (1 == total_ha_2pc && opt_bin_log)) {
    if (opt_bin_log)
      ...
      assert(plugin_is_ready({"Dstore", 6}, MYSQL_STORAGE_ENGINE_PLUGIN) ||
             storage_engine_mode == static_cast<ulong>(ONLY_INNODB));
      tc_log = storage_engine_mode != static_cast<ulong>(ONLY_INNODB)
                   ? static_cast<TC_LOG *>(&tc_log_dstore)
                   : static_cast<TC_LOG *>(&tc_log_mmap);
    }
  }
...
#endif
{
  ...
  */
  ...
  /* Init sql filter system */
  sqlfilter_init();
  /* Init outline system */
  im::outline_init();

  /* Init recycle_bin system */
  im::recycle_bin::recycle_scheduler_init();
  ...
  */
  ...
  if (rds_scc_conf != NULL) {
    if ((scc_init_with_conf_retry(rds_scc_conf, SCC_INIT_MAX_RETRY))) {
      LogErr(WARNING_LEVEL, ER_RDS_SCC_INIT);
      /*
        SCC itself will not abort by initialization fail.
        Abort by modules dependent on SCC, like TDE
      */
      rds_scc_initialized = false;
    } else {
      rds_scc_initialized = true;
      sql_print_information("Secrypto component load success");
    }
  }
...
#ifdef WITH_JEMALLOC_HEADER
  if (!get_background_thread()) {
    /** re-create background threads while
     * grand child cleanup background threads meta-data
     * (e.g. cond) postfork properly.
     */
    switch_background_thread(true);
    check_background_thread(true);
  }
#endif
  ...
  */
  ...
  if (opt_bin_log) {
    ...
    */
    ...
    if (rds_dstore_support_binlog_check && !rds_write_binlog_into_redo &&
        (storage_engine_mode == ONLY_DSTORE ||
         (storage_engine_mode == BOTH_STORAGE_ENGINE &&
          get_instance_storage_engine_type() == DB_TYPE_INNODB))) {
      check_binlog_dstore_consistent();
    } else {
      rds_binlog_dstore_consistent = true;
    }
    if (get_instance_storage_engine_type() == DB_TYPE_DSTORE &&
        !rds_write_binlog_into_redo && rds_dstore_support_binlog_check &&
        !opt_binlog_order_commits && !Clone_handler::need_commit_order() &&
        global_gtid_mode.get() == Gtid_mode::ON) {
      ha_init_max_start_recovery_point();
    }
  }
  ...
  */
  ...
  initialize_mysql_db_acl();
  initialize_sys_db_acl();
  statement_sqlfilter_init(opt_initialize);
  initialize_recyclebin_schema_acl();
  ...
  */
  ...
  LogErr(SYSTEM_LEVEL, ER_JEMALLOC_TYPE, my_progname, MALLOC_LIBRARY);
  if (opt_bin_log && rds_purge_binary_logs_asynchronously) {
  ...
  */
  ...
  VfdManager::RemoveLeftoverFiles();
  ...
  if (rds_use_ddl_info_replay) {
    restore_replay_ddl_info();
  }
  rm_ddl_info();
...
static void adjust_open_files_limit(ulong *requested_open_files) {
  ...
  limit_1 = 10 + max_connections + max_rds_reserved_connections +
            opt_rds_admin_max_connections + table_cache_size * 2;
  ...
  /*
    ...
    be available).
  */
  limit_2 = (max_connections + max_rds_reserved_connections +
...
struct my_option my_long_options[] = {
    ...
    */
    ...
    {"transaction-isolation", 0, "Default transaction isolation level.",
     ...
     GET_ENUM, REQUIRED_ARG, ISO_READ_COMMITTED, 0, 0, nullptr, 0, nullptr},
...
static int show_sql_filter_block_num(THD *, SHOW_VAR *var, char *buff) {
  var->type = SHOW_LONG;
  var->value = buff;
  *((long *)buff) = (long)sql_filter_block_num.load(std::memory_order_relaxed);
  return 0;
}
...
SHOW_VAR status_vars[] = {
    ...
    {"Slave_open_temp_tables", (char *)&show_replica_open_temp_tables,
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"Sql_filter_block_num", (char *)&show_sql_filter_block_num, SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
#ifndef NDEBUG
    ...
    {"Admin_threads_connected",
     (char *)&Connection_handler_manager::rds_admin_connection_count, SHOW_INT,
     SHOW_SCOPE_GLOBAL},
    {"Reserved_threads_connected",
     (char *)&Connection_handler_manager::rds_reserved_connection_count,
     SHOW_INT, SHOW_SCOPE_GLOBAL},
...
static int mysql_init_variables() {
  ...
  opt_general_logname = opt_binlog_index_name = opt_slow_logname =
      opt_oma_logname = nullptr;
...
static PSI_mutex_info all_server_mutexes[]=
{
  ...
  { &key_BINLOG_LOCK_redo, "MYSQL_BIN_LOG::LOCK_redo", 0, 0, PSI_DOCUMENT_ME},
  ...
  { &key_LOCK_purge_binlog, "MYSQL_BIN_LOG::purge", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
  ...
  { &key_LOCK_wal_flushed_lsn, "MYSQL_WAL::m_lock_wal_flushed_lsn", 0, 0, PSI_DOCUMENT_ME},
...
PSI_rwlock_key key_rwlock_table_share_lock_m_histogram;
PSI_rwlock_key key_rwlock_Wallog_IO_delegate_lock;
PSI_rwlock_key key_rwlock_Wallog_transmit_delegate_lock;
PSI_rwlock_key key_rwlock_Wallog_trx_delegate_lock;
PSI_rwlock_key key_rwlock_LOCK_restricted_dynamic_privileges_list;
...
static PSI_rwlock_info all_server_rwlocks[]=
{
  ...
  { &key_rwlock_table_share_lock_m_histogram, "TABLE_SHARE::LOCK_m_histograms", 0, 0, PSI_DOCUMENT_ME},
  { &key_rwlock_Wallog_IO_delegate_lock, "Wallog_IO_delegate::lock", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
  { &key_rwlock_Wallog_transmit_delegate_lock, "Wallog_transmit_delegate::lock", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
  { &key_rwlock_Wallog_trx_delegate_lock, "Wallog_trx_delegate::lock", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
  {&key_rwlock_LOCK_restricted_dynamic_privileges_list, "LOCK_restricted_dynamic_privileges_list", PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
#ifdef _WIN32
...
static PSI_cond_info all_server_conds[]=
{
  ...
  { &key_COND_purge_binlog, "MYSQL_BIN_LOG::purge", 0, 0, PSI_DOCUMENT_ME},
  ...
  { &key_COND_wal_update_cond, "MYSQL_WAL::m_wal_update_cond", 0, 0, PSI_DOCUMENT_ME},
...
PSI_thread_key key_thread_purge_binlog;
...
PSI_FLAG_USER | PSI_FLAG_NO_SEQNUM, 0, PSI_DOCUMENT_ME},
  ...
  { &key_thread_purge_binlog, "purge_expired_binary_logs","purge_expired",PSI_FLAG_SINGLETON, 0, PSI_DOCUMENT_ME},
...
PSI_stage_info stage_logging_oma = { 0, "logging oma", 0, PSI_DOCUMENT_ME };
...
PSI_stage_info stage_requesting_wallog_dump= { 0, "Requesting Wallog dump", 0, PSI_DOCUMENT_ME};
PSI_stage_info stage_waiting_for_source_to_send_wallog= { 0, "Waiting for source to send Wallog", 0, PSI_DOCUMENT_ME};
PSI_stage_info stage_write_source_wallog_to_file= { 0, "Writing source Wallog to the log file", 0, PSI_DOCUMENT_ME};
PSI_stage_info stage_rpl_wal_wait_flush= { 0, "Waiting for wal flush on master", 0, PSI_DOCUMENT_ME};
/* clang-format on */
...
PSI_stage_info *all_server_stages[] = {
    ...
    &stage_requesting_wallog_dump,
    &stage_waiting_for_source_to_send_wallog,
    &stage_write_source_wallog_to_file,
    &stage_rpl_wal_wait_flush};
...
static void init_server_psi_keys(void) {
  ...
  */
  ...
  mysql_statement_register(category, &com_statement_info[(int)COM_QUERY + 1],
                           count);
  /*
    Register [COM_CLONE + 1 .. COM_GAP_BEG - 1] as "statement/com/..."
  */
  count = (int)COM_GAP_BEG - (int)COM_CLONE - 1;
  mysql_statement_register(category, &com_statement_info[(int)COM_CLONE + 1],
                           count);

  /*
    Register [COM_STMT_BULK_EXECUTE] as "statement/com/bulk_execute"
  */
  mysql_statement_register(category,
                           &com_statement_info[(int)COM_STMT_BULK_EXECUTE], 1);

  /*
    Register [COM_END] as "statement/com/error"
  */
  mysql_statement_register(category, &com_statement_info[(int)COM_END], 1);

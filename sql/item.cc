...
#include "sql/parallel_query/pq_resolver.h"  // resolve_item_in_base_ref_items
...
void Item_ident::print(const THD *thd, String *str, enum_query_type query_type,
  ...
  if (lower_case_table_names == 1 ||
    ...
    if (!(query_type & QT_NO_TABLE) && table_name_arg && table_name_arg[0]) {
      my_stpncpy(t_name_buff, table_name_arg, MAX_ALIAS_NAME - 1);
      t_name_buff[MAX_ALIAS_NAME - 1] = '\0';
      my_casedn_str(files_charset_info, t_name_buff);
      t_name = t_name_buff;
    }
    if (!(query_type & QT_NO_DB) && db_name_arg && db_name_arg[0]) {
      my_stpncpy(d_name_buff, db_name_arg, MAX_ALIAS_NAME - 1);
      d_name_buff[MAX_ALIAS_NAME - 1] = '\0';
...
bool Item_field::eq(const Item *item, bool) const {
  return eq_with_binary_cmp_arg(item, true);
}

bool Item_field::eq_with_binary_cmp_arg(const Item *item,
  ...
  */
  if (binary_cmp && fixed && item_field->fixed)
  ...
  /*
    ...
    When we clone resolved Item_field, their item_name might
    be not equal to field_name, e.g.,
       `SELECT a AS d,SUM(b) FROM t1`, Item_field (a) has field_name "a" and
       item_name "d".
    we should compare their item_name and field_name. We also believe that
    when the item_name and field_name of two items are equal, these two items
    should be equal in one query block because duplicate names in one query
    block are not allowed.
    when exist derived table, item_field->db_name may be empty string, see
    Item_field::set_field, table_ref->db may be empty string, e.g.
    SELECT 1 FROM (SELECT c0, 1 c1 FROM t0) t0 JOIN t0 USING (c0) GROUP BY
    t0.c0;
  */
  bool is_derived = item_field->field && item_field->field->table &&
  ...
  const char *cmp_db_name =
      (is_derived && item_field->field->orig_db_name && db_name &&
       !strcmp(item_field->field->orig_db_name, db_name))
          ? item_field->field->orig_db_name
          : item_field->db_name;
  return (item_field->item_name.eq_safe(field_name) ||
          (item_field->item_name.eq_safe(item_name) && item_field->field_name &&
           field_name && !strcmp(item_field->field_name, field_name))) &&
         (!item_field->table_name || !table_name ||
          (!my_strcasecmp(table_alias_charset, item_field->table_name,
                          table_name) &&
           (!item_field->db_name || !db_name ||
            (cmp_db_name && !strcmp(cmp_db_name, db_name)))));
}

table_map Item_field::used_tables() const {
  if (!table_ref) return 1;  // Temporary table; always table 0
  // const item
  if (table_ref->table && table_ref->table->const_table) return 0;
...
static bool resolve_ref_in_select_and_group(THD *thd, Item_ident *ref,
  ...
  *found = nullptr;
  ...
  // When fields has already been resolved (for parallel query),
  // we should be able to find "ref[0]" in fields.
  if (select->pq_is_clone()) {
    return resolve_item_in_base_ref_items(
        thd, select->base_ref_items, select->get_fields_list(), counter, ref);
  }
  ...
  for (; outer_context; outer_context = outer_context->outer_context) {
    ...
    */
    ...
    if (select_alias_referencable(place) &&
        ...
        pq_can_resolve_Item_field_in(this, select)) {
...
bool Item_view_ref::eq(const Item *item, bool) const {
  if (item->type() == REF_ITEM) {
    ...
    if (item_ref->ref_type() == VIEW_REF) {
      ...
      // when cloning view_ref, we also clone the referred item ref[0].
      // Thus, we cannot directly use the "pointer" address to determine
...
void Item_default_value::bind_fields() {
  ...
  // For a prepared query with a DEFAULT(col), at end of executing
  // 'prepare stmt from ..', it will close tables if table_open_cache is small.
  // And when executing 'execute stmt', it firstly open all tables but it does
  // not refix items in thd->stmt_arena->item_list(). As a result, field->table
  // is not updated and table has been released before. So it must bind field
  // to the current TABLE object.
  if (table_ref) field->table = table_ref->table;
...
*/
// We use this function in reduce_cond_for_table in sql_optimizer.cc
// and we rely on it staying the way it is. Should modifications be made to that
// function, we would need to carefully evaluate their impact on the behavior of
// reduce_cond_for_table, we therefore put a canon in place as a safeguard:
// HUAWEI_ASSERT_MATCH_CANON stored_field_cmp_to_item
...
void Item_cache_datetime::store_value(Item_cache_datetime *orig_item) {
  value_cached = true;
  int_value = orig_item->int_value;

...
bool Explain_table_base::explain_extra_common(int range_scan_type, uint keyno) {
  if (table->file->get_pushed_offset() != 0) {
    if (push_extra(ET_USING_OFFSET_PUSHDOWN)) return true;
  }
  ...
  */
  ...
  if (tab && tab->m_aggregate_context && push_extra(ET_USING_AGGREGATE)) {
    return true;
  }
  return false;
}
...
bool Explain_join::shallow_explain() {
  ...
  for (size_t t = 0, cnt = fmt->is_hierarchical() ? join->primary_tables
    ...
    if (join->qep_tab[t].gather) break;
  }

  /*
    When format is hierarchical text, only explain primary_tables qep_tab
    above. But for parallel query, primary_tables of leader's plan is zero,
...
bool Explain_join::explain_qep_tab(size_t tabnum) {
  ...
  enum_parsing_context c = sjm ? CTX_MATERIALIZATION : CTX_QEP_TAB;

  if (tab->gather) {
    need_tmp_table = false;
    need_order = false;
    // This is qep_tab of leader in parallel query
    c = CTX_PQ;
  }
...
bool Explain_join::explain_extra() {
  ...
  /*
    If there is only const items in the SELECT list, tmp table of leader
    has no fields, its read_set->n_bits is zero which can not be checked
    by bitmap_is_clear_all(). For example:
        SELECT 1 FROM t

    At same time, there is no need to explain "used_columns" of leader.
    So skip it in leader here.
  */
  if (fmt->is_hierarchical() && !tab->gather &&
  ...
  if (tab->gather) {
    ...
    buff.append(" workers");
    for (auto tabType : tab->gather->tab_set) {
      buff.append(", ");
      buff.append(tab->gather->pq_tabs[tabType].m_table->s->db.str);
      buff.append(".");
      buff.append(tab->gather->pq_tabs[tabType].m_table->alias);
      /*
        For hierarchical EXPLAIN we output "cut_table" and "divided_table"
        in leader's plan.
      */
      if (fmt->is_hierarchical() &&
          ((tabType == CUT_TAB &&
            fmt->entry()->col_cut_table_name.set(
                tab->gather->pq_tabs[CUT_TAB].m_table->alias)) ||
           (tabType == DIV_TAB &&
            fmt->entry()->col_divided_table_name.set(
                tab->gather->pq_tabs[DIV_TAB].m_table->alias))))
        return true;
    }
    if (!fmt->is_hierarchical() && push_extra(ET_PARALLEL_EXE, buff))
      return true;
    fmt->entry()->col_pq_dop.set(tab->gather->m_dop);
  }

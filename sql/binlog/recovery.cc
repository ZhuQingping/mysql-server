...
binlog::Binlog_recovery &binlog::Binlog_recovery::recover() {
  ...
  if (!this->m_is_malformed && total_ha_2pc > 1) {
    ...
    // If the server running in DStore mode, then we will not use
    // m_internal_xids to coordinator DML transactions. Howerver, we will use
    // this mechanism to coordinator the DDL transactions which need 2PC commit
    // as one DDL will modify both innodb and dstore in a transaction. The
    // consistency(of SE and binlog) of DML will be resolved by other means.
    this->m_no_engine_recovery =
        ha_recover((storage_engine_mode == static_cast<int>(ONLY_DSTORE) &&
                    rds_dstore_enable_atomic_ddl)
                       ? &g_restore_xid_map
                       : &this->m_internal_xids,
                   &xa_list);

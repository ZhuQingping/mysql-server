...
#define IS_LAST_STMT_ID(id) (id == 0xFFFFFFFF)
...
class Prepared_statement final {
  ...
  */
  ...
  struct {
    ...
    PS_PARAM *parameters{nullptr};
  } m_bulk;  ///< groups variables used by COM_STMT_BULK_EXECUTE
  ...
  */
  ...
  // Disable trsnaform to subq in re-prepare if execute failed.
 ...
 public:
  ...
  bool set_parameters_bulk(THD *thd, String *expanded_query);
  void trace_parameter_types(THD *thd);
  void close_cursor();
  bool more_bulk_iterations() const { return m_bulk.packet_left != 0; }
 ...
 private:
  ...
  bool insert_parameters(THD *thd, String *query, bool has_new_types,
                         PS_PARAM *parameters);
  bool insert_parameters_bulk(THD *thd, String *query);
};

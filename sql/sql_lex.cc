...
#include "sql/sql_plan_cache.h"  // plan_cache:is_ready
  ...
  if (nested_join) {
    ...
    if (my_strcasecmp(table_alias_charset, cmp_name, alias)) {
      ...
      if (lower_case_table_names == 1) {
        ...
        {
          my_stpncpy(t_alias_buff, alias, MAX_ALIAS_NAME - 1);
          t_alias_buff[MAX_ALIAS_NAME - 1] = '\0';
...
void Query_expression::store_limit() {
  saved_offset_limit_cnt = offset_limit_cnt;
  saved_select_limit_cnt = select_limit_cnt;
}

void Query_expression::restore_limit() {
  offset_limit_cnt = saved_offset_limit_cnt;
  select_limit_cnt = saved_select_limit_cnt;
}
/**
...
void Query_expression::accumulate_used_tables(table_map map) {
  ...
  if (item)
    ...
    m_lateral_deps |= (map & ~PSEUDO_TABLE_BITS);
}
...
void Query_block::restore_cmd_properties() {
  ...
  assert(join == nullptr || plan_cache::is_ready(this));

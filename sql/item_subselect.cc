...
bool Item_subselect::fix_fields(THD *thd, Item **ref) {
  ...
  if ((substype() != SINGLEROW_SUBS) && (substype() != EXISTS_SUBS)) {
    /*
      Subqueries can be transformed from "ANY/ALL/SOME" in two cases:
        1) transform "ANY/ALL" to "SELECT (MIN)". Then one item_sum is formed to
           evaluate the aggregation. As PQ leader has to rebuild the item_sum
           and the upper query needs it, it is complex for our current PQ clone
           paradigm and we disable it.
        2) transform "ANY/ALL" to "MIN(SELECT)". Then one item_maxmin_subselect
           is formed as unit->item. As we have not cloned this item, we should
           also disable PQ. Then, subquery_suite_for_parallel_query() can
           capture it.

        the line below disables PQ for the subquery; and PQ is also disabled for
        the outer query by having Item_func_not_all in the list of non-clonable,
...
bool Item_subselect::exec(THD *thd) {
  ...
  */
  ...
  // If the subquery has been executed or assigned, PQ worker can directly
  ...
  if ((unit->is_executed() || assigned()) &&
    ...
    assert(unit->subquery_suite_for_parallel_query() ==
           PQSubqueryExecution::kOnceByLeader);
    switch (substype()) {
      case SINGLEROW_SUBS: {
#ifndef NDEBUG
        auto origin_item = down_cast<Item_singlerow_subselect *>(unit->item);
        auto cloned_item = down_cast<Item_singlerow_subselect *>(this);
        assert(origin_item && cloned_item);
        for (uint i = 0; i < max_columns; i++) {
          assert(origin_item->element_index(i) ==
                 cloned_item->element_index(i));
        }
#endif  // NDEBUG
      } break;
      case EXISTS_SUBS: {
        // synchronize value
        auto origin_item = down_cast<Item_exists_subselect *>(unit->item);
        auto cloned_item = down_cast<Item_exists_subselect *>(this);
        cloned_item->pq_share_value(origin_item);
      } break;
      default:
        assert(false);
    }
    if (unit->item->assigned()) assigned(true);
    return false;
  }
...
void Item_subselect::get_item_params(const THD *thd,
  ...
  /*
    ...
    SELECT #1 (SELECT #2 (SELECT #3 t2.a+t1.a FROM t3) FROM t2) FROM t1
    imagine we are setting up PTRC to cache the result of #2. We walk #2
    to find references to #1 (cache keys). These may be in #3 too, so the
    walk uses WALK_SUBQUERY. When we are in #3, we find t2.a ; as it is an
    outer reference it has depended_from!=0 (==#2). But we must not
    collect it, as it is not outer _to_#2_.

    IN(subquery) deserves a special discussion. In something like

    (t1.col+(SELECT t1.col2 FROM ..)) IN (SELECT ...),

    assuming we want to cache the result of IN, t1.col and t1.col2 must be
    found as "cache keys". Item_in_subselect::walk() will travel left_expr and
    Item_subselect; its walk on left_expr will look for references which are
    higher-up than the subquery's unit, thus will find t1.col2 and t1.col1.
  */
  WalkItem(this, enum_walk::PREFIX | enum_walk::SUBQUERY,
           [params](Item *item_inner) {
             ...
             if (item->type() == Item::FIELD_ITEM) {
               ...
               /*
                 This test is more reliable than depended_from, which is
                 sometimes set only for Item_ref and not the underlying
                 Item_field-s.
               */
               if (item_field->table_ref &&
                   item_field->table_ref->query_block->nest_level <
                       params->nest_level) {
                 // Tables might come from father or grandfather etc.
                 Query_block *sl = item_field->table_ref->query_block;
                 Query_block *pl = params->first_select;
                 while (pl) {
                   if (pl != sl)
                     pl = pl->outer_query_block();
                   else
                     break;
                 }
                 if (pl) params->parameters.insert(item_field->field);
               }
             }
             return false;
           });
...
inline bool Item_singlerow_subselect::is_value_shared() {
  return !unit->uncacheable && cloned_origin_item;
}

void Item_singlerow_subselect::reset() {
  null_value = true;
  if (!is_value_shared() && value) value->null_value = true;
}
...
bool Item_subselect::eq(const Item *item, bool cmp) const {
  if (this == item) return true;

  /*
    Outside of PQ, Item_subselect are never cloned, so the test above is good
    enough. In PQ, things are more complicated, as there are clones.
    super::eq() is not very good as it uses name comparison and subquery items
    don't always have names. So we try a little harder: if one item is a clone
    of the other, they are the same.
  */
  if (item->type() != Item::SUBSELECT_ITEM) return false;

  auto compared_item = down_cast<const Item_subselect *>(item);

  if (this->substype() != compared_item->substype()) return false;

  if (this->m_pq_last_clone.second == compared_item ||
      compared_item->m_pq_last_clone.second == this)
    return true;

  return super::eq(item, cmp);
}

void SubqueryWithResult::cleanup() {
  DBUG_TRACE;
  // Because of when filed is const_item(const value and no table),
  // make_pq_worker_plan does't set result_field, SubqueryWithResult::result
  // is nullptr, However make_pq_worker_plan push back subselect_item to
  // tmp_fields0. when pq_worker_exec free join, causing segv.
  // e.g. SELECT (SELECT 1 WHERE 2 IS NULL) FROM t0 GROUP BY t0.c0;
...
void SubqueryWithResult::fix_length_and_dec(Item_cache **row) {
  ...
  for (Query_block *sl = unit->first_query_block(); sl;
       sl = sl->next_query_block()) {
    if (guaranteed_one_row(sl) && !sl->wrapped_in_intersect_except()) {
...
Item_subselect *Item_subselect::pq_clone_common(THD *thd, Query_block *select) {
  ...
  if (!new_item || new_item->pq_copy_from(thd, select, this)) {
    return nullptr;
  }
  if (unit->uncacheable) {
    // Correlated subq uses the unit's _copy_ made by top unit's
    // pq_dup_select().
    if (auto new_query_block = unit->first_query_block()->pq_last_clone()) {
      auto new_unit = new_query_block->master_query_expression();
      // check that this new unit is not yet taken:
      assert(new_unit && new_unit->item == nullptr);
      new_item->unit = new_unit;
      // test consistency of the matching of units:
      assert(new_unit->first_query_block()->select_number ==
             unit->first_query_block()->select_number);
      new_unit->item = new_item;
    } else {
      /*
        This means we have not found a unit to link to. It is abnormal. But it
        can happen: if we have WHERE subq OR true_condition, WHERE is copied
        to saved_where_cond at the end of resolution, then WHERE is emptied
        during optimization, thus the subquery is _not_ validated for PQ when
        we decide if we want to do PQ, then in pq_dup_select() the unit is not
        cloned (as it's not suitable for cloning it's skipped by
        list_query_blocks_to_clone(), it is treated as a "non-PQ subquery")),
        then we clone saved_where_cond (instead of where_cond - alas), which
        still contains the subquery.
      */
      return nullptr;  // failure, retry without PQ if possible.
    }
  }

  // clone subquery
  if (subquery) {
    auto new_subquery_with_result = subquery->pq_clone(thd, new_item);
    if (!new_subquery_with_result) return nullptr;
    new_item->subquery.reset(new_subquery_with_result);
    if (unit->uncacheable) {
      Query_result_subquery *new_query_result;
      switch (substype()) {
        case SINGLEROW_SUBS:
          new_query_result =
        ...
        default:
          assert(false);
          new_query_result = nullptr;
      }
      if (!new_query_result || new_item->subquery->change_query_result(
                                   thd, new_item, new_query_result))
        return nullptr;
    }
  }
  return new_item;

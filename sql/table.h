...
using Table_ref_ptr = std::unique_ptr<Table_ref>;
/**
...
struct ORDER {
  ...
  */
  ...
  bool convert_to_list(SQL_I_List<ORDER> &list, THD *thd);
};
...
/*
  Definition of Guard of mysql_rwlock_t. it will unlock the mutex when guard
  destructor.
*/
class Histogram_rdlock_guard {
 public:
  Histogram_rdlock_guard(mysql_rwlock_t *mutex) : m_mutex(mutex) {
    mysql_rwlock_rdlock(m_mutex);
  }
  ~Histogram_rdlock_guard() { mysql_rwlock_unlock(m_mutex); }

 private:
  mysql_rwlock_t *m_mutex;
};
...
struct HistogramAndMemRoot {
  HistogramAndMemRoot() : mem_root(key_memory_histograms, /*block_size=*/512) {}

  histograms::Histogram *histogram{nullptr};

  // The MEM_ROOT that the above histogram is allocated on
  MEM_ROOT mem_root;
};
...
struct TABLE_SHARE {
  ...
  */
  malloc_unordered_map<uint, HistogramAndMemRoot> *m_histograms{nullptr};

  /*
    Previous issue:
    When we trigger ANALYZE TABLE ... UPDATE HISTOGRAM.
    we explicitly evict the TABLE_SHARE from the table definition cache so that
    the next query will re-load the TABLE_SHARE. and thus get the newest
    histogram. When we evict the TABLE_SHARE from the table definition cache.
    all existing users of the TABLE_SHARE must finish before new queries are
    allowed to load a new TABLE_SHARE. and this is what's causing the
    waits/locks.

    So our solution:
    1) Do not evict the TABLE_SHARE when a histogram is updated.
    2) push the new histogram directly to m_histograms.
    So we need to add a mutex to protect access to m_histograms
  */
  mysql_rwlock_t LOCK_m_histograms;
...
struct TABLE {
  ...
  */
  ...
  // Temp table created for shared leader's records
  bool pq_shared_table{false};
  ...
  */
  ...
  /**
    Backup const_table values to enable fallback when switching from parallel
    to serial execution.
  */
  bool pq_saved_const_table{false};
...
class Table_ref {
  ...
  */
  ...
  bool is_base_table() const {
    return !(is_view_or_derived() || is_table_function() ||
             is_recursive_reference() || is_transformed_in_predicate());
  }
  /**
  ...
  */
  bool is_placeholder() const {
    return is_view_or_derived() || is_recursive_reference() ||
           is_table_function() || is_transformed_in_predicate() ||
           schema_table || table == nullptr;
  }
  ...
  */
  ...
  void set_transformed_in_predicate(Item *in_predicate) {
    transformed_in_predicate = in_predicate;
  }

  Item *get_transformed_in_predicate() const {
    DBUG_ASSERT(transformed_in_predicate);
    return transformed_in_predicate;
  }
  ...
  */
  bool process_index_hints(THD *thd, TABLE *table);
  ...
  */
  ...
  List<Index_hint> *outline_index_hints{nullptr};
 ...
 public:
  Key_map get_covering_keys_saved() const { return covering_keys_saved; }
};
...
/* sys schema */
extern LEX_CSTRING SYS_SCHEMA_NAME;
...
inline bool is_mysql_db(const char *name) {
  return !my_strcasecmp(system_charset_info, MYSQL_SCHEMA_NAME.str, name);
}

/**
  Check if sys db
*/
inline bool is_sys_db(const char *name) {
  return !my_strcasecmp(system_charset_info, SYS_SCHEMA_NAME.str, name);
}
...
//   2. If we have a NULL row, calling position() would give a random row ID
//      back, as there has not been any row read.
//

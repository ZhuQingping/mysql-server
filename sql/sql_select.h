...
class Key_use {
  ...
  */
  ...
  // Dstore use: a covering index with primary key as a prefix is
  // chosen instead of the primary key.
  bool m_promoted_covering_index{false};

  Key_use *pq_clone(Key_use *new_key_use) {
    *new_key_use = Key_use(nullptr, nullptr, used_tables, key, keypart,
                           optimize, keypart_map, ref_table_rows,
                           null_rejecting, cond_guard, sj_pred_no);
...
struct POSITION {
  ...
  */
  ...
  [[nodiscard]] bool pq_copy(THD *thd, QEP_TAB *orig_tab);
...
void offset_pushdown(JOIN *join);

...
class SJ_TMP_TABLE {
 public:
  SJ_TMP_TABLE_TAB *tabs{nullptr};
  SJ_TMP_TABLE_TAB *tabs_end{nullptr};
  ...
  */
  ...
  Field_longlong *hash_field{nullptr};
};
 ...
 public:
  Semijoin_mat_exec(Table_ref *sj_nest, bool is_scan, uint table_count,
      ...
      : sj_nest(sj_nest),
        ...
        table_param(),
        table(nullptr),
        m_pq_shared_info(pq_shared_info) {}
  ~Semijoin_mat_exec() {}
  ...
  /// Points to information necessary for PQ workers to share the temporary
  /// table. Nullptr if inapplicable.
  PQ_shared_info **m_pq_shared_info{nullptr};
  Semijoin_mat_exec *pq_clone(THD *thd, Query_block *select);
};
...
class QEP_TAB : public QEP_shared_owner {
 public:
  ...
  void set_split_table(TABLE *t) { m_qs->set_split_table(t); }
  ...
  */
  ...
  [[nodiscard]] bool pq_copy(THD *thd, QEP_TAB *qep_tab);

  bool pushdown_aggregate();
  ...
  */
  ...
  enum enum_op_type {
    ...
    // Leader's temp table which read record from message queue.
    OT_RECV_MSG_TMP_TABLE,
  ...
  */
  ...
  Aggregate_context *m_aggregate_context;
  ...
  bool pq_div_tab{false};
  bool pq_cut_tab{false};
...
/*
 * This class is used to cache aggregate context
 *
 * The pointer to this class is cached in
 * QEP_TAB and the correspoinding handler, so that
 * the SQL layer and the storage engine can access
 * the same information
 */
class Aggregate_context {
 public:
  JOIN *m_join;
  AggregateIterator *m_aggr_iter;
  UnqualifiedCountIterator *m_count_iter;
  Aggregate_context(JOIN *join)
      : m_join(join), m_aggr_iter(nullptr), m_count_iter(nullptr) {}
  friend bool aggregate_in_engine(Aggregate_context *ctx);
  friend bool unqualified_count_in_engine(Aggregate_context *ctx);
};

...
bool PT_base_part_definition::contextualize(Partition_parse_context *pc) {
  if (super::contextualize(pc)) return true;

  partition_info *const part_info = pc->part_info;
  auto *curr_part = pc->curr_part_elem;
  ...
  switch (type) {
    case partition_type::HASH: {
      if (part_info->part_type == partition_type::NONE) {
        ...
        errorf(pc, pos, ER_THD(pc->thd, ER_PARTITION_REQUIRES_VALUES_ERROR),
               "RANGE", "LESS THAN");
        return true;
    ...
    case partition_type::RANGE: {
      ...
      if (opt_part_values == nullptr)  // MAX_VALUE_SYM
      {
        ...
        if (pc->init_column_part() || pc->add_max_value()) return true;
      } else if (opt_part_values->contextualize(pc))
        return true;
    } break;
    case partition_type::LIST: {
      ...
      if (opt_part_values->contextualize(pc)) return true;
    } break;
    default:
      assert(false);
      error(pc, pos, ER_THD(pc->thd, ER_UNKNOWN_ERROR));
      return true;
  }

  if (opt_part_options != nullptr) {
    for (auto option : *opt_part_options) {
      if (option->contextualize(pc)) return true;
    }
...
bool PT_subpart_definition::contextualize(Partition_parse_context *pc) {
  sub_partition_info *part_info = pc->part_info->sub_part_info;

  if (!part_info) {
    part_info = new (pc->thd->mem_root) sub_partition_info(pc->part_info);
    pc->part_info->sub_part_info = part_info;
    if (!part_info) return true;
  }
  if (part_info->use_default_partitions &&
      pc->part_info->partitions.elements >= 2) {
    /*
  ...
  if (super::contextualize(&subpart_pc)) return true;
  if (part_info->partitions.push_back(sub_p_elem)) return true;

  if (opt_part_options != nullptr) {
    if (sub_p_elem->engine_type != nullptr)
      pc->part_info->default_engine_type = sub_p_elem->engine_type;
  }
  ...
  if (opt_sub_partitions == nullptr) {
    ...
    if (part_info->has_sub_part) {
      if (part_info->sub_part_info->part_type == partition_type::RANGE) {
        ...
        return true;
      } else if (part_info->sub_part_info->part_type == partition_type::LIST) {
    ...
    if (part_info->get_num_subparts() != 0) {
      if (part_info->get_num_subparts() != ppc.count_curr_subparts) {
        error(&ppc, sub_partitions_pos,
              ER_THD(pc->thd, ER_PARTITION_WRONG_NO_SUBPART_ERROR));
        return true;
      }
      ...
      part_info->sub_part_info->num_parts = ppc.count_curr_subparts;
    }
    Sub_partition *sub_part = new (pc->thd->mem_root) Sub_partition(
        part_info->sub_part_info, part_info->partitions.size() - 1);
    if (!sub_part || part_info->partition.subpartitions.push_back(sub_part))
      return true;
  }
  return false;
}

bool PT_sub_partition::contextualize(Partition_parse_context *pc) {
  if (super::contextualize(pc)) return true;

  if (part_type_def->contextualize(pc)) return true;
...
bool PT_partition::contextualize(Parse_context *pc) {
  ...
  if (opt_sub_part != nullptr) {
    part_info.sub_part_info = &opt_sub_part->part_info;
    part_info.sub_part_info->set_parent(&part_info);
    Partition_parse_context subpart_pc(pc->thd, &opt_sub_part->part_info,
                                       false);
    if (opt_sub_part->contextualize(&subpart_pc)) return true;

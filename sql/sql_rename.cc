...
bool mysql_rename_tables(THD *thd, Table_ref *table_list,
                         std::set<handlerton *> *recycle_post_ddl_htons,
                         Foreign_key_parents_invalidator *rb_fk_invalidator) {
  Table_ref *ren_table = nullptr;
  DBUG_TRACE;

  if (thd->recycle_state->get_type() != im::recycle_bin::RECYCLE_BIN_RECYCLE_DB)
  ...
  */
  ...
  std::set<handlerton *> *post_ddl_htons_ptr =
      recycle_post_ddl_htons ? recycle_post_ddl_htons : &post_ddl_htons;
  Foreign_key_parents_invalidator fk_invalidator;
  Foreign_key_parents_invalidator *fk_invalidator_ptr =
      rb_fk_invalidator ? rb_fk_invalidator : &fk_invalidator;
  /*
    An exclusive lock on table names is satisfactory to ensure
    no other thread accesses this table.
  */
  if ((ren_table = rename_tables(thd, table_list, &int_commit_done,
                                 post_ddl_htons_ptr, fk_invalidator_ptr))) {
    ...
    if (int_commit_done) {
      ...
      */
      int_commit_full_revert = !rename_tables(
          thd, table, &int_commit_done, post_ddl_htons_ptr, fk_invalidator_ptr);
  ...
  if (!error &&
      (thd->recycle_state->get_type() == im::recycle_bin::RECYCLE_BIN_NONE ||
       thd->recycle_state->get_type() ==
           im::recycle_bin::RECYCLE_BIN_RESTORE_TABLE)) {
    ...
    case dd::enum_table_type::BASE_TABLE: {
      ...
      if (!thd->recycle_state->is_recycle()) {
        if (check_table_triggers_are_not_in_the_same_schema(
                ren_table->db, *from_table, new_db))
          return true;
      }
...

{
  ...
  for (ren_table = table_list; ren_table; ren_table = new_table->next_local) {
    new_table = ren_table->next_local;
    DBUG_EXECUTE_IF("rename_last_table_error", {
      if (new_table->next_local == nullptr) {
        my_error(ER_ERROR_ON_RENAME, MYF(0), ren_table->table_name,
                 new_table->table_name, -1, "injected error");
        return ren_table;
      }
    });

    DBUG_EXECUTE_IF("rename_last_table_crash", {
      if (new_table->next_local == nullptr) DBUG_SUICIDE();
    });

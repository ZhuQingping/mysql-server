...
string HashJoinTypeToString(RelationalExpression::Type join_type,
                            ...
                            bool executed_using_parallel_query,
                            bool shared_hash_table) {
  string ret;
  switch (join_type) {
  ...
  if (executed_using_parallel_query) {
    if (shared_hash_table) {
      *explain_json_value += ", hash table shared by worker threads";
      ret += ", hash table shared by worker threads";
    } else {
      *explain_json_value += ", one hash table per worker";
      ret += ", one hash table per worker";
    }
  }
...
bool GetAccessPathsFromItem(Item *item_arg, const char *source_text,
  ...
  return WalkItem(
      item_arg, enum_walk::POSTFIX, [children, source_text](Item *item) {
        ...
        if (query_block->is_dependent()) {
          snprintf_s(description, sizeof(description), sizeof(description) - 1,
                     "Select #%u (subquery in %s; dependent)",
                     query_block->select_number, source_text);
        } else if (!query_block->is_cacheable()) {
          snprintf_s(description, sizeof(description), sizeof(description) - 1,
                     "Select #%u (subquery in %s; uncacheable)",
                     query_block->select_number, source_text);
        } else {
          snprintf_s(description, sizeof(description), sizeof(description) - 1,
                     "Select #%u (subquery in %s; run only once)",
                     query_block->select_number, source_text);
        }
...
bool GetAccessPathsFromSelectList(JOIN *join, vector<ExplainChild> *children) {
  if (join == nullptr) {
    return false;
  }
...
static std::unique_ptr<Json_object> ExplainMaterializeAccessPath(
  ...
  /*
    Save original param and use serial param to print materialize
  ...
  */
  ...
  if (param->limit_rows != HA_POS_ERROR) {
    ...
    if (MaterializeIsDoingDeduplication(param->table)) {
      snprintf_s(heading, sizeof(heading), sizeof(heading) - 1,
                 "Limit table size: %llu unique row(s)", param->limit_rows);
    } else {
      snprintf_s(heading, sizeof(heading), sizeof(heading) - 1,
                 "Limit table size: %llu row(s)", param->limit_rows);
    }
  }
...
static bool PrintRanges(const QUICK_RANGE *const *ranges, unsigned num_ranges,
  ...
  for (unsigned range_idx = 0; range_idx < num_ranges; ++range_idx) {
    if (range_idx == 2 && num_ranges > 3) {
      char str[256];
      snprintf_s(str, sizeof(str), sizeof(str) - 1, " OR (%u more)",
                 num_ranges - 2);
  ...
  if (explain_analyze) {
    ...
    if (path->iterator != nullptr) {
      ...
      if ((num_init_calls = profiler->GetNumInitCalls()) != 0) {
        ...
        std::string parallel_timing_s;
        if (path->iterator->GetParallelTimingString(parallel_timing_s)) {
          error |= AddMemberToObject<Json_string>(obj, "parallel_timing_info",
                                                  parallel_timing_s);
        }
      }
    }
...
static std::unique_ptr<Json_object> SetObjectMembers(
  ...
  switch (path->type) {
    ...
    case AccessPath::REF: {
      ...
      error |= AddChildrenFromPushedCondition(table, children);
      AddChildrenFromIndexKey(table, path->ref().ref, children);
      break;
    }
    case AccessPath::REF_OR_NULL: {
      ...
      error |= SetIndexInfoInObject(
          ...
          obj);
      error |= AddChildrenFromPushedCondition(table, children);
      AddChildrenFromIndexKey(table, path->ref_or_null().ref, children);
      break;
    }
    case AccessPath::EQ_REF: {
      ...
      error |= SetIndexInfoInObject(
          ...
          obj);
      error |= AddChildrenFromPushedCondition(table, children);
      AddChildrenFromIndexKey(table, path->eq_ref().ref, children);
      break;
    }
    ...
    case AccessPath::UNQUALIFIED_COUNT:
      ...
      description += join->qep_tab->table()->file->explain_extra();
      break;
    case AccessPath::NESTED_LOOP_JOIN: {
      ...
      // Keyword distinct means that nested loop join can stop traverse
      // inner table when find one new record.
    ...
    case AccessPath::HASH_JOIN: {
      ...
      const bool uses_shared_hash_table =
    ...
    case AccessPath::SORT: {
      ...
      if (const ha_rows limit = path->sort().limit; limit != HA_POS_ERROR) {
        ...
        snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
                   ", limit input to %llu row(s) per chunk", limit);
        description += buf;
      }
    ...
    case AccessPath::LIMIT_OFFSET: {
      ...
      if (path->limit_offset().offset == 0) {
        if (join && join->tables > join->const_tables &&
          ...
          snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "Limit: %llu row(s)",
                     path->limit_offset().limit);
        }
      } else if (path->limit_offset().limit == HA_POS_ERROR) {
        snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "Offset: %llu row(s)",
                   path->limit_offset().offset);
      } else {
        snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
                   "Limit/Offset: %llu/%llu row(s)",
    ...
    case AccessPath::MATERIALIZE:
      ...
      if (ret_obj == nullptr) return nullptr;
      {
        ...
        if (path->materialize().table_path->type == AccessPath::REF) {
          ref = path->materialize().table_path->ref().ref;
        } else if (path->materialize().table_path->type ==
                   AccessPath::REF_OR_NULL) {
          ref = path->materialize().table_path->ref_or_null().ref;
        } else if (path->materialize().table_path->type == AccessPath::EQ_REF) {
          ref = path->materialize().table_path->eq_ref().ref;
        }
        AddChildrenFromIndexKey(/*only_base_table=*/nullptr, ref, children);
      }
      break;
    ...
    case AccessPath::PARALLEL_SCAN: {
      Gather_operator *_gather = path->parallel_scan().qep_tab->gather;
      assert(_gather);
      string str = string("Gather: ") + std::to_string(_gather->m_dop) +
                   " workers, parallel scan on ";
      for (auto tabType : _gather->tab_set) {
        str += _gather->pq_tabs[tabType].m_table->alias;
        str += ", ";
      }
      str = str.substr(0, str.length() - 2);  // remove last ", "
      description = str;
      children->push_back({_gather->m_template_join->m_root_access_path});
      // Copy iterator timing info from non-parallel execution tree
      // to parallel execution tree.
      if (current_thd->lex->is_explain_analyze) {
        CopyLeaderIterTimingInfo(path->parallel_scan().m_root_access_path,
                                 _gather->m_template_join->m_root_access_path);
      }
    ...
    case AccessPath::PQ_REF_SCAN: {
      ...
      const KEY *key = &table->key_info[path->pq_ref_scan().ref->key];
      string str = string("Parallel");
      error |= SetIndexInfoInObject(
          &description, "index_lookup", str.c_str(), table, key, "lookup",
          RefToString(*path->pq_ref_scan().ref, key, /*include_nulls=*/false),
          /*ranges=*/nullptr, nullptr, path->pq_ref_scan().reverse,
          table->file->pushed_idx_cond, obj);
      AddChildrenFromPushedCondition(table, children);
      AddChildrenFromIndexKey(table, path->pq_ref_scan().ref, children);
      break;
    }
    case AccessPath::PQ_BLOCK_SCAN: {
      auto type = path->pq_block_scan().qep_tab->type();
      TABLE *table = path->pq_block_scan().table;
      if (type == JT_ALL) {
        description += string("Parallel table scan on ") + table->alias;
        assert(!table->s->is_secondary_engine());
        description += table->file->explain_extra();

        error |=
            AddMemberToObject<Json_string>(obj, "table_name", table->alias);
        error |= AddMemberToObject<Json_string>(obj, "access_type", "table");
        if (!table->file->explain_extra().empty())
          error |= AddMemberToObject<Json_string>(obj, "message",
                                                  table->file->explain_extra());
        ...
        string str = string("Parallel");
        assert(type == JT_INDEX_SCAN || type == JT_RANGE || type == JT_REF);
        const KEY *key;
        if (type == JT_INDEX_SCAN) {
          key = &table->key_info[path->pq_block_scan().qep_tab->index()];

          error |= SetIndexInfoInObject(
              &description, "index_scan", str.c_str(), table, key, "scan",
              /*lookup condition*/ "", /*range*/ nullptr, nullptr,
              path->pq_block_scan().qep_tab->m_reversed_access,
              /*push_condition*/ nullptr, obj);
        } else if (type == JT_RANGE) {
          AccessPath *range_scan_path =
              path->pq_block_scan().qep_tab->range_scan();
          assert(range_scan_path);
          const auto &param = range_scan_path->index_range_scan();
          key = &table->key_info[used_index(range_scan_path)];

          std::unique_ptr<Json_array> range_arr(new (std::nothrow)
                                                    Json_array());
          if (range_arr == nullptr) return nullptr;
          string ranges;
          error |= PrintRanges(param.ranges, param.num_ranges, key->key_part,
                               /*single_part_only=*/false, range_arr, &ranges);
          error |= SetIndexInfoInObject(
              &description, "index_range_scan", str.c_str(), table, key,
              "range scan", /*lookup condition*/ "", &ranges,
              std::move(range_arr), range_scan_path->index_range_scan().reverse,
              table->file->pushed_idx_cond, obj);
        } else if (type == JT_REF) {
          auto table_ref = path->pq_block_scan().qep_tab->ref();
          key = &table->key_info[table_ref.key];
          error |= SetIndexInfoInObject(
              &description, "index_lookup", str.c_str(), table, key, "lookup",
              RefToString(table_ref, key, /*include_nulls=*/false),
              /*ranges=*/nullptr, nullptr,
              path->pq_block_scan().qep_tab->m_reversed_access,
              table->file->pushed_idx_cond, obj);
        }
      ...
      error |=
          AddChildrenFromPushedCondition(path->pq_block_scan().table, children);
      AddChildrenFromIndexKey(table, &path->pq_block_scan().qep_tab->ref(),
                              children);
      break;
    }
  }
...
/// Walks a key value used as reference in an index access method to collect
/// subqueries in it. Such subqueries have been removed from the WHERE
/// condition by test_if_ref(), so this function is the only way to find them,
/// to be able to later print them in EXPLAIN. This is similar in idea to
/// AddChildrenFromPushedCondition().
///
/// @param only_base_table  If not nullptr, it is the index-accessed table and
/// we want collection only if this is a base table
/// @param ref  Description of lookup method
/// @param[out] children  Paths for subqueries will be put here
/// @note The justification for the only_base_table argument is: if this index
/// lookup is for the temporary table of a Materialize access path, then
/// AddChildrenFromIndexKey() is called twice (by the Materialize path, and by
/// the JT_REF path of the tmp table), but we want a single printout, so one
/// call has to be a no-op.
/// @todo make this function apply to range-scan too, as we are about
/// to remove range conditions from filters too
/// (@see rds_empty_redundant_check_in_range_scan).
void AddChildrenFromIndexKey(TABLE *only_base_table, Index_lookup *ref,
                             std::vector<ExplainChild> *children) {
  if (ref == nullptr) return;
  if (only_base_table && only_base_table->pos_in_table_list &&
      !only_base_table->pos_in_table_list->is_base_table())
    return;
  for (unsigned key_part_idx = 0; key_part_idx < ref->key_parts;
       ++key_part_idx) {
    auto ref_item = ref->items[key_part_idx];
    GetAccessPathsFromItem(ref_item, "reference key of index access", children);
  }
}
...
std::string PrintQueryPlan(THD *ethd, const THD *query_thd,
  ...
  if (path->type == AccessPath::PARALLEL_SCAN) {
    join = path->parallel_scan().gather->m_template_join;
  }
...
string GetForceSubplanToken(const Json_object *obj,
  ...
  char ret[8 * 2 + 2 + 1];
  snprintf_s(ret, sizeof(ret), sizeof(ret) - 1,
             "0x%02x%02x%02x%02x%02x%02x%02x%02x", sha256sum[0], sha256sum[1],
             sha256sum[2], sha256sum[3], sha256sum[4], sha256sum[5],
...
void Explain_format_tree::ExplainPrintCosts(const Json_object *obj,
  ...
  if (obj->get("actual_rows") != nullptr) {
    ...
    if (obj->get("actual_rows")->json_type() == enum_json_type::J_NULL) {
      if (obj->get("parallel_timing_info") == nullptr) {
        *explain += "(never executed)";
      } else {
        *explain +=
            down_cast<Json_string *>(obj->get("parallel_timing_info"))->value();
      }

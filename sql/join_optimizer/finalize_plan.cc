...
void ReplaceOrderItemsWithTempTableFields(THD *thd, ORDER *order,
  ...
  for (; order != nullptr; order = order->next) {
    ...
    if (temp_field_item != *order->item) {
      ...
      // For the two aforementioned accesses to the function change_item_tree,
      // the reason the third parameter passed is different is as follows:
      // (1) For first access, the second argument of change_item_tree is
      // supposed to be Item* and here we are passing an Item** (what 'new'
      // returns is Item**). So, change_item_tree, which assumes this is Item*,
      // in change_item_tree function, it tries to access origin_item member of
      // the Item, by dereferencing, and this fails. So add a 3rd arguments to
      // change_item_tree, pass true and it can not access origin_item member of
      // the Item.
      // (2) For second access, it pass a Item * to the second argument, so
      // it can access origin_item member of Item and no need pass 3rd arguments
      // because default value of 3rd is false.

...
void EstimateMaterializeCost(THD *thd, AccessPath *path) {
  ...
  for (const MaterializePathParameters::QueryBlock &block :
    ...
    if (block.subquery_path && block.subquery_path->num_output_rows() >= 0.0) {
  ...
  if (table_path->type == AccessPath::TABLE_SCAN ||
      table_path->type == AccessPath::PQ_BLOCK_SCAN) {
    double table_path_num_output_rows = 0.0;
    TABLE *table_path_table = nullptr;
    if (table_path->type == AccessPath::TABLE_SCAN) {
    ...
    path->init_cost = 0.0;
    path->init_once_cost = 0.0;
    table_path->set_num_output_rows(table_path_num_output_rows);
    ...
    if (Overlaps(test_flags, TEST_NO_TEMP_TABLES)) {
      ...
      table_path->cost = subquery_cost + table_path_num_output_rows * 0.1;
    } else {
      TABLE dummy_table;
      TABLE *temp_table = table_path_table;
      ...
      temp_table->file->stats.records =
          min(table_path_num_output_rows, LLONG_MAX_DOUBLE);
      table_path->cost =
...
double EstimateAggregateNoRollupRows(const TermArray &terms, double child_rows,
  ...
  for (TermArray::const_iterator term = terms.cbegin(); term < terms.cend();
    ...
    if (!IsBitSet(term - terms.cbegin(), index_estimator.GetConsumedTerms()) &&
      ...
      Histogram_rdlock_guard s_histo_guard(&field->table->s->LOCK_m_histograms);
...
void EstimateLimitOffsetCost(AccessPath *path, bool pq_rewrite) {
  auto &lim = path->limit_offset();
  AccessPath *&child = lim.child;
  // The PQ worker needs to return at most offset + limit rows of data.
  ha_rows offset = pq_rewrite ? 0 : lim.offset;

  if (child->num_output_rows() >= 0.0) {
    path->set_num_output_rows(
        offset >= child->num_output_rows()
            ? 0.0
            : (std::min<double>(child->num_output_rows(), lim.limit) - offset));
  } else {

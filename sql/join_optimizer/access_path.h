...
struct AccessPath {
  ...
  bool is_pq() { return type == PARALLEL_SCAN; }
  auto &ptrc() {
 ...
 private:
  ...
  union {
    ...
    struct {
      ...
      // Whether the build and/or the probe input yields partial results. If
      // true, it means that the build/probe input contains a divided table or a
      // cut table. Only used in parallel query.
      bool partial_results_from_build_input;
      bool partial_results_from_probe_input;
      QEP_TAB *outer_qep_tab;
    } hash_join;
    struct {
      AccessPath *outer, *inner;
      JoinType join_type;
    ...
    struct {
      ...
      // Whether the StreamingIterator is on a query block boundary.
    ...
    struct {
      ...
      // Whether the MaterializeIterator is on a query block boundary.
      bool query_block_boundary;
    } materialize;
    struct {
      AccessPath *table_path;
...
static_assert(sizeof(AccessPath) <= 144 + 148,
...
inline AccessPath *NewNestedLoopSemiJoinWithDuplicateRemovalAccessPath(
  ...
  path->nested_loop_semijoin_with_duplicate_removal().pos_inner = nullptr;
  return path;
}
...
inline AccessPath *NewLimitOffsetAccessPath(THD *thd, AccessPath *child,
                                            ...
                                            bool reject_multiple_rows,
                                            ha_rows *send_records_override) {
  void EstimateLimitOffsetCost(AccessPath * path, bool pq_rewrite = false);
  AccessPath *path = new (thd->mem_root) AccessPath;
...
inline AccessPath *NewStreamingAccessPath(THD *thd, AccessPath *child,
  ...
  path->stream().query_block_boundary = query_block_boundary;
...
inline AccessPath *NewMaterializeAccessPath(
    ...
    bool reject_multiple_rows, bool query_block_boundary,
    Semijoin_mat_exec *sj_mat_exec, bool pq_can_share) {
  MaterializePathParameters *param =
  ...
  param->sj_mat_exec = sj_mat_exec;
  param->pq_can_share = pq_can_share;
  param->saved_param = nullptr;
  ...
  path->materialize().query_block_boundary = query_block_boundary;
  return path;
}
...
inline AccessPath *NewParallelScanAccessPath(THD *thd, QEP_TAB *qep_tab,
  ...
  path->parallel_scan().stab_output = stab_output;
  path->parallel_scan().m_root_access_path = root_path;
  return path;
}

inline AccessPath *NewPQblockScanAccessPath(THD *thd, TABLE *table,
                                            Gather_operator *gather,
                                            PQTabType tabType, QEP_TAB *qep_tab,
                                            bool need_rowid) {
  AccessPath *path = new (thd->mem_root) AccessPath;
  ...
  path->pq_block_scan().tabType = tabType;
  path->count_examined_rows = true;
  return path;
}
...
inline AccessPath *NewPQrefScanAccessPath(THD *thd, TABLE *table,
                                          Gather_operator *gather,
                                          PQTabType tabType, bool need_rowid,
                                          Index_lookup *used_ref,
  ...
  path->pq_ref_scan().tabType = tabType;
  path->count_examined_rows = true;
  return path;
}
...
void SetCostOnTableAccessPath(const Cost_model_server &cost_model,
                              ...
                              AccessPath *path, bool pq_rewrite = false);
...
/*
  ...
  be included whenever you store NULL flags and the likes, but if you don't
  want them (perhaps to specifically check for conditions referring to pruned
  tables), you can set it to false, default value is true.
 */
Prealloced_array<TABLE *, 4> GetUsedTables(AccessPath *path,
                                           bool include_pruned_tables = true);
...
// A function that simply forwards the call to "WalkAccessPaths" (defined in
// access_path.cc), so we can access said function from other files. Adding the
// definition of WalkAccessPaths to the header file is not trivial, as it would
// require us to add explicit template instantiations where the templated
// function has a lambda argument. And we do not want to move the entire
// function as it will cause problems when merging from upstream MySQL in the
// future.
void WalkAccessPathsProxy(AccessPath *path, bool cross_query_blocks,

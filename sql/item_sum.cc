...
bool Item_sum::eq(const Item *cmp_item, bool binary_cmp) const {
  /* Assume we don't have rtti */
  // cmp_item may be a ref item, thus we use its real item.
  Item *item = const_cast<Item *>(cmp_item)->real_item();
...
Item *Item_sum::set_arg(THD *thd, uint i, Item *new_val) {
  ...
  new_val->origin_sum_arg = new_val->origin_item;
  return new_val;
}
...
static bool create_tmp_table_from_item(THD *thd, Item_sum *m_item_sum,
                                       TABLE **m_table,
                                       Temp_table_param *tmp_param,
                                       uint *const_distinct) {
  mem_root_deque<Item *> list(thd->mem_root);
  Query_block *query_block = m_item_sum->aggr_query_block;

  uint const_items = 0;
  uint num_args = m_item_sum->argument_count();
  assert(num_args);
  for (uint i = 0; i < num_args; i++) {
    Item *item = m_item_sum->get_arg(i);
    list.push_back(item);
    if (item->const_item()) {
      const bool is_null = item->is_null();
      if (thd->is_error()) return true;  // is_null can fail
      if (is_null) {
        *const_distinct = Aggregator_distinct::CONST_NULL;
        return false;
      } else
        const_items++;
    }
  ...
  if (num_args == const_items) {
    *const_distinct = Aggregator_distinct::CONST_NOT_NULL;
    return false;
  }
  count_field_types(query_block, tmp_param, list, false, false);
  tmp_param->force_copy_fields = m_item_sum->has_force_copy_fields();
  ...
  */
  ...
  if (!((*m_table) =
            create_tmp_table(thd, tmp_param, list, nullptr, true, false,
                             query_block->active_options(), HA_POS_ERROR, "")))
    return true;
  (*m_table)->file->ha_extra(HA_EXTRA_NO_ROWS);  // Don't update rows
  (*m_table)->no_rows = true;
  ...
  */
  ...
  if (item_sum->sum_func() == Item_sum::COUNT_FUNC ||
    ...
    if (query_block->pq_context().parallel_exec &&
      ...
      if (current_thd->is_pq_leader()) {
        Item *item = item_sum->get_arg(0);
        assert(item->type() == Item::FIELD_ITEM);
        Item_field *field = dynamic_cast<Item_field *>(item);
        assert(field->field->item_sum_ref);
        orig_func = field->field->item_sum_ref;
        target_item = dynamic_cast<Item_sum *>(orig_func);
      }
      /**
        If it is a constant or NULL, there is no need to subsequently create a
        temporary table or construct a red-black tree; the result can be
        returned directly.
      */
      if (orig_func) {
        Item_sum *orig_sum_func = static_cast<Item_sum *>(orig_func);
        if (orig_sum_func->get_aggr() &&
            orig_sum_func->get_aggr()->Aggrtype() ==
                Aggregator::DISTINCT_AGGREGATOR) {
          const_distinct =
              static_cast<Aggregator_distinct *>(orig_sum_func->get_aggr())
                  ->get_const_distinct();
    ...
    /**
      ...
      does not happen.
     */
    if (create_tmp_table_from_item(thd, target_item, &table, tmp_table_param,
                                   (uint *)&const_distinct))
      return true;
    if (const_distinct != NOT_CONST) return false;
    assert(table);
...
bool Aggregator_distinct::merge_count_distinct_tree(Item_sum *item_sum_arg) {
  ...
  buf->rewind();
  while (true) {
    int ret = buf->read((uchar *)key, tree_key_length);
    if (ret < 0 || (uint)ret != tree_key_length) {
      if (!buf->finish_read()) {
        buf->release();
        my_error(ER_PARALLEL_QUERY_ERROR, MYF(0),
                 "count distinct batch_buffer read error.");
        return true;
      }
      buf->release();
      break;
    }
    if (tree->unique_add(key)) return true;
  }
  return false;
}
...
static int copy_tree_to_cache(void *element, element_count, void *item) {
  return ((Batch_buffer *)item)
      ->write((uchar *)element, *(uint *)((Batch_buffer *)item)->args);
}

bool Aggregator_distinct::save_distinct_to_mq(Batch_buffer *buffer) {
  if (const_distinct != NOT_CONST) return false;
  assert(tree);
  buffer->args = &tree_key_length;
  return tree->walk(copy_tree_to_cache, buffer);
}
...
bool Item_sum_avg::resolve_type(THD *thd) {
  ...
  /**
   * @TODO: remove this code as Item_sum_avg will be not warpped
   *        as Item_sum_sum.
   *
  ...
  set_nullable(true);
  null_value = true;
  // For rebuild avg item, item_field's precision and scale has already
  // added prec_increment, do not add it again.
  prec_increment =
      pq_avg_type == PQ_REBUILD ? 0 : thd->variables.div_precincrement;
  if (hybrid_type == DECIMAL_RESULT) {
    ...
    // For avg in leader and worker, the max_length should add
    // DECIMAL_LONGLONG_DIGITS Because we copy sum and count instead
    // of avg from worker to leader.
    set_data_type_decimal((pq_avg_type == PQ_LEADER || pq_avg_type == PQ_WORKER)
                              ? f_precision
                              : precision,
                          scale);
...
bool Item_sum_avg::add() {
  ...
  if (!aggr->arg_is_null(true)) {
    if (pq_avg_type == PQ_REBUILD) {
      uint32 extra_len;
      m_count += uint8korr(args[0]->val_extra((uint32 *)&extra_len));
    } else {
      m_count++;
    }
  }
...
my_decimal *Item_sum_avg::val_decimal(my_decimal *val) {
  ...
  if (m_is_window_function) {
    ...
    */
    ...
    // TODO input param val is not changed in this
    if (pq_avg_type == PQ_WORKER) {
...
void Item_sum_count::reset_field() {
  ...
  if (!args[0]->is_nullable() || !args[0]->is_null())
...
size_t need_extra(Item_sum *ref_item, bool group) {
  if (ref_item->sum_func() == Item_sum::AVG_FUNC) {
    /* for 'avg with rollup' ref_item will be a Item_rollup_sum_switcher,
     * its sum_func() is master()->sum_func(). See similar logic in
     reset_avg_property(). */
    Item_sum_avg *sum_avg = dynamic_cast<Item_sum_avg *>(ref_item);
    if (sum_avg != nullptr) return sum_avg->pq_extra_len(group);
  }
  return 0;
}

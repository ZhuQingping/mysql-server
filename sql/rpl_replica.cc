...
static int get_wal_master_version_and_clock(MYSQL *mysql, Master_info *mi);
...
bool start_slave_threads(bool need_lock_slave, bool wait_for_start,
  ...
  **/
  ...
  if (thread_mask & SLAVE_IO) {
    if (strcmp(mi->get_channel(), REPLICA_WAL_CHANNEL) == 0) {
      is_error = start_slave_thread(key_thread_replica_io, handle_slave_wal_io,
...
/*
  Obtain the version and clock information from the master, and set the
  heartbeat time.
...
static int get_wal_master_version_and_clock(MYSQL *mysql, Master_info *mi) {
  ...
  */
  ...
  DBUG_EXECUTE_IF("get_unix_timestamp.ER_NET_READ_INTERRUPTED", {
    DBUG_SET("+d,inject_ER_NET_READ_INTERRUPTED");
    DBUG_SET(
        "-d,get_unix_timestamp."
        "ER_NET_READ_INTERRUPTED");
  });

  DBUG_EXECUTE_IF("get_unix_timestamp.ER_NOT_NET_ERROR", {
    DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
    DBUG_SET(
        "-d,get_unix_timestamp."
        "ER_NOT_NET_ERROR");
  });

  DBUG_EXECUTE_IF("get_unix_timestamp.ER_SLAVE_KILLED", {
    DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
    mi->abort_slave = true;
    DBUG_SET(
        "-d,get_unix_timestamp."
  ...
  if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT UNIX_TIMESTAMP()")) &&
    ...
    mysql_mutex_unlock(&mi->data_lock);
    LogErr(WARNING_LEVEL, ER_WAL_RPL_REPLICA_SECONDS_BEHIND_SOURCE_DUBIOUS,
  ...
  /*
    ...
    thus copying some my.cnf), replication will work but all wal will be
    skipped.
  ...
  */
  ...
  DBUG_EXECUTE_IF("get_source_server_id.ER_NOT_NET_ERROR", {
    DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
    DBUG_SET(
        "-d,get_source_server_id."
        "ER_NOT_NET_ERROR");
  ...
  DBUG_EXECUTE_IF("get_source_server_id.ER_SLAVE_KILLED", {
    DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
    mi->abort_slave = true;
    DBUG_SET(
        "-d,get_source_server_id."
  ...
  if (!mysql_real_query(mysql, STRING_WITH_LEN("SELECT @@GLOBAL.SERVER_ID")) &&
    ...
    /* Fatal error */
    errmsg =
        "The replica wal I/O thread stops because a fatal error is encountered "
  ...
  if (mi->heartbeat_period != 0.0) {
    ...
    */
    ...
    DBUG_EXECUTE_IF("set_heartbeat_period.ER_NET_READ_INTERRUPTED", {
      DBUG_SET("+d,inject_ER_NET_READ_INTERRUPTED");
      DBUG_SET(
          "-d,set_heartbeat_period."
          "ER_NET_READ_INTERRUPTED");
    });

    DBUG_EXECUTE_IF("set_heartbeat_period.ER_NOT_NET_ERROR", {
      DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
      DBUG_SET(
          "-d,get_source_server_id."
          "ER_NOT_NET_ERROR");
    ...
    DBUG_EXECUTE_IF("set_heartbeat_period.ER_SLAVE_KILLED", {
      DBUG_SET("+d,inject_ER_NOT_NET_ERROR");
      mi->abort_slave = true;
      DBUG_SET(
          "-d,set_heartbeat_period."
    ...
    if (mysql_real_query(mysql, query, static_cast<ulong>(strlen(query)))) {
      ...
      if (is_network_error(mysql_errno(mysql))) {
        ...
        /* Fatal error */
        errmsg =
            "The replica wal I/O thread stops because a fatal error is "
            "encountered when it tries to SET @source_heartbeat_period "
            "on source.";
        err_code = ER_REPLICA_FATAL_ERROR;
        sprintf(err_buff, "%s Error: %s", errmsg, mysql_error(mysql));
        mysql_free_result(mysql_store_result(mysql));
...
/**
   Get the wal log information of the slave.

   @param thd             THD object.
   @param flushed_lsn    return the lsn of the wal log that has been flushed.
   @param replayed_lsn   return the lsn of the wal log that has been flushed.
   @param wal_stream_id   return the wal stream id of the wal log.
*/
static void get_slave_wal_info(THD *thd, uint64_t &flushed_lsn,
                               uint64_t &replayed_lsn, uint64 &wal_stream_id) {
  st_plugin_int *cde_plugin = nullptr;
  handlerton *cde_ha = nullptr;
  {
    ...
    cde_ha = (handlerton *)cde_plugin->data;
  }

  flushed_lsn = cde_ha->get_wal_flushed_lsn(thd);
  wal_stream_id = cde_ha->get_wal_stream_id(thd);
  replayed_lsn = 0;
}
...
/**
   ...
   @param thd            THD object.
   @param start_lsn[out]  the flushed lsn(stream state) of the wal
   log received on replica before replication is interrupted.
*/
static void get_replica_wal_stream_start_lsn(THD *thd, uint64_t &start_lsn) {
  DBUG_EXECUTE_IF("replica_skip_wal_start_lsn", {
    start_lsn = 50;
    return;
  };);
  st_plugin_int *cde_plugin = nullptr;
  ...
  start_lsn = cde_ha->get_replica_wal_stream_start_lsn(thd);
}

/**
   Release handler layer resources.

   @param thd   THD object.
*/
static void release_handler_resource(THD *thd) {
  st_plugin_int *cde_plugin = nullptr;
  handlerton *cde_ha = nullptr;
  {
    LEX_CSTRING engine_name = {STRING_WITH_LEN(DSTORE_ENGINE_NAME)};
  ...
  if (*thd_ha_data(thd, cde_ha) != nullptr)
    cde_ha->close_connection(cde_ha, thd);
}

static int request_wal_dump(THD *thd, MYSQL *mysql, MYSQL_WAL_RPL *rpl,
                            Master_info *mi, bool *suppress_warnings) {
  DBUG_TRACE;
  enum_server_command command = COM_WAL_DUMP;

  uint wallog_flags = 0;
  wallog_flags |= USE_HEARTBEAT_EVENT_V2;
  if (RUN_HOOK(wallog_io, before_request_wal_transmit, (thd, mi, wallog_flags)))
    return 1;

  *suppress_warnings = false;
  rpl->server_id = server_id;
  rpl->flags = 0;
  get_slave_wal_info(thd, rpl->flushed_lsn, rpl->replayed_lsn, rpl->stream_id);

  /* Wait until after getting the valid wal_stream_start_lsn of replica. */
  while (!io_slave_killed(thd, mi)) {
    get_replica_wal_stream_start_lsn(thd, rpl->flushed_lsn);
    DBUG_EXECUTE_IF("replica_skip_wal_start_lsn",
                    { rpl->flushed_lsn = UINT64_MAX; };);
    if (rpl->flushed_lsn != 0) {
      break;
    }
    /*
      wal_stream_start_lsn == 0 indicates that REPLICA is replaying local wal
      logs, so we need to wait. Also, wal_stream_start_lsn == 0 only appears
      when the process of mysqld is first started, once the value of
      wal_stream_start_lsn is not 0, it will never become 0 again.
    */
    my_sleep(10000); /* 10ms */
  }
  mysql_wal.init_wal_flushed_lsn(rpl->flushed_lsn);
  sql_print_information(
      "Slave request wal dump, wal stream id is %lu, "
      "read flushed lsn is %lu.",
      rpl->stream_id, rpl->flushed_lsn);

  if (mysql_wallog_open(mysql, rpl)) {
    /*
    ...
    */
    ...
    if (err == ER_NET_READ_INTERRUPTED)
      *suppress_warnings = true;  // Suppress reconnect warning
      ...
      LogErr(ERROR_LEVEL, ER_WAL_RPL_REPLICA_ERROR_RETRYING,
...
static ulong read_wal_log(MYSQL *mysql, MYSQL_WAL_RPL *rpl, Master_info *mi,
                          bool *suppress_warnings) {
  DBUG_TRACE;

  DBUG_EXECUTE_IF("rpl_read_wal_log_failed", {
    DBUG_SET("-d, rpl_read_wal_log_failed");
    return packet_error;
  });

  *suppress_warnings = false;

  if (mysql_wallog_fetch(mysql, rpl)) {
    uint err{mysql_errno(mysql)};
    if (err == ER_NET_READ_INTERRUPTED) {
      /*
      ...
      *suppress_warnings = true;
      ...
      LogErr(ERROR_LEVEL, ER_WAL_RPL_REPLICA_ERROR_READING_FROM_SERVER,
  ...
  /* Check if eof packet */
  if (rpl->size == 0) {
    LogErr(SYSTEM_LEVEL, ER_WAL_RPL_REPLICA_DUMP_THREAD_KILLED_BY_SOURCE,
...
/**
   Check whether the received message is a heartbeat message.

   @param buf   pointer to message buffer.

   @return returns true if a heartbeat message is received, and false if a
   wallog message is received.
*/
static bool is_wal_heartbeat(const char *buf) {
  DBUG_TRACE;

  const Rpl_comm_header *header = pointer_cast<const Rpl_comm_header *>(buf);
  if (header->type == RplWalMsgType::RPL_WAL_HEARTBEAT_MSG) {
    return true;
  }
  return false;
}
...
/**
   Write the wal log to the log file.

   @param thd       THD object.
   @param wal_buf   pointer to message buffer.
   @param end_lsn  pointer to message buffer.

   @retval        0                   OK.
   @retval        1                   There was an error.
*/
static int write_wal_log(THD *thd, MYSQL_WAL_RPL *rpl, const char *wal_buf,
                         uint64_t &end_lsn) {
  DBUG_TRACE;

  const Rpl_wal_data_msg *wal_msg =
      pointer_cast<const Rpl_wal_data_msg *>(wal_buf);
  if (wal_msg->common_header.type != RplWalMsgType::RPL_WAL_DATA_MSG) {
    return 1;
  }
  end_lsn = wal_msg->start_lsn + wal_msg->wal_len;
  ...
  cde_ha->append_wal(thd, rpl->stream_id,
                     reinterpret_cast<const uint8_t *>(wal_msg->wal_data),
                     wal_msg->wal_len, wal_msg->start_lsn, end_lsn);
  return 0;
}

/**
  Slave WAL IO thread entry point.
  ...
  @return Always 0.
*/
extern "C" void *handle_slave_wal_io(void *arg) {
  ...
  my_thread_init();
  {
    ...
    if (init_replica_thread(thd, SLAVE_THD_IO)) {
      ...
      mi->report(ERROR_LEVEL, ER_REPLICA_FATAL_ERROR,
                 ...
                 "Failed during replica wal I/O thread initialization ");
      goto err;
    }
    ...
    if (RUN_HOOK(wallog_io, wal_thread_start, (thd, mi))) {
      mi->report(ERROR_LEVEL, ER_REPLICA_FATAL_ERROR,
                 ER_THD(thd, ER_REPLICA_FATAL_ERROR),
  ...
  connected:
    ...
    ret = get_wal_master_version_and_clock(mysql, mi);
    if (!ret) {
    ...
    switch (ret) {
      case 0: {
        LogErr(SYSTEM_LEVEL,
               ER_WAL_RPL_REPLICA_CONNECTED_TO_SOURCE_RPL_STARTED_FILE_BASED,
    ...
    */
    ...
    if (register_slave_on_master(mysql, mi, &suppress_warnings)) {
      if (!check_io_slave_killed(thd, mi,
        ...
        LogErr(ERROR_LEVEL,
               ER_WAL_RPL_REPLICA_IO_THREAD_CANT_REGISTER_ON_SOURCE);
        if (try_to_reconnect(thd, mysql, mi, &retry_count, suppress_warnings,
                             reconnect_messages_after_failed_registration))
          goto err;
      } else
    ...
    DBUG_PRINT("info", ("Starting reading wal log from source"));
    while (!io_slave_killed(thd, mi)) {
      MYSQL_WAL_RPL wal_rpl;
      THD_STAGE_INFO(thd, stage_requesting_wallog_dump);
      if (request_wal_dump(thd, mysql, &wal_rpl, mi, &suppress_warnings)) {
        LogErr(ERROR_LEVEL, ER_WAL_RPL_REPLICA_ERROR_REQUESTING_WALLOG_DUMP,
               mi->get_for_channel_str());
        if (check_io_slave_killed(thd, mi,
                                  "Replica wal I/O thread killed while "
      ...
      while (!io_slave_killed(thd, mi)) {
        ...
        THD_STAGE_INFO(thd, stage_waiting_for_source_to_send_wallog);
        wal_len = read_wal_log(mysql, &wal_rpl, mi, &suppress_warnings);
        if (check_io_slave_killed(thd, mi,
                                  "Replica wal I/O thread killed while "
                                  "reading wal log"))
          goto err;

        if (wal_len == packet_error) {
          DBUG_EXECUTE_IF("rpl_read_wal_packet_too_large", {
            DBUG_SET("-d, rpl_read_wal_packet_too_large");
            mysql->net.last_errno = CR_NET_PACKET_TOO_LARGE;
          });
          DBUG_EXECUTE_IF("rpl_read_wal_read_error", {
            DBUG_SET("-d, rpl_read_wal_read_error");
            mysql->net.last_errno = ER_SOURCE_FATAL_ERROR_READING_BINLOG;
          });
          DBUG_EXECUTE_IF("rpl_read_wal_out_of_resources", {
            DBUG_SET("-d, rpl_read_wal_out_of_resources");
            mysql->net.last_errno = ER_OUT_OF_RESOURCES;
          });
        ...
        retry_count = 0;
        THD_STAGE_INFO(thd, stage_write_source_wallog_to_file);
        wal_buf = (const char *)mysql->net.read_pos + 1;
        DBUG_PRINT("info",
                   ("Wal IO thread received message of type %d", wal_buf[0]));

        if (RUN_HOOK(wallog_io, after_read_wal,
                     (thd, mi, (const char *)mysql->net.read_pos + 1, wal_len,
                      &wal_buf, &wal_len))) {
          mi->report(ERROR_LEVEL, ER_REPLICA_FATAL_ERROR,
                     ER_THD(thd, ER_REPLICA_FATAL_ERROR),
                     "Failed to run 'after_read_wal' hook");
          goto err;
        }

        // If the heartbeat message is received, just return an ACK response.
        if (!is_wal_heartbeat(wal_buf)) {
          if (write_wal_log(thd, &wal_rpl, wal_buf, wal_rpl.received_lsn)) {
            mi->report(ERROR_LEVEL, ER_WAL_RPL_REPLICA_WALLOG_WRITE_FAILURE,
                       ER_THD(thd, ER_WAL_RPL_REPLICA_WALLOG_WRITE_FAILURE),
                       "could not write wal log from source");
            goto err;
          }
          mysql_wal.wait_for_update_wal(wal_rpl.received_lsn);
        }

        get_slave_wal_info(thd, wal_rpl.flushed_lsn, wal_rpl.replayed_lsn,
                           wal_rpl.stream_id);
        if (RUN_HOOK(wallog_io, after_write_wal,
                     (thd, mi, wal_buf, wal_len, wal_rpl.received_lsn,
                      wal_rpl.flushed_lsn, wal_rpl.replayed_lsn))) {
          mi->report(ERROR_LEVEL, ER_REPLICA_FATAL_ERROR,
                     ER_THD(thd, ER_REPLICA_FATAL_ERROR),
                     "Failed to run 'after_write_wal' hook");
          goto err;
        }

        /*
          After wal log is flushed, memory used
  ...
  err:
    ...
    LogErr(INFORMATION_LEVEL, ER_WAL_RPL_REPLICA_IO_THREAD_EXITING,
           mi->get_for_channel_str());
    /* At this point the I/O thread will not try to reconnect anymore. */
    mi->atomic_is_stopping = true;
    (void)RUN_HOOK(wallog_io, wal_thread_stop, (thd, mi));

    release_handler_resource(thd);
    thd->reset_query();

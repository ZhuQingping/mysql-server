...
bool trans_begin(THD *thd, uint flags) {
  ...
  */
  ...
  /* For an explicit transaction, the start time of the transaction is
     recorded when the `begin` operation is performed. */
  thd->trx_start_time_in_secs = time(nullptr);
...
/**
  Check whether transaction already has an active read-write handlerton.

  @param[in]  thd     Current thread.
  @param[in]  scope   transaction scope to check, STMT or SESSION

  @retval false   dosen't have read write handlerton
  @retval true    has active read-write handlerton
 */
static inline bool trans_check_active_read_write(
    THD *thd, Transaction_ctx::enum_trx_scope scope) {
  Transaction_ctx *trn_ctx = thd->get_transaction();
  assert(trn_ctx != nullptr);
  if (trn_ctx->ha_trx_info(scope) == nullptr ||
      (!trn_ctx->ha_trx_info(scope)->is_started()))
    return false;

  Ha_trx_info_list ha_list{trn_ctx->ha_trx_info(scope)};
  for (auto const &ha_info : ha_list) {
    if (ha_info.ht()->db_type != DB_TYPE_DSTORE &&
...
bool trans_commit_stmt(THD *thd, bool ignore_global_read_lock) {
  ...
  */
  ...
  /* Dummy transactions will not be registered in DStore if there is no
  read-write transaction in InnoDB (e.g. CREATE TEMPORARY TABLE). We
  register dummy transactions to ensure DDL crash safety via 2PC. And this
  is not needed for temporary objects as they don't require crash safety.*/
  if (unlikely(get_ddl_command_type(thd) != DDLCOM_NONE &&
               trans_check_active_read_write(thd, Transaction_ctx::STMT))) {
    ha_register_dummy_trx(thd);
  }

...
bool lock_schema_name(THD *thd, const char *db) {
  ...
  /*
    If the __recyclebin__ schema is created in set variables operation,
    the time for waiting for mdl lock is reduced to avoid blocking users
    from creating sessions.
  */
  if (thd->mdl_context.acquire_locks(
          &mdl_requests, (thd->lex->sql_command == SQLCOM_SET_OPTION &&
                          im::recycle_bin::is_recycle_db(db))
                             ? 0
                             : thd->variables.lock_wait_timeout))
    return true;
...
bool Global_read_lock::lock_global_read_lock(THD *thd) {
  ...
  if (!m_state) {
    ...
    /*
       Do not allow upgrading binlog locks to FTWRL. Otherwise we can end up
       with deadlocks being reported for queries previously blocked on a binlog
       lock.
    */
    if (thd->backup_binlog_lock.abort_if_acquired()) return true;
...
bool Global_backup_lock::acquire_protection(THD *thd,
  ...
  if (duration == MDL_EXPLICIT) m_prot_lock = mdl_request.ticket;

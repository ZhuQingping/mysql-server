...
#include "sql/key.h"
#include "sql/parallel_query/pq_handler.h"        // PQ_shared_info
#include "sql/parallel_query/pq_resource_stat.h"  // RANGE_type
...
/**
  Supports aggregate pushdown
*/
#define HA_AGGREGATE_PUSHDOWN (1LL << 56)
...
constexpr const uint64_t HA_CREATE_USED_DSTORE_DDL_COMMENT{1ULL << 41};
/*
  End of bits used in used_fields
*/

/* Dstore engine name */
static constexpr const char DSTORE_ENGINE_NAME[7] = "Dstore";
/* InnoDB engine name */
static constexpr const char INNODB_ENGINE_NAME[7] = "INNODB";
/* CSV engine name */
static constexpr const char CSV_ENGINE_NAME[4] = "CSV";
/* Memory engine name */
static constexpr const char MEMORY_ENGINE_NAME[7] = "MEMORY";
/* MyISAM engine name */
static constexpr const char MYISAM_ENGINE_NAME[7] = "MyISAM";
/* MRG_MYISAM engine name */
static constexpr const char MRG_MYISAM_ENGINE_NAME[11] = "MRG_MYISAM";
...
/**
  post cleanup statistics information after DDL statement (e.g. in
  case of alter table engine=xxx really remove table files from disk).
*/
typedef void (*post_delete_statistics_t)(THD *thd, const char *name);
...
typedef void (*set_extra_commit_t)(THD *thd, const unsigned char *buffer,
                                   unsigned long length, bool write_extra_wal);

typedef void (*set_max_start_recovery_point_t)(THD *thd);

typedef void (*register_dummy_trx_t)(THD *thd);
...
/*
  Declaring some Function Pointers for local backup including full backup and
  wal archive
*/
struct local_backup_config {
  std::string root_path;       // Root directory of the current local backup
  std::string data_base_path;  // Directory where the archived data is stored
  std::string meta_base_path;  // Directory for storing archived metadata
};
struct local_backup_point {  // Dstore only
  std::uint64_t start_recovery_lsn;
  time_t time_point;
  std::uint64_t lsn_point;
};

struct local_backup_arch_point : public local_backup_point {  // Dstore only
  bool is_disjoin;
};

/**
  Start full local backup.
  @param config local backup config used by callback called inside storage.
  @param local_backup_point if it's not nullptr, will carry full localback
  point will be written in local backup metafile.
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*start_full_local_backup_t)(local_backup_config *config,
                                          local_backup_point *backup_point,
                                          bool involve_wal_archive,
                                          bool involve_standby);

/**
  Stop full local backup.
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*stop_full_local_backup_t)();
...
/**
  ...
  @retval true Error.
*/
typedef bool (*start_wal_archive_t)(local_backup_config *config);

/**
  stop wal archive
*/
typedef bool (*stop_wal_archive_t)();

/**
  write full backup point to full local backup meta file
  @param backupPoint these full local backup point needed to be persisted
  @param path the path of full local backup meta file
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*write_full_backup_meta_info_t)(local_backup_point &backupPoint,
                                              std::string &path);

/**
  write full backup point to full local backup meta file
  @param  backup_point       full local backup point of restore meta
  @param  meta_path          the path of full local backup meta
  @param  restore_meta_path  the path of restore meta
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*write_full_backup_restore_meta_t)(
    local_backup_point &backup_point, std::string &meta_path,
    std::string &restore_meta_path);

/**
  finish full backup binlog point logic
  @param backupPoint these full local backup point needed to be persisted
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*finish_full_backup_binlog_t)(local_backup_point &backupPoint);

/**
  Write non-dstore part status and error message, backup point info and binlog
  info to full local backup meta json file
  @param errMsg The non-dstore part error message
  @param backupPoint these full local backup point needed to be persisted
  @param path the path of full local backup meta json file
  @retval false Succeed.
  @retval true Error.
*/
typedef bool (*write_full_backup_meta_json_t)(std::string *errMsg,
                                              local_backup_point *backupPoint,
                                              std::string &path);

/**
This function is used to get the current flushed plsn of the writing walstream
of the PDB.
@param[out]   flushedPlsn output the current flushed plsn
@return flushedPlsn
*/
typedef uint64_t (*get_current_flushed_plsn_t)();

/**
This function is used to set the max start recovery plsn of the PDB for
TaurusDB.
@param[in]    plsn   the plsn that server wants to set the max start recovery
plsn
@return DSTORE_SUCC if success
*/
typedef int (*set_recovery_plsn_for_taurus_t)(uint64_t plsn);

using get_table_dd_info_t = void (*)(String *packet,
                                     const dd::Table *table_obj);
using get_index_dd_info_t = void (*)(String *packet, const dd::Table *table_obj,
                                     size_t key_no);

using show_dstore_info_t = void (*)(THD *thd, const char *parameter,
                                    const char *value, String *res);
...
/**
  ...
  @retval  current flushed lsn.
*/
typedef uint64_t (*get_wal_flushed_lsn_t)(THD *thd);
...
/**
  ...
  @retval  current wal stream id.
*/
typedef uint64_t (*get_wal_stream_id_t)(THD *thd);

/**
  Append wal to flush on disk.

  @param[in] thd The thread context.
  @param[in] wal_stream_id Wal stream id.
  @param[in] data Wal data.
  @param[in] len Wal len.
  @param[in] start_lsn Start postion of lsn to flush.
  @param[in] end_lsn End postion of lsn to flush.
*/
typedef void (*append_wal_t)(THD *thd, uint64_t wal_stream_id,
...
/**
  ...
  @param[in] thd The thread context.
  @param[in] wal_stream_id Wal stream id.
  @param[in] lsn Start postion of lsn to read.
  @param[in] data Read buffer.
  @param[in] read_len Max read length of wal data.
  @param[in] result_len Actual read length of wal data.

  @retval 0 success.
  @retval 1 failed.
*/
typedef int (*read_wal_t)(THD *thd, uint64_t wal_stream_id, uint64_t lsn,
                          uint8_t *data, uint64_t read_len,
                          uint64_t *result_len);
...
/**
  ...
  @retval 1 failed.
*/
typedef bool (*pdb_promote_t)(THD *thd, char *&promote_info);
...
/**
  ...
  @retval 1 failed.
*/
typedef bool (*pdb_demote_t)(THD *thd, char *&demote_info);
...
/**
  ...
  @retval lock wait timeout(ms).
*/
typedef int (*get_lock_wait_timeout_t)(THD *thd);

/**
  stop the log thread of dstore storage engine, confirm flush.
*/
typedef void (*stop_dstore_log_t)();
...
struct handlerton {
  ...
  */
  ...
  post_delete_statistics_t post_delete_statistics;
  ...
  */
  ...
  set_extra_commit_t set_extra_commit;
  register_dummy_trx_t register_dummy_trx = nullptr;

  set_max_start_recovery_point_t set_max_start_recovery_point;
  /**
  ...
  */
  ...
  start_full_local_backup_t start_full_local_backup;
  stop_full_local_backup_t stop_full_local_backup;
  start_wal_archive_t start_wal_archive;
  stop_wal_archive_t stop_wal_archive;
  finish_full_backup_binlog_t finish_full_backup_binlog;
  write_full_backup_meta_info_t write_full_backup_meta_info;
  write_full_backup_restore_meta_t write_full_backup_restore_meta;
  write_full_backup_meta_json_t write_full_backup_meta_json;
  get_current_flushed_plsn_t get_current_flushed_plsn;
  set_recovery_plsn_for_taurus_t set_recovery_plsn_for_taurus;
  /** Page tracking interface */
  Page_track_t page_track;

  get_wal_flushed_lsn_t get_wal_flushed_lsn;
  get_wal_flushed_lsn_t get_replica_wal_stream_start_lsn;
  get_wal_stream_id_t get_wal_stream_id;
  append_wal_t append_wal;
  read_wal_t read_wal;
  /** get dstore table info */
  get_table_dd_info_t get_table_dd_info;
  /** get dstore index info */
  get_index_dd_info_t get_index_dd_info;

  /** get dstore info */
  show_dstore_info_t show_dstore_info;

  /** single pdb promote */
  pdb_promote_t pdb_promote;
  /** single pdb demote */
  pdb_demote_t pdb_demote;

  /** get lock wait timeout */
  get_lock_wait_timeout_t get_lock_wait_timeout;

  stop_dstore_log_t stop_dstore_log;
};
...
#define HTON_SUPPORTS_RECYCLE_BIN (1 << 22)
...
struct HA_CREATE_INFO {
  ...
  LEX_STRING dstore_ddl_comment{nullptr, 0};
...
/*
 For parallel query, if current table is being split, do not use mrr.
 Because worker scan ranges are inside every mrr range.
*/
#define HA_MRR_SKIP_BY_PQ 512
...
class handler {
  ...
  */
  ...
  /**
    Pointer to aggregation context
   */
  Aggregate_context *m_pushed_aggregate;
  ...
  */
  ...
  /* ===== Start parallel query related variables ===== */
 public:
  /** reverse (index) scan */
  ...
  /// table access method for PQ
  /** range search type */
  uint pq_range_type{0};
  /** ref search key */
  key_range pq_ref_key{};
  PQ_shared_info m_shared_info{};
  /** parallel reader context. It is set only if this handler is owned by the
      leader and is being divided or cut. */
  void *pq_ctx{nullptr};
  /* ===== End parallel query related variables ===== */
  ...
  */
  ...
  /**
    Assign old name to table. This is usefull when
    renaming table with ALTER TABLE RENAME COPY.

    @param oldName - old name of the table
                     or
                     null (to reset old name assigned to the table)
  */
  virtual void set_old_name(const char *oldName [[maybe_unused]]) {}
  ...
  /**
    ...
    @param [in]     to_table_def    New version of definition for table
                                    being renamed.

    @retval   >0               Error.
    @retval    0               Success.
  */
  virtual int post_rename_table_statistics(const char *from [[maybe_unused]],
  ...
  */
  ...
  /** Set the aggregation context to storage engine
  @param ctx  the aggregation context
  */
  void aggregate_push(Aggregate_context *ctx) { m_pushed_aggregate = ctx; }

  /** Reset information about pushed index conditions */
  void cancel_pushed_aggregate() { m_pushed_aggregate = nullptr; }
 ...
 protected:
  ...
  /* ===== Start parallel query related interface ===== */
 public:
  int ha_pq_init(uint dop, uint keyno);
  bool ha_reverse_scan();
  void ha_set_reverse_scan(bool);
  int ha_pq_end();
  int ha_pq_next(uchar *buf, void *scan_ctx);
  PQ_shared_info *get_shared_info() { return &m_shared_info; }
  ...
  virtual int pq_ref_build_ranges(void *scan_ctx MY_ATTRIBUTE((unused)),
                                  Key_ref &key_ref MY_ATTRIBUTE((unused))) {
    assert(false);  // not supported
    return (0);
  }
  ...
  /// Implementations must be idempotent; it must be possible to call this
  /// twice without calling reset_shared_info() in between.
  virtual void set_shared_info(PQ_shared_info *info MY_ATTRIBUTE((unused))) {
    assert(false);  // not supported
    return;
  }

  virtual bool is_sharing_data() {
    assert(false);  // not supported
    return (0);
  }
  ...
  /**
     Must be idempotent, as it may be called several times without any call
     to pq_worker_scan_init() in between. This surprising case happens within
     MaterializeIterator, whose both Init() and End() call the underlying
     iterators' End(), which themselves call pq_worker_scan_end() if they read
     the divided table. Must work even if open() has not been called.
  */
  virtual int pq_worker_scan_end() {
    assert(false);  // not supported
    return (0);
  }
  /* ===== End parallel query related interface ===== */
 public:
...
void ha_commit_wals(THD *thd, const unsigned char *buffer, unsigned long length,
                    bool write_extra_wal = false);

void ha_set_max_start_recovery_point(THD *thd);

void ha_init_max_start_recovery_point();

/** Invoke handlerton::register_dummy_trx() on a plugin.
@param thd	  Thread context. */
void ha_register_dummy_trx(THD *thd);
...
/* Check if storage_engine_mode matches with disabled_storage_engines */
bool ha_check_storage_engine_mode(ulong storage_engine_mode,
...
/**
  Get instance storage engine type, can be InnoDB or Dstore. Currently
  this project support InnoDB or Dstore as default storage engine, but
  do not support create InnoDB tables and Dstore tables on same instance,
  since Dstore not support XA transaction and can not handle transaction
  across multiple storage engines. Also some SQL syntax are not supported
  for Dstore(e.g. XA, read committed isolation level, etc), then we need
  to provide one interface to distinguish instance storage engine type.
*/
extern ulong storage_engine_mode;
extern const char *default_storage_engine;
inline legacy_db_type get_instance_storage_engine_type() {
  /*
    Use storage_engine_mode and default_storage_engine to
    distinguish instance storage engine type is Dstore or InnoDB.
  */
  if (storage_engine_mode == static_cast<ulong>(ONLY_DSTORE)) {
    return DB_TYPE_DSTORE;
  } else if (storage_engine_mode == static_cast<ulong>(ONLY_INNODB)) {
    return DB_TYPE_INNODB;
  } else {
    /*
      storage_engine_mode in BOTH_STORAGE_ENGINE option,
      if storage engine is InnoDB then return InnoDB,
      or it will return Dstore.
      strcmp return 0 if it's the same string.
    */
    return my_strcasecmp(system_charset_info, default_storage_engine,
                         DSTORE_ENGINE_NAME)
               ? DB_TYPE_INNODB
               : DB_TYPE_DSTORE;
  }
...
/* Get default stoarge engine from current storage engine mode */
inline void ha_get_default_storage_engine_from_mode(LEX_CSTRING *se_name) {
  legacy_db_type type = get_instance_storage_engine_type();
  if (type == DB_TYPE_DSTORE) {
    lex_cstring_set(se_name, DSTORE_ENGINE_NAME);
  } else {
    lex_cstring_set(se_name, INNODB_ENGINE_NAME);
  }
}

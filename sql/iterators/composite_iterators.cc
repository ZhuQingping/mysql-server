    ...
    case READING_FIRST_ROW: {
      ...
      if (err == -1) {
        ...
        if (m_join->grouped || m_join->group_optimized_away ||
            // (a) this is a PQ worker (b) and the final result is built by
          ...
          // no_rows_in_result and clear fields to send an empty row to MQ. This
          // would cause leader receive an unexpected empty row and produce
          // wrong result, ISSUE 557.
          //
          // For example, "select count(a), a from t1". If worker w1 has not
          // fetched any data blocks, it will send (0, NULL) to MQ. Then,
          // leader will re-aggregate this record, which may produce an NULL
          // value in field "a". However, NULL-value does not exist in table
          // "t1" at all.

          // If all workers fail to read record from m_source, then no records
          // will be send to message queue. Leader will also fall into here, and
          // send an empty record to client. That's exactly what we want.
          ...
          // Reading from empty table: we must inform the leader of the constant
          // value it should return for Items with a result field because it has
          // not received any information from the workers that would fill their
          // result_field with the correct values: when their result_field is
          // not nullptr, the leader will have an Item_field (whose field is
          // pointing towards the result_field from the original item) in
          // tmp_table_param->grouped_expressions, expecting that the
            ...
            for (Item *item : *m_join->query_block->get_fields_list()) {
              ...
              if (result_field) {
                /*
                  ...
                  error and Item::save_in_field() don't return error code.
                  So need to check the THD error state.
                */
                int err_code = item->save_in_field(result_field, false);
                DBUG_EXECUTE_IF("pq_simulate_save_in_field_error1",
                                { err_code = TYPE_NOTE_TRUNCATED; });
                DBUG_EXECUTE_IF("pq_simulate_save_in_field_error2", {
                  err_code = TYPE_ERR_BAD_VALUE;
                  my_error(ER_CANT_CREATE_GEOMETRY_OBJECT, MYF(0));
                });
                if (err_code != TYPE_OK &&
                    err_code != TYPE_NOTE_TIME_TRUNCATED &&
                    err_code != TYPE_NOTE_TRUNCATED &&
                    err_code != TYPE_WARN_TRUNCATED) {
                  if (thd()->is_error()) thd()->clear_error();
                  my_error(ER_PARALLEL_QUERY_ERROR, MYF(0),
...
/**Aggregate in storage layer

  @param join  join object of the pushed Iterator

  @return  0 the caller should not contine
           1 OK, the caller should contine to aggregate
*/

int AggregateIterator::Aggregate(const JOIN *join) {
  if (m_join != join || m_first_row) {
    return 0;
  }
  m_agg_in_storage = true;
  ...
  for (Item_sum **item = m_join->sum_funcs; *item != nullptr; ++item) {
    if (m_rollup) {
      if (down_cast<Item_rollup_sum_switcher *>(*item)->aggregator_add_all()) {
        m_agg_error_code = 1;  // keep same as the original err code
        return 0;
      }
    } else {
      if ((*item)->aggregator_add()) {
...
bool NestedLoopIterator::Init() {
  DBUG_EXECUTE_IF("nested_loop_init_error", {
    // Just raise a random error to keep later asserts on thd->is_error() happy.
    my_error(ER_SUBQUERY_NO_1_ROW, MYF(0));
    return true;
  });
  assert(m_join);
...
int NestedLoopIterator::Read() {
  ...
  for (;;) {  // Termination condition within loop.
    if (m_state == NEEDS_OUTER_ROW) {
      ...
      // Store found_records for each outer table access loop.
      m_found_records = m_join->found_records;
    }
    assert(m_state == READING_INNER_ROWS || m_state == READING_FIRST_INNER_ROW);

    // If find one new record for join, we can stop to traverse following
    // records of inner table due to inner table data not used for distinct.
    // Continue next loop of outer table.
...
class DummyIteratorProfiler final : public IteratorProfiler {
 public:
  ...
  void SetFromRunningInfo(const IteratorTimingInfo &info
                          [[maybe_unused]]) override {
    assert(false);
  }

  void CopyToRunningInfo(IteratorTimingInfo &info
...
class MaterializeIterator final : public TableRowIterator {
 public:
  /**
    ...
    @param path_params MaterializePath settings.
      path_params->sj_mat_exec when semijoin is finished with materialization,
      we can share the materialized table between worker and leader to avoid
      duplicate materialization. All threads sharing the same table have their
      own MaterializeIterator and Semijoin_mat_exec (passed to the constructor
      here), but the Semijoin_mat_exec-s communicate through their equal
      member 'm_pq_shared_info'.
      path_params->pq_can_share is true if PQ workers can share the temporary
      table.
  ...
  bool Init() override;
  int Read() override;
  bool init_shared_table();

  int End() override {
    // No need to call query_block's subquery_iterator::End() here, as it has
    // been called in MaterializeIterator::MaterializeQueryBlock() already.

    // reset shared info
    if (table()->file->is_sharing_data()) {
      table()->file->reset_shared_info();
    }
    return m_table_iterator->End();
  }
  ...
  /// Whether we share the materialized table in parallel query,
  /// including derived table and semijoin materialized table.
  bool pq_sharing_table() override;

  // Gather iterator running info from worker to leader
  void GatherTimingInfo(RowIterator *from) override {
    PQGatherTimingInfo(from, m_worker_info);
  }

  // Copy running info from non-parallel iterator to parallel iterator for
  // parallel query leader.
  void CopyTimingInfo(RowIterator *from) override {
    PQCopyTimingInfo(from, m_worker_info, &m_profiler);
  }

  bool GetParallelTimingString(std::string &out) const override {
    if (!m_worker_info || !m_worker_info->copy_num) return false;
    out = ParallelIteratorTimingString(m_worker_info);
    return true;
  }
 ...
 private:
  ...
  /// Semijoin materialized structure, including the shared materialized table
  /// if PQ. We don't push this member into QueryBlock because the subquery is
  /// flattened into one query block. Thus, there is only one materialized
  /// structure in this iterator.
  Semijoin_mat_exec *m_sj_mat_exec{nullptr};

  /// True if PQ supports that workers share the temporary table. Indeed, PQ
  /// does not yet have support for all types of materializations.
  bool m_pq_can_share{false};
  ...
  */
  ...
  ParallelIterTimingInfo *m_worker_info{nullptr};
...
MaterializeIterator<Profiler>::MaterializeIterator(
    ...
    : TableRowIterator(thd, path_params->table),
      ...
      m_sj_mat_exec(path_params->sj_mat_exec),
      m_pq_can_share(path_params->pq_can_share) {
...
bool MaterializeIterator<Profiler>::Init() {
  ...
  // set shared info for worker's materialized table. The owner of this
  // iterator may be a correlated subquery, so Init() and End() may be called
  // multiple times. Therefore, each Init() must call set_shared_info(), as
  // each End() calls reset_shared_info().
  if (pq_sharing_table()) {
    assert(thd() == table()->in_use);
    assert(m_query_expression || m_sj_mat_exec);
    // set worker's shared info
    if (thd()->is_pq_worker()) {
      PQ_shared_info *shared_info =
          m_query_expression
              ? m_query_expression->m_pq_shared_info
              : m_sj_mat_exec ? *m_sj_mat_exec->m_pq_shared_info : nullptr;
      assert(shared_info);
      PQ_temp_table_type tmp_table_type = get_tmp_table_type(table()->s);
      if (tmp_table_type != shared_info->m_type) {
        // In-memory tmp table has changed to disk table.
        assert(shared_info->m_type == PQ_temp_table_type::TEMP_INNODB);
        bool is_duplicate;
        if (create_ondisk_from_heap(thd(), table(), HA_ERR_RECORD_FILE_FULL,
                                    /*insert_last_record=*/true,
                                    /*ignore_last_dup=*/true, &is_duplicate)) {
          return true;
        }
      }
      table()->file->set_shared_info(shared_info);
    }
  }
  ...
  if (m_query_expression != nullptr && !table()->file->is_sharing_data())
    if (m_query_expression->clear_correlated_query_blocks()) return true;

  if (m_cte != nullptr && !table()->file->is_sharing_data()) {
...
bool MaterializeIterator<Profiler>::MaterializeRecursive() {
  ...
  */
  ...
  for (materialize_iterator::QueryBlock &query_block :
       m_query_blocks_to_materialize) {
    if (!query_block.is_recursive_reference) {
      if (MaterializeQueryBlock(query_block, &stored_rows)) return true;
    }
...
bool MaterializeIterator<Profiler>::MaterializeQueryBlock(
  ...
  // directly read data from shared tmp table
  if (query_block.subquery_iterator == nullptr) {
    assert(pq_sharing_table());
    return false;
  }
  ...
  while (true) {
    ...
    if (m_join) m_join->found_records++;
    ...
    // Now semi-join materialized table only supports parallel execution of
    // outer query. The materialization of shared table is still executed only
    // by leader thread, which should be improved in the future. One possible
    // method is add one "SEND/RECEIVE" iterator which can send/receive the
    // partial results from worker to leader such as "send_data" after
    // obtaining each row. This work can be combined into group-aware
    // aggregation.
  }
...
int MaterializeIterator<Profiler>::Read() {
  ...
  */
  ...
  if ((err == 0) && pq_sharing_table() && thd()->is_pq_worker()) {
...
void MaterializeIterator<Profiler>::EndPSIBatchModeIfStarted() {
  for (const materialize_iterator::QueryBlock &query_block :
       m_query_blocks_to_materialize) {
    if (query_block.subquery_iterator)
...
template <typename Profiler>
bool MaterializeIterator<Profiler>::pq_sharing_table() {
  /**
     We allow sharing materialized table in parallel query for two cases:
       1) derived table; and,
       2) semi-join materialization.
     In these cases, m_pq_can_share was set to true when the iterator was
     made, but it was a non-final decision: at that time we didn't know
     if the query owning this iterator would be doing PQ. Now we check this.
     For derived table, we can utilize the unit of "subquery". For semijoin
     materialization, we can use the member "m_pq_shared_info".
   */
  if (m_pq_can_share) {
    // derived table
    ...
    if (m_sj_mat_exec && m_sj_mat_exec->m_pq_shared_info) {
      return true;
    }
  }
  return false;
...
template <typename Profiler>
bool MaterializeIterator<Profiler>::init_shared_table() {
  assert(current_thd->is_pq_leader());
  if (pq_sharing_table()) {
    if (Init()) return true;

    assert(!table()->file->is_sharing_data());
    table()->file->init_shared_info();
    if (m_query_expression) {
      m_query_expression->m_pq_shared_info = table()->file->get_shared_info();
    } else {
      assert(m_sj_mat_exec);
      *m_sj_mat_exec->m_pq_shared_info = table()->file->get_shared_info();
    }
  }
...
class TemptableAggregateIterator final : public TableRowIterator {
 public:
  ...
  int Read() override;
  int End() override {
    m_table_iterator->End();
    m_subquery_iterator->End();
    return -1;
  }
...
bool materialize_iter_init_shared_table(bool explain_analyze,
                                        RowIterator *iterator) {
  if (explain_analyze) {
    auto mat_it =
        down_cast<MaterializeIterator<IteratorProfilerImpl> *>(iterator);
    return mat_it->init_shared_table();
  } else {
    auto mat_it =
        down_cast<MaterializeIterator<DummyIteratorProfiler> *>(iterator);
    return mat_it->init_shared_table();
  }
}

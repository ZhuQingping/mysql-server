...
class HashJoinChunk {
 public:
  HashJoinChunk(VfdManager *vfd_manager) : m_vfd_manager(vfd_manager) {
    assert(m_vfd_manager != nullptr);
  }

  /// Move constructor. May leave "other" in an invalid state, so "other" is not
  /// usable after calling this.
  HashJoinChunk(HashJoinChunk &&other);  // Movable.

  /// Move assignment. May leave "other" in an invalid state, so "other" is not
  /// usable after calling this.
  ...
  ~HashJoinChunk() {
    if (likely(m_vfd_manager != nullptr)) {
      m_vfd_manager->Close(&m_vfd);
    }
  }
  ...
  void Init(bool uses_match_flags);
 ...
 private:
  ...
  // The VfdManager, which is responsible of opening/closing "m_vfd" as
  // necessary. All file operations must go through this manager.
  VfdManager *m_vfd_manager;
};

...
/// Parallel query
///
/// As of writing, HashJoinIterator only supports in-memory hash join with
/// partial data from either the build input, the probe input, or both inputs.
/// "Partial data" means that there is no data overlap between different
/// workers. In the case of partial data from the build input, each worker has
/// its own in-memory hash table without any data sharing between workers.
/// During the probe phase, all workers reads the entire probe input, probing
/// for matches in their own hash table. The only modification compared to
/// single-threaded hash join is that during the build phase, a Bloom filter is
/// created. The Bloom filter is used as pre-filtering in the probe phase. The
/// reasoning for having a Bloom filter is that;
///
/// a) probing a Bloom filter is much cheaper than doing a lookup in the hash
///    table.
/// b) as the number of worker increases, the number of matching rows
///    between the probe input and each hash table reduces. Thus, the Bloom
///    filter should have a reasonable filtering effect.
///
/// Partial data from the probe input is very similar to partial data from the
/// build input; each worker will have their own in-memory hash table, where
/// each worker has the entire build input in the hash table. During the probe
/// phase, each worker will read a non-overlapping subset of the probe input and
/// return all matching rows to the probe input. Since all workers has the
/// entire build input in the hash table, we expect that matches between the
/// probe and build input to be very likely. Thus, we do not create Bloom filter
/// in these cases.
///
/// For partial data from both inputs (also known as parallel-aware hash join),
/// each worker will insert data into their own local hash table. During the
/// probe phase, all workers will read from all other workers' hash table.
/// Having these shared hash tables adds some complexity, and these are the most
/// notable differences compared to "normal" hash join:
/// - There is a barrier after the build phase, meaning that all workers must
///   agree that the build phase is done before proceeding with the probe phase.
/// - There is also a barrier after the probe phase. This is necessary so that
///   one worker does not suddenly start the build phase again while other
///   workers still are in the probe phase.
class HashJoinIterator final : public RowIterator {
 public:
  ...
  /// @param use_bloom_filter
  ///   If true, a Bloom filter is created during the build phase. This Bloom
  ///   filter is used as early filtering during the probe phase. Currently only
  ///   enabled for parallel query.
  /// @param pq_hash_join
  ///   Context for if we are doing a parallel hash join.
  /// @param access_path
  ///   The access path from which this iterator was created.
  ...
  ~HashJoinIterator() override {
    if (m_pq_hash_join != nullptr && !m_end_is_called) {
      // If End() has been called, the number of workers in the barriers have
      // already been decremented inside End(). If End() has not been called, it
      // means than an error happened between the construction of the
      // HashJoinIterator and query execution. If that is the case, decrement
      // the number of participants in the barriers as we do not want other
      // possibly launched workers to wait for these "dead" workers. And since
      // execution was never started, the MEM_ROOT in this worker does not
      // contain any data that other workers may be interested in. So the
      // MEM_ROOT can safely be destroyed without any synchronization.
      m_pq_hash_join->ExecutionBarrier().ArriveAndDrop();
      m_pq_hash_join->DestructionBarrier().ArriveAndDrop();
    }
  }
  ...
  int End() override {
    m_end_is_called = true;
    m_build_input->End();
    m_probe_input->End();

    if (m_pq_hash_join != nullptr) {
      // Reduce the number of workers for the barrier by one so that other
      // workers will not wait for this worker to arrive at any of the
      // barriers. In some cases, a worker may exit early while other
      // workers are still returning rows. So if other workers starts
      // waiting for a worker that has exited, we will end up in a deadlock.
      m_pq_hash_join->ExecutionBarrier().ArriveAndDrop();
      // Since the shared hash table may contain data that is allocated on this
      // worker's MEM_ROOT, wait until all workers are done executing the join
      // so we do not release memory that is being used.
      m_pq_hash_join->DestructionBarrier().ArriveAndWait(
          /*completion_function=*/nullptr);
    }

    return -1;  // only in parallel query
  }
  ...
  int ChunkCount() { return m_chunk_files->Size(); }

  const BloomFilter &GetBloomFilter() const { return m_bloom_filter; }
 ...
 private:
  ...
  bool on_disk_hash_join() const { return m_chunk_files->Size() > 0; }
  ...
  /// Create a Bloom filter over the data that exists in the hash table. Any
  /// previous data in the Bloom filter will be cleared. Note that this function
  /// does not have any effect if m_use_bloom_filter == false.
  ...
  hash_join_buffer::HashMapReader m_reader;
  ...
  // The ChunkFiles abstraction layer in case we degrade into on-disk hash join.
  ...
  // the probe input is consumed. It is not usable in a parallel hash join.
  ...
  //   the entire probe input, probing for matching rows in the hash table. When
  //   the probe input returns EOF, the hash table is refilled with the rows
  //   that did not fit the first time. The entire probe input is read again,
  //   and this is repeated until the entire build input is consumed.
  ...
  // Whether a Bloom filter should be created during the build phase.
  bool m_use_bloom_filter{false};
  BloomFilter m_bloom_filter;

  // In case of parallel hash join, this will be the shared area for all
  // workers. It is used for accessing the shared hash tables, synchronizing
  // state between workers etc.
  HashJoin::PQHashJoinSharedContext *m_pq_hash_join{nullptr};

  // Whether End() has been called on this iterator. This is used by parallel
  // query to avoid calling ArriveAndDrop both in End() and in the destructor.
  bool m_end_is_called{false};

  // The last seen ChunkIndex. That is, the last seen value from
  // "chunk_files_on_disk->CurrentChunkIndex()". Used to determine whether
  // calling "chunk_files_on_disk->PossiblyMoveToNextChunk()" actually caused a
  // move.
  int m_last_seen_chunk_index{-1};
  // The access path from which this iterator was created. Used to access other
  // iterators in the execution tree (see HashJoinIterator::Init() where it is
  // used.)
  const AccessPath *const m_access_path;
};

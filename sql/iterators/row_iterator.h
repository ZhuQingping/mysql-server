...
class IteratorProfiler {
 public:
  ...
  virtual void SetFromRunningInfo(const IteratorTimingInfo &info) = 0;

  virtual void CopyToRunningInfo(IteratorTimingInfo &info) const = 0;
};
...
class RowIterator {
 public:
  ...
  /**
     Synchronizes the status between worker threads. Generally Init()
     has been called before, but this is not always true, so don't rely on it.
  */
  ...
  virtual void GatherTimingInfo(RowIterator * /* from */) {}
  virtual void CopyTimingInfo(RowIterator * /* from */) {}
  virtual bool GetParallelTimingString(std::string &out
                                       [[maybe_unused]]) const {
    return false;
  }
...
class TableRowIterator : public RowIterator {
 public:
  ...
  // Only implemented by MaterializeIterator
  virtual bool pq_sharing_table() {
    assert(false);
    return false;
  }

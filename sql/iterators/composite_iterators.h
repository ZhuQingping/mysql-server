...
class AggregateIterator final : public RowIterator {
 ...
 private:
  ...
  bool m_first_row = false;
  /*the group is aggregated in storage layer*/
  bool m_agg_in_storage = false;
  /*agg in storage has error*/
  int m_agg_error_code = 0;
...
class NestedLoopIterator final : public RowIterator {
 public:
  NestedLoopIterator(THD *thd,
                     ...
                     JoinType join_type, bool pfs_batch_mode, JOIN *join,
      ...
      : RowIterator(thd),
        ...
        m_join(join),
        m_inner_not_used_in_distinct(distinct_optimize) {
    ...
    // be reading one row. Also no need to do distinct optimization for
    // anti-join or semi-join.
    if (join_type == JoinType::ANTI || join_type == JoinType::SEMI) {
      assert(!pfs_batch_mode);
      assert(!m_inner_not_used_in_distinct);
    }
  }
 ...
 private:
  ...
  /* Need to access join->found_records for distinct optimization */
  const JOIN *m_join = nullptr;
  /* All inner tables not used in distinct */
  bool m_inner_not_used_in_distinct = false;
  /* Record join->found_records for each outer loop */
  ha_rows m_found_records = 0;
};
...
class StreamingIterator final : public TableRowIterator {
 public:
  ...
  int End() override { return m_subquery_iterator->End(); }
...
/**
  Init shared table for MaterializeIterator, this interface used for parallel
  query, due to class MaterializeIterator defined in file
  composite_iterators.cc, we need to provide one interface to call
  MaterializeIterator::init_shared_table.

  @param explain_analyze Query is explain analyze or not.
  @param iterator        The MaterializeIterator.

  @returns
    Return false if init shared table success, return true if error.
*/

...
void JOIN::destroy() {
  ...
  */
  ...
  // group fields has been destory above, but group_fields_cache needs to be
  // cleared to set elements 0 so that they won't be reused when applying
  // cached plan. Details can be referenced by make_group_fields.
  group_fields_cache.clear();
...
void calc_length_and_keyparts(Key_use *keyuse, QEP_shared_owner *tab,
                              const uint key, table_map used_tables,
                              Key_use **chosen_keyuses, uint *length_out,
                              uint *keyparts_out, table_map *dep_map,
                              bool *maybe_null) {
  assert(!dep_map || maybe_null);
  uint keyparts = 0, length = 0;
  uint found_part_ref_or_null = 0;
  KEY *const keyinfo = tab->table()->key_info + key;
  // Here we need a backup for Table_ref because QEP_shared_owner has no way
  // to get the same Table_ref. One possible way is checking
  // QEP_shared_owner::table()->pos_in_table_list. But one exception is
  // multiple UPDATE, such a pos_in_table_list might point to different
  // Table_ref from keyuse->table_ref.
  Table_ref *table_ref = keyuse->table_ref;
  ...
  do {
    ...
    */
    ...
    keyuse++;
  } while (keyuse->table_ref == table_ref &&
           keyuse->table_ref->table == tab->table() && keyuse->key == key);
  if (keyparts <= 0) {
    assert(false);
  ...
  */
  if (condition() &&
    ...
    DBUG_EXECUTE("where", print_where(join_->thd, condition(), "full cond",
                                      QT_ORDINARY););
    if (pq_div_tab) {
      // During the process of pq_clone, some cloned items may be
      // fixed. Thus, we should skip the privilege check.
      ulong saved_thd_want_privilege = join_->thd->want_privilege;
...
/**
  Check whether the sumfunc  supported pushdown

  @param sum_type the sumfunc type

  @retval true the sumfucn can be pushed down
          false the sumfunc can not be pushed down
 */
bool is_sumfunc_supported(Item_sum::Sumfunctype sum_type) {
  switch (sum_type) {
    case Item_sum::COUNT_FUNC:         // fall through
    case Item_sum::SUM_FUNC:           // fall through
    case Item_sum::AVG_FUNC:           // fall through
    case Item_sum::MIN_FUNC:           // fall through
    case Item_sum::MAX_FUNC:           // fall through
    case Item_sum::STD_FUNC:           // fall through
    case Item_sum::VARIANCE_FUNC:      // fall through
    case Item_sum::SUM_BIT_FUNC:       // fall through
    case Item_sum::GROUP_CONCAT_FUNC:  // fall through
...
/**
  Check aggregate pushdown feasibility, and if so, push it down.

  @retval
    false	pushdown is ok
  @retval
    true	There is some error when pushdown
*/
bool QEP_TAB::pushdown_aggregate() {
  TABLE *const tbl = table();
  const tmp_table_type tmptp = tbl->s->tmp_table;
  const join_type access_type = type();
  const AccessPath *range_scan_ap = range_scan();
  JOIN *const join_ = join();
  if (!join_ || (!join_->implicit_grouping && !join_->group_list.order)) {
    return false;
  }

  const ORDER *group = join_->group_list.order;
  Item_sum *const *func_ptr = join_->sum_funcs;
  Item_sum *func = NULL;

  /*
   * a. permanent table or user temp table.
   *    test TRANSACTIONAL_TMP_TABLE for user temp table.
   * b  not distinct and not filesort (because in either case, table has to be
   *    "pre-processed" before aggregate can take place)
   * c. not inner table (null-producing side) of outer join
   * d. not use join cache
   * e. not use dynamic range. With dynamic range, the "quick type" is
   *    not determined until runtime.
   * f. not semi-join duplicate elimination.
   * g. the storage engine supports aggregate pushdown. so far only InnoDB.
   * h. disable partitioned table for now. just to limit test scope.
   */
  if ((NO_TMP_TABLE == tmptp || TRANSACTIONAL_TMP_TABLE == tmptp) &&  // a
      !needs_duplicate_removal &&                                     // b
      NULL == filesort &&                                             // b
      !is_inner_table_of_outer_join() &&                              // c
      op_type != QEP_TAB::OT_BNL && op_type != QEP_TAB::OT_BKA &&     // d
      !dynamic_range() &&                                             // e
      !do_firstmatch() &&                                             // f
      !finishes_weedout() &&                                          // f
      (tbl->file->ha_table_flags() & HA_AGGREGATE_PUSHDOWN) &&        // g
      NULL == tbl->part_info                                          // h
  ) {
    /*assert_cond must be true at this point.
     To be safe, we assert in debug build but bail out in customer build
     "not_used_in_distinct" optimization can't be used when there is aggregate
     "do_loosescan" is never set at the last table in the join sequence.*/
    {
      const bool assert_cond = (!not_used_in_distinct && !do_loosescan());
      assert(assert_cond);
      if (!assert_cond) {
        return false;
      }
    ...
    // only allow five access types: JT_INDEX_SCAN, JT_ALL, JT_REF,
    // JT_REF_OR_NULL, or JT_RANGE.
    // condition() is not allowed unless the access type uses index
    // and we haven't previously tried ICP
    switch (access_type) {
      case JT_ALL:          // fall through
      case JT_REF:          // fall through
      case JT_REF_OR_NULL:  // fall through
      case JT_RANGE:        // fall through
      case JT_INDEX_SCAN:
        if (condition()) {
          return false;
        }
        break;
      default:  // access_type not allowed
        return false;
    }

    // Further check the types of "quick".
    // Only allow AccessPath::INDEX_RANGE_SCAN,
    // but disallow disk-sweep MRR
    // Other "quick" types, such as QS_TYPE_GROUP_MIN_MAX (aka index loose scan)
    // INDEX_MERGE, etc, are not eligible.
    if (range_scan_ap) {
      if (range_scan_ap->type == AccessPath::INDEX_RANGE_SCAN) {
        if (!(range_scan_ap->index_range_scan().mrr_flags &
              HA_MRR_USE_DEFAULT_IMPL)) {
          // don't pushdown aggregate if disk-sweep mrr access
          return false;
        }
      } else {
        return false;
    ...
    // check all aggregate functions are pushdownable
    for (; (func = *func_ptr); func_ptr++) {
      const Item_sum::Sumfunctype sum_type = func->sum_func();
      const uint arg_count = func->argument_count();
      // Check the type of aggregate function for safety
      if (!is_sumfunc_supported(sum_type)) {
        return false;
      }

      for (uint i = 0; i < arg_count; i++) {
        if (!item_pushdownable(func->get_arg(i), tbl, 0, true)) {
          return false;
        }  // end if (!item_pushdowable
      }    // end for (unit i=0
    }      // end for (; (func= *func_ptr

    // check all groupby expressions are pushdownable
    for (; group; group = group->next) {
      if (!item_pushdownable(*group->item, tbl, 0, true)) {
        return false;
      }  // end if (!item_pushdownable
    }    // end for (; group

    Opt_trace_context *const trace = &join_->thd->opt_trace;
    Opt_trace_object wrapper(trace);
    Opt_trace_object trace_aggr_pushdown(trace, "aggregate_pushdown");
    trace_aggr_pushdown.add_utf8_table(table_ref);

    m_aggregate_context = new (join_->thd->mem_root) Aggregate_context(join_);
    if (NULL == m_aggregate_context) {
      // error
      return true;
    }
    tbl->file->aggregate_push(m_aggregate_context);
    trace_aggr_pushdown.add("chosen", true);
  }
...
bool JOIN::setup_semijoin_materialized_table(JOIN_TAB *tab, uint tableno,
  ...
  */
  ...
  query_block->consume_tableno(tableno);
...
bool make_join_readinfo(JOIN *join, uint no_jbuf_after) {
  ...
  for (uint i = join->const_tables; i < join->tables; i++) {
    ...
    */
    ...
    /*
      We think that a materialized derived table doesn't do range scan, as its
      columns are indexed only if they are involved in an equality, and the
      optimizer prefers 'ref' to 'range' for equalities.
      We also think that such table doesn't do index scan, as the Temptable
      engine doesn't support index_first().
      This assertion verifies the reasoning above. If it fires in the future,
      it will mean we have new scenarios to test in PQ for temporary tables.
    */
    assert(!(qep_tab->table_ref->is_view_or_derived() &&
             (qep_tab->type() == JT_INDEX_SCAN ||
              qep_tab->range_scan() != nullptr)));
...
void JOIN::offset_pushdown() {
  ...
  if (offset_value != 0 && (primary_tables - const_tables) == 1) {
    ...
    if (!select_distinct && !grouped && !group_optimized_away &&
      ...
      /*
        Parallel query and offset pushdown cannot be enabled simultaneously.
        Row skipping in InnoDB is not compatible with parallel thread. At the
        very least, row ordering would not be conserved.

        When the offset value is large, offset pushdown will definitely bring
        significant performance improvements, while the performance benefits
        of parallel query are not clear. In this case, disable parallel query.

        When the offset value is small, the performance improvement brought
        by offset pushdown is minimal. In this case, disable offset pushdown.
        Note that if we have a correlated subquery inside a top query which
        does PQ, the subquery is executed inside each worker, so the subquery
        could do offset pushdown; now it does not because its m_suite_for_pq
...
void QEP_TAB::cleanup() {
  ...
  if (op_type == QEP_TAB::OT_MATERIALIZE ||
      ...
      op_type == QEP_TAB::OT_RECV_MSG_TMP_TABLE) {
...
bool JOIN::make_worker_tmp_table() {
  DBUG_TRACE;
  TABLE *table = nullptr;
  ...
  bool flag = (last_slice_before_pq == REF_SLICE_SAVED_BASE);
  /* field comes from field_list or tmp_table's fields */
  curr_fields = flag ? fields : &tmp_fields[last_slice_before_pq];
  query_result =
      down_cast<Query_result_mq *>(query_expression()->query_result());
  ...
  /*
   * set saved_sum_func= true, then temp table will create a Item_field for sum
   * funcs in tmplist, which we call it sum_field that use for receiving PQ
   * workers's sum data
   */
  ...
  if (alloc_ref_item_slice(thd, REF_SLICE_PQ_TMP)) goto err;

  for (uint i = 0; i < curr_fields->size(); i++) {
    tmp_item = curr_fields->operator[](i);
    if (tmp_item->has_aggregation() &&
        tmp_item->type() != Item::SUM_FUNC_ITEM &&
        tmp_item->used_tables() & ~OUTER_REF_TABLE_BIT) {
      continue;
    }

    if (tmp_item->get_result_field())
      tmp_item->get_result_field()->pq_worker_fields = true;
    tmp_fields[REF_SLICE_PQ_TMP].push_back(tmp_item);
  }
...
bool JOIN::make_leader_tmp_table() {
  DBUG_TRACE;
  ...
  int curr_tmp_table = primary_tables;

  QEP_TAB *tab = &qep_tab[curr_tmp_table];
  pq_replace_avg_func(thd, query_block, curr_fields,
                      (nesting_map)1 << query_block->nest_level);
  Temp_table_param *tmp_param =
      new (thd->mem_root) Temp_table_param(*THR_MALLOC, tmp_table_param);
  if (!tmp_param) return true;
  ...
  /*
   * set saved_sum_func= true, then temp table will create a Item_field for sum
   * funcs in tmplist, which we call it sum_field that use for recieving PQ
   * workers's sum data
   */
  TABLE *table = create_tmp_table(thd, tmp_param, *curr_fields, nullptr, false,
                                  true, query_block->active_options(),
                                  HA_POS_ERROR, "<receive_data>", false, true);
  if (!table) return true;
  table->materialized = false;
  tmp_tables++;

  // check the rewritten table, and will remove them in the future
  assert(tab->gather);
  tab->gather->pq_check_fields = table->s->fields;
  tab->gather->pq_check_reclen = table->s->reclength;

  tab->set_table(table);
  tab->table_ref->table = table;
  tab->tmp_table_param = tmp_param;

  /*
     Allocate a slice of ref items that describe the items to be copied
     from the second temporary table and re-create sum funcs:
       1. alloc_ref_item
       2. change to use result field
       3. re-build sum funcs

  */
  if (alloc_ref_item_slice(thd, REF_SLICE_PQ_TMP) ||
      change_to_use_tmp_fields(curr_fields, thd, ref_items[REF_SLICE_PQ_TMP],
                               &tmp_fields[REF_SLICE_PQ_TMP],
                               query_block->m_added_non_hidden_fields) ||
      pq_build_sum_funcs(thd, query_block, ref_items[REF_SLICE_PQ_TMP],
                         &tmp_fields[REF_SLICE_PQ_TMP],
                         (nesting_map)1 << query_block->nest_level)) {
    return true;
  }

  curr_fields = &tmp_fields[REF_SLICE_PQ_TMP];
...
bool JOIN::make_tmp_tables_info() {
  ...
  if (need_tmp_before_win) {
    ...
    */
    ...
    if (select_distinct && group_list.empty() && !m_windowing_steps) {
      ...
      /*
        Can't push down select_distinct to woker because BUG2023080901645.
        Example:
        QUERY :
        create table t(c0 int);
        insert into t values(NULL),(1),(2),(NULL),(1);
        select distinct count(*) from t group by t.c0;
        ...
        leader:
          ...
          (2) distinct `count(*)`
              count(*)
              1
        Query results without PQ:
          count(*)
          1
          2

        TODO:
        If want to push down select_distinct to worker, in woker, `Sort with
        duplicate removal(distinct)` operation should use `c0, count(*)`
        and in leader, `Sort with duplicate removal` operation uses `count(*)`.
      */
      if (!thd->is_pq_worker()) {
  ...
  */
  ...
  if (qep_tab && (!group_list.empty() ||
    ...
    */
    if (best_ref) {
      // The code below supports the case where best_ref is not set (happens
      // for PQ and when a cached plan is reused).
      ASSERT_BEST_REF_IN_JOIN_ORDER(this);
    }
    ...
    if (grouped)
      ...
      */
      for (uint i = const_tables + 1; i < primary_tables; i++) {
        ...
        Key_use *key = nullptr;
        if (!thd->is_pq_worker()) {
          key = best_ref[tab->idx()]->keyuse();
        } else {  // JOIN::best_ref is not copied during pq_clone, is nullptr.
          key = tab->position() ? tab->position()->key : nullptr;
        }
        if (tab->condition() ||                          // 1
            (key && tab->first_inner() == NO_PLAN_IDX))  // 2
        {
  ...
  if (qep_tab) {
    ...
    // determine aggregate pushdown feasibility
    if (thd->optimizer_switch_flag(OPTIMIZER_SWITCH_AGGREGATE_PUSHDOWN)) {
      // The aggregation operation logically occurs after all table joins are
      // completed, while the executor processes tables one by one in the join
      // order. A complete "joined row" is only formed when the last table's
      // rows are read, at which point the aggregation calculation can be safely
      // triggered or the fast path of the storage engine can be used.
      // So, set the grouped info on the last table
      QEP_TAB *const last_tab = get_last_tab();
      const QEP_TAB *const first_non_const_tab = &(qep_tab[const_tables]);
      if (likely(last_tab >= first_non_const_tab)) {
        if (last_tab->pushdown_aggregate()) {
          return true;
        }
      }
    }
...
void JOIN::refresh_base_slice() {
  // Here use tmp_fields[REF_SLICE_PQ_TMP] fields list to refresh
  // it, otherwise the ref_items might be replaced by some 'old' item, and
  // at last, in Exchange_sort::heap sort, it will use wrong fields to get
  ...
  for (unsigned i = 0; i < curr_fields->size(); ++i) {
    Item *item = (*curr_fields)[i];
    size_t pos;
    ...
    if (i < num_hidden_fields) {
      pos =
          curr_fields->size() - i - 1 - query_block->m_added_non_hidden_fields;
    } else {
...
void JOIN::unplug_join_tabs() {
  // PLAN_CACHE_PORT imported from PQ code. Needed by plan cache.
  if (tables != 0 && !(best_ref && !join_tab)) return;
...
bool JOIN::add_sorting_to_table(uint idx, ORDER_with_src *sort_order,
  ...
  DBUG_TRACE;
  // ASSERT_BEST_REF_IN_JOIN_ORDER(this) is not necessary, as this function
  // doesn't use best_ref anymore.

...
bool setup_sum_funcs(THD *thd, Item_sum **func_ptr) {
  ...
  while ((func = *(func_ptr++))) {
    ...
    if ((func->sum_func() == Item_sum::COUNT_DISTINCT_FUNC) &&
        (!func->get_aggr() ||
...
/**
  Check whether can do distinct optimization for nested loop join.

  @param[in] join_type what kind of join we are executing
  @param[in] inner_tables tablemap of nested loop join inner tables.
  @param[in] join pointer of JOIN plan
  @retval true if we can do distinct optimization for nested loop join, false
  otherwise.
*/
static bool CheckNljDistinctOptimze(JoinType join_type,
                                    qep_tab_map inner_tables,
                                    const JOIN *join) {
  DBUG_ASSERT(join);
  // Distinct optimization only support Inner or Outer join type. No need to do
  // distinct optimization for anti-join or semi-join, since access inner table
  // will stop immediately once find/not find one record.
  bool inner_not_used_in_distinct = false;
  if (join_type == JoinType::INNER || join_type == JoinType::OUTER) {
    inner_not_used_in_distinct = true;
    plan_idx first_idx = NO_PLAN_IDX;
    /* Access all inner tables, check whether table not used in distinct. */
    for (QEP_TAB *tab : TablesContainedIn(join, inner_tables)) {
      first_idx = (NO_PLAN_IDX == first_idx) ? tab->idx() : first_idx;
      inner_not_used_in_distinct =
          (inner_not_used_in_distinct && tab->not_used_in_distinct);
    }
#ifndef NDEBUG
    if (inner_not_used_in_distinct && (first_idx != NO_PLAN_IDX)) {
      // If we can do distinct optimization for nested loop join inner table,
      // then all tables after current table in the slice should not used in
      // distinct.
      bool distinct_optimize = true;
      for (uint i = first_idx; i < join->primary_tables; i++) {
        distinct_optimize =
            (distinct_optimize && join->qep_tab[i].not_used_in_distinct);
      }
      assert(distinct_optimize);
    }
#endif  // NDEBUG
  }
  return inner_not_used_in_distinct;
}

/**
  Create a Nested loop join path.

  @param[in] thd pointer of session
  @param[in] outer outer path of nested loop join
  @param[in] inner inner path of nested loop join
  @param[in] join_type join type of nested loop join
  @param[in] outer_tables tablemap which is used to construct key tables of
  partial result cache.
  @param[in] inner_tables tablemap which is used to construct result tables
  of partial result cache, also used for distinct optimize check.
  @param[in] join pointer of JOIN plan, which is used to recognize whether
  partial result cache should be considered, also used for distinct optimize
  check.

  @retval path of created nested loop join
*/
AccessPath *CreateNestedLoopAccessPath(THD *thd, AccessPath *outer,
  ...
  path->nested_loop_join().distinct_optimize = false;

  if (join) {
    inner = ptrc::CreateAccessPath(thd, nullptr, inner, outer_tables,
                                   inner_tables, join);
    path->nested_loop_join().distinct_optimize =
        thd->optimizer_switch_flag(OPTIMIZER_SWITCH_NLJ_DISTINCT_OPTIMIZE) &&
        CheckNljDistinctOptimze(join_type, inner_tables, join);
  }
  ...
  if (join_type == JoinType::ANTI || join_type == JoinType::SEMI) {
    ...
    assert(!path->nested_loop_join().distinct_optimize);
  } else {
...
table_map ConvertQepTabMapToTableMap(JOIN *join, qep_tab_map tables) {
  table_map map = 0;
  for (QEP_TAB *tab : TablesContainedIn(join, tables)) {
    if (tab->table_ref) map |= tab->table_ref->map();
  }
  return map;
}
...
void ConvertItemsToCopy(const mem_root_deque<Item *> &items, Field **fields,
  ...
  if (param->items_to_copy) {
    destroy(param->items_to_copy);
...
static AccessPath *NewWeedoutAccessPathForTables(
  ...
  // The present function is used only for the rare case where the decision to
  // use Weedout is taken as a last resort when we build access paths in
  // ConnectJoins(), not for the more common case where the decision is taken
  // cost-based and earlier in choose_table_order(). As a result, this
  // decision is not reflected in the QEP_TABs, which makes it invisible to
  // the way PQ's plan cloning works (which is based on QEP_TABs). So, if the
...
AccessPath *GetAccessPathForDerivedTable(
  ...
  // Leader has filled the results of inner subquery into the shared
  // derived-table in first phase, thus each worker in second phase has
  // no need to generate tmp_table_param's copy structure.
  if (!(thd->is_pq_worker() && query_expression->outer_query_block())) {
  ...
  if (query_expression->unfinished_materialization()) {
    ...
    path = NewMaterializeAccessPath(
        ...
        query_expression->offset_limit_cnt == 0
            ...
            : false,
        /*query_block_boundary=*/true, /*sj_mat_exec=*/nullptr,
        /*pq_can_share=*/true);
    ...
    if (query_expression->offset_limit_cnt != 0) {
      ...
      path = NewLimitOffsetAccessPath(
          ...
          /*send_records_override=*/nullptr);
      query_expression->wrapped_materialize_with_limit = true;
    }
    ...
    // query_expression->root_access_path() is nullptr for pq because it is
    // created at a later stage. So, MaterializeQueryBlock->subquery_path is
    // nullptr for pq.
    path = NewMaterializeAccessPath(
        thd,
    ...
    if (thd->is_pq_worker() &&
        query_expression->wrapped_materialize_with_limit) {
      assert(query_expression->offset_limit_cnt &&
             query_expression->is_union());
      // When unit has UNION, it will generate m_query_blocks_to_materialize to
      // wrap each query block. At the same time, if there is offset in this
      // unit, MaterializeAccessPath will be wrapped into a
      // LimitOffsetAccessPath. However, once leader has finished to create the
      // access path, it will release m_query_blocks_to_materialize, and thus
      // worker encounters here.

      // Therefore, we should wrap LimitOffsetAccessPath above
      // MaterializeAccessPath for worker. In the future, when we finish the
      // clone of whole query unit, each worker does not share the same inner
      // subquery's unit and thus we can remove this flag
      // "wrapped_materialize_with_limit".
      path = NewLimitOffsetAccessPath(
          thd, path, query_expression->select_limit_cnt,
          query_expression->offset_limit_cnt,
          /*count_all_rows=*/false, query_expression->m_reject_multiple_rows,
...
AccessPath *GetTableAccessPath(THD *thd, QEP_TAB *qep_tab, QEP_TAB *qep_tabs) {
  ...
  if (qep_tab->materialize_table == QEP_TAB::MATERIALIZE_DERIVED) {
    ...
    table_path = NewMaterializeAccessPath(
        ...
        /*reject_multiple_rows=*/false, /*query_block_boundary=*/true,
        /*sj_mat_exec=*/sjm, /*pq_can_share=*/true);
...
static inline void pq_rewrite_rowcount(AccessPath *path, double &rows_fetched,
                                       double &prefix_rowcount) {
  // For worker plan, worker's rows_fetched is set to the average of
  // origin rows_fetched. Same as prefix_rowcount to get exact cost.
  if ((path->type == AccessPath::PQ_BLOCK_SCAN) ||
...
void SetCostOnTableAccessPath(const Cost_model_server &cost_model,
                              ...
                              AccessPath *path, bool pq_rewrite) {
  double rows_fetched = pos->rows_fetched;
  double prefix_rowcount = pos->prefix_rowcount;
  if (pq_rewrite) pq_rewrite_rowcount(path, rows_fetched, prefix_rowcount);
  double num_rows_after_filtering = rows_fetched * pos->filter_effect;
  if (is_after_filter) {
    path->set_num_output_rows(num_rows_after_filtering);
  } else {
    path->set_num_output_rows(rows_fetched);
  }
  ...
  if (prefix_rowcount <= 0.0) {
    path->cost = cost;
  } else {
    // Scale the estimated cost to being for one loop only, to match the
    // measured costs.
    path->cost = cost * num_rows_after_filtering / prefix_rowcount;
  }
}
...
/*
   In the context of an Outer Join, if the number of rows in the inner table
   is zero, the actual join rows should equal the number of rows in the outer
   table, not the multiplied result of zero.
   If the return value is false, joined_rows has been adjusted to
   outer->num_output_rows; if the return value is true, joined_rows has not
   been adjusted.
*/
bool AdjustOutLefJoinRows(AccessPath *path, double inner_rows,
                          double &joined_rows) {
  AccessPath *outer = nullptr;
  if (path->type == AccessPath::NESTED_LOOP_JOIN &&
      path->nested_loop_join().join_type == JoinType::OUTER) {
    outer = path->nested_loop_join().outer;
  } else if (path->type == AccessPath::HASH_JOIN &&
             path->hash_join().join_predicate->expr->type ==
                 RelationalExpression::LEFT_JOIN) {
    outer = path->hash_join().outer;
  } else {
    return true;
  }
  if (IsEqual(inner_rows, 0.0)) {
    joined_rows = outer->num_output_rows();
    return false;
  }
  return true;
}

static void UpdateNestedLoopAccessPathCost(
    const Cost_model_server &cost_model, const POSITION *pos_inner,
    AccessPath *path, table_map div_table_map, AccessPath *inner,
    AccessPath *outer, double joined_rows,
    double inner_expected_rows_before_filter) {
  /*
    NESTLOOP-JOIN estimate total cost of reading inner table:
      scan_and_filter_cost =
      prefix_rowcount *
      (single_scan_read_cost + cost_model->row_evaluate_cost(
                                    tab->records() - *rows_after_filtering))
    prefix_rowcount corresponds to the outer table row count.
    single_scan_read_cost corresponds to the inner table scan cost. Therefore,
  ...
  */
  ...
  // The number of rows in the join result will not exceed the number of rows
  // in the outer table when the inner table uses a single-row index.
  if ((inner_expected_rows_before_filter > 1.0) &&
      ((inner->type == AccessPath::EQ_REF) ||
       (inner->type == AccessPath::MATERIALIZE &&
        inner->materialize().table_path != nullptr &&
        inner->materialize().table_path->type == AccessPath::EQ_REF))) {
    path->set_num_output_rows(outer->num_output_rows() *
                              pos_inner->filter_effect);
  }
}

void SetCostOnNestedLoopAccessPath(const Cost_model_server &cost_model,
                                   const POSITION *pos_inner, AccessPath *path,
  ...
  if (path->type == AccessPath::NESTED_LOOP_SEMIJOIN_WITH_DUPLICATE_REMOVAL) {
    ...
    path->nested_loop_semijoin_with_duplicate_removal().pos_inner = pos_inner;
  } else {
    assert(path->type == AccessPath::NESTED_LOOP_JOIN);
    outer = path->nested_loop_join().outer;
    inner = path->nested_loop_join().inner;
    path->nested_loop_join().pos_inner = pos_inner;
  }
  ...
  if (!AdjustOutLefJoinRows(path, inner_expected_rows_before_filter,
                            joined_rows))
    path->set_num_output_rows(
        joined_rows);  // The rows_number of the inner table is 0
  else
    path->set_num_output_rows(joined_rows * pos_inner->filter_effect);
  path->cost = outer->cost + pos_inner->read_cost +
               cost_model.row_evaluate_cost(joined_rows);
  UpdateNestedLoopAccessPathCost(cost_model, pos_inner, path, div_table_map,
                                 inner, outer, joined_rows,
                                 inner_expected_rows_before_filter);
}
...
/// Do a rough estimation of the number of times we expect the hash table to be
/// re-filled. The number of expected re-fills is also the expected number of
/// times we have to re-scan the entire probe input.
static double EstimateNumRefillsForHashJoin(AccessPath *build_path,
                                            size_t join_buffer_size,
                                            const JOIN *join [[maybe_unused]]) {
  if (build_path->num_output_rows() == -1.0) {
    // The number of produced rows for the build input is unknown. In this case,
    // we assume that we do not get any refills of the hash table.
    return 0;
  }

  pack_rows::TableCollection tables(
      GetUsedTables(build_path),
      /*store_rowids=*/false, 0,
      /*tables_to_store_contents_of_null_rows_for*/ 0);

  const size_t row_size_upper_bound =
      pack_rows::ComputeRowSizeUpperBound(tables);

  const double estimated_build_input_size =
      row_size_upper_bound * build_path->num_output_rows();

  const double estimated_num_refills =
      estimated_build_input_size / join_buffer_size;
  return estimated_num_refills;
}
...
static AccessPath *CreateHashJoinAccessPath(
    ...
    table_map *conditions_depend_on_outer_tables) {
  // If parallel hash_join_spill_to_disk is OFF, go back to the old behavior
  // where spill to disk was not supported for parallel hash join.
  if (!thd->pq_support_features_switch_flag(
          PQ_SUPPORT_FEATURES_SWITCH_HASH_JOIN_SPILL_TO_DISK)) {
    double expected_num_refills = EstimateNumRefillsForHashJoin(
        build_path, thd->variables.join_buff_size, qep_tab->join());

    DBUG_EXECUTE_IF("ignore_pq_hash_join_num_refills",
                    expected_num_refills = 0.0;);

    if (expected_num_refills >
        thd->variables.pq_hash_join_max_hash_table_refills) {
      // If we have to re-fill the hash table more than one time, disable
      // parallel query. This is because spill to disk is is disabled for
  ...
  const table_map probe_used_tables = GetUsedTableMap(probe_path, false);
  const table_map build_used_tables = GetUsedTableMap(build_path, false);
  for (const HashJoinCondition &condition : hash_join_conditions) {
  ...
  // Will be set later in MarkPartialInputsForHashJoin().
  path->hash_join().partial_results_from_build_input = false;
  path->hash_join().partial_results_from_probe_input = false;
  path->hash_join().outer_qep_tab = qep_tab;
...
bool QueryMixesOuterBKAAndBNL(JOIN *join) {
...
AccessPath *ConnectJoins(plan_idx upper_first_idx, plan_idx first_idx,
  ...
  for (plan_idx i = first_idx; i < last_idx;) {
      ...
      *conditions_depend_on_outer_tables |=
      ...
      if (qep_tab->table()->reginfo.not_exists_optimize) {
        ...
        for (auto it = subtree_pending_conditions.begin();
          ...
          if (it->table_index_to_attach_to == int(i) &&
              (it->cond->item_name.ptr() == antijoin_null_cond ||
               (it->cond->item_name.ptr() && antijoin_null_cond &&
                0 == strcmp(it->cond->item_name.ptr(), antijoin_null_cond)))) {
        ...
        for (auto it = subtree_pending_join_conditions.begin();
          ...
          if (it->table_index_to_attach_to == int(i) &&
              (it->cond->item_name.ptr() == antijoin_null_cond ||
               (it->cond->item_name.ptr() &&
                !strcmp(it->cond->item_name.ptr(), antijoin_null_cond)))) {
            assert(nullptr != dynamic_cast<Item_func_false *>(it->cond));
            join_type = JoinType::ANTI;
      ...
      if (i != first_idx && qep_tabs[i - 1].do_loosescan() &&
        ...
        // For PTRC, all tables from the first non-const tables up to the last
        // table of the left side of the nested loop join, will be used as
        // dependencies. It may be excessive, for example in
        // (t1,t2) JOIN t3 ON t2.a=t3.b , the rows to be found for t3 don't
        // depend on t1.
    ...
    if (is_bka) {
      ...
      uint mrr_mode;
      if (qep_tab->position()->table) {
        mrr_mode = qep_tab->position()->table->join_cache_flags;
      } else {
        assert(thd->is_pq_worker());
        mrr_mode = qep_tab->join_cache_flags;
      }

      table_path = NewMRRAccessPath(thd, qep_tab->table(), &ref, mrr_mode);
    ...
    if (!qep_tab->condition_is_pushed_to_sort()) {  // See the comment on #2.
      ...
      if (expected_rows >= 0.0 && !predicates_below_join.empty() &&
          pos != nullptr) {
        if (table_path->type == AccessPath::FILTER)
          table_path->filter().qep_tab = qep_tab;
...
void JOIN::create_access_paths() {
  // For pq + explain analyze scenario, leader's m_root_access_path is not
  // nullptr (store non-pq's access path), update to pq leader's access path.
  assert(m_root_access_path == nullptr ||
         (thd->is_pq_leader() /*&& thd->lex->is_explain_analyze*/));
      ...
      if (const_tables > 0) {
        ...
        if (qep_tab->op_type == QEP_TAB::OT_MATERIALIZE) {
          ...
          path = NewMaterializeAccessPath(
              ...
              /*reject_multiple_rows=*/false, /*query_block_boundary=*/false,
              /*sj_mat_exec=*/nullptr, /*pq_can_share=*/false);
          EstimateMaterializeCost(thd, path);
        }
      }
    }
...
int join_read_const_table(JOIN_TAB *tab, POSITION *pos) {
  ...
  /*
    When PTRC takes key columns from const table, it will use record to do
    hashing. Const table will have no chance to do such a null_flags reset
    when PTRC is decided because data has already been read. So here we do
    such a reset firstly. During PTRC, we will skip const_table to reset
    null_flags.
  */
  if (table->s->null_bytes > 0) {
    DBUG_ASSERT(!table->has_null_row());
    memset_s(table->null_flags, table->s->null_bytes, 0, table->s->null_bytes);
  }
...
int read_system(TABLE *table) {
  ...
  if (!table->is_started())  // If first read
  {
    ...
    if (error) {
      ...
      /* CanHaveNullInNonNullable
        Currently, during read_const() or read_system(), when it finds no rows,
        sets all columns to NULL. So MySQL accepts that a non-nullable column
        returns a NULL, which result in some assertion failure for can not be
        NULL in Non-Nullable Item(e.g. Item_typecast_signed::val_int,
        Item_typecast_unsigned::val_int, etc.).

        That is very necessary if the said table is to the right of a left
        join(I.e., TABLE::is_nullable() is true), otherwise, read_const() or
        read_system() could instead set the columns to their default
        values(I.e., TABLE::is_nullable() is false).

        Even though this solution sounds very logical, it is touching a very old
        piece of code; it is changing logic which has been here for at least 20
        years, so, for this reason, there could be bad side-effects.

        We need to double check all calls to TABLE::has_null_row(); because,
        with this patch, sometimes these calls, which used to return TRUE, will
        start returning FALSE. So these calls need to be analyzed.

        Since above solution is dangerous, we choose another solution which is
        safest and lowest-impact, remove the unreasonable assertions in
        items(e.g. item_func.cc, item_geofunc,cc, item_json_func,cc, etc.), and
        add corresponding mtr tests to safeguard this scenario.
      */
      table->set_null_row();
...
int read_const(TABLE *table, Index_lookup *ref) {
  ...
  if (!table->is_started())  // If first read
  {
    ...
    if (error) {
      ...
      table->set_no_row();
      // grep for CanHaveNullInNonNullable
      table->set_null_row();
      empty_record(table);
      return -1;
    }
...
AccessPath *QEP_TAB::access_path() {
  ...
  switch (type()) {
    ...
    case JT_INDEX_MERGE:
      if (using_dynamic_range) {
        ...
        path = create_table_access_path(
            join()->thd, table(), range_scan(), table_ref, position(),
            /*count_examined_rows=*/true, &pq_replace_iter);
      }
      break;
    default:
      assert(false);
  ...
  if (join()->thd->is_pq_worker() && pq_replace_iter &&
      (pq_div_tab || pq_cut_tab)) {
    // Evaluates whether we are doing a constant look up or not:
    // if yes: we can do a block scan
    // otherwise we should do a ref scan
    // Reason: create_ref_for_key decides whether to store the key value in the
    // ref().key_buffer (constant look up), if it does so, we can use a block
    // scan, if not (non constant look up) we need to evaluate it during
    // execution and only the ref scan is able to do that. When it decides not
    // to store the key value in the key_buffer,  it populates ref().key_copy
    // for retrieval during execution, this is what we assess here.
    bool non_constant_look_up = false;
    if (type() == JT_REF) {
      for (uint part_no = 0; part_no < used_ref->key_parts; part_no++) {
        if (used_ref->key_copy[part_no]) {
  ...
  if (filesort) {
    ...
    if (condition()) {
      ...
      path = PossiblyAttachFilter(path, predicates_below_join, join()->thd,
                                  &conditions_depend_on_outer_tables);
      if (path->type == AccessPath::FILTER) path->filter().qep_tab = this;
...
bool change_to_use_tmp_fields(mem_root_deque<Item *> *fields, THD *thd,
  ...
  for (size_t i = 0; it != fields->end(); ++i, ++it) {
    ...
    if (item->has_aggregation() && item->type() != Item::SUM_FUNC_ITEM)
      ...
      } else if (item->type() == Item::DEFAULT_VALUE_ITEM) {
        new_item = down_cast<Item_default_value *>(item);
      } else {
...
/**
  Perform aggregates inside storage engine.

  @param  ctx  input, Aggregate_context

  @retval
    true   ok and should continue in storage engine
    false   error or should exit storage engine
*/
bool aggregate_in_engine(Aggregate_context *ctx) {
  if (ctx->m_aggr_iter) {
    return ctx->m_aggr_iter->Aggregate(ctx->m_join);
  }
  return false;
}

/**
  Unqualified count inside storage engine, skip decoding

  @param  ctx  input, Aggregate_context

  @retval
    true   can skip decoding
    false  can not skip decoding
*/
bool unqualified_count_in_engine(Aggregate_context *ctx) {
  return ctx->m_count_iter != nullptr &&
         ctx->m_count_iter->join() == ctx->m_join;
}
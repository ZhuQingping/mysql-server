...
bool trx_coordinator::commit_in_engines(THD *thd, bool all,
  ...
  if (likely(rds_write_binlog_into_redo))
...
int TC_LOG_DSTORE::open(const char *) {
  // Always try to do recovery.
  return recover();
}

int TC_LOG_DSTORE::recover() {
  bool err = ha_recover(&g_restore_xid_map);
  if (err) {
    LogErr(ERROR_LEVEL, ER_TC_RECOVERY_FAILED_THESE_ARE_YOUR_OPTIONS);
    return 1;
  }
...
TC_LOG::enum_result TC_LOG_DSTORE::commit(THD *thd, bool all) {
  // We have already reordered handlers in Ha_trx_info::register_ha() to make
  // the dstore handler is at the front of the list. So we can ensure that
  // dstore can commit firstly.
  if (trx_coordinator::commit_in_engines(thd, all)) return RESULT_INCONSISTENT;
  return RESULT_SUCCESS;
}

int TC_LOG_DSTORE::rollback(THD *thd, bool all) {
  return trx_coordinator::rollback_in_engines(thd, all);
}

int TC_LOG_DSTORE::prepare(THD *thd, bool all) {
  assert(thd->get_transaction()->rw_ha_count(all ? Transaction_ctx::SESSION
                                                 : Transaction_ctx::STMT) > 1);
...
void TC_LOG_DSTORE::close() {}

TC_LOG *tc_log;
TC_LOG_DUMMY tc_log_dummy;
TC_LOG_MMAP tc_log_mmap;
TC_LOG_DSTORE tc_log_dstore;

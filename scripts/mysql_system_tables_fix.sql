...
# If the column does not exist, add a heap_table_lob_pages column to store the number of pages for LOB.
SET @have_dstore_table_stats =
  (SELECT COUNT(table_name) FROM information_schema.tables
     WHERE table_schema = 'mysql' AND table_name = 'dstore_table_stats' AND
           table_type = 'BASE TABLE');
SET @have_heap_table_lob_pages =
  (SELECT COUNT(column_name) FROM information_schema.columns
     WHERE table_schema = 'mysql' AND
           table_name = 'dstore_table_stats' AND
           column_name = 'heap_table_lob_pages');
SET @cmd="ALTER TABLE mysql.dstore_table_stats ADD COLUMN heap_table_lob_pages BIGINT UNSIGNED NOT NULL DEFAULT 0";
SET @str = IF(@have_heap_table_lob_pages = 0 AND @have_dstore_table_stats,
              @cmd, "SET @dummy = 0");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
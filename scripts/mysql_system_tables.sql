...
SET @cmd = "CREATE TABLE IF NOT EXISTS rds_comment 
(
...
) ENGINE=InnoDB CHARACTER SET=utf8 COLLATE=utf8_bin STATS_PERSISTENT=0";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
...
)engine=InnoDB STATS_PERSISTENT=0 CHARACTER SET utf8 COLLATE utf8_bin comment='Statement outline' ROW_FORMAT=DYNAMIC";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;
...
SET @cmd= "CREATE TABLE IF NOT EXISTS ddl_info_replay_trace (
    lsn BIGINT NOT NULL,
    ddl_info TEXT,
    PRIMARY KEY(lsn)) ENGINE=INNODB STATS_PERSISTENT=0 CHARACTER SET utf8mb3 COLLATE utf8mb3_bin comment='log ddl_info that is replayed' ROW_FORMAT=DYNAMIC TABLESPACE=mysql";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;

SET @cmd = "CREATE TABLE IF NOT EXISTS dstore_table_stats (
  database_name varchar(64) NOT NULL, 
  table_name varchar(199) NOT NULL, 
  last_update timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  n_rows bigint unsigned NOT NULL,
  heap_table_pages bigint unsigned NOT NULL,
  sum_of_index_pages bigint unsigned NOT NULL,
  heap_table_free_pages bigint unsigned NOT NULL,
  heap_table_lob_pages bigint unsigned NOT NULL,
  PRIMARY KEY(database_name, table_name)
  ) engine=InnoDB STATS_PERSISTENT=0 CHARACTER SET utf8mb3 COLLATE utf8mb3_bin comment='dstore table stats' ROW_FORMAT=DYNAMIC TABLESPACE=mysql";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;

SET @cmd = "CREATE TABLE IF NOT EXISTS dstore_index_stats (
  database_name varchar(64) NOT NULL,
  table_name varchar(199) NOT NULL, 
  ...
  ) engine=InnoDB STATS_PERSISTENT=0 CHARACTER SET utf8mb3 COLLATE utf8mb3_bin comment='dstore index stats' ROW_FORMAT=DYNAMIC TABLESPACE=mysql";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;

SET @cmd = "CREATE TABLE IF NOT EXISTS rds_sql_filter_rules (
 item_id bigint AUTO_INCREMENT NOT NULL,
 type enum('SELECT', 'UPDATE', 'DELETE', 'INSERT', 'REPLACE') COLLATE utf8mb3_general_ci NOT NULL,
 max_concurrency bigint NOT NULL DEFAULT 0,
 key_str text COLLATE utf8mb3_general_ci NOT NULL,
 node_id varchar(64),
 PRIMARY KEY (item_id)
)engine=InnoDB STATS_PERSISTENT=0 CHARACTER SET utf8 COLLATE utf8_bin comment='sql filter info' ROW_FORMAT=DYNAMIC";
SET @str = CONCAT(@cmd, " ENCRYPTION='", @is_mysql_encrypted, "'");
PREPARE stmt FROM @str;
EXECUTE stmt;
DROP PREPARE stmt;

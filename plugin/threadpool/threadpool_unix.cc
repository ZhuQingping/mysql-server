...
// copy from percona 8.0 includ/my_sys.h
#define my_microsecond_getsystime() (my_getsystime() / 10)

inline unsigned long long int my_cputime_clock() {
#ifdef HAVE_CLOCK_GETTIME
  struct timespec tp;
  clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &tp);
  return (static_cast<unsigned long long int>(tp.tv_sec) * 1000000 +
          static_cast<unsigned long long int>(tp.tv_nsec) / 1000);
#else
  return clock();
...
/* previous value is 1024, ev will exceeds the max single use of 10000 bytes of
stack use change it from 1024 to 800 to follow coverity check rule */
...
alignas(CPU_LEVEL1_DCACHE_LINESIZE) TP_atomic_counter tp_waits[THD_WAIT_LAST];

/** Indicates that threadpool was initialized*/
static bool g_threadpool_started = false;
...
thread_group_t g_all_groups[MAX_THREAD_GROUPS];
static uint g_group_count;

/**
 Used for printing "pool blocked" message, see
 print_pool_blocked_message();
*/
static ulonglong g_pool_block_start;
...
struct pool_timer_t {
  ...
  int tick_interval;
  bool shutdown;
  /* true if turn off the threadpool online,
   * then timer will sleep after all connections in pool are closed */
  bool sleep;
};

static pool_timer_t g_pool_timer;
...
@param[in]  connection  pointer to the connection object
*/
static void queue_put_connection(thread_group_t *thread_group,
                                 connection_t *connection);

/**put events to queue.

@param[in]  thread_group  current thread group
@param[in]  ev  pointer to the events object
@param[in]  cnt the event count
*/
static void queue_put_events(thread_group_t *thread_group, native_event *ev,
                             int cnt);

/**Wake sleeping thread from idle list.

@param[in]  thread_group  current thread group
@param[in]  due_to_stall  flag for stats stall information

@return  0 if success
         1  no thread in waiter list
*/

static int wake_thread(thread_group_t *thread_group,
                       bool due_to_stall) noexcept;
/**hanle the events from this connection.
...
/**Wake sleeping thread from idle list.
create one if the idle list is empty.

@param[in]  thread_group  current thread group
@param[in]  reason  why need to wake or create new thread

@return  0 if success, non-zero otherwise
*/

static int wake_or_create_thread(thread_group_t *thread_group,
                                 worker_creation_reason_t reason);

/**Entry for worker thread

@param[in]  param  user passed parameter
*/
...
@param[in]  thread_group  current thread group
@param[in]  reason  why need to create a new thread

@return  0 if success, non-zero otherwise
*/
static int create_worker(thread_group_t *thread_group,
                         worker_creation_reason_t reason) noexcept;

/**check the threads working status of this group.
...
@param[in]  connection  the connection will be closed
*/
static void connection_abort(connection_t *connection);

/**set the next check time.

@param[in]  abstime  the absolute wait time
*/
...
@param[in]  max_threads_reached  flag for print different msg
*/
...
#define EPOLLRDHUP 0
#endif
/**create a epoll descriptor

@return  epoll descriptor or error.
*/
static int io_poll_create() noexcept { return epoll_create(1); }

/**Add fd to the the epoll listening list

@param[in]      pollfd the epoll file descriptor
@param[in]      fd the file descriptor will be listened
@param[in]      data the user private data
...
/**Reactivate event listening on the fd
...
/**Remove fd from the epoll listening list
...
@param[in]      pollfd the epoll file descriptor
@param[out]     native_events the events data
@param[in]      maxevents upper limit of the events number
...
/**
Get user passed private data from io_poll_start_read or
io_poll_associate_fd

@param[in]      event pointer to the event

@return  user passed private data.
*/
...
@return  true if too many active threads, false otherwise
*/
...
@return  true if too many busy threads.
*/
...
@return  true if the connection is high priority, false otherwise
*/
inline bool connection_is_high_prio(const connection_t &c) noexcept {
  const ulong mode = TP_HIGH_PRIO_MODE_TRANSACTIONS;
...
@return  a connection msg, nullptr otherwise
*/
static connection_t *queue_get(thread_group_t *thread_group) noexcept {
  ...
  */
  ...
  if (c) {
    thread_group->dequeued_event_count++;
  }
  DBUG_RETURN(c);
}
...
Currently it wakes up rather often on an idle server.
*/

TP_atomic_counter threadpool_events_handled_cur;
static ulonglong stats_collection_period = 10000000;
static ulonglong timer_stopwatch, timer_cpu_watch;
ulonglong threadpool_cpu_timer;
...
static void *timer_thread(void *param) noexcept {
  ...
  for (;;) {
    ...
    /* turn off threadpool online, just sleep the timer until turn on threadpool
     * again */
    if (timer->sleep && tp_get_active_connections() == 0) {
      threadpool_running = false;
      /* there is nothing to do, sleep until turn on threadpool again */
      err = mysql_cond_wait(&timer->cond, &timer->mutex);
      threadpool_running = !timer->shutdown;
    } else {
      set_timespec_nsec(&ts, (ulonglong)timer->tick_interval * 1000000);
      /* Periodic control for check_stall  */
    ...
    if (err == ETIMEDOUT) {
      ...
      for (size_t i = 0; i < array_elements(g_all_groups); i++) {
        if (g_all_groups[i].connection_count) check_stall(&g_all_groups[i]);
      }
    ...
    auto current_microtime =
        g_pool_timer.current_microtime.load(std::memory_order_relaxed);
    ulonglong now = threadpool_exact_stats ? my_microsecond_getsystime()
                                           : current_microtime;
    if (now - timer_stopwatch > stats_collection_period) {
      timer_stopwatch = now;

      ulonglong cpu_time = my_cputime_clock();
      for (size_t i = 0; i < array_elements(g_all_groups); i++) {
        g_all_groups[i].cpu_time = cpu_time - g_all_groups[i].cpu_watch;
        g_all_groups[i].cpu_watch = cpu_time;
      }

      threadpool_cpu_timer = cpu_time - timer_cpu_watch;
      timer_cpu_watch = cpu_time;

      threadpool_events_handled_cur = 0;
    }
...
@param[in]      tg the thread_group to be checked

@return  true if the querys are empty.
*/
...
/**
Notify one worker thead in the group to exit.

@param[in]      thread_group current thread group

@return  0 if notify one idle thread to exit,
         1 if there is no idle worker thread.
*/
static int remove_worker(thread_group_t *thread_group) {
  DBUG_ENTER("remove_worker");
  worker_thread_t *thread = thread_group->idle_threads.front();
  if (thread) {
    DBUG_PRINT("info", ("A thread %p to be removed from group %p", thread,
                        thread_group));
    thread->exit.store(true, std::memory_order_relaxed);
    mysql_cond_signal(&thread->cond);
    DBUG_RETURN(0);
  } else {
    bool exit = thread_group->exit_one_worker.load(std::memory_order_relaxed);
    if (!exit) {
      thread_group->exit_one_worker.compare_exchange_strong(
          exit, true, std::memory_order_relaxed);
    }
  }
  DBUG_RETURN(1);
}
...
static void check_stall(thread_group_t *thread_group) {
  ...
  /* huaweisql: add prio_kickup_timer check in the below for loop */
  /* we boost connections from low priority queue to high priority queue if it
   * waits too long */
  while (auto c = thread_group->queue.front()) {
    auto current_microtime =
        g_pool_timer.current_microtime.load(std::memory_order_relaxed);
    if (current_microtime - c->enqueue_time <=
        threadpool_prio_kickup_timer * 1000ULL)
      break;

    TP_INCREMENT_GROUP_COUNTER(thread_group, kickups);
    ulonglong now = threadpool_exact_stats ? my_microsecond_getsystime()
                                           : current_microtime;
    c->enqueue_htime = now;
    c->kickup = 1;
  ...
  if (threadpool_enable_active_threads_check &&
      thread_group->active_thread_count > (int)threadpool_oversubscribe + 1) {
    TP_INCREMENT_GROUP_COUNTER(thread_group, thread_removes);
    remove_worker(thread_group);
  }

  /*
    Check if there is a valid listener and how many IO events were read.
    If there is no listener and there were no IO events it means we
    stuck somewhere, for example, on some waiting timeout callback. The
    solution is to create new thread, which likely will become a
    listener and read events from connections. If not, it will dequeue
    one of threadpool queues and process will continue by one way or
    another.
  */
  if (!thread_group->listener && !thread_group->io_event_count) {
    wake_or_create_thread(thread_group, WORKER_CREATION_REASON_STALL);
  ...
  /*
    ...
    1. There is a counter thread_group->dequeued_event_count for the number of
    ...
    will be faster.
  */
  if (!thread_group->dequeued_event_count && !queues_are_empty(*thread_group)) {
    thread_group->stalled = true;
    TP_INCREMENT_GROUP_COUNTER(thread_group, stalls);
    wake_or_create_thread(thread_group, WORKER_CREATION_REASON_STALL);
  }

  /* Reset dequeued events count */
  thread_group->dequeued_event_count = 0;

  mysql_mutex_unlock(&thread_group->mutex);
}

/**start the backgroud timer.
...
static void start_timer(pool_timer_t *timer) noexcept {
  ...
  mysql_mutex_lock(&timer->mutex);
  timer->tick_interval = threadpool_stall_limit;
  ...
  error = mysql_thread_create(key_timer_thread, &thread_id, &thr_attr,
                              timer_thread, timer);
  if (error) {
    sql_print_error("Threadpool: Fail to start timer thread (errno=%d)", error);
  }
  my_thread_attr_destroy(&thr_attr);
  DBUG_VOID_RETURN;
}

/**stop the backgroud timer.

@param[in]  timer  the timer to be stoped
*/
...
/**pause the backgroud timer.
 */
void pause_pool_timer() noexcept {
  if (g_threadpool_started) {
    g_pool_timer.sleep = true;
  }
}

/**resume the backgroud timer.

@param[in]  timer  the timer to be resumed
*/
void resume_timer(pool_timer_t *timer) noexcept {
  DBUG_ENTER("resume_timer");
  mysql_mutex_lock(&timer->mutex);
  timer->sleep = false;
  mysql_cond_signal(&timer->cond);
  mysql_mutex_unlock(&timer->mutex);
  ...
  for (;;) {
    ...
    */
    ...
    const bool listener_picks_event =
        ...
        thread_group->queue.is_empty() && !threadpool_dedicated_listener;
    ...
    */
    queue_put_events(thread_group, ev, cnt);
        ...
        */
        if (thread_group->thread_count == 1) {
          /*
            ...
            pointless delay.
          */
          create_worker(thread_group, WORKER_CREATION_REASON_SINGLE_LISTENER);
        }
      }
    }
    mysql_mutex_unlock(&thread_group->mutex);
...
@param[in] count  1, when new thread is created
...
static void add_thread_count(thread_group_t *thread_group,
  ...
  g_tp_stats.num_worker_threads.fetch_add(count, std::memory_order_relaxed);
}
...
@return    0, when new thread is created
...
static int create_worker(thread_group_t *thread_group,
  ...
  assert(reason < WORKER_CREATION_REASON_ARRAY_SIZE);

  TP_INCREMENT_GROUP_COUNTER(thread_group, thread_creation_attempts);

  if (g_tp_stats.num_worker_threads >= (int)threadpool_max_threads &&
  ...
  if (threadpool_enable_active_threads_check &&
      (uint)thread_group->active_thread_count >= threadpool_oversubscribe + 1 &&
      reason != WORKER_CREATION_REASON_QUEUE_PUT_CONNECTION) {
    err = 0;
    goto end;
  }
  ...
  if (!err) {
    ...
    TP_INCREMENT_GROUP_COUNTER(thread_group,
                               thread_creations_by_reason[reason]);
  } else {
    set_my_errno(errno);
  }
  ...
  if (err)
    ...
    g_pool_block_start = 0; /* Reset pool blocked timer, if it was set */
...
@return    the throttle value, non-zero otherwise
*/
...
*/
static int wake_or_create_thread(thread_group_t *thread_group,
                                 worker_creation_reason_t r) {
  DBUG_ENTER("wake_or_create_thread");

  if (thread_group->shutdown) DBUG_RETURN(0);

  if (wake_thread(thread_group, r == WORKER_CREATION_REASON_STALL) == 0)
    DBUG_RETURN(0);
  ...
  if (thread_group->active_thread_count == 0) {
    ...
    */
    DBUG_RETURN(create_worker(thread_group, r));
  }
...
/**
Init the specified thread group

@parma[in] index current group index in the group array
@param[in] thread_group the thread group to be inited
@param[in] thread_attr the connection attribute

@return    0 if success, non-zero otherwise
*/
static int thread_group_init(int index, thread_group_t *thread_group,
  ...
  thread_group->event_count = 0;
  thread_group->wait_time = 0;
  thread_group->cpu_watch = my_cputime_clock();
  thread_group->index = index;
  DBUG_RETURN(0);
}

/**
Destroy the specified thread group

@param[in] thread_group the thread group to be destroyed
@param[in] thread_attr the connection attribute
...
static int wake_thread(thread_group_t *thread_group,
  ...
  if (thread) {
    thread->woken = true;
    thread_group->idle_threads.remove(thread);
    mysql_cond_signal(&thread->cond);
...
@param[in]  thread_group  the group to be shutdown
...
static void queue_put_connection(thread_group_t *thread_group,
                                 connection_t *connection) {
  DBUG_ENTER("queue_put_connection");

  mysql_mutex_lock(&thread_group->mutex);
  connection->tickets = threadpool_high_prio_tickets;
  // huaweisql: add enqueue time of this connection to common queue
  connection->enqueue_time = my_microsecond_getsystime();
...
@param[in] current_thread  current worker thread
@param[in] thread_group  current thread group
@param[in] abstime  absolute wait timeout

@return  connection with pending event.
...
static connection_t *get_event(worker_thread_t *current_thread,
  ...
  for (;;) {
    ...
    */
    ...
    /* In case the listener is NULL we don't need to go to sleep. We better
       make further attempt to pick event or become listener */
    if (!thread_group->listener) continue;

    /* And now, finally sleep */
    current_thread->woken = false; /* wake() sets this to true */

    /*
      Add current thread to the head of the idle threads list and wait.
    ...
    */
    thread_group->idle_threads.push_front(current_thread);
    ...
    if (!current_thread->woken) {
      ...
      */
      thread_group->idle_threads.remove(current_thread);
    }
...
@param[in] wait_type  wait on one of thd_wait_type
@param[in] thread_group  current thread group
*/
static inline void wait_begin(thd_wait_type wait_type,
                              thread_group_t *thread_group) {
  ulonglong current_microtime =
      g_pool_timer.current_microtime.load(std::memory_order_relaxed);
  thread_group->wait_watch =
      threadpool_exact_stats ? my_microsecond_getsystime() : current_microtime;
  DBUG_ENTER("wait_begin");
  mysql_mutex_lock(&thread_group->mutex);
  thread_group->active_thread_count--;
  if (THD_WAIT_BL_DUMPER != wait_type) {
  ...
  if (!queues_are_empty(*thread_group) ||
      (!thread_group->listener && thread_group->connection_count > 1)) {
    /*
      Group might stall while this thread waits, thus wake
      or create a worker to prevent stall.
    */
    wake_or_create_thread(thread_group, WORKER_CREATION_REASON_WAIT_BEGIN);
  }
#endif
...
@param[in] thread_group  current thread group
*/
static inline void wait_end(thd_wait_type wait_type,
  ...
  ulonglong now =
      threadpool_exact_stats ? my_microsecond_getsystime() : current_microtime;
  thread_group->wait_time += now - thread_group->wait_watch;
  DBUG_VOID_RETURN;
}
...
@return the connection on success,  nullptr otherwise
*/
static connection_t *alloc_connection(THD *thd) noexcept {
  ...
  if (connection) {
    ...
    connection->conn_flags = 0;
    connection->wait_type = THD_WAIT_NONE;
    connection->num_nested_waits = 0;
...
@param[in] channel_info  current channel
...
bool add_connection(Channel_info *channel_info) {
  ...
  thd->conn_event_functions = &thd_event_functions;
  ...
  thread_group_t *group = &g_all_groups[thd->thread_id() % g_group_count];
...
static void connection_abort(connection_t *connection) {
  ...
  connection->thd = NULL; /* after remove, thd is NULL */
...
void tp_wait_begin(THD *thd, int type MY_ATTRIBUTE((unused))) {
  ...
  assert(type > THD_WAIT_NONE && type < THD_WAIT_LAST);
  thd_wait_type wait_type = (thd_wait_type)type;
  tp_waits[type].inc();
  if (!thd) {
    DBUG_VOID_RETURN;
  }
  assert(thd);
  DBUG_EXECUTE_IF("simulate_tp_active_threads_limit", DBUG_VOID_RETURN;);
  connection_t *connection = (connection_t *)thd->scheduler.data;
  if (connection) {
    // We are not supposed to have nested wait begin/end. Most of wait
    // begin/end is wrapped around a short code path, so it is easy
    // to ensure that there is no nested begin/end. However,
    // THD_WAIT_BL_DUMPER is wrapped around a long code path.
    // We tolerate nested begin/end in customer build, just in case.
    // In debug build, we assert.
    assert(THD_WAIT_NONE == connection->wait_type ||
           THD_WAIT_BL_DUMPER == connection->wait_type);
    if (THD_WAIT_NONE == connection->wait_type) {
      connection->wait_type = wait_type;
      wait_begin(wait_type, connection->thread_group);
    } else {
      connection->num_nested_waits++;
    }
  }
  DBUG_VOID_RETURN;
}
...
void tp_wait_end(THD *thd) {
  ...
  if (connection) {
    assert(THD_WAIT_NONE != connection->wait_type &&
           0 == connection->num_nested_waits);

    // tolerate nested begin/end. see comments in tp_wait_begin()
    if (0 == connection->num_nested_waits) {
      wait_end(connection->wait_type, connection->thread_group);
      connection->wait_type = THD_WAIT_NONE;
    } else {
      connection->num_nested_waits--;
      assert(connection->num_nested_waits >= 0);
    }
  }
  DBUG_VOID_RETURN;
}
...
static void set_next_timeout_check(ulonglong abstime) {
  ...
  uint64 old = g_pool_timer.next_timeout_check.load(std::memory_order_relaxed);
  while (abstime < old) {
    old = g_pool_timer.next_timeout_check.load();
    g_pool_timer.next_timeout_check.compare_exchange_weak(
        old, abstime, std::memory_order_relaxed);
  }
  DBUG_VOID_RETURN;
}
...
static void set_wait_timeout(connection_t *c) noexcept {
  ...
  /*
    ...
    overhead, use g_pool_timer.current_microtime and take
  ...
  */
  ...
  c->abs_wait_timeout =
      g_pool_timer.current_microtime.load(std::memory_order_relaxed) +
      1000LL * g_pool_timer.tick_interval +
...
migrate to a different group because g_group_count has changed
after thread_pool_size setting.

@param[in]  c            current connection
@param[in]  old_group    the old thread group
@param[in]  new_group    the new thread group
...
static int change_group(connection_t *c, thread_group_t *old_group,
  ...
  if (c->is_bound_to_poll_descriptor()) {
    ...
    c->clear_bound_to_poll_descriptor();
  } else {
    mysql_mutex_lock(&old_group->mutex);
  }
  ...
  if (!new_group->thread_count)
    ret = create_worker(new_group, WORKER_CREATION_REASON_QUEUE_PUT_CONNECTION);
  mysql_mutex_unlock(&new_group->mutex);
  return ret;
}

/**
Reactivate the fd listening
...
static int start_io(connection_t *connection) {
  /*
    ...
    connection's life. However, we do allow g_group_count to
  ...
  */
  thread_group_t *const group =
      &g_all_groups[connection->thd->thread_id() % g_group_count];
  ...
  */
  ...
  if (!connection->is_bound_to_poll_descriptor()) {
    connection->set_bound_to_poll_descriptor();
...
static void *worker_main(void *param) {
  ...
  this_thread.exit = false;
  this_thread.woken = true;
  ...
  for (;;) {
    ...
    threadpool_events_handled_cur.inc();
    if (this_thread.exit) break;
    bool exit = thread_group->exit_one_worker.load(std::memory_order_relaxed);
    if (exit &&
        thread_group->exit_one_worker.compare_exchange_strong(exit, false)) {
      break;
    }
  }
...
bool tp_init() {
  ...
  /** when turn on threadpool online, the threadpool already started,
  and the resources already inited,
  just wake up the timer. */
  if (g_threadpool_started) {
    resume_timer(&g_pool_timer);
    DBUG_RETURN(0);
  }
  g_threadpool_started = true;

  for (uint i = 0; i < array_elements(g_all_groups); i++) {
    thread_group_init(i, &g_all_groups[i], get_connection_attrib());
  }
  tp_set_threadpool_size(threadpool_size);
  if (g_group_count != threadpool_size) {
...
void tp_end() noexcept {
  ...
  if (!g_threadpool_started) DBUG_VOID_RETURN;
  stop_timer(&g_pool_timer);
  for (uint i = 0; i < array_elements(g_all_groups); i++) {
    thread_group_close(&g_all_groups[i]);
  }
  g_threadpool_started = false;
  threadpool_running = false;
  DBUG_VOID_RETURN;
}
...
void tp_set_threadpool_size(uint size) noexcept {
  if (!g_threadpool_started) return;
  ...
  for (i = 0; i < size; i++) {
    ...
    if (group->pollfd == -1) {
      ...
      if (!success) {
        ...
        mysql_mutex_unlock(&g_all_groups[i].mutex);
        break;
      }
    }
    mysql_mutex_unlock(&g_all_groups[i].mutex);
...
@param[in]    limit   the stall time threshold.
*/
void tp_set_threadpool_stall_limit(uint limit) noexcept {
  if (!g_threadpool_started) return;
  mysql_mutex_lock(&(g_pool_timer.mutex));
  g_pool_timer.tick_interval = limit;
  mysql_mutex_unlock(&(g_pool_timer.mutex));
  mysql_cond_signal(&(g_pool_timer.cond));
}

/**
Calculate number of idle threads in the pool.
...
int tp_get_idle_thread_count() noexcept {
  int sum = 0;
  for (uint i = 0;
       i < array_elements(g_all_groups) && (g_all_groups[i].pollfd >= 0); i++) {
    sum += g_all_groups[i].thread_count - g_all_groups[i].active_thread_count -
           g_all_groups[i].waiting_thread_count -
           (g_all_groups[i].listener ? 1 : 0);
  }
  return sum;
}

/**
Calculate number of active threads in the pool.
...
int tp_get_active_thread_count() {
  ...
  for (uint i = 0;
       i < array_elements(g_all_groups) && (g_all_groups[i].pollfd >= 0); i++) {
    sum += g_all_groups[i].active_thread_count;
  }
  return sum;
}
...
int tp_get_waiting_thread_count() {
  ...
  for (uint i = 0;
       i < array_elements(g_all_groups) && (g_all_groups[i].pollfd >= 0); i++) {
    sum += g_all_groups[i].waiting_thread_count;
  }
  return sum;
}
...
*/
int tp_get_active_connections() {
  uint sum = 0;
  for (uint i = 0; i < array_elements(g_all_groups); i++) {
    sum += g_all_groups[i].connection_count;
  }
  return sum;
}
...
static void print_pool_blocked_message(bool max_threads_reached) noexcept {
  ...
  if (g_pool_block_start == 0) {
    g_pool_block_start = now;
    msg_written = false;
  }

  if (!msg_written && ((now > g_pool_block_start + BLOCK_MSG_DELAY) ||
                       (now == g_pool_block_start))) {
    if (max_threads_reached)
      sql_print_warning(MAX_THREADS_REACHED_MSG);
    else
      sql_print_error(CREATE_THREAD_ERROR_MSG, my_errno);

    if (now > g_pool_block_start) {
      sql_print_information("Threadpool has been blocked for %u seconds\n",
                            (uint)((now - g_pool_block_start) / 1000000));
    }

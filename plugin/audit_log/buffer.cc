...
static PSI_mutex_info mutex_key_list[] = {{&key_log_mutex,
                                           "audit_log_buffer::mutex",
                                           PSI_FLAG_SINGLETON, 0, nullptr}};
...
static void audit_log_flush(audit_log_buffer_t *log) {
  ...
  DBUG_EXECUTE_IF("audit_log_rotate_pause", log->state = LOG_RECORD_COMPLETE;);
  if (log->flush_pos >= log->write_pos % log->size) {
    log->state = (log->write_pos % log->size == 0) ? LOG_RECORD_COMPLETE
                                                   : LOG_RECORD_INCOMPLETE;

    mysql_mutex_unlock(&log->mutex);
    log->write_func(log->write_func_data, log->buf + log->flush_pos,  // NOLINT
...
static void *audit_log_flush_worker(void *arg) {
  auto *log = static_cast<audit_log_buffer_t *>(arg);

  my_thread_init();
  while (!(log->stop != 0 && log->flush_pos == log->write_pos)) {
    audit_log_flush(log);
  }
  my_thread_end();
...
audit_log_buffer_t *audit_log_buffer_init(size_t size, int drop_if_full,
                                          audit_log_write_func write_func,
                                          void *data) {
  auto *log = static_cast<audit_log_buffer_t *>(
      my_malloc(key_memory_audit_log_buffer, sizeof(audit_log_buffer_t) + size,
                MY_ZEROFILL));

#ifdef HAVE_PSI_INTERFACE
  mysql_mutex_register(AUDIT_LOG_PSI_CATEGORY, mutex_key_list,  // NOLINT
                       array_elements(mutex_key_list));
  mysql_cond_register(AUDIT_LOG_PSI_CATEGORY, cond_key_list,  // NOLINT
                      array_elements(cond_key_list));
#endif /* HAVE_PSI_INTERFACE */

  if (log != nullptr) {
    log->buf = ((char *)log + sizeof(audit_log_buffer_t));  // NOLINT
...
loop:
  if (log->write_pos + len <= log->flush_pos + log->size) {
    size_t wrlen = min(len, log->size - (log->write_pos % log->size));
    memcpy(log->buf + (log->write_pos % log->size), buf, wrlen);  // NOLINT
    if (wrlen < len) {
      memcpy(log->buf, buf + wrlen, len - wrlen);  // NOLINT
    }

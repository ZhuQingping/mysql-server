...
inline int audit_handler_rotate(audit_handler_t *handler) {
  if (handler != nullptr && handler->rotate != nullptr) {
    return handler->rotate(handler);
  }
  return 0;
}
...
extern bool audit_log_thread_safe;
#endif

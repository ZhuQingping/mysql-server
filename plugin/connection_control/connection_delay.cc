...
bool Connection_delay_action::notify_event(
  ...
  if (subclass != MYSQL_AUDIT_CONNECTION_CONNECT &&
    ...
    return error;
  /*
    Under high load, agent connections may fail and trigger continuous retries.
    If connection_control introduces a delay longer than the retry interval,
    the agent will keep retrying and may exhaust all available connections.
    To avoid this, reserved users are excluded from connection delay control.
  */

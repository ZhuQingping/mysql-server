...
class Ack_receiver : public ReplSemiSyncBase {
 public:
  Ack_receiver(bool wal_replication_channel = false);
  ~Ack_receiver();
  ...
  /**
     ...
     @param[in] flags  Used to distinguish between sending wal or binlog.

     @return it return false if succeeds, otherwise true is returned.
  */
  bool add_slave(THD *thd, uint32 flags);
  ...
  */
  ...
  bool init() {
    if (m_wal_replication_channel) {
      setTraceLevel(rpl_semi_sync_source_trace_level_wal);
      if (rpl_semi_sync_source_enabled_wal) return start();
    } else {
  ...
  */
  ...
  bool m_wal_replication_channel;
...
extern Ack_receiver *wal_ack_receiver;
#endif

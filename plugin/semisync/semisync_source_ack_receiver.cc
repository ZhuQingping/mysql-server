...
extern Wal_repl_semi_sync_master *wal_repl_semisync;
...
extern PSI_stage_info stage_waiting_for_wal_semi_sync_ack_from_replica;
extern PSI_stage_info stage_waiting_for_wal_semi_sync_replica;
extern PSI_stage_info stage_reading_wal_semi_sync_ack;
extern PSI_mutex_key key_ss_mutex_Wal_Ack_receiver_mutex;
extern PSI_cond_key key_ss_cond_Wal_Ack_receiver_cond;
extern PSI_thread_key key_ss_thread_Wal_Ack_receiver_thread;
#endif
...
Ack_receiver::Ack_receiver(bool wal_replication_channel) {
  const char *kWho = "Ack_receiver::Ack_receiver";
  function_enter(kWho);

  m_status = ST_DOWN;
  m_wal_replication_channel = wal_replication_channel;
  PSI_mutex_key ack_receiver_mutex_key =
      m_wal_replication_channel ? key_ss_mutex_Wal_Ack_receiver_mutex
                                : key_ss_mutex_Ack_receiver_mutex;
  PSI_cond_key ack_receiver_cond_key = m_wal_replication_channel
                                           ? key_ss_cond_Wal_Ack_receiver_cond
                                           : key_ss_cond_Ack_receiver_cond;
  mysql_mutex_init(ack_receiver_mutex_key, &m_mutex, MY_MUTEX_INIT_FAST);
  mysql_cond_init(ack_receiver_cond_key, &m_cond);
...
bool Ack_receiver::start() {
  ...
  if (m_status == ST_DOWN) {
    ...
    m_status = ST_UP;
    PSI_thread_key psi_thread_key = m_wal_replication_channel
                                        ? key_ss_thread_Wal_Ack_receiver_thread
...
#endif
        mysql_thread_create(psi_thread_key, &m_pid, &attr, ack_receive_handler,
                            this)) {
...
bool Ack_receiver::add_slave(THD *thd, uint32 flags) {
  Slave slave;
...
inline void Ack_receiver::wait_for_slave_connection() {
  PSI_stage_info psi_stage_info = m_wal_replication_channel
                                      ? stage_waiting_for_wal_semi_sync_replica
                                      : stage_waiting_for_semi_sync_replica;
  set_stage_info(psi_stage_info);
  mysql_cond_wait(&m_cond, &m_mutex);
}
...
void Ack_receiver::run() {
  ...
  while (true) {
    ...
    psi_stage_info = m_wal_replication_channel
    ...
    psi_stage_info = m_wal_replication_channel ? stage_reading_wal_semi_sync_ack
        ...
        */
        ...
        do {
          ...
          if (likely(len != packet_error)) {
            if (slave_obj.flags & RPL_FLAGS_REPLICA_WAL) {
              wal_repl_semisync->report_reply_wal_packet(slave_obj.server_id,
                                                         net.read_pos, len);
            } else {

...
Wal_repl_semi_sync_slave *wal_repl_semisync = nullptr;
...
bool semi_sync_need_reply_wal = false;
...
static void fix_rpl_semi_sync_replica_enabled_wal(MYSQL_THD, SYS_VAR *,
                                                  void *ptr, const void *val) {
  *static_cast<char *>(ptr) = *static_cast<const char *>(val);
  wal_repl_semisync->set_slave_enabled(rpl_semi_sync_replica_enabled_wal != 0);
}

static void fix_rpl_semi_sync_trace_level_wal(MYSQL_THD, SYS_VAR *, void *ptr,
                                              const void *val) {
  *static_cast<unsigned long *>(ptr) = *static_cast<const unsigned long *>(val);
  wal_repl_semisync->set_trace_level(rpl_semi_sync_replica_trace_level_wal);
}
...
static MYSQL_SYSVAR_BOOL(
    enabled_wal, rpl_semi_sync_replica_enabled_wal, PLUGIN_VAR_OPCMDARG,
    "Enable wal semi-synchronous replication on this replica (disabled by "
    "default). ",
    nullptr,                                 // check
    &fix_rpl_semi_sync_replica_enabled_wal,  // update
    0);
...
static MYSQL_SYSVAR_ULONG(trace_level_wal,
                          rpl_semi_sync_replica_trace_level_wal,
                          PLUGIN_VAR_OPCMDARG,
                          "The tracing level for wal semi-sync replication.",
                          nullptr,                             // check
                          &fix_rpl_semi_sync_trace_level_wal,  // update
                          32, 0, ~0UL, 1);
...
static SHOW_VAR semi_sync_slave_status_vars[] = {
    ...
    {STATUS_VAR_PREFIX "status_wal", (char *)&rpl_semi_sync_replica_status_wal,
     SHOW_BOOL, SHOW_SCOPE_GLOBAL},
...
static int repl_semi_slave_wal_io_start(Wallog_IO_param *param) {
  return wal_repl_semisync->slave_wal_start(param);
}

static int repl_semi_slave_wal_io_end(Wallog_IO_param *param) {
  return wal_repl_semisync->slave_wal_stop(param);
}

static int repl_semi_slave_request_wal_dump(Wallog_IO_param *param, uint32) {
  MYSQL *mysql = param->mysql;

  if (!wal_repl_semisync->get_slave_enabled()) return 0;
  ...
  if (source_state == 0) {
    ...
    if (source_state == 0) {
      /* Source does not support semi-sync */
      LogPluginErr(WARNING_LEVEL, ER_WAL_RPL_NOT_SUPPORTED_BY_SOURCE);
      rpl_semi_sync_replica_status_wal = 0;
      return 0;
    }
  }
  if (source_state == -1) return 1;
  ...
  */
  const char *query =
      "SET @rpl_semi_sync_replica_wal = 1, @rpl_semi_sync_slave_wal = 1";
  if (mysql_real_query(mysql, query, static_cast<ulong>(strlen(query)))) {
    LogPluginErr(ERROR_LEVEL, ER_WAL_RPL_REPLICA_SET_FAILED);
    return 1;
  }
  mysql_free_result(mysql_store_result(mysql));
...
static int repl_semi_slave_read_wal(Wallog_IO_param *, const char *packet,
                                    unsigned long len, const char **wal_buf,
                                    unsigned long *wal_len) {
  if (rpl_semi_sync_replica_status_wal)
    return wal_repl_semisync->slave_read_sync_header(
        packet, len, &semi_sync_need_reply_wal, wal_buf, wal_len);
  *wal_buf = packet;
  *wal_len = len;
  return 0;
}

static int repl_semi_slave_write_wal(Wallog_IO_param *param, const char *,
                                     unsigned long, uint32) {
  if (rpl_semi_sync_replica_status_wal && semi_sync_need_reply_wal) {
    /*
      We deliberately ignore the error in slave_wal_reply, such error
      should not cause the slave IO thread to stop, and the error
      messages are already reported.
    */
    (void)wal_repl_semisync->slave_wal_reply(
        param->mysql, param->server_id, param->received_lsn, param->flushed_lsn,
        param->replayed_lsn);
  }
...
static int repl_semi_reset_wal_slave(Wallog_IO_param *) {
  // TODO: reset semi-sync slave status here
  return 0;
}

static int repl_semi_apply_wal_slave(Wallog_IO_param *, Trans_param *, int &) {
  // TODO: implement
  return 0;
}

Wallog_IO_observer wal_io_observer = {
    sizeof(Wallog_IO_observer),  // len

    repl_semi_slave_wal_io_start,      // start
    repl_semi_slave_wal_io_end,        // stop
    repl_semi_slave_request_wal_dump,  // request_wal_transmit
    repl_semi_slave_read_wal,          // after_read_wal
    repl_semi_slave_write_wal,         // after_write_wal
    repl_semi_reset_wal_slave,         // reset
    repl_semi_apply_wal_slave          // apply
};
...
static int semi_sync_replica_plugin_check_uninstall(void *) {
  int ret = (rpl_semi_sync_replica_status || rpl_semi_sync_replica_status_wal)
                ? 1
                : 0;
  if (ret) {
    my_error(
...
static int semi_sync_slave_plugin_deinit(void *p) {
  ...
  if (unregister_wallog_io_observer(&wal_io_observer, p)) return 1;

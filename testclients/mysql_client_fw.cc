...
static int opt_skip_long_blob_tests = 0;
static int opt_skip_result_clean_for_bulk_test = 0;
...
static struct my_option client_test_long_options[] = {
    ...
    {"default_auth", 0, "Default authentication client-side plugin to use.",
     ...
     0, nullptr, 0, nullptr},
    {"skip_long_blob_tests", 'B',
     // Must be used when the default storage engine is DStore, which does not
     // support MEDIUMTEXT, LONGBLOB...
     "Skips tests using long or medium blob/text columns", nullptr, nullptr,
     nullptr, GET_NO_ARG, NO_ARG, 0, 0, 0, nullptr, 0, nullptr},
    {"skip_result_clean_for_bulk_test", 'C',
     "Skips result cleaning phase for stmt_bulk_test*", nullptr, nullptr,
     nullptr, GET_NO_ARG, NO_ARG, 0, 0, 0, nullptr, 0, nullptr},
...
static bool get_one_option(int optid,
  ...
  switch (optid) {
    ...
    case 'B':
      opt_skip_long_blob_tests = 1;
      break;
    case 'C':
      opt_skip_result_clean_for_bulk_test = 1;
      break;
    case '?':
    case 'I': /* Info */
      usage();
...
/* Macros and functions for COM_STMT_BULK_EXECUTE test. */

#define BT_ENABLE_BULK                                                     \
  {                                                                        \
    int rq =                                                               \
        mysql_query(mysql, "set global rds_support_bulk_execution = ON;"); \
    myquery2(mysql, rq);                                                   \
  }

#define BT_DISABLE_BULK                                                     \
  {                                                                         \
    int rq =                                                                \
        mysql_query(mysql, "set global rds_support_bulk_execution = OFF;"); \
    myquery2(mysql, rq);                                                    \
  }

#define BT_INIT_CLIENT(cname)                                           \
  int rc = 0;                                                           \
  MYSQL *cname = mysql_client_init(nullptr);                            \
  DIE_UNLESS(cname);                                                    \
  cname = mysql_real_connect(cname, opt_host, opt_user, opt_password,   \
                             current_db, opt_port, opt_unix_socket, 0); \
  DIE_UNLESS(cname);

#define BT_INIT_CLIENT2(cname, cuser, cpwd, cdb)                          \
  int rc = 0;                                                             \
  MYSQL *cname = mysql_client_init(nullptr);                              \
  DIE_UNLESS(cname);                                                      \
  cname = mysql_real_connect(cname, opt_host, cuser, cpwd, cdb, opt_port, \
                             opt_unix_socket, 0);                         \
  DIE_UNLESS(cname);

#define BT_CREATE_DEF_TABLES(cname)                                        \
  rc = mysql_query(cname, "drop database if exists bulk_test;");           \
  myquery2(cname, rc);                                                     \
  rc = mysql_query(cname, "create database bulk_test;");                   \
  myquery2(cname, rc);                                                     \
  rc = mysql_query(cname, "use bulk_test;");                               \
  myquery2(cname, rc);                                                     \
  rc = mysql_query(cname,                                                  \
                   "create table tbl (a int, b varchar(20) primary key)"); \
  myquery2(cname, rc);

#define BT_CLOSE_CLIENT(cname)                           \
  if (!opt_skip_result_clean_for_bulk_test) {            \
    rc = mysql_query(cname, "drop database bulk_test;"); \
    myquery2(cname, rc);                                 \
  }                                                      \
  mysql_close(cname);

#define BT_INIT_STMT(cname, sname, bname, bnum) \
  MYSQL_STMT *sname = mysql_stmt_init(cname);   \
  MYSQL_BIND bind[bnum];                        \
  memset(bind, 0, sizeof(bind));

#define BT_CLEAR_STMT(sname)     \
  mysql_stmt_free_result(sname); \
  mysql_stmt_close(sname);

#define BT_CHECK_STMT_RC                                      \
  if (rc != 0) {                                              \
...
#define BT_DOUBLE_EQUAL(v1, v2) (((v1) - (v2) < 1e-6) && ((v2) - (v1) < 1e-6))

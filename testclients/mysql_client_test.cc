...
static void test_long_data() {
  if (opt_skip_long_blob_tests) return;
  MYSQL_STMT *stmt;
  int rc, int_data;
...
static void stmt_bulk_test_basic_dml() {
  myheader("stmt_bulk_test_basic_dml");
  BT_ENABLE_BULK;
  ...
  // Insert
  {
    ...
    affected_rows = mysql_affected_rows(bmysql);
    DIE_UNLESS(affected_rows == 5);

    // Check result
    rc = mysql_query(bmysql, "SELECT a, b FROM tbl ORDER BY b;");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    ...
    for (int i = 0; i < 5; ++i) {
      row = mysql_fetch_row(result);
      DIE_UNLESS(row);
      DIE_UNLESS(std::stoi(row[0]) == ids[i]);
      DIE_UNLESS(strncmp(row[1], names[i], name_lengths[i]) == 0);
    }
  ...
  // Update
  {
    ...
    MYSQL_RES *result;
    MYSQL_ROW row;
    const char *UPDATE_QUERY = "UPDATE tbl SET a = ? WHERE b = ?;";
    ...
    int ids[5] = {11, 12, 13, 14, 15};
    // Only affect 4 rows
    const char *names[5] = {"Apple", "Bob", "Charlie", "David", "Eve"};
    unsigned long name_lengths[5];
    for (int i = 0; i < 5; ++i) {
      name_lengths[i] = strlen(names[i]);
    }
    ...
    rc = mysql_stmt_execute(stmt);
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(strstr(mysql_stmt_error(stmt),
                      "This bulk execution requires to not be in AUTOCOMMIT"
                      " mode") != nullptr);

    // switch autocommit off and then it should work:
    rc = mysql_autocommit(bmysql, false);
    myquery(rc);
    ...
    affected_rows = mysql_affected_rows(bmysql);
    DIE_UNLESS(affected_rows == 4);

    // According to MySQL's documentation, ROW_COUNT's result should be
    // similar to that of mysql_affected_rows().
    rc = mysql_query(bmysql, "SELECT ROW_COUNT()");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    ...
    row = mysql_fetch_row(result);
    DIE_UNLESS(row);
    DIE_UNLESS(std::stoi(row[0]) == 1);
    DIE_UNLESS(strncmp(row[1], "Alice", sizeof("Alice")) == 0);
  ...
  // Replace
  {
    ...
    MYSQL_RES *result;
    MYSQL_ROW row;
    const char *REPLACE_QUERY = "REPLACE INTO tbl (a, b) VALUES (?, ?);";
    rc = mysql_stmt_prepare(stmt, REPLACE_QUERY, strlen(REPLACE_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    int ids[3] = {22, 23, 26};
    // Update 2 existing rows and insert 1 new rows
    const char *names[3] = {"Bob", "Charlie", "Frank"};
    ...
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 6);

    const int res_ids[6] = {1, 22, 23, 14, 15, 26};
    const char *res_names[6] = {"Alice", "Bob", "Charlie",
                                "David", "Eve", "Frank"};

    for (int i = 0; i < 6; ++i) {
      row = mysql_fetch_row(result);
      DIE_UNLESS(row);
      DIE_UNLESS(std::stoi(row[0]) == res_ids[i]);
      DIE_UNLESS(strncmp(row[1], res_names[i], strlen(res_names[i])) == 0);
    }
  ...
  // Delete
  {
    BT_INIT_STMT(bmysql, stmt, bind, 1);
    unsigned int array_size = 5;
    uint64_t affected_rows = 0;
    MYSQL_RES *result;
    MYSQL_ROW row;
    const char *DELETE_QUERY = "DELETE FROM tbl WHERE a = ?";
    rc = mysql_stmt_prepare(stmt, DELETE_QUERY, strlen(DELETE_QUERY));
    DIE_UNLESS(rc == 0);

    rc = mysql_stmt_attr_set(stmt, STMT_ATTR_ARRAY_SIZE, &array_size);
    DIE_UNLESS(rc == 0);

    int ids[5] = {1, 22, 23, 14, 15};
    ...
    row = mysql_fetch_row(result);
    DIE_UNLESS(row);
    DIE_UNLESS(std::stoi(row[0]) == 26);
    DIE_UNLESS(strncmp(row[1], "Frank", sizeof("Frank")) == 0);

    mysql_free_result(result);
    BT_CLEAR_STMT(stmt);
  }

  DIE_UNLESS(mysql_commit(bmysql) == 0);
...
static void stmt_bulk_test_multi_update() {
  ...
  rc = mysql_query(bmysql,
                   "CREATE TABLE tbl2 (x INT, y VARCHAR(20) PRIMARY KEY)");
  DIE_UNLESS(rc == 0);

  // Prepare data
  DIE_UNLESS(mysql_query(bmysql,
                         "INSERT INTO tbl VALUES (1, 'Alice'), (2, 'Bob'), (3, "
                         "'Charlie'), (4, 'David'), (5, 'Eve')") == 0);
  DIE_UNLESS(mysql_query(bmysql,
                         "INSERT INTO tbl2 VALUES (1, 'Alice'), (2, 'Bob'), "
                         "(3, 'Charlie'), (4, 'Dog'), (5, 'Elephant')") == 0);
  ...
  // Multi update
  {
    ...
    const char *UPDATE_QUERY =
        "UPDATE tbl JOIN tbl2 ON tbl.b = tbl2.y SET tbl.a = tbl.a + ?, tbl2.x "
        "= tbl2.x + 1 WHERE tbl.b = ?;";
    rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 5);
    int tbl_ids[5] = {1, 4, 6, 4, 5};
    const char *tbl_names[5] = {"Alice", "Bob", "Charlie", "David", "Eve"};
    for (int i = 0; i < 5; ++i) {
      row = mysql_fetch_row(result);
      DIE_UNLESS(row);
      DIE_UNLESS(std::stoi(row[0]) == tbl_ids[i]);
      DIE_UNLESS(strncmp(row[1], tbl_names[i], strlen(tbl_names[i])) == 0);
    }

    mysql_free_result(result);

    // Check result on tbl2
    rc = mysql_query(bmysql, "SELECT x, y FROM tbl2 ORDER BY y;");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 5);
    int tbl2_ids[5] = {1, 3, 4, 4, 5};
    const char *tbl2_names[5] = {"Alice", "Bob", "Charlie", "Dog", "Elephant"};
    for (int i = 0; i < 5; ++i) {
      row = mysql_fetch_row(result);
      DIE_UNLESS(row);
      DIE_UNLESS(std::stoi(row[0]) == tbl2_ids[i]);
      DIE_UNLESS(strncmp(row[1], tbl2_names[i], strlen(tbl2_names[i])) == 0);
    }
...
static void stmt_bulk_test_option() {
  myheader("stmt_bulk_test_protocol");

  // Server doesn't support bulk, but client supports.
  BT_DISABLE_BULK;
  {
    BT_INIT_CLIENT(bmysql);
    BT_CREATE_DEF_TABLES(bmysql);
    ...
    unsigned int array_size = 5;
    const char *REPLACE_QUERY = "DELETE FROM tbl WHERE a = ?";

    rc = mysql_stmt_prepare(stmt, REPLACE_QUERY, strlen(REPLACE_QUERY));
    DIE_UNLESS(rc == 0);

    // Failed to set array_size
    rc = mysql_stmt_attr_set(stmt, STMT_ATTR_ARRAY_SIZE, &array_size);
    DIE_UNLESS(rc != 0);
  ...
  // Server supports bulk and disables the option right after new connection.
  // In this case, the existing connection should keep enabling bulk.
  BT_ENABLE_BULK;
  {
    BT_INIT_CLIENT(bmysql);
    BT_CREATE_DEF_TABLES(bmysql);
    // Disable the global option right after new connection.
    BT_DISABLE_BULK;
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    unsigned int array_size = 5;
    uint64_t affected_rows = 0;
  ...
  // Server doesn't support bulk and enable the option right after new
  // connection. In this case, the existing connection should keep disabling it.
  BT_DISABLE_BULK;
  {
    BT_INIT_CLIENT(bmysql);
    BT_CREATE_DEF_TABLES(bmysql);
    // Enable the option right after new connection
    BT_ENABLE_BULK;
    BT_INIT_STMT(bmysql, stmt, bind, 1);
...
#ifdef NDEBUG
  fprintf(stdout, "\n==> Skip rest test in release mode\n");
  return;
#endif

  // Server supports bulk, but client doesn't support.
  BT_ENABLE_BULK;
  {
    DBUG_SET("+d,bulk_test_client_dont_support_bulk");
    BT_INIT_CLIENT(bmysql);
    BT_CREATE_DEF_TABLES(bmysql);
    BT_INIT_STMT(bmysql, stmt, bind, 1);
    ...
    rc = mysql_stmt_execute(stmt);
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1047);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt), "Unknown command") == 0);
...
static void stmt_bulk_test_imitate_mariadb() {
  myheader("stmt_bulk_test_imitate_mariadb");

  // Connect to server with normal mode
  {
    MYSQL *bmysql = mysql_client_init(nullptr);
    DIE_UNLESS(bmysql);
    bmysql = mysql_real_connect(bmysql, opt_host, opt_user, opt_password,
                                current_db, opt_port, opt_unix_socket, 0);
    DIE_UNLESS(bmysql);
    DIE_UNLESS(strstr(bmysql->server_version, "MariaDB") == NULL);
    DIE_UNLESS(strstr(bmysql->server_version, "-maria-") == NULL);
    DIE_UNLESS(bmysql->server_capabilities & CLIENT_LONG_PASSWORD);
    mysql_close(bmysql);
  }

  int ret = mysql_query(mysql, "SET GLOBAL rds_imitate_mariadb_server = ON;");
  DIE_UNLESS(ret == 0);
  ...
  ret = mysql_query(mysql, "SET GLOBAL rds_imitate_mariadb_server = OFF;");
  DIE_UNLESS(ret == 0);
}

static void stmt_bulk_test_data_types() {
  myheader("stmt_bulk_test_data_types");
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);

  // Dstore doesn't support medium and big text/blob
  const char *multi_types_sql =
      "CREATE TABLE multi_types_tbl"
      "("
      " c0  tinyint,"
      " c1  smallint,"
      " c2  mediumint,"
  ...
  rc = mysql_query(bmysql, multi_types_sql);
  myquery2(bmysql, rc);

  BT_INIT_STMT(bmysql, stmt, bind, 28);
  unsigned int array_size = 2;
  uint64_t affected_rows = 0;
  MYSQL_RES *result;
  MYSQL_ROW row;

  const char *multi_types_insert =
      "INSERT INTO multi_types_tbl (c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, "
      "c10, c11, c12, c13, c14, c15, c16, c17, c18, c19, c20, c21, c22, c23, "
      "c24, c25, c26, c27) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
      "?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
  rc = mysql_stmt_prepare(stmt, multi_types_insert, strlen(multi_types_insert));
  DIE_UNLESS(rc == 0);
  ...
  char c0_arr[2] = {127, 0};
  short c1_arr[2] = {32767, 0};
  int c2_arr[2] = {8388607, 0};
  int c3_arr[2] = {2147483647, 0};
  int c4_arr[2] = {2147483647, 0};
  longlong c5_arr[2] = {9223372036854775807, 0};
  float c6_arr[2] = {3.14, 0};
  double c7_arr[2] = {2.7182818, 0};
  double c8_arr[2] = {2.7182818, 0};
  float c9_arr[2] = {3.14, 0};
  const char *c10_arr[2] = {"123.4567", nullptr};
  unsigned long c10_arr_len[2] = {strlen(c10_arr[0]), 0};
  const char *c11_arr[2] = {"1234.4567", nullptr};
  unsigned long c11_arr_len[2] = {strlen(c11_arr[0]), 0};
  MYSQL_TIME c12_arr[2] = {
      {2025, 6, 18, 11, 0, 3, 200, 0, MYSQL_TIMESTAMP_DATE, 28800}, {}};
  MYSQL_TIME c13_arr[2] = {
      {2025, 6, 18, 11, 0, 4, 200, 0, MYSQL_TIMESTAMP_DATETIME, 28800}, {}};
  MYSQL_TIME c14_arr[2] = {
      {2025, 6, 18, 11, 0, 5, 200, 0, MYSQL_TIMESTAMP_DATETIME, 28800}, {}};
  MYSQL_TIME c15_arr[2] = {
      {0, 0, 0, 12, 11, 6, 200, 0, MYSQL_TIMESTAMP_TIME, 28800}, {}};
  MYSQL_TIME c16_arr[2] = {
      {2025, 6, 18, 11, 0, 7, 200, 0, MYSQL_TIMESTAMP_DATE, 28800}, {}};
  char c17_arr[2] = {1, 0};
  char c18_arr[2] = {1, 0};
  const char *c19_arr[2] = {"1", nullptr};
  unsigned long c19_arr_len[2] = {strlen(c19_arr[0]), 0};
  const char *c20_arr[2] = {"Hello!", nullptr};
  unsigned long c20_arr_len[2] = {strlen(c20_arr[0]), 0};
  const char *c21_arr[2] = {"Hello world!", nullptr};
  unsigned long c21_arr_len[2] = {strlen(c21_arr[0]), 0};
  const char *c22_arr[2] = {"tinyblob", nullptr};
  unsigned long c22_arr_len[2] = {strlen(c22_arr[0]), 0};
  const char *c23_arr[2] = {"tinytext", nullptr};
  unsigned long c23_arr_len[2] = {strlen(c23_arr[0]), 0};
  const char *c24_arr[2] = {"blob", nullptr};
  unsigned long c24_arr_len[2] = {strlen(c24_arr[0]), 0};
  const char *c25_arr[2] = {"text", nullptr};
  unsigned long c25_arr_len[2] = {strlen(c25_arr[0]), 0};
  const char *c26_arr[2] = {"two", nullptr};
  unsigned long c26_arr_len[2] = {strlen(c26_arr[0]), 0};
  const char *c27_arr[2] = {"wednesday", nullptr};
  unsigned long c27_arr_len[2] = {strlen(c27_arr[0]), 0};

  bool null_arr[2] = {false, true};

  // tinyint
  bind[0].buffer_type = MYSQL_TYPE_TINY;
  bind[0].buffer = c0_arr;
  bind[0].is_unsigned = 0;
  bind[0].is_null = null_arr;

  // smallint
  bind[1].buffer_type = MYSQL_TYPE_SHORT;
  bind[1].buffer = c1_arr;
  bind[1].is_unsigned = 0;
  bind[1].is_null = null_arr;

  // mediumint
  bind[2].buffer_type = MYSQL_TYPE_LONG;
  bind[2].buffer = c2_arr;
  bind[2].is_unsigned = 0;
  bind[2].is_null = null_arr;

  // int
  bind[3].buffer_type = MYSQL_TYPE_LONG;
  bind[3].buffer = c3_arr;
  bind[3].is_unsigned = 0;
  bind[3].is_null = null_arr;

  // integer
  bind[4].buffer_type = MYSQL_TYPE_LONG;
  bind[4].buffer = c4_arr;
  bind[4].is_unsigned = 0;
  bind[4].is_null = null_arr;

  // bigint
  bind[5].buffer_type = MYSQL_TYPE_LONGLONG;
  bind[5].buffer = c5_arr;
  bind[5].is_unsigned = 0;
  bind[5].is_null = null_arr;

  // float
  bind[6].buffer_type = MYSQL_TYPE_FLOAT;
  bind[6].buffer = c6_arr;
  bind[6].is_null = null_arr;

  // double
  bind[7].buffer_type = MYSQL_TYPE_DOUBLE;
  bind[7].buffer = c7_arr;
  bind[7].is_null = null_arr;

  // double precision
  bind[8].buffer_type = MYSQL_TYPE_DOUBLE;
  bind[8].buffer = c8_arr;
  bind[8].is_null = null_arr;

  // real
  bind[9].buffer_type = MYSQL_TYPE_FLOAT;
  bind[9].buffer = c9_arr;
  bind[9].is_null = null_arr;

  // decimal
  bind[10].buffer_type = MYSQL_TYPE_DECIMAL;
  bind[10].buffer = c10_arr;
  bind[10].length = c10_arr_len;
  bind[10].is_null = null_arr;

  // numeric
  bind[11].buffer_type = MYSQL_TYPE_DECIMAL;
  bind[11].buffer = c11_arr;
  bind[11].length = c11_arr_len;
  bind[11].is_null = null_arr;

  // date
  bind[12].buffer_type = MYSQL_TYPE_DATE;
  bind[12].buffer = c12_arr;
  bind[12].is_null = null_arr;

  // datetime
  bind[13].buffer_type = MYSQL_TYPE_DATETIME;
  bind[13].buffer = c13_arr;
  bind[13].is_null = null_arr;

  // timestamp
  bind[14].buffer_type = MYSQL_TYPE_TIMESTAMP;
  bind[14].buffer = c14_arr;
  bind[14].is_null = null_arr;

  // time
  bind[15].buffer_type = MYSQL_TYPE_TIME;
  bind[15].buffer = c15_arr;
  bind[15].is_null = null_arr;

  // year
  bind[16].buffer_type = MYSQL_TYPE_DATE;
  bind[16].buffer = c16_arr;
  bind[16].is_null = null_arr;

  // bit
  bind[17].buffer_type = MYSQL_TYPE_TINY;
  bind[17].buffer = c17_arr;
  bind[17].is_null = null_arr;

  // bool
  bind[18].buffer_type = MYSQL_TYPE_TINY;
  bind[18].buffer = c18_arr;
  bind[18].is_null = null_arr;

  // char
  bind[19].buffer_type = MYSQL_TYPE_STRING;
  bind[19].buffer = c19_arr;
  bind[19].length = c19_arr_len;
  bind[19].is_null = null_arr;

  // char(10)
  bind[20].buffer_type = MYSQL_TYPE_STRING;
  bind[20].buffer = c20_arr;
  bind[20].length = c20_arr_len;
  bind[20].is_null = null_arr;

  // varchar(30)
  bind[21].buffer_type = MYSQL_TYPE_VARCHAR;
  bind[21].buffer = c21_arr;
  bind[21].length = c21_arr_len;
  bind[21].is_null = null_arr;

  // tinyblob
  bind[22].buffer_type = MYSQL_TYPE_TINY_BLOB;
  bind[22].buffer = c22_arr;
  bind[22].length = c22_arr_len;
  bind[22].is_null = null_arr;

  // tinytext
  bind[23].buffer_type = MYSQL_TYPE_STRING;
  bind[23].buffer = c23_arr;
  bind[23].length = c23_arr_len;
  bind[23].is_null = null_arr;

  // blob
  bind[24].buffer_type = MYSQL_TYPE_BLOB;
  bind[24].buffer = c24_arr;
  bind[24].length = c24_arr_len;
  bind[24].is_null = null_arr;

  // text
  bind[25].buffer_type = MYSQL_TYPE_STRING;
  bind[25].buffer = c25_arr;
  bind[25].length = c25_arr_len;
  bind[25].is_null = null_arr;

  // enum
  bind[26].buffer_type = MYSQL_TYPE_STRING;
  bind[26].buffer = c26_arr;
  bind[26].length = c26_arr_len;
  bind[26].is_null = null_arr;

  // set
  bind[27].buffer_type = MYSQL_TYPE_STRING;
  bind[27].buffer = c27_arr;
  bind[27].length = c27_arr_len;
  bind[27].is_null = null_arr;
  ...
  affected_rows = mysql_affected_rows(bmysql);
  DIE_UNLESS(affected_rows == 2);

  // Check result
  rc = mysql_query(bmysql,
                   "SELECT c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, "
                   "c12, c13, c14, c15, c16, c17, c18, c19, c20, c21, c22, "
                   "c23, c24, c25, c26, c27 FROM multi_types_tbl ORDER BY c0;");
  DIE_UNLESS(rc == 0);
  result = mysql_store_result(bmysql);
  DIE_UNLESS(result);
  affected_rows = mysql_num_rows(result);
  ...
  DIE_UNLESS(std::stoi(row[0]) == c0_arr[0]);
  DIE_UNLESS(std::stoi(row[1]) == c1_arr[0]);
  DIE_UNLESS(std::stoi(row[2]) == c2_arr[0]);
  DIE_UNLESS(std::stoi(row[3]) == c3_arr[0]);
  DIE_UNLESS(std::stoi(row[4]) == c4_arr[0]);
  DIE_UNLESS(std::stoll(row[5]) == c5_arr[0]);

  DIE_UNLESS(BT_DOUBLE_EQUAL(std::stod(row[6]), c6_arr[0]));
  DIE_UNLESS(BT_DOUBLE_EQUAL(std::stod(row[7]), c7_arr[0]));
  DIE_UNLESS(BT_DOUBLE_EQUAL(std::stod(row[8]), c8_arr[0]));
  DIE_UNLESS(BT_DOUBLE_EQUAL(std::stod(row[9]), c9_arr[0]));

  DIE_UNLESS(strncmp(row[10], c10_arr[0], c10_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[11], c11_arr[0], c11_arr_len[0]) == 0);

  DIE_UNLESS(strncmp(row[12], "2025-06-18", 10) == 0);
  DIE_UNLESS(strncmp(row[13], "2025-06-18 11:00:04", 19) == 0);
  DIE_UNLESS(strncmp(row[14], "2025-06-18 11:00:05", 19) == 0);
  DIE_UNLESS(strncmp(row[15], "12:11:06", 8) == 0);
  DIE_UNLESS(strncmp(row[16], "2025", 4) == 0);

  DIE_UNLESS((char)*row[17] == 1);
  DIE_UNLESS(std::stoll(row[18]) == c18_arr[0]);

  DIE_UNLESS(strncmp(row[19], c19_arr[0], c19_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[20], c20_arr[0], c20_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[21], c21_arr[0], c21_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[22], c22_arr[0], c22_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[23], c23_arr[0], c23_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[24], c24_arr[0], c24_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[25], c25_arr[0], c25_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[26], c26_arr[0], c26_arr_len[0]) == 0);
  DIE_UNLESS(strncmp(row[27], c27_arr[0], c27_arr_len[0]) == 0);
...
void stmt_bulk_test_mix_bulk_non_bulk() {
  myheader("stmt_bulk_test_mix_bulk_non_bulk");
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);
  ...
  // Insert without bulk
  {
    ...
    // Disable bulk by setting array_size to 0
    rc = mysql_stmt_attr_set(stmt, STMT_ATTR_ARRAY_SIZE, &array_size);
    DIE_UNLESS(rc == 0);
  ...
  // Insert with bulk again
  {
    ...
    int ids[3] = {5, 6};
    const char *names[3] = {"Eve", "Frank"};
    unsigned long name_lengths[2];
    for (int i = 0; i < 2; ++i) {
      name_lengths[i] = strlen(names[i]);
    }
    ...
    DIE_UNLESS(affected_rows == 6);
    {
      int tmp_ids[6] = {1, 2, 3, 4, 5, 6};
      const char *tmp_names[6] = {"Alice", "Bob", "Charlie",
                                  "David", "Eve", "Frank"};
      unsigned long tmp_name_lengths[6];
      for (int i = 0; i < 6; ++i) {
        tmp_name_lengths[i] = strlen(tmp_names[i]);
      }

      for (int i = 0; i < 6; ++i) {
        row = mysql_fetch_row(result);
        DIE_UNLESS(row);
        DIE_UNLESS(std::stoi(row[0]) == tmp_ids[i]);
        DIE_UNLESS(strncmp(row[1], tmp_names[i], tmp_name_lengths[i]) == 0);
      }
    }
...
void stmt_bulk_test_permission() {
  myheader("stmt_bulk_test_permission");
  ...
  {
    int rc = 0;
    BT_CREATE_DEF_TABLES(mysql);
  }

  ret = mysql_query(mysql, "DROP USER IF EXISTS 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "drop database if exists bulk_test;");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "create database bulk_test;");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "use bulk_test;");
  myquery2(mysql, ret);
  ret = mysql_query(mysql,
                    "create table tbl (a int auto_increment, b varchar(20) "
                    "default 'noname', primary key(a))");
  myquery2(mysql, ret);

  // Create a user with no INSERT priviledge on bulk_test.tbl table.
  {
    ret = mysql_query(mysql,
                      "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
    myquery2(mysql, ret);
    ret = mysql_query(
        mysql, "GRANT ALL PRIVILEGES ON bulk_test.tbl TO 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql,
                      "REVOKE INSERT ON bulk_test.tbl FROM 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
    myquery2(mysql, ret);

    BT_INIT_CLIENT2(bmysql, "bulk_user", "123456", "bulk_test");
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    ...
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1142);

    BT_CLEAR_STMT(stmt);
    mysql_close(bmysql);

    ret = mysql_query(mysql, "DROP USER 'bulk_user'@'%';");
    myquery2(mysql, ret);
  }

  // Create a user with no INSERT(column) priviledge on bulk_test.tbl table.
  {
    ret = mysql_query(mysql,
                      "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql,
                      "GRANT INSERT(b) ON bulk_test.tbl TO 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret =
        mysql_query(mysql, "GRANT SELECT ON bulk_test.tbl TO 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
    myquery2(mysql, ret);
    ...
    // Insert the column with no priviledge
    {
      BT_INIT_STMT(bmysql, stmt, bind, 1);
      const char *INSERT_QUERY = "INSERT INTO tbl (a) VALUES (?)";
      rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
      DIE_UNLESS(rc == 1);
      // Error ER_COLUMNACCESS_DENIED_ERROR
      DIE_UNLESS(mysql_stmt_errno(stmt) == 1143);
      BT_CLEAR_STMT(stmt);
    }

    // Insert the column with priviledge
    {
      BT_INIT_STMT(bmysql, stmt, bind, 1);
      unsigned int array_size = 5;
      uint64_t affected_rows = 0;
      ...
      const char *INSERT_QUERY = "INSERT INTO tbl (b) VALUES (?)";
      rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
      DIE_UNLESS(rc == 0);
  ...
  // Create a user with no update priviledge
  {
    ret = mysql_query(mysql,
                      "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
    myquery2(mysql, ret);
    ...
    myquery2(mysql, ret);
    ret = mysql_query(mysql,
                      "REVOKE UPDATE ON bulk_test.tbl FROM 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
    myquery2(mysql, ret);
    ...
    const char *UPDATE_QUERY = "UPDATE tbl SET a = a + 1 WHERE b = ?";
    rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
    DIE_UNLESS(rc == 1);
    // Error ER_TABLEACCESS_DENIED_ERROR
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1142);
  ...
  // Create a user with no UPDATE(column) priviledge on bulk_test.tbl table.
  {
    ret = mysql_query(mysql,
                      "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql,
                      "GRANT UPDATE(a) ON bulk_test.tbl TO 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret =
        mysql_query(mysql, "GRANT SELECT ON bulk_test.tbl TO 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ...
    // Update the column with no priviledge
    {
      BT_INIT_STMT(bmysql, stmt, bind, 1);
      const char *UPDATE_QUERY =
          "UPDATE tbl SET b = CONCAT(b, '-') WHERE a = ?";
      rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
      DIE_UNLESS(rc == 1);
      // Error ER_COLUMNACCESS_DENIED_ERROR
      DIE_UNLESS(mysql_stmt_errno(stmt) == 1143);
    ...
    // Update the column with priviledge
    {
      BT_INIT_STMT(bmysql, stmt, bind, 1);
      unsigned int array_size = 5;
      uint64_t affected_rows = 0;
      ...
      const char *UPDATE_QUERY = "UPDATE tbl SET a = a + 100 WHERE b = ?";
      rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
      DIE_UNLESS(rc == 0);
  ...
  // Create a user with no delete priviledge
  {
    ret = mysql_query(mysql,
                      "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
    myquery2(mysql, ret);
    ...
    myquery2(mysql, ret);
    ret = mysql_query(mysql,
                      "REVOKE DELETE ON bulk_test.tbl FROM 'bulk_user'@'%';");
    myquery2(mysql, ret);
    ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
    myquery2(mysql, ret);
  ...
  ret = mysql_query(mysql, "DROP DATABASE bulk_test;");
  myquery2(mysql, ret);

  BT_DISABLE_BULK;
}

void stmt_bulk_test_duplicate() {
  myheader("stmt_bulk_test_duplicate");
  // This tests expects some rollback-s, so assumes the default storage engine
  // is transactional (InnoDB, DStore, ...).
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);
  rc = mysql_autocommit(bmysql, false);
  ...
  // Insert another 3 rows, the 3rd row is duplicated, make sure the first 2
  // rows are rollback-ed.
  {
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    unsigned int array_size = 3;
    uint64_t affected_rows = 0;
    ...
    int ids[3] = {6, 7, 8};
    const char *names[3] = {"Frank", "George", "Charlie"};
    unsigned long name_lengths[3];
    for (int i = 0; i < 3; ++i) {
      name_lengths[i] = strlen(names[i]);
    }
    ...
    rc = mysql_stmt_execute(stmt);
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                      "Duplicate entry 'Charlie' for key 'tbl.PRIMARY'") == 0);
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1062);

    affected_rows = mysql_affected_rows(bmysql);
    DIE_UNLESS(affected_rows == ~(uint64_t)0);
    ...
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 5);
    int tmp_ids[5] = {1, 2, 3, 4, 5};
    const char *tmp_names[5] = {"Alice", "Bob", "Charlie", "David", "Eve"};
    unsigned long tmp_name_lengths[5];
    for (int i = 0; i < 5; ++i) {
      tmp_name_lengths[i] = strlen(tmp_names[i]);
    }
    for (int i = 0; i < 5; ++i) {
  ...
  // Update 3 rows, the 3rd row is duplicated, make sure the first 2 rows
  // are rollback-ed.
  {
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    unsigned int array_size = 3;
    ...
    const char *UPDATE_QUERY = "UPDATE tbl SET b = ? WHERE a = ?";
    rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    int ids[3] = {3, 4, 5};
    const char *names[3] = {"Frank", "George", "Alice"};
    unsigned long name_lengths[3];
    for (int i = 0; i < 3; ++i) {
      name_lengths[i] = strlen(names[i]);
    }
    ...
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                      "Duplicate entry 'Alice' for key 'tbl.PRIMARY'") == 0);
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1062);
    ...
    // Check result
    rc = mysql_query(bmysql, "SELECT a, b FROM tbl ORDER BY a;");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 5);
    int tmp_ids[5] = {1, 2, 3, 4, 5};
    const char *tmp_names[5] = {"Alice", "Bob", "Frank", "George", "Eve"};
    unsigned long tmp_name_lengths[5];
    for (int i = 0; i < 5; ++i) {
      tmp_name_lengths[i] = strlen(tmp_names[i]);
    }
...
void stmt_bulk_test_last_stmt_id() {
  myheader("stmt_bulk_test_last_stmt_id");
#ifdef NDEBUG
  fprintf(stdout,
          "\n==> Skip test in release mode: stmt_bulk_test_last_stmt_id\n");
  return;
#endif
  int ret = mysql_query(mysql, "SET GLOBAL rds_imitate_mariadb_server = OFF;");
  DIE_UNLESS(ret == 0);
  ...
  DBUG_SET("+d,bulk_test_last_stmt_id");

  // bulk execution with statement id to -1 with rds_imitate_mariadb_server
  // disabled.
  {
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
    ...
    rc = mysql_stmt_execute(stmt);
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1243);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                      "Unknown prepared statement handler (4294967295) given "
                      "to mysql_stmt_precheck") == 0);
    BT_CLEAR_STMT(stmt);
  }

  // non-bulk execution with statement id -1 with rds_imitate_mariadb_server
  // disabled.
  {
    BT_INIT_STMT(bmysql, stmt, bind, 2);
    rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
  ...
  ret = mysql_query(mysql, "SET GLOBAL rds_imitate_mariadb_server = ON;");
  DIE_UNLESS(ret == 0);
...
void stmt_bulk_test_packet_error() {
  myheader("stmt_bulk_test_packet_error");
#ifdef NDEBUG
  fprintf(stdout,
          "\n==> Skip test in release mode: stmt_bulk_test_packet_error\n");
  return;
#endif
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  ...
  /*
    Invalid indicator flags
  */
  DBUG_SET("+d,bulk_test_invalid_indicator_flag");

  rc = mysql_stmt_execute(stmt);
  DIE_UNLESS(rc != 0);
  DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
  DIE_UNLESS(
      strcmp(mysql_stmt_error(stmt), "Incorrect bulk execution packet") == 0);

  DBUG_SET("-d,bulk_test_invalid_indicator_flag");

  /*
    Invalid STMT_BULK_FLAG_SEND_UNIT_RESULTS flag.
  */
  DBUG_SET("+d,bulk_test_invalid_unit_test_flag");

  rc = mysql_stmt_execute(stmt);
  DIE_UNLESS(rc != 0);
  DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
  DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                    "Flags 192 is invalid for bulk execution") == 0);

  DBUG_SET("-d,bulk_test_invalid_unit_test_flag");

  /*
    Packet error: extra bytes in packet. Since there's
    no parameter sets number in the packet, so more
    bytes and less bytes case has no difference.
  */
  DBUG_SET("+d,bulk_test_invalid_packet_data");
...
void stmt_bulk_test_variable_permission() {
  myheader("stmt_bulk_test_variable_permission");

  int ret;
  /* Create user with no priviledge. */
  ret = mysql_query(mysql, "DROP USER IF EXISTS 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret =
      mysql_query(mysql, "CREATE USER 'bulk_user'@'%' IDENTIFIED BY '123456';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);
  ...
  /* No priviledge */
  {
    ...
    BT_INIT_CLIENT2(bmysql, "bulk_user", "123456", nullptr);
    rc = mysql_query(bmysql, "set global rds_support_bulk_execution = ON;");
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(mysql_errno(bmysql) == 1227);
    rc = mysql_query(bmysql, "set session rds_support_bulk_execution = ON;");
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(mysql_errno(bmysql) == 1227);
    rc = mysql_query(bmysql, "set global rds_imitate_mariadb_server = ON;");
    DIE_UNLESS(rc != 0);
    DIE_UNLESS(mysql_errno(bmysql) == 1227);
    /* Close session. */
    mysql_close(bmysql);
  ...
  /* With SUPER_ACL priviledge. */
  ret = mysql_query(mysql, "GRANT SUPER ON *.* TO 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);

  /* Create session with new user to enable new priviledge. */
  {
    BT_INIT_CLIENT2(bmysql, "bulk_user", "123456", nullptr);
    rc = mysql_query(bmysql, "set global rds_support_bulk_execution = ON;");
    myquery2(bmysql, rc);
  ...
  ret = mysql_query(mysql, "REVOKE SUPER ON *.* FROM 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);

  /* With SYSTEM_VARIABLES_ADMIN priviledge. */
  ret = mysql_query(mysql,
                    "GRANT SYSTEM_VARIABLES_ADMIN ON *.* TO 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);
  ...
  ret = mysql_query(
      mysql, "REVOKE SYSTEM_VARIABLES_ADMIN ON *.* FROM 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);

  /* With SESSION_VARIABLES_ADMIN priviledge. */
  ret = mysql_query(mysql,
                    "GRANT SESSION_VARIABLES_ADMIN ON *.* TO 'bulk_user'@'%';");
  myquery2(mysql, ret);
  ret = mysql_query(mysql, "FLUSH PRIVILEGES;");
  myquery2(mysql, ret);
...
void stmt_bulk_test_unsupported_query() {
  myheader("stmt_bulk_test_unsupported_query");
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);
  ...
  /* No parameter dml */
  {
    MYSQL_STMT *stmt = mysql_stmt_init(bmysql);
    unsigned int array_size = 5;
    const char *UPDATE_QUERY = "UPDATE tbl SET a = a + 1;";
    rc = mysql_stmt_prepare(stmt, UPDATE_QUERY, strlen(UPDATE_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
    DIE_UNLESS(
        strcmp(mysql_stmt_error(stmt),
               "Statement with no parameter is rejected in bulk execution") ==
        0);
  ...
  /* SELECT query */
  {
    BT_INIT_STMT(bmysql, stmt, bind, 1);
    unsigned int array_size = 5;
    const char *SELECT_QUERY = "SELECT * FROM tbl WHERE a = ?";
    rc = mysql_stmt_prepare(stmt, SELECT_QUERY, strlen(SELECT_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                      "Command is not supported in bulk execution") == 0);

    BT_CLEAR_STMT(stmt);
  }

  // LONG_DATA_VALUE 1, reject by c client because bind[0].buffer==nullptr
  {
    rc = mysql_query(bmysql, "create table tbl2 (a text);");
    DIE_UNLESS(rc == 0);

    BT_INIT_STMT(bmysql, stmt, bind, 1);
    unsigned int array_size = 5;
    const char *INSERT_QUERY = "INSERT INTO tbl2 (a) VALUES (?)";
    rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    const char *chunk1 = "MySQL";
    const char *chunk2 = " support bulk execution protocol";
    rc = mysql_stmt_send_long_data(stmt, 0, chunk1, strlen(chunk1));
    DIE_UNLESS(rc == 0);
    rc = mysql_stmt_send_long_data(stmt, 0, chunk2, strlen(chunk2));
    DIE_UNLESS(rc == 0);
  ...
  // LONG_DATA_VALUE 2, reject by server because not supported with Bulk
  // execution
  {
    BT_INIT_STMT(bmysql, stmt, bind, 1);
    unsigned int array_size = 5;
    ...
    DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
    DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                      "Long data value is not supported in bulk execution") ==
               0);
  }
...
void stmt_bulk_test_insert_trigger() {
  myheader("stmt_bulk_test_insert_trigger");
  BT_ENABLE_BULK;
  {
    int rc = 0;
    BT_CREATE_DEF_TABLES(mysql);
  ...
  BT_INIT_CLIENT2(bmysql, opt_user, opt_password, "bulk_test");

  rc = mysql_query(bmysql,
                   "create table tbl2 (a int, b varchar(20) primary key)");
  myquery2(bmysql, rc);

  const char *before_trigger =
      "CREATE TRIGGER before_insert_tbl "
      "BEFORE INSERT ON tbl "
      "FOR EACH ROW "
      "BEGIN "
      "  INSERT INTO tbl2 VALUES (NEW.a, NEW.b);"
      "END";

  rc = mysql_query(mysql, before_trigger);
  myquery2(mysql, rc);
  ...
  // Bulk insert
  {
    ...
    // Check result in tbl2
    rc = mysql_query(bmysql, "SELECT a, b FROM tbl2 ORDER BY b;");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
...
void stmt_bulk_test_pfs() {
...

  {
    ...
    rc = mysql_query(
        ...
        "WHERE EVENT_NAME = 'statement/com/Bulk_execute';");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(mysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
...

  {
    ...
    DIE_UNLESS(row);
    DIE_UNLESS(affected_rows == 1);
    int pfs_count = std::stoi(row[0]);
    mysql_free_result(result);
    DIE_UNLESS(pfs_count - prev_pfs_count == 1);
  }
...
void stmt_bulk_test_binlog_format() {
  myheader("stmt_bulk_test_basic_dml");
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);

  bool log_bin_disable_or_row_format = false;
  {
    MYSQL_RES *result;
    MYSQL_ROW row;
    uint64_t affected_rows;
    int ret =
        mysql_query(bmysql, "select not @@log_bin or @@binlog_format = 'ROW';");
    DIE_UNLESS(ret == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    ...
    row = mysql_fetch_row(result);
    DIE_UNLESS(row);
    log_bin_disable_or_row_format = std::stoi(row[0]) > 0;
    mysql_free_result(result);
  }

  fprintf(stdout,
          "--> stmt_bulk_test_binlog_format log_bin_disable_or_row_format=%d\n",
          log_bin_disable_or_row_format);
  ...
  // Insert
  {
    ...
    rc = mysql_stmt_execute(stmt);
    if (log_bin_disable_or_row_format) {
      /* ROW format, accepted by server */
      BT_CHECK_STMT_RC;
      ...
      /* STATEMENT or MIX format, reject by server. */
      DIE_UNLESS(rc != 0);
      DIE_UNLESS(mysql_stmt_errno(stmt) == 1295);
      DIE_UNLESS(strcmp(mysql_stmt_error(stmt),
                        "This bulk execution requires binlog_format to be ROW "
                        "if binlog is enabled.") == 0);
    }
    BT_CLEAR_STMT(stmt);
  }
...
void stmt_bulk_test_general_and_slow_log() {
  myheader("stmt_bulk_test_general_and_slow_log");
#ifdef NDEBUG
  fprintf(
      stdout,
      "\n==> Skip test in release mode: stmt_bulk_test_general_and_slow_log\n");
  return;
#endif
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  ...
  rc = mysql_query(bmysql, "SET DEBUG='+d,bulk_test_slow_query';");
  DIE_UNLESS(rc == 0);
...
void stmt_bulk_test_warning_count() {
  myheader("stmt_bulk_test_warning_count");
  BT_ENABLE_BULK;
  BT_INIT_CLIENT(bmysql);
  BT_CREATE_DEF_TABLES(bmysql);
  DIE_UNLESS(mysql_query(bmysql, "CREATE TABLE tbl2 (a TINYINT)") == 0);
  DIE_UNLESS(mysql_query(bmysql, "SET SESSION sql_mode = '';") == 0);
  ...
  // Insert
  {
    ...
    const char *INSERT_QUERY = "INSERT INTO tbl2 VALUES (?)";
    rc = mysql_stmt_prepare(stmt, INSERT_QUERY, strlen(INSERT_QUERY));
    DIE_UNLESS(rc == 0);
    ...
    warning_count = mysql_warning_count(bmysql);
    DIE_UNLESS(warning_count == 3);

    // Check result
    rc = mysql_query(bmysql, "SELECT a FROM tbl2 ORDER BY a;");
    DIE_UNLESS(rc == 0);
    result = mysql_store_result(bmysql);
    DIE_UNLESS(result);
    affected_rows = mysql_num_rows(result);
    DIE_UNLESS(affected_rows == 5);
    int tbl2_ids[5] = {1, 2, 127, 127, 127};
    for (int i = 0; i < 5; ++i) {
      row = mysql_fetch_row(result);
      DIE_UNLESS(row);
      DIE_UNLESS(std::stoi(row[0]) == tbl2_ids[i]);
...
static struct my_tests_st my_tests[] = {
    ...
    {"stmt_bulk_test_data_types", stmt_bulk_test_data_types},
    {"stmt_bulk_test_mix_bulk_non_bulk", stmt_bulk_test_mix_bulk_non_bulk},
    {"stmt_bulk_test_permission", stmt_bulk_test_permission},
    {"stmt_bulk_test_duplicate", stmt_bulk_test_duplicate},
    {"stmt_bulk_test_last_stmt_id", stmt_bulk_test_last_stmt_id},
    {"stmt_bulk_test_packet_error", stmt_bulk_test_packet_error},
    {"stmt_bulk_test_variable_permission", stmt_bulk_test_variable_permission},
    {"stmt_bulk_test_unsupported_query", stmt_bulk_test_unsupported_query},
    {"stmt_bulk_test_insert_trigger", stmt_bulk_test_insert_trigger},
    {"stmt_bulk_test_pfs", stmt_bulk_test_pfs},
    {"stmt_bulk_test_binlog_format", stmt_bulk_test_binlog_format},
    {"stmt_bulk_test_general_and_slow_log",
     stmt_bulk_test_general_and_slow_log},
    {"stmt_bulk_test_warning_count", stmt_bulk_test_warning_count},
    {nullptr, nullptr}};

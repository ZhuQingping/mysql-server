...
This implementation is scoped to the classic optimizer. Its integration
points are the classic ref/range planning path:

- `find_best_ref()` for ref-candidate tournament rewards;
- `get_key_scans_params()` for range-candidate tournament rewards;
- `best_access_path()` / `get_best_combination()` for persisting the final
  ON decision into `POSITION` and `JOIN_TAB`.

The Hypergraph optimizer is intentionally out of scope for this cut. It has a
separate access-path enumeration and costing flow under `sql/join_optimizer/`,
so the classic optimizer hooks above are not sufficient to make Hypergraph
plans consider ICP benefit. When a query is planned by the Hypergraph
optimizer, `icp_cost_based=on` should not be treated as a guarantee that
cost-based ICP rewards participated in access-path selection; the plan falls

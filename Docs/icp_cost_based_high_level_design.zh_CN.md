...
本次实现只覆盖经典优化器（classic optimizer）。接入点都在经典的
ref/range 规划链路上：

- `find_best_ref()`：用于 ref 候选擂台中的 ICP 奖励；
- `get_key_scans_params()`：用于 range 候选擂台中的 ICP 奖励；
- `best_access_path()` / `get_best_combination()`：用于把最终 ON 决策写入
  `POSITION` 并传递到 `JOIN_TAB`。

Hypergraph optimizer 不在本轮实现范围内。它在 `sql/join_optimizer/` 下有
独立的访问路径枚举与代价流，经典优化器里的这些 hook 不能让
Hypergraph 计划自动把 ICP 收益纳入访问路径选择。因此，当查询由
Hypergraph optimizer 规划时，`icp_cost_based=on` 不应被理解为“基于代价的
ICP 奖励已经参与了选路”；该路径仍回落到 Hypergraph 优化器自身已有的

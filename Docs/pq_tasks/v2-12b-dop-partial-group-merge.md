...
- 新增 `pq_groupby_dop2_partial_unsupported` MTR；
- 覆盖 `COUNT(*)`、表达式 aggregate、字符串 group key 等 DOP2 partial
  unsupported shape；
...
- DOP2/DOP4 GROUP BY COUNT(nullable field) 单测通过；
...
- 完整 `parallel_query` suite 通过，共 69 项。

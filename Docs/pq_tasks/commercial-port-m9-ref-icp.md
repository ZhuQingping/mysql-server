...
- M9-F: MVI unique filter、reverse scan、partition、secondary index MIN、record buffer/prefetch 等边角，任务书见 [commercial-port-m9-f-edge-cases.md](commercial-port-m9-f-edge-cases.md)。

M9 当前已打开能力仍是 leader-local secondary/ref/ICP 子集，不等同于商用
worker-side `PQRefIterator` / `PQblockScanIterator`。worker/MQ 级 ref/ICP
迁移转入 M11-F，任务书见
...
- access-shape read-only confirmation 已完成，不改源码；
- 当前未发现稳定 constant covering `JT_REF + pushed_idx_cond` 正例，E2
  user-visible coding blocked；
- covering `k = const AND extra_index_col predicate` 在当前 8.0 行为下要么
  规划成 `range`，要么是 `ref` 但只显示 `Using where; Using index`；
- 非覆盖 constant ref 可以形成 `type=ref` + `Using index condition`，但这不是
  E2 的 covering 范围，且当前 reference commercial 正例也主要覆盖该形态；
- 只处理 constant covering ref ICP；保持 existing C2 no-ICP constant ref 和
  existing D3d no-ICP dependent ref 行为不变；
- E2 不新增、不重审 range ICP，保留既有 E1c-2a non-covering secondary
  range ICP 安全行为；
- dependent-ref ICP、non-covering ref ICP、worker/MQ 仍禁止。
- Design Review Agent 首轮 `REVISE`，修正 no-ICP C2/D3d 既有行为表述后
  复审 `ACCEPT`。
- 仅靠 SQL / EXPLAIN / optimizer trace 只能证明 traditional plan
  `type=ref` 和 `pushed_index_condition`，不能证明 PQ iterator factory 实际
  看到 `AccessPath::REF + pushed_idx_cond_keyno == ref->key`；若后续要重启
  E2，应先做 debug-only shape probe，不直接打开执行路径。

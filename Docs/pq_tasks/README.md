...
- Current task selected: D7 parallel_max_threads cap alignment 已完成。
  ...
  helper。首轮 review 发现大值窄化风险，已修复并补充
...
- Current task selected: D9 commercial control sysvars 已完成。D9 补齐商用
  ...
  InnoDB count 或 hash join 行为。`mysqld` build、`git diff --check` 和
...
- Current task selected: D5 default fullscan multi-range coverage 已完成
  test/docs-only 实现、targeted 验证和独立 review。D5 不改生产源码，新增
  no-debug `pq_commercial_fullscan_multirange`，证明默认 DOP2/DOP4 threaded
  visible fullscan 能完整 drain 多个 InnoDB PQ ranges。任务书和完成报告见

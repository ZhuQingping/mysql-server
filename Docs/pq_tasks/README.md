...
- Active update: M11-E5h ORDER BY Runtime Sort-State Owner Contract 设计已通过 review；4f 已拒绝 executable 4g，后续聚焦 saved ORDER state、`Filesort`、`Sort_param` runtime owner 缺口，仍保持 `HAS_ORDER_BY` 默认串行边界。
...
- Next recommended action: 完成 M11-E5h design review；若通过，再拆 M11-E5h-1 sort-state owner shape。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

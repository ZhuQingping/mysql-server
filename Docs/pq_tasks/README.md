...
- Next recommended action: 进入 M11-D `ParallelScanIterator` lifecycle contract design-only；先明确商用 iterator lifecycle 与当前 fail-closed skeleton 的迁移边界，再决定 M11-D1 compile-only/coding 子任务。

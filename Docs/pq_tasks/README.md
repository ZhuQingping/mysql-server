...
- Next recommended action: 推进 GROUP BY PQ partial state merge，逐步替换 leader-local temp-table update loop 的 aggregate state 来源；`pq_worker_scan_next()` 继续 disabled。
...
- Remaining risk: 基础 implicit aggregate 已可通过 DOP=1/DOP=2/DOP=4 threaded row stream 由上层 MySQL 聚合算子消费；locking read EXPLAIN fallback、DOP2 read-view concurrency、aggregate fallback read-view cleanup、DOP4 correctness/hardening/no-debug gate 和性能基线材料已覆盖；release/目标环境性能采样、GROUP BY partial aggregation、ORDER BY、secondary index/ICP、partition table 仍未完成。
...
| V2-12A-3 - DOP1 Partial Group Execution | V2-12A-3.8 Unsupported Shape Coverage Completed | Codex Orchestrator + Explorer Agent | [v2-12a-3-dop1-partial-group-execution.md](v2-12a-3-dop1-partial-group-execution.md) | PQ wrapper 已在 supported shape 下接管 temp-table aggregate iterator ownership，并由 PQ wrapper 自己写 output temp table；当前支持单整数 COUNT/SUM/MIN/MAX，并覆盖主要 unsupported shape；worker partial state 仍待替换；完整 suite 57 项通过 |

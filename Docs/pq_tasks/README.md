...
- Next recommended action: 进入 M11-E4 user-visible ORDER BY gate design；先设计解除 `HAS_ORDER_BY` 的最小条件和 diagnostics，不混入 M11-F ref/ICP worker path，不直接打开真实 ORDER BY PQ。

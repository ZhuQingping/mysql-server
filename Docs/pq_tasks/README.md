...
- Latest commits:
  ...
  - M11-E5d-4c optimizer-side Filesort/Sort_param scalar handoff: Design Review Agent `ACCEPT`；waiting for commit
- Phase 8 validation:
...
- Next recommended action: 完成 M11-E5d-4c optimizer-side Filesort/Sort_param scalar handoff design review；若通过，再进入 E5d-4c-1 DBUG-only scalar handoff 编码。真实 `Filesort::make_sortorder()`、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

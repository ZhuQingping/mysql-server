...
- Active update: M11-F5-A1 debug-only reverse diagnostic taskbook Review
  `ACCEPT`，recommendation 为 `RECORD_BLOCKED`。结论：商用 reverse
  range/index/ref 正例存在，但当前分支不打开 positive；reverse range/index
  的稳定观测点在 forbidden 的 `access_path.cc` / serial reverse iterator
  选择路径，当前只保留 M9-F2 zero-execution guard 和 reverse ref 既有
  counters。下一项进入 F5-C partition worker ownership design。F5-A
  boundary contract 已提交为 `e799008575e`；F5a-1 debug-only optimizer
  shortcut diagnostic 已提交为 `377de6edd70`，完整 `parallel_query` suite
  89/89 通过。
...
- Next recommended action: enter F5-C partition worker ownership docs-first
  design；继续禁止直接打开 native

...
- Active update: M11-E5g-4c DBUG-only controlled ordered reader skeleton 已完成本地实现、构建、目标 MTR 和完整 `parallel_query` suite 验证；该 reader 只读 controlled `PQOF` 输入并返回 row image/status，不打开真实 ORDER BY execution、默认 worker MQ consumption 或默认 ordered `Read()`。
...
- Current M11-E5g correction: M11-E5g-2 和 M11-E5g-3 已提交；M11-E5g-4 design-only 已完成；M11-E5g-4a 和 M11-E5g-4b 已提交；M11-E5g-4c 本地实现、构建、目标 MTR 和完整 suite 验证已完成，等待 Code/Doc/Test review。下方超长历史摘要如仍出现 5g-2 waiting final review，不作为当前状态来源。
...
- Next recommended action: 进入 M11-E5g-4b fail-closed ordered materialization API skeleton；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

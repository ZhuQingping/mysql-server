...
- Active update: M11-E5i Worker `PQOF` Producer Runtime Boundary 已通过 Design Re-review；E5i-1 将串行收敛现有 E5g-4d adapter skeleton 的 worker-local controlled owner contract，不新增第三套 producer。
...
- Next recommended action: 串行进入 M11-E5i-1 worker-local controlled `PQOF` producer owner contract 收敛编码。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`Exchange_sort` default heap-reader、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

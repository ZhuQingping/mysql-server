...
- Next recommended action: 评估 debug-only threaded DOP=1 full scan 是否进入受系统变量保护的实验路径；默认启用前补齐更真实的 external KILL 测试和 DOP>1 range correctness，`pq_worker_scan_next()` 继续 disabled。

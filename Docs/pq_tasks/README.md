...
- Next recommended action: 继续 V2-8J-4 DOP=1 real full scan gate；把 limited producer 从 smoke-only 推进到 shadow `Read()` producer，先在 debug gate 下验证多行/EOF，`pq_worker_scan_next()` 继续 disabled。

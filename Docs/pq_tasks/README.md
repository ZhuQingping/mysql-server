...
- Active update: M11-F3 Worker-side ICP Clone / Refix Contract 已完成；
  F3a 提交为 `bb395aca64b`，F3b debug-only ICP ownership mismatch smoke
  提交为 `a8ad45ecf0c`；`mysqld` build、targeted MTR、
  `pq_locking_read_fallback` record/replay 和完整 `parallel_query` suite
  89/89 通过。真实 worker-side ICP positive row production、
  `PQRefIterator::Read()`、`PQblockScanIterator::Read()` 和
  `pq_worker_scan_next()` 继续 blocked；下一步进入 M11-F4 Native
  `Record_buffer` / Prefetch Worker-owned Adapter design。
...
- Next recommended action: enter M11-F4 Native `Record_buffer` / Prefetch
  Worker-owned Adapter design。继续禁止直接打开

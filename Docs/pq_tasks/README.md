...
- Active update: M11-E5g-4d DBUG-only worker `PQOF` producer adapter skeleton 已完成本地实现、构建、目标 MTR 和完整 `parallel_query` suite 验证；该 adapter 只向 controlled `MQueue_handle` 写 `PQOF` frame，不接真实 worker thread、InnoDB、PQWR 或默认 ORDER BY SQL。
...
- Current M11-E5g correction: M11-E5g-2 和 M11-E5g-3 已提交；M11-E5g-4 design-only 已完成；M11-E5g-4a、M11-E5g-4b、M11-E5g-4c 已提交；M11-E5g-4d 本地实现、构建、目标 MTR 和完整 suite 验证已完成，等待 Code/Doc/Test review。下方超长历史摘要如仍出现 5g-2 waiting final review，不作为当前状态来源。

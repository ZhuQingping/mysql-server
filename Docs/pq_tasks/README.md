...
- Active update: M11-E5g-4 default ORDER BY execution path boundary design 已完成并通过 Design Review，当前仅改文档，不打开真实 ORDER BY execution、默认 worker MQ consumption 或默认 ordered `Read()`。
...
- Current M11-E5g correction: M11-E5g-2 和 M11-E5g-3 已提交；M11-E5g-4 design-only 已完成。下方超长历史摘要如仍出现 5g-2 waiting final review，不作为当前状态来源。
...
- Next recommended action: 进入 M11-E5g-4a default ORDER BY execution state contract / preflight readiness detail；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

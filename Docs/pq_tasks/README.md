...
- Next recommended action: 扩展 GROUP BY DOP1 负向 MTR，然后把 leader-local temp-table 聚合内部替换为 PQ partial state merge；`pq_worker_scan_next()` 继续 disabled。

...
- Active update: M11-E5g-2 streaming `Exchange_sort` heap-read state-machine smoke 已完成本地实现和验证，等待 final Code/Doc/Test Review 后提交。真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
...
- Next recommended action: 5g-2 final review 通过后提交，然后进入 M11-E5g-3 ordered leader materialization smoke from streaming reader；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

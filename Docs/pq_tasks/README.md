...
- Active update: M11-E5h-2 runtime saved ORDER attach smoke 已完成本地编码、构建、目标 MTR 和完整 `parallel_query` suite 验证；该 smoke 留在 optimizer-side DBUG 路径，复用 `PQ_owned_order_chain_sidecar`，不让 `Exchange_sort` 依赖 `JOIN` / `ORDER`，等待 Code/Doc/Test Review。
...
- Next recommended action: 完成 M11-E5h-2 Code/Doc/Test Review；若通过，再进入 M11-E5h-3 `Sort_param` lifetime smoke。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

...
- Active update: M11-F6a-2 worker constant-ref ownership / cleanup smoke coding.
  ...
  `18cfa6a7414`；当前 F6a-2 coding 已完成本地验证，等待独立
  Code / Docs / Test review。F6a-2 仍不打开 `pq_worker_scan_next()`、
  `PQRefIterator::Read()`、native `Record_buffer`、partition、MVI、reverse
  或任何 ORDER BY visible gate。
...
- Next recommended action: 完成 M11-F6a-2 worker constant-ref ownership /
  cleanup smoke code review；继续禁止在当前 F5 closure 后直接打开

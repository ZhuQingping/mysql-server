...
- Active update: M11-E5r-3a worker `position(record)` / MQ ref lifetime design-only taskbook 已提交为 `c191ac8d505`；M11-E5r-3b private handler-ref lifetime validation shape 已完成编码、构建、focused MTR 和复审，等待提交。
...
- Next recommended action: 提交 M11-E5r-3b private handler-ref lifetime validation shape。仍不直接做 `cmp_ref()` comparator；禁止触碰 `Query_result_mq`、`PQblockScanIterator::Read()`、默认 ORDER BY path。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

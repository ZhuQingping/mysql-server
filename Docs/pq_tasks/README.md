...
- Active update: M11-E5h-1 ORDER BY runtime sort-state owner shape 已完成本地编码、构建、目标 MTR 和完整 `parallel_query` suite 验证；该 owner shell 仍 fail-closed，不构造 `Filesort`、不初始化 `Sort_param`、不改变 `HAS_ORDER_BY` 串行边界，等待 Code/Doc/Test Review。
...
- Next recommended action: 完成 M11-E5h-1 Code/Doc/Test Review；若通过，再进入 M11-E5h-2 runtime saved ORDER attach smoke。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

...
- Active update: M11-F1a Reverse Ref Reject Probe 已完成本地编码和验证；新增 reverse ref reject counters，只在 ref factory fail-closed 分支增长，真实 worker row production 仍为 0。
...
- Next recommended action: review M11-F0 Ref / ICP Worker Path Continuation Contract；F0 为 design-only / read-only。review accept 后，再进入 M11-F1 worker-side ref/ICP shape probe/guard-only。继续保持 M11-E ORDER BY 真实执行 blocked。

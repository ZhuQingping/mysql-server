...
- Next recommended action: 继续 V2-8C handler worker init smoke，把 worker open context 传给 `worker_handler->pq_worker_scan_init()` 并立即 `pq_worker_scan_end()`，仍不设置 `EXECUTED`、不进入 `Read()` row stream。

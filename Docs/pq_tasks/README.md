...
- Active update: M11-F4b debug-only Record_buffer diagnostic 已通过 Code /
  Docs / Test Review，准备提交；新增 worker native record-buffer null/non-null
  probe 状态变量，验证当前 worker open 边界不 attach native
  `Record_buffer`，不 dispatch range，不产 row；`mysqld` build、targeted MTR、
  `pq_stats` record/replay 和完整 `parallel_query` suite 89/89 通过。
  真实 worker-side ICP positive row production、native `Record_buffer`
  positive path、`PQRefIterator::Read()`、`PQblockScanIterator::Read()` 和
...
- Next recommended action: commit M11-F4b, then enter M11-F5 backlog triage。

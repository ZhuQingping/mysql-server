...
- Active update: M11-E5r-4 docs-only `cmp_ref()` comparator compatibility design 已提交为 `f9635804b40`；M11-E5r-5 docs-only MQ handler-ref wire contract design 正在固化。
...
- Next recommended action: 提交 M11-E5r-5 docs-only MQ handler-ref wire contract design。仍不直接做 `cmp_ref()` comparator；禁止触碰 `Query_result_mq`、`PQblockScanIterator::Read()`、默认 ORDER BY path。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

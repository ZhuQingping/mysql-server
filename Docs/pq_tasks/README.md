...
- Next recommended action: 完成 M11-E5d-5c controlled ORDER BY MQ-to-record-group loader 设计检视；通过后再编码。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

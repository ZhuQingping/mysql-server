...
- Active update: M11-E5g-3 ordered leader materialization smoke from streaming reader 已完成本地实现、验证和 Code/Doc/Test Review。真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
...
- Current M11-E5g correction: M11-E5g-2 已提交；M11-E5g-3 本地实现、验证和独立 Code/Doc/Test Review 已完成。下方超长历史摘要如仍出现 5g-2 waiting final review，不作为当前状态来源。
...
- Next recommended action: 进入 M11-E5g-4 默认执行路径边界设计；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

...
- Active update: M11-E5q default `Exchange_sort` heap reader integration contract 已提交为 `c909a0efd19`；M11-E5r rowid duplicate-key tie-break 经独立资料检视后调整为 design-only/read-only inventory，不直接编码。
...
- Next recommended action: 完成 M11-E5r design-only/read-only inventory 的文档检视和提交后，再拆分 handler `ref` / `cmp_ref()` controlled contract 或 wait policy。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

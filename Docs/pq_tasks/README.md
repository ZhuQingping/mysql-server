...
- Next recommended action: 进入 M11-D2 lifecycle status smoke design-only；只为 fail-closed cleanup 增加可观测性，不启用 `PARALLEL_SCAN`、不启动 worker、不触碰 handler/InnoDB。

...
- Active update: M11-E5r-1 handler ref ownership contract design inventory 已提交为 `af2ae4f9d10`；M11-E5r-2-pre worker `position(record)` / MQ handler ref format contract 已完成只读设计检视和 Docs/Source Review，等待 docs-only commit。
...
- Next recommended action: 提交 M11-E5r-2-pre docs-only taskbook。提交后仍不直接做 `cmp_ref()` comparator；下一步只能在独立 Review 后考虑 private frame metadata / validation helper，禁止触碰 `Query_result_mq`、`PQblockScanIterator::Read()`、默认 ORDER BY path。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

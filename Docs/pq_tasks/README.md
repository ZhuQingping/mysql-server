...
- Active orchestration:
  ...
  - [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md): V2-8J 已拆分真实 `Read()` 激活前的硬阻塞；V2-8J-1 callback row producer smoke 和 V2-8J-2 FINISH/EOF、ERROR skeleton 已完成，下一步补 abort 分支。
...
- Next recommended action: 继续 V2-8J-2 worker producer loop skeleton，补 abort 分支；在该 gate 完成前，`PQTableScanIterator::Read()` 和 `pq_worker_scan_next()` 继续 disabled。
...
| V2-8J - Real Read Activation Plan | In Progress | Codex Orchestrator | [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md) | 已完成 callback row producer smoke 和 worker producer FINISH/EOF、ERROR skeleton；下一步补 abort 分支 |

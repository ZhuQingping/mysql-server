...
- Active update: M11-E6e Exchange_sort Handler-ref Comparator Adapter Smoke
  任务书已创建，等待 design review。M11-E6a 已提交为 `70197ead960`，证明
  private/debug-only
  ...
  readiness flags。M11-E6e 下一步只设计 private/DBUG-only comparator
  adapter：equal sort-key 下使用 still-open leader handler `cmp_ref()` 做
  tie-break，不替换默认 heap comparator，不进入 visible ORDER BY gate。
...
- Next recommended action: 若继续 positive commercial path，必须另开一个
  单一路径的新设计阶段；继续禁止在当前 F5 closure 后直接打开
  worker-side ICP + native `Record_buffer`、MVI unique filter、partition、
  reverse、secondary MIN positive path。

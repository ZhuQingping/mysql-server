...
- Latest commits:
  ...
  - M11-E5d-4c-1 debug-only optimizer-to-Exchange_sort scalar handoff: coding completed；Code/Doc/Test Review Agent `ACCEPT`；`git diff --check`、`mysqld` build、targeted MTR、full suite 89/89 passed；waiting for commit
- Phase 8 validation:
...
- Next recommended action: 完成 M11-E5d-4c-1 Code/Doc/Test Review、完整 `parallel_query` suite 和提交；随后再设计下一步 real `Exchange_sort` init/MQ/Read 边界。真实 `Filesort::make_sortorder()`、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

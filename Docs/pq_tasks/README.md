...
- Active update: M11-E5g-0 worker ORDER BY frame producer and streaming heap read boundary design 已提交为 `2e290b3dee0`；真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
...
- Next recommended action: 进入 M11-E5g-1 DBUG-only worker ORDER BY `PQOF` producer smoke；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

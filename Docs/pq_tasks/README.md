...
- Active update: M11-E5m ordered read richer status API 已提交为 `2bc1b98c81e`；M11-E5n ordered materializer owner shape 已完成本地编码、构建、targeted MTR、完整 `parallel_query` suite 和 Code/Doc/Test Review，等待提交。
...
- Next recommended action: 提交 M11-E5n 后再评估默认 ordered read / visible gate 的下一步拆分。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

...
- Active update: M11-E5d-5f runtime prerequisite diagnostics / per-prerequisite fail-closed matrix 已提交为 `e38ed2e3c75`；真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
...
- Next recommended action: 继续拆分下一个 ORDER BY runtime prerequisite，不直接打开 executable ORDER BY PQ；真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 和 executable ORDER BY PQ 仍禁止。

...
- Next recommended action: 继续 V2-8J-4 DOP=1 real full scan gate；拆多行 producer ROW/FINISH API，再实现 wait/kill policy，`pq_worker_scan_next()` 继续 disabled。
...
| V2-8J - Real Read Activation Plan | In Progress | Codex Orchestrator | [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md) | 已完成 callback row producer smoke、worker producer FINISH/EOF/ERROR/abort skeleton、`Read()` shadow scaffold、PROBE diagnostics 和 materialize status helper；下一步拆多行 producer ROW/FINISH API |

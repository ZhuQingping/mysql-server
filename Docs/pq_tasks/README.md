...
- Active update: M11-F5a-1 debug-only optimizer shortcut diagnostic 已完成
  本地实现和验证，等待 Code / Docs / Test Review。新增 debug-only
  opt_sum MIN/MAX shortcut counters，release build 不新增用户可见 status
  variables；`mysqld` build、targeted MTR、`pq_stats` record/replay 和完整
  `parallel_query` suite 89/89 通过。结论仍是 不平移 secondary MIN
  positive execution，只做 debug-only guard / diagnostic。真实 worker-side
  ICP positive row production、native `Record_buffer` positive path、
...
- Next recommended action: Code / Docs / Test Review M11-F5a-1；若 review
  `ACCEPT`，提交该阶段。继续禁止直接打开 native

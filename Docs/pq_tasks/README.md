...
- Active orchestration:
  ...
  - [v2-8f-parallel-reader-pull-adapter.md](v2-8f-parallel-reader-pull-adapter.md): V2-8F callback row accessor 已完成，`InnoDB_pq_scan_ctx` 可检查 leader active read view / DOP=1 whole-range gate、复用 `Parallel_reader::check_visibility(...)`、包装 `row_sel_store_mysql_rec()`，且 `Parallel_reader::Ctx` 提供 `record()`/`offsets()`；不接 SQL `Read()`，不设置 EXECUTED；build 和完整 `parallel_query` suite 通过。
...
| V2-8F - Parallel_reader Pull Adapter | Callback row accessor completed | Codex Orchestrator | [v2-8f-parallel-reader-pull-adapter.md](v2-8f-parallel-reader-pull-adapter.md) | `Parallel_reader::Ctx` 已新增 `record()` / `offsets()` accessor，现有 upstream adapter 改用 `record()`；先不接 SQL `Read()`，不打开真实 row stream；`mysqld` build 和完整 `parallel_query` suite 通过 |

...
- Active orchestration:
  ...
  - [v2-8f-parallel-reader-pull-adapter.md](v2-8f-parallel-reader-pull-adapter.md): V2-8F conversion helper 已完成，`InnoDB_pq_scan_ctx` 可检查 leader active read view / DOP=1 whole-range gate、复用 `Parallel_reader::check_visibility(...)`，并包装 `row_sel_store_mysql_rec()`；不接 SQL `Read()`，不设置 EXECUTED；build 和完整 `parallel_query` suite 通过。
...
| V2-8F - Parallel_reader Pull Adapter | Conversion helper completed | Codex Orchestrator | [v2-8f-parallel-reader-pull-adapter.md](v2-8f-parallel-reader-pull-adapter.md) | `InnoDB_pq_scan_ctx` 已新增 `store_mysql_record()` 包装 `row_sel_store_mysql_rec()`；先不接 SQL `Read()`，不打开真实 row stream；`mysqld` build 和完整 `parallel_query` suite 通过 |

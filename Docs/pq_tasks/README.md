...
- Active update: M11-E5d-5c controlled ORDER BY MQ-to-record-group loader 已完成本地编码和 Code/Doc/Test Review，targeted MTR 与 full `parallel_query` suite 均通过，等待提交；真实 ORDER BY eligibility、默认 worker MQ consumption 和 ordered `Read()` 仍未打开。
...
- Next recommended action: 提交 M11-E5d-5c 后，进入后续 ordered Read shadow boundary。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

...
- Active update: M11-F3 Worker-side ICP Clone / Refix Contract 已通过
  Design / Docs / Source Review；M11-F3b debug-only ICP ownership mismatch
  smoke 已本地实现，`mysqld` build、targeted MTR、`pq_locking_read_fallback`
  record/replay 和完整 `parallel_query` suite 89/89 通过；Code / Docs /
  Test Review 待完成。真实 worker-side ICP positive row production、
...
- Next recommended action: finish M11-F3b validation and Code / Docs / Test
  Review。若 review `ACCEPT`，提交 F3b，然后进入 M11-F4 Native
  `Record_buffer` / Prefetch Worker-owned Adapter design。继续禁止直接打开

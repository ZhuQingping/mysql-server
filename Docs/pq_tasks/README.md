...
- Active update: M11-E5d-5e-1 ORDER BY eligibility contract helper 已完成编码、验证和 Code/Doc/Test Review，等待提交；真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
- Current phase correction: M11-E5d-5e-1 只识别 future visible ORDER BY candidate 并保持 execution disabled；现有 `HAS_ORDER_BY` serial boundary 仍保留；Review 已 ACCEPT。下方超长历史摘要中的 5c 旧尾句不作为当前状态来源。
...
- Next recommended action: 提交 M11-E5d-5e-1 后，进入 M11-E5d-5e-2 execution preflight blocker。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 和 executable ORDER BY PQ 放开仍禁止。

...
- Active update: M11-F5-C partition worker ownership design taskbook 已草拟，
  等待 Design / Source / Test Review。当前分支 partition table 已由
  `PARTITIONED_TABLE` 全局拒绝，M9-F3 覆盖 full/range/ref/dependent-ref
  zero-execution guard；商用参考支持受限 explicit single partition 正例，
  但依赖 `ha_innopart` worker init、partition_info clone/pruning、worker
  TABLE/handler/prebuilt ownership、ICP/native `Record_buffer` 等当前未
  ready 能力。F5-A1 已记录 `RECORD_BLOCKED`；F5-A 已提交为
...
- Next recommended action: Design / Source / Test Review M11-F5-C；若
  `ACCEPT`，提交 docs-only taskbook，并进入 M11-F5-C1 debug-only
  partition reject diagnostic taskbook。继续禁止直接打开 native

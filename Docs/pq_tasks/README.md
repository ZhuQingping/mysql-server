...
- Next recommended action: 继续 V2-8J-4 DOP=1 real full scan gate；在 debug gate 下验证真实 DOP=1 full scan，并决定是否把 limited producer 从 smoke-only 推进到 shadow `Read()` producer，`pq_worker_scan_next()` 继续 disabled。

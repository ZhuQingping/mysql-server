...
- Next recommended action: 保持实验变量默认 OFF；下一步评估 V2-12A-3 DOP1 Partial Group Execution；`pq_worker_scan_next()` 继续 disabled。
...
- Remaining risk: 基础 implicit aggregate 已可通过 DOP=1/DOP=2/DOP=4 threaded row stream 由上层 MySQL 聚合算子消费；locking read EXPLAIN fallback、DOP2 read-view concurrency、DOP4 correctness/hardening/no-debug gate 和性能基线材料已覆盖；release/目标环境性能采样、GROUP BY partial aggregation、ORDER BY、secondary index/ICP、partition table 仍未完成。
...
| V2-11A/B/C/D/E - Regression Expansion | MDL pending | Codex Orchestrator + Worker Agents | [v2-11-regression-expansion.md](v2-11-regression-expansion.md) | 新增 prepare state、row image datatypes、LIMIT/counter、large external KILL 和 experimental vars noop 回归 |
...
| V2-12A-3 - DOP1 Partial Group Execution | V2-12A-3.3 Completed | Codex Orchestrator + Explorer Agent | [v2-12a-3-dop1-partial-group-execution.md](v2-12a-3-dop1-partial-group-execution.md) | 默认 OFF gate、`AccessPath::AGGREGATE` factory skeleton 和 typed state smoke；GROUP BY 执行仍 fallback；完整 suite 52 项通过 |

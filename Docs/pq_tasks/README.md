...
- Active update: M11-E5l controlled `Exchange_sort::read_mq_message()` typed `PQOF` branch smoke 已完成本地编码、构建、目标 MTR、完整 `parallel_query` suite 和 Code/Doc/Test Review；等待提交后进入 ordered materializer / richer status API 设计。
...
- Next recommended action: 提交 M11-E5l；随后进入 ordered materializer / richer status API 的下一步设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

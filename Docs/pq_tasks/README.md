...
- Active update: M11-F2 Worker TABLE / Handler / Prebuilt Ownership Contract
  已完成；M11-F2a/F2b 已通过 Code/Task Review 并提交；完整
  `parallel_query` suite 89/89 通过。M11-F3 Worker-side ICP Clone /
  Refix Contract 已完成 current-branch 与商用实现 Explorer 调研，双方一致
  建议先做 contract + debug-only fail-closed smoke；真实 worker-side ICP
  positive row production、`PQRefIterator::Read()`、`PQblockScanIterator::Read()`
  和 `pq_worker_scan_next()` 继续 blocked。
...
- Next recommended action: review M11-F3 Worker-side ICP Clone / Refix
  Contract。F3a 是 docs-only Design / Docs / Source Review；Review `ACCEPT`
  后再进入 F3b debug-only ICP ownership mismatch smoke。继续禁止直接打开

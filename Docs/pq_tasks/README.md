...
- Active update: M11-F5-C1 debug-only partition reject diagnostic 已通过
  Code / Docs / Test Review 并提交为 `df74acaf881`。新增
  `Parallel_partition_reject_probes`，只在 `pq_check_single_table()` 的
  single-table `PARTITIONED_TABLE` reject 点增长；dependent-ref partition
  join 继续通过 `MULTI_TABLE` fail-closed，不计入 partition counter。
  `mysqld` build、`pq_commercial_ref_icp` targeted record/replay、`pq_stats`
  replay 和完整 `parallel_query` suite 89/89 通过。M11-F5-B MVI unique
  filter inventory / design 已通过 Design / Source / Test Review，保持
  MVI fail-closed，不打开 `HA_EXTRA_ENABLE_UNIQUE_RECORD_FILTER`；下一步是
  M11-F5-B1 debug-only MVI reject diagnostic。F5-C design 已提交为
...
- Next recommended action: 提交 M11-F5-B docs-only 任务书，然后创建
  M11-F5-B1 debug-only MVI reject diagnostic coding taskbook。继续禁止
  直接打开 native `Record_buffer` positive path、
  `PQRefIterator::Read()`、`PQblockScanIterator::Read()`、真实
  `pq_worker_scan_next()`、worker-side ICP positive row production、MVI
  unique filter positive path 和 M11-E visible ORDER BY positive path。

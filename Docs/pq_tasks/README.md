...
- Next recommended action: 进入 M11-D4c commit-point error cleanup smoke；先完成 design review，再编码 DBUG-only post-commit ERROR cleanup 验证，不启用默认 `PARALLEL_SCAN`、不启动真实 worker、不触碰 handler/InnoDB。

...
- Active update: M11-E5d-5e user-visible ORDER BY eligibility gate 设计已通过独立检视，等待提交；真实 ORDER BY execution、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
- Current phase correction: M11-E5d-5e 设计目标是识别 future visible ORDER BY candidate 但 execution 仍 disabled；Design Review 已 ACCEPT。下方超长历史摘要中的 5c 旧尾句不作为当前状态来源。
...
- Next recommended action: 提交 M11-E5d-5e 设计后，进入 M11-E5d-5e-1 eligibility contract helper。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 和 executable ORDER BY PQ 放开仍禁止。

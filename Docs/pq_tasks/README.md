...
- Next recommended action: 执行 V2-12C-2 Multi Aggregate First Implementation，先支持 DOP2/DOP4 的 `COUNT(*) + SUM(field)`、`COUNT(nullable field) + SUM(same field)` 和 `MIN(field) + MAX(field)`；`pq_worker_scan_next()` 继续 disabled。

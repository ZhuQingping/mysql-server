...
- Next recommended action: 编码并验证 M11-E5d-5b sort-key buffer / record-group allocation smoke；真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

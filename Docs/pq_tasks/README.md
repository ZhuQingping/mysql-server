...
- Next recommended action: 基于 D4a-D5 已通过的 worker attach、leader row stream、post-commit ERROR cleanup 和 row-value correctness 护栏，进入 M11-D6 design-only；目标是把最小正路径从 `PQTableScanIterator` debug hook 迁向 commercial `ParallelScanIterator`，但仍保持 debug-only、DOP=1、无 worker thread、无 clone/JOIN。

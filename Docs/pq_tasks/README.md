...
- Active orchestration:
  ...
  - [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md): V2-8J 已拆分真实 `Read()` 激活前的硬阻塞；callback row conversion gate 已完成，下一步做 typed MQ producer smoke。
...
- Next recommended action: 继续 V2-8J-1 typed MQ producer smoke，把 callback-converted worker record image 发送到 MQ，并由 leader `materialize_next_record_image()` 消费；在该 gate 完成前，`PQTableScanIterator::Read()` 和 `pq_worker_scan_next()` 继续 disabled。
...
| V2-8J - Real Read Activation Plan | In Progress | Codex Orchestrator | [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md) | 已完成 callback row conversion gate；下一步做 typed MQ producer smoke |

...
- Next recommended action: 保持实验变量默认 OFF；下一步实施 V2-12A-1 GROUP BY Gate And Diagnostics；`pq_worker_scan_next()` 继续 disabled。
...
| V2-11A/B/C - Regression Expansion | Completed | Codex Orchestrator + Worker Agents | [v2-11-regression-expansion.md](v2-11-regression-expansion.md) | 新增 prepare state、row image datatypes 和 LIMIT/counter 回归；完整 suite 47 项通过 |
| V2-12A - GROUP BY Partial Aggregation Design | Completed | Codex Orchestrator + Explorer Agent | [v2-12a-groupby-partial-aggregation-design.md](v2-12a-groupby-partial-aggregation-design.md) | 结论：worker partial aggregate state + leader merge；下一步 V2-12A-1 gate/diagnostics |

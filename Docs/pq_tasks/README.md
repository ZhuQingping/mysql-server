...
- Next recommended action: 基于 D4a-D4c 已通过的 worker attach、leader row stream、post-commit ERROR cleanup 护栏，先做 M11-D5 design-only 任务书；重点判断是否先收敛 row-value correctness，还是进入 commercial `ParallelScanIterator` 最小正路径。

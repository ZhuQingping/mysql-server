...
- Active update: M11-E5j Exchange_sort default heap-reader state owner shape 已完成并通过 Code/Doc/Test Review；owned heap-reader state 仍只由 controlled DBUG smoke 驱动。
...
- Next recommended action: 提交 M11-E5j 后，进入 `Exchange_sort::read_mq_message()` typed `PQOF` branch 的下一步设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`Exchange_sort::read_mq_message()` real branch、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

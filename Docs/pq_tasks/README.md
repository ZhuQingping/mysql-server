...
- Active orchestration:
  ...
  - [v2-8k-worker-thread-producer.md](v2-8k-worker-thread-producer.md): V2-8K-1 已完成 worker thread lifecycle scaffold；下一步把 worker entry 接入 debug-only callback producer，解决 full scan row-count 完整性和 producer/consumer 背压问题。
...
- Next recommended action: 启动 V2-8K-2 threaded callback producer debug path；worker thread 写 Exchange，leader `Read()` 并发消费，`pq_worker_scan_next()` 继续 disabled。
...
| V2-8K - Worker Thread Producer | V2-8K-1 Completed | Codex Orchestrator + Explorer | [v2-8k-worker-thread-producer.md](v2-8k-worker-thread-producer.md) | 已完成 joinable worker thread scaffold、worker thread 内部 THD create/destroy、wait/cleanup join 和原子 worker status CAS；下一步接入 debug-only threaded callback producer |

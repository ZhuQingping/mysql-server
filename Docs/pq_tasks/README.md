...
- Next recommended action: 实现 `TEMPTABLE_AGGREGATE` PQ iterator，复用 temp table output contract；`pq_worker_scan_next()` 继续 disabled。
...
| V2-12A-3 - DOP1 Partial Group Execution | V2-12A-3.4g Eligibility Whitelist Completed | Codex Orchestrator + Explorer Agent | [v2-12a-3-dop1-partial-group-execution.md](v2-12a-3-dop1-partial-group-execution.md) | 默认 OFF gate、factory skeleton、typed state smoke、factory counters、ownership/temp-table carriers、`GROUP BY int_col COUNT(*)` shape counters；当前打开 gate+DOP1 的显式 GROUP BY 候选可到达 factory 诊断，GROUP BY 执行仍由原生 iterator 输出；完整 suite 55 项通过 |

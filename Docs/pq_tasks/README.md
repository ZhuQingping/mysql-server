...
- Active orchestration:
  ...
  - [v2-9c-debug-dop2-threaded-shadow.md](v2-9c-debug-dop2-threaded-shadow.md): V2-9C 已完成 debug-only DOP=2 threaded shadow path；完整 suite 30 项通过。
...
- Next recommended action: 保持实验变量默认 OFF；继续补齐 DOP=2 debug shadow 的 worker ERROR、external KILL 和多 range correctness 风险用例；`pq_worker_scan_next()` 继续 disabled。
...
| V2-9C - Debug DOP2 Threaded Shadow | Completed | Codex Orchestrator | [v2-9c-debug-dop2-threaded-shadow.md](v2-9c-debug-dop2-threaded-shadow.md) | 新增 debug-only DOP=2 threaded shadow path；多 worker 启动、empty worker FINISH、Exchange 消费和结果正确性已通过小表 smoke |

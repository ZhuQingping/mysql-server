...
- Active update: M11-E5r-2-pre worker `position(record)` / MQ handler ref format contract 已提交为 `3c6dbfd8cd8`；M11-E5r-2a private row-id metadata validation helper 已提交为 `f8f27df3bc0`；M11-E5r-2b controlled validation smoke 已提交为 `976885fb58c`；M11-E5r-3 worker `position(record)` / MQ ref lifetime design review 已启动。
...
- Next recommended action: 完成 M11-E5r-3 worker `position(record)` / MQ ref lifetime design-only taskbook 和独立 review。仍不直接做 `cmp_ref()` comparator；禁止触碰 `Query_result_mq`、`PQblockScanIterator::Read()`、默认 ORDER BY path。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

...
- Active update: M11-E5g-4e DBUG-only ordered diagnostics consolidation 已完成本地实现、构建、目标 MTR 和完整 `parallel_query` suite 验证；该诊断只记录 ordered shadow path 的 kill 未接入边界，不接真实 THD kill、worker thread、InnoDB、PQWR 或默认 ORDER BY SQL。
...
- Current M11-E5g correction: M11-E5g-2 和 M11-E5g-3 已提交；M11-E5g-4 design-only 已完成；M11-E5g-4a、M11-E5g-4b、M11-E5g-4c、M11-E5g-4d 已提交；M11-E5g-4e 本地实现、构建、目标 MTR 和完整 suite 验证已完成，等待 Code/Doc/Test review。下方超长历史摘要如仍出现 5g-2 waiting final review，不作为当前状态来源。
...
- Next recommended action: 进入 M11-E5g-4f minimal ASC-only visible ORDER BY gate design；不直接打开 executable ORDER BY PQ，真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，除非 4f review 明确接受 4g coding 条件。

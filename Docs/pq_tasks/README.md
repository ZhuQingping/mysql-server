...
- Active update: M11-E5d-5c controlled ORDER BY MQ-to-record-group loader 已提交；M11-E5d-5d debug-only ordered leader Read shadow path 设计已起草，等待独立检视；真实 ORDER BY eligibility、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
- Current phase correction: M11-E5d-5c 已提交；M11-E5d-5d 设计已通过独立检视，下一步进入 5d 编码。下方超长历史摘要中的 5c 旧尾句不作为当前状态来源。
...
- Next recommended action: 完成 M11-E5d-5d debug-only ordered leader Read shadow path 设计检视；通过后再编码。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 和 ORDER BY eligibility 放开仍禁止。

...
- Next recommended action: 继续 V2-8C DOP=1 safe-window smoke gate，评估是否在 `PQTableScanIterator::Init()` 中仅对 `actual_dop == 1` 调用 `run_worker_open_table_smoke()`；仍不设置 `EXECUTED`、不进入 `Read()` row stream。

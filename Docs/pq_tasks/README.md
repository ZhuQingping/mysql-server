...
- Active update: M11-F5-A reverse boundary contract 已进入 docs-only
  taskbook。当前分支只读 Explorer 确认 visible ORDER BY 仍由
  `HAS_ORDER_BY` 统一串行拒绝，reverse ref 已有 reject counters，
  reverse range/index 只保留 zero-execution guard；F5-A 不打开 reverse
  positive path，等待商用参考 Explorer 和 Design / Source / Test Review。
  M11-F5a-1 debug-only optimizer shortcut diagnostic 已提交为
  `377de6edd70`，Code / Docs / Test Review `ACCEPT`；`mysqld` build、
  targeted MTR、`pq_stats` record/replay 和完整 `parallel_query` suite
  89/89 通过。
...
- Next recommended action: complete F5-A commercial-reference read-only
  confirmation and Design / Source / Test Review；若 `ACCEPT`，提交
  docs-only taskbook。继续禁止直接打开 native
  ...
  M11-E visible ORDER BY positive path。

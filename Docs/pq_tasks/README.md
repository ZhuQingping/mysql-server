...
- Active update: M11-E5o Filesort / Sort_param runtime owner readiness 已提交为 `7a5d9b2ea27`；M11-E5p worker ORDER BY `PQOF` default producer owner contract 已完成本地编码、构建、targeted MTR、完整 `parallel_query` suite 和 Code/Doc/Test Review，等待提交。
...
- Next recommended action: 完成 M11-E5p validation 和 Code/Doc/Test Review；若通过，提交 E5p 后继续拆分 default heap reader / rowid tie-break / wait policy / visible gate prerequisites。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

...
- Next recommended action: 继续 `Parallel_reader` pull adapter 设计与小步实现；在 adapter 完成前，`pq_worker_scan_next()` 继续 disabled。

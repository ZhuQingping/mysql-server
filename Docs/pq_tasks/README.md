...
- Active update: M11-F2 Worker TABLE / Handler / Prebuilt Ownership Contract
  已根据两个只读 Explorer 的 REVISE 意见修订任务书，并通过 Review Agent
  `ACCEPT`；当前聚焦 worker
  TABLE、handler、`row_prebuilt_t`、read view、dependent ref per-probe key
  和 abort cleanup 契约，不改源码，真实 worker ref/ICP row production 仍为
  0。下一步进入 F2a debug-only ownership hardening/smoke，而不是直接平移
  商用 `PQRefIterator::Read()`。

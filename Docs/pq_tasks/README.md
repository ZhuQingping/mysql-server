...
- Active update: M11-E5h-4 preflight evidence update 已做 docs-only closure；结论是不修改 `pq_build_orderby_execution_preflight()`，不新增 owner evidence 字段，不把 E5h-1/2/3 DBUG evidence 解释为 runtime readiness。
...
- Next recommended action: 完成 M11-E5h-4 docs review；若通过，再进入下一个 ORDER BY runtime 缺口设计，优先评估 default worker `PQOF` producer 或 `Exchange_sort` default heap-reader 接入边界。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

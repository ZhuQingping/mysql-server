...
- Next recommended action: 保持实验变量默认 OFF；下一步新增 GROUP BY eligibility 白名单但仍原生 fallback，然后实现 `TEMPTABLE_AGGREGATE` PQ iterator；`pq_worker_scan_next()` 继续 disabled。

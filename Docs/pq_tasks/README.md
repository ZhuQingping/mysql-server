...
- Active update: M11-E5p worker ORDER BY `PQOF` default producer owner contract 已提交为 `2c84c61a19c`；M11-E5q default `Exchange_sort` heap reader integration contract 已完成本地编码、构建、targeted MTR、完整 `parallel_query` suite 和 Code/Doc/Test Review，等待提交。
...
- Next recommended action: 提交 M11-E5q 后继续拆分 rowid tie-break / wait policy / visible gate prerequisites。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

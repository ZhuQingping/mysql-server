...
- Active update: M11-E5r-2-pre worker `position(record)` / MQ handler ref format contract 已提交为 `3c6dbfd8cd8`；M11-E5r-2a private row-id metadata validation helper 已提交为 `f8f27df3bc0`；下一步等待 M11-E5r 后续拆分 review。
...
- Next recommended action: 基于独立 Planning/Source Review 细化 M11-E5r 下一步。仍不直接做 `cmp_ref()` comparator；禁止触碰 `Query_result_mq`、`PQblockScanIterator::Read()`、默认 ORDER BY path。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

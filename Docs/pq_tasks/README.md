...
- Active update: M11-E5g-4f minimal ASC-only visible ORDER BY gate design 已通过 Design Explorer 和 Code/Doc/Test Review；结论是保持 `HAS_ORDER_BY` 默认串行边界，disabled future-candidate contract 可保留为诊断，但 executable ORDER BY PQ gate 当前不允许打开。
...
- Next recommended action: 不进入 executable M11-E5g-4g；后续只允许补 disabled diagnostics 或继续拆真实 runtime readiness 缺口。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止。

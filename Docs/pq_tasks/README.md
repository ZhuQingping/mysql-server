...
- Next recommended action: 推进 DOP>1 GROUP BY worker partial state merge，将当前 DOP1 typed-state temp-table 输出扩展为 worker partial aggregate + leader merge；`pq_worker_scan_next()` 继续 disabled。
...
| V2-12B - DOP Partial Group Merge | V2-12B-2b Payload Error Smoke Completed | Codex Orchestrator + Explorer Agents | [v2-12b-dop-partial-group-merge.md](v2-12b-dop-partial-group-merge.md) | 已完成 DOP partial counters、payload v1 schema smoke、malformed payload smoke；不打开 SQL GROUP BY DOP>1；完整 suite 59 项通过 |

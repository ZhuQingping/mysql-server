...
- Next recommended action: 进入 M11-E5d-5 real `Exchange_sort` init / MQ / Read boundary design；真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止，直到对应设计 review 通过。

...
- Next recommended action: 继续 V2-8C first-row positioning/read-view commit point 设计收敛；在证明 worker read-view 绑定和 `index_first()` 等价定位安全前，`pq_worker_scan_next()` 继续 disabled。

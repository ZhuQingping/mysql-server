...
- Next recommended action: 完成 M11-E5d-5 设计检视并提交；随后按 review 结论进入 M11-E5d-5a `Exchange_sort` real-init state owner shape。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止，直到对应设计 review 通过。

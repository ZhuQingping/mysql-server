...
- Next recommended action: 继续 V2-8J-4 DOP=1 real full scan gate；用 PROBE 细分诊断定位 shadow gate unsupported 原因，并决定是否把 limited producer 从 smoke-only 推进到 shadow `Read()` producer，`pq_worker_scan_next()` 继续 disabled。

...
- Latest commits:
  ...
  - M11-E5d-4b Exchange_sort real-state adapter boundary: Design Review Agent first `REVISE`, re-review `ACCEPT`；waiting for commit
- Phase 8 validation:
...
- Next recommended action: 完成 M11-E5d-4b design review 修订并提交；随后进入 M11-E5d-4b-1 debug-only Exchange_sort scalar sort-state adapter shape。真实 `Filesort::make_sortorder()`、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

...
- Active update: M11-F4a Native `Record_buffer` / Prefetch Worker-owned
  Adapter contract 已通过 Design / Docs / Source Review；当前分支与商用
  实现调研均建议先做 docs-only contract，再做 debug-only diagnostic，
  不直接打开 native `Record_buffer` positive path。真实 worker-side ICP
  positive row production、`PQRefIterator::Read()`、
...
- Next recommended action: enter M11-F4b debug-only Record_buffer diagnostic；

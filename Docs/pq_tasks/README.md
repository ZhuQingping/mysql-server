...
- Latest commits:
  ...
  - M11-E5d-4b-1 debug-only Exchange_sort scalar sort-state adapter shape: coding completed；Code/Doc/Test Review Agent first `REVISE` for docs status only；`git diff --check`、`mysqld` build、targeted MTR、full suite 89/89 passed；waiting for re-review and commit
- Phase 8 validation:
...
- Next recommended action: 完成 M11-E5d-4b-1 Code/Doc/Test Review、完整 `parallel_query` suite 和提交；随后再设计 optimizer-side Filesort/Sort_param scalar handoff。真实 `Filesort::make_sortorder()`、持久化 `Sort_param`、worker MQ consumption、`ParallelScanIterator::Read()` 和 ORDER BY eligibility 放开仍禁止。

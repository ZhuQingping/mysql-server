...
- Active update: M11-E5d-5d debug-only ordered leader Read shadow path 已完成编码、验证和 Code/Doc/Test Review，等待提交；真实 ORDER BY eligibility、默认 worker MQ consumption 和默认 ordered `Read()` 仍未打开。
- Current phase correction: M11-E5d-5c 已提交；M11-E5d-5d 已完成编码和独立检视，等待提交。下方超长历史摘要中的 5c 旧尾句不作为当前状态来源。
...
- Next recommended action: 提交 M11-E5d-5d 后，进入 M11-E5d-5e user-visible ORDER BY eligibility gate design。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 和 ORDER BY eligibility 放开仍禁止。

...
- Next recommended action: enter M11-F3 Worker-side ICP Clone / Refix
  Contract design。F2/F2a/F2b 已完成并提交；下一步仍为 design-first，
  不直接打开 `PQRefIterator::Read()`、`pq_worker_scan_next()` 或
  worker-side ICP positive row production。继续保持 M11-E ORDER BY 真实执行
  blocked。

...
- Active update: M11-E5k Exchange_sort typed `PQOF` read helper smoke 已完成并通过 Code/Doc/Test Review；post-review compare-state reset 修正后目标 MTR 通过。
...
- Next recommended action: 提交 M11-E5k 后，进入 `Exchange_sort::read_mq_message()` typed `PQOF` branch 的下一步设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`Exchange_sort::read_mq_message()` real branch、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

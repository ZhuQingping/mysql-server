...
- Next recommended action: 补 V2-12B DOP4 worker error / external kill 专门回归；后续评估更复杂 GROUP BY shape；`pq_worker_scan_next()` 继续 disabled。
...
| V2-12B - DOP Partial Group Merge | DOP2/DOP4 COUNT/SUM/MIN/MAX Result Path Completed | Codex Orchestrator + Explorer Agents | [v2-12b-dop-partial-group-merge.md](v2-12b-dop-partial-group-merge.md) | 已完成 DOP partial counters、payload v1 schema smoke、malformed payload smoke、leader in-memory merge helper、worker local partial producer smoke、DOP2 COUNT/SUM/MIN/MAX temp-table result path、unsupported shape fallback、worker error 和 external kill 覆盖；DOP4 COUNT/SUM/MIN/MAX 正向路径已打开；显式 experimental gate；完整 suite 65 项通过 |

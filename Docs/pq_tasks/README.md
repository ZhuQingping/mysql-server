...
- Active update: M11-E5s wait/kill/detach/error private diagnostic contract 已提交为 `84a387f4674`；M11-E5r-1 handler ref ownership contract design inventory 已通过独立 Review，等待提交。
...
- Next recommended action: 提交 M11-E5r-1 后，不直接做 `cmp_ref()` comparator；先拆 worker `position(record)` / MQ ref deep-copy / leader handler ownership 的前置设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

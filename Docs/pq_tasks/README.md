...
- Active orchestration:
  ...
  - [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md): V2-8J 已拆分真实 `Read()` 激活前的硬阻塞；V2-8J-1 callback row producer smoke 已完成，下一步做 worker producer loop skeleton。
...
- Next recommended action: 进入 V2-8J-2 worker producer loop skeleton，补 FINISH/ERROR/abort 语义；在该 gate 完成前，`PQTableScanIterator::Read()` 和 `pq_worker_scan_next()` 继续 disabled。
...
| V2-8J - Real Read Activation Plan | In Progress | Codex Orchestrator | [v2-8j-real-read-activation-plan.md](v2-8j-real-read-activation-plan.md) | 已完成 callback row producer smoke；下一步做 worker producer loop skeleton |

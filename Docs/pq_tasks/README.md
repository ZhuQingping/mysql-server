...
- Next recommended action: 继续 V2-8C disabled real-worker smoke，把 `pq_create_worker_thd()` / `pq_open_worker_table()` / `pq_close_worker_table()` / `pq_destroy_worker_thd()` 串入独立 smoke helper，但仍不设置 `EXECUTED`、不进入 `Read()` row stream。

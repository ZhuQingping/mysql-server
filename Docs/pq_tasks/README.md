...
- Active update: M11-E5i-1 worker-local controlled `PQOF` producer owner contract 收敛已完成并通过 Code/Doc/Test Review；旧 producer smoke 复用 E5g-4d adapter owner contract，不新增第三套 producer。
...
- Next recommended action: 提交 M11-E5i-1 后，进入 `Exchange_sort` default heap-reader / default MQ consumption 的下一步设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、`Exchange_sort` default heap-reader、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

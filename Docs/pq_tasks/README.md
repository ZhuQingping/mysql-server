...
- Active update: M11-E5h-3 runtime `Sort_param` scalar lifetime smoke 已完成本地编码、构建、目标 MTR 和完整 `parallel_query` suite 验证；该 smoke 只持久化 scalar metadata，不构造或持久化真实 `Sort_param`，等待 Code/Doc/Test Review。
...
- Next recommended action: 完成 M11-E5h-3 Code/Doc/Test Review；若通过，再进入 M11-E5h-4 preflight evidence update 设计。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

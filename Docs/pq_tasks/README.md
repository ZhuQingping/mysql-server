...
- Active update: M11-E5r rowid duplicate-key tie-break design inventory 已提交为 `7f6eeda9038`；M11-E5s wait/kill/detach/error private diagnostic contract 已完成本地编码、`mysqld` build、targeted MTR、完整 `parallel_query` suite 和 Code/Doc/Test Review，等待提交。
...
- Next recommended action: 提交 M11-E5s 后继续拆分 handler `ref` / `cmp_ref()` controlled contract、真实 wait owner、或最终 visible gate prerequisites。真实 `Filesort::make_sortorder()` visibility、持久化 `Sort_param`、默认 worker MQ consumption、默认 `ParallelScanIterator::Read()` ORDER BY path 仍禁止，直到对应 runtime owner 被单独 review。

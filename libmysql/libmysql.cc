...
#define DEFAULT_ARRAY_SIZE (ulong)0
...
MYSQL_STMT *STDCALL mysql_stmt_init(MYSQL *mysql) {
  ...
  stmt->array_size = DEFAULT_ARRAY_SIZE;
  my_stpcpy(stmt->sqlstate, not_error_sqlstate);
...
static bool execute(MYSQL_STMT *stmt, char *packet, ulong length,
  ...
  unsigned long stmt_id = stmt->stmt_id;
  DBUG_EXECUTE_IF("bulk_test_last_stmt_id", { stmt_id = 0xFFFFFFFF; });
  int4store(buff, stmt_id); /* Send stmt id to server */
  uchar flags = (uchar)stmt->flags;

  if (stmt->array_size == 0 /* not a BULK command */) {
    ...
    */
    ...
    /* Do not set STMT_BULK_FLAG_SEND_UNIT_RESULTS here because it's not
    supported in mysql server. */
    uint16 bulk_flags = STMT_BULK_FLAG_CLIENT_SEND_TYPES;
    DBUG_EXECUTE_IF("bulk_test_invalid_unit_test_flag",
                    { bulk_flags |= STMT_BULK_FLAG_SEND_UNIT_RESULTS; });
    int2store(buff + 4, bulk_flags);
    buff_size = 6;
  }

  res = (cli_advanced_command(mysql, command, buff, buff_size, (uchar *)packet,
                              length, true, stmt) ||
...
int cli_stmt_execute(MYSQL_STMT *stmt) {
  ...
  */
  if (stmt->param_count || send_named_params) {
    ...
    if ((stmt->array_size > 0 /* BULK command */)
            ? mysql_int_serialize_param_data_bulk(stmt, &param_data,
                                                  &param_length)
...
bool STDCALL mysql_stmt_attr_set(MYSQL_STMT *stmt,
  ...
  switch (attr_type) {
    ...
    case STMT_ATTR_ARRAY_SIZE: {
      if (!(stmt->mysql->extended_server_capabilities &
            CLIENT_STMT_BULK_OPERATIONS)) {
        /*
          If server doesn't support bulk execution, report an error.
        */

...
struct MEM_ROOT {
  ...
  void *Alloc(size_t length) MY_ATTRIBUTE((malloc)) {
    ...
    if (allocCBFunc && (m_allocated_size - old_alloc_size))
      allocCBFunc(
          ...
          ((reinterpret_cast<long>(this) >> PQ_MEMORY_USED_BUCKET) & 0xf));
...
#ifndef NDEBUG
  /**
   * Calculate the HASH digest value of all blocks in sequence and save them to
   * the return value. The current HASH algorithm used is MurmurHash3.
   */
  std::vector<uint32_t> CalcMemDigest() const;
#endif  // NDEBUG
 ...
 public:
  /** memory alloc and free callback for parallel query */

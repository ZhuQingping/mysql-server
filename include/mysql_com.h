...
/*
  Indicator types for PS COM_STMT_BULK_EXECUTE protocol.
*/
enum enum_indicator_type {
  /* Default value. Indicate that this field should be parsed from
  MYSQL_BIND::buffer array directly. And if it's a string field, the length
  should be parsed from MYSQL_BIND::length array. */
  STMT_INDICATOR_NONE = 0,

  /* Indicate that this field should be filled with NULL value. */
  STMT_INDICATOR_NULL = 1

  /* MySQL server doesn't support the following indicator, so put them in
     comment. */

  /* Similar to STMT_INDICATOR_NONE, but this indicates that string field is
  Null-Terminated String so the length should be calculated from strlen(). */
  // STMT_INDICATOR_NTS = -1,

  /* Indicate that this field should be filled by the default value of this
     column. */
  // STMT_INDICATOR_DEFAULT = 2,

  /* Indicate that the update behaviour of this field should be ignored. */
  // STMT_INDICATOR_IGNORE = 3,

  /* Indicate that this row in the array should be ignored. */
  // STMT_INDICATOR_IGNORE_ROW = 4
};

/*
  whether the client sends field types in COM_STMT_BULK_EXECUTION packet. In
  libmysql this flag is always set, and the server requires it"
*/
#define STMT_BULK_FLAG_CLIENT_SEND_TYPES 128

/*
  Corresponds to CLIENT_BULK_UNIT_RESULTS flag in server capabilities. Since
  MySQL server doesn't support it, this macro is defined only for verifying
  but never set.
*/
...
#define REFRESH_OMA_LOG 0x800000L         /**< Flush the oma log */
...
/**
  Provide COM_STMT_BULK_EXECUTE functionality compatible with MariaDB. If
  variable rds_support_bulk_execution is enabled, mysql server will set this
  flag in extended capability field to notify client that server supports
  PreparedStatement bulk execution.

  CLIENT_BULK_UNIT_RESULTS is the flag for permitting PreparedStatement bulk
  execution to return all individual results defined in Mariadb server. Since
  this feature is not necessary for COM_STMT_BULK_EXECUTE functionality, don't
  define macro CLIENT_BULK_UNIT_RESULTS.
*/
#define CLIENT_STMT_BULK_OPERATIONS (1ULL << 34)
...
void my_net_shrink_buffer(NET *net, unsigned long min_buf_size,

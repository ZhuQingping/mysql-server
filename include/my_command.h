...
enum enum_server_command {
  ...
  */
  ...
  /* Since now COM_END is changed to 255 and enum values are no longer
  consecutive, com_statement_info registration in init_server_psi_keys() will
  stop at COM_WAL_DUMP. All meaningful server commands larger than COM_GAP_BEG
  should be registered independently in the above function, including COM_END.
  */
  COM_GAP_BEG,
  COM_GAP_END = 249,
  COM_STMT_BULK_EXECUTE = 250,  ///< Must be same value as in MariaDB
  /* Must be last */
  COM_END = 255 /**< Not a real command. Refused. */
};

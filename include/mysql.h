...
typedef struct MYSQL {
  ...
  /* Extended server capabilites, will only be set when
  rds_support_bulk_execution is enabled in server */
  uint64_t extended_server_capabilities;
...
typedef struct MYSQL_WAL_RPL {
  unsigned int server_id;      /** Server ID to use when identifying  */
                               /*  with the master                    */
  unsigned int flags;          /** Flags, unused nodw                 */
  uint64_t received_lsn;       /** lsn of the wal log that has been received */
  uint64_t flushed_lsn;        /** lsn of the wal log that has been flushed */
  uint64_t replayed_lsn;       /** lsn of the wal log that has been replayed */
  uint64_t stream_id;          /** wal log stream id */
  unsigned long size;          /** Size of the packet returned by     */
                               /*  read_wal_log()               */
  const unsigned char *buffer; /** Pointer to returned data           */
} MYSQL_WAL_RPL;
...
void STDCALL mysql_wallog_close(MYSQL *mysql, MYSQL_WAL_RPL *rpl);
...
typedef struct MYSQL_STMT {
  ...
  */
  ...
  /* Array size of PreparedStatement execution. Set by mysql_stmt_attr_set
  with STMT_ATTR_ARRAY_SIZE flag. */
...
enum enum_stmt_attr_type {
  ...
  */
  ...
  /*
    Corresponds to MYSQL_STMT::array_size, which is number of parameter sets
    in one PreparedStatement execution. mysql_stmt_execute will use
    COM_STMT_BULK_EXECUTE protocol instead of COM_STMT_EXECUTE if: a) Server
    supports COM_STMT_BULK_EXECUTE protocol, which means
    CLIENT_STMT_BULK_OPERATIONS is set in MYSQL::extended_server_capabilities.
    b) MYSQL_STMT::array_size is larger than 0.
    Must be same enum value as in MariaDB, for compatibility.
  */

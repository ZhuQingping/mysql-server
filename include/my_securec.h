 ...
 */
inline int my_sprintf(char *strDest, size_t destMax, const char *format, ...)
 ...
 * violation
 * @param strDest -  produce output according to a format ,write to the
 * character string strDest
 * @param destMax - The maximum length of destination buffer(including the
 * terminating null  byte ('\0'))
 * @param count - do not write more than count bytes to strDest(not including
 * the terminating null  byte ('\0'))
 * @param format - fromat string
 * @param argptr - instead of  a variable  number of arguments
 * @return:return the number of characters printed(not including the terminating
 * null byte ('\0')), If an error occurred return -1.
 * for example my_snprintf(char[10], 10, 3, "%s", "123");
 */

inline int my_snprintf(char *strDest, size_t destMax, size_t count,
                       const char *format, ...) {
  int rc = 0;
  va_list args;
  va_start(args, format);
 ...
 *   sql_print_error - The function to print detailed error message to error
 * log. Can be NULL: then not print any error message
 *
 ...
 */
inline char *my_strcpy(char *strDest, size_t destMax, const char *strSrc) {
  char *retStr = strDest;

  int rc = strcpy_s(strDest, destMax, strSrc);
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_strcpy() function argument! "
        "errno=%d, strDest:%p, destMax:%zu, strSrc:%p\n",
        rc, strDest, destMax, strSrc);
    retStr =
        NULL; /* The caller should check the return value and the strDest */
  }
 ...
 *       error (may also need to check if strDest is NULL)
 */
inline char *my_strncpy(char *strDest, size_t destMax, const char *strSrc,
                        size_t count) {
  char *retStr = strDest;
  ...
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_strncpy() function argument! "
 ...
 *       error (may also need to check if strDest is NULL)
 */
inline char *my_strcat(char *strDest, size_t destMax, const char *strSrc) {
  char *retStr = strDest;

  int rc = strcat_s(strDest, destMax, strSrc);
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_strcat() function argument! "
        "errno=%d, strDest:%p, destMax:%zu, strSrc:%p\n",
        rc, strDest, destMax, strSrc);
    retStr =
        NULL; /* The caller should check the return value and the strDest */
 ...
 *          or an error occurs.
 */
inline char *my_strtok(char *strToken, const char *strDelimit, char **context) {
  char *retStr = NULL;
  ...
  if (retStr == NULL) {
    if (strDelimit == NULL || context == NULL) {
      sql_print_error(
          "Security violation from my_strtok() function argument! "
 ...
 *       error (may also need to check if strDest is NULL)
 */
inline void *my_memset(void *dest, size_t destMax, int c, size_t count) {
  void *retPtr = dest;
  if (0 == count) {
    return retPtr;
  }
  ...
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_memset() function argument! "
        "errno=%d, dest:%p, destMax:%lu, c:%d, count:%lu\n",
        rc, dest, destMax, c, count);
    retPtr = NULL;
  }

  // Follow what the system memset() does -- returns a pointer to the memory
  // area
  return retPtr;
}
 ...
 *       error (may also need to check if strDest is NULL)
 */
inline void *my_memmove(void *dest, size_t destMax, const void *src,
                        size_t count) {
  void *retPtr = dest;
  /*
  ...
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_memmove() function argument! "
 ...
 *       error (may also need to check if strDest is NULL)
 */
inline void *my_memcpy(void *dest, size_t destMax, const void *src,
                       size_t count) {
  void *retPtr = dest;
  /*
  ...
  if (rc != EOK) {
    sql_print_error(
        "Security violation from my_memcpy() function argument! "
        "errno=%d, dest:%p, destMax:%lu, src:%p, count:%lu\n",
        rc, dest, destMax, src, count);
    retPtr = NULL;
  }

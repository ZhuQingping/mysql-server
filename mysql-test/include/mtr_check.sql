...
BEGIN
  ...
  SELECT /*+SET_VAR(use_secondary_engine=OFF)*/ * FROM INFORMATION_SCHEMA.EVENTS WHERE EVENT_NAME<>'event_dstore_histogram';
  ...
  SELECT /*+SET_VAR(use_secondary_engine=OFF)*/ USER, HOST, DB, COMMAND, INFO FROM INFORMATION_SCHEMA.PROCESSLIST
    WHERE COMMAND NOT IN ('Sleep', 'Daemon', 'Killed')
      ...
      AND INFO <> 'call sys.sp_update_dstore_histogram'  -- will remove when event histogram remove
        ORDER BY COMMAND;

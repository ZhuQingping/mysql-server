...
# DStore environment-related parameters
# 1. Path of tenant_isoland_start.json
our $dstore_tenant_config_path;
...
sub print_global_resfile {
  ...
  resfile_global("dstore",           $opt_dstore           ? 1 : 0);
...
sub command_line_setup {
  ...
  if ($opt_mem) {
    mtr_warning("DStore does not support opening file with 'O_DIRECT' option, ignore mem option");
    $opt_mem = 0;
  }
  ...
  if ($opt_dstore) {
    ...
    push(@opt_extra_mysqld_opt, "--disabled_storage_engines=");
    ...
    # temporary turn this variable ON for mtr tests. Need to remove this variable
    # when histogram statistics auto update work is complete, since calculate
    # exact rows for range result in poor performance, it should not be used for
    # production environment.
    push(@opt_extra_mysqld_opt, "--dstore_calculate_exact_rows_for_range=ON");
    # add default suites for dstore, keep same suite with dstore_suites in local_ci script
    $DEFAULT_SUITES = "";
    $DEFAULT_SUITES .= "main,audit_log,binlog,binlog_gtid,binlog_nogtid,dstore_main,".
                       "dstore_binlog,dstore_binlog_gtid,dstore_binlog_nogtid,dstore_rpl,".
                       "dstore_rpl_gtid,dstore_rpl_nogtid,dstore,collations,".
                       "connection_control,engines/funcs,engines/iuds,engines/rr_trx,encryption,".
                       "federated,funcs_1,funcs_2,gcol,innodb,innodb_fts,innodb_gis,innodb_stress,".
                       "innodb_undo,innodb_zip,information_schema,interactive_utilities,json,jp,local_backup,".
                       "network_namespace,opt_trace,perfschema,query_rewrite_plugins,rpl,rpl_gtid,rpl_nogtid,".
                       "service_status_var_registration,service_sys_var_registration,service_udf_registration,".
                       "test_service_sql_api,stress,sysschema,sys_vars,x,threadpool,dbms_package,dstore_inplace_ddl,dstore_rpl_wal,dstore_parts";
    $ENV{'DSTORE_TEST'} = 1;
  } else {
    push(@opt_extra_mysqld_opt, "--default_storage_engine=InnoDB");
    push(@opt_extra_mysqld_opt, "--default_tmp_storage_engine=InnoDB");
    push(@opt_extra_mysqld_opt, "--storage_engine_mode=BOTH_STORAGE_ENGINE");
    push(@opt_extra_mysqld_opt, "--disabled_storage_engines=");
    push(@opt_extra_mysqld_opt, "--rds_dstore_support_binlog_check=off");
    $ENV{'DSTORE_TEST'} = 0;
  }
  ...
  # Do not support run mtr with --pq and --dstore simultaneously
  if ($opt_pq and $opt_dstore) {
    mtr_error("Dstore table do not support parallel query feature, can not run mtr with --pq and --dstore simultaneously");
  }
  ...
  # Block plan cache for ps-protocol so as keep result file stable
  if ($opt_ps_protocol) {
    push(@opt_extra_mysqld_opt, "--rds_plan_cache=0");
  }
  ...
  if ($opt_pq) {
    ...
    push(@opt_extra_mysqld_opt, "--parallel_rows_threshold=0");
    push(@opt_extra_mysqld_opt, "--parallel_tuple_cost=0.0");
    push(@opt_extra_mysqld_opt, "--parallel_setup_cost=0.0");
     if ($opt_ps_protocol) {
      mtr_error(
          "PQ and Prepared-statement protocol are currently incompatible");
    }
  }
...
# Setup the configuration file specific to dstore.
# Currently, there is only tenant_isoland_start.json.
sub setup_dstore_conf {
  my $install_tc_path = "$basedir/lib/dstore/tenant_isoland_start.json";
  my $build_tc_path = "$bindir/storage/dstore/config/tenant_isoland_start.json";
  if (-e $install_tc_path) {
    $dstore_tenant_config_path = $install_tc_path;
  } elsif (-e "$build_tc_path") {
    $dstore_tenant_config_path = $build_tc_path;
  }
}
...
sub mysql_install_db {
  ...
  environment_setup();  # setup environment for Sanitizer
...
sub mysqld_arguments ($$$) {
  ...
  # Add dstore parameters if startup with no-defaults or other defaults-file.
...
sub start_servers($) {
  ...
  foreach my $mysqld (mysqlds()) {
    ...
    my $name = $mysqld->name();
    my $dstore_data_dir = "$opt_vardir/$name/dstore_data/";
    if ($basedir eq $mysqld_basedir) {
      ...
      if (!$opt_start_dirty) {
        ...
        if (!-d $datadir or
          ...
          # Copy dstore_data_dir.
          my $install_dstore_data_dir = "$path/dstore_data/";
          copytree($install_dstore_data_dir, $dstore_data_dir) if -d $install_dstore_data_dir;
        }
...
Options to control what test suites or cases to run
  ...
  dstore                Also run dstore testcases.

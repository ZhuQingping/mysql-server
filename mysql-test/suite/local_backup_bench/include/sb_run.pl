...
my $opt_debug;
my $opt_database = "sbtest";
my $opt_pid_file = "pid_file";

print "# ENV CRUNCH_ENGINE: $ENV{CRUNCH_ENGINE}\n"; 
print "# opt_engine: $opt_engine\n";
print "# opt_bench_name: $opt_bench_name\n";
print "# opt_rows: $opt_rows\n";  
print "# opt_time: $opt_time\n"; 
print "# opt_threads:  $opt_threads\n"; 
...
print "# bench_check  $bench_check\n";
if ($? == -1) {
  $sb = "/home/ci/mregress/sysbench-1.1/sysbench";
  $bench_check = `$sb --version 2>&1`;
...
my $pid = $$;
open(my $fh, '>', $opt_pid_file) or die "Could not open pid file $opt_pid_file: $!";
print $fh $pid;
close($fh);
print "# opt_pid_file :  $opt_pid_file\n";
...
push(@args, "--mysql-user=sbuser");
push(@args, "--mysql-password=");
push(@args, "--mysql-db=$opt_database");
push(@args, "--report-interval=$opt_report_interval");
push(@args, "--mysql-ignore-errors=1062,1213,1205");

...
sub create_disabled_test_list($$) {
  ...
  if ($::opt_dstore) {
    # Add 'disabled-dstore.def' to the list of disabled files if dstore is enabled.
    unshift(@disabled_collection,
          "$::glob_mysql_test_dir/collections/disabled-dstore.def");
  } else {
    # Add 'innodb_tmp_disabled.def' to the list of disabled files if dstore is disabled.
    unshift(@disabled_collection,
          "$::glob_mysql_test_dir/collections/innodb_tmp_disabled.def");
   }
...
sub collect_one_test_case {
  ...
  # When enable pq and choose InnoDB as default storage engine and xxx.result-pq file exist,
  # choose xxx.result-pq as expected result file.
  if ($::opt_pq and !$::opt_dstore and ($::opt_record or -f "$resdir/$tname.result-pq")) {
  ...
  if ($tinfo->{'pq_test'} and !$::opt_pq) {
    skip_test($tinfo, "Test needs 'pq' option.");
    return $tinfo;
  }
...
my @tags = (
  ...
  # Tests with below .inc file needs pq.
  ["include/pq_test.inc", "pq_test", 1]);
